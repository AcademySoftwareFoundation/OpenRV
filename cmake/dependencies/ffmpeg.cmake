...
IF (CMAKE_BUILD_TYPE STREQUAL "Debug")
    LIST(APPEND _ffmpeg_c_flags "-g")
ELSE ()
    LIST(APPEND _ffmpeg_c_flags "-O2")
ENDIF ()

LIST(APPEND _ffmpeg_cxx_flags "-stdlib=libc++")

LIST(APPEND _ffmpeg_c_flags "-I${RV_DEPS_BASE_DIR}/RV_DEPS_OPENSSL/install/include")
...
IF (RV_TARGET_DARWIN)
    FOREACH (
        ...
        LIST(APPEND _ffmpeg_c_flags "-arch" "${_cmake_arch}")
        LIST(APPEND _ffmpeg_cxx_flags "-arch" "${_cmake_arch}")
        LIST(APPEND _ffmpeg_link_flags "-arch" "${_cmake_arch}")
    ENDFOREACH ()

    LIST(APPEND _ffmpeg_c_flags "-mmacosx-version-min=${CMAKE_OSX_DEPLOYMENT_TARGET}")
    LIST(APPEND _ffmpeg_cxx_flags "-mmacosx-version-min=${CMAKE_OSX_DEPLOYMENT_TARGET}")
    LIST(APPEND _ffmpeg_link_flags "-mmacosx-version-min=${CMAKE_OSX_DEPLOYMENT_TARGET}")

    LIST(APPEND _ffmpeg_link_flags "-L${RV_DEPS_OPENSSL_LIB_DIR}")
...
ELSEIF (RV_TARGET_WINDOWS)
    ...
    LIST(APPEND _ffmpeg_link_flags "-LIBPATH:${RV_DEPS_OPENSSL_LIB_DIR}")
    LIST(APPEND _ffmpeg_link_flags "-LIBPATH:${RV_DEPS_DAVID_LIB_DIR}")
ENDIF ()

STRING(REPLACE ";" " " _ffmpeg_c_flags_joined "${_ffmpeg_c_flags}")
STRING(REPLACE ";" " " _ffmpeg_cxx_flags_joined "${_ffmpeg_cxx_flags}")
STRING(REPLACE ";" " " _ffmpeg_link_flags_joined "${_ffmpeg_link_flags}")
...
EXTERNALPROJECT_ADD(
        ...
        --enable-openssl --enable-libdav1d --disable-iconv --disable-outdevs ${_toolchain} "--extra-ldflags=${_ffmpeg_link_flags_joined}"
        "--extra-cflags=${_ffmpeg_c_flags_joined}" "--extra-cxxflags=${_ffmpeg_cxx_flags_joined}" ${_disabled_decoders} ${_disabled_encoders} ${_disabled_filters}
        ${_disabled_parsers} ${_disabled_protocols}

...
IF(RV_USE_SYSTEM_DEPS)
  ...
  IF(NOT TARGET LibRaw::raw)
    ...
    FIND_LIBRARY(LIBRAW_LIB_PATH NAMES raw libraw PATHS ${LIBRAW_LIBRARY_DIRS} ${LIBRAW_LIBDIR})
    TARGET_LINK_LIBRARIES(LibRaw::raw INTERFACE ${LIBRAW_LIB_PATH})
    TARGET_INCLUDE_DIRECTORIES(LibRaw::raw INTERFACE ${LIBRAW_INCLUDE_DIRS})
  ENDIF()
  
    IF(DEFINED PkgConfig_VERSION)
      SET(RV_DEPS_RAW_VERSION "${PkgConfig_VERSION}")
      SET(RV_DEPS_RAW_VERSION "${PkgConfig_VERSION}" CACHE STRING "" FORCE)
    ELSEIF(DEFINED PKGCONFIG_VERSION)
      SET(RV_DEPS_RAW_VERSION "${PKGCONFIG_VERSION}")
      SET(RV_DEPS_RAW_VERSION "${PKGCONFIG_VERSION}" CACHE STRING "" FORCE)
    ELSEIF(DEFINED PkgConfig_VERSION_STRING)
      SET(RV_DEPS_RAW_VERSION "${PkgConfig_VERSION_STRING}")
      SET(RV_DEPS_RAW_VERSION "${PkgConfig_VERSION_STRING}" CACHE STRING "" FORCE)
    ELSEIF(DEFINED PKGCONFIG_VERSION_STRING)
      SET(RV_DEPS_RAW_VERSION "${PKGCONFIG_VERSION_STRING}")
      SET(RV_DEPS_RAW_VERSION "${PKGCONFIG_VERSION_STRING}" CACHE STRING "" FORCE)
    ENDIF()

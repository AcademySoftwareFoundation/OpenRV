...
IF(RV_USE_OCIO_OPENEXR)
  ...
  ###############################################################################
  #
  # Expose OCIO's own OpenEXR Targets 
  #
  ...
  SET(_include_dir "${RV_DEPS_OCIO_DIST_DIR}/include/OpenEXR")
  SET(_install_dir ${RV_DEPS_OCIO_DIST_DIR})
  SET(RV_DEPS_OPENEXR_ROOT_DIR ${RV_DEPS_OCIO_DIST_DIR})

  IF(RV_TARGET_WINDOWS)
    SET(_lib_dir "${RV_DEPS_OCIO_DIST_DIR}/lib")

    SET(_ilmthread_lib "${_lib_dir}/${CMAKE_${_lib_type}_LIBRARY_PREFIX}${_ilmthread_name}${RV_DEBUG_POSTFIX}.lib")
    SET( _iex_lib "${_lib_dir}/${CMAKE_${_lib_type}_LIBRARY_PREFIX}${_iex_name}${RV_DEBUG_POSTFIX}.lib")
    SET(_openexr_lib "${_lib_dir}/${CMAKE_${_lib_type}_LIBRARY_PREFIX}${_openexrcore_name}${RV_DEBUG_POSTFIX}.lib")
  ELSE()
    IF(RV_TARGET_LINUX)
      SET(_lib_dir "${RV_DEPS_OCIO_DIST_DIR}/lib64")
    ELSE()
      SET(_lib_dir "${RV_DEPS_OCIO_DIST_DIR}/lib")
    ENDIF()
  ...
  ADD_CUSTOM_TARGET( ${_target} DEPENDS RV_DEPS_OCIO)
...
ELSE()
  ...
  IF(RV_TARGET_DARWIN)
    SET(_openexrcore_name
        ${CMAKE_SHARED_LIBRARY_PREFIX}OpenEXRCore-3_1${RV_DEBUG_POSTFIX}.30.5.1${CMAKE_SHARED_LIBRARY_SUFFIX}
    )
  ELSEIF(RV_TARGET_LINUX)
    SET(_openexrcore_name
        ${CMAKE_SHARED_LIBRARY_PREFIX}OpenEXRCore-3_1${RV_DEBUG_POSTFIX}${CMAKE_SHARED_LIBRARY_SUFFIX}.30.5.1
    )
  ...
  IF(RV_TARGET_DARWIN)
    SET(_ilmthread_name
        ${CMAKE_SHARED_LIBRARY_PREFIX}IlmThread-3_1${RV_DEBUG_POSTFIX}.30.5.1${CMAKE_SHARED_LIBRARY_SUFFIX}
    )
  ELSEIF(RV_TARGET_LINUX)
    SET(_ilmthread_name
        ${CMAKE_SHARED_LIBRARY_PREFIX}IlmThread-3_1${RV_DEBUG_POSTFIX}${CMAKE_SHARED_LIBRARY_SUFFIX}.30.5.1
    )
  ELSEIF(RV_TARGET_WINDOWS)
    SET(_ilmthread_name
  ...
  IF(RV_TARGET_DARWIN)
    SET(_iex_name
        ${CMAKE_SHARED_LIBRARY_PREFIX}Iex-3_1${RV_DEBUG_POSTFIX}.30.5.1${CMAKE_SHARED_LIBRARY_SUFFIX}
    )
  ELSEIF(RV_TARGET_LINUX)
    SET(_iex_name
        ${CMAKE_SHARED_LIBRARY_PREFIX}Iex-3_1${RV_DEBUG_POSTFIX}${CMAKE_SHARED_LIBRARY_SUFFIX}.30.5.1
    )
  ELSEIF(RV_TARGET_WINDOWS)
    SET(_iex_name

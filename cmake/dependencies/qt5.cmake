...
IF(RV_TARGET_DARWIN
  ...
  EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} -E env -- cp -Rnv "${RV_DEPS_QT5_LOCATION}/lib/" "${RV_STAGE_LIB_DIR}" )
  EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} -E env -- cp -Rnv "${RV_DEPS_QT5_LOCATION}/plugins/" "${RV_STAGE_PLUGINS_QT_DIR}" )
ENDIF()

IF(RV_TARGET_LINUX)
  EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} -E env -- cp -Rnv "${RV_DEPS_QT5_LOCATION}/libexec/" "${RV_STAGE_ROOT_DIR}" )
ENDIF()

IF(NOT RV_TARGET_DARWIN)
  EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} -E env -- cp -Rnv "${RV_DEPS_QT5_RESOURCES_FOLDER}/" "${RV_STAGE_ROOT_DIR}" )
  EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} -E env -- cp -Rnv "${RV_DEPS_QT5_TRANSLATIONS_FOLDER}/" "${RV_STAGE_ROOT_DIR}" )
ENDIF()
  ...
  FUNCTION(COPY_ONLY_LIBS_MATCHING_BUILD_TYPE SRC_DIR DST_DIR)
    ...
    FOREACH(
      ...
      IF(NOT IS_DIRECTORY "${SRC_DIR}/${_qt_lib}")
        EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} -E env -- cp -nv "${SRC_DIR}/${_qt_lib}" "${DST_DIR}/${_qt_lib}" )
      ENDIF()
    ENDFOREACH()
  ENDFUNCTION()

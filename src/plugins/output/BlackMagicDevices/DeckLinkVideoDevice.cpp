...
DeckLinkVideoDevice::DeckLinkVideoDevice(BlackMagicModule* m,
                                         const std::string& name, IDeckLink* d,
                                         IDeckLinkOutput* p)
...
DeckLinkVideoDevice::VideoFormat DeckLinkVideoDevice::format() const
{
    const DeckLinkVideoFormat& f =
        m_decklinkVideoFormats[m_internalVideoFormat];
    return DeckLinkVideoDevice::VideoFormat(f.width, f.height, f.pa, 1.0f, f.hz,
                                            f.desc);
}
...
void
DeckLinkVideoDevice::audioFrameSizeSequence(AudioFrameSizeVector& fsizes) const
{
    // same as AJA
    const DeckLinkVideoFormat& f =
        m_decklinkVideoFormats[m_internalVideoFormat];
...
void DeckLinkVideoDevice::open(const StringVector& args)
{
    ...
    for (int i = 0; i < m_videoFrameBufferSize; i++)
    {
        ...
        if (m_outputAPI->CreateVideoFrame(
                ...
                bmdFrameFlagFlipVertical, &outputFrame)
            != S_OK)
        {
    ...
    if (m_outputAPI->EnableAudioOutput(m_audioSampleRate, m_audioSampleDepth,
                                       m_audioChannelCount,
    ...
    IDeckLinkMutableVideoFrame* readbackVideoFrame) const
{
...

    {
        ...
        hopMsg +=
            std::string(" - width=") + std::to_string(m_frameWidth)
            + std::string(", height=") + std::to_string(m_frameHeight)
            + std::string(", pixelSize=") + std::to_string(pixelSize)
            + std::string(", textureFormat=") + std::to_string(m_textureFormat)
            + std::string(", textureType=") + std::to_string(m_textureType);

...
namespace BlackMagicDevices
{
  ...
  //
  // Optional env var that can be set to specify the Blackmagic Decklink device
  // profile to use. Note that the device profile directly impacts the number of
  // devices that are visible to RV. Therefore the env var, is specified needs
  // to be taken into account before listing the devices which is why it needs
  // to be handled at the video output module level, not at the device level.
  // Please refer to the Blackmagic Decklink SDK documentation about the
  // possible values that the device profile can take (under the Device Profile
  // section). Example: export TWK_BLACKMAGIC_PROFILE =
  //    "bmdProfileFourSubDevicesHalfDuplex".
  // Note that an another simpler way to select the
  // Blackmagic device profile is to use the Blackmagic Desktop Video Setup app.
  // The Blackmagic device profile selected via this app is persisted between
  // reboot.
  //
  ...
  class RAII
  {
   public:
    ...
    RAII( RAII&& other ) noexcept : ptr_( other.ptr_ )
    {
      other.ptr_ = nullptr;
    }
  ...
  // Note: Will throw an exception if the device iterator interface could not be
  // retrieved
  //
  ...
  void setBmdDeviceProfile( const std::string& bmdDeviceProfile )
  {
    ...
    {
      std::cerr << "ERROR: Unrecognized Blackmagic DeckLink device profile: "
    ...
    // Note:
    // In order to get the DeckLink device profile currently active, we need an
    ...
    // Retrieve an IDeckLink interface from the first device (or sub-device) if
    // any
    ...
    if( deckLinkDevice.get()->QueryInterface(
            ...
            (void**)&deckLinkProfileAttributes.getRef() ) != S_OK )
    {
      std::cerr
          << "ERROR: Could not get the IDeckLinkProfileAttributes interface "
    ...
    if( deckLinkProfileAttributes.get()->GetInt(
            BMDDeckLinkProfileID, &currentProfileIDInt ) != S_OK )
    {
    ...
    // Nothing to do if the requested DeckLinck device profile is already active
    ...
    if( deckLinkDevice.get()->QueryInterface(
            ...
            (void**)&deckLinkProfileManager.getRef() ) != S_OK )
    {
    ...
    // Get the IDeckLinkProfile associated with the specified DeckLink device
    // profile id
    ...
    if( deckLinkProfileManager.get()->SetCallback(
            deckLinkProfileCallback.get() ) != S_OK )
    {
    ...
    {
      std::cerr
          << "ERROR: Could not activate the requested DeckLink device profile: "
          << bmdDeviceProfile << std::endl;
    ...
    {
      std::cerr << "ERROR: Timed out waiting for the new profile "
                << bmdDeviceProfile
    ...
    std::cout << "INFO: DeckLink device profile activated: " << bmdDeviceProfile
              << std::endl;
  }

  BlackMagicModule::BlackMagicModule( NativeDisplayPtr p ) : VideoModule()
  {
    open();
  ...
  BlackMagicModule::~BlackMagicModule()
  {
    close();
  }

  string BlackMagicModule::name() const
  {
    return "BlackMagic";
  }
  ...
  string BlackMagicModule::SDKInfo() const
  {
    return "";
  }
  ...
  bool BlackMagicModule::isOpen() const
  {

...
static int
lineStrideInBytesFromNDIVideoType(const NDIlib_FourCC_video_type_e ndiVideoType,
                                  const int resX)
{
    switch (ndiVideoType)
    {
...
static size_t
bufferSizeInBytesFromNDIVideoType(const NDIlib_FourCC_video_type_e ndiVideoType,
...
void NDIVideoDevice::open(const StringVector& args)
{
    ...
    description.add_options()("help,h", "Usage Message")(
        "verbose,v", "Verbose")("method,m", value<std::string>(),
                                "Method (ipbo, basic)")(
        "ring-buffer-size,s",
        value<int>()->default_value(defaultRingBufferSize), "Ring Buffer Size");
        ...
        if (m_audioFormat == TwkAudio::Int16Format)
        {
            m_ndiInterleaved16AudioFrame.p_data = reinterpret_cast<short*>(
                &m_audioData[index * m_audioSamplesPerFrame
                             * m_audioChannelCount * m_audioFormatSizeInBytes]);

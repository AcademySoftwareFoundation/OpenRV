...
int utf8Main(int argc, char** argv)
{
    ...
    for (GenericIO::Plugins::const_iterator i = plugs.begin(); i != plugs.end(); ++i)
    {
        ...
        const FrameBufferIO::ImageTypeInfos& infos = plugin->extensionsSupported();
        ...
        for (int q = 0; q < infos.size(); q++)
        {
            ...
            writer.property("codecs", Gto::String, info.compressionSchemes.size(), 2);
            writer.property("encodeParams", Gto::String, info.encodeParameters.size(), 2);
            writer.property("decodeParams", Gto::String, info.decodeParameters.size(), 2);
            writer.endComponent();

...
int computeRMS(const FrameBuffer* a,
                const FrameBuffer* b,
                bool doFloat,
                bool findMax,
                bool doCompare,
                ...
                if (findMax)
                {
                    ...
                    if (sumSq > maxDiff)
                    {
                      ...
                      for (int c=0; c < a->numChannels(); ++c)
                      {
                          ...
                          {
                              cout << "Pixel difference at (" << x << ", " << y << ") channel[" << c 
                                   << "] = " << fabs((double) cd[c]) << " found." << endl;
                              noMatch = true;
                              break;
                          }
                      }
...
int utf8Main(int argc, char *argv[])
{
    ...
    string usage = string("usage: ") + argv[0] + " [-f] [-m] [-cmp] [-dmax <value>] <image1> <image2>\n" +
        ...
        "    -dmax: Maximum allow channel error during comparison (default=0.0)";

    for (int i = 1; i < argc; ++i)
    {
        if      (strcmp(argv[i], "-f") == 0) doFloat = true;
        else if (strcmp(argv[i], "-m") == 0) findMax = true;
        else if (strcmp(argv[i], "-cmp") == 0) doCompare = true;
        else if (strcmp(argv[i], "-dmax") == 0 && (i+1 < argc)) dmax = atof(argv[++i]);
        else if (!input1) input1 = argv[i];
        else if (!input2) input2 = argv[i];
    }
...
//    cout << "a -> "; ai->outputInfo(cout); cout << " planar: " << ai->isPlanar() << endl;
//    cout << "b -> "; bi->outputInfo(cout); cout << " planar: " << bi->isPlanar() << endl;
    if (ai->isPlanar())
    {
...
//    cout << "a -> "; a->outputInfo(cout); cout << " planar: " << a->isPlanar() << endl;
//    cout << "b -> "; b->outputInfo(cout); cout << " planar: " << b->isPlanar() << endl;

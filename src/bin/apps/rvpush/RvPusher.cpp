        ...
        m_rvStarted(false)
{
    ...
    connect(m_client, SIGNAL(newContact(const QString &)),
            this, SLOT(newContact(const QString &)));
    ...
    connect(m_client, SIGNAL(contactLeft(const QString &)),
            this, SLOT(contactLeft(const QString &)));
...
RvPusher::attemptConnection()
{
    ...
    {
        //
        //  We've run out of files with no successful connection, so start a new rv, or exit.
        //

        QString rvExe;

        if (getenv("RVPUSH_RV_EXECUTABLE_PATH")) rvExe = getenv("RVPUSH_RV_EXECUTABLE_PATH");
        else
        {
            //
        ...
        else
        {
            ...
            if      (m_command == "mu-eval")        qargs.append ("-eval");
            else if (m_command == "mu-eval-return") qargs.append ("-eval");
            else if (m_command == "py-eval")        qargs.append ("-pyeval");
            else if (m_command == "py-eval-return") qargs.append ("-pyeval");
            else if (m_command == "py-exec")        qargs.append ("-pyeval");
            ...
            for (int i = 0; i < qargs.size(); ++i) DB ("        " << qargs[i].toStdString());
...
void
RvPusher::newContact(const QString &contact)
{
    //
    //  We've successfully connected, so send our one eval event, and exit unless we're
    ...
    if (    m_command == "mu-eval" ||
            m_command == "mu-eval-return" ||
            m_command == "py-eval" ||
            m_command == "py-eval-return" ||
            m_command == "py-exec")
    {
        QString src = "";
        for (int i = 0; i < m_argv.size(); ++i)
        ...
        if      (m_command == "mu-eval" || m_command == "mu-eval-return") com = "remote-eval";
        else if (m_command == "py-eval" || m_command == "py-eval-return") com = "remote-pyeval";
        else                                                              com = "remote-pyexec";
    ...
    if (m_command == "set" || m_command == "merge")
    {
        ...
        for (int i = 0; i < m_argv.size(); ++i)
        {
            if (m_argv[i] == "[") inSource = true;
            else
            if (m_argv[i] == "]") inSource = false;
            else
            if (inSource && m_argv[i][0] == '-' && isalpha(m_argv[i][1])) m_argv[i][0] = '+';

            // convert relative to absolute file path
            QFileInfo curFileInfo(QString::fromStdString(m_argv[i]));
            // Not all arguements are file path. We change only if the arguement is an existing file.
...
void
RvPusher::connectionFailed(QAbstractSocket::SocketError error)
{
    //
...
void
RvPusher::newMessage (const QString& sender, const QString& inmessage)
{

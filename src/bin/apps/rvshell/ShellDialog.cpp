...
ShellDialog::ShellDialog(QWidget *parent) : QMainWindow(parent)
{
    if (getenv("RVSHELL_PIXEL_UPDATE_ONLY")) updateOnly = true;
...
void ShellDialog::contactLeft(const QString& contact)
{
    ...
    QList<QListWidgetItem *> items = listWidget->findItems(contact,
                                                           Qt::MatchExactly);
...
{
    m_client->sendEvent(m_contact,
                        "remote-eval", 
                        eventTargetLineEdit->text(),
                        "play();"
                        "\"FRAME = \" + frame();",
                        true);
}
...
void ShellDialog::loadImage()
{
    ...
    if (m_pixmapItem) m_scene->removeItem(m_pixmapItem);
...
void ShellDialog::sendImage()
{
    ...
    for (size_t tx = 0; !m_stop && tx < nx; tx++)
    {
        size_t tw = iw / nx;
        size_t ix = tw * tx;
        if (tx == (nx - 1)) tw = iw - tw * tx; // pick up the leftover

        for (size_t ty = 0; !m_stop && ty < ny; ty++)
        {
            size_t th = ih / ny;
            size_t iy = th * ty;
            if (ty == (ny - 1)) th = ih - th * ty; // pick up the leftover
            ...
            QString interpLeft = QString( "PIXELTILE("
                ...
                .arg(m_mediaName).arg(tw).arg(th).arg(ix).arg(iy);

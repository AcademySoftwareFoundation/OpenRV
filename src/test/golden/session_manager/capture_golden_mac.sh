#!/usr/bin/env bash
# Capture Mac-native Mu baselines for session_manager into golden-mac/<id>/.
#
# Usage:
#   ./capture_golden_mac.sh [scenario_id ...]
#   SM_MERIDIAN_DIR=/path/to/clips ./capture_golden_mac.sh
#
set -euo pipefail
...
# FORCE=1 re-captures scenarios that already have a baseline. Needed whenever the
# RV build has moved under the goldens: a stale baseline fails every gate for
...
if [ ${#ids[@]} -eq 0 ]; then
    while IFS= read -r id; do
        if [ "$FORCE" != "1" ] && [ -f "$GOLDEN/$id/session.rv" ]; then
            continue
        fi
        ids+=("$id")
...
echo "Capturing ${#ids[@]} session_manager scenario(s) --impl mu --no-xvfb, 2x determinism"
...
for id in "${ids[@]}"; do
    ...
    out1="/tmp/golden_mac_capture_sm_${id}_a"
    out2="/tmp/golden_mac_capture_sm_${id}_b"
    dest="$GOLDEN/$id"
    echo "==> $id (run 1/2)"
    rm -rf "$out1"
    mkdir -p "$out1"
    ...
    # is a scenario bug — capturing it would bake in a baseline the pixel gate
    # cannot check.
    if ! compgen -G "$out1/*.png" >/dev/null 2>&1; then
        echo "FAIL $id: scenario captured no PNG"
        det_ok=0
    fi
    if [ "$det_ok" -ne 1 ]; then
        fail=$((fail + 1)); fail_list="$fail_list $id"

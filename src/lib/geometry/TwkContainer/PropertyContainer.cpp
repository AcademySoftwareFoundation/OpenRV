...
void
PropertyContainer::parseFullName(const string& fullname, StringVector& parts)
{
    algorithm::split(parts, fullname, is_any_of(string(".")));
}

void 
PropertyContainer::copyNameAndProtocol(const PropertyContainer* other)
{
    setName(other->name());
...
PropertyContainer*
PropertyContainer::shallowCopy() const
{
    PropertyContainer* c = emptyContainer();
    c->copyNameAndProtocol(this);
...
PropertyContainer::copy(const PropertyContainer* container)
{
    ...
    if (!errors.str().empty()) throw TypeMismatchExc(errors.str().c_str());
}

void
PropertyContainer::relinkPointers()
{
    // nothing
}
...
Component*
PropertyContainer::createComponent(NameIterator i, NameIterator end, bool synchronized)
{
...
Component*
PropertyContainer::createComponent(const string& name, bool synchronized)
{
    StringVector parts;
...
Component*
PropertyContainer::component(NameIterator begin, NameIterator end)
{
...
const Component*
PropertyContainer::component(NameIterator begin, NameIterator end) const
{
    if (begin == end) return 0;
...
const Property*
PropertyContainer::find(NameIterator begin, NameIterator end) const
{
    if (begin == end) return 0;
...
Property*
PropertyContainer::find(NameIterator begin, NameIterator end) 
{
    if (begin == end) return 0;
...
Property*
PropertyContainer::find(const std::string& fullname)
{
    StringVector parts;
...
const Property*
PropertyContainer::find(const std::string& fullname) const
{
    StringVector parts;
    parseFullName(fullname, parts);
    return find(parts.begin(), parts.end());
...
{
    //
    //  Try quick access via property, fallback to slower lookup if that fails.
    //

    if (Component* c = static_cast <Component*> (p->component()))
    {
        if (c->container() == p->container()) return c;
    }
...
void 
PropertyContainer::propertiesAsMap(NamedPropertyMap& pmap) const
{
    for (int i=0; i<m_components.size(); i++)
    {
...
bool
PropertyContainer::propertyPathInternal(const Property* p, ConstComponents& comps) const
{
    for (size_t i=0; i < m_components.size(); i++)
    {
        const Component* c = m_components[i];
        if (c->propertyPath(comps, p)) return true;
    }
...
string
PropertyContainer::propertyFullName(const Property* p, bool withContainer) const
{
    ostringstream str;
    ConstComponents comps;

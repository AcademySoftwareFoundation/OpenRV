...
GTOReader::Containers
GTOReader::read(const char *filename)
{
    if ( open(filename) )
    {
        return m_containers;
    ...
    else
    {
        ...
        if ( filename ) msg+=string(filename);
        msg += "\", " + why();
...
GTOReader::Containers 
GTOReader::read(std::istream& in, const char* name, unsigned int ormode)
{
...
PropertyContainer*
GTOReader::newContainer(const string& protocol)
{
    return new PropertyContainer;
}

Property* 
GTOReader::newProperty(const std::string& name,
...
GTOReader::Request
GTOReader::object(const string& name, 
                  ...
                  const ObjectInfo& info) 
{
    ...
    if (!found) m_containers.push_back(g);

    return Request(true, g);
}

GTOReader::Request
GTOReader::component(const string& name, 
                     const string& interp,
...
GTOReader::Request
GTOReader::property(const string& name, 
                    const string& ininterp,
                    const PropertyInfo& info) 
{ 
    string interp;
    PropertyContainer *pc  = 
        reinterpret_cast<PropertyContainer*>(info.component->object->objectData);
    ...
    if (info.dims.y > 0)
    {
        ...
        {
          default:
          case Gto::Int:     layout = Property::IntLayout; break;
          case Gto::Float:   layout = Property::FloatLayout; break;
          case Gto::Boolean: layout = Property::BoolLayout; break;
          case Gto::Double:  layout = Property::DoubleLayout; break;
          case Gto::Half:    layout = Property::HalfLayout; break;
          case Gto::Byte:    layout = Property::ByteLayout; break;
          case Gto::Short:   layout = Property::ShortLayout; break;
        }
...
void*
GTOReader::data(const PropertyInfo& info, size_t bytes)
{
...
void
GTOReader::dataRead(const PropertyInfo& info)
{
    Property *p  = reinterpret_cast<Property*>(info.propertyData);
...
PropertyContainer*
GTOReader::findByName(const string& name) const
{
...
PropertyContainer*
GTOReader::findByProtocol(const string& protocol) const
{
    for (size_t i = 0; i < m_containers.size(); i++)
    {
        PropertyContainer* pc = m_containers[i];
        if (pc->protocol() == protocol) return pc;
    }

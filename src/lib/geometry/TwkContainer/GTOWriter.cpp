...
GTOWriter::GTOWriter(const char *stamp) : m_writer()
{
    if (stamp) 
    {
        m_stamp = stamp;
...
Gto::DataType
GTOWriter::gtoType(const Property* p)
{
    switch (p->layoutTrait())
    {
      case Property::FloatLayout:   return Gto::Float;
      case Property::HalfLayout:    return Gto::Half;
      case Property::DoubleLayout:  return Gto::Double;
      case Property::ShortLayout:   return Gto::Short;
      case Property::IntLayout:     return Gto::Int;
      case Property::ByteLayout:    return Gto::Byte;
      case Property::StringLayout:  return Gto::String;
      default: break;
    }
...
string
GTOWriter::interpretationString(const Property* p)
{
    if (p->info())
    {
        if (p->info()->interp() != "") return p->info()->interp();
    }
...
bool
GTOWriter::shouldWriteProperty(const Property* property) const
{
    return !property->info() || property->info()->isPersistent();
}

void
GTOWriter::writeProperty(bool header, const Property *property)
{
    if (!shouldWriteProperty(property)) return;
...
void
GTOWriter::writeComponent(bool header, const Component *component)
{
    if (component->isPersistent())
    {
        ...
        if (declare) m_writer.beginComponent(component->name().c_str(), flags);
        for (size_t i = 0; i < props.size(); i++) writeProperty(header, props[i]);
        for (size_t i = 0; i < comps.size(); i++) writeComponent(header, comps[i]);
        if (declare) m_writer.endComponent();
    }
}

bool
GTOWriter::write(ostream& out,
		 const GTOWriter::ObjectVector& objects,
                 FileType type)
{
    if (!m_writer.open(out, type)) return false;
    return writeInternal(objects, type);
}

bool
GTOWriter::write(const char *filename,
		 const GTOWriter::ObjectVector& objects,
                 FileType type)
{
    if (!m_writer.open(filename, type)) return false;
    return writeInternal(objects, type);
}

bool
GTOWriter::writeInternal(const GTOWriter::ObjectVector& objects, FileType type)
{
    if (m_stamp != "")
    {
	...
	const Object&      o         = objects[i];
        ...
        if (protocol == "") protocol = container->protocol();
        if (version == 0)   version  = container->protocolVersion();

...
class PropertyContainer
{
  public:
    ...
    template<class T>
    T*			    property(const std::string& fullname);

    template<class T>
    const T*		    property(const std::string& fullname) const;
    ...
    template<class T>
    T*			    createProperty(const std::string& fullname);

    template <class T>
    T*                      createProperty(NameIterator, NameIterator);
    ...
    template<typename T>    
    typename T::value_type  propertyValue(const std::string& fullname,
                                          const typename T::value_type& defaultValue) const;

    template<typename T>    
    typename T::container_type& propertyContainer(const std::string& fullname);

    template<typename T>    
    const typename T::container_type& propertyContainer(const std::string& fullname) const;

    template<typename T>
    typename T::value_type propertyValue(const T*, const typename T::value_type& defaultValue) const;
    ...
    template<class T>
    T*                      uniqueProperty(const std::string& comp,
                                           const std::string& name);
    ...
    template<class T>
    void		    removeProperty(const std::string& comp,
                                           const std::string& name);

    template<class T>
    void		    removeProperty(const std::string& fullname);
...
template<class T>
T*
PropertyContainer::property(const std::string& fullname)
{
    StringVector parts;
    parseFullName(fullname, parts);

    if (Component* c = component(parts.begin(),
...
template<class T>
const T*
PropertyContainer::property(const std::string& fullname) const
{
    StringVector parts;
    parseFullName(fullname, parts);

    if (const Component* c = component(parts.begin(),
                                       parts.begin() + (parts.size() - 1)))
    {
...
template<class T>
T*
PropertyContainer::createProperty(const std::string& fullname)
{
    StringVector parts;
    parseFullName(fullname, parts);

    if (Component* c = createComponent(parts.begin(),
                                       parts.begin() + parts.size() - 1,
                                       false))
    {
                                   ...
                                   bool forceValue)
{
    ...
    if (forceValue) p->resize(0);
    if (info) p->setInfo(info);
    return p;
}
...
template<typename T>
void
PropertyContainer::setProperty(const std::string& fullname,
                               const typename T::value_type& value)
{
...
template<typename T>
void
PropertyContainer::setProperty(T* p, const typename T::value_type& value)
{
    p->resize(1);
    p->front() = value;
}
...
template<class T>
T*
PropertyContainer::uniqueProperty(const std::string& comp, 
                                  const std::string& name)
{
...
template<class T>
void
PropertyContainer::removeProperty(const std::string& fullname)
{
    if (T* p = property<T>(fullname))
    {
        removeProperty(p);
...
PropertyContainer::propertyValue(const std::string& fullname,
                                 const typename T::value_type& defaultValue) const
{
    if (const T* p = property<T>(fullname))
    {
        if (!p->empty() && p->size() == 1) return p->front();
    }
...
template<typename T>    
typename T::value_type
PropertyContainer::propertyValue(const T* p, const typename T::value_type& defaultValue) const
{
    if (p && !p->empty() && p->size() == 1) return p->front();
    return defaultValue;
}

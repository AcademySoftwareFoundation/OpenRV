...
size_t 
ImageProperty::sizeOfDimension(size_t dim) const 
{ 
    return m_sizes[dim]; 
}

void 
ImageProperty::restructure(size_t s0, size_t s1, size_t s2, size_t s3, size_t num)
{
    delete [] m_data;
    m_data     = 0;
    m_sizes[0] = s0;
    ...
    if (nbytes > 0) m_data = new unsigned char[nbytes];
}

size_t 
ImageProperty::sizeInBytes() const
{
    size_t total = m_num;
    for (size_t i = 0; i < 4; i++)
        if (m_sizes[i] > 0) total *= m_sizes[i];
...
void 
ImageProperty::swap(size_t a, size_t b)
{
    size_t esize = sizeofElement();
    byte* p0 = m_data + esize * a;
    byte* p1 = m_data + esize * b;
    byte* e  = p0 + esize;
    for (byte* p = p0; p != e; p++, p1++) std::swap(*p, *p1);
}

size_t 
ImageProperty::size() const
{
    return m_num;
}

bool 
ImageProperty::empty() const
{
    return m_num == 0;
}

void 
ImageProperty::resize(size_t n)
{
    if (n != m_num) 
...
void 
ImageProperty::erase(size_t start, size_t num)
{
    abort();
}

void 
ImageProperty::eraseUnsorted(size_t start, size_t num)
{
    abort();
}

size_t 
ImageProperty::sizeofElement() const
{
    size_t total = 1;
    for (size_t i = 0; i < 4; i++)
        if (m_sizes[i] > 0) total *= m_sizes[i];
...
void 
ImageProperty::insertDefaultValue(size_t index, size_t len)
{
    abort();
}

void 
ImageProperty::clearToDefaultValue()
{
    abort();
}

Property* 
ImageProperty::copy(const char* newName) const
{
...
void 
ImageProperty::copyRange(const Property*, size_t begin, size_t end)
{
    abort();
}

void 
ImageProperty::concatenate(const Property*)
{
    abort();
}
...
bool 
ImageProperty::equalityCompare(const Property* prop) const
{
    if (const ImageProperty* p = dynamic_cast<const ImageProperty*>(prop))
    {
...
bool 
ImageProperty::structureCompare(const Property* prop) const
{
    if (const ImageProperty* p = dynamic_cast<const ImageProperty*>(prop))
    {
        if (p->layoutTrait() == m_layout &&
...
string 
ImageProperty::valueAsString() const
{
    ostringstream str;
    str << sizeOfDimension(0) 
        << "x" << sizeOfDimension(1) 
        << "x" << sizeOfDimension(2) 
        << "x" << sizeOfDimension(3)

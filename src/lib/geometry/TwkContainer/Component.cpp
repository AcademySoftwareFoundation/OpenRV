...
void
Component::remove(const std::string& name)
{
    if (Property *p = find(name)) remove(p);
}

void
Component::add(Property *p)
{
    if (component(p->name()))
    {
    ...
    p->ref();
    if (!p->m_propertyContainer) p->m_propertyContainer = m_container;
...
bool
Component::hasProperty(const Property* p) const
{
    for (int i=0; i<m_properties.size(); i++)
    {
	if (m_properties[i] == p) return true;
    }
...
bool
Component::propertyPath(ConstComponents& path, const Property* p) const
{
    if (hasProperty(p))
    {
        path.push_back(this);
    ...
    for (size_t i = 0; i < m_components.size(); i++)
    {
        ...
        if (c->propertyPath(path, p)) return true;
    }
...
const Property*
Component::find(NameIterator begin, NameIterator end) const
{
    if (begin == end) return 0;

    for (size_t i = 0; i < m_properties.size(); i++)
    {
	if (m_properties[i]->name() == *begin) return m_properties[i];
    }
...
Property*
Component::find(NameIterator begin, NameIterator end) 
{
    if (begin == end) return 0;
...
Component::copy() const
{
    ...
    for (int i=0; i < m_properties.size(); i++)
    {
        ...
        if (!p->info() || p->info()->isCopyable()) c->add( p->copy() );
    }

    for (int i = 0; i < m_components.size(); i++)
    {
        if (m_components[i]->isCopyable()) c->add( m_components[i]->copy() );
    }
...
Component::shallowCopy()
{
    ...
    {
        Property* p = m_properties[i];
        if (!p->info() || p->info()->isCopyable()) c->add( p );
    }

    for (int i = 0; i < m_components.size(); i++)
    {
        if (m_components[i]->isCopyable()) c->add( m_components[i]->shallowCopy() );
    }
...
void
Component::concatenate(const Component *c)
{
    for (int i=0; i < m_properties.size(); i++)
    {
...
void
Component::erase(size_t start, size_t num)
{
    for (int i=0; i < m_properties.size(); i++)
    {
        Property *p = m_properties[i];
...
void
Component::eraseUnsorted(size_t start, size_t num)
{
    for (int i=0; i < m_properties.size(); i++)
    {
        Property *p = m_properties[i];
...
bool
Component::isPersistent() const
{
    for (int i=0; i < m_properties.size(); i++)
    {
        Property *p = m_properties[i];
        if (!p->info() || p->info()->isPersistent()) return true;
    }

    for (int i=0; i<m_components.size(); i++)
    {
        if (m_components[i]->isPersistent()) return true;
    }
...
bool
Component::isCopyable() const
{
    if (m_properties.empty()) return true;

    for (int i=0; i < m_properties.size(); i++)
    {
        Property *p = m_properties[i];
        if (!p->info() || p->info()->isCopyable()) return true;
    }

    for (int i=0; i<m_components.size(); i++)
    {
        if (m_components[i]->isCopyable()) return true;
    }
...
const Component*
Component::component(NameIterator begin, NameIterator end) const
{
...
Component*
Component::component(NameIterator begin, NameIterator end)
{
    if (begin == end) return this;
...
bool
Component::hasComponent(Component* c) const
{
    for (int i=0; i<m_components.size(); i++)
    {
	if (m_components[i] == c) return true;
    }
...
bool
Component::hasComponentRecursive(Component* c) const
{
    for (int i=0; i<m_components.size(); i++)
    {
	if (m_components[i] == c || m_components[i]->hasComponentRecursive(c)) return true;
    }
...
void 
Component::propertiesAsMap(NamedPropertyMap& pmap, std::string prefix) const
{
    for (int i=0; i < m_properties.size(); i++)
    {
        Property *p = m_properties[i];

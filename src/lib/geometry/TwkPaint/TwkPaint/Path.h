...
class Path
{
  public:
    ...
    void setContantWidth(Scalar w) { m_widths.clear(); m_width = w; }
    ...
    void computeGeometry(JoinStyle,
                         CapStyle,
                         TextureStyle,
                         Algorithm,
                         bool smooth = true,
                         float smoothInterval = 1.0,
  ...
  protected:
    ...
    void computeSegmentQuads(SegmentVector&,
                             const PointArray& fpoints,
                             const ScalarArray& fwidths,
                             const ColorArray& fcolors,
                             const PointTypeVector&,
                             bool);

    void computeJoins(JoinVector& joins,
    ...
    void buildSegmentGeometry(const Segment& segment,
                              Join& j0,
                              const float dir0,
                              Join& j1,
                              const float dir1,
    ...
    void sampleCentripetalCatmull (const Point p0,
                                   const Point p1,
                                   const Point p2,
                                   const Point p3,
                                   size_t n,
  ...
  private:
    ...
    ScalarArray        m_directionCoords; //stores the 'direction/location' of current stroked point on the stroke
    PointArray         m_rPoints;
    PointArray         m_rTexCoords;
    ScalarArray        m_rDirectionCoords; //stores the 'direction/location' of current vertex on the stroke

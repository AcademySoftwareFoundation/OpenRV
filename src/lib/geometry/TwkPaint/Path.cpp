...
void
Path::add(const Point& p, Scalar width)
{ 
    m_points.push_back(p);
...
void
Path::add(const Point& p, const Color& color)
{ 
    m_points.push_back(p);
...
void
Path::add(const Point& p, Scalar width, const Color& color)
{ 
    m_points.push_back(p);
    m_widths.push_back(width);
    m_colors.push_back(color); 
...
static void roundCap (size_t i0, size_t i1, size_t i2,
                      const float dir, 
                      PointArray& rPoints,
...
void
Path::addStamp(const float dir, Point point, float w, Color c,
               PointArray& rPoints,
               PointArray& rTexCoords,
               IndexTriangleArray& rTris,
               ScalarArray& rDirectionCoords,
               ColorArray& rColors)
{
    ...
    rPoints.push_back(p0); rTexCoords.push_back(Point(0,0)); rDirectionCoords.push_back(dir);
    rPoints.push_back(p1); rTexCoords.push_back(Point(1,0)); rDirectionCoords.push_back(dir);
    rPoints.push_back(p2); rTexCoords.push_back(Point(1,1)); rDirectionCoords.push_back(dir);
    rPoints.push_back(p3); rTexCoords.push_back(Point(0,1)); rDirectionCoords.push_back(dir);
...
//
// on top of filtering, this function also computes the 'location' of each fpoint on the whole stroke. 
// represented by m_directionCoords
// in this example: f0
//                   |
//                   |
//                  f1 
...
//                     |
//                     f3
// f0 will have 'location' 0, and f3 will have 'location' 1, f1 will have 'location' 2.0 / 5.0
// f2 will have 'location' 4.0 / 5.0
// this is a very simple mechanism, further improvements might be needed
//
void
Path::filterPoints(PointArray& fpoints,
                   ScalarArray& directionCoords, 
                   ScalarArray& fwidths,
                   ColorArray& fcolors,
...
void 
Path::computeSegmentQuads(SegmentVector& segments,
...
void 
Path::computeJoins(JoinVector& joins,
                   ...
                   bool constWidth)
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        else if (j.type == BodyPoint)
        {
            ...
            const Vec3     c  = cross(Vec3(v0.x, v0.y, 0),
                                      Vec3(v1.x, v1.y, 0));
            ...
            else if (c.z > 0)
            {
                ...
                {
                    //cout << "overlap at " << i << ", c.z = " << c.z << endl;
                }
                else
                {
...
void 
Path::buildSegmentGeometry(const Segment& segment,
                           Join& j0,
                           const float dir0,
                           Join& j1,
                           const float dir1,
                ...
                ScalarArray& rDirectionCoords)
{
    if (j.type == SplatPoint)
    {
        addStamp(dir, j.center, j.width, j.colors[0],
                 rPoints, rTexCoords, rTris, rDirectionCoords,
                 m_rColors);
        return;
    }
    ...
    switch (js)
    {
      ...
      case BevelJoin:
          if (j.threePoints)
          {
              rTris.push_back(IndexTriangle(j.indices[0],
                                            j.indices[1],
                                            j.indices[2]));
          }
          break;
...
void
Path::buildCap(CapStyle cs,
               const Join& j,
               const float directionality, 
               PointArray& rPoints, 
               PointArray& rTexCoords,
               IndexTriangleArray& rTris,
               ScalarArray& rDirectionCoords)
{
    switch (cs)
    {
      ...
      case RoundCap:
          ...
          {
              roundCap(j.indices[2], j.indices[0], j.indices[1], directionality,
                       rPoints, rTexCoords, rTris, rDirectionCoords);
          }
          else if (j.type == EndPoint)
          {
              roundCap(j.indices[2], j.indices[1], j.indices[0], directionality,
                       rPoints, rTexCoords, rTris, rDirectionCoords);
          }
          break;
    }
...
void
Path::computeGeometry(JoinStyle js,
                      CapStyle cs,
                      TextureStyle ts,
                      Algorithm algo,
                      bool dosmooth,
                      float smoothInterval,
                      bool splatOnly)
{
    ...
    if (n == 1) 
    {
        ...
        addStamp(0, fpoints.front(),
                 constWidth ? m_width : fwidths.front(),
                 hasColor ? fcolors.front() : black,
                 m_rPoints, m_rTexCoords, m_rTris, m_rDirectionCoords, m_rColors);
        return;
    }
...
void
Path::sampleCentripetalCatmull(const Point p0,
                               const Point p1,
                               const Point p2,
                               const Point p3,
                               size_t n,

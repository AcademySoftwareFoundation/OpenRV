...
//#define USE_SAFE_ALSA

#ifndef USE_SAFE_ALSA
...
static void
alsaErrorHandler (const char *file,
                  int line,
                  const char *function,
                  int err,
                  const char *fmt,
...
static int 
snd_pcm_open_real_hard(snd_pcm_t **pcm,
                       const char *name, 
                       ...
                       size_t iterations = 1)
{
    ...
    if (!err && *pcm) snd_pcm_nonblock(*pcm, 0); // 0 means block
    return err;
}

ALSAAudioRenderer::ALSAAudioRenderer(const RendererParameters& p)
    : AudioRenderer(p),
      m_threadGroup(1,2),
      m_pcm(0),
      ...
      m_audioThreadRunning(false)
{
    ...
    if (m_parameters.rate == 0) m_parameters.rate = TWEAK_AUDIO_DEFAULT_SAMPLE_RATE;
    if (m_parameters.framesPerBuffer == 0) m_parameters.framesPerBuffer = 512;
    if (m_parameters.device == "") m_parameters.device = "default";
...
void 
ALSAAudioRenderer::createDeviceList()
{
    ...
    for (void** d = deviceList; *d; d++)
    {
        char* namep = snd_device_name_get_hint(*d, "NAME");
        char* descp = snd_device_name_get_hint(*d, "DESC");
        ...
        //
        if (name == "null" || name.find("surround") != string::npos) continue;
    ...
    if (const char* xdevices = getenv("RV_ALSA_EXTRA_DEVICES"))
    {
        ...
        for (size_t i = 0; i < tokens.size(); i++)
        {
            ...
            {
                cerr << "ERROR: RV_ALSA_EXTRA_DEVICES env variable -- syntax error" << endl;
                break;
            }
...
void 
ALSAAudioRenderer::availableLayouts(const Device& d, LayoutsVector& layouts)  
{
    layouts.clear();
...
void 
ALSAAudioRenderer::availableFormats(const Device& d, FormatVector& formats)  
{
    //
    ...
    for (size_t i = 0; i < 3; i++)
    {
        ...
        if ((err = snd_pcm_open_real_hard(&pcm,
                                          deviceName.c_str(),
                                          ...
                                          10)) == 0)
        {
            ...
            switch (format)
            {
              case Float32Format: alsaformat = SND_PCM_FORMAT_FLOAT_LE; break;
              case Int32Format: alsaformat = SND_PCM_FORMAT_S32_LE; break;
              case Int16Format: alsaformat = SND_PCM_FORMAT_S16_LE; break;
              case Int8Format: alsaformat = SND_PCM_FORMAT_S8; break;
            }
...
void 
ALSAAudioRenderer::availableRates(const Device& device, Format format, RateVector& rates) 
{
    //
    //  The base class ensures that this function is not called when a
    //  device is opened.
    ...
    for (size_t i = 0; standardSampleRates[i] > 0; i++)
    {
                                          ...
                                          10)) == 0)
        {
            ...
            if (rrate == rate) rates.push_back((unsigned int)rate);
...
{
    ALSAAudioRenderer* a = (ALSAAudioRenderer*)data;
    a->threadMain();
}

void
ALSAAudioRenderer::configureDevice()
{
    int err;
    ...
    if (!m_pcm)
    {
        ...
        if ((err = snd_pcm_open_real_hard(&m_pcm,
                                          alsaDeviceName.c_str(),
                                          SND_PCM_STREAM_PLAYBACK,
                                          SND_PCM_NONBLOCK)) < 0) 
        {
            cerr << "WARNING: ALSA: Playback open error ("
                 << err << "): " << snd_strerror(err) << endl;
        ...
        {
            cerr << "ERROR: ALSA: access (interleaved) failed - can't continue" << endl;
        ...
        if (snd_pcm_hw_params_set_format(m_pcm, params, alsaformat) < 0)
        {
            ...
            {
                if (debug) cerr << "WARNING: ALSA: format falling back to Int16" << endl;
...
void
ALSAAudioRenderer::threadMain()
{
    //
    //  Set the thread priority if asked to do so
    //

    size_t sched_priority = Application::optionValue<size_t>("displayPriority", size_t(-1));
    string schedulePolicy = Application::optionValue<string>("schedulePolicy", "");
    ...
    if (schedulePolicy != "")
    {
        ...
        if (schedulePolicy == "SCHED_RR") policy = SCHED_RR;
        else if (schedulePolicy == "SCHED_FIFO") policy = SCHED_FIFO;
        ...
        {
            if (setpriority(PRIO_PROCESS, 0, sp.sched_priority))
            {
                cout << "ERROR: can't set thread priority" << endl;
            }
        }
    ...
    AudioBuffer buffer(state.framesPerBuffer,
                       state.layout,
                       state.rate);

    m_abuffer.reconfigure(state.framesPerBuffer,
                          state.layout,
                          Time(state.rate));
         ...
         count++)
    {
        ...
        buffer.reconfigure(state.framesPerBuffer,
                           state.layout,
                           Time(state.rate), 
        ...
        for (size_t wcount = 0;
             (pcmState == SND_PCM_STATE_RUNNING ||
	    ...
	    if (debug)
	    {
	        cerr << "INFO: pcm_writei: wcount " << wcount << 
			", count " << count << 
			", total " << total << 
			", chan " << channelsCount << 
            ...
            frames = snd_pcm_writei(m_pcm, 
                                    out + (total * channelsCount * formatSize),
                                    buffer.size() - total);
	    if (debug) cerr << "INFO: pcm_writei complete, frames " << frames << endl;
            ...
            else
            {
                ...
                if (debug)
                {
                    ...
                    switch (frames)
                    {
                      case -EBADFD: cerr << "bad pcm state"; break;
                      case -EPIPE: cerr << "underrun occured"; break;
                      case -ESTRPIPE: cerr << "suspend event occured"; break;
                    }
        ...
        {
            if (!m_pcm) return;
            if (snd_pcm_prepare(m_pcm) < 0) break;
            if (!m_pcm) return;
            if ((pcmState = snd_pcm_state(m_pcm)) == SND_PCM_STATE_XRUN) break;
        }
    }
    
    if (!m_pcm) return;
    if (pcmState == SND_PCM_STATE_RUNNING) snd_pcm_drop(m_pcm);

    m_threadGroup.lock(m_runningLock);
    m_audioThreadRunning = false;
    m_threadGroup.unlock(m_runningLock);

    if (debug) cout << "DEBUG: audio thread exiting" << endl;
}

void
ALSAAudioRenderer::play(Session* s)
{
...
ALSAAudioRenderer::play()
{
    ...
    if (notconfigured) m_threadGroup.dispatch(trampoline, this);
}

void
ALSAAudioRenderer::stop(Session* s)
{

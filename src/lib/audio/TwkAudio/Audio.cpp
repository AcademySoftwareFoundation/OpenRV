...
AudioBuffer::AudioBuffer(size_t numSamples, 
...
AudioBuffer::AudioBuffer(Time start,
                         Time duration,
                         Time rate,
                         ChannelsVector channels,
                         size_t margin)
...
AudioBuffer::AudioBuffer(AudioBuffer& inbuffer,
                         size_t startOffsetSample,
                         size_t numSamples,
                         Time startTime)
{
...
AudioBuffer::ownData()
{
    if (!bufferOwnsData())
    {
        ...
        if (!m_buffer.empty())
        {
            memcpy(&m_buffer.front(), 
                   m_data - m_margin, 
                   sizeof(float) * m_channels.size() * (m_numSamples + 2 * m_margin));
...
void
AudioBuffer::reconfigure(size_t numSamples,
                         ChannelsVector channels,
                         Time rate,
                         Time startTime,
                         size_t margin)
...
AudioBuffer::reconfigure(size_t numSamples,
                         ...
                         size_t margin)
{
    return reconfigure(numSamples,
                       layoutChannels(layout),
                       rate,
                       startTime,
                       margin);
}
...
AudioBuffer::zero()
{
    if (m_data) memset(m_data - m_margin * m_channels.size(), 0, sizeInBytesIncludingMargin());
}

void
AudioBuffer::zeroRegion(size_t start, size_t n)
{
    if (!m_data) return;
...
bool
AudioBuffer::bufferOwnsData() const
{
    if (m_buffer.empty())
    {
        return m_data == 0;
...
void
AudioBuffer::reverse(bool reverseChannels)
{
    float* data = m_data;
...
bool
AudioBuffer::checkBuffer(const char* label) const
{
    size_t zeroCount = 0;

...
size_t
Resampler::process(const float* in, 
                   size_t inSize,
                   float* out, 
                   size_t outSize, 
                   bool endFlag,
                   bool enableClamp)
{
    ...
    for (int i=0; true; i++)
    {
        ...
        size_t outUsed = resample_process(m_handle,
                                          m_factor,
                                          (float*)(in + inUsedTotal), 
                                          bsize,
                                          endFlag && left < m_blocksize,
                                          &inUsed,
                                          out + outUsedTotal, 
                                          outSize - outUsedTotal);
...
void
Resampler::process(const SampleVector& in, SampleVector& out, bool endFlag, bool enableClamp)
{
...
MultiResampler::MultiResampler(int numChannels,
                               double factor,
                               size_t blocksize)
...
size_t
MultiResampler::process(const float* in, 
                        size_t inSize,
                        float* out, 
                        size_t outSize, 
                        bool endFlag,
                        bool enableClamp)
{
    ...
    for (int ch = 0; ch < m_resamplers.size(); ch++)
    {
        size_t nsamps = m_resamplers[ch]->process(&(m_inbuffers[ch].front()),
                                                  inSize,
                                                  &(m_outbuffers[ch].front()),
                                                  outSize,
                                                  endFlag,
                                                  enableClamp);
#ifndef NDEBUG

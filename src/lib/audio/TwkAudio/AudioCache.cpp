...
AudioCache::clear()
{
    for (PacketMap::iterator i = m_map.begin();
         i != m_map.end();
         ++i)
    {
        //delete [] i->second;
...
void
AudioCache::clear(Time time0, Time time1)
{
...
void
AudioCache::configurePacket(size_t samples, 
                            TwkAudio::Layout layout,
                            Time rate)
{
    if (m_packetSize   != samples  || 
...
const AudioCache::Packet
AudioCache::find(SampleTime s) const
{
    s -= packetOffset(s);
...
void
AudioCache::add(const AudioBuffer& buffer)
{
    Time t = buffer.startTime();
    SampleTime s = sampleAtTime(t);

    //
    // Since all samples in the cache are stored in packet size clumps we must
    // make sure that this sample set starts exactly on a packet boundary,
    // is the right amount of samples and channels, and is at the correct
    // sampling rate.
    //

    if (packetOffset(s) != 0 ||
        buffer.size() != m_packetSize ||
        buffer.channels() != channels() ||
        buffer.rate() != m_packetRate)
    {
        cout << "WARNING: s = " << s 
             << ", packetOffset = " << packetOffset(s)
             << ", m_packetSize = " << m_packetSize 
             << ", size = " << buffer.size()
             << ", m_packetLayout = " << TwkAudio::layoutString(m_packetLayout)
...
bool
AudioCache::fillBuffer(AudioBuffer& buffer) const
{
    ...
    if (d0 <= m_packetSize)
    {
        ...
        if (p0 && p0 == p1)
        {
            ...
            memcpy(buffer.pointer(), 
                   p0 + packetOffset(s0) * numChannels, 
                   buffer.sizeInBytes());
        ...
        else if (p0 && p1)
        {
            ...
            memcpy(buffer.pointer() + n0 * numChannels, 
                   p1,
    ...
    else
    {
        ...
        for (size_t i = 0; i < n; i++)
        {
            ...
            AudioBuffer newBuffer(buffer.pointer() + offset * buffer.numChannels(),
                                  buffer.channels(),
                                  m_packetSize,
                                  buffer.startTime() + double(i * m_packetSize) / m_packetRate,
                                  m_packetRate);

            if (!fillBuffer(newBuffer)) return false;
        }
        ...
        if (r)
        {
            ...
            AudioBuffer newBuffer(buffer.pointer() + offset * buffer.numChannels(),
                                  buffer.channels(),
                                  r,
                                  buffer.startTime() + double(n * m_packetSize) / m_packetRate,
                                  m_packetRate);
...
//
void
AudioCache::computeCachedRangesStat(double frameRate, FrameRangeVector& array)
{
    lock();

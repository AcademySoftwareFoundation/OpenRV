...
#define AUDIOBUFFER_CHECK(_audioBufferInst, _label) { (_audioBufferInst).checkBuffer(_label); }
#else
...
/// Convert Audio Time to sample count. Make sure to round "down" for negative times
...
public:
    ...
    AudioBuffer(BufferPointer externalMemory,
                ChannelsVector channels,
                size_t numSamples,
                Time start,
                Time rate,
    ...
    AudioBuffer(AudioBuffer& inbuffer,
    ...
    void reconfigure(size_t numSamples,
                     ChannelsVector channels,
                     Time rate, 
                     Time startTime=0,
                     size_t margin=0);
    ...
    size_t sizeInBytesIncludingMargin() const 
        { return (m_numSamples + 2 * m_margin) * m_channels.size() * sizeof(float); }

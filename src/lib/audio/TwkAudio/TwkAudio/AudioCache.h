...
public:
    ...
    {
        AudioCacheLock(AudioCache& c) : cache(c) { cache.lock(); }
    ...
    void configurePacket(size_t samples, 
    ...
    bool exists(SampleTime packet) const
        { return find(packet) != 0; }
...
protected:
    ...
    SampleTime packetOffset(SampleTime s) const
        { return s % m_packetSize; }

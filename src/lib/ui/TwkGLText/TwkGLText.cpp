...
{
    GLTextContext() : color(0.0, 0.0, 0.0, 1.0),
                      initialized(false),
                      size(12),
                      fontName("")
    {
...
void GLtext::deleteContext(Context c)
{
    delete (GLTextContext*)c;
}
...
int GLtext::writeAtNL( float x, float y, std::string text, float mult )
{
    ...
    float advance = mult * ( (*ctx->fonts[ctx->fontName])[ctx->size]->Ascender()
...
int GLtext::writeAtNL( const TwkMath::Vec2f &pos, std::string text, float mult )
{
...
float GLtext::globalAscenderHeight()
{
    GLTextContext* ctx = (GLTextContext*)getContext();
    if (ctx->fonts.count(ctx->fontName) == 0 || ctx->fonts[ctx->fontName]->size() <= ctx->size
            || !(*ctx->fonts[ctx->fontName])[ctx->size]) init();

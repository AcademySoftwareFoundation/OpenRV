...
OSMesaVideoDevice::OSMesaVideoDevice(VideoModule* m,
                                     int w, int h, 
                                     bool alpha,
                                     bool floatbuffer,
                                     bool topleft)
...
static OSMesaVideoDevice::VoidFunc
GetAddressProc(const char* name)
{
    return OSMesaGetProcAddress(name);
}

OSMesaVideoDevice::ProcAddressFunc 
...
void
OSMesaVideoDevice::resize(int w, int h, bool alpha)
{
    m_width = w;
    m_height = h;
    m_alpha = alpha;
...
{
    OSMesaMakeCurrent(m_imp->context,
                      fb->pixels<void>(),
                      m_float ? GL_FLOAT : GL_UNSIGNED_BYTE,
                      fb->width(),
                      fb->height());
...
string 
OSMesaVideoDevice::hardwareIdentification() const
{
    return "osmesa";
}

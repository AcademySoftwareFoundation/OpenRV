      ...
      m_alpha(alpha)
{
    ...
    unsigned long hwAttrs[] =
        {
            ...
            kCGLPFAColorSize, 3 * 16,
            kCGLPFAAlphaSize, 1 * 16,
            0
        };
    ...
    unsigned long swAttrs[] =
        {
            kCGLPFAAllRenderers,
            kCGLPFARendererID, kCGLRendererAppleSWID,
            kCGLPFAColorFloat,	/* color buffers store floating point pixels    */
            kCGLPFAColorSize, 3 * 32,
            kCGLPFAAlphaSize, 1 * 32,
            0
        };
    ...
    if (CGLError err =
        CGLChoosePixelFormat((CGLPixelFormatAttribute*)attrs, &m_imp->pfo, &m_imp->npfo))
    {
    ...
    swa.colormap    = XCreateColormap(m_imp->display,
                                      m_imp->root,
                                      m_imp->vis->visual,
                                      AllocNone);
    ...
    m_imp->tiny = XCreateWindow(m_imp->display, m_imp->root,
                                0, 0, 64, 64, 0,
                                m_imp->vis->depth, InputOutput,
                                m_imp->vis->visual, CWColormap | CWEventMask, &swa);

    m_imp->ctx   = glXCreateContext(m_imp->display,
                                    m_imp->vis,
                                    0,
                                    True);
    ...
    HINSTANCE hInstance = GetModuleHandle(0); assert(hInstance != NULL);
...
std::string
FBOVideoDevice::hardwareIdentification() const
{
    return "fbo-rb";
}

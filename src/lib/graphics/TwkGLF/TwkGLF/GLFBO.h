...
//
//  Somebody (system header) is defining "check" and leaving it defined -- brilliant
//
#ifdef check
#undef check
#endif
...
//
//  Each GLFBO object keeps track of how much card memory it uses in m_totalSizeInBytes
//  right now we only track colorTexture
//
...
class GLFBO
{
  public:
    ...
    struct Attachment
    {
        Attachment(GLuint _id          = 0,
                   GLuint _attachPoint = 0,
    ...
    void copyRegionTo(const GLFBO* destinationFBO,
  ...
  private:
    ...
    // these 3 defaults are the properties of GL default framebuffers (as opposed to FBOs)

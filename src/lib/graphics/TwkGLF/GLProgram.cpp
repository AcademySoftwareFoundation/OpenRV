...
void 
GLProgram::collectUniforms()
{
    // get all the active uniforms in this gl program, and their locations.
    // cache these for future use. 
    // we want to avoid calling glGetUniformLocation at run time as much as possible, 
    // because these commands force the system to execute all the queued gl calls before
    ...
    {
        glGetActiveUniform(pid, i, sizeof(uniformName)-1,
                           &nameLength, &uniformSize, &uniformType, uniformName);
...
unsigned int 
GLProgram::uniformLocation(const string& name) const
{
...
unsigned int 
GLProgram::attribLocation(const string& name) const
{

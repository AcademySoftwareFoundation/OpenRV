...
GLFence::~GLFence()
{
    ...
    {
#if defined(HAVE_ARB_SYNC_API)
        if (m_imp->arbSync && glIsSync(m_imp->arbSync)) glDeleteSync(m_imp->arbSync);
#endif
    }
    else if (hasAPPLE)
    {
...
void
GLFence::wait(bool client) const
{
    if (hasARB)
    {
#if defined(HAVE_ARB_SYNC_API)
        ...
        if (!glIsSync(m_imp->arbSync))
        {
            ...
            //cout << "ERROR: GL fence no longer valid: " << m_imp->arbSync << endl;
            return;
        }

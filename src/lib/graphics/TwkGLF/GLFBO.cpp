...
GLFBO::~GLFBO()
{
    ...
    if (m_pbo)
    {
        if (m_fence) m_fence->wait();
        delete m_fence;
        glDeleteBuffers(1, &m_pbo); TWK_GLDEBUG;
    }
}
  
std::string
GLFBO::identifier() const
{
    ostringstream o;
    o << "fbo" << m_id;
    return o.str();    
...
GLFBO::width() const 
{ 
    if (m_device) return m_device->width();
    return m_width; 
}
...
GLFBO::height() const 
{ 
    if (m_device) return m_device->height();
    return m_height; 
}

const GLFBO::Attachment*
GLFBO::colorAttachment(size_t i) const
{
    try
    {
...
const GLFBO::Attachment*
GLFBO::colorTextureAttachment(size_t i) const
{
    try
    {
        for (size_t q = 0, count = 0; q < m_attachments.size(); q++)
...
GLuint
GLFBO::colorTarget(size_t i) const
{
    const Attachment* attach = colorAttachment(i);
    assert(attach);
...
GLuint
GLFBO::primaryColorTarget() const
{
    if (m_attachments.empty()) return m_defaultTarget;
...
GLenum
GLFBO::primaryColorFormat() const
{
    return m_colorFormat;
}

bool 
GLFBO::isColorTexture(size_t i) const
{
    const Attachment* attach = colorAttachment(i);
    assert(attach);
...
GLFBO::Attachment
GLFBO::attachColorRenderBuffer(GLuint id)
{
    bind();
    ...
    m_attachments.push_back(Attachment(id,
                                       GL_COLOR_ATTACHMENT0_EXT + m_colorCount,
                                       GL_UNSIGNED_BYTE,
                                       false,
                                       false,
...
GLFBO::Attachment
GLFBO::newColorRenderBuffer()
{
    bind(); TWK_GLDEBUG;

    GLuint id;
    glGenRenderbuffersEXT(1, &id); TWK_GLDEBUG;
    glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, id); TWK_GLDEBUG;
...
GLFBO::Attachment
GLFBO::attachColorTexture(GLuint target, GLuint id)
{
    bind();
...
GLFBO::Attachment
GLFBO::newColorTexture(GLenum target,
                       GLenum format, 
                       GLenum type,
                       GLenum minFilter,
                       GLenum magFilter,
...
GLFBO::Attachment
GLFBO::newPackedDepthStencilBuffer()
{
    bind();
    ...
    glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, 0); TWK_GLDEBUG;
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0); TWK_GLDEBUG;
...
GLFBO::Attachment
GLFBO::attachPackedDepthStencilBuffer(GLuint id)
{
    bind();
...
void
GLFBO::bind(GLenum kind) const
{
    waitForExternalReadback();
    glBindFramebufferEXT(kind, m_id); TWK_GLDEBUG;
}

void
GLFBO::unbind(GLenum kind) const
{
    glBindFramebufferEXT(kind, 0); TWK_GLDEBUG;
}
...
void 
GLFBO::bindColorTexture(size_t i) const
{
...
void 
GLFBO::unbindColorTexture() const
{
...
void 
GLFBO::copyTo(const GLFBO* destinationGLFBO, GLenum mask, GLenum filter) const
{
...
void 
GLFBO::copyRegionTo(const GLFBO* destinationGLFBO,
...
size_t
pixelSizeOfInternalFormat(GLenum format)
{
    size_t n = 0;

    switch (format)
    {
      case GL_RGB8:         n = sizeof(GLbyte) * 3;         break;
      case GL_RGBA8:        n = sizeof(GLbyte) * 4;         break;
      case GL_RGB16:        n = sizeof(GLshort) * 3;        break;
      case GL_RGBA16:       n = sizeof(GLshort) * 4;        break;
      case GL_RGB10_A2:     n = sizeof(GLuint);             break;
      case GL_RGB16F_ARB:   n = sizeof(unsigned short) * 3; break;
      case GL_RGBA16F_ARB:  n = sizeof(unsigned short) * 4; break;
      case GL_RGB32F_ARB:   n = sizeof(GLfloat) * 3;        break;
      case GL_RGBA32F_ARB:  n = sizeof(GLfloat) * 4;        break;
    }
...
GLenumPair
dataTypeOfColorFormat(GLenum format)
{
    switch (format)
    {
      case GL_RGB8:    return GLenumPair(GL_RGB, GL_UNSIGNED_BYTE);
      default:
      case GL_RGBA8:   return GLenumPair(GL_RGBA, GL_UNSIGNED_BYTE);
      case GL_RGB16:   return GLenumPair(GL_RGB, GL_UNSIGNED_SHORT);
      case GL_RGBA16:  return GLenumPair(GL_RGBA, GL_UNSIGNED_SHORT);
      case GL_RGB10_A2: return GLenumPair(GL_RGBA, GL_UNSIGNED_INT_10_10_10_2);
      case GL_RGB16F_ARB:  return GLenumPair(GL_RGB, GL_HALF_FLOAT_ARB);
      case GL_RGBA16F_ARB: return GLenumPair(GL_RGBA, GL_HALF_FLOAT_ARB);
      case GL_RGB32F_ARB:  return GLenumPair(GL_RGB, GL_FLOAT);
      case GL_RGBA32F_ARB: return GLenumPair(GL_RGBA, GL_FLOAT);
    }
}
...
void
GLFBO::initReadBack(GLenum hint)
{
    if (m_pbo) return;
    size_t totalSizeInBytes = m_width * m_height * pixelSizeOfInternalFormat(m_colorFormat);
    glGenBuffers(1, &m_pbo); TWK_GLDEBUG;
    glBindBuffer(GL_PIXEL_PACK_BUFFER_ARB, m_pbo); TWK_GLDEBUG;
    glBufferData(GL_PIXEL_PACK_BUFFER_ARB, totalSizeInBytes, NULL, hint); TWK_GLDEBUG;
    glBindBuffer(GL_PIXEL_PACK_BUFFER_ARB, 0); TWK_GLDEBUG;
}

void
GLFBO::insertFence() const
{
    assert(m_fence == NULL);
    m_fence = new GLFence();
    m_fence->set();
...
void
GLFBO::waitForFence(bool client) const
{
    assert(m_fence);
    m_fence->wait(client);
    m_state = FenceWait;
...
void
GLFBO::beginExternalReadback() const
{
...
void
GLFBO::endExternalReadback() const
{
    ScopedLock lock(m_mutex);
    m_state = Renderable;
    readCond().notify_all();
...
void 
GLFBO::waitForExternalReadback() const
{
    ScopedLock lock(m_mutex);
    while (m_state == ExternalReading) readCond().wait(lock);
}

void 
GLFBO::beginAsyncReadBack() const
{
    assert(m_pbo);
    GLenumPair p = dataTypeOfColorFormat(m_colorFormat);
    glBindBuffer(GL_PIXEL_PACK_BUFFER_ARB, m_pbo); TWK_GLDEBUG;
    glReadPixels(0, 0, m_width, m_height, p.first, p.second, NULL); TWK_GLDEBUG;
    m_state = Reading;
}

void*
GLFBO::mapBuffer() const
{
    glBindBuffer(GL_PIXEL_PACK_BUFFER_ARB, m_pbo); TWK_GLDEBUG;
    m_mappedBuffer = glMapBuffer(GL_PIXEL_PACK_BUFFER_ARB, GL_READ_ONLY_ARB); TWK_GLDEBUG;
    m_state = Mapped;

...
public:
   ...
   // the previous mapped data will not be affected by the resize until the next map()
...
{
   if ( _id > 0 ) {
      RV_LOG( "~BufferObject (#%d) -- _id = %d, size = %u kB alignment = %u\n",
              countBOs(-1) + 1, _id, _numBytes/1024, _alignment);

      HOP_PROF_FUNC();
      glDeleteBuffers(1, &_id); TWK_GLDEBUG;
   }
}
//------------------------------------------------------------------------------
void BufferObject::bind()
{
   if (_id == 0) {
      // the 1st bind of an unintilaized buffer object will intialize it
      {
         ...
         glGenBuffers(1, &_id); TWK_GLDEBUG;
         glBindBuffer(_target, _id); TWK_GLDEBUG;    
         RV_ASSERT_INTERNAL( _id != 0 );
      
         glBufferData(_target, _numBytes + _alignment, 0, _usage); TWK_GLDEBUG;
      }

      RV_LOG( "BufferObject::bind new (#%d) -- _id = %d, size = %u kB alignment = %u\n",
              countBOs(1), _id, _numBytes/1024, _alignment);
   } else {
      RV_LOG( "BufferObject::bind (#%d) -- _id = %d, size = %u kB alignment = %u\n",
   ...
   if (!_mappedPtr) {
      {
         ...
         bind();
         _mappedPtr = glMapBuffer(_target, access); TWK_GLDEBUG;
         if ( _alignment > 1 ) {
      ...
      RV_LOG( "BufferObject::map -- _id=%d, _mappedPtr=%p _alignmentOffset=%u\n",
...
{
   if (_mappedPtr) {
      RV_LOG( "BufferObject::unmap -- _id=%d, _mappedPtr=%p _alignmentOffset=%u\n",
              _id, _mappedPtr, _alignmentOffset);

      HOP_PROF_FUNC();

      bind();
      glUnmapBuffer(_target); TWK_GLDEBUG;
      unbind();
...
void BufferObject::copyBufferData( void * data, unsigned size )
{
   ...
   bind();
   // We don't want to read more than the output buffer, nor do we want to read
   // more than the actual gl buffer contains otherwise GL_INVALID_VALUE occurs
                                     ...
                                     unsigned int alignment)
{
   ...
   _bufferObject = new BufferObject(d==TO_GPU ? GL_PIXEL_UNPACK_BUFFER : GL_PIXEL_PACK_BUFFER,
                                    d==TO_GPU ? GL_STREAM_DRAW         : GL_STREAM_READ,
                                    num_bytes,
                                    alignment);
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void GLPixelBufferObject::bind()
{
   _bufferObject->bind();
}
//------------------------------------------------------------------------------
void GLPixelBufferObject::unbind()
{
...
//------------------------------------------------------------------------------
void GLPixelBufferObject::unmap()
{
   _bufferObject->unmap();
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void GLPixelBufferObject::release()
{
   _bufferObject->release();
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
GLuint GLPixelBufferObject::getId() const
{
   return _bufferObject->getId();
}
//------------------------------------------------------------------------------
GLPixelBufferObject::PackDir GLPixelBufferObject::getPackDir() const
{
   return
      _bufferObject->getTarget() == GL_PIXEL_UNPACK_BUFFER ? TO_GPU : FROM_GPU;
}

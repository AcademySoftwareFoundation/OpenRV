...
GLSyncObjectARBSync::~GLSyncObjectARBSync()
{
   if ( _sync ) 
   {
      glDeleteSync( _sync ); TWK_GLDEBUG
   }
}
...
void GLSyncObjectARBSync::setFence()
{
   ...
   _sync = glFenceSync( GL_SYNC_GPU_COMMANDS_COMPLETE, 0 ); TWK_GLDEBUG    
   _fenceSet = true;
...
void GLSyncObjectARBSync::waitFence() const
{
   ...
   while (
      ( ret = glClientWaitSync( _sync, GL_SYNC_FLUSH_COMMANDS_BIT, ~0ULL ) ) ==
      GL_TIMEOUT_EXPIRED
   );
...
{
   RV_ASSERT_INTERNAL( _fenceSet && _sync != 0 );
   GLenum ret = glClientWaitSync( _sync, GL_SYNC_FLUSH_COMMANDS_BIT, 0 );TWK_GLDEBUG
...
SyncObjectFenceNV::~SyncObjectFenceNV()
{
   if ( _fence ) glDeleteFencesNV( 1, &_fence );TWK_GLDEBUG
}
...
void SyncObjectFenceNV::setFence()
{
   ...
   if ( !_fence ) 
   {
      glGenFencesNV( 1, &_fence );TWK_GLDEBUG
   }
   glSetFenceNV( _fence, GL_ALL_COMPLETED_NV );TWK_GLDEBUG
   _fenceSet = true;
}
...
{
   RV_ASSERT_INTERNAL( _fenceSet );
   glFinishFenceNV( _fence ); TWK_GLDEBUG
   _fenceSet = false;
}
...
{
   RV_ASSERT_INTERNAL( _fenceSet );
   bool done = glTestFenceNV( _fence ); TWK_GLDEBUG
   if ( done ) _fenceSet = false;
   return done;
}
...
SyncObjectFenceAPPLE::~SyncObjectFenceAPPLE()
{
   if ( _fence ) 
   {
      glDeleteFencesAPPLE( 1, &_fence );TWK_GLDEBUG
   }
}
...
void SyncObjectFenceAPPLE::setFence()
{
   ...
   if ( !_fence ) 
   {
      glGenFencesAPPLE( 1, &_fence );TWK_GLDEBUG
   }
   glSetFenceAPPLE( _fence );TWK_GLDEBUG
   _fenceSet = true;
}
...
{
   RV_ASSERT_INTERNAL( _fenceSet );
   glFinishFenceAPPLE( _fence );TWK_GLDEBUG
   _fenceSet = false;
}
...
{
   RV_ASSERT_INTERNAL( _fenceSet );
   bool done = glTestFenceAPPLE( _fence );TWK_GLDEBUG
   if ( done ) _fenceSet = false;
   return done;
}
...
//------------------------------------------------------------------------------
//
bool SyncObjectStub::testFence() const
{
   return false;
}
...
//------------------------------------------------------------------------------
//
void GLSyncObject::waitFence() const
{
   _imp->waitFence();
}
...
//------------------------------------------------------------------------------
//
bool GLSyncObject::testFence() const
{
   return _imp->testFence();
}

//------------------------------------------------------------------------------
//
bool GLSyncObject::tryTestFence() const
{
   return _imp->tryTestFence();
}

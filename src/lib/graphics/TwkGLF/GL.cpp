...
string
safeGLGetString (GLenum name)
{
    if (const GLubyte *s = glGetString (name)) return string((const char *) s);
    else return string();
}

string
errorString(GLenum errorCode)
{
    for (size_t i = 0; GLErrors[i].String; i++) 
    {
        if (GLErrors[i].Token == errorCode) return string(GLErrors[i].String);
    }
...
bool
glSupportsExtension(const char *extstring)
{
    char *s = (char*) glGetString(GL_EXTENSIONS); //Get our extension-string
    char *temp = (s) ? strstr(s, extstring) : NULL; //Is our extension a string?
    return temp != NULL;                          //Return false.
}

void
glDrawBox(const Box3f &bbox)
{
    ...
    glNormal3f( 0, 0,-1); glVertex(a); glVertex(e); glVertex(g); glVertex(c);
    glNormal3f(-1, 0, 0); glVertex(a); glVertex(c); glVertex(d); glVertex(b);
    glNormal3f( 0,-1, 0); glVertex(a); glVertex(b); glVertex(f); glVertex(e);

    glNormal3f( 0, 1, 0); glVertex(h); glVertex(d); glVertex(c); glVertex(g);
    glNormal3f( 1, 0, 0); glVertex(h); glVertex(g); glVertex(e); glVertex(f);
    glNormal3f( 0, 0, 1); glVertex(h); glVertex(f); glVertex(b); glVertex(d);
                         ...
                         GLdouble focus)
{
    ...
    glJitterFrustum (left, right, bottom, top, nearPlane, farPlane,

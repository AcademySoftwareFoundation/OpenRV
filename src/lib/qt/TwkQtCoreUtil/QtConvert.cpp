...
QString 
qconvert(const string& s)
{
    return QString::fromUtf8(s.c_str(), s.size());
}

QString 
qconvert(const char* s)
{
    if (s)
    {
...
QString
qconvert(const std::ostringstream& str)
{
    string s = str.str();
    return QString::fromUtf8(s.c_str(), s.size());
}
...
QStringList
qconvert(const std::vector<std::string>& v)
{
    QStringList list;
    for (size_t i = 0, s = v.size(); i < s; i++) list << v[i].c_str();
    return list;
}

...
#
#
#   Qt 6.8 variant of qt6_to_mu.py (which targets Qt 6.5.3).
#
#   Differences from qt6_to_mu.py:
#
#     - htmlDir/qtapifile point at the Qt 6.8 documentation and its own pickle
#       cache, so the two generators do not overwrite each other's parse.
#     - operator members are emitted using operator syntax ("a == b") instead of
#       an explicit member call ("a.operator==(b)"). Qt 6.8 turned the comparison
#       operators of many value types into hidden friends, which an explicit
#       member call cannot reach. See operatorCallToOperatorSyntax() below.
#
...
#
#   Point this at the Qt 6.8 docs. Override without editing the file by setting
#   RV_QT_DOCS_DIR, e.g. RV_QT_DOCS_DIR=~/Qt/Docs/Qt-6.8.3
#
htmlDir = os.environ.get("RV_QT_DOCS_DIR", "/home/cfuoco/Qt6.8.3/Docs/Qt-6.8.3")
...
#
#   Qt 6.8 replaced the member comparison operators of many value types
#   (QUrl, QByteArray, QDir, QModelIndex, QTimeZone, ...) with hidden friends
#   generated by Q_DECLARE_EQUALITY_COMPARABLE/comparesEqual. A hidden friend is
#   only found by operator syntax or ADL, so the explicit member call this script
#   used to emit:
#
#       return arg0.operator==(arg1);
#
#   no longer compiles on 6.8 ("no member named 'operator==' in 'QUrl'"). Emit
#   operator syntax instead:
#
#       return arg0 == arg1;
#
#   which binds to the member on Qt <= 6.5 and to the hidden friend on 6.8+, so
#   one form serves every supported Qt. Note the free-function form
#   "operator==(arg0, arg1)" would NOT do: it reaches hidden friends but misses
#   the operators that are still plain members.
#
#   Only the operators listed here are rewritten; anything else keeps the old
#   emission so this cannot silently change unrelated output.
#
binaryOperatorNames = set(
    [
        "==",
        "!=",
        "<",
...
unaryOperatorNames = set(["!", "~"])

memberBinaryOpRE = re.compile(r"^arg0(\.|->)operator(.+?)\(arg1\)$")
memberUnaryOpRE = re.compile(r"^arg0(\.|->)operator(.+?)\(\)$")


def operatorCallToOperatorSyntax(expr):
    """Rewrite a generated member-operator call into operator syntax.

    "arg0.operator==(arg1)" -> "arg0 == arg1"
    "arg0->operator==(arg1)" -> "(*arg0) == arg1"
    "arg0.operator!()"      -> "!arg0"

    No outer parentheses are added: every place the result is embedded (a
    function argument, a cast, or a branch of ?:) binds looser than these
    operators, and staying paren-free keeps the output identical to the
    hand-applied fix already in the checked-in sources.

    Anything that is not a recognized member-operator call is returned
    unchanged.
    """
    m = memberBinaryOpRE.match(expr)
    if m:
        lhs = "(*arg0)" if m.group(1) == "->" else "arg0"
        op = m.group(2)
        if op in binaryOperatorNames:
            return "%s %s arg1" % (lhs, op)
        return expr
    m = memberUnaryOpRE.match(expr)
    if m:
        operand = "(*arg0)" if m.group(1) == "->" else "arg0"
        op = m.group(2)
        if op in unaryOperatorNames:
...
class MuFunction:
    ...
    def compiledFunction(self):
        ...
        #
        #   Member operators have to be called with operator syntax so that Qt
        #   6.8's hidden-friend comparison operators are found. Done here, after
        #   the call expression is complete, so every branch below benefits.
        #
        if self.operator:
            expr = operatorCallToOperatorSyntax(expr)
            if altexpr:
                altexpr = operatorCallToOperatorSyntax(altexpr)

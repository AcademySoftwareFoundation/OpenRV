...
{
    if (!_env) return QFile::permissions();
    MuLangContext* c = (MuLangContext*)_env->context();
...
bool qt_QFile_open_bool_QFile_int_int_int(Mu::Thread& NODE_THREAD, Pointer param_this, int param_fd, int param_mode, int param_handleFlags)
{
    MuLangContext* c = static_cast<MuLangContext*>(NODE_THREAD.context());
    QFile* arg0 = object<QFile>(param_this);
...
{
    NODE_RETURN(qt_QFile_QFile_QFile_QFile(NODE_THREAD, NONNIL_NODE_ARG(0, Pointer)));
}
...
{
    NODE_RETURN(qt_QFile_open_bool_QFile_int_int_int(NODE_THREAD, NONNIL_NODE_ARG(0, Pointer), NODE_ARG(1, int), NODE_ARG(2, int), NODE_ARG(3, int)));
}

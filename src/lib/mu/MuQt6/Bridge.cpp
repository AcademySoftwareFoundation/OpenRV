...
void dumpMetaInfo(const QMetaObject& m)
{
    ...
    for (int i=0; i < m.methodCount(); i++)
    {
        ...
        if ((parent &&
             parent->indexOfMethod(parent->normalizedSignature(mm.methodSignature())) != -1) ||
    ...
    for (int i=0; i < m.propertyCount(); i++)
    {
        ...
        if (parent && parent->indexOfProperty(mp.name()) != -1) continue;
    ...
    for (int i=0; i < m.enumeratorCount(); i++)
    {
        ...
        if (parent && parent->indexOfEnumerator(e.name()) != -1) continue;
...
const char*
qtType(const Mu::Type* t)
{
    Name tname = t->fullyQualifiedName();

    if (tname == "string") return "const QString&";
    if (tname == "int") return "int";
    if (tname == "bool") return "bool";
    if (tname == "double") return "qreal";
    if (tname == "qt.QIcon") return "QIcon";
    if (tname == "qt.QSize") return "QSize";
    if (tname == "qt.QRect") return "QRect";
    if (tname == "qt.QPoint") return "QPoint";
    if (tname == "qt.QUrl") return "QUrl";
    if (tname == "qt.QColor") return "QColor";
    if (tname == "qt.QFont") return "QFont";
    if (tname == "qt.QAction") return "QAction*";
    if (tname == "qt.QTreeWidgetItem") return "QTreeWidgetItem*";
    if (tname == "qt.QListWidgetItem") return "QListWidgetItem*";
    if (tname == "qt.QTableWidgetItem") return "QTableWidgetItem*";
    if (tname == "qt.QStandardItem") return "QStandardItem*";
    if (tname == "qt.QModelIndex") return "QModelIndex";
    if (tname == "qt.QKeySequence") return "QKeySequence";
    if (tname == "qt.QPixmap") return "QPixmap";
    if (tname == "qt.QItemSelection") return "QItemSelection";
    return 0;
}

static QMetaProperty
getprop(const Node& NODE_THIS, Thread& NODE_THREAD, QObject* o)
{
...
static QMetaMethod
getmethod(const Node& NODE_THIS, Thread& NODE_THREAD, QObject* o)
{
    ...
    for (size_t i = 1; i < F->numArgs(); i++)
    {
        ...
        if      (tname == "int") { aname = "int"; hasInt = true; }
        else if (tname == "bool") { aname = "bool"; }
        else if (tname == "string") { aname = "QString"; }
        else if (tname == "qt.QIcon") { aname = "QIcon"; }
        else if (tname == "qt.QSize") { aname = "QSize"; }
        else if (tname == "qt.QRect") { aname = "QRect"; }
        else if (tname == "qt.QPoint") { aname = "QPoint"; }
        else if (tname == "qt.QFont") { aname = "QFont"; }
        else if (tname == "qt.QUrl") { aname = "QUrl"; }
        else if (tname == "qt.QColor") { aname = "QColor"; }
        else if (tname == "qt.QAction") { aname = "QAction*"; }
        else if (tname == "qt.QTreeWidgetItem") { aname = "QTreeWidgetItem*"; }
        else if (tname == "qt.QListWidgetItem") { aname = "QListWidgetItem*"; }
        else if (tname == "qt.QTableWidgetItem") { aname = "QTableWidgetItem*"; }
        else if (tname == "qt.QStandardItem") { aname = "QStandardItem*"; }
        else if (tname == "qt.QModelIndex") { aname = "QModelIndex"; }
        else if (tname == "qt.QKeySequence") { aname = "QKeySequence"; }
        else if (tname == "qt.QPixmap") { aname = "QPixmap"; }
        else if (tname == "qt.QItemSelection") { aname = "QItemSelection"; }
        else if (tname == "double") { aname = "qreal"; }
    ...
    if (methodIndex == -1 && hasInt)
    {
        ...
        for (size_t i = 0; i < args.size(); i++)
        {
            ...
            if (!strcmp(args[i], "int")) rstr << "[a-zA-Z0-9_:]+Flags?";
            else rstr << args[i];
        }
...
static
NODE_IMPLEMENTATION(getpropInt, int)
{
...
static
NODE_IMPLEMENTATION(getpropBool, bool)
{
    QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance));
    QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
    QVariant v = prop.read(o);
...
static
NODE_IMPLEMENTATION(getpropDouble, double)
{
    QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance));
    QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
    QVariant v = prop.read(o);
...
static
NODE_IMPLEMENTATION(getpropString, Pointer)
{
...
static
NODE_IMPLEMENTATION(getpropIcon, Pointer)
{
    const MuLangContext* c = static_cast<MuLangContext*>(NODE_THREAD.context());
    const QIconType* iconType =
        c->findSymbolOfTypeByQualifiedName<QIconType>(c->internName("qt.QIcon"), false);
    QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance));
    QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
    QVariant v = prop.read(o);
...
static
NODE_IMPLEMENTATION(getpropSize, Pointer)
{
    const MuLangContext* c = static_cast<MuLangContext*>(NODE_THREAD.context());
    const QSizeType* sizeType =
        c->findSymbolOfTypeByQualifiedName<QSizeType>(c->internName("qt.QSize"), false);
    QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance));
    QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
    QVariant v = prop.read(o);
    ClassInstance* i = 0;
...
static
NODE_IMPLEMENTATION(getpropRect, Pointer)
{
    const MuLangContext* c = static_cast<MuLangContext*>(NODE_THREAD.context());
    const QRectType* rectType =
        c->findSymbolOfTypeByQualifiedName<QRectType>(c->internName("qt.QRect"), false);
    QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance));
    QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
    QVariant v = prop.read(o);
    ClassInstance* i = 0;
...
static
NODE_IMPLEMENTATION(getpropPoint, Pointer)
{
    const MuLangContext* c = static_cast<MuLangContext*>(NODE_THREAD.context());
    const QPointType* pointType =
        c->findSymbolOfTypeByQualifiedName<QPointType>(c->internName("qt.QPoint"), false);
    QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance));
    QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
    QVariant v = prop.read(o);
    ClassInstance* i = 0;
...
static
NODE_IMPLEMENTATION(getpropFont, Pointer)
{
    const MuLangContext* c = static_cast<MuLangContext*>(NODE_THREAD.context());
    const QFontType* type =
        c->findSymbolOfTypeByQualifiedName<QFontType>(c->internName("qt.QFont"), false);
    QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance));
    QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
    QVariant v = prop.read(o);
    ClassInstance* i = 0;
...
static
NODE_IMPLEMENTATION(getpropUrl, Pointer)
{
    const MuLangContext* c = static_cast<MuLangContext*>(NODE_THREAD.context());
    const QUrlType* type =
        c->findSymbolOfTypeByQualifiedName<QUrlType>(c->internName("qt.QUrl"), false);
    QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance));
    QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
    QVariant v = prop.read(o);
    ClassInstance* i = 0;
...
static
NODE_IMPLEMENTATION(getpropKeySeq, Pointer)
{
    const MuLangContext* c = static_cast<MuLangContext*>(NODE_THREAD.context());
    const QKeySequenceType* type =
        c->findSymbolOfTypeByQualifiedName<QKeySequenceType>(c->internName("qt.QKeySequence"), false);
    QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance));
    QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
    QVariant v = prop.read(o);
    ClassInstance* i = 0;
...
static
NODE_IMPLEMENTATION(getpropColor, Pointer)
{
    const MuLangContext* c = static_cast<MuLangContext*>(NODE_THREAD.context());
    const QColorType* type =
        c->findSymbolOfTypeByQualifiedName<QColorType>(c->internName("qt.QColor"), false);
    QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance));
    QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
    QVariant v = prop.read(o);
    ClassInstance* i = 0;
...
static
NODE_IMPLEMENTATION(putpropInt, void)
{
...
static
NODE_IMPLEMENTATION(putpropBool, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(putpropDouble, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(putpropString, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(putpropIcon, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(putpropSize, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(putpropPoint, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(putpropRect, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(putpropFont, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(putpropUrl, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(putpropKeySeq, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(putpropColor, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        QMetaProperty prop = getprop(NODE_THIS, NODE_THREAD, o);
...
static
NODE_IMPLEMENTATION(invokeMethod0, void)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
...
static
NODE_IMPLEMENTATION(invokeMethod0_bool, bool)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        bool rval;
...
static
NODE_IMPLEMENTATION(invokeMethod0_int, int)
{
    if (QObject* o = object<QObject>(NODE_ARG_OBJECT(0, ClassInstance)))
    {
        int rval;
...
static
NODE_IMPLEMENTATION(invokeMethod1, void)
{
    const Function* F = static_cast<const Function*>(NODE_THIS.symbol());
...
static
NODE_IMPLEMENTATION(invokeMethod2, void)
{
    const Function* F = static_cast<const Function*>(NODE_THIS.symbol());
...
static
NODE_IMPLEMENTATION(invokeMethod3, void)
{
    const Function* F = static_cast<const Function*>(NODE_THIS.symbol());
...
static
NODE_IMPLEMENTATION(invokeMethod4, void)
{
    const Function* F = static_cast<const Function*>(NODE_THIS.symbol());
...
static
NODE_IMPLEMENTATION(invokeMethod5, void)
{
    const Function* F = static_cast<const Function*>(NODE_THIS.symbol());
...
void
populate(Class* c, const QMetaObject& m, const char** propExclusions)
{
    bool verbose = false;
    ...
    const QMetaObject* parent = m.superClass();
    QIconType* iconType =
        context->findSymbolOfTypeByQualifiedName<QIconType>(context->internName("qt.QIcon"),
                                                            false);
    QSizeType* sizeType =
        context->findSymbolOfTypeByQualifiedName<QSizeType>(context->internName("qt.QSize"),
                                                            false);

    QPointType* pointType =
        context->findSymbolOfTypeByQualifiedName<QPointType>(context->internName("qt.QPoint"),
                                                            false);

    QRectType* rectType =
        context->findSymbolOfTypeByQualifiedName<QRectType>(context->internName("qt.QRect"),
                                                            false);

    QFontType* fontType =
        context->findSymbolOfTypeByQualifiedName<QFontType>(context->internName("qt.QFont"),
                                                            false);

    QUrlType* urlType =
        context->findSymbolOfTypeByQualifiedName<QUrlType>(context->internName("qt.QUrl"),
                                                            false);

    QColorType* colorType =
        context->findSymbolOfTypeByQualifiedName<QColorType>(context->internName("qt.QColor"),
                                                            false);

    QKeySequenceType* keysequenceType =
        context->findSymbolOfTypeByQualifiedName<QKeySequenceType>(context->internName("qt.QKeySequence"),
                                                            false);
    ...
    for (int i=0; i < m.enumeratorCount(); i++)
    {
        ...
        for (int q = 0; q < e.keyCount(); q++)
        {
            SymbolicConstant* sc = new SymbolicConstant(context,
                                                        e.key(q),
                                                        "int",
                                                        Value(e.keyToValue(e.key(q))));
    ...
    for (int i=0; i < m.propertyCount(); i++)
    {
        ...
        if (propExclusions)
        {
            ...
            {
                if (getname == *e) getexclude = true;
                if (setname == *e) setexclude = true;
            }
        }
    ...
    for (int i=0; i < m.methodCount(); i++)
    {
        ...
        for (size_t i = 0; i < pnames.size(); i++)
        {
            ...
            {
                params.push_back(new ParameterVariable(context,
                                                       n.c_str(),
                                                       context->intType()));
            }
            else if (t == "QString")
        ...
        switch (params.size())
        {
          case 1: 
              ...
              if (isBoolType) func = invokeMethod0_bool;
              else if (isIntType) func = invokeMethod0_int;
              else func = invokeMethod0;
              break;
          case 2: func = invokeMethod1; break;
          case 3: func = invokeMethod2; break;
          case 4: func = invokeMethod3; break;
          case 5: func = invokeMethod4; break;
          case 6: func = invokeMethod5; break;
          default:
              failed = true;
              break;
        }
        ...
        if (!failed)
        {
            ...
            if (isBoolType) rtype = context->boolType();
            else if (isIntType) rtype = context->intType();

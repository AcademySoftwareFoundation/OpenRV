...
FixedArrayType::FixedArrayType(Context* c,
                               const char *name,
                               Class *super,
...
Type::MatchResult
FixedArrayType::match(const Type* t, Bindings& b) const
{
    // if (const FixedArrayType* atype = dynamic_cast<const FixedArrayType*>(t))
    // {
...
void
FixedArrayType::outputValueRecursive(ostream &o, 
...
const Type*
FixedArrayType::fieldType(size_t) const
{
    return _elementType;
}

ValuePointer
FixedArrayType::fieldPointer(Object* o, size_t index) const
{
...
const ValuePointer
FixedArrayType::fieldPointer(const Object* o, size_t index) const
{
...
FixedArrayType::load()
{
    ...
    s->addSymbols( new ReferenceType(c, rn,this),
                   ...
                   new Function(c, tn, FixedArrayType::fixed_construct_aggregate, 
                                Mapped,
                                Args, et, Optional, "?+", 
                                Maximum, fixedSize(),
                                Return, ftn, End),

                   new Function(c, tn, FixedArrayType::fixed_copyconstruct, None,
                                Return, ftn,
                                Args, ftn, End),
    ...
    globalScope()->addSymbols(
                   ...
                   new Function(c, "==", FixedArrayType::fixed_equals, Mapped,
                                Return, "bool", 
                                Args, ftn, ftn, End),
                   ...
                   new Function(c, "print", FixedArrayType::fixed_print, None,
                                Return, "void",
                                Args, ftn, End),
...
NODE_IMPLEMENTATION(FixedArrayType::fixed_copyconstruct, Pointer)
{
    ...
    memcpy(n->data<unsigned char>(), 
           o->data<unsigned char>(),
...
NODE_IMPLEMENTATION(FixedArrayType::fixed_equals, bool)
{
    ...
    if (a && b && a->size() == b->size())
    {
        return !memcmp(a->elementPointer(0), 
                       b->elementPointer(0), 
                       a->size() * a->arrayType()->elementType()->machineRep()->size());
    }

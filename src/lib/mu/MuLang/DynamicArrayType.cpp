...
DynamicArrayType::DynamicArrayType(Context* context,
                                   const char *name,
                                   Class *super,
...
Type::MatchResult
DynamicArrayType::match(const Type* t, Bindings& b) const
{
...
Object*
DynamicArrayType::newObject() const
{
    return new DynamicArray(this, _dimensions);
}

size_t
DynamicArrayType::objectSize() const
{
    return sizeof(DynamicArray);
}

void 
DynamicArrayType::constructInstance(Pointer obj) const
{
    new (obj) DynamicArray(this, 1);
}

void 
DynamicArrayType::copyInstance(Pointer a, Pointer b) const
{
    ...
    memcpy(dst->data<byte>(),
           src->data<byte>(),
           src->size() * src->elementType()->machineRep()->size());
}

void
DynamicArrayType::deleteObject(Object *obj) const
{
    delete static_cast<DynamicArray*>(obj);
}

Value
DynamicArrayType::nodeEval(const Node* n, Thread& thread) const
{
    return Value((*n->func()._PointerFunc)(*n,thread));
}

void
DynamicArrayType::outputValueRecursive(ostream &o,
                                       ...
                                       ValueOutputState& state) const
{
    ...
    if (a)
    {
        ...
        else
        {
            ...
            {
                if (i) o << ", ";
                etype->outputValueRecursive(o, 
                                            ValuePointer(a->elementPointer(i)),
                                            state);
...
const Type*
DynamicArrayType::fieldType(size_t) const
{
    return _elementType;
}

ValuePointer
DynamicArrayType::fieldPointer(Object* o, size_t index) const
{
    DynamicArray* array = reinterpret_cast<DynamicArray*>(o);
    if (index < array->size()) return array->elementPointer(index);
    return 0;
}

const ValuePointer
DynamicArrayType::fieldPointer(const Object* o, size_t index) const
{
...
void
DynamicArrayType::serialize(std::ostream& o,
...
void
DynamicArrayType::deserialize(std::istream& in,
...
DynamicArrayType::load()
{
    ...
    //
    //	If a new MachineRep is added, this section of code must be updated.
    //
    ...
    s->addSymbols( new ReferenceType(c, rn, this),
                   ...
                   new Function(c, tn, DynamicArrayType::dyn_construct_aggregate, 
                                Mapped,
                                Args, et, Optional, "?+", Maximum, 999999,
                                Return, ftn, End),

                   new Function(c, tn, DynamicArrayType::dyn_copyconstruct, None,
                                Return, ftn,
                                Args, ftn, End),
    ...
    globalScope()->addSymbols(
                   ...
                   new Function(c, "==", DynamicArrayType::dyn_equals, Mapped,
                                Return, "bool", 
                                Args, ftn, ftn, End),
		       
                   new Function(c, "print", DynamicArrayType::dyn_print, None,
                                Return, "void",
                                Args, ftn, End),
    ...
    if (_dimensions > 1)
    {
        ...
        addSymbol( new Function(c, "[]", 
                                (Type*)elementType()->referenceType(),
                                parameters.size(),
                                &parameters.front(),
                                DynamicArrayType::dyn_indexN, 
                                Mapped & Function::Native &
                                Function::MemberOperator) );
            
        addSymbol( new Function(c, "resize", voidType,
                                parameters.size(),
                                &parameters.front(),
                                DynamicArrayType::dyn_resizeN, 
                                Function::Native) );
    }
    ...
    if (_dimensions == 1)
    {
        ...
        addSymbols( new Function(c, "front", DynamicArrayType::front, None,
                    ...
                    new Function(c, "[]", DynamicArrayType::dyn_index1, 
                                 Mapped & Function::MemberOperator,
                                 Return, re,
                                 Args, ftn, "int", End),
                        
                    new Function(c, "resize", 
                                 DynamicArrayType::dyn_resize1, None,
                                 Return, "void",
                                 Args, ftn, "int", End),
    ...
    addSymbols( new Function(c, "clear", DynamicArrayType::clear, None,
                ...
                new Function(c, "push_back", pushBackFunc, Retaining,
                             Return, fet,
                             Args, ftn, fet, End),

                new Function(c, "pop_back", popBackFunc, None,
                             Return, fet,
                             Args, ftn, End),

                new Function(c, "erase", eraseFunc, None,
                             Return, "void",
...
NODE_IMPLEMENTATION(DynamicArrayType::dyn_rest, Pointer)
{
    ...
    if (!in) throw NilArgumentException(NODE_THREAD);
    DynamicArray* o = new DynamicArray(c, c->_dimensions);
...
NODE_IMPLEMENTATION(DynamicArrayType::dyn_equals, bool)
{
    ...
    if (a && b)
    {
        if (a->size() == b->size())
        {
            return !memcmp(a->elementPointer(0), 
                           b->elementPointer(0), 
                           a->size() * a->elementType()->machineRep()->size());
        }
    }

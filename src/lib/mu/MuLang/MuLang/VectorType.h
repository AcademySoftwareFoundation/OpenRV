...
template <class T>
class VectorType : public PrimitiveType
{
  public:
    MU_GC_NEW_DELETE
...
template <>
inline float
VectorType<Vector4f>::elementArg(const Node &n, Thread &t, int i) 
{ 
...
template <>
inline float
VectorType<Vector3f>::elementArg(const Node &n, Thread &t, int i) 
{ 
    return n.floatArg(i,t);
}
...
template <>
inline float
VectorType<Vector2f>::elementArg(const Node &n, Thread &t, int i) 
{ 
    return n.floatArg(i,t);
}
...
template <>
inline Vector4f
VectorType<Vector4f>::vectorArg(const Node &n, Thread &t, int i) 
{ 
...
template <>
inline Vector3f
VectorType<Vector3f>::vectorArg(const Node &n, Thread &t, int i)
{
...
template <>
inline Vector2f
VectorType<Vector2f>::vectorArg(const Node &n, Thread &t, int i)
{
...
template<>
inline Vector4f
VectorType<Vector4f>::nodeData(const Node &n)
{
...
template<>
inline Vector3f
VectorType<Vector3f>::nodeData(const Node &n)
{
    const DataNode &dn = static_cast<const DataNode&>(n);
...
template<>
inline Vector2f
VectorType<Vector2f>::nodeData(const Node &n)
{
    const DataNode &dn = static_cast<const DataNode&>(n);
...
template <class T>
Value
VectorType<T>::nodeEval(const Node *n, Thread &thread) const
{
...
template <class T>
void
VectorType<T>::nodeEval(void* p, const Node *n, Thread &thread) const
{
...
template <>
void
VectorType<Vector4f>::nodeEval(void* p, const Node *n, Thread &thread) const
{
...
template <>
void
VectorType<Vector3f>::nodeEval(void* p, const Node *n, Thread &thread) const
{
...
template <>
void
VectorType<Vector2f>::nodeEval(void* p, const Node *n, Thread &thread) const
{
...
template <>
Value
VectorType<Vector4f>::nodeEval(const Node *n, Thread &thread) const
{
    return Value((*n->func()._Vector4fFunc)(*n,thread));
}
...
template <>
Value
VectorType<Vector3f>::nodeEval(const Node *n, Thread &thread) const
{
...
template <>
Value
VectorType<Vector2f>::nodeEval(const Node *n, Thread &thread) const
{
...
template <class T>
void
VectorType<T>::outputValue(std::ostream &o, const Value &value, bool full) const
{
    // nothing
}
...
template <class T>
void
VectorType<T>::outputValueRecursive(std::ostream &o, 
...
template <>
void
VectorType<Vector4f>::outputValue(std::ostream &o, const Value &value, bool full) const
{
    o << value._Vector4f;
}
...
template <>
void
VectorType<Vector3f>::outputValue(std::ostream &o, const Value &value, bool full) const
{
    o << value._Vector3f;
}
...
template <>
void
VectorType<Vector2f>::outputValue(std::ostream &o, const Value &value, bool full) const
{
    o << value._Vector2f;
}
...
VectorType<T>::load()
{
	...
	if (i == 2)
	{
	    s->addSymbols(new Function(c, tn, VectorType<T>::construct3, Mapped,
				       Return, tn,
				       Args, en, en, en,
				       End),
	...
	if (i == 3)
	{
	    s->addSymbol( new Function(c, tn, VectorType<T>::construct4, Mapped,
				       Return, tn,
				       Args, en, en, en, en,
				       End) );
	}
    ...
    s->addSymbols( new ReferenceType(c, rn, this),
		   ...
		   new Function(c, "+", VectorType<T>::add, CommOp,
				Return, tn,
				Args, tn, tn, End),

		   new Function(c, "-", VectorType<T>::sub, Op,
				Return, tn,
				Args, tn, tn, End),

		   new Function(c, "-", VectorType<T>::negate, Op,
				Return, tn,
				Args, tn, End),

		   new Function(c, "*", VectorType<T>::mult, CommOp,
				Return, tn,
				Args, tn, tn, End),

		   new Function(c, "/", VectorType<T>::div, Op,
				Return, tn,
				Args, tn, tn, End),
		   ...
		   new Function(c, "=", VectorType<T>::assign, AsOp,
				Return, rn,
				Args, rn, tn, End),
		   ...
		   new Function(c, "dot", VectorType<T>::dot, Mapped,
				Return, en, 
				Args, tn, tn, End),

		   new Function(c, "mag", VectorType<T>::mag, Mapped,
				Return, en, 
				Args, tn, End),

		   new Function(c, "normalize", VectorType<T>::normalize, Mapped,
				Return, tn, 
				Args, tn, End),

		   EndArguments);

    addSymbols( new Function(c, "[]", VectorType<T>::indexop, Mapped,
                             Return, en,
                             Args, tn, "int",
                             End),
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::dereference,T)
{
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::defaultVector,T)
{
    T vp;
    assignOp(vp, 0.0f);
    NODE_RETURN(vp);
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::construct4,T)
{
    T v;
    v[0] = NODE_EARG(0);
    v[1] = NODE_EARG(1);
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::construct3,T)
{
    T v;
    v[0] = NODE_EARG(0);
    v[1] = NODE_EARG(1);
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::construct2,T)
{
    T v;
    v[0] = NODE_EARG(0);
    v[1] = NODE_EARG(1);
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::construct1,T)
{
    T v;
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::print,void)
{
    T v = NODE_VARG(0);
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::negate,T)
{
    NODE_RETURN(-NODE_VARG(0));
}

template <class T>
NODE_IMPLEMENTATION(VectorType<T>::cross, T)
{
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::indexopr, Pointer)
{
    int index = NODE_ARG(1, int);
    if (index < 0 || index > T::dimension()) 
        throw OutOfRangeException(NODE_THREAD);
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::normalize, T)
{
    NODE_RETURN( Mu::normalize(NODE_VARG(0)) );
}

template <class T>
NODE_IMPLEMENTATION(VectorType<T>::conditionalExpr,T)
{
    NODE_RETURN( NODE_ARG(0,bool) ? NODE_VARG(1) : NODE_VARG(2) );
}

template <class T>
NODE_IMPLEMENTATION(VectorType<T>::__assign,void)
{
...
template <class T>
NODE_IMPLEMENTATION(VectorType<T>::assign,Pointer)
{
    T* vp = reinterpret_cast<T*>(NODE_ARG(0,Pointer));
    *vp = NODE_VARG(1);
    NODE_RETURN(vp);

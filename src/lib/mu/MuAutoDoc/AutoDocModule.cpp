...
AutoDocModule::load()
{
    ...
    addSymbols( new Function(c, "document_symbol",
                             AutoDocModule::document_symbol, None,
                             Return, "string",
                             Parameters,
                             new Param(c, "symbol", "string"),
                             new Param(c, "format", "int", Value(0)),
                             new Param(c, "indent_level", "int", Value(0)),
                             End),

                //
                // NOTE: document_modules probably belongs in 'runtime'.
                //
                new Function(c, "document_modules",
                             AutoDocModule::document_modules, None,
                             Return, "string[]",
                             End ),
...
static String
textDocSymbol(Process* p, Symbol *s, int indent, bool overloaded=true)
{
    ostringstream str;
    ...
    {
        if (!f) f = dynamic_cast<Function*>(q);
        if (!t) t = dynamic_cast<Type*>(q);
        if (!a) a = dynamic_cast<Alias*>(q);
        if (!m) m = dynamic_cast<Module*>(q);
        if (!c) c = dynamic_cast<Class*>(q);
        if (!i) i = dynamic_cast<Interface*>(q);
        if (!v) v = dynamic_cast<Variable*>(q);
        if (!sc) sc = dynamic_cast<SymbolicConstant*>(q);
        n++;
        if (!overloaded) break;
    }
...
static String
htmlDocSymbol(Symbol *s, bool overloaded=true)
{
    ostringstream str;
...
static void
texiDoc(ostream& o, Function *f)
{
    o << "@deftypefn {Function} {} "
      << f->name()
      << " ("
    ...
    for (int i=0; i < f->numArgs(); i++)
    {
        ...
        if (f->hasParameters())
        {
            ...
            o << p->storageClass()->fullyQualifiedName()
              << " @var{"
              << p->name()
              << "}";
...
static String
texiDocSymbol(Symbol *s, bool overloaded=true)
{
    ostringstream str;

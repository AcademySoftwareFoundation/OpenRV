...
Vector4f
ImageType::ImageStruct::sample(float x, float y)
{
...
static void
readTIFF(ImageType::ImageStruct* im, const Mu::String& imgFileName, Thread& thread)
{
    ...
    DynamicArrayType* dataType = 
        static_cast<DynamicArrayType*>(context->arrayType(context->vec4fType(), 1, 0));
    ...
    switch (bps)
    {
        case 32:
            for (int y = 0; y < height; y++ )
            {
                ...
                if (flop) reverse(im->row(oy), im->row(oy) + width);
            }
            break;
...
ImageType::ImageType(Context* c, Class *super) : Class(c, "image", super)
{
}
...
ImageType::load()
{
    ...
    addSymbols( new MemberVariable(c, "name", "string"), 
		...
		new Function(c, "()", ImageType::sample, Mapped,
                             Return, "vector float[4]", 
                             Args, tn, "float", "float", End),
...
NODE_IMPLEMENTATION(ImageType::sample, Vector4f)
{
    ...
    if (!o) throw NilArgumentException();

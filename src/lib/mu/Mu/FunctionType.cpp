...
void
FunctionType::deleteObject(Object* o) const
{
...
void 
FunctionType::nodeEval(void *p, const Node* n, Thread& thread) const
{
...
void
FunctionType::outputNode(std::ostream&, const Node*) const
{
}

void
FunctionType::outputValueRecursive(std::ostream& o,
...
void
FunctionType::serialize(std::ostream& o, 
                        ...
                        const ValuePointer p) const
{
    ...
    if (obj->function()->native()) flags |= 0x1;
    if (obj->dependent()) flags |= 0x2;
    archive.writeByte(o, flags);
    archive.writeNameId(o, obj->function()->fullyQualifiedName());
}

void
FunctionType::deserialize(std::istream& i,
...
void
FunctionType::reconstitute(Archive::Reader& archive, Object* o) const
{
...
FunctionType::load()
{
    ...
    if (signature() && signature()->size())
    {
        ...
        {
            const Type* t = 
                static_cast<const Type*>(signature()->types().front().symbol);
...
//     }
    ...
    s->addSymbols( new ReferenceType(c, rn, this), 
		   ...
		   new Function(c, "=", FunctionType::assign, AsOp,
				Return, rn, 
				Args, rn, tn, End),
...
NODE_IMPLEMENTATION(FunctionType::disambiguate, Pointer)
{
    ...
    if (o)
    {
        ...
        {
            for (const Function* f = func->firstFunctionOverload(); 
                 f; 
...
NODE_IMPLEMENTATION(FunctionType::call, void)
{
    cout << "CALL" << endl;
}
...
Type::MatchResult
FunctionType::match(const Type* other, Bindings& b) const
{
    if (const FunctionType* ft = dynamic_cast<const FunctionType*>(other))
    {
        ...
        if (a->size() != b->size()) return NoMatch;
        ...
        for (int i=0; i < a->size(); i++)
        {
            ...
            if (i)
            {
                ...
                if (!btype->match(atype)) return NoMatch;
            }
            else
            {
                //
                ...
                if (!atype->match(btype)) return NoMatch;
            }
        }

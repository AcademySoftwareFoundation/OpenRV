...
FunctionSpecializer::FunctionSpecializer(const Function* f,
                                         Process* p,
                                         Thread* t)
    : _as(p->context(), p, t), 
      _root(0), 
...
void
FunctionSpecializer::doit(const char* name, SymbolList newParams, bool lambda)
{
    ...
    if (newParams.empty())
    {
        _lambda = new Function(_as.context(),
                               name,
                               translate(_f->returnType()),
                               0, 0, 0,
                               _f->baseAttributes() |
    ...
    else
    {
        _lambda = new Function(_as.context(),
                               ...
                               translate(_f->returnType()),
                               newParams.size(),
                               (ParameterVariable**)&(newParams.front()),
                               0,
                               _f->baseAttributes() |
                               Function::ContextDependent|
                               Function::LambdaExpression);
    ...
    if (!_root) throw InconsistantSignatureException();
...
Function*
FunctionSpecializer::partiallyEvaluate(const ArgumentVector& args,
                                       const ArgumentMask& mask)
{
    ...
    for (int i=0; i < _f->numArgs() + _f->numFreeVariables(); i++)
    {
        ...
        if (!_mask[i])
        {
            Param nv = new ParameterVariable(_as.context(),
                                             p->name().c_str(),
                                             p->storageClass());
...
Function*
FunctionSpecializer::specialize(const TypeBindings& bindings)
{
...
Node*
FunctionSpecializer::callDirectly()
{
...
void
FunctionSpecializer::declareVariables()
{
    char temp[80];
    ...
    for (int i=0; i < _variables.size(); i++)
    {
	    ...
	    Variable* nv = _as.declareStackVariable(t,
                                                    _as.context()->internName(temp),
                                                    Variable::ReadWrite);
            _variableMap[v] = nv;
        }
	    ...
	    Variable* nv = new GlobalVariable(_as.context(),
                                              temp,
                                              t,
                                              _as.process()->globals().size(),
                                              Variable::ReadWrite,
                                              0);
...
void
FunctionSpecializer::accumulateVariables(const Symbol* sym)
{
    if (!sym->symbolTable()) return;
    ...
    for (SymbolHashTable::Iterator i(ht); i; ++i)
    {
        ...
        for (Symbol* s = is->firstOverload(); s; s = s->nextOverload())
        {
                ...
                dynamic_cast<const ParameterVariable*>(s))
            {
                ...
                if (!_mask.empty() && _mask[index]) continue;
            }
...
Node*
FunctionSpecializer::translate(const Node* n)
{
    ...
    else if (dynamic_cast<const UnresolvedCall*>(sym))
    {
        ...
        if (uname == "[]")
        {
            ...
            Node* rn = _as.memberOperator("[]",
                                         translate(n->argNode(0)),
                                         nl);
            _as.removeNodeList(nl);
        ...
        if (uname == "=")
        {
            ...
            if (lhs->type() == _as.context()->unresolvedType() &&
                lhs->symbol() == _as.context()->unresolvedStackReference())
            {
                    ...
                    if (nv->storageClass() == _as.context()->unresolvedType())
                    {
                        ...
                        if (rhs->type()->isReferenceType())
                        {
                            const ReferenceType* rt = 
                                static_cast<const ReferenceType*>(rhs->type());
    ...
    if (const Variable* v = dynamic_cast<const Variable*>(sym))
    {
        ...
        if (sv || gv)
        {
            ...
            if (n->func() != nn->func()) nn = _as.dereferenceLValue(nn);
            return nn;
        }

...
bool
VariantTagType::resolveSymbols() const
{
...
Type::MatchResult
VariantTagType::match(const Type *type, Bindings& b) const
{
...
Object*
VariantTagType::newObject() const
{
    if (symbolState() != ResolvedState) resolve();
...
size_t
VariantTagType::objectSize() const
{
    if (symbolState() != ResolvedState) resolve();
...
void
VariantTagType::constructInstance(Pointer p) const
{
    new (p) VariantInstance(this);
}

void
VariantTagType::copyInstance(Pointer a, Pointer b) const
{
...
const Type*
VariantTagType::nodeReturnType(const Node*) const
{
    if (symbolState() != ResolvedState) resolve();
    return variantType();
}
...
const Type*
VariantTagType::fieldType(size_t t) const
{
    if (t > 0) return 0;
    if (symbolState() != ResolvedState) resolve();
...
ValuePointer
VariantTagType::fieldPointer(Object* o, size_t index) const
{
    if (index > 0) return 0;
...
const ValuePointer
VariantTagType::fieldPointer(const Object* o, size_t index) const
{
    if (index > 0) return 0;
...
void 
VariantTagType::outputValueRecursive(ostream &o,
        ...
        else
        {
            ...
            if (representationType() != globalModule()->context()->voidType())
            {
                ...
                if (const Class* c = dynamic_cast<const Class*>(representationType()))
                {
                    ...
                    representationType()->outputValueRecursive(o, 
                                                               ValuePointer(&vobj),
                                                               state);
                }
                else
...
void 
VariantTagType::serialize(ostream& o, Archive::Writer& writer, const ValuePointer p) const
{
...
void                
VariantTagType::deserialize(istream& in, Archive::Reader& reader, ValuePointer p) const
{
    VariantInstance* i = *(VariantInstance**)p;
    Pointer pp = i->structure();
    const Type* rtype = representationType();
...
void                
VariantTagType::reconstitute(Archive::Reader& reader, Object* obj) const
{
...
VariantTagType::load()
{
    ...
    if (reptype != context->voidType())
    {
        ...
        Function* OpAs = new Function(context,
                                      ...
                                      Return, rt->fullyQualifiedName().c_str(),
                                      Args, rtf, ftn,
                                      End);
    
        g->addSymbol(OpAs);

        Function* FT = new Function(context,
                                    tn,
                                    reptype->machineRep()->variantConstructorFunc(),
                                    Mapped,
                                    Return, v->fullyQualifiedName().c_str(),
                                    Args, reptype->fullyQualifiedName().c_str(),
                                    End);
    ...
    {
        addSymbol(new Function(context,
                               tn,
                               reptype->machineRep()->variantConstructorFunc(),
                               Mapped,
                               Return, v->fullyQualifiedName().c_str(),
    ...
    s->addSymbols( new Function(context,
                                tn, VariantTagType::upcast, Cast,
                                Return, ftn,
                                Args, fsn,
                                End),
    ...
    addSymbols ( new Function(context,
                              "__unpack", reptype->machineRep()->unpackVariant(), Cast,
                              Return, tt,
                              Args, ftn,
                              End),
...
const Type* 
VariantTagType::representationType() const
{ 

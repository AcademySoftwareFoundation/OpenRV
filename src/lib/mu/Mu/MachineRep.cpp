...
FloatRep::FloatRep() : MachineRep("float", "f")
{
    assert(_rep==0);
    _rep 		  	= this;
    _size		  	= sizeof(float);
...
{									\
    ...
    argv[0] = &dn;                                                      \
    argv[nargs] = 0;                                                    \
    for (size_t i = 1; i < nargs; i++) argv[i] = NODE_THIS.argNode(i);  \
    Node n((Node**)argv, F);                                            \
                                                                        \
    try                                                                 \
    {                                                                   \
...
{                                                                       \
    ...
    int n = NODE_NUM_ARGS() - 1;                                        \
                                                                        \
    for (int i=0; i < n; i++) NODE_ANY_TYPE_ARG(i);                     \
...
{                                                                       \
    const Function *f =                                                 \
	static_cast<const Function*>(NODE_THIS.symbol());               \
    int n = NODE_NUM_ARGS();                                            \
    int s = f->stackSize();                                             \
    Thread::StackRecord record(NODE_THREAD);                            \
	...
	if (i < n)                                                      \
	{                                                               \
	    val = NODE_EVAL_VALUE(NODE_THIS.argNode(i),                 \
                                  NODE_THREAD);                         \
	}                                                               \
	else                                                            \
	{                                                               \
	...
	{                                                               \
        ...
        {                                                               \
            return CLASS::functionActivationFunc(*NODE_THREAD.continuation(), \
                                                 NODE_THREAD);          \
        }                                                               \
	else                                                            \
	{                                                               \
...
{                                                                       \
	...
	static_cast<const Function*>(NODE_THIS.symbol());               \
    const Interface *i =                                                \
        static_cast<const Interface*>(f->scope());                      \
    ClassInstance* o =                                                  \
	reinterpret_cast<ClassInstance*>(NODE_ARG(0,Pointer));          \
                                                                        \
    if (const InterfaceImp* imp =                                       \
        o->classType()->implementation(i))                              \
    {                                                                   \
...
DoubleRep::DoubleRep() : MachineRep("double", "F")
{
    assert(_rep==0);
    _rep 		  	= this;
    _size		  	= sizeof(double);
...
IntRep::IntRep() : MachineRep("int", "i")
{
    assert(_rep==0);
    _rep 			= this;
    _size		  	= sizeof(int);
...
Int64Rep::Int64Rep() : MachineRep("int64", "L")
{
    assert(_rep==0);
    _rep 			= this;
    _size		  	= sizeof(int64);
...
ShortRep::ShortRep() : MachineRep("short", "s")
{
    assert(_rep==0);
    _rep 			= this;
    _size		  	= sizeof(short);
...
CharRep::CharRep() : MachineRep("char", "c")
{
    assert(_rep==0);
    _rep 			= this;
    _size		  	= sizeof(char);
...
BoolRep::BoolRep() : MachineRep("bool", "b")
{
    assert(_rep==0);
    _rep 			= this;
    _size		  	= sizeof(bool);
...
PointerRep::PointerRep() : MachineRep("Pointer", "p")
{
    assert(_rep==0);
    _rep 			= this;
    _size		  	= sizeof(void*);
...
Vector4FloatRep::Vector4FloatRep() : MachineRep("Vector4f", "4f",
						FloatRep::rep(), 4)
{
    assert(_rep==0);
    _rep 			= this;
...
Vector3FloatRep::Vector3FloatRep() : MachineRep("Vector3f", "3f",
						FloatRep::rep(), 3)
{
    assert(_rep==0);
    _rep 			= this;
...
Vector2FloatRep::Vector2FloatRep() : MachineRep("Vector2f", "2f",
						FloatRep::rep(), 2)
{
    assert(_rep==0);
    _rep 			= this;
...
VoidRep::VoidRep() : MachineRep("void", "v")
{
    assert(_rep==0);
    _size 	      	    = 0;
    _structAlignment  	    = 0;
          ...
          case JumpReturnCode::TailFuse:
              {
                  ...
                  return VoidRep::functionActivationFunc(*NODE_THREAD.continuation(),
                                                         NODE_THREAD);
              }
          case JumpReturnCode::NoJump:

      ...
      _nebulousAncestry(false)
{
    ...
    {
        if (supers[i]) addSuperClass(supers[i]);
    }
...
void 
Class::addSuperClass(const Class* s)
{
...
void
Class::findSymbols(QualifiedName name, SymbolVector& symbols)
{
...
void
Class::findSymbols(QualifiedName name, ConstSymbolVector& symbols) const
{
    Symbol::findSymbols(name, symbols);
...
void
Class::deleteObject(Object *obj) const
{
...
static bool
memberMatch(const Signature* a, const Signature* b)
{
...
void 
Class::outputValueRecursive(ostream &o, 
...
void
Class::symbolDependancies(ConstSymbolVector& symbols) const
{
...
const Type*
Class::fieldType(size_t t) const
{
    if (t >= _memberVariables.size()) return 0;
    return _memberVariables[t]->storageClass();
}

ValuePointer
Class::fieldPointer(Object* o, size_t index) const
{
    if (index >= _memberVariables.size()) return 0;
    ClassInstance* i = static_cast<ClassInstance*>(o);
    return i->field(index);
}

const ValuePointer
Class::fieldPointer(const Object* o, size_t index) const
{
    if (index >= _memberVariables.size()) return 0;
...
const Function*
Class::findFunction(Name name, const Signature* sig) const
{
...
Class::isA(const Class *c) const
{
    ...
    else
    {
        ...
        for (size_t i = 0, s = _superClasses.size(); i < s; i++)
        {
            if (_superClasses[i]->isA(c)) return true;
        }
    }
...
bool 
Class::substitutable(const Class* c) const
{
    if (this == c) 
    {
        return true;
...
ClassInstance*
Class::dynamicCast(ClassInstance* obj, const Class* c, bool upcastOK) const
{
    if (_nebulousAncestry)
    {
        return obj;
    ...
    {
        if (c == this) return obj;

        for (size_t i = 0, s = _superClasses.size(); i < s; i++)
        {
            const Class* super = _superClasses[i];
            ClassInstance* sobj = i ? (reinterpret_cast<ClassInstance*>
                                       (obj->structure() + _superOffsets[i])) : obj;
            ...
            if (dynamic_cast<const Class*>(base->type()))
            {
                ...
                for (size_t i = 1; i < offsets.size(); i++)
                {
                    if (off < offsets[i])
                    {
                        ...
                        {
                            ClassInstance* sobj = 
                                reinterpret_cast<ClassInstance*>(bobj->structure() + offsets[i-1]);
                    ...
                    {
                        ClassInstance* sobj = 
                            reinterpret_cast<ClassInstance*>(bobj->structure() + offsets[i]);
...
bool
Class::nebulousIsA(const Class*) const
{
    cerr << "IMPLEMENTATION ERROR: Class " << name() 
         << " did not implement Class::nebulousIsA()" 
         << endl;
    abort();
...
Type::MatchResult
Class::match(const Type *type, Bindings& b) const
{
...
Class::dynamicLookup(const MemberFunction* F) const
{
    //
    ...
    if (const Symbol* sym = findSymbol(F->name()))
    {
        for (const Symbol* s = sym; s; s = s->nextOverload())
        {
            ...
            {
                if (memberMatch(f->signature(), F->signature())) return f;
            }
        }
    }
...
Class::freeze()
{
    ...
    for (size_t i = 0, s = _superClasses.size(); i < s; i++)
    {
        ...
        if (super && !super->isFrozen()) super->freeze();
    }
    ...
    if (!_superClasses.empty()) _superOffsets.front() = 0;
    ...
    for (size_t i = 0; i < _superClasses.size(); i++)
    {
        ...
        for (size_t q = 0; q < size; q++)
        {
                ...
                dynamic_cast<InternalTypeMemberVariable*>(m0))
            {
                ...
                InternalTypeMemberVariable* im = 
                    new InternalTypeMemberVariable(c, 
                                                   m0->name().c_str(), 
                                                   tm->value());
                m = im;
                _typeMembers.push_back(im);
...
const InterfaceImp*
Class::implementation(const Interface* intface) const
{
    //
...
void
Class::findOverridingFunctions(const MemberFunction* mf,
...
void
Class::constructInstance(Pointer p) const
{
    new (p) ClassInstance(this);
}

void
Class::copyInstance(Pointer a, Pointer b) const
{
    memcpy(b, a, objectSize());
}

bool 
Class::isInBaseClass(const MemberVariable* v) const
{
    for (size_t i = 0; i < _superClasses.size(); i++)
    {
        const Class* super = _superClasses[i];
        if (super->findSymbolOfType<MemberVariable>(v->name())) return true;
        else if (super->isInBaseClass(v)) return true;
    }
...
void 
Class::allMemberVariables(MemberVariableVector& vars) const
{
    // if (_superClasses.empty())
    // {
        copy(_memberVariables.begin(),

...
class NodeAssembler
{
  public:
    ...
    {
        MU_GC_NEW_DELETE
	Initializer() : name(), node(0) {}
	Initializer(Name n,Node *nn) : name(n), node(nn) {}
	Name	name;
	Node*	node;
    };
	...
	ScopeState(Symbol *s, const ScopeState* p, bool d=true) 
            : symbol(s), parent(p), declaration(d) {}
    ...
    struct Pattern
    {
        MU_GC_NEW_DELETE
        Pattern(Name n) 
            : children(0), name(n), next(0), 
              typePattern(0), constructor(0),
              expression(0) {}
        Pattern(Node* n) 
            : children(0), name(), next(0), 
              typePattern(0), constructor(0),
              expression(n) {}
        explicit Pattern(Pattern* list, const TypePattern* tp)
            : children(list), name(), next(0), 
              typePattern(tp), constructor(0),
              expression(0) {}
    ...
    Function*           declareFunction(const char* name,
    ...
    bool                checkRedeclaration(const char* name,
                                           const Type* returnType,
    ...
    //	becomes the object in which dynamic look up of the method function
    //	occurs. The second version requires self be the first argument.
    //
    ...
    Node*               dynamicPartialEvalOrApply(Node* self,
                                                  NodeList arguments,
                                                  bool reduce=false,
    ...
    const
    InitializerList&	initializers() const { return _initializerList; }
    ...
    Node*		declareVariables(const Type*,
                                         const char *asop, VariableType);
    ...
    //
    //	endStackFrame() (no arguments) will finish the stack frame. Its up
    //	to the caller to deal with the proper node type.
    //
    ...
    Node*               functionConstant(const Function*, 
                                         bool literal=false);
...
template<class T>
T* NodeAssembler::findScopeOfType() const
{
...
template<class T>
T* NodeAssembler::findTypeInScope(Name name) const
{
    //
...
template<class T>
bool
NodeAssembler::findTypesInScope(Name name, 
                                typename STLVector<const T*>::Type& array) const
{
    array.clear();

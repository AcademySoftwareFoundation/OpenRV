...
class Context
{
  public:
    ...
    {
        ScoredFunction(const Function* f, 
                       const TypeBindings& b,
                       const Scores& s)
            : func(f), bindings(b), scores(s), permutation(), totalScore(-1) 
            { computeTotalScore(); }
        ScoredFunction(const Function* f, 
                       const TypeBindings& b,
                       const Scores& s,
                       const Permutation& pt)
            : func(f), bindings(b), scores(s), permutation(pt), totalScore(-1) 
            { computeTotalScore(); }
    ...
    struct SourceRecord
    {
        SourceRecord() : filename(), linenum(0), charnum(0) {}
        SourceRecord(Name n, unsigned short l, unsigned short c)
            : filename(n), linenum(l), charnum(c) {}
    ...
    struct PushInputStream
    {
        PushInputStream(Context* c, std::istream& i)
            : context(c), current(&c->inputStream()) { context->setInput(i); }
    ...
    struct PrimaryBit
    {
        PrimaryBit(Context* c, bool val) 
            : _context(c), _previous(c->primaryState())
    ...
    void popSourceRecordStack() { _sourceRecord = _sourceRecordStack.back(); _sourceRecordStack.pop_back(); }
    ...
    template<class T>
    T*                      findSymbolOfType(Name);

    template<class T>
    const T*                findSymbolOfType(Name) const;

    template<class T>
    T*                      findSymbolOfTypeByQualifiedName(Name,
    ...
    StructType* structType(Symbol* root,
                           const char* name, 
    ...
    virtual 
    const Function* matchFunction(const FunctionVector&,
                                  const TypeVector& args,
                                  TypeBindings&,
                                  MatchType& matchType) const;

    virtual 
    const Function* matchSpecializedFunction(Process*,
                                             Thread*,
    ...
    const Function* specializeFunction(Process*,
                                       Thread*,
                                       const TypeBindings&,
                                       const Function*,
                                       const TypeVector&);
    ...
    const Symbol*       unresolvedCall() const
                        { return _unresolvedCall; }

    const Symbol*       unresolvedCast() const
                        { return _unresolvedCast; }
  ...
  private:
    bool                scoreArgs(Scores& scores, 
                                  TypeBindings& bindings,
                                  const Function* F, 
                                  const TypeVector& args) const;

    int                 score(const Function* F,
                              TypeBindings& bindings,
                              const Type* ftype,
...
template<class T>
T* Context::findSymbolOfType(Name name)
{
...
template<class T>
const T* Context::findSymbolOfType(Name name) const
{
...
template<class T>
T* Context::findSymbolOfTypeByQualifiedName(QualifiedName name, bool restricted)
{
...
template<class T>
Context::SymbolVector
Context::findSymbolsOfType(QualifiedName name)
{
    SymbolVector s;
    findSymbols(name, s);

...
static bool
CompareSymbolType(const Symbol* a, const Symbol* b)
{
    const Module* ma = dynamic_cast<const Module*>(a);
    const Module* mb = dynamic_cast<const Module*>(b);

    if (ma && !mb) return true;
    else if (mb && !ma) return false;
    else if (ma && mb) return a->fullyQualifiedName() < b->fullyQualifiedName();
    ...
    else if (na && nb) return a->fullyQualifiedName() < b->fullyQualifiedName();
    ...
    {
        if (ca->isA(cb)) return false;
        else if (cb->isA(ca)) return true;
        else return ca->fullyQualifiedName() < cb->fullyQualifiedName();
    }
...
void
Writer::NodeSymbolCollector::preOrderVisit(Node* n, int depth)
{
...
void
Writer::internAnnotation(const Node* n)
{
...
void 
Writer::collectSymbolsFromFile(Name filename, SymbolVector& array, bool userOnly) const
{
    const Context::SymbolDefinitionMap& symdefs = _context->symbolDefinitions();

    for (Context::SymbolDefinitionMap::const_iterator i = symdefs.begin();
         i != symdefs.end();
         ++i)
    {
...
void
Writer::collectPrimarySymbols(const Symbol *s, SymbolVector& array) const
{
    if (s->isPrimary()) 
    {
        bool ok = true;
...
void
Writer::collectAllPrimarySymbols(SymbolVector& array) const
{
    collectPrimarySymbols(_context->globalScope(), array);
}

static void
addNameToVector(Names& names, const Symbol *s, bool fully=false)
{
    Name n = s->fullyQualifiedName();
    if (names.find(n) == names.end()) names.insert(n);
...
void
Writer::collectNames(const SymbolVector& symbols, Names& names) const
{
...
void 
Writer::internName(Name n) 
{ 
    _nameMap[n.c_str()] = -1; 
}

void 
Writer::internModule(const Module* t) 
{ 
    _moduleMap[t] = -1;
    internNames(t); 
}
...
void 
Writer::internFunction(const Function* f) 
{ 
    if (f->isGenerated()) return;

    if (_functionMap.count(f) > 0) return;
...
void 
Writer::internNames(const Symbol* s) 
{ 
    internName(s->name());
...
void
Writer::internType(const Type* t)
{
    ...
    if (!t->isTypePattern())
    {
        if (t->isSerializable()) _typeMap[t] = -1; 
        internNames(t);
    }
}
...
Writer::freeze()
{
         ...
         ++s)
    {
        ...
        {
            if (_primarySymbolSet.count(p) > 0) top = p;
        }
    ...
    for (TypeMap::iterator i = _typeMap.begin();
         i != _typeMap.end();
         ++i)
    {
        i->second = count++;
    ...
    for (NameMap::iterator i = _nameMap.begin();
         i != _nameMap.end();
         ++i)
    {
        i->second = count++;
...
void
Writer::add(const SymbolVector& symbols)
{
    for (int i = 0; i < symbols.size(); i++) add(symbols[i]);
}

void
Writer::add(const Names& names)
{
    for (Names::const_iterator i = names.begin(); 
         i != names.end(); 
         ++i) 
    {
        internName(*i);
...
void
Writer::addModuleRequirement(const Module* m)
{
    //
...
IDNumber
Writer::objectId(const Object* o) const
{
    if (!o) return 0;
...
void
Writer::collect(const Object* o)
{
    if (!o || _objects.find(o) != _objects.end()) return;

    if (!o->type()->isSerializable()) throw UnarchivableObjectException();
    ...
    if (const FunctionType* ftype = dynamic_cast<const FunctionType*>(o->type()))
    {
        ...
        if (fo->dependent()) collect(fo->dependent());
    }
...
void
Writer::collectRecursive(const Symbol* s)
{
    internNames(s);
    ...
    else if (const Function* f = dynamic_cast<const Function*>(s))
    {
        ...
        if (f->body()) NodeSymbolCollector(f->body(), this);
    }
}

void
Writer::writeObjectId(ostream& o, const Object* obj)
{
...
void
Writer::writeSize(ostream& o, SizeType s)
{
    o.write((const char*)&s, sizeof(SizeType));
}

void
Writer::writeByte(ostream& o, Byte b)
{
    o.write((const char*)&b, sizeof(Byte));
}

void
Writer::writeBool(ostream& o, Boolean b)
{
    o.write((const char*)&b, sizeof(Boolean));
}

void
Writer::writeOp(ostream& o, OpCode op)
{
...
void
Writer::writeName(ostream& o, Name n)
{
    o << n;
    o.put(0);
}
...
void
Writer::writeNameId(ostream& o, Name n)
{
...
void
Writer::writeU32(ostream& o, U32 u)
{
    o.write((const char*)&u, sizeof(U32));
}

void
Writer::writeU16(ostream& o, U16 u)
{
...
Writer::writeHeader(ostream& o)
{
    ...
    if (_annotationOutput) f |= Header::AnnotationFlag;

    writeU32(o, f);
}

void
Writer::writeNameTable(ostream& o)
{
...
void
Writer::writeRequiredModules(ostream& o)
{
    size_t ignoreCount = 0;

    for (ModuleSet::const_iterator i = _requiredModules.begin();
         i != _requiredModules.end();
         ++i)
    {
        const Module* m = *i;
        if (_primarySymbolSet.count(m) > 0) ignoreCount++;
    }
    ...
    {
        const Module* m = *i;
        if (_primarySymbolSet.count(m) > 0) continue;
...
void
Writer::writeDerivedTypes(ostream& o)
{
         ...
         ++i)
    {
        ...
        if (t->isCollection() ||
            dynamic_cast<const TupleType*>(t) ||
    ...
    for (size_t i = 0; i < types.size(); i++)
    {
        ...
        if (_debugOutput) cout << "< derived " << types[i]->fullyQualifiedName() << endl;
    }
}

void 
Writer::writePartialDeclarations(ostream& o, const Symbol* s, bool root)
{
    if (root)
    {
    ...
    if (const Function* F = dynamic_cast<const Function*>(s))
    {
        if (_passNum == 1) writeAnnotationInfo(o, s);
    ...
    {
        if (_passNum == 0) writeAnnotationInfo(o, s);
...
void
Writer::writePartialChildDeclarations(ostream& o, const Symbol* s)
{
    if (SymbolTable* st = s->symbolTable())
    {
        ...
        for (SymbolTable::Iterator i(st); i; ++i)
        {
            ...
            {
                if (_primarySymbolSet.count(S) > 0) symbols.push_back(S);
            }
        }
...
void
Writer::writePartialStackDeclaration(ostream& o, const StackVariable* v)
{
    if (_passNum == 1)
    {
        if (_debugOutput)
...
void
Writer::writePartialGlobalDeclaration(ostream& o, const GlobalVariable* v)
{
    if (_passNum == 1)
    {
        if (_debugOutput)
...
void
Writer::writePartialSymbolicConstantDeclaration(ostream& o, const SymbolicConstant* s)
{
    if (_passNum == 1)
    {
        if (_debugOutput)
...
void
Writer::writePartialMemberVariableDeclaration(ostream& o, const MemberVariable* v)
{
    if (_passNum == 1)
    {
        if (_debugOutput)
...
void
Writer::writePartialAliasDeclaration(ostream& o, const Alias* a)
{
    if (_passNum == 1)
    {
        if (_debugOutput) cout << "< declaration of alias " << a->fullyQualifiedName() << endl;
    }
}

void
Writer::writePartialClassDeclaration(ostream& o, const Class* c)
{
    if (_passNum == 0)
    {
        if (_debugOutput) cout << "< declaration of class " << c->fullyQualifiedName() << endl;
...
void
Writer::writePartialVariantDeclaration(ostream& o, const VariantType* V)
{
    if (_passNum == 0)
    {
        if (_debugOutput) cout << "< declaration of variant type " << V->fullyQualifiedName() << endl;
        //writeNameId(o, V->name());
    }
...
void
Writer::writePartialFunctionDeclaration(ostream& o, const Function* f)
{
    if (_passNum == 0) return;
    if (_debugOutput) cout << "< declaration of " << f->fullyQualifiedName() << endl;
...
void
Writer::writePartialModuleDeclaration(ostream& o, const Module* m)
{
    if (_passNum == 0)
    if (_debugOutput) cout << "< declaration of module " << m->fullyQualifiedName() << endl;
    //writeNameId(o, m->name());
    writePartialChildDeclarations(o, m);
}

void
Writer::writePartialNamespaceDeclaration(ostream& o, const Namespace* n)
{
    if (_passNum == 0)
    if (_debugOutput) cout << "< declaration of namespace " << n->fullyQualifiedName() << endl;
    //writeNameId(o, n->name());
    writePartialChildDeclarations(o, n);
}

void
Writer::writeFullDeclarations(ostream& o, const Symbol* s, bool root)
{
    if (root)
    {
        writeOp(o, OpPopScopeToRoot);
...
void
Writer::writeChildDeclarations(ostream& o, const Symbol* s)
{
    if (SymbolTable* st = s->symbolTable())
    {
        SymbolVector symbols;
...
void
Writer::writeFunctionDeclaration(ostream& o, const Function* f)
{
    if (_debugOutput)
    {
...
void
Writer::writeVariantDeclaration(ostream& o, const VariantType* V)
{
    writeChildDeclarations(o, V);
}

void
Writer::writeVariantTagDeclaration(ostream& o, const VariantTagType* V)
{
    if (_debugOutput) cout << "< declaration of variant tag type " << V->fullyQualifiedName() << endl;
    writeNameId(o, V->name());
...
void
Writer::writeAliasDeclaration(ostream& o, const Alias* a)
{
    if (_debugOutput) cout << "< declaration of alias " << a->fullyQualifiedName() << endl;
...
void
Writer::writeClassDeclaration(ostream& o, const Class* c)
{
...
void
Writer::writeModuleDeclaration(ostream& o, const Module* m)
{
    writeChildDeclarations(o, m);
}

void
Writer::writeNamespaceDeclaration(ostream& o, const Namespace* n)
{
    writeChildDeclarations(o, n);
}

void
Writer::writeStackDeclaration(ostream& o, const StackVariable* v)
{
}

void
Writer::writeGlobalDeclaration(ostream& o, const GlobalVariable* v)
{
}


size_t
Writer::writeObjects(ostream& o)
{
    //
...
void
Writer::writeExpression(ostream& o, const Node* n)
{
    //
    ...
    if (const Function* f = dynamic_cast<const Function*>(s))
    {
...

        {
            ...
            writeSize(o, nargs);
                 ...
                 n->func() == n->type()->machineRep()->callMethodFunc())
        {
        ...
        else
        {
            ...
            if (exact && nargs == f->numArgs() + f->numFreeVariables())
            {
                ...
                {
                    if (f->argType(i) != n->argNode(i)->type()) { exact = false; break; }
                }
            }
            else
            {
...
size_t
Writer::writeDocumentation(ostream& o)
{
    freeze();
...
void
Writer::writeSymbolDocumentation(ostream& o, const Symbol* s, Object* docs)
{
...
Object*
Reader::objectOfId(IDNumber i) const
{
    if (i >= _inverseObjectMap.size()) return 0;
    return _inverseObjectMap[i];
}

U32
Reader::readU32(istream& i)
{
    U32 x;
...
string
Reader::readString(istream& i)
{
    string s;
    for (int ch; ch = i.get();) s += char(ch);
    return s;
}

Name
Reader::readName(istream& i)
{
...
IDNumber
Reader::readObjectId(istream& i)
{
    return readIDNumber(i);
}

SizeType
Reader::readSize(istream& i)
{
    SizeType x;
...
Reader::read(istream& in)
{
    ...
    {
        if (_debugOutput) cout << "> loading module " << _requiredModules[i] << endl;
        Module::load(_requiredModules[i], _process, _context);
    }

    SizeType count = readSize(in);
    _passNum = 0;
    for (size_t i = 0; i < count; i++) readPartialDeclarations(in);
    _passNum = 1;
    for (size_t i = 0; i < count; i++) readPartialDeclarations(in);
    readDerivedTypes(in);
    for (size_t i = 0; i < count; i++) readFullDeclarations(in);
...
void
Reader::readHeader(istream& in)
{
    in.read((char*)&_header, sizeof(Header));
}

void
Reader::readRequiredModules(istream& in)
{
...
void
Reader::readNameTable(istream& in)
{
...
void
Reader::readDerivedTypes(istream& in)
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        if (_debugOutput) cout << "> derived " << t->fullyQualifiedName() << endl;
    }
}

void 
Reader::readPartialDeclarations(istream& in)
{
    OpCode op = readOp(in);
...
void
Reader::readPartialChildDeclarations(istream& in)
{
    OpCode op = readOp(in);
...
void
Reader::readPartialStackDeclaration(istream& in)
{
    ...
    if (_passNum == 1)
    {
        ...
        if (_debugOutput) cout << "> declare stack variable " << typeName << " " << name << endl;
...
void
Reader::readPartialGlobalDeclaration(istream& in)
{
    Name name = readNameId(in);
    ...
    if (_passNum == 1)
    {
        ...
        if (_debugOutput) cout << "> declare global variable " << typeName << " " << name << endl;
...
void
Reader::readPartialSymbolicConstantDeclaration(istream& in)
{
    Name name = readNameId(in);
    ...
    if (_passNum == 1)
    {
        ...
        if (_debugOutput) cout << "> declare symbolic constant " << c->fullyQualifiedName() << endl;
...
void
Reader::readPartialMemberVariableDeclaration(istream& in)
{
    Name name = readNameId(in);
    if (_passNum == 0) return;
    Name typeName = readNameId(in);
    ...
    if (_debugOutput) cout << "> declare variable " << typeName << " " << name << endl;
...
void
Reader::readPartialModuleDeclaration(istream& in)
{
    Name name = readNameId(in);

    if (_passNum == 0)
    {
        if (_debugOutput) cout << "> declare module " << name << endl;
        bool exists = _as->scope()->findSymbolOfType<Module>(name) != 0;
        _as->pushModuleScope(name);
        Module* m = static_cast<Module*>(_as->scope());

        if (!exists) _modules.push_back(m);
...
void
Reader::readPartialNamespaceDeclaration(istream& in)
{
    Name name = readNameId(in);
    ...
    if (_passNum == 0)
    {
        ...
        if (_debugOutput) cout << "> declare namespace " << n->fullyQualifiedName() << endl;
...
void
Reader::readPartialFunctionDeclaration(istream& in, bool member)
{
    Name name = readNameId(in);
    if (_passNum == 0) return;
    ...
    //_as->newStackFrame();
    Function* F = member ? _as->declareMemberFunction(name.c_str(), rtype, l, attrs)
...
void
Reader::readPartialClassDeclaration(istream& in)
{
    Name name = readNameId(in);
    ...
    if (_passNum == 0)
    {
        ...
        if (_debugOutput) cout << "> declared class " << c->fullyQualifiedName() << endl;
...
void
Reader::readPartialVariantDeclaration(istream& in)
{
    Name name = readNameId(in);
    ...
    if (_passNum == 0)
    {
        ...
        if (_debugOutput) cout << "> declared variant " << v->fullyQualifiedName() << endl;
...
void
Reader::readPartialVariantTagDeclaration(istream& in)
{
    //_allSymbols.push_back(0);
}

void
Reader::readPartialAliasDeclaration(istream& in)
{
    //_allSymbols.push_back(0);
}

void 
Reader::readFullDeclarations(istream& in)
{
    OpCode op = readOp(in);
...
void
Reader::readChildDeclarations(istream& in)
{
    OpCode op = readOp(in);
...
void
Reader::readStackDeclaration(istream& in, StackVariable* v)
{
    // nothing
}

void
Reader::readGlobalDeclaration(istream& in, GlobalVariable* v)
{
    // nothing
}

void
Reader::readFunctionDeclaration(istream& in, Function* F)
{
    if (_debugOutput) 
    {
        cout << ">> func: "; 
...
void
Reader::readVariantDeclaration(istream& in, VariantType* v)
{
    _current = v;
    readChildDeclarations(in);
}
...
void
Reader::readVariantTagDeclaration(istream& in)
{
    Name name = readNameId(in);
    Name typeName = readNameId(in);
    const Type* t = findType(typeName);
    ...
    _current = tt;
    if (_debugOutput) cout << "> declared variant tag " << tt->fullyQualifiedName() << endl;

    //readChildDeclarations(in);
}

void
Reader::readAliasDeclaration(istream& in)
{
    Name name = readNameId(in);
...
void
Reader::readClassDeclaration(istream& in, Class* C)
{
    _current = C;
    assert(!C->isFrozen());
    C->freeze();
...
void
Reader::readModuleDeclaration(istream& in, Module* M)
{
    _as->pushScope(M);
    _current = M;
    readChildDeclarations(in);
...
void
Reader::readNamespaceDeclaration(istream& in, Namespace* N)
{
    _as->pushScope(N);
    _current = N;
    readChildDeclarations(in);
...
Reader::readObjects(istream& in)
{
    ...
    if (_debugOutput) cout << "> " << nObjects << " objects serialized in file" << endl;
    ...
    for (int i=0; i < _inverseObjectMap.size(); i++)
    {
        ...
        if (obj) obj->type()->reconstitute(*this, obj);
    }
...
Node*
Reader::readExpression(istream& in, NodeAssembler& as)
{
    OpCode op = readOp(in);
    ...
    switch (op)
    {
      ...
      case OpCallMethod:
      {
          ...
          if (!f) 
          {
              ...
              cout << "source location is: " << as.sourceName() 
                   << ", line " << as.lineNum() 
                   << ", char " << as.charNum() << endl;

              Symbol* ec = as.context()->findSymbolOfTypeByQualifiedName<Module>(as.context()->internName("extra_commands"));
              if (ec) cout << "found " << ec->fullyQualifiedName() << endl;
              else cout << "no extra_commands found" << endl;
          ...
          //if (!f) f = as.context()->globalScope()->findSymbolOfType<Function>(n);
      ...
      case OpReference:
      {
          ...
          Variable* v = 
              _context->findSymbolOfTypeByQualifiedName<Variable>(n, false);
          assert(v);
      ...
      case OpDereferenceClassMember:
      {
          ...
          if (op == OpDereferenceClassMember) rnode = as.dereferenceLValue(rnode);
          return rnode;
      }
...
Variable*
Reader::findStackVariableInScope(QualifiedName n, NodeAssembler& as)
{
...
const Type*
Reader::findType(Name name)
{
    const Type* t = _context->findSymbolOfTypeByQualifiedName<Type>(name, false);
    if (!t) t = _context->parseType(name.c_str(), _process);
    assert(t);

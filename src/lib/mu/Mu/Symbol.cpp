...
Symbol::Symbol(Context* context, const char* name) : _context(context)
{
...
void 
Symbol::output(std::ostream &o) const
{
    o << fullyQualifiedName();
}

void
Symbol::outputNode(std::ostream &o, const Node* n) const
{
    output(o);
}
...
bool 
Symbol::resolveSymbols() const
{
    return true;
}

const Type* 
Symbol::nodeReturnType(const Node *) const
{
    return 0;
}
...
void 
Symbol::addSymbols(Symbol *symbol, ...)
{
    if (!symbol) return;
    ...
    while( s = va_arg(ap,Symbol*) ) addSymbol(s);
    va_end(ap);
}
...
Symbol*
Symbol::findSymbolByQualifiedName(Name name, bool restricted)
{
    const Symbol* s = 
        const_cast<const Symbol*>(this)->findSymbolByQualifiedName(name, restricted);
    return const_cast<Symbol*>(s);
}

static size_t
findSeparator(const String& s)
{
...
static void
qualifiedNameLookup(const Symbol::NameVector& name, 
                    Symbol* base, 
...
void
Symbol::symbolDependancies(ConstSymbolVector&) const
{
}

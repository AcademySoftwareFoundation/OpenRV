...
void
NodeAssembler::dumpNodeStack()
{
    NodeVector &n = _nodeStack;
    cout << "stack = (" << n.size() << ")";
    for (int i=0; i<n.size(); i++)
    {
	cout << " (" << n[i]->symbol()->name() << ")="
	     << hex << n[i] << dec;
    }
    cout << endl << flush;
}
      ...
      _docmodule(0)
{
    if (p)
    {
        if (!t) _thread = p->newApplicationThread();
...
Process*
NodeAssembler::process() const
{
    return _process;
}

Thread*
NodeAssembler::thread() const
{
    return _thread;
}

void
NodeAssembler::reportError(const char* msg) const
{
    reportWarning(msg);
    _error = true;
}
...
void
NodeAssembler::reportError(Node* n, const char* msg) const
{
    reportWarning(n, msg);
    _error = true;
}
...
void
NodeAssembler::freportError(const char* msg, ...) const
{
    char temp[256];
    va_list ap;
    va_start(ap,msg);
...
void
NodeAssembler::freportError(Node* n, const char* msg, ...) const
{
    char temp[256];
    va_list ap;
    va_start(ap,msg);
...
void
NodeAssembler::reportWarning(const char* msg) const
{
    if (lineNum() > 0)
    {
        context()->errorStream() << sourceName() << ", line "
                                 << lineNum() << ", char "
                                 << charNum() << ": "
                                 << msg << endl;
    }
...
void
NodeAssembler::reportWarning(Node* n, const char* msg) const
{
    //
...
void
NodeAssembler::freportWarning(const char* msg, ...) const
{
    char temp[256];
    va_list ap;
    va_start(ap,msg);
...
void
NodeAssembler::freportWarning(Node* n, const char* msg, ...) const
{
    char temp[256];
    va_list ap;
    va_start(ap,msg);
...
void
NodeAssembler::pushModuleScope(Name name)
{
    Module* m = 0;
...
NodeAssembler::NodeList
NodeAssembler::newNodeList(Node *n)
{
...
NodeAssembler::NodeList
NodeAssembler::newNodeList(const NodeVector& nodes)
{
    NodeList nl = newNodeList(nodes.front());
    for (int i=1; i < nodes.size(); i++) nl.push_back(nodes[i]);
    return nl;
}

NodeAssembler::NodeList
NodeAssembler::emptyNodeList()
{
    NodeList nl;
    nl._c      = &_nodeStack;
...
NodeAssembler::NodeList
NodeAssembler::newNodeListFromArgs(Node* n)
{
...
void
NodeAssembler::insertNodeAtFront(NodeList nl, Node* n)
{
    nl.push_back(0);
    for (int i=nl.size()-1; i > 0; i--) nl[i] = nl[i-1];
    nl[0] = n;
}

void
NodeAssembler::removeNodeList(NodeList l)
{
    _nodeStack.resize(l.start());
}

bool
NodeAssembler::containsNoOps(NodeList l) const
{
...
NodeAssembler::SymbolList
NodeAssembler::newSymbolList(Symbol *symbol)
{
...
void
NodeAssembler::removeSymbolList(SymbolList l)
{
    _symbolStack.resize(l.start());
}

void
NodeAssembler::insertSymbolAtFront(SymbolList sl, Symbol* symbol)
{
    sl.push_back(0);
    for (int i=sl.size()-1; i > 0; i--) sl[i] = sl[i-1];
    sl[0] = symbol;
}

NodeAssembler::NameList
NodeAssembler::emptyNameList()
{
    NameList nl;
    nl._c      = &_nameStack;
...
NodeAssembler::NameList
NodeAssembler::newNameList(Name n)
{
...
void
NodeAssembler::removeNameList(NameList l)
{
    _nameStack.resize(l.start());
}

DataNode*
NodeAssembler::constant(const Type* type, Object* o) const
{
...
DataNode*
NodeAssembler::constant(const SymbolicConstant* sym) const
{
...
Node*
NodeAssembler::newNode(const Function* f, int nargs) const
{
    Node* n = 0;
...
Node*
NodeAssembler::functionConstant(const Function* f, bool literal)
{
    DataNode* n = 0;
    FunctionObject* o = 0;
...
void
NodeAssembler::prefixScope(const Symbol* symbol)
{
...
Node*
NodeAssembler::functionReduce(const Function* f, Node* n)
{
...
Node*
NodeAssembler::constReduce(Node* n) const
{
...
Node*
NodeAssembler::constReduce(const Function *f, Node *n) const
{
    //
	...
	for (int i=0,s=n->numArgs(); i<s; i++)
	{
	    ...
	    Node *argNode = n->argNode(i);
            if (!isConstant(argNode)) return n;
            ...
            if (f->maybePure())
            {
                    ...
                    dynamic_cast<const FunctionType*>(argNode->type()))
                {
                        ...
                        reinterpret_cast<FunctionObject*>(dn->_data._Pointer))
                    {
                        ...
                        if (!fn->isPure() && !fn->maybePure()) return n;
                    }
                    else
                    {
                        freportWarning("While trying to reduce function \"%s\", "
                                       "argument %d which should have been a function object "
...
SymbolicConstant*
NodeAssembler::newSymbolicConstant(Name name, Node* n)
{
...
Node*
NodeAssembler::retainOrRelease(Node *node, bool retain) const
{
	...
	if (const Function *f = type->findSymbolOfType<Function>(name))
	{
	    ...
	    else
	    {
                freportError("Function \"%s\" of type %s does not have proper "
                             "signature", f->name().c_str(), type->name().c_str());
	    }
	}
    }

    freportError("Type \"%s\" requires \"%s\" function but does not implement it",
...
Node*
NodeAssembler::retain(Node *n) const
{
    return retainOrRelease(n, true);
}

Node*
NodeAssembler::release(Node *n) const
{
    return retainOrRelease(n, false);
}

Node*
NodeAssembler::cast(Node *node, const Type *toType)
{
    if (!node)
    {
        //
    ...
    if (fromType == toType) return node;
    ...
    if ((toInterface && fromClass) || 
        (toClass && fromInterface) || 
        (match && toClass && fromClass))
    {
        ...
        if (_constReduce) return constReduce(context()->dynamicCast(), cast);
        else return cast;
    }

    //
    //	Find an overloaded symbol which casts directly to toType. The while
    //	loop allows checkin inside the type for constructors in its scope.
    //
	...
	if (_constReduce) return constReduce(function, cast);
	else return cast;
    }
    else
    {
...
Node*
NodeAssembler::unaryOperator(const char *op, Node *node1)
{
...
Node*
NodeAssembler::binaryOperator(const char *op, Node *node1, Node *node2)
{
    NodeList nl = newNodeList(node1);
    nl.push_back(node2);
    Node* n = callBestFunction(op, nl);
...
Node*
NodeAssembler::assignmentOperator(const char *op, Node *node1, Node *node2)
{
    Node* n = 0;
        ...
        node2->type() == context()->unresolvedType())
    {
        ...
        n = new ASTAssign(*this,
                          context()->unresolvedAssignment(),
                          node1,
                          node2);
    }
    else
    {
            ...
            dynamic_cast<const ReferenceType*>(node1->type()))
        {
            ...
            {
                freportError("cannot cast \"%s\" to \"%s\" for assignment.",
...
void
NodeAssembler::setProcessRoot(Node *node)
{
    ...
    {
        if (node) node->deleteSelf();
        throw ParseSyntaxException();
    }
}

Node*
NodeAssembler::memberOperator(const char* op, Node* n, NodeList nl)
{
    ...
    if (unresolved)
    {
        ...
        return new ASTIndexMember(*this, 
                                  nl.size(), 
                                  &nl.front(),
    ...
    {
        if (const Function *f =
            type->findSymbolOfType<Function>(iname))
        {
        ...
        {
            freportError("operator%s not defined for type %s",
                         op,
                         type->fullyQualifiedName().c_str());
            return 0;
        }
...
Node*
NodeAssembler::callBestFunction(const char *name, NodeList nodes)
{
...
Node*
NodeAssembler::callBestOverloadedFunction(const Function* func, NodeList nodes)
{
    FunctionVector functions;
...
Node*
NodeAssembler::callFunction(const Function* F, NodeList nodes)
{
    int  nargs     = F->numArgs();
...
Node*
NodeAssembler::callExactOverloadedFunction(const Function* Fin, NodeList nodes)
{
    Context::TypeVector argTypes(nodes.size());
    const Function* F = 0;
    ...
    for (int i=0; i < nodes.size(); i++)
    {
        ...
        if (!t || t->isUnresolvedType())
        {
            freportError("error calling NodeAssembler::callExactOverloadedFunction(): "
                         "argument %d to function \"%s\" is unresolved",
                         i, F->fullyQualifiedName().c_str());
            return 0;
        }
    ...
    for (size_t i = 0; i < nodes.size(); i++) node->setArg(nodes[i], i);
    return node;
}

Node*
NodeAssembler::callBestFunction(const FunctionVector& functions, NodeList nodes)
{
    Context::TypeVector argTypes(nodes.size());
	...
	context()->matchSpecializedFunction(process(),
                                            thread(),
                                            functions, 
                                            argTypes, matchType))
    {
        ...
        if (MemberFunction* func = findScopeOfType<MemberFunction>())
        {
            ...
            for (int i=0; i < functions.size(); i++)
            {
                ...
                if ((g->minimumArgs() <= nodes.size() + 1 && g->maximumArgs() >= nodes.size() + 1) && 
                    (g->scope() == func->scope() ||
                     (fscope &&
                      fscope->isA(dynamic_cast<const Class*>(g->scope())))))
                {
                    mfuncs.push_back(g);
                }
            }
...
void
NodeAssembler::showOptions(const FunctionVector& functions, NodeList nodes)
{
    ostream &err = context()->errorStream();
    context()->error("No match found for function \"");
    err << functions.front()->name()
        << "\" with " << nodes.size() << " argument"
	<< (nodes.size() == 1 ? "" : "s")
	<< ": ";
...
void
NodeAssembler::showArgs(NodeList nodes)
{
    ostream &err = context()->errorStream();
...
Node*
NodeAssembler::unresolvableCall(Name name, 
                                NodeList nl, 
                                const Symbol* s)
{
    ASTCall* astcall = new ASTCall(*this,
                                   nl.size(),
                                   s ? s : context()->unresolvedCall(),           
                                   name);

    if (nl.size()) astcall->setArgs(&nl.front(), nl.size());
...
Node*
NodeAssembler::unresolvableStackDeclaration(Name n)
{
    Variable::Attributes rw = Variable::ReadWrite;
    Variable::Attributes rwi = rw | Variable::ImplicitType;

    StackVariable* svar = new StackVariable(context(),
                                            n.c_str(), 
                                            context()->unresolvedType(),
                                            _stackOffset++,
                                            rwi);

    _stackVariables.push_back(svar);

    ASTNode* astnode = new ASTStackDeclaration(*this,
                                               context()->unresolvedDeclaration(),
                                               n, svar);
...
Node*
NodeAssembler::unresolvableCast(Name name, NodeList nl)
{
    ASTCast* astnode = new ASTCast(*this,
                                   nl.size(),
                                   context()->unresolvedCast(),           
                                   name);

    if (nl.size()) astnode->setArgs(&nl.front(), nl.size());
...
Node*
NodeAssembler::unresolvableMemberCall(Name name, NodeList nl)
{
    ASTMemberCall* astnode = new ASTMemberCall(*this,
                                               nl.size(),
                                               context()->unresolvedMemberCall(),                       
                                               name);
...
Node*
NodeAssembler::unresolvableConstructor(const Type* t, NodeList nl)
{
    DataNode* dn = new DataNode(nl.size(),
                                BaseFunctions::unresolved,
...
Node*
NodeAssembler::unresolvableStackReference(const StackVariable* sv)
{
    ...
    if (dynamic_cast<Function*>(s))
    {
        ASTStackReference* astnode = 
            new ASTStackReference(*this, 0, context()->unresolvedStackReference(), sv);
...
Node*
NodeAssembler::unresolvableReference(Name name)
{
    Symbol* s = nonAnonymousScope();
    markCurrentFunctionUnresolved();
    return new ASTReference(*this,
                            context()->unresolvedStackReference(),
                            name);
}

Node*
NodeAssembler::callMethod(const Function* method, NodeList nl)
{
...
Node*
NodeAssembler::callMethod(const Function* method,
                          Node* self,
                          NodeList arguments)
{
...
Node*
NodeAssembler::methodThunk(const MemberFunction* f, Node* self)
{
    NodeList nl = newNodeList(self);
...
Node*
NodeAssembler::call(Node* node, NodeList nl, bool dynamicDispatch)
{
    ...
    else
    {
        if (n->type() == context()->unresolvedType())
        {
            ...
            {
                freportError("unresolved in this context (needs implementing?)",
...
Node*
NodeAssembler::call(const Symbol* sym, NodeList nl, bool dynamicDispatch)
{
    ...
    if (const Type* type = dynamic_cast<const Type*>(sym))
    {
        ...
        {
            Symbol::SymbolVector symbols =
                ((Symbol*)sym)->scope()->findSymbolsOfType<Function>(sym->name());
            functions.resize(symbols.size());
            transform(symbols.begin(),
                      symbols.end(),
                      functions.begin(),
    ...
    if (found)
    {
        ...
        else 
        {
            ...
            {
                if (dynamic_cast<const MemberFunction*>(functions[i])) mcount++;
                else fcount++;
            }
            ...
            if (fcount && mcount)
            {
                ...
                if (const Function* f = 
                    context()-> matchSpecializedFunction(process(), thread(), 
                                                         functions, types, 
                                                         matchType))
                {
    ...
    else if (Variable *v = findTypeInScope<Variable>(sym->name()))
    {
        ...
        if (const Function *f =
            t->findSymbolOfType<Function>(iname))
        {
            if (containsNoOps(nl))
            {
                return dynamicPartialEvalOrApply(rn, nl, false, dynamicDispatch);
            ...
            else
            {
                ...
                for (size_t i=0; i < nl.size(); i++) nnl.push_back(nl[i]);
...
Node*
NodeAssembler::dynamicPartialEvalOrApply(Node* self,
                                         NodeList arguments,
                                         bool reduce,
                                         bool dynamicDispatch)
{
    ...
    if (!outSig->resolved()) outSig->resolve(context());
    ...
    if (_constReduce) return constReduce(F, fn);
    return fn;
}


void
NodeAssembler::newStackFrame()
{
...
void
NodeAssembler::declareParameters(SymbolList parameters)
{
    //
...
FreeVariable*
NodeAssembler::declareFreeVariable(const Type* t, Name n)
{
    //
...
bool
NodeAssembler::declareMemberVariables(const Type* type)
{
...
Node*
NodeAssembler::declareVariables(const Type* declarationType,
				const char *asOp,
				NodeAssembler::VariableType kind)
{
	...
	if (!in.node)
	{
	    ...
	    for (const Symbol *s = type->firstOverload();
                 s; s = s->nextOverload())
	    {
		if (const Function *f = dynamic_cast<const Function*>(s))
		{
	...
	else if (kind == Global)
	{
	    ...
	    var  = new GlobalVariable(context(),
                                      temp.c_str(),
				      type,
	...
	if (in.node)
	{
	    ...
	    // }
            ...
            if (Node* r = referenceVariable(var))
            {
                ...
                {
                    // memory leak?
                    freportError("Unable to cast \"%s\" to \"%s\"",
                                 in.node->type()->fullyQualifiedName().c_str(),
                                 type->fullyQualifiedName().c_str());
...
StackVariable*
NodeAssembler::declareStackVariable(const Type* t, 
                                    Name n,
...
GlobalVariable*
NodeAssembler::declareGlobalVariable(const Type* t, Name n)
{
    GlobalVariable* var = new GlobalVariable(context(),
                                             n.c_str(),
                                             t,
                                             process()->globals().size(),
                                             Variable::ReadWrite,
...
Namespace*
NodeAssembler::declareNamespace(Name n)
{
...
NodeAssembler::endStackFrame()
{
    ...
    stable_partition(_stackVariables.begin(),
                     _stackVariables.end(),
...
Node*
NodeAssembler::endStackFrame(NodeList nl)
{
    Node *n = 0;
    Function* I = 0;
	...
	DataNode *dn = new DataNode(nl.size(),
        ...
        if (nl.size()) dn->setArgs(&nl.front(), nl.size());
	...
	dn->_data._int = _stackVariables.size();
        ...
        {
            I = new Function(context(), 
                             uniqueNameInScope("__init").c_str(),
                             context()->voidType(), 0, 0, n, Function::None);
        }
    }
...
Name
NodeAssembler::uniqueNameInScope(const char* name) const
{
...
void
NodeAssembler::pushAnonymousScope(const char* name)
{
...
Node*
NodeAssembler::dereferenceLValue(Node* n)
{
    if (!n)
    {
        return 0;
    ...
    if (t == context()->unresolvedType())
    {
        ...
        {
            return new ASTDereference(*this,
                                      context()->unresolvedDereference(),
                                      n);
        }
    }
...
StackVariable*
NodeAssembler::findStackVariable(StackVariable *var) const
{
    for (int i=_stackVariables.size(); i--;)
    {
	if (_stackVariables[i] == var) return var;
    }
...
Node*
NodeAssembler::referenceVariable(const Variable* var)
{
...
Node*
NodeAssembler::dereferenceVariable(const char* name)
{
...
Node*
NodeAssembler::dereferenceVariable(const Variable* v)
{
    return dereferenceLValue(referenceVariable(v));
}

Node*
NodeAssembler::unresolvableMemberReference(Name name, Node* n)
{
    ASTMemberReference* astnode = new ASTMemberReference(*this,
                                                         1,
...
Node*
NodeAssembler::referenceMemberVariable(const MemberVariable *var, Node *a)
{
...
Node*
NodeAssembler::replaceNode(Node *n, const Symbol* s, NodeFunc f) const
{
    n->_symbol = s;
    n->_func   = f;
    return n;
...
void
NodeAssembler::patchUnresolved()
{
    for_each(_unresolvedFunctions.begin(),
             _unresolvedFunctions.end(),
             Patch(this));
...
void
NodeAssembler::patchFunction(Function* f)
{
...
void
NodeAssembler::pushScope(Symbol* s, bool declarative)
{
    _scope = new ScopeState(s, _scope, declarative);
}

void
NodeAssembler::setScope(const ScopeState* ss)
{
    _scope = ss;
}

void
NodeAssembler::popScope()
{
    //
    ...
    while (!_scope->declaration && _scope->parent) _scope = _scope->parent;
    if (_scope->parent) _scope = _scope->parent;
}

void
NodeAssembler::popScopeToRoot()
{
    while (_scope->parent) _scope = _scope->parent;
}

Symbol*
NodeAssembler::scope() const
{
    for (const ScopeState* ss = _scope; ss; ss = ss->parent)
    {
        if (ss->declaration) return ss->symbol;
    }
...
Symbol*
NodeAssembler::nonAnonymousScope()
{
    for (const ScopeState* ss = _scope; ss; ss = ss->parent)
    {
...
void
NodeAssembler::pushType(const Type *t)
{
    _typeStack.push_back(t);
}

Interface*
NodeAssembler::declareInterface(const char* name, SymbolList inheritance)
{
...
void
NodeAssembler::generateDefaults(Interface* i)
{
    //
    ...
    Function* OpAs = new Function(context(),
                                  "=",
                                  BaseFunctions::assign,
                                  Function::MemberOperator | Function::Operator,
                                  Function::Return,
                                  rt->fullyQualifiedName().c_str(),
                                  Function::Args,
                                  rt->fullyQualifiedName().c_str(),
                                  i->fullyQualifiedName().c_str(),
                                  Function::End);
...
VariantType*
NodeAssembler::declareVariantType(const char* name)
{
    ...
    Function* dr = new Function(context(),
                                t->name().c_str(), 
                                BaseFunctions::dereference,
                                Function::Cast,
                                Function::Return,
                                t->fullyQualifiedName().c_str(),
                                Function::Args,
...
VariantTagType*
NodeAssembler::declareVariantTagType(const char* name, const Type* type)
{
        ...
        if (r != context()->voidType())
        {
            ...
            if (r->symbolTable())
            {
                for (SymbolTable::Iterator i(r->symbolTable()); i; ++i)
                {
                    for (const Symbol* s = *i; s; s = s->nextOverload())
                    {
                        if (const Function* F = dynamic_cast<const Function*>(s))
                        {
                            if (F->name() == r->name()) constructors.push_back(F);
                        }
                    }
                }
            }
            ...
            for (int i=0; i < constructors.size(); i++)
            {
                ...
                if (F->numArgs() == 1)
                {
                    ...
                    if (a == r || a == r->referenceType()) continue;
                }
                ...
                Function* FC = new Function(context(),
                                            ...
                                            (ParameterVariable**)
                                            (sl.size() ? &sl[0] : NULL), 
                                            (Node*)0, Function::Cast 
                                            | Function::Pure | Function::Generated);
                t->addSymbol(FC);
                pushScope(FC);
                ...
                if (needsAlloc)
                {
                    ...
                    {
                        freportError("Internal failure generating union constructor %s: __allocate() not found in type %s",
...
Class*
NodeAssembler::declareClass(const char* name, 
                            ...
                            bool globalScope)
{
    ...
    Function* A = new Function(context(),
                               "__allocate",
                               BaseFunctions::classAllocate,
                               Function::None,
                               Function::Return,
                               c->fullyQualifiedName().c_str(),
                               Function::End);
...
void
NodeAssembler::generateDefaults(Class* c)
{
    //
    //  Create the default constructor
    //
    ...
    types[0] = c;
    for (int i=0; i < mvars.size(); i++) types[i+1] = mvars[i]->storageClass();
    Symbol::SymbolVector symbols = 
    ...
    if (!hasConstructor)
    {
        ...
        Function* F = new Function(context(),
                                   c->name().c_str(), c,
                                   params.size(),
                                   (ParameterVariable**) &params.front(),
                                   0, Function::ContextDependent);
        c->addSymbol(F);
        pushScope(F);
        ...
        for (int i=0; i < mvars.size(); i++)
        {
            ...
            Node* lval = referenceMemberVariable(mvars[i],
                                                 dereferenceVariable(thisv));
...
bool
NodeAssembler::checkRedeclaration(const char* name,
                                  ...
                                  SymbolList parameters)
{
    ...
    if (const Function* G = 
        scope()->findSymbolOfType<Function>(context()->internName(name)))
    {
                ...
                if (G0->numArgs() == parameters.size())
                {
                    ...
                    for (size_t i = 0; i < parameters.size(); i++)
                    {
                        const ParameterVariable* p = 
                            static_cast<const ParameterVariable*>(parameters[i]);
...
Function*
NodeAssembler::declareFunction(const char* name,
                               const Type* returnType,
                               SymbolList parameters,
                               unsigned int attrs,
                               bool addToContext)
{
    if (!checkRedeclaration(name, returnType, parameters)) return 0;
    ...
    else
    {
        f = new Function(context(),
                         ...
                         returnType,
                         parameters.size(),
                         (ParameterVariable**)&(parameters.front()),
                         0,
                         attrs);
    }
...
MemberFunction*
NodeAssembler::declareMemberFunction(const char* name,
                                     const Type* returnType,
                                     SymbolList parameters,
                                     unsigned int attrs)
{
    ...
    if (odoc) process()->addDocumentation(f, odoc);
...
Function*
NodeAssembler::declareFunctionBody(Function* F, Node* n)
{
...
void
NodeAssembler::declarationType(const Type* t, bool global)
{
...
Node*
NodeAssembler::declareInitializer(Name n, Node* node)
{
    ...
    else
    {
        ...
        {
            Node* rn = declareStackVariables(_initializerType, "=");
            if (!rn) freportError("Illegal assignment to %s.", n.c_str());
            return rn;
        }
    }
}
...
Class*
NodeAssembler::declareTupleType(SymbolList types)
{
...
Node*
NodeAssembler::tupleNode(NodeList nl)
{
    SymbolList sl = emptySymbolList();
    bool ok = true;
    ...
    for (int i=0; i < nl.size(); i++) 
    {
        ...
        {
            //Node* n = unresolvableCall(context()->internName("(type*)"), nl);
            Node* n = new ASTTupleConstructor(*this,
                                              nl.size(), 
                                              &nl.front(),
                                              context()->unresolvedCall());
            markCurrentFunctionUnresolved();
...
Node*
NodeAssembler::listNode(NodeList nl)
{
    for (size_t i = 0; i < nl.size(); i++)
    {
        if (nl[i]->type()->isUnresolvedType())
        {
            ...
            return new ASTListConstructor(*this,
                                          nl.size(), 
                                          &nl.front(),
                                          context()->unresolvedCall());
    ...
    for (int i=1, s=nl.size(); i < s; i++)
    {
        ...
        if (!etype->match(ntype))
        {
            freportError("cannot construct \"%s\" list "
                         ...
                         t->fullyQualifiedName().c_str(),
                         i+1, ntype->fullyQualifiedName().c_str(),
...
TypeVariable*
NodeAssembler::declareTypeVariable(const char* bname)
{
    String n = "'";
    n += bname;
...
Node*
NodeAssembler::suffix(Node* n, Name suffixName)
{
    if (!_suffixModule)
    {
...
void
NodeAssembler::addDocumentation(Name n, Object* o, const Type* t)
{
    if (_docmodule)
    {
        if (Symbol* symbol = _docmodule->findSymbolByQualifiedName(n, false))
        {
            ...
            for (Symbol* sym = symbol->firstOverload();
                 sym;
                 sym = sym->nextOverload())
            {
                ...
                if (add) process()->addDocumentation(sym, o);
            }
        }
    }
    else
    {
        if (_scope) _scope->docMap[n] = o;
    }
}

void
NodeAssembler::setDocumentationModule(Symbol* s)
{
    _docmodule = s;
}

Object*
NodeAssembler::retrieveDocumentation(Name n)
{
    for (const ScopeState* ss = _scope; ss; ss = ss->parent)
    {
...
NodeAssembler::newPattern(Name n)
{
    return new Pattern(n);
}

NodeAssembler::Pattern*
NodeAssembler::newPattern(Node* n)
{
...
NodeAssembler::Pattern*
NodeAssembler::newPattern(Pattern* p, const char* name)
{
...
void
NodeAssembler::appendPattern(Pattern* pattern, Pattern* appendee)
{
    Pattern* p = pattern;
    for (;p->next; p = p->next);
    p->next = appendee;
...
Node*
NodeAssembler::resolvePatterns(Pattern* pattern, Node* rhs)
{
    ...
    if (pattern->constructor)
    {
        ...
        if (!rhs) return 0;
        const Function* F = 
            pattern->constructor->findSymbolOfType<Function>(context()->lookupName("__unpack"));
        ...
        {
            if (tt->representationType() == context()->voidType()) return rhs;
        }
    }
    ...
    else if (pattern->expression)
    {
        ...
        n = binaryOperator(rhs->type() == context()->nilType() ? "eq" : "==",
                           rhs, 
                           pattern->expression);
...
NodeAssembler::NodeList
NodeAssembler::resolvePatternList(Pattern* pattern,
                                  Node* rhs, 
                                  Variable* var)
{
    NodeList nl = emptyNodeList();
    ...
    if (const Class* c = dynamic_cast<const Class*>(t))
    {
        ...
        for (Pattern* q = pattern; q; q = q->next) count++;
...
void
NodeAssembler::unflattenPattern(Pattern* pattern,
                                const char* tname,
                                bool keepLeaf)
{
    if (pattern->next) 
    {
        if (keepLeaf && !pattern->next->next) return;
...
Node*
NodeAssembler::beginCase(Node* n, const Function* predicate)
{
...
Node*
NodeAssembler::finishCase(Node* n, NodeList nl)
{
    insertNodeAtFront(nl, n);
...
Node*
NodeAssembler::casePatternStatement(Node* n, NodeList nl)
{
    if (n) insertNodeAtFront(nl, n);
...
Node*
NodeAssembler::casePattern(Pattern *p)
{
    if (p->constructor) 
    {
        if (p->constructor->scope() != _caseStack.back().exprType)
        {
            freportError("case pattern constructor \"%s\" does not match case expr type \"%s\"",
    ...
    else
    {
        ...
        {
            if (Node* r = resolvePatterns(p, n)) return r;
        }
    }
...
bool
NodeAssembler::casePattern(const Type* constructor)
{
...
void 
NodeAssembler::setSourceName(const std::string& name)
{
...
void                
NodeAssembler::markCurrentFunctionUnresolved()
{
    if (currentFunction())
    {
...
Node*
NodeAssembler::foreachStatement(Node* dnode, Node* cnode, Node* bnode)
{
        ...
        (stype->isUnresolvedType() || bnode->type()->isUnresolvedType()))
    {
        ...
        Node *astforeach = new ASTForEach(*this,
                                          args,
                                          context()->unresolvedCall());

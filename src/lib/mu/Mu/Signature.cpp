...
unsigned long
SignatureTraits::hash(const Signature* sig)
{
    unsigned long l = 0;
...
Signature::operator==(const Signature& other) const
{
    ...
    {
        if (_types[i].symbol != other._types[i].symbol) return false;
    }
...
void
Signature::push_back(const Type* t)
{
...
void
Signature::resolve(const Context *context) const
{
    if (resolved()) return;
    ...
    for (int i=0; i < types.size(); i++) _types[i].symbol = types[i];
    const_cast<Signature*>(this)->_resolved = true;
    MU_GC_END_CHANGE_STUBBORN(this);
}

String
Signature::functionTypeName() const
{

...
PartialApplicator::PartialApplicator(const Function* f,
                                     Process* p,
                                     Thread* t,
      ...
      _method(dynamic_cast<const MemberFunction*>(f) != 0 && dynamicDispatch)
{
    ...
    for (int i=0; i < f->numArgs() + f->numFreeVariables(); i++)
    {
        ...
        if (!mask[i])
        {
            ...
            if (p)
            {
                nv = new ParameterVariable(_as.context(),
                                           p->name().c_str(),
                                           p->storageClass());
            }
            else
    ...
    if (newParams.empty())
    {
        _closure = new Function(_as.context(),
...
Node*
PartialApplicator::generate(const ArgumentVector& args,
                            ParameterVector& closureParams)
{

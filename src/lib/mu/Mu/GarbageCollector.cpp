...
GarbageCollector::API::API(GarbageCollector::API* next) : _next(next) { } 
...
{
  public:
    GCHeapNoOptAPI(API* next) : API(next) {}
...
{
  public:
    GCPoolAPI(API* next) : GCHeapAPI(next), _total(0) {}
    virtual ~GCPoolAPI();
    virtual Pointer allocate(size_t);
    virtual Pointer allocateAtomic(size_t);
    virtual Pointer allocateAtomicOffPage(size_t);
...
GCPoolAPI::~GCPoolAPI()
{
    ...
    //cout << "freed " << _total << " bytes in " << _objects.size() << " objects" << endl;
}
...
{
  public:
    GCMallocPoolAPI(API* next) : API(next), _total(0) {}
...
{
    for (size_t i = 0; i < _objects.size(); i++) free(_objects[i]);
    //cout << "freed " << _total << " bytes in " << _objects.size() << " objects" << endl;
}
...
{
  public:
    GCStaticPoolAPI(API* next) : API(next), _total(0) {}
...
Pointer GCStaticPoolAPI::allocate(size_t s) 
{ 
    return _arena.allocate(s);
}
...
GCStatAPI::~GCStatAPI()
{
    ...
    for (StatMap::iterator i = _map.begin();
         i != _map.end();
         ++i)
    {
...
static void
arenaCollector(block_alloc_arena*)
{
...
void
GarbageCollector::mark(const Process* process)
{
}
...
size_t
GarbageCollector::collectionThreshold()
{
    return 0;
}

void
GarbageCollector::setCollectionThreshold(size_t s)
{
}

size_t
GarbageCollector::secondaryCollectionThreshold()
{
    return 0;
}

void
GarbageCollector::setSecondaryCollectionThreshold(size_t s)
{
}

size_t
GarbageCollector::mainArenaSize()
{
    return 0;
}

size_t
GarbageCollector::auxArenaSize()
{
    return 0;
}
...
void GarbageCollector::disable() { _disabled++; GC_disable(); }
void GarbageCollector::enable() { _disabled--; GC_enable(); }
bool GarbageCollector::needsCollection() { return _needsCollection; }
...
void
GarbageCollector::debugFinalizer(void* obj, void* data)
{
    cout << "DEBUG: finalizer " << obj;
    if (data) cout << ", name = " << (char*)data;
    cout << endl;
}
...
bool 
GarbageCollector::isGCPointer(Pointer p) 
{
    return GC_base(p) != 0;
}

bool 
GarbageCollector::isGCPointerSymbol(Pointer p)
{
    Pointer b = GC_base(p);
    if (b != p) return false;

    size_t size = GC_size(p);
    if (size < sizeof(Symbol)) return false;
    
    Pointer* objp = (Pointer*)p;
    for (size_t i = 0; i < 5; i++) if (!isGCPointer(objp[i])) return false;

...
Context::Context(const char *impName, const char *name) : _primaryState(false), _uniqueID(0)
{
...
void 
Context::setInput(istream& stream)
{
    _istream = &stream;
}

void 
Context::setOutput(ostream& stream)
{
    _ostream = &stream;
}

void 
Context::setError(ostream& stream)
{
    _estream = &stream;
}

void
Context::error(const char *fmt,...)
{
    char p[1024];
    va_list ap;
    va_start(ap, fmt);
...
void
Context::warning(const char *fmt,...)
{
    char p[1024];
    va_list ap;
    va_start(ap, fmt);
...
const Signature*
Context::internSignature(Signature* s) const
{
...
FunctionType*
Context::functionType(const char* name)
{
...
FunctionType*
Context::functionType(const Signature* s)
{
    if (s->resolved())
    {
...
bool
operator < (const Context::ScoredFunction& a, const Context::ScoredFunction& b) 
{ 
    return a.totalScore > b.totalScore; 
}

void
Context::ScoredFunction::computeTotalScore()
{
...
const Function*
Context::findCast(const Type *from, const Type *to) const
{
...
int
Context::score(const Function* F,
               TypeBindings& bindings,
               const Type* ftype,
               const Type* type) const
{
    if (!type || !ftype || ftype == type) return 0;
...
bool
Context::scoreArgs(Scores& scores,
                   TypeBindings& bindings,
                   const Function* F,
                   const TypeVector& args) const
{
...
const Function*
Context::matchFunction(const FunctionVector& functions,
                       ...
                       MatchType& matchType) const
{
	...
	{
	    F->resolve();
	    if (F->symbolState() != Symbol::ResolvedState) continue;
	}
        ...
        if (argTypes.size() < minArgs || argTypes.size() > maxArgs) continue;
    ...
    {
        if (F->numArgs() != argTypes.size())    matchType = BestPartialMatch;
        else if (choice.permutation.empty())    matchType = ExactMatch;
        else                                    matchType = TypeMatch;
    }
    else if (F->numArgs() != argTypes.size())   matchType = BestPartialMatch;
    else if (choice.permutation.empty())        matchType = BestMatch;
    else                                        matchType = TypeMatch;
...
const Function* 
Context::matchSpecializedFunction(Process* p, Thread* thread,
                                  const FunctionVector& fvec,
...
const Function*
Context::specializeFunction(Process* p,
                            Thread* thread,
...
//----------------------------------------------------------------------
TypedValue
Context::evalFunction(Process* p, Function* F, ArgumentVector& args)
{
...
void
Context::parseFile(Process* process, const char* filename)
{
...
void
Context::parseStdin(Process* process, const char* inputName)
{
    if (!inputName) inputName = "Standard Input";
    parseStream(process, cin, inputName);
}

void
Context::parseStream(Process*, istream&, const char* inputName)
{
    abort();
}
...
TupleType*
Context::tupleType(const TypeVector& types)
{
    PrimaryBit fence(this, false);
    String tupleName("(");
...
StructType*
Context::structType(Symbol* root,
                    const char* n,
...
ListType*
Context::listType(const Type* elementType)
{
    PrimaryBit fence(this, false);
    String name = "[";
...
void
Context::separateName(Name name, NameVector& names) const
{
...
Context::encodeName(Name n)
{
    ...
    for (int i=0; i < s.size(); i++)
    {
        switch (s[i])
        {
          ...
          case '@':     o += "At_";      break;
          case '#':     o += "Pound_";      break;
          case '$':     o += "Dollar_";      break;
          case '^':     o += "Caret_";      break;
          case '&':     o += "Amp_";      break;
          case '*':     o += "Star_";      break;
          case '(':     o += "BParen_";      break;
          case ')':     o += "EParen_";      break;
          case '=':     o += "EQ_";      break;
          case '{':     o += "BCB_";      break;
          case '}':     o += "ECB_";      break;
          case '[':     o += "BSB_";      break;
          case ']':     o += "ESB_";      break;
          case '|':     o += "Pipe_";      break;
          case '\\':    o += "BSlash_";      break;
          case ':':     o += "Colon_";      break;
          case ';':     o += "SColon_";      break;
          case '"':     o += "Quote_";      break;
          case '\'':    o += "SQuote_";      break;
          case '<':     o += "LT_";      break;
          case '>':     o += "GT_";      break;
          case ',':     o += "Comma_";      break;
          case '?':     o += "QMark_";      break;
...
Name 
Context::uniqueName(const Symbol* scope, const char* prefix)
{
    bool found = true;
    Name name;
...
void
Context::setShowConstructed(bool b) { showConstructed = b; }

void                
Context::symbolConstructed(const Symbol* s)
{
...
void                
Context::symbolDestroyed(const Symbol* s)
{

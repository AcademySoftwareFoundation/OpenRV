...
NodePrinter::NodePrinter(Node *n, ostream& o, NodePrinter::Style s) 
    : NodeVisitor(n), _ostream(&o), _style(s), _state(0) {}
NodePrinter::NodePrinter(Node *n, ostream& o, Type::ValueOutputState& state, NodePrinter::Style s) 
    : NodeVisitor(n), _ostream(&o), _style(s), _state(&state) {}
NodePrinter::~NodePrinter() {}

void
NodePrinter::preOrderVisit(Node *node, int depth)
{
    if (_style == Lispy)
    {
...
void
NodePrinter::postOrderVisit(Node *node, int depth)
{
    if (_style == Tree)
    {
...
void
NodePrinter::childVisit(Node *parent, Node* child, size_t)
{
    if (_style == Lispy)
    {
	out() << " ";

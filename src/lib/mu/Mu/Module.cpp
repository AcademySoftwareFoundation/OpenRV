...
void 
Module::setCompileOnDemand(bool muc, bool mud)
{
...
void 
Module::setDebugArchive(bool b)
{
    _debugArchive = b;
}

int
Module::findDSOModule(const String& filename)
{
    for (int i=0; i < Module::_dsoModules.size(); i++)
    {
	const DSOInterface &m = Module::_dsoModules[i];
	if (m._filename == filename) return i;
    }
...
Module*
Module::loadDSO(const String& soName, 
                Name name,
                int i,
                Process* process,
                Context* context)
{
    ...
    if (dsoInterface._handle = LoadLibrary((LPCTSTR)uSoName.c_str()))
    {
...
#endif
        {
            ...
            if (Module *m = (*dsoInterface._init)(name.c_str(), 
                                               context,
                                               process))
            {
        ...
        {
            cerr << "WARING: there is a bogus compiled module at "
                 << soName << endl;
...
Module*
Module::loadMUC(const String& mucName, 
                Name name,
                Process* process,
                Context* context)
{
    ...
    if (infile)
    {
        try
        {
            ...
            if (!mainModule) mainModule = new Module(context, name.c_str());
...
Module*
Module::loadSource(const String& muName,
                   Name name,
                   Process* process,
                   Context* context)
{
    ...
    if (m) m->_location = muName;
    return m;
}

const String&
Module::findAssociatedDocFile() const
{
    if (_checkedDocs) return _mudfile;
    ...
    if (_location != "")
    {
        ...
        if (pos != string::npos)
        {
            ...
            if (fileOK(filename)) _mudfile = filename;
        }
    }
    ...
    if (_mudfile == "")
    {
        ...
        for (int i=0; i < path.size(); i++)
        {
            ...
            if (testpath[testpath.size()-1] != '/') testpath += "/";
...
bool 
Module::hasDocFile() const
{
    if (_loadedDocs) return true;
    if (!_checkedDocs) findAssociatedDocFile();
    return _mudfile != "";
}

bool
Module::loadDocs(Process* process, Context* context) const
{
    if (_loadedDocs) return true;
    if (!_checkedDocs) findAssociatedDocFile();
    if (_mudfile == "") return false;
...
bool
Module::fileOK(const String& filename)
{
#if defined(_MSC_VER)
...
Module*
Module::load(Name name, Process* process, Context* context)
{
    if (Module* m = 

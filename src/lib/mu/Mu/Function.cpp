...
Function::Function(Context* context, const char *name)  : Symbol(context, name) {}
...
void
Function::init(NodeFunc func, Attributes attrs, va_list &ap)
{
    //
    ...
    if ( !(attrs & NonNative) ) attrs |= Native;
    ...
    for (int i=0; i < typeNames.size(); i++) sig->push_back(typeNames[i]);
}
...
Function::Attributes
Function::baseAttributes() const
{
    Attributes a = 0;

    if (isOperator())               a |= Operator;
    if (isMemberOperator())         a |= MemberOperator;
    if (isCast())                   a |= Cast;
    if (isCommutative())            a |= Commutative;
    if (isMapped())                 a |= Mapped;
    if (isLossy())                  a |= Lossy;
    if (!hasSideEffects())          a |= NoSideEffects;
    if (isRetaining())              a |= Retaining;
    if (isDynamicActivation())      a |= DynamicActivation;
    if (isLambda())                 a |= LambdaExpression;
    if (hasHiddenArgument())        a |= HiddenArgument;
    if (hasDependentSideEffects())  a |= DependentSideEffects;

    return a;
}

NodeFunc
Function::func(Node*) const
{
...
Function::type() const
{
    if (!_type)
    {
        if (symbolState() != ResolvedState) resolve();
...
{
    if (symbolState() != ResolvedState) resolve();
    if (symbolState() != ResolvedState) return 0;
    return _signature;
}

void
Function::symbolDependancies(ConstSymbolVector& symbols) const
{
    if (symbolState() != ResolvedState) resolve();
    symbols.push_back(returnType());
    for (int i=0, s=numArgs(); i < numArgs(); i++) symbols.push_back(argType(i));
}

void
Function::addSymbol(Symbol* s)
{
    //
    ...
    if (ParameterVariable* pv = dynamic_cast<ParameterVariable*>(s))
    {
        ...
        if (pv->storageClass() && 
            pv->storageClass()->isTypeVariable() && 
            !native())
        {
            _polymorphic = true;
        }
...
void
Function::output(std::ostream &o) const
{
    if (symbolState() != ResolvedState) resolve();
    ...
    else
    {
        for (int i=0; i < _signature->size(); i++)
        {
            ...
            if (t->isTypePattern()) 
            {
                ...
                if (ptype->variadic()) _variadic = true;
            }
...
int
Function::numArgs() const
{
    return _signature->size() - 1;
}

QualifiedName
Function::argTypeName(int i) const
{
    if (symbolState() == ResolvedState)
    {
...
QualifiedName
Function::returnTypeName() const
{
    return argTypeName(-1);
}

const Type*
Function::argType(int i) const
{
    if (symbolState() != ResolvedState) resolve();
    if (symbolState() != ResolvedState) return 0;
...
bool
Function::expandArgTypes(TypeVector& types, size_t size) const
{
    if (symbolState() != ResolvedState) resolve();
    if (symbolState() != ResolvedState) return false;
...
const Type*
Function::nodeReturnType(const Node*) const
{
...
bool
Function::matches(const Function* f) const
{
    if (name() == f->name())
    {
	int n = numArgs();
...
const Function*
Function::firstFunctionOverload() const
{
...
const Function*
Function::nextFunctionOverload() const
{
...
bool
Function::isFunctionOverloaded() const
{
...
purity(const Function* self, const Node* n)
{
    if (!n) return Function::None;
    ...
    if (const Function* f = dynamic_cast<const Function*>(n->symbol()))
    {
        ...
        if (f == self) return Function::Pure;
        ...
        if (f->isPure() || f->maybePure())
        {
            ...
            if (maybe) return Function::MaybePure;
...
void
Function::setReturnType(const Type* t)
{
    //
...
Function::setBody(Node* n)
{
    ...
    if (sig.resolved())
    {
        if (!sig[0].symbol) setReturnType(_code ? _code->type() : 0);
    }
    else
    {

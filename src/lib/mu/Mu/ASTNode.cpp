...
ASTAssign::ASTAssign(NodeAssembler& as,
                     const Symbol* s,
                     Node* lhs,
                     Node* rhs)
...
Node*
ASTAssign::resolve(NodePatch& visitor)
{
    ...
    {
        as.freportError(this,
                        "Failed to resolve assignment from \"%s\" to \"%s\".",
                        rhs->type()->fullyQualifiedName().c_str(),
                        lhsType ? lhsType->fullyQualifiedName().c_str() : "?");
    }
...
Node*
ASTCall::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
    ...
    for (const ScopeState* ss = scope; ss; ss = ss->parent)
    {
        ...
        if (const Function* f =
            ss->symbol->findSymbolOfTypeByQualifiedName<Function>(uname))
        {
            for (const Function* fo = f->firstFunctionOverload();
                 fo;
                 fo = fo->nextFunctionOverload())
                    ...
                    if (fo->scope() == f->scope())
                    {
                        ...
                        if (nl.size() == fo->numArgs() - 1)
                        {
                            ...
                            ParameterVariable* pthis = 
                                visitor.function()->findSymbolOfType<ParameterVariable>(thisname);
...
Node*
ASTCast::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
    const Symbol* s = symbol();
    Name uname   = name;
...
Node*
ASTMemberCall::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
    Name n = name;
...
Node*
ASTMemberReference::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
    Name uname   = name;
...
Node*
ASTDereference::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
...
Node*
ASTStackReference::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
    ...
    if (dereference)
    {
        ...
        {
            as.freportError(this,
                            "Unresolved stack dereference of \"%s\" (probably an internal error)",
...
Node*
ASTReference::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
    Name uname   = name;
    ...
    if (inclass)
    {
        if (const MemberVariable* v =
            visitor.function()->scope()->findSymbolOfType<MemberVariable>(uname))
        {
            Name thisname = as.context()->internName("this");
            ParameterVariable* pthis = 
                visitor.function()->findSymbolOfType<ParameterVariable>(thisname);
            ...
            if (pthis)
            {
                if (Node* prefix = as.dereferenceVariable(pthis))
                {
                 ...
                 visitor.function()->scope()->findSymbolOfType<MemberFunction>(uname))
        {
            Name thisname = as.context()->internName("this");
            ParameterVariable* pthis = 
                visitor.function()->findSymbolOfType<ParameterVariable>(thisname);
    ...
    for (const ScopeState* ss = scope; ss; ss = ss->parent)
    {
        if (const Variable* var = 
            ss->symbol->findSymbolOfTypeByQualifiedName<Variable>(uname))
        {
...
Node*
ASTDeclaration::resolve(NodePatch& visitor)
{
    return this;
}

Node*
ASTStackDeclaration::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
...
Node*
ASTListConstructor::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
...
Node*
ASTTupleConstructor::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
    NodeAssembler::NodeList nl = as.emptyNodeList();
...
Node*
ASTIndexMember::resolve(NodePatch& visitor)
{
    NodeAssembler& as = visitor.as();
    NodeAssembler::NodeList nl = as.emptyNodeList();
...
void
ASTForEach::childVisit(NodePatch& visitor, Node* child, size_t index)
{
    NodeAssembler& as = visitor.as();
        ...
        if (stype->isCollection())
        {
            if (vtype->isUnresolvedType())
            {
                if (const Mu::Type* ftype = stype->fieldType(0))
                {
                    ...
                    StackVariable* svar = new StackVariable(as.context(),
                                                            astdecl->name.c_str(), 
                                                            ftype,
                                                            astdecl->dummy->address(),
                                                            rwi);
...
Node* 
ASTForEach::resolve(NodePatch& visitor)
{

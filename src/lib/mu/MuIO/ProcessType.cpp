...
Object*
ProcessType::newObject() const
{
    return new PipeStream(this);
}

void
ProcessType::deleteObject(Object *obj) const
{
    delete static_cast<ProcessType::PipeStream*>(obj);
}

void 
ProcessType::outputValue(ostream &o, const Value &value, bool full) const
{
...
void 
ProcessType::outputValueRecursive(ostream &o,
...
ProcessType::load()
{
    ...
    s->addSymbols( new ReferenceType(c, r, this), 
		   ...
		   new Function(c, n, BaseFunctions::dereference, Cast,
				...
				Args, rn, End),
                   ...
                   new Function(c, n, ProcessType::construct, None,
                                Return, tn,
                                Args, "string", "[string]", "int64",
                                End),
    ...
    globalScope()->
        addSymbols( new Function(c, "=", BaseFunctions::assign, AsOp,
                    ...
                    new Function(c, "bool", ProcessType::toBool, None,
                                 Return, "bool",
                                 Args, tn, End),
                    
                    EndArguments);

    addSymbols( new Function(c, "in", ProcessType::inStream, None,
                             Return, "io.ostream",
                             Args, tn, End),

                new Function(c, "out", ProcessType::outStream, None,
                             Return, "io.istream",
                             Args, tn, End),
                 
                new Function(c, "err", ProcessType::errStream, None,
                             Return, "io.istream",
                             Args, tn, End),

                new Function(c, "close", ProcessType::close, None,
                             Return, "bool",
                             Args, tn, End),
                ...
                new Function(c, "kill", ProcessType::kill, None,
                             Return, "void",
                             Args, tn, End),
...
NODE_IMPLEMENTATION(ProcessType::construct, Pointer)
{
    ...
    for (List list(p, NODE_ARG_OBJECT(1, ClassInstance));
         list.isNotNil();
         list++)
    {
...
NODE_IMPLEMENTATION(ProcessType::toBool, bool)
{
    ...
    if (!pstream) throw NilArgumentException(NODE_THREAD);
...
NODE_IMPLEMENTATION(ProcessType::inStream, Pointer)
{
    PipeStream*  pstream = NODE_ARG_OBJECT(0, PipeStream);
    if (!pstream) throw NilArgumentException(NODE_THREAD);
    if (!pstream->exec_stream) throw BadArgumentException(NODE_THREAD);
...
NODE_IMPLEMENTATION(ProcessType::close, bool)
{
    PipeStream*  pstream = NODE_ARG_OBJECT(0, PipeStream);
    if (!pstream) throw NilArgumentException(NODE_THREAD);
    if (!pstream->exec_stream) NODE_RETURN(true);
...
NODE_IMPLEMENTATION(ProcessType::exit_code, int)
{
    PipeStream*  pstream = NODE_ARG_OBJECT(0, PipeStream);
    if (!pstream) throw NilArgumentException(NODE_THREAD);
    if (!pstream->exec_stream) return 0;

...
// Helper function to detect if an image is actually a tile in a tiled-source.
// For safety, it first checks if source-tiling is enabled, then it checks if
// the image has the tile token in its name (eg.:
// sourceGroup000000_source/tile_x0_y1.0//1
// If the image is a tile, it truncates the sourceName so it does not include
// the tile name (eg.: sourceGroup000000_source).
...
NODE_IMPLEMENTATION(setSessionName, void)
{
    ...
    if (!name) throwBadArgumentException(NODE_THIS, NODE_THREAD, "setSessionName: nil node name");
...
NODE_IMPLEMENTATION(setOutPoint, void)
{
    ...
    int newOut = f+1, oldIn = s->inPoint();
    {
        ...
        if (newOut < oldIn) s->setInPoint(newOut - (s->outPoint()-oldIn));
...
NODE_IMPLEMENTATION(setInPoint, void)
{
    ...
    {
        IPGraph::GraphEdit  edit(s->graph());
        if (f > oldOut) s->setOutPoint(f + (oldOut-s->inPoint()));
        s->setInPoint(f);
    }
    s->askForRedraw(true);
}
...
NODE_IMPLEMENTATION(setPlayMode, void)
{
    ...
    switch (m)
    {
      case 0: s->setPlayMode(Session::PlayLoop); break;
      case 1: s->setPlayMode(Session::PlayOnce); break;
      case 2: s->setPlayMode(Session::PlayPingPong); break;
    }
}
...
NODE_IMPLEMENTATION(playMode, int)
{
    ...
    switch (s->playMode())
    {
      case Session::PlayLoop: rval = 0; break;
      case Session::PlayOnce: rval = 1; break;
      case Session::PlayPingPong: rval = 2; break;
    }
...
NODE_IMPLEMENTATION(loadChangedFrames, void)
{
	...
	if (!n) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil sourceName");

        if (IPNode* node = s->graph().findNode(n->c_str()))
        {
            nodes.push_back(node);
        }
        else throwBadArgumentException(NODE_THIS, NODE_THREAD, "sourceName does not exist");
    }
    ...
    for (int i = 0; i < nodes.size(); ++i)
    {
        ...
        else throwBadArgumentException(NODE_THIS, NODE_THREAD, "given sourceName is not an RVFileSource");
    }

    //  If we had caching on, turn it back on here.  At this point any
    //  newly-required framebuffers for frames that were previously cached will
    //  be cached again.
...
NODE_IMPLEMENTATION(metaEvaluate, Pointer)
{
    ...
    {
        if (!rootNode) rootNode = s->graph().root();
        if (rootNode)
        {
    ...
    struct Info { Pointer nodeName; Pointer nodeType; int frame; };
    ...
    if (infos.size() > 0)
    {
        ...
        {
            //
            //  Construct new list of unique Infos, but with order preserved.
            //
...
NODE_IMPLEMENTATION(metaEvaluate2, Pointer)
{
    ...
    if (!typeName) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil type name");
...
NODE_IMPLEMENTATION(mapPropertyToGlobalFrames, Pointer)
{
    ...
    for (size_t i = 0; i < frames.size(); i++) frameSet.insert(frames[i]);
    ...
    for (set<int>::const_iterator i = frameSet.begin();
         i != frameSet.end();
         ++i)
    {
...
NODE_IMPLEMENTATION(resetMbps, void)
{
    TwkUtil::FileStream::resetMbps();
}
...
NODE_IMPLEMENTATION(markFrame, void)
{
    ...
    if (mark) s->markFrame(frame);
    else s->unmarkFrame(frame);
}
                               ...
                               PixelImageVector& pixelImages)
{
    for (size_t i = 0; i < images.size(); ++i)
    {
        ...
        //
        const IPImage::Matrix IP = image.projectionMatrix(0,0) == image.projectionMatrix(0,0) ? image.projectionMatrix : IPImage::Matrix();
        ...
        else
        {
            ...
            if (p.x < xmin)
            {
                ...
                else if (p.y >= ymax)   p0 = Vec3f(xmin, ymax, 0);
                else                    p0 = Vec3f(xmin, p.y, 0);
            }
            else if (p.x > xmax)
            {
                if (p.y <= ymin)        p0 = Vec3f(xmax, ymin, 0);
                else if (p.y >= ymax)   p0 = Vec3f(xmax, ymax, 0);
                else                    p0 = Vec3f(xmax, p.y, 0);
            }
            else
            {
...
NODE_IMPLEMENTATION(inputAtPixel, Pointer)
{
    ...
    for (size_t i = 0; i < images.size(); ++i)
    {
        ...
        while (imageNode->group()) imageNode = imageNode->group();
    ...
    if (closestInput) inputName = stype->allocate(closestInput->name());
...
NODE_IMPLEMENTATION(imagesAtPixel, Pointer)
{
    ...
    if (sourcesOnly)
    {
        ...
        for (size_t i = 0; i < images.size(); i++)
        {
            ...
            if (ps.inside) wantedOnes.push_back(i);
            if (ps.edgeDistance < closestEdge)
            {
                closestEdge = ps.edgeDistance;
                closestIndex = i;
...
NODE_IMPLEMENTATION(imageGeometryByTag, Pointer)
{
    ...
    if (!name || !value) throwBadArgumentException(NODE_THIS, NODE_THREAD, "missing tag name or value");
...
NODE_IMPLEMENTATION(setSessionFileName, void)
{
    ...
    if (!name) throwBadArgumentException(NODE_THIS, NODE_THREAD, "setSessionFileName: nil file name");
...
NODE_IMPLEMENTATION(readProfile, void)
{
    ...
    if (!nodename) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil node argument");
    if (!filename) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil file name argument");
...
NODE_IMPLEMENTATION(saveSession, void)
{
    ...
    if (getenv("RV_COMPLETE_SESSION_FILES")) sparse = false;
...
NODE_IMPLEMENTATION(newSession, void)
{
    ...
    if (s->multipleVideoDevices())
    {
        throwBadArgumentException(NODE_THIS,
                                  NODE_THREAD,
                                  "newSessions cannot be created when presentation mode is active");
    }
...
NODE_IMPLEMENTATION(setFloatProperty, void)
{
    ...
    for (int i=0; i < props.size(); i++)
    {
        ...
        if (node) node->propertyWillChange(prop);
        if (allowResize) prop->resize(newValues->size() / width);
...
NODE_IMPLEMENTATION(setStringProperty, void)
{
    ...
    for (int i=0; i < props.size(); i++)
    {
        ...
        bool throwBad = false;
        if (node) node->propertyWillChange(prop);
        if (allowResize) prop->resize(newValues->size());
        ...
        if (node) node->propertyChanged(prop);
        if (throwBad) throwBadPropertyType(NODE_THREAD, NODE_THIS, name->c_str());
    }
...
NODE_IMPLEMENTATION(fileKind, int)
{
    ...
    if (!name) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil file");
...
NODE_IMPLEMENTATION(bgMethod, Pointer)
{
    ...
    if (s && s->renderer())
    {
        ...
        {
          default:
          case ImageRenderer::Solid0:     n = "black";      break;
          case ImageRenderer::Solid100:   n = "white";      break;
          case ImageRenderer::Solid18:    n = "grey18";     break;
          case ImageRenderer::Solid50:    n = "grey50";     break;
          case ImageRenderer::Checker:    n = "checker";    break;
          case ImageRenderer::CrossHatch: n = "crosshatch"; break;
        }
    }
...
NODE_IMPLEMENTATION(setBGMethod, void)
{
    ...
    if (!method) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil bg method");

    ImageRenderer::BGPattern bg = ImageRenderer::Solid0;

    if      (*method == "black")      { bg = ImageRenderer::Solid0; }
    else if (*method == "white")      { bg = ImageRenderer::Solid100; }
    else if (*method == "grey18")     { bg = ImageRenderer::Solid18; }
    else if (*method == "grey50")     { bg = ImageRenderer::Solid50; }
    else if (*method == "checker")    { bg = ImageRenderer::Checker; }
    else if (*method == "crosshatch") { bg = ImageRenderer::CrossHatch; }
...
{
    if (!name) throwBadArgumentException(node, thread, "nil property name");
    ...
    switch (Property::Layout(type))
    {
      case Property::IntLayout:
          switch (dimensions.x)
          {
            ...
            case 2: prop = object->createProperty<Vec2iProperty>(tokens[1], tokens[2]); break;
            case 3: prop = object->createProperty<Vec3iProperty>(tokens[1], tokens[2]); break;
            case 4: prop = object->createProperty<Vec4iProperty>(tokens[1], tokens[2]); break;
          }
          break;
      case Property::FloatLayout:
          switch (dimensions.x)
          {
            case 1: prop = object->createProperty<FloatProperty>(tokens[1], tokens[2]); break;
            case 2: prop = object->createProperty<Vec2fProperty>(tokens[1], tokens[2]); break;
            case 3: prop = object->createProperty<Vec3fProperty>(tokens[1], tokens[2]); break;
            case 4: prop = object->createProperty<Vec4fProperty>(tokens[1], tokens[2]); break;
            default: break;
          }
          break;
      case Property::HalfLayout:
          switch (dimensions.x)
          {
            case 1: prop = object->createProperty<HalfProperty>(tokens[1], tokens[2]); break;
            case 2: prop = object->createProperty<Vec2hProperty>(tokens[1], tokens[2]); break;
            case 3: prop = object->createProperty<Vec3hProperty>(tokens[1], tokens[2]); break;
            case 4: prop = object->createProperty<Vec4hProperty>(tokens[1], tokens[2]); break;
            default: break;
          }
          break;
      ...
      case Property::ByteLayout:
          ...
          {
            default:
            case 1: prop = object->createProperty<ByteProperty>(tokens[1], tokens[2]); break;
            case 2: prop = object->createProperty<Vec2ucProperty>(tokens[1], tokens[2]); break;
            case 3: prop = object->createProperty<Vec3ucProperty>(tokens[1], tokens[2]); break;
      ...
      case Property::ShortLayout:
          switch (dimensions.x)
          {
            ...
            case 2: prop = object->createProperty<Vec2usProperty>(tokens[1], tokens[2]); break;
            case 3: prop = object->createProperty<Vec3usProperty>(tokens[1], tokens[2]); break;
            case 4: prop = object->createProperty<Vec4usProperty>(tokens[1], tokens[2]); break;
            default: break;
          }
          break;
      default:
    ...
    if (object) object->newPropertyCreated(prop);
}
...
NODE_IMPLEMENTATION(deleteProperty, void)
{
    ...
    if (!name) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil property name");
    ...
    {
        Property* p = props[i];
        if (object) object->propertyWillBeDeleted(p);

        //  Causes unref which causes deletion if required
        if (p->container()) p->container()->removeProperty(p);
    }

    if (object) object->propertyDeleted(name->c_str());
}
...
NODE_IMPLEMENTATION(nodes, Pointer)
{
    ...
    for (IPGraph::NodeMap::const_iterator i = map.begin();
         i != map.end();
         ++i, count++)
    {
...
NODE_IMPLEMENTATION(nodeType, Pointer)
{
    ...
    if (!node) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil node name");
...
NODE_IMPLEMENTATION(nextViewNode, Pointer)
{
    ...
    if (next.empty()) NODE_RETURN(0);
    else NODE_RETURN(c->stringType()->allocate(next));
}
...
NODE_IMPLEMENTATION(previousViewNode, Pointer)
{
    ...
    if (prev.empty()) NODE_RETURN(0);
    else NODE_RETURN(c->stringType()->allocate(prev));
}
...
NODE_IMPLEMENTATION(setViewNode, void)
{
    ...
    if (!name) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil view name");
...
NODE_IMPLEMENTATION(newNode, Pointer)
{
    ...
    if (!type) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil node type");

    string nodeName = "";
    if (name) nodeName = name->c_str();
...
NODE_IMPLEMENTATION(setNodeInputs, void)
{
	...
	if (!n) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil in input array");
...
NODE_IMPLEMENTATION(testNodeInputs, Pointer)
{
	...
	if (!n) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil in inputs array");
...
NODE_IMPLEMENTATION(existingFilesInSequence, Pointer)
{
    ...
    if (!seq) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil sequence string");

    ExistingFileList list = TwkUtil::existingFilesInSequence(seq->c_str(), false);

    int existingCount = 0;
    for (int i = 0; i < list.size(); ++i) if (list[i].exists) ++existingCount;
...
NODE_IMPLEMENTATION(ioFormats, Pointer)
{
    ...
    for (Plugins::const_iterator i = plugins.begin();
         i != plugins.end();
         ++i)
    {
        const MovieIO* io = *i;
...
NODE_IMPLEMENTATION(ioParameters, Pointer)
{
    ...
    if (!ext)   throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil extension");
    if (!codec) throwBadArgumentException(NODE_THIS, NODE_THREAD, "nil codec");
...
NODE_IMPLEMENTATION(videoState, Pointer)
{
    ...
    if (!session->multipleVideoDevices()) NODE_RETURN(Pointer(0));
...
NODE_IMPLEMENTATION(sourceMediaInfo, Mu::Pointer)
{
    ...
    if (!tokens.empty())
    {
        if (SourceIPNode* snode = s->graph().findNodeOfType<SourceIPNode>(name->c_str()))
        {
            ...
            {
                throwBadArgumentException(NODE_THIS, NODE_THREAD, "Source is not active. A source can be activated by the Multiple Media Representations drop down menu or via the setActiveSourceMediaRep() command.");
            }
...
NODE_IMPLEMENTATION(hopProfDynName, void)
{
    ...
    if (!name) throwBadArgumentException(NODE_THIS, NODE_THREAD, "hopProfDynName: nil node name");
...
{
    // Note: This command is no longer relevant in RV Open Source but has been
...
void initCommands(Mu::MuLangContext* context)
{
    ...
    //
    //  WARNING: sourceMediaInfo() uses fieldType(16) to access the view info
    //  array, so if you change this struct you potentially need to modify that code
    //
    ...
    commands->addSymbols(
                         ...
                         new Function(c, "sessionNames", sessionNames, None,
                                      Return, "string[]",
                                      End),
                         ...
                         new Function(c, "markFrame", markFrame, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "frame", "int"),
                         ...
                         new Function(c, "isMarked", isMarked, None,
                                      Return, "bool",
                                      Parameters,
                                      new Param(c, "frame", "int"),
                                      End),

                         new Function(c, "markedFrames", markedFrames, None,
                                      Return, "int[]",
                                      End),

                         new Function(c, "scrubAudio", scrubAudio, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "on", "bool"),
                                      new Param(c, "chunkDuration", "float", Value(0.0f)),
                                      new Param(c, "loopCount", "int", Value(int(0))),
                                      End),

                         new Function(c, "play", play, None,
                                      Return, "void",
                                      End),

                         new Function(c, "redraw", redraw, None,
                                      Return, "void",
                                      End),

                         new Function(c, "clearAllButFrame", clearAllButFrame, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "frame", "int"),
                                      End),
                         ...
                         new Function(c, "isPlaying", isPlaying, None,
                                      Return, "bool",
                                      End),

                         new Function(c, "stop", stop, None,
                                      Return, "void",
                                      End),

                         new Function(c, "frame", frame, None,
                                      Return, "int",
                                      End),

                         new Function(c, "metaEvaluate", metaEvaluate, None,
                                      Return, "MetaEvalInfo[]",
                                      Parameters,
                                      new Param(c, "frame", "int"),
                                      new Param(c, "root", "string", Value(Pointer(0))),
                                      new Param(c, "leaf", "string", Value(Pointer(0))),
                                      new Param(c, "unique", "bool", Value(true)),
                                      End),
                         ...
                         new Function(c, "mapPropertyToGlobalFrames", mapPropertyToGlobalFrames, None,
                                      Return, "int[]",
                                      Parameters,
                                      new Param(c, "propName", "string"),
                         ...
                         new Function(c, "closestNodesOfType", closestNodesOfType, None,
                                      ...
                                      new Param(c, "typeName", "string"),
                                      new Param(c, "root", "string", Value(Pointer(0))),
                                      new Param(c, "depth", "int", Value(int(0))),
                                      End),

                         new Function(c, "frameStart", frameStart, None,
                                      Return, "int",
                                      End),

                         new Function(c, "frameEnd", frameEnd, None,
                                      Return, "int",
                                      End),
                         ...
                         new Function(c, "narrowedFrameEnd", narrowedFrameEnd, None,
                                      Return, "int",
                                      End),

                         new Function(c, "narrowToRange", narrowToRange, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "frameStart", "int"),
                                      new Param(c, "frameEnd", "int"),
                                      End),

                         new Function(c, "setRealtime", setRealtime, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "realtime", "bool"),
                                      End),

                         new Function(c, "isRealtime", isRealtime, None,
                                      Return, "bool",
                                      End),
                         ...
                         new Function(c, "inPoint", inPoint, None,
                                      Return, "int",
                                      End),

                         new Function(c, "outPoint", outPoint, None,
                                      Return, "int",
                                      End),

                         new Function(c, "setInPoint", setInPoint, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "frame", "int"),
                                      End),

                         new Function(c, "setOutPoint", setOutPoint, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "frame", "int"),
                                      End),

                         new Function(c, "setMargins", setMargins, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "margins", "vector float[4]"),
                                      new Param(c, "allDevices", "bool", Value(false)),
                                      End),

                         new Function(c, "margins", margins, None,
                                      Return, "vector float[4]",
                                      End),

                         new Function(c, "setPlayMode", setPlayMode, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "mode", "int"),
                                      End),

                         new Function(c, "playMode", playMode, None,
                                      Return, "int",
                                      End),

                         new Function(c, "setFPS", setFPS, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "fps", "float"),
                                      End),

                         new Function(c, "setFrameStart", setFrameStart, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "frame", "int"),
                                      End),

                         new Function(c, "setFrameEnd", setFrameEnd, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "frame", "int"),
                                      End),

                         new Function(c, "realFPS", realFPS, None,
                                      Return, "float",
                                      End),

                         new Function(c, "fps", fps, None,
                                      Return, "float",
                                      End),

                         new Function(c, "mbps", mbps, None,
                                      Return, "float",
                                      End),

                         new Function(c, "resetMbps", resetMbps, None,
                                      Return, "void",
                                      End),

                         new Function(c, "setInc", setInc, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "inc", "int"),
                                      End),

                         new Function(c, "setFiltering", setFiltering, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "filterType", "int"),
                                      End),

                         new Function(c, "getFiltering", getFiltering, None,
                                      Return, "int",
                                      End),

                         new Function(c, "setCacheMode", setCacheMode, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "mode", "int"),
                                      End),

                         new Function(c, "releaseAllCachedImages", releaseAllCachedImages, None,
                                      Return, "void",
                                      End),

                         new Function(c, "releaseAllUnusedImages", releaseAllUnusedImages, None,
                                      Return, "void",
                                      End),
                         ...
                         new Function(c, "audioCacheMode", audioCacheMode, None,
                                      Return, "int",
                                      End),

                         new Function(c, "cacheMode", cacheMode, None,
                                      Return, "int",
                                      End),
                         ...
                         new Function(c, "setCacheOutsideRegion", setCacheOutsideRegion, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "cacheOutside", "bool"),
                                      End),

                         new Function(c, "isCaching", isCaching, None,
                                      Return, "bool",
                                      End),

                         new Function(c, "cacheInfo", cacheInfo, None,
                                      Return, "(int64,int64,int64,float,float,float,int[])",
                                      End),

                         new Function(c, "audioCacheInfo", audioCacheInfo, None,
                                      Return, "(float,int[])",
                                      End),

                         new Function(c, "isBuffering", isBuffering, None,
                                      Return, "bool",
                                      End),

                         new Function(c, "inc", inc, None,
                                      Return, "int",
                                      End),

                         new Function(c, "cacheSize", cacheSize, None,
                                      Return, "int",
                                      End),

                         new Function(c, "fullScreenMode", fullScreenMode, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "active", "bool"),
                                      End),

                         new Function(c, "isFullScreen", isFullScreen, None,
                                      Return, "bool",
                                      End),

                         new Function(c, "inputAtPixel", inputAtPixel, None,
                                      Return, "string",
                                      Parameters,
                                      new Param(c, "point", "vector float[2]"),
                                      new Param(c, "strict", "bool", Value(true)),
                                      End),

                         new Function(c, "eventToImageSpace", event2image, None,
                         ...
                         new Function(c, "imageToEventSpace", image2event, None,
                                      Return, "vector float[2]",
                                      Parameters,
                                      new Param(c, "sourceName", "string"),
                                      new Param(c, "point", "vector float[2]"),
                         ...
                         new Function(c, "eventToCameraSpace", event2camera, None,
                                      Return, "vector float[2]",
                                      Parameters,
                                      new Param(c, "sourceName", "string"),
                                      new Param(c, "point", "vector float[2]"),
                         ...
                         new Function(c, "imagesAtPixel", imagesAtPixel, None,
                         ...
                         new Function(c, "sourceMediaInfo", sourceMediaInfo, None,
                         ...
                         new Function(c, "sourcesAtFrame", sourcesAtFrame, None,
                                      Return, "string[]",
                                      Parameters,
                                      new Param(c, "frame", "int"),
                                      End),

                         new Function(c, "renderedImages", renderedImages, None,
                                      Return, "RenderedImageInfo[]",
                                      End),

                         new Function(c, "imageGeometry", imageGeometry, None,
                                      Return, "(vector float[2])[]",
                                      Parameters,
                                      new Param(c, "name", "string"),
                                      new Param(c, "useStencil", "bool", Value(true)),
                                      End),
                         ...
                         new Function(c, "sessionFileName", sessionFileName, None,
                                      Return, "string",
                                      End),
                         ...
                         new Function(c, "undoPathSwapVars", undoPathSwapVars, None,
                                      Return, "string",
                                      Parameters,
                                      new Param(c, "pathWithVars", "string"),
                                      End),

                         new Function(c, "redoPathSwapVars", redoPathSwapVars, None,
                                      Return, "string",
                                      Parameters,
                                      new Param(c, "pathWithoutVars", "string"),
                                      End),

                         new Function(c, "readProfile", readProfile, None,
                                      Return, "void",
                                      Parameters,
                         ...
                         new Function(c, "writeProfile", writeProfile, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "fileName", "string"),
                                      new Param(c, "node", "string"),
                         ...
                         new Function(c, "saveSession", saveSession, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "fileName", "string"),
                                      new Param(c, "asACopy", "bool", Value(false)),
                                      new Param(c, "compressed", "bool", Value(false)),
                                      new Param(c, "sparse", "bool", Value(false)),
                                      End),
                         ...
                         new Function(c, "updateNodeDefinition_", updateNodeDefinition, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "defitionName", "string"),
                                      End),

                         new Function(c, "writeNodeDefinition", writeNodeDefinition, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      new Param(c, "fileName", "string"),
                                      new Param(c, "inlineSourceCode", "bool", Value(true)),
                                      End),

                         new Function(c, "writeAllNodeDefinitions", writeAllNodeDefinitions, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "fileName", "string"),
                                      new Param(c, "inlineSourceCode", "bool", Value(true)),
                         ...
                         new Function(c, "nodeTypes", nodeTypes, None,
                                      Return, "string[]",
                                      Parameters,
                                      new Param(c, "userVisibleOnly", "bool", Value(false)),
                                      End),

                         // --

                         new Function(c, "newSession", newSession, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "files", "string[]"),
                                      End),

                         new Function(c, "clearSession", clearSession, None,
                                      Return, "void",
                                      End),
                         ...
                         new Function(c, "getFloatProperty", getFloatProperty,
                                      None,
                                      Return, "float[]",
                                      Parameters,
                         ...
                         new Function(c, "setFloatProperty", setFloatProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "insertFloatProperty", insertFloatProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "getHalfProperty", getHalfProperty,
                                      None,
                                      Return, "half[]",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "setHalfProperty", setHalfProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "insertHalfProperty", insertHalfProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "getIntProperty", getIntProperty,
                                      None,
                                      Return, "int[]",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "setIntProperty", setIntProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "insertIntProperty", insertIntProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "getByteProperty", getByteProperty,
                                      None,
                                      Return, "byte[]",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "setByteProperty", setByteProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "insertByteProperty", insertByteProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "getStringProperty", getStringProperty,
                                      None,
                                      Return, "string[]",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "setStringProperty", setStringProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "insertStringProperty", insertStringProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "newNDProperty", newProperty2,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                                      new Param(c, "propertyType", "int"),
                                      new Param(c, "propertyDimensions", "(int,int,int,int)"),
                                      End),

                         new Function(c, "newProperty", newProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                                      new Param(c, "propertyType", "int"),
                                      new Param(c, "propertyWidth", "int"),
                                      End),

                         new Function(c, "deleteProperty", deleteProperty,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "nodes", nodes, None,
                                      Return, "string[]",
                                      End),

                         new Function(c, "nodeType", nodeType, None,
                                      Return, "string",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      End),

                         new Function(c, "deleteNode", deleteNode, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      End),

                         new Function(c, "properties", properties, None,
                                      Return, "string[]",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      End),

                         new Function(c, "propertyInfo", propertyInfo,
                                      None,
                                      Return, "PropertyInfo",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "propertyExists", propertyExists,
                                      None,
                                      Return, "bool",
                                      Parameters,
                                      new Param(c, "propertyName", "string"),
                         ...
                         new Function(c, "viewSize", viewSize,
                                      None,
                                      Return, "vector float[2]",
                                      End),

                         new Function(c, "bgMethod", bgMethod, None,
                                      Return, "string",
                                      End),

                         new Function(c, "setBGMethod", setBGMethod, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "methodName", "string"),
                                      End),

                         new Function(c, "setRendererType", setRendererType,
                                      None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "name", "string"),
                         ...
                         new Function(c, "getRendererType", getRendererType,
                                      None,
                                      Return, "string",
                                      End),
                         ...
                         new Function(c, "fileKind", fileKind,
                                      None,
                                      Return, "int",
                                      Parameters,
                                      new Param(c, "filename", "string"),
                         ...
                         new Function(c, "audioTextureID", audioTextureID, None,
                                      Return, "int",
                                      End),
                         ...
                         new Function(c, "sendInternalEvent", sendInternalEvent, None,
                                      Return, "string",
                                      Parameters,
                                      new Param(c, "eventName", "string"),
                                      new Param(c, "contents", "string", Value(Pointer(0))),
                                      new Param(c, "senderName", "string", Value(Pointer(0))),
                                      End),

                        new Function(c, "setFilterLiveReviewEvents", setFilterLiveReviewEvents, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "shouldFilterEvents", "bool"),
                                      End),

                        new Function(c, "filterLiveReviewEvents",
                                      filterLiveReviewEvents, None,
                                      Return, "bool",
                                      End),

                         new Function(c, "previousViewNode", previousViewNode, None,
                                      Return, "string",
                                      End),

                         new Function(c, "nextViewNode", nextViewNode, None,
                                      Return, "string",
                                      End),

                         new Function(c, "setViewNode", setViewNode, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      End),

                         new Function(c, "viewNodes", viewNodes, None,
                                      Return, "string[]",
                                      End),

                         new Function(c, "viewNode", viewNode, None,
                                      Return, "string",
                                      End),

                         new Function(c, "nodeImageGeometry", nodeImageGeometry, None,
                                      Return, "NodeImageGeometry",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      new Param(c, "frame", "int"),
                                      End),

                         new Function(c, "nodeRangeInfo", nodeRangeInfo, None,
                                      Return, "NodeRangeInfo",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      End),

                         new Function(c, "newNode", newNode, None,
                                      Return, "string",
                                      Parameters,
                                      new Param(c, "nodeType", "string"),
                                      new Param(c, "nodeName", "string", Value(Pointer(0))),
                                      End),

                         new Function(c, "nodeConnections", nodeConnections, None,
                         ...
                         new Function(c, "nodesInGroup", nodesInGroup, None,
                                      Return, "string[]",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      End),

                         new Function(c, "nodeGroup", nodeGroup, None,
                                      Return, "string",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      End),

                         new Function(c, "nodeGroupRoot", nodeGroupRoot, None,
                                      Return, "string",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      End),

                         new Function(c, "nodeExists", nodeExists, None,
                                      Return, "bool",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      End),

                         new Function(c, "setNodeInputs", setNodeInputs, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      new Param(c, "inputNodes", "string[]"),
                                      End),

                         new Function(c, "testNodeInputs", testNodeInputs, None,
                                      Return, "string",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                                      new Param(c, "inputNodes", "string[]"),
                         ...
                         new Function(c, "flushCachedNode", flushCachedNode, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "nodeName", "string"),
                         ...
                         new Function(c, "existingFilesInSequence", existingFilesInSequence, None,
                                      Return, "string[]",
                                      Parameters,
                                      new Param(c, "sequence", "string"),
                                      End),

                         new Function(c, "existingFramesInSequence", existingFramesInSequence, None,
                                      Return, "int[]",
                                      Parameters,
                                      new Param(c, "sequence", "string"),
                                      End),

                         new Function(c, "ioFormats", ioFormats, None,
                                      Return, "IOFormat[]",
                                      End),

                         new Function(c, "ioParameters", ioParameters, None,
                                      Return, "IOParameter[]",
                                      Parameters,
                                      new Param(c, "extension", "string"),
                                      new Param(c, "forEncode", "bool"),
                                      new Param(c, "codec", "string", Value(Pointer(0))),
                                      End),

                         new Function(c, "videoState", videoState, None,
                         ...
                         new Function(c, "logMetricsWithProperties", logMetricsWithProperties, None,
                                      Return, "void",
                                      Parameters,
                                      new Param(c, "event", "string"),

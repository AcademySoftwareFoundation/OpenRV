...
GroupIPNode::GroupIPNode(const std::string& name,
...
void
GroupIPNode::add(IPNode* n)
{
    m_members.insert(n);
}

void
GroupIPNode::remove(IPNode* n)
{
    m_members.erase(n);
}

void
GroupIPNode::setInputs(const IPNodes& newInputs)
{
    IPNode::setInputs(newInputs);
}

string
GroupIPNode::internalNodeNameForInput(IPNode* input,
...
string
GroupIPNode::internalNodeName(const string& smallIdString)
{
    ostringstream nout;
...
void
GroupIPNode::internalOutputNodesFor(IPNode* node, IPNodes& nodes) const
{
    nodes.clear();
    ...
    for (IPNodeSet::const_iterator i = m_members.begin(); i != m_members.end(); ++i)
    {
        if (AdaptorIPNode* anode = dynamic_cast<AdaptorIPNode*>(*i))
        {
            ...
            {
                //cout << "found anode " << anode->name() << " in " << name() << endl;
                const IPNodes& outputs = anode->outputs();
                for (size_t q = 0; q < outputs.size(); q++) nodes.push_back(outputs[q]);
            }
        }
    }
}
...
IPImage*
GroupIPNode::evaluate(const Context& context)
{
    //
...
void
GroupIPNode::metaEvaluate(const Context& c, MetaEvalVisitor& visitor)
{
    if (m_root)
    {
        visitor.enter(c, this);
        if (visitor.traverseChild(c, 0, this, m_root)) m_root->metaEvaluate(c, visitor);
        visitor.leave(c,this);
    }
}

void
GroupIPNode::visitRecursive(NodeVisitor& visitor)
{
    if (m_root)
    {
        visitor.enter(this);
        if (visitor.traverseChild(0, this, m_root)) m_root->visitRecursive(visitor);
        visitor.leave(this);
    }
}

void
GroupIPNode::testEvaluate(const Context& context,
                          TestEvaluationResult& results)
{
    if (m_root) m_root->testEvaluate(context, results);
}

IPImageID*
GroupIPNode::evaluateIdentifier(const Context& context)
{
    return m_root ? m_root->evaluateIdentifier(context) : 0;
}

IPNode::ImageRangeInfo
GroupIPNode::imageRangeInfo() const
{
...
void
GroupIPNode::mediaInfo(const Context& context, MediaInfoVector& infos) const
{
    if (m_root) m_root->mediaInfo(context,infos);
}

bool
GroupIPNode::isMediaActive() const
{
    if (m_root) 
    {
...
void
GroupIPNode::setMediaActive(bool state)
{
    if (m_root) 
    {
...
void
GroupIPNode::inputChanged(int index)
{
...
void
GroupIPNode::inputRangeChanged(int index, PropagateTarget target)
{
    for (IPNodeSet::iterator i = m_members.begin(); i != m_members.end(); ++i)
    {
        if (AdaptorIPNode* anode = dynamic_cast<AdaptorIPNode*>(*i))
...
void
GroupIPNode::inputImageStructureChanged(int index, PropagateTarget target)
{
...
void
GroupIPNode::propagateFlushToInputs(const FlushContext& c)
{
    if (m_root) m_root->propagateFlushToInputs(c);
...
size_t
GroupIPNode::audioFillBuffer(const AudioContext& c)
{
    return m_root ? m_root->audioFillBuffer(c) : 0;
}

void
GroupIPNode::propagateAudioConfigToInputs(const AudioConfiguration& c)
{
    if (m_root) m_root->propagateAudioConfigToInputs(c);
...
{
    if (m_root) m_root->propagateGraphConfigToInputs(config);
    IPNode::propagateGraphConfigToInputs(config);
}


IPNode*
GroupIPNode::newSubGraphForInput(size_t index, const IPNodes& nodes)
{
    return nodes[index];
}

IPNode*
GroupIPNode::modifySubGraphForInput(size_t index,
...
void
GroupIPNode::setInputsWithReordering(const IPNodes& newInputs, IPNode* fanInNode, int inputIndexOnly)
{
    HOP_PROF_FUNC();
    if (isDeleting())
    {
...

    {
        ...
        for (size_t q = 0; q < removeNodes.size(); q++) delete removeNodes[q];
    }
}

void
GroupIPNode::flushIDsOfGroup()
{
...
void
GroupIPNode::readCompleted(const string& t, unsigned int v)
{
    setInputs(inputs());
...
void
GroupIPNode::swapNodes(IPNode* A, IPNode* B)
{
...
void
GroupIPNode::addAuxillaryOutput(IPNode* node)
{
    m_auxOutputs.push_back(node);
}

IPNode*
GroupIPNode::newMemberNode(const std::string& typeName, const std::string& nameStem)
{
    return graph()->newNode(typeName, internalNodeName(nameStem), this);
}

IPNode*
GroupIPNode::newMemberNodeForInput(const std::string& typeName,
                                   IPNode* input,
...
AdaptorIPNode*
GroupIPNode::newAdaptorForInput(IPNode* input)
{
    AdaptorIPNode* n = dynamic_cast<AdaptorIPNode*>(graph()->newNode("Adaptor",
                                                                     internalNodeNameForInput(input, "a"),
                                                                     this));
...
IPNode*
GroupIPNode::memberByType(const std::string& typeName)
{
    for (IPNodeSet::iterator i = m_members.begin();
         i != m_members.end();
         ++i)
    {
        IPNode* n = *i;
        if (n->protocol() == typeName) return n;
    }
    return 0;
}

void
GroupIPNode::collectMemberNodes(IPNodeSet& nodeSet, size_t depth)
{
    for (IPNodeSet::iterator i = m_members.begin();
         i != m_members.end();
         ++i)
    ...
    if (depth != 0) nodeSet.insert(const_cast<GroupIPNode*>(this));
}

namespace {
struct TypeFilter
{
    TypeFilter(const string& n) : name(n) {}
    string name;
...
void
GroupIPNode::collectMemberNodesByTypeName(const string& typeName,
                                          IPNodeSet& nodeSet,
                                          size_t depth)
{
    IPNodeSet nodes;
...
GroupIPNode::isolate()
{
    ...
    for (IPNodeSet::iterator i = members.begin();
         i != members.end();
         ++i)
    {
        IPNode* mnode = *i;
        if (mnode == this) continue;
...
GroupIPNode::restore()
{
    ...
    if (sp && ip)
    {
        for (size_t i = 0; i < sp->size(); i++)
        {
            ...
            if (mnode && externalNode) externalNode->insertInput(mnode, index);
        }
...
void
GroupIPNode::copyInputs(IPNode* A, const IPNode* B)
{
...
void
GroupIPNode::copyNode(const IPNode* node)
{

...
ExpressionVisitor::ExpressionVisitor(Expression* Fexpr,
                                     bool onEnter,
                                     bool onLeave,
                                     bool eachSymbol)
    : m_root(Fexpr),
...
void
ExpressionVisitor::visitRecursive(Expression* Fexpr)
{
    if (m_enter) 
    {
        visit(Fexpr, true);
...
void 
ExpressionQuery::visit(Expression* Fexpr, bool)
{
    if (Fexpr->function()->type() & m_type)
    {
        m_fexprs.push_back(Fexpr);
        if (m_fexprs.size() >= m_max) cancel();
    }
}

void
installAdaptiveBoxResizeRecursive(IPImage* image)
{
    if (!image->shaderExpr)
    {
...
size_t
sourceFunctionCount(Expression* expr, size_t limit)
{
...
size_t
filterFunctionCount(Expression* expr, size_t limit)
{

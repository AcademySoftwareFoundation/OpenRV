...
void
DynamicIPNode::setInputs(const IPNodes& inputs)
{
...
string 
DynamicIPNode::dynamicName() const
{
    ostringstream str;
    str << this;
    return str.str();
...
void
DynamicIPNode::publish(const string& filename)
{
    NodeManager::NodeDefinitionVector defs(1);
    defs[0] = m_definition;
    setStringValue("node.export", filename);
    
    static const char* copyProps[] = { "node.name", "node.export", "node.author", 
                                       "node.company", "node.comment", "documentation.html",
                                       "documentation.summary", 0 };
...
void
DynamicIPNode::setOutput(bool valid, bool error, const string& str)
{
...
void
DynamicIPNode::setOutputNames(const string& name, const string& callName)
{
...
DynamicIPNode::rebuild()
{
    ...
    for (size_t i = 0; i < pprops.size(); i++)
    {
        ...
        {
            newProp = iprop->copy();
            if (prop) m_parameters->remove(prop);
    ...
    {
        if (usedProps.count(props[i]) == 0) unusedProps.push_back(props[i]);
    }
    ...
    if (evalType == "transition")
    {
        ...
        if (numFunctionParams != 2)
        {
            str << "WARNING: transition requires 2 inputs, but "
                ...
                << (numFunctionParams > 1 ? "s were " : " was ")
                << "found" << endl;
        }
        ...
        for (size_t i = 0; i < F->parameters().size(); i++)
        {
            ...
            if (name == "startFrame" && type == Shader::Symbol::FloatType) startFrameFound = true;
            else if (name == "numFrames" && type == Shader::Symbol::FloatType) numFramesFound = true;
            else if (name == "frame" && type == Shader::Symbol::FloatType) frameFound = true;
        }
...
void 
DynamicIPNode::propertyChanged(const Property* p)
{
    if (p == m_glslCurrent) rebuild();
...
void 
DynamicIPNode::readCompleted(const string& p, unsigned int v)
{
    //
...
string 
DynamicIPNode::stringValue(const string& name) const
{
    return propertyValue<StringProperty>(name, "");
}

IPNode::StringProperty*
DynamicIPNode::stringProp(const string& name)
{
    return property<StringProperty>(name);
}

void 
DynamicIPNode::setStringValue(const string& name,
                              const string& value,
...
int 
DynamicIPNode::intValue(const string& name) const
{
    return propertyValue<IntProperty>(name, 0);
}

IPNode::IntProperty*
DynamicIPNode::intProp(const string& name)
{
    return property<IntProperty>(name);
}

void 
DynamicIPNode::setIntValue(const string& name,
                           int value,
                           bool withNotification)
{
...
bool 
DynamicIPNode::outputError() const
{
    return property<IntProperty>("output.error")->front() != 0;
}

bool 
DynamicIPNode::outputValid() const
{
    return property<IntProperty>("output.valid")->front() != 0;
}

string 
DynamicIPNode::outputMessage() const
{
    return property<StringProperty>("output.message")->front();
}

string 
DynamicIPNode::outputFunctionName() const
{
    return m_outputMain->front();
}

string 
DynamicIPNode::outputFunctionCallName() const
{

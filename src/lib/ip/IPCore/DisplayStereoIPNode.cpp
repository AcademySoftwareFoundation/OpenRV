...
void
DisplayStereoIPNode::setStereoType(const string& type)
{
    setProperty(m_stereoType, type);
}

string
DisplayStereoIPNode::stereoType() const
{
...
prepareForStereo(IPImage* img,
                      ...
                      Vec3f scale)
{
    if (img->destination == IPImage::OutputTexture) return;
...
IPImage*
DisplayStereoIPNode::evaluate(const Context& context)
{
    ...
    if (!oneEye && !mono && !passthrough)
    {
        ...
        root = new IPImage(this,
                           isMerge ? IPImage::MergeRenderType : IPImage::BlendRenderType,
                           w, h, 1.0,
                           isMerge ? IPImage::IntermediateBuffer : IPImage::CurrentFrameBuffer,
        ...
        if (isMerge)
        {
            ...
            if (type == "anaglyph")      root->mergeExpr = Shader::newStereoAnaglyph(root, inExpressions);
            else if (type == "lumanaglyph") root->mergeExpr = Shader::newStereoLumAnaglyph(root, inExpressions);
            else if (type == "checker")  root->mergeExpr = Shader::newStereoChecker(root, parityX, parityY, inExpressions);
            else if (type == "scanline")
            {
                if (DisplayStereoIPNode::m_swapScanlines) parityY = 1 - parityY;
    ...
    {
        if (scontext.stereo) scontext.stereoContext.active = true;
        root = StereoTransformIPNode::evaluate(scontext);
        //
        //  Note that we are in mono or passthrough mode and in either case we
        //  should be doing nothing to img matrix here, so do _not_ call
        //  prepareForStereo.
        //
        return root;
    }
...
IPImageID*
DisplayStereoIPNode::evaluateIdentifier(const Context& context)
{
    bool   passthrough = context.stereo;
    bool   oneEye      = false;
    bool   mono        = true;
...
void 
DisplayStereoIPNode::metaEvaluate(const Context& context, MetaEvalVisitor& visitor)
{

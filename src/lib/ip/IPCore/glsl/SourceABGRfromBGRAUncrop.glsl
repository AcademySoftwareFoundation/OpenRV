...
//
//
//  Look up ABGR value from rect sampler. 
//
//  Channel indicies:  b g r a
//  Incoming channels: A B G R
//  Desired channels:  R G B A
//  Output reording:   a r g b

vec4 SourceABGRfromBGRAUncrop (const in sampler2DRect RGBA_sampler,
                               const in vec2 ST,
                               ...
                               const in float orientation)
{
        ...
        uncropDimension.x != samplerSize.x || uncropDimension.y != samplerSize.y)
    {
        ...
        if (adjustedTexCoord.x < 0.0 || adjustedTexCoord.y < 0.0 || 
            ...
            return vec4(0.0);
        return texture(RGBA_sampler, adjustedTexCoord).argb;
    }

    return texture(RGBA_sampler, texCoord).argb;// NOTE: swizzle here
}

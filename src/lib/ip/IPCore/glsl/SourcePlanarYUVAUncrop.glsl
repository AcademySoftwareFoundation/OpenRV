...
vec4 SourcePlanarYUVAUncrop (const in sampler2DRect Y_sampler,
                             ...
                             const in float orientation)
{
        ...
        uncropDimension.x != Y_samplerSize.x || uncropDimension.y != Y_samplerSize.y)
    {
        ...
        return vec4((YUVmatrix * vec4( texture(Y_sampler, adjustedTexCoord).r,
    ...
    {
        return vec4((YUVmatrix * vec4( texture(Y_sampler, st).r,

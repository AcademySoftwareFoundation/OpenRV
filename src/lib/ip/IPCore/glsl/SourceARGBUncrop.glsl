...
vec4 SourceARGBUncrop (const in sampler2DRect RGBA_sampler,
                       const in vec2 ST,
                       ...
                       const in float orientation)
{
        ...
        uncropDimension.x != samplerSize.x || uncropDimension.y != samplerSize.y)
    {
        ...
        if (adjustedTexCoord.x < 0.0 || adjustedTexCoord.y < 0.0 || 
            ...
            return vec4(0.0);
        return texture(RGBA_sampler, adjustedTexCoord).gbar;
    }

    return texture(RGBA_sampler, texCoord).gbar;// NOTE: swizzle here
}

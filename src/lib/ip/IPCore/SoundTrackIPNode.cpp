...
bool 
SoundTrackIPNode::setInternalOffset(float t)
{
...
float
SoundTrackIPNode::audioTextureComplete() const
{
    return (graph()->audioCachingMode() == IPGraph::BufferCache) ?
        1.0 : double(m_sampleCurrent - m_sampleStart) / double(m_sampleEnd - m_sampleStart);
}

IPImage*
SoundTrackIPNode::evaluate(const Context& context)
{
    if (inputs().empty()) return IPImage::newNoImage(this, "No Input");
    IPImage* head = inputs().front()->evaluate(context);
    return head;
}

IPImage*
SoundTrackIPNode::evaluateAudioTexture(const Context& context)
{
    if (!inputs().empty() && m_fb)
    {
        ...
        IPImage* image = new IPImage(this,
...
void
SoundTrackIPNode::propertyChanged(const Property* p)
{
...
void
SoundTrackIPNode::audioConfigure(const AudioConfiguration& config)
{
...
void
SoundTrackIPNode::graphConfigure(const GraphConfiguration& config)
{
...
void
SoundTrackIPNode::setInputs(const IPNodes& nodes)
{
...
SoundTrackIPNode::imageRangeInfo() const
{
...
void
SoundTrackIPNode::renderAudio(const AudioContext& context)
{
    if (!m_fb) return;

    AudioBuffer& buffer = context.buffer;
    const SampleTime s = buffer.startSample();
    if ((s + SampleTime(buffer.size()) < m_sampleStart) || (s > m_sampleEnd)) return;

    lockFB();
    //
    //  Now that we have acquired the lock, check again that we want to render
    //  this buffer (in/out points  could have been reset in the meantime).
    //
...
void
SoundTrackIPNode::renderAudioWaveform(const AudioContext& context)
{
    if (m_sampleEnd == m_sampleStart) return;
    ...
    for (   size_t i = firstSampleIndexInWindow * channels; 
            i < firstSampleIndexBeyondWindow * channels; 
            i += channels)
    {
        float v = 0.0f;
        for (int j  = 0; j < channels; ++j ) v += p[i+j];
        v *= normalize;

        v = ::pow(double(fabs(v)), 0.5) * (v < 0 ? -1.0 : 1.0);
        //float v = float(i)/float(nsamples*channels);  /* show position of sample in buffer graphically */
        ...
        if (v >= 0.0f) { s.totalPos += v; s.nPos++; }
        else { s.totalNeg += v; s.nNeg++; }
    }
...
size_t
SoundTrackIPNode::audioFillBuffer(const AudioContext& context)
{
    //

...
void 
Expression::output(std::ostream& o) const
{
...
void 
Expression::outputHash(std::ostream& o) const
{
    o << m_function->callName() << "(";
...
bool 
Expression::lessThan(const Expression* other) const
{
    //
    ...
    for (size_t i = 0, s = m_arguments.size(); i < s; i++)
    {
        ...
        if (isF && isG)
        {
            ...
            if (Fa->lessThan(Ga)) return true;
            if (Ga->lessThan(Fa)) return false;
        }
        else if (isF || isG)
        {
            //
...
Expression::copyUnbound() const
{
    ...
    for (size_t i = 0; i < nargs; i++)
    {
        ...
        else if (BoundImageCoord* b = dynamic_cast<BoundImageCoord*>(s))
        {
            ...
            args[i] = new BoundImageCoordName(s->symbol(), 

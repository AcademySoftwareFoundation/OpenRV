...
TextureOutputGroupIPNode::TextureOutputGroupIPNode(const std::string& name,
...
void 
TextureOutputGroupIPNode::setFrame(int f)       
{ 
...
void 
TextureOutputGroupIPNode::setActive(bool b)
{
...
void
TextureOutputGroupIPNode::setGeometry(int w, int h, const string& dt)
{
...
void 
TextureOutputGroupIPNode::initContext(Context& context) const
{
...
IPImage* 
TextureOutputGroupIPNode::evaluate(const Context& context)
{
    //
    //  If the tag has not been set, or has been re-set to "", do nothing
    //
    if (tag() == "" || !isActive()) return 0;
    ...
    //          caching threads is explicitly caching this node) evaluate as usual.
    //
    //  (2b)    and the context.cacheNode is _not_ set (because the cache
    //          thread is just caching frames), return nothing

    if (context.thread != CacheEvalThread && !graph()->cache().perNodeCacheContents(this)) return 0;
    else
    if (context.thread == CacheEvalThread && context.cacheNode != this) return 0;
    ...
    if      (tname == "uint16")     dataType = IPImage::UInt16DataType;
    else if (tname == "uint10rev")  dataType = IPImage::UInt10A2RevDataType;
    else if (tname == "uint10")     dataType = IPImage::UInt10A2DataType;
    else if (tname == "float")      dataType = IPImage::FloatDataType;
    else if (tname == "half")       dataType = IPImage::HalfDataType;

    if (m_ndcCoords->front() != 0) stype = IPImage::NDC2DSampler;
...
bool
TextureOutputGroupIPNode::cached() const
{

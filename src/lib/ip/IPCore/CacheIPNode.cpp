...
#define DB(x)           if (DB_GENERAL & DB_LEVEL) cerr << "CacheIPNode: " << x << endl
#define DBL(level, x)   if (level & DB_LEVEL)      cerr << "CacheIPNode: " << x << endl
#else
#define DB(x)
#define DBL(level, x)
#endif

CacheIPNode::CacheIPNode(const std::string& name,
...
{
    IPImageTreeFromIDTree(const IPNode::Context& c,
                          const CacheIPNode* n) 
        : context(c), 
          missed(true),
          node(n) {}
        ...
        try
        {
            ...
            if ( fb )
            {
                if (const TwkFB::TypedFBVectorAttribute<float>* transformMatrixAtt =
                        dynamic_cast<const TwkFB::TypedFBVectorAttribute<float>*>(
...
{
    CheckInImageFBs(const IPNode::Context& c) : context(c) {}
    const IPNode::Context& context;
    ...
    void operator () (IPImage* img)
    {
        ...
        else DB ("evaluate:    after miss, skipping null fb");
    }
};
...
struct UseCacheImageIfExists
{
    UseCacheImageIfExists(const IPNode::Context& c, bool m, IPNode::ThreadType t, IPImage* r)
        : context(c), missing(m), thread(t), root(r) {}
    bool missing;
    const IPNode::Context& context;
    IPImage* root;
    ...
    void operator () (IPImage* img)
    {
        ...
        // We support visiting IPImage nodes with no FrameBuffer. We just skip 
        // them.
        if ( !fb ) return;
        ...
        {
            //
            //  This one was in the cache. So use the cached version (it might
            //  be locked) and delete the one that was just evaluated.
            //
            //  But note that the frame buffer _is_ required for this frame (it
            //  just came back from evaluate), so make sure it's associated
            //  with this frame in the cache.
            //
            //  If context.cacheNode, we are evaluating for an explice per-node
            //  cache of this fb, but since we found it in the cache this fb
            //  must already have been cached for some other reason.  In this
            //  case, we need to _not_ associated it with any frame, and need
            //  to instead reference it in the per-node cache.
            //
        ...
        {
            DB ("UseCacheImageIfExists: calling add, thread " << context.thread << ":" << context.threadNum << " id " << fb->identifier());

            //
            //  If we're going to possibly shove the fb into the cache, make
            //  sure we are not at least already overflowing.
            //
            if (context.cache.used() > context.cache.capacity()) context.cache.emergencyFree();

            //
            //  XXX  We are forcing ("true" in next line) this fb into the cache
            //  even if it is already full.  We find that avfoundation reader
            //  can cause giant hiccup if we ask for out of order frames during
            //  playback, which is what happens here if we discard a frame when
            //  the cache fills during playback (we ask for the same frame
            //  twice).  Further investigation needed.
            //
            ...
            if (!addSucceeded && thread == IPNode::CacheEvalThread)
            {
                ...
                try
                {
                    ...
                    DBL (DB_ADD, "UseCacheImageIfExists:     checkInAndDelete after add failure, root " << root);
                    context.cache.checkInAndDelete(root);
                    DBL (DB_ADD, "UseCacheImageIfExists:     checkInAndDelete after add failure, done");
                }
                catch (...)
                {
                    //
            ...
            try
            {
                if (fb->inCache()) { context.cache.checkOut(fb); }
...
IPImage*
CacheIPNode::evaluate(const Context& context)
{
    ...
    DB ("evaluate: calling transform_ip with callable IPImageTreeFromIDTree ");
    ...
    {
        HOP_PROF("CacheIPNode::evaluate() - root->shaderExpr = Shader::newDerivativeDownSample");
...
{
    UsePerNodeCacheContents(const IPNode::Context& c,
                          const CacheIPNode* n) 
        : context(c), 
          cacheNode(n) {}
    const IPNode::Context& context;
    ...
    void operator () (IPImageID* id)
    {
        if (context.cacheNode)
        {
            ...
            if (fb) id->id = fb->identifier();
        }
    }
};

IPImageID* 
CacheIPNode::evaluateIdentifier(const Context& context)
{
...
void
CacheIPNode::flushAllCaches(const FlushContext& context)
{
    try
    {

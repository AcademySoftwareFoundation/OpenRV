...
bool
equals(const IPNode::StringVector& a, const IPNode::StringVector& b)
{
...
void
PipelineGroupIPNode::setInputs(const IPNodes& inputs)
{
...
void
PipelineGroupIPNode::clearPipeline()
{
    //
...
void
PipelineGroupIPNode::buildPipeline()
{
    if (inputs().empty()) 
    {
        clearPipeline();
    ...
    for (size_t i = 0; i < m_nodes.size(); i++)
    {
        ...
        if (index > 0) oldNodes.push_back(node); // don't reuse the AdaptorIPNode
        else delete node;
    }
...
void 
PipelineGroupIPNode::propertyChanged(const Property* p)
{
    if (p == m_nodeTypeNames) buildPipeline();
    GroupIPNode::propertyChanged(p);
}

void
PipelineGroupIPNode::readCompleted(const string& type, unsigned int version)
{
    buildPipeline();
...
IPNode::StringVector
PipelineGroupIPNode::pipeline() const
{
    return m_nodeTypeNames->valueContainer();
}

void 
PipelineGroupIPNode::setPipeline(const StringVector& array)
{
    if (equals(array, m_nodeTypeNames->valueContainer())) return;
...
void 
PipelineGroupIPNode::setPipeline1(const string& nodeType)
{
...
void 
PipelineGroupIPNode::setPipeline2(const string& a, const string& b)
{
...
void 
PipelineGroupIPNode::setPipeline3(const string& a, const string& b, const string& c)
{
...
void 
PipelineGroupIPNode::setPipeline4(const string& a, const string& b, const string& c, const string& d)
{
...
IPNode* 
PipelineGroupIPNode::findNodeInPipelineByTypeName(const std::string& name) const
{
    for (size_t i = 1; i < m_nodes.size(); i++)
    {
        if (m_nodes[i]->protocol() == name) return m_nodes[i];
    }
...
string 
PipelineGroupIPNode::profile() const
{
    return propertyValue<StringProperty>("profile.name", "");
}

void 
PipelineGroupIPNode::copyNode(const IPNode* node)
{

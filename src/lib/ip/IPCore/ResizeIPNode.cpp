...
bool 
ResizeIPNode::isActive()
{
    return propertyValue(m_active, 0);
}

void 
ResizeIPNode::setActive(bool b)
{
    setProperty(m_active, b ? 1 : 0);
}

IPImage*
ResizeIPNode::evaluate(const Context& context)
{
    ...
    if (image->isNoImage()) return image;
    
    if (outWidth <= 0 || outHeight <= 0) return image;
    ...
    if (upSampling)
    {
        ...
        if (scale < 2.0) return image;
        if (!image->shaderExpr && !image->mergeExpr)
        {
            if (!image->children || image->children->next) return image; // should never happen
        ...
        IPImage* insertImage2 = new IPImage(this,
    ...
    else //down sampling
    {
        ...
        // NOTE: TODO: figure out how to manage the downsize when the scale is big (like 20)
        // for now, if scale is great than 4, we will just do nothing
        if (scale <= 1) return image;

        if (scale > 4 && image->width >= 2048) scale = 4;

...
void
StereoTransformIPNode::prepareStereoContext(StereoContext& scontext)
{
    if (!scontext.active) return;
    
    if (IntProperty* sp = m_stereoSwap)
    {
        if(sp->size()) scontext.swap = (scontext.swap != bool(sp->front()));
    }

    if (FloatProperty* fp = m_stereoRelativeOffset)
    {
        if (fp->size()) scontext.offset += fp->front();
    }

    if (FloatProperty* fp = m_stereoRightOffset)
    {
        if (fp->size()) scontext.roffset += fp->front();
    }

    if (IntProperty* flip = m_rightTransformFlip)
    {
        if (flip->front()) scontext.flip = !scontext.flip;
    }

    if (IntProperty* flop = m_rightTransformFlop)
    {
        if (flop->front()) scontext.flop = !scontext.flop;
    }

    if (FloatProperty* rot = m_rightTransformRotate)
    {
        if (rot->size() > 0) scontext.rrotate += rot->front();
    }

    if (Vec2fProperty* vp = m_rightTransformTranslate)
    {
        if (vp->size()) scontext.rtranslate += vp->front();
    }
}

IPImage*
StereoTransformIPNode::evaluate(const Context& context)
{
    if (!context.stereo) return IPNode::evaluate(context);
    ...
    if (!root) root = IPImage::newNoImage(this, "No Input");

    return root;
}

IPImageID* 
StereoTransformIPNode::evaluateIdentifier(const Context& context)
{
    if (!context.stereo) return IPNode::evaluateIdentifier(context);
...
void
StereoTransformIPNode::metaEvaluate(const Context& context, MetaEvalVisitor& visitor)
{

...
namespace { ofstream* audioDumpFile = 0; }

//
//  The compiled in modules
//

static AudioRenderer* createPerFrameAudio(const AudioRenderer::RendererParameters& p) 
{ return new PerFrameAudioRenderer(p); }

static AudioRenderer* createNothng(const AudioRenderer::RendererParameters& p) 
{ 
    TWK_THROW_EXC_STREAM("No audio module available");
}

void 
AudioRenderer::addModuleInitFunc(ModuleInitializationFunc F, std::string name)
{
...
void
addModuleIfAllowed(
        AudioRenderer::ModuleVector&    modules, 
        set<string>&                    badMods, 
        string                          name, 
        string                          plugName, 
...
#ifdef PLATFORM_WINDOWS
//------------------------------------------------------------------------------
// Note: GetVersion() is deprecated and IsWindows10OrGreater() always return
// the windows version corresponding to the manifest of the app.
// This is not a bug but rather a Microsoft design to defeat version check.
// Here is a back door to truly retrieve the running windows version.
// 
...
void
AudioRenderer::setDebug(bool b)
{
    AudioRenderer::debug = b;
}

void
AudioRenderer::setDebugVerbose(bool b)
{
    AudioRenderer::debugVerbose = b;
}

void
AudioRenderer::setDumpAudio(bool b)
{
    AudioRenderer::dump = b;
}
...
AudioRenderer::initialize()
{
    ...
    for (size_t i = 0; i < m_moduleInitObjects.size(); i++)
    {
        ...
        if (!badMods.count(o.name))
        {
            ...
            {
                // Make Platform Audio the default audio module; i.e. first entry.
    ...
    {
        if (m_modules[i].name == audioModule) m_moduleIndex = i;
    }
}

void 
AudioRenderer::outputParameters(const RendererParameters& p)
{
...
void 
AudioRenderer::setDefaultParameters(const RendererParameters& p)
{
...
void
AudioRenderer::loadModule(Module& module)
{
    ...
    if (const char* audiopath = getenv("LD_LIBRARY_PATH"))
    {
        ...
        if (files.size())
        {
            ...
            {
                cerr << "ERROR: opening audio module "
                     << file
                     << ": "
                     << dlerror() << endl;
            }
    ...
    if (!module.func) module.func = createNothng;
}

void
AudioRenderer::reset(const RendererParameters& params)
{
    if (m_disableAudio) 
    {
        m_noaudio = true;
    ...
    try
    {
        ...
        if (!module.func) loadModule(module);
...
void 
AudioRenderer::setModule(const std::string& m)
{
...
void
AudioRenderer::pushPerFrameModule(const RendererParameters& p)
{
...
const std::string& 
AudioRenderer::currentModule()
{
    return m_modules[m_moduleIndex].name;
}

void
AudioRenderer::transformFloat32ToInt24 (TwkAudio::AudioBuffer::BufferPointer inData,
                                        char *outData,
                                        size_t dataCount,
                                        bool isLittleEndian)
{
...
void 
AudioRenderer::setDeviceState(const DeviceState& state)
{
    m_deviceState = state;
}

int 
AudioRenderer::findDeviceByName(const std::string& name) const
{
    for (size_t i = 0; i < m_outputDevices.size(); i++)
    {
        if (m_outputDevices[i].name == name) return i;
    }
...
int 
AudioRenderer::findDefaultDevice() const
{
    for (size_t i = 0; i < m_outputDevices.size(); i++)
    {
        if (m_outputDevices[i].isDefaultDevice) return i;
    }
...
void AudioRenderer::play() { lockPlaying(); m_playing = true; m_lastSampleTime = 0.0; unlockPlaying(); }
void AudioRenderer::stop() { lockPlaying(); m_playing = false; unlockPlaying(); }
void AudioRenderer::shutdown() { }
bool AudioRenderer::isPlaying() const { lockPlaying(); bool b = m_playing; unlockPlaying(); return b; }
...
{ 
    if (deleteSession(s))
    {
        ...
        if (!m_parameters.holdOpen) shutdown();
    }
}
...
{
    if (AudioRenderer::debug) TwkUtil::Log("AUDIO") << "stopOnTurnAround";
...
{ 
    lockSessions();
    if (m_sessions.empty()) shutdown(); 
    unlockSessions();
}

bool
AudioRenderer::hasSessions()
{
    lockSessions();
...
{
    if (m_noaudio) return 0;
    if (!m_renderer) reset();
    return m_renderer;
}

std::string 
AudioRenderer::errorString() const
{
...
void 
AudioRenderer::clearErrorCondition() 
{
    pthread_mutex_lock(&m_errorLock);
    m_error = false; 
    m_errorString = "";
...
void
AudioRenderer::resetErrorCondition()
{
    setErrorCondition(errorString());
}

void
AudioRenderer::setErrorCondition(const std::string& msg)
{
    pthread_mutex_lock(&m_errorLock);
    m_errorString = msg;
    m_error = true;
    ...
    if (AudioRenderer::debug) cerr << "ERROR: " << msg << endl;
...
void
AudioRenderer::audioFillBuffer(AudioBuffer& outBuffer)
{
    ...
    {
        audioDumpFile->write(reinterpret_cast<const char*>(outBuffer.pointer()), 
                             outBuffer.sizeInBytes());
    }
}

void
AudioRenderer::audioFillBuffer(Session* s, AudioBuffer& outBuffer)
{
    float*             out            = outBuffer.pointer();
    ...
    if (!s->isScrubbingAudio())
    {
        if (m_parameters.hardwareLock)
        {
            ...
            // case because the timer in elapsedPlaySecondsRaw() includes the
            // preRollDelay while the elapsed time computed from (startTime - s->audioSampleShift()) does
            // not.
            // NB: if m_parameters.preRoll is false preRollDelay() is zero.
    ...
    {
        TwkUtil::Log("AUDIO") << "AudioRenderer audio time shift set to " << s->audioTimeShift() << " " 
        << startTime << " " 
        << s->audioSampleShift() << " "
        << s->elapsedPlaySecondsRaw() << " "
        << preRollDelay() << " ";
    ...
    try
    {
        m_abuffer.reconfigure(numSamples, 
                              state.layout, 
                              Time(state.rate), 
    ...
    if (looping)
    {
        ...
        if (n < numSamples)
        {
            //cerr << "Zeroing (looping) n sample=" << numSamples - n << endl;
            float* pout = m_abuffer.pointer() + n * numChannels;
            memset(pout, 0, (numSamples - n) * numChannels * sizeof(float));
        }
    }
    ...
    if (looping) s->setAudioLoopCount(s->audioLoopCount()-1);
    s->audioVarUnLock();

    if (backwards)  m_abuffer.reverse();

...
ImageFBO*
ImageFBOManager::newImageFBO(size_t width,
                             size_t height,
                             GLenum target,
                             ...
                             size_t samples)
{
    ...
    for (size_t i = 0; i < m_imageFBOs.size(); i++)
    {
        ...
        if (imageFBO->available &&
            imageFBO->type == type &&
            imageFBO->format == format &&
            imageFBO->target == target)
        {
...
ImageFBO*
ImageFBOManager::newImageFBO(const IPImage* image,
                             size_t fullSerialNum,
                             const string& identifier)
{
    GLenum type;
    ...
    return newImageFBO(image->width,
                       image->height,
                       image->samplerType == IPImage::Rect2DSampler ? GL_TEXTURE_RECTANGLE_ARB : GL_TEXTURE_2D,
                       iformat,
                       GL_RGBA,
                       type,
                       fullSerialNum,
                       identifier,
                       image->node->outputs().size(),
                       0);
}

ImageFBO*
ImageFBOManager::newImageFBO(const GLFBO* fbo,
                             size_t fullSerialNum,
                             const string& identifier)
{
    return newImageFBO(fbo->width(),
                       fbo->height(),
                       fbo->primaryColorTarget(),
                       fbo->primaryColorFormat(),
                       GL_RGBA,
                       fbo->primaryColorType(),
                       fullSerialNum,
                       identifier);
}
...
ImageFBO*
ImageFBOManager::findExistingPaintFBO(const GLFBO* fbo,
                                      const string& identifier,
                                      bool& found,
                                      size_t& lastCmdNum,
                                      size_t fullSerialNum)
{
    for (size_t i = 0; i < m_imageFBOs.size(); i++)
    {
        ...
        if (loc != string::npos)
        {
            ...
            {
                // this means we found a fbo containing the IPImage and a number of commands
                // (could be zero)
                // we need to find out how many commands had been painted by this fbo. and continue
                // to paint the rest
                if (m_imageFBOLog)
                {
                    ImageFBO* i = imageFBO;
                ...
                // the identfifier finishes with "paintCmdNo" and the number of commands painted
                // in this imageFBO
...
ImageFBO*
ImageFBOManager::findExistingImageFBO(const string& identifier, size_t fullSerialNum)
{
...
ImageFBO*
ImageFBOManager::findExistingImageFBO(GLuint textureID)
{
    for (size_t i = 0; i < m_imageFBOs.size(); i++)
    {
        ImageFBO* imageFBO = m_imageFBOs[i];
        assert(imageFBO->fbo()->hasColorAttachment());
        if (imageFBO->fbo()->colorID(0) == textureID) return imageFBO;
    }
...
void
ImageFBOManager::gcImageFBOs(size_t fullSerialNum)
{
    //
...
void
ImageFBOManager::insertFBOFence(const GLFBO* fbo)
{
...
void
ImageFBOManager::waitForFBOFence(const GLFBO* fbo)
{
    ScopedLock lock(m_fboFenceMutex);
    if (fbo->state() == GLFBO::FenceInserted) fbo->waitForFence();
    m_fboSet.erase(fbo);
...
void
ImageFBOManager::deleteFBOFence(const GLFBO* fbo)
{
    ScopedLock lock(m_fboFenceMutex);
    if (fbo->state() == GLFBO::FenceInserted) fbo->waitForFence();
    m_fboSet.erase(fbo);
...
void
ImageFBOManager::insertTextureFence(const size_t id)
{
    ...
    if (it != m_textureFenceMap.end())
    {
        if (m_imageFBOLog)
        {
            cout << "Func insertTextureFence: Fence already exists. This should never happen." << endl;
        }
        return; 
    }
    GLFence* fence = new GLFence();
...
void
ImageFBOManager::waitForTextureFence(const size_t id)
{
    ScopedLock lock(m_textureFenceMutex);
    if (m_textureFenceMap.empty()) return;
...
void
ImageFBOManager::clearAllFences()
{
    //
...
void
ImageFBOManager::releaseImageFBO(const GLFBO* fbo)
{
...
void
ImageFBOManager::flushImageFBOs()
{
...
ImageFBO*
ImageFBOManager::newOutputOnlyImageFBO(GLenum format,
                                       size_t w,
                                       size_t h,
                                       size_t samples)
{

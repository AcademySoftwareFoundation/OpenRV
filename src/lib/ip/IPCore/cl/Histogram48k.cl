...
#define NBANKS 16 // each work group uses 16 sub histograms to reduce mem conflicts
                         ...
                         uint imageHeight) 
{
    ...
    {  
        uint x = i % NBANKS; uint y = i / NBANKS;  
        for (uint j = 0; j < CHANNEL * NBANKS; j++)  
        {  
            subHistograms[(j * NBANKS + y * CHANNEL * NBANKS * NBANKS) + x] = 0;  
        }  
    }  

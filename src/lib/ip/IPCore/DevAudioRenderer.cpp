...
void DevAudioRenderer::play()
{
    ...
    try
    {
        ...
        //cout << "movInfo audioRate = " << m_movInfo->audioSampleRate << endl;
...
double DevAudioRenderer::elapsed()
{
    return m_elapsedSincePlay;
}
...
void DevAudioRenderer::playerThread()
{
    ...
    try
    {
        ...
        while (m_isPlaying)
        {
            ...
            const double ae     = double(psamps - m_sampleShift)
                                    / double(m_deviceRate);
    ...
    {
        std::cerr << "DevAudioRenderer::playerThread: " 
                  << e.what() << std::endl;
    }
}
...
void DevAudioRenderer::playAudioChunk()
{
    ...
    unsigned int n = session->rootNode()->audioFillBuffer(fout,
                                                          m_deviceRate,
                                                          m_startSample,
                                                          numSamples);
...
void
DevAudioRenderer::setTime(double t)
{

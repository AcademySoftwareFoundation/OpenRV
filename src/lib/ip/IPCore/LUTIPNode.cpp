...
LUTIPNode::LUTIPNode(const std::string& name,
...
void
LUTIPNode::copyNode(const IPNode* node)
{
    IPNode::copy(node);
    if (node->definition() == definition()) updateProperties();
}

void
LUTIPNode::propertyChanged(const Property* property)
{
    IPNode::propertyChanged(property);

    if (property == m_lutFile ||
        property == m_lutPreLUTSize)
    {
        updateProperties();
    }
    
    if (property == m_lutLUT      ||
        property == m_lutPreLUT   ||
        property == m_lutSize     ||
        property == m_lutInMatrix ||
...
void
LUTIPNode::readCompleted(const std::string& type, unsigned int version)
{
...
LUTIPNode::updateProperties()
{
    ...
    if (file != "")
    {
        ...
        std::copy(lutdata->data.begin(),
             lutdata->data.end(),
             m_lutLUT->begin());

        m_lutPreLUT->resize(lutdata->prelutData.size());
        std::copy(lutdata->prelutData.begin(),
             lutdata->prelutData.end(),
             m_lutPreLUT->begin());

        m_lutSize->resize(lutdata->dimensions.size());
        std::copy(lutdata->dimensions.begin(),
             lutdata->dimensions.end(),
             m_lutSize->begin());
...
LUT::LUTData*
LUTIPNode::parseLUT(string filename)
{
    ...
    try
    {
        ...
        if (getenv("RV_NO_LUT_REUSE")) lutdata = new LUT::LUTData();
        else 
        {
        ...
        if (newLUT)
        {
            ...
            if (m_usePowerOf2) LUT::resamplePowerOfTwo(*lutdata);
...
bool 
LUTIPNode::lutActive() const
{
    return (propertyValue<IntProperty>(m_lutActive, 0) == 1);
}

inline float
lerp1DLUT(FloatProperty* lut, float range, size_t c, float v)
{
...
LUTIPNode::generate3DLUT()
{
    ...
    if (3 * n != lut->size()) 
    {
        ...
        exc << "3D LUT size mismatch "
            << (n*3) << " != " << lut->size();
        if (fileP && fileP->size())  exc << " (" << fileP->front() << ")";
        cerr << exc << endl;
        throw exc;
    }
    ...
    if (!ImageRenderer::hasFloatFormats() && !m_floatOutLUT)
    {
        ...
        for (int i=0, s = outProp->size(); i < s; i++) 
        {
            if (out32)
            {
                m_lowlut3[i] = (unsigned short)( ((*out32)[i]-minv) / rangev 
                                       * double(numeric_limits<unsigned short>::max()));
            }
            else
            {
                m_lowlut3[i] = (unsigned short)( ((*out16)[i]-minv) / rangev 
                                       * double(numeric_limits<unsigned short>::max()));
            }
    ...
    m_lutfb = new FrameBuffer(FrameBuffer::NormalizedCoordinates,
    ...
    if (fileP) m_lutfb->idstream() << ":" << fileP->front();
    ...
    {
        generate1DLUT(true);
        if (m_prelut) m_lutfb->appendPlane(m_prelut);
    }
}


void
LUTIPNode::generate1DLUT(bool asprelut)
{
    if (!asprelut) 
    { 
        delete m_lutfb;
    ...
    for (int i=0; i < n; i++)
    {
        ...
        if (c.z > maxv) maxv = c.z;
        if (c.x < minv) minv = c.x;
        if (c.y < minv) minv = c.y;
        if (c.z < minv) minv = c.z;
    ...
    if (!ImageRenderer::hasFloatFormats() && !m_floatOutLUT)
    {
        ...
        for (int i=0, s=outProp->size(); i < s; i++) 
        {
            ...
            if (out32) f = ((*out32)[i]-minv) / double(rangev);
            else f = ((*out16)[i]-minv) / double(rangev);
    ...
    FrameBuffer* fb = new FrameBuffer(n, 1, 3,
                                      dataType,
                                      m_lowlut1 ? (unsigned char*)m_lowlut1 
                                          : (unsigned char*)outProp->rawData(),
                                      0, FrameBuffer::NATURAL,
                                      false);
...
TwkFB::FrameBuffer*
LUTIPNode::evaluateLUT(const Context& context)
{
...
LUTIPNode::prepareForWrite()
{
    ...
    if (StringProperty* sp = m_lutFile)
    {
        if (sp->size() && sp->front() != "")
        {
            ...
            std::copy(lut->begin(),
                 lut->end(),
                 m_lutdata.data.begin());

            std::copy(prelut->begin(),
                 prelut->end(),
...
LUTIPNode::writeCompleted()
{
    if (m_lutdata.data.size() || m_lutdata.prelutData.size())
    {
        ...
        std::copy(m_lutdata.data.begin(),
                m_lutdata.data.end(),
                lut->begin());

        std::copy(m_lutdata.prelutData.begin(),
                m_lutdata.prelutData.end(),
                prelut->begin());
...
void 
LUTIPNode::addLUTPipeline(const Context& context, IPImage* img)
{
    ...
    if (LUT->depth() > 1)
    {
        if (preLUT)
        {
            ...
            if (preLUT->hasAttribute("scale")) outScale = Vec3f(preLUT->attribute<float>("scale"));
            if (preLUT->hasAttribute("offset")) outOffset = Vec3f(preLUT->attribute<float>("offset"));

            img->shaderExpr = Shader::newColorChannelLUT(img->shaderExpr,
                                                         preLUT,
                                                         outScale,
                                                         outOffset);
        }

        Vec3f grid = Vec3f(1.0 / LUT->width(),
                           1.0 / LUT->height(),
                           1.0 / LUT->depth());
        ...
        if (LUT->hasAttribute("scale")) outScale = Vec3f(LUT->attribute<float>("scale"));
        if (LUT->hasAttribute("offset")) outOffset = Vec3f(LUT->attribute<float>("offset"));
        ...
        {
            img->shaderExpr = Shader::newColor3DLUTGLSampling(img->shaderExpr, LUT,
                                                              scale, grid / 2.0f,
                                                              outScale, outOffset);
        }
    }
...
IPImage*
LUTIPNode::evaluate(const Context& context)
{

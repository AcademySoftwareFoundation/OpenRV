      ...
      m_audioThreadRunning(false)
{
    ...
    if (m_parameters.rate == 0) m_parameters.rate = 48000;
...
void
PerFrameAudioRenderer::threadMain()
{
    // nothing
}

void
PerFrameAudioRenderer::play(Session* s)
{
    DeviceState nstate;
...
void
PerFrameAudioRenderer::stop(Session* s)
{
...
void
PerFrameAudioRenderer::shutdown()
{
    //
...
PerFrameAudioRenderer::dataForFrame(int f, size_t seqIndex, bool silence)
{
    ...
    size_t start = nsequences * m_totalAudioFrameSizes
                    + accumulate(state.frameSizes.begin(), 
                                 state.frameSizes.begin() + seqIndex,
                                 size_t(0));

    AudioBuffer buffer(nframes, 
                       state.layout,
                       state.rate, 
    ...
    switch (m_parameters.format)
    {
        ...
        case Int24Format:
            AudioRenderer::transformFloat32ToInt24(buffer.pointer(),
                                                   (char*) out,
                                                   buffer.sizeInFloats());
            break;

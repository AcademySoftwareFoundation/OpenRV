...
#define DB(x)           if (DB_GENERAL & DB_LEVEL) cerr << dec << "FBCache: " << x << endl
#define DBL(level, x)   if (level & DB_LEVEL)      cerr << dec << "FBCache: " << x << endl
#else
#define DB(x)    
#define DBL(level, x)
#define DB_LEVEL 0
...
class CacheEdges 
{
  public:
    ...
    {
      public:
        Edge() : frame(NAF), type(NeitherSide) {};
        Edge(int f, EdgeType t) : frame(f), type(t) {};
    ...
    CacheEdges(FBCache *c) : m_cache(c) {};
...
void
CacheEdges::addEdge (Edge e)
{
    m_edges[e.frame] = e;
    DBL (DB_EDGES, "addEdge: added frame " << e.frame << 
            " type " << typeToText(e.type) << " ( " << m_edges.size() << " edges)");

}

void
CacheEdges::removeEdge (int frame)
{
    EdgeSet::iterator i = m_edges.find (frame);
    if (m_edges.end() != i)
    {
        DBL (DB_EDGES, "removeEdge: removed frame " << i->second.frame << 
              " type " << typeToText(i->second.type) << " ( " << m_edges.size() << " edges)");
        m_edges.erase (i);
    }
    else {DBL (DB_EDGES, "removeEdge: removing frame " << frame << 
            ": not found! ( " << m_edges.size() << " edges)");}
}

void
CacheEdges::dumpEdges()
{
...
CacheEdges::rationalizeEdges()
{
    for (EdgeSet::iterator i = m_edges.begin(); i != m_edges.end(); )
    {
        ...
        if (leftIsCached && rightIsCached) newType = NeitherSide;
        else if (leftIsCached) newType = RightOnly;
        else if (rightIsCached) newType = LeftOnly;
        else newType = BothSides;
        ...
        ++i;
        if (newType == NeitherSide) m_edges.erase(tmp);
        else (i->second.type = newType);
    }
}

string
CacheEdges::typeToText (EdgeType t)
{
    switch (t) 
    {
      ...
      case NeitherSide : return "NeitherSide";
    }
    return "Unknown!";
}

void
CacheEdges::addCacheEdge (int frame)
{
    ...
    //
    {
        ...
        {
            thisType = RightOnly;
            DBL (DB_EDGES, "addCacheEdge: frame " << frame << ", edge on left: " << 
                    leftEdge->second.frame << " " << typeToText(leftEdge->second.type));

                 if (BothSides == leftEdge->second.type) leftEdge->second.type = LeftOnly;
            else if (RightOnly == leftEdge->second.type) removeEdge (leftEdge->second.frame);
        }
        else if (m_cache->isFrameCached(frame - 1)) thisType = RightOnly;
    }
    else thisType = RightOnly;
    ...
    //
    {
        ...
        //
        {
            ...
            DBL (DB_EDGES, "addCacheEdge: frame " << frame << ", edge on right: " << 
                    rightEdge->second.frame << " " << typeToText(rightEdge->second.type));

                 if (BothSides == rightEdge->second.type) rightEdge->second.type = RightOnly;
            else if (LeftOnly  == rightEdge->second.type) removeEdge (rightEdge->second.frame);
        }
        else if (m_cache->isFrameCached(frame + 1)) thisType = (thisType == RightOnly) ? NeitherSide : LeftOnly;
    }
    else thisType = (thisType == RightOnly) ? NeitherSide : LeftOnly;

    if (NeitherSide != thisType) addEdge (Edge(frame, thisType));
...
void
CacheEdges::removeCacheEdge (int frame)
{
    ...
    //
    {
        ...
        //  
        {
            ...
            if (LeftOnly == leftEdge->second.type) 
            {
                ...
                DBL (DB_EDGES, "removeCacheEdge: adjusting edge to the left: " << 
                        leftEdge->second.frame << " " << typeToText(leftEdge->second.type));
            }
        }
    ...
    //
    {
        ...
        //  
        {
            ...
            if (RightOnly == rightEdge->second.type) 
            {
                ...
                DBL (DB_EDGES, "removeCacheEdge: adjusting edge to the right: " << 
                        rightEdge->second.frame << " " << typeToText(rightEdge->second.type));
            }
        }
        else if (m_cache->isFrameCached(frame + 1))
    ...
    if (m_edges.find (frame) != m_edges.end()) removeEdge (frame);
...
void 
CacheEdges::possibleFreeTargets (EdgeVector &edges)
{
...
void 
CacheEdges::possibleCacheTargets (EdgeVector &edges)
{
...
//
//  The "per-node cache" is a supplementary special-purpose cache designed to
//  hold framebuffers required for rendering, but which have no obvious
//  relation to the frames in the timeline.  In particular the Crank "preview"
//  image and the "tiles" in the shelf.  These framebuffers must be cached, but
//  there is no point in caching more than one framebuffer per node.  Hence
//  this extra cache.
//
//  Notes:
//
//  * XXX  Beware: If a node uses the per-node cache, the subtree rooted at the
...
//    that it really only manages referencing.  The lower-level TwkFB::Cache is
//    the thing that will delete framebuffers whose ref count falls too low, etc.
//
...
class PerNodeCache 
{
  public:
    ...
    //
    //  Returns true only if the fb was actually added to the per-node cache.
    //
    ...
    void clear();
    //bool isCached(string id) const { return (m_idMap.find(id) != m_idMap.end()); }
...
PerNodeCache::PerNodeCache(FBCache* c) : m_cache(c) 
{ 
...
bool
PerNodeCache::add(FrameBuffer* fb, const IPNode* node)
{
    ...
    if (i != m_nodeMap.end())
    {
        if (fb && i->second->identifier() == fb->identifier()) return false;

        //  cerr << "per-node: flushing " << i->second->identifier() << endl;
...
void
PerNodeCache::flush(FrameBuffer* fb, const IPNode* node)
{
    LockObject l(m_mapsLock);
...
void
PerNodeCache::flushInternal(FrameBuffer* fb, const IPNode* node)
{
    m_nodeMap.erase(node);

    //   XXX this is wrong!  What if two nodes end up caching same FB hence
    //   same identifier.  then this erases that from idMap and dereferences it, so could cause deletion ?
...
TwkFB::FrameBuffer*
PerNodeCache::fbOfNode(const IPNode* node)
{
    LockObject l(m_mapsLock);
...
void
PerNodeCache::pushItem(string name)
{
...
bool
FBCache::hasPartialFrameCache(int frame) const
{
    FBVector cached;
    ...
    for (int i = 0; i < cached.size(); ++i)
    {
        ...
        if (ifi != m_itemMap.end())
        {
            ...
            //
            if (1 == ifi->second.size()) return true;
        }
    }
...
void 
FBCache::setMemoryUsage(size_t bytes)
{
...
{
    FramePromoter(FBCache& c) : cache(c), missedOne(false) {}
    FBCache& cache;
    FBCache::IDSet ids;
    bool missedOne;
...
bool
FBCache::promoteFrame (int frame, IPImageID *idTree)
{
    ...
    if (fp.missedOne) return false;

    //
    //  Add fbs to list of fbs for this frame.
    //
    for (IDSet::iterator i = fp.ids.begin();
        i != fp.ids.end();
        ++i)
    {
...
bool
FBCache::add(FrameBuffer* fb, int frame, bool force, const IPNode* node)
{
    if (node)
    {
        if (!fb->inCache()) TwkFB::Cache::add(fb, true);
        bool added = m_perNodeCache->add(fb, node);
        if (added) m_graph->textureCacheUpdated()();
        //
    ...
    DB ("add() frame " << frame << " (fb " << fb << ", " << fb->identifier() <<
        ") force " << force << " frame in cache " << isFrameCached(frame));
    if (frame == NAF)
    {
        cout << "Frame == NAF" << endl;
        abort();
    ...
    if (i != m_frames.end()) cerr << "FRAME " << frame << ", " << i->second.size() << " items" << endl;
    */

    DB ("    itemInCache " << itemInCache << 
            " this frame in itemMap entry: " << m_itemMap[fb].count(frame));
    if (itemInCache && m_itemMap[fb].count(frame) > 0)
    {
        ...
        if (m_full && !partiallyCached) return false;
    ...
    if (!fb->inCache()) 
    {
        ...
        //
        {
            if (Cache::debug()) cout << "ADDED: " << fb << " : " << fb->identifier() << endl;
            ...
            {
                DBL (DB_EDGES, "add() calling addCacheEdge (1)" << frame << 
                        ", " << m_frames[frame].size() << " ids for this frame");
                m_cacheEdges->addCacheEdge(frame);
            }
    ...
    else
    {
        ...
        {
            m_cacheEdges->addCacheEdge(frame);
            DBL (DB_EDGES, "add() calling addCacheEdge (2)" << frame << 
                    ", " << m_frames[frame].size() << " ids for this frame");
        }
    }
...
void
FBCache::flushPerNodeCache(const IPNode* node)
{
    m_perNodeCache->add(0, node);
}

TwkFB::FrameBuffer*
FBCache::perNodeCacheContents(const IPNode* node) const
{
...
bool
FBCache::freeIDSet(const IDSet& idset, int frame)
{
    ...
    for (IDSet::const_iterator id = myIDset.begin();
         id != myIDset.end();
         ++id)
    {
...
bool
FBCache::freeInRangeForwards(size_t bytes, 
...
bool
FBCache::freeInRangeBackwards(size_t bytes, 
                              FrameVector& toBeErased,
                              int minFrame,
                              int maxFrame,
                              bool greedy)
{
    ...
    for (FrameMap::reverse_iterator fi = m_frames.rbegin(); 
         fi != m_frames.rend() && (!enough || greedy); 
         ++fi)
    {
        int frame = fi->first;
...
FBCache::clearFrameCaches()
{
    ...
    /* CBB
       ...
       It's possible that the above sorting, which helps things go into and out
       of the TrashCan in the right order (IE prevent thrashing), is
...
void
FBCache::clearInternal()
{
    DBL (DB_CLEAR, "clearInternal() current " << m_currentBytes << 
            " max " << m_maxBytes << " " << 
...
void
FBCache::clearAllButFrame(int frame, bool force)
{
    DBL (DB_CLEAR, "clearAllButFrame() frame " << frame << 
            " m_frames size " << m_frames.size() << 
            " current " << m_currentBytes << 
            " max " << m_maxBytes << " " << 
            double(m_currentBytes)/double(m_maxBytes));
            ...
            (m_frames.size() == 1 && m_frames.begin()->first == frame))
    {
        ...
        //  Nothing to do.
        //
        if (force) TwkFB::Cache::freeTrash (m_currentBytes);
        return;
    }
     
    FrameVector toBeErased;

    freeInRangeForwards(numeric_limits<size_t>::max(),
                         toBeErased,
                         m_minFrame, frame-1, 
                         true);

    freeInRangeBackwards(numeric_limits<size_t>::max(),
                        toBeErased,
                        frame+1, m_maxFrame-1, 
                        true);
...
inline float
FBCache::utility (int frame, UtilityMode mode)
{
    ...
    if (m_displayFrame == frame) return utilityMax;

    int lastFrame = m_outFrame - 1;
    int firstFrame = m_inFrame;
    bool forward = (frame > m_displayFrame);

    const float fact = ( mode == FOR_CACHING && !isActiveTailCachingEnabled() )
        ? 0.001f : min(max(m_lookBehindFraction/100.0f, 0.001f), 0.999f);
    //
    ...
    float ffact = fact;
    if ((m_displayInc < 0 &&  forward) || 
        (m_displayInc > 0 && !forward))
    {
        ffact = 1.0-fact;
    }
    ...
    else 
    {
        ...
        if (m_displayFrame < firstFrame || m_displayFrame > lastFrame) testFrame = firstFrame;
        ...
        //
        if (m_displayInc < 0) dRoundFront *= fact,       dRoundBack  *= (1.0-fact);
        else                  dRoundFront *= (1.0-fact), dRoundBack  *= fact; 

        DBL (DB_UTIL, "d " << 1.0/d << " drf " << 1.0/dRoundFront << " drb " << 1.0/dRoundBack << 
                " ffact " << ffact << " inc " << m_displayInc << " fwd " << forward);

        if (dRoundFront < d) d = dRoundFront;
        if (dRoundBack  < d)  d = dRoundBack;
        d = 1.0 + 1.0/d;
    }
    ...
    for (CacheEdges::EdgeVector::iterator i = edges.begin(); i != edges.end(); ++i)
    {
        ...
        if (CacheEdges::BothSides == type) 
        {
            ...
            while ( isFrameCached(f) || 
                    ...
                    m_framesScheduledForFreeing.count(f) || 
                    f == m_displayFrame) --f;
            while ( isFrameCached(f2) || 
                    m_framesBeingCached.count(f2) || 
                    m_framesScheduledForFreeing.count(f2) || 
                    f2 == m_displayFrame) ++f2;
        }
        ...
        if (f >= m_minFrame && f < m_maxFrame && !isFrameCached(f))
        {
            ...
            DBL (DB_EDGES, "consider frame for caching: frame " << f << " utility "  << u << 
                    " target " << targetCacheFrame << " utility " << targetCacheFrameUtility);
...
FBCache::findBestFreeTarget(const CacheFrame& cacheTarget)
{
    ...
    for (CacheEdges::EdgeVector::iterator i = edges.begin(); i != edges.end(); ++i)
    {
        ...
        if (CacheEdges::LeftOnly  == type) 
        {
            while (m_framesScheduledForFreeing.count(f)) ++f;
        }
        else 
        if (CacheEdges::RightOnly == type) 
        {
        ...
        if (CacheEdges::BothSides == type) 
        {
            ...
            if (m_framesScheduledForFreeing.count(f)) continue;
        }
        else continue; /* unexpected type !*/
...
void
FBCache::initCacheFreePair(int cacheFrame, int freeFrame)
{
    if (NAF != cacheFrame)
    {
        ...
        DB (endl << "************** initiateCachingOfFrame cacheTarget " << cacheFrame << 
                " freeTarget " << freeFrame << " over " << overflowing()); 
        DB ("    freeingList size " << m_framesScheduledForFreeing.size() <<
                " cachingList size " << m_framesBeingCached.size());
        //  DB ("    target cached " << isFrameCached(targetCacheFrame));
    }
}

void 
FBCache::initiateCachingOfBestFrameGroup(FrameVector& frames, int maxGroupSize)
{
    frames.clear();

    CacheFrame cacheTarget = findBestCacheTarget();

    if (0.0 == cacheTarget.utility || cacheTarget.frame == NAF) return;
    ...
    if (overflowing())
    {
        ...
        {
            if (freeTarget.utility >= cacheTarget.utility) return;
        }
        else
        {
            if (freeTarget.utility >= (utility(cacheTarget.frame, FOR_FREEING)-0.001)) return;
        }
    } 
            ...
            testFrame += cacheTarget.inc, ++cacheFrameCount)
    {
        ...
        //
        //  testFrame is cachable, now see if we can find corresponding free target.
        //

        CacheFrame testCF(testFrame, utility(testFrame, FOR_CACHING));

        if (testCF.utility == 0.0) break;
        ...
        if (overflowing())
        {
            if (isActiveTailCachingEnabled())
            {
                if (freeCF.utility >= testCF.utility) break;
            }
            else
            {
                if (freeCF.utility >= (utility(testFrame, FOR_FREEING)-0.001)) break;
            }
        }

        initCacheFreePair(testCF.frame, freeCF.frame);
    }
    //  if (cacheFrameCount > 1) cerr << "cacheFrameCount " << cacheFrameCount << endl;

    //
    //  Now fill return vector of frame numbers to cache, always in decreasing order.
    //

    for (   int f = (cacheTarget.inc > 0) ? cacheTarget.frame + cacheFrameCount - 1 : cacheTarget.frame, 
...
void
FBCache::completeCachingOfFrame (int frame)
{
...
void
FBCache::considerFrameForFreeing (int f, int &targetFrame, float &targetUtility)
{
    DBL (DB_EDGES, "considerFrameForFreeing: frame " << f << 
            " target " << targetFrame << " utility " << targetUtility);
    if (    f < m_minFrame || f >= m_maxFrame ||
            m_framesBeingCached.count(f) ||
            f == m_displayFrame)
    {
        return;
...
//
bool
FBCache::free(size_t inbytes)
{
    return freeInternal (inbytes, true);
}

bool
FBCache::freeInternal (size_t inbytes, bool freeMemory)
{
    ...
    while (m_maxBytes < m_currentBytes || m_maxBytes - m_currentBytes < inbytes)
    {
        ...
        for (CacheEdges::EdgeVector::iterator i = edges.begin(); i != edges.end(); ++i)
        {
            ...
            if (CacheEdges::BothSides == type) 
            {
                ...
                if (alreadyTried.count(f)) continue;
            }
            else continue; /* unexpected type !*/

            considerFrameForFreeing (f, targetFreeFrame, targetFreeFrameUtility);
        }
        DBL (DB_FREE, "free targetFreeFrame " << targetFreeFrame << " utility " << 
                targetFreeFrameUtility << " < " << m_targetCacheFrameUtility);
    ...
    if (Cache::debug()) showCacheContents();
    ...
    DBL (DB_FREE, "free() complete" << " current " << m_currentBytes << 
            " max " << m_maxBytes << ", " << double(m_currentBytes)/double(m_maxBytes) <<
            ", ok " << ok);
...
void
FBCache::emergencyFree()
{
    DBL (DB_FREE, "******************* emergencyFree " << " current " << m_currentBytes << 
            " max " << m_maxBytes << " " << double(m_currentBytes)/double(m_maxBytes) << " trash count " << trashCount());
...
bool
FBCache::cacheStats(CacheStats& c) const
{
    //if (m_cacheStatsDisabled) cerr << "ERROR: cache stats requested, but have been disabled" << endl;
...
void
FBCache::setCacheStatsDirty()
{
    if (m_cacheStatsDisabled) return;

    LockObject sl(m_statMutex, true);

    if (sl.locked()) m_cacheStatsDirty = true;

    //
    //  If we failed to lock, something else is setting the dirty flag _or_ the
    //  stats are being updated.  There's a chance here that the flag could get
    //  out of sync (IE be set to true even though we just called
    //  setCacheStatsDirty, we just don't care enough that the stats be
    //  accurate.  We'd rather eliminate the possibility of a perf hit due to
    //  lock contention.
    //
}

void
FBCache::updateCacheStatsIfDirty()
{
    if (m_cacheStatsDisabled) return;

    LockObject sl(m_statMutex);

    if (! m_cacheStatsDirty) return;
    ...
    try
    {
        ...
        m_cacheStats.lookAheadSeconds = computeLookAheadSecondsStat(
                                                    m_cacheStats.cachedRanges);
...
//
void
FBCache::computeCachedRangesStat(FrameRangeVector& array) const
{
    array.clear();

    for (FrameMap::const_iterator i = m_frames.begin();
         i != m_frames.end();
         ++i)
    {
        int frame = i->first;
...
//
float
FBCache::computeLookAheadSecondsStat(const FrameRangeVector& vecFrameRange) const
{
    if (m_displayFrame == NAF ||
        m_inFrame == NAF ||
        m_outFrame == NAF ||
        m_displayInc == 0 ||
        vecFrameRange.empty())
             ...
             ++i)
        {
            ...
            if ( m_displayFrame>=i->first && m_displayFrame<=i->second )
            {
                ...
                while ( i != vecFrameRange.end() &&
                        ( i->first <= (endFrameRangeIter->second + m_displayInc) ) )
                {
                    endFrameRangeIter = i;
                    i++;
                };
    ...
    {
        // Negative increment
        for (FrameRangeVector::const_reverse_iterator i = vecFrameRange.rbegin();
             i != vecFrameRange.rend();
             ++i)
        {
            // Locate the range where the display frame is located if any
            if ( m_displayFrame>=i->first && m_displayFrame<=i->second )
            {
                ...
                while ( i != vecFrameRange.rend() &&
                        ( i->second <= (endFrameRangeIter->first + m_displayInc) ) )
                {
                    endFrameRangeIter = i;
                    i++;
                };
...
struct CheckInEach
{
    ...
    void operator () (IPImage* l)
    {
        ...
        {
            DB ("checkInAndDelete() fb " << l->fb << 
                " ref count " << cache.fbReferenceCount(l->fb) << 
...
void
FBCache::checkInAndDelete(IPImage* img)
{
    if (img)
    {
...
void 
FBCache::checkInAndDelete(const IPImageVector& images)
{
    for (size_t i = 0; i < images.size(); i++) checkInAndDelete(images[i]);
}
...
bool
FBCache::frameItems(int frame, FBVector& items) const
{
    ...
    if (i != m_frames.end())
    {
        ...
        for (IDSet::const_iterator id = idset.begin();
             id != idset.end();
             ++id)
        {
            //
            ...
            {
               found = true;
               if (mi->second) items.push_back(mi->second);
            }
        }
    }
...
{
    IDSetCollector(FBCache::IDSet& s) : ids(s) {}
    FBCache::IDSet& ids;
...
void
FBCache::trimFBsOfFrame(int frame, IPImage *img)
{
...
void
FBCache::dereferenceFrame (int frame, FrameBuffer* fb)
{
    //
    ...
    DB ("dereferernceFrame frame " << frame << " fb " << fb << " id " << fb->identifier() << " reffed: " << reffed << " refcount " << fbReferenceCount(fb));
    ...
    if (reffed)
    {
        ...
        //  DeReference FB so it can be discarded from cache if this was last external ref.
        //
        dereferenceFB(fb);
        DB ("frame " << frame << " size " << m_frames[frame].size() << " de-reffed '" << fb->identifier() << "' new count " << fbReferenceCount(fb));
    }
}

bool
FBCache::referenceFrame (int frame, FrameBuffer* fb)
{
    bool reffed = (m_frames[frame].count(fb->identifier()) != 0);

    DB ("referenceFrame frame " << frame << " fb " << fb << " id " << fb->identifier() << " already reffed: " << reffed << " refcount " << fbReferenceCount(fb));
    ...
    if (!reffed)
    {
        ...
        //  Reference FB so will not be discarded from cache, until we decide to discard this frame.
        //
        referenceFB(fb);
...
bool
FBCache::flush(const IDString& idstring)
{
    ...
    if (i != m_map.end())
    {
        ...
        if (q != m_itemMap.end())
        {
            ...
            for (FrameSet::iterator fs = fset.begin();
                 fs != fset.end();
                 ++fs)
            //
            ...
            //  
            {
                ...
                if (idset.count(idstring))
                {
                    ...
                    if (idset.empty()) toBeErased.push_back(*fs);
        ...
        bool ret = Cache::flush(idstring);
            ...
            for (FrameMap::iterator fmi = m_frames.begin(); fmi != m_frames.end(); ++fmi)
            {
                IDSet& idset = fmi->second;
                for (IDSet::iterator i = idset.begin(); i != idset.end(); ++i)
                {
                    ids.insert(*i);
                }
            }
            ...
            DBL (DB_FLUSH, "after flushing (ret " << ret << ") " << idstring <<
                    ", FBCache holds " << ids.size() << " ids, Cache holds " << m_map.size());
        #endif
...
bool
FBCache::flushIDSetSubstr (const IDSet& subStrings)
{
    IDSet idsToBeFlushed;

    for (   FBMap::iterator i = m_map.begin();
            i != m_map.end();
            ++i)
    {
        for (   IDSet::const_iterator ss = subStrings.begin();
                ...
                ++ss)
        {
            if (i->first.find(*ss) != string::npos) idsToBeFlushed.insert(i->first);
        }
    }

    bool ret = false;
    for (   IDSet::iterator id = idsToBeFlushed.begin();
            id != idsToBeFlushed.end();
            ++id)
    {
...
FBCache::TreeResults
FBCache::testInCache(const IDTree& tree)
{
    int n = 0, c = 0;
    ...
    if (c == 0) result = HasNoIDs;
    else if (c != n) result = HasSomeIDs;

    return result;
}

void
FBCache::garbageCollect(bool force)
{
    //
    ...
    for (FBMap::const_iterator i = m_map.begin();
         i != m_map.end();
         ++i)
    {
    ...
    for (ItemFrames::iterator i = m_itemMap.begin();
         i != m_itemMap.end();
         ++i)
    {
        ...
        for (FrameSet::iterator q = fset.begin();
             q != fset.end();
             ++q)
        {
            int f = *q;
        ...
        //
        if (fset.empty()) garbageItems.push_back(fb);
    }
    ...
    if (Cache::debug() && collected) cout << "INFO: CACHE GC: collected " << collected << " items" << endl;
...
void
FBCache::showCacheContents() const
{
...
void 
FBCache::setDisplayFrame(int f) 
{ 
    if (f != m_displayFrame && m_graph->cachingMode() == IPGraph::BufferCache) m_utilityStateChanged = true;
    m_displayFrame = f; 
}

void 
FBCache::setDisplayInc(int i) 
{ 
    if (i != m_displayInc && m_graph->cachingMode() == IPGraph::BufferCache) m_utilityStateChanged = true;
    m_displayInc = i; 
}

void 
FBCache::setInOutFrames (int a, int b, int c, int d) 
{ 
...
void
FBCache::setLookBehindFraction (float f)
{
    if (f != m_lookBehindFraction && m_graph->cachingMode() == IPGraph::BufferCache) m_utilityStateChanged = true;
...
void
FBCache::pushCachableOutputItem (string name)
{
...
string
FBCache::popCachableOutputItem()
{
    return m_perNodeCache->popItem();
}

void
FBCache::checkMetadata()
{
    #if (DB_LEVEL)
        for (FBMap::iterator i = m_map.begin(); i != m_map.end(); ++i)
        {
            ...
            {
                cerr << "FB LOST: " << i->first << "(" << fb->identifier() << "), refs " << refs << ", in trash " << trashContains(fb) << endl;
            }
        }
    #endif
}

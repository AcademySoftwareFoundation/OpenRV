...
NodeDefinition::NodeDefinition(const string& typeName,
...
IPNode*
NodeDefinition::newNode(const std::string& name, IPGraph* graph, GroupIPNode* group) const
{
    IPNode* node = 0;
      ...
      case AtomicNodeType:
          {
              ...
              {
                  TWK_THROW_EXC_STREAM("unknown node.evaluationType \"" << ntype
                                       << "\" for definition " << this->name());
              }
          }
          break;
...
string
NodeDefinition::stringValue(const string& name, const string& defaultValue) const
{
    return propertyValue<StringProperty>(name, defaultValue);
}

vector<string>
NodeDefinition::stringArrayValue(const string& name) const
{
...
void
NodeDefinition::setString(const string& name, const string& value)
{
    setProperty<StringProperty>(name, value);
}

int
NodeDefinition::intValue(const string& name, int defaultValue) const
{
    return propertyValue<IntProperty>(name, defaultValue);
}

void
NodeDefinition::setInt(const string& name, int value)
{
    setProperty<IntProperty>(name, value);
}

bool
NodeDefinition::reify()
{
    switch (m_type)
    {
...
string
readURL(const string& url, const string& origin)
{
    //
    ...
    if (source.size() > 3 &&
        source[0] == string::value_type(0xEF) &&
        source[1] == string::value_type(0xBB) &&
        source[2] == string::value_type(0xBF))
    {
        source.erase(0,3);
        //cout << "WARINING: file " << fileName << " has a BOM -- removed it" << endl;
    }
...
Shader::Function*
NodeDefinition::buildFunctionCore()
{
    ...
    if      (ftype == "merge")          type = Shader::Function::Merge;
    else if (ftype == "filter")         type = Shader::Function::Filter;
    else if (ftype == "morphological")  type = Shader::Function::MorphologicalFilter;
    else if (ftype == "color")          type = Shader::Function::Color;
    else if (ftype == "linear color")   type = Shader::Function::LinearColor; // do we need this?
    else if (ftype == "source")         type = Shader::Function::Source;

    Shader::Function* f = NULL;

    try
    {
        f = new Shader::Function(fname,
                                 source,
                                 type,
                                 intValue("function.fetches"),
                                 docs);
        ...
        if (type == Shader::Function::UndecidedType)
        {
            switch (f->type())
            {
              case Shader::Function::Source:                ftype = "source"; break;
              default:
              case Shader::Function::Color:                 ftype = "color"; break;
              case Shader::Function::LinearColor:           ftype = "linear color"; break;
              case Shader::Function::Merge:                 ftype = "merge"; break;
              case Shader::Function::Filter:                ftype = "filter"; break;
              case Shader::Function::MorphologicalFilter:   ftype = "morphological"; break;
            }
        ...
        switch (f->type())
        {
          case Shader::Function::Source:                eguess = "generator"; break;
          default:
          case Shader::Function::Color:                 eguess = "color"; break;
          case Shader::Function::LinearColor:           eguess = "color"; break;
          case Shader::Function::Merge:                 eguess = "merge"; break;
          case Shader::Function::Filter:
          case Shader::Function::MorphologicalFilter:   eguess = "filter"; break;
        }
...
bool
NodeDefinition::rebuildFunction()
{

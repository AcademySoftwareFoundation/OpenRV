...
IPNode::IPNode(const string& name,
...
IPNode::~IPNode()
{
    ...
    if (graph()) graph()->removeNode(this);
}

void
IPNode::copyNode(const IPNode* node)
{
...
void
IPNode::copy(const PropertyContainer* p)
{
    PropertyContainer::copy(p);
}

bool
IPNode::isWritable() const
{
    if (group())
    {
        if (!group()->isWritable()) return false;
    }
...
int
IPNode::indexOfChild(IPNode* n)
{
    IPNodes::iterator it = std::find(m_inputs.begin(),
                                     m_inputs.end(),
                                     n);

    if (it == m_inputs.end()) return -1;
    return int(it - m_inputs.begin());
}

void
IPNode::appendInput(IPNode* n)
{
...
void
IPNode::insertInput(IPNode* n, size_t index)
{
...
bool
IPNode::isInput(IPNode* n) const
{
    IPNodes::const_iterator it = std::find(m_inputs.begin(),
                                           m_inputs.end(),
                                           n);
...
void
IPNode::disconnectOutputs()
{
    while (!m_outputs.empty()) m_outputs.back()->removeInput(this);
}

void
IPNode::disconnectInputsAtomic()
{
...
void
IPNode::setInputs(const IPNodes& nodes)
{
    HOP_PROF_FUNC();
    if (m_inputs == nodes) return;
    ...
    for (size_t i=0; i < nodes.size(); i++)
    {
        if (!nodes[i])
        {
            cout << "ERROR: NULL node passed to setInputs in node " << name()
                 << endl;
        }
    }
...

    {
        HOP_PROF_DYN_NAME(
            std::string(
                std::to_string(m_inputs.size()) +
                std::string(" nodes - outputDisconnect")).c_str() );
...

    {
        ...
        for (size_t i=0; i < m_inputs.size(); i++)
        {
            if (!m_unconstrainedInputs && m_inputs[i]->group() != group())
            {
                ...
                if (m_inputs[i]->group()) cout << " is member of " << m_inputs[i]->group()->name();
                    else cout << " is top level node";
                cout << endl;
                cout << "         rhs node " << name();
                if (group()) cout << " is member of " << group()->name();
                    else cout << " is top level node";
                cout << endl;
            }

            //
            //  For N copies of an input there will be N copies at the output
            //
    ...
    }
    {
        ...
        if (graph()) graph()->inputsChanged(this);
    }
}

void
IPNode::outputDisconnect(IPNode* node)
{
    IPNodes::iterator i = std::find(m_outputs.begin(), m_outputs.end(), node);
    if (i != m_outputs.end()) m_outputs.erase(i);
...
IPNode::Context
IPNode::contextForFrame(int frame)
{
    return graph()->contextForFrame(frame);
}

IPImage*
IPNode::evaluate(const Context& context)
{
    const IPNodes& nodes = inputs();
    if (nodes.empty()) return IPImage::newNoImage(this, "No Input");
    IPImage* head = 0;
...
IPImageID*
IPNode::evaluateIdentifier(const Context& context)
{
...
IPNode::Matrix
IPNode::localMatrix(const Context&) const
{
    return Matrix();
}
...
void
IPNode::metaEvaluate(const Context& context, MetaEvalVisitor& visitor)
{
    const IPNodes& nodes = inputs();
...
void
IPNode::visitRecursive(NodeVisitor& visitor)
{
    const IPNodes& nodes = inputs();
...
void
IPNode::testEvaluate(const Context& context, TestEvaluationResult& result)
{
...
void
IPNode::mediaInfo(const Context& context, MediaInfoVector& infos) const
{
    for (size_t i = 0; i < m_inputs.size(); i++)
    {
...
void
IPNode::setMediaActive(bool state)
{
    if (!m_inputs.empty())
    {
...
size_t
IPNode::audioFillBuffer(const AudioContext& context)
{
    //
...
void
IPNode::propagateFlushToInputs(const FlushContext& context)
{
...
void
IPNode::propagateFlushToOutputs(const FlushContext& context)
{
    flushAllCaches(context);
...
void
IPNode::propagateRangeChange(PropagateTarget target)
{
    if (m_deleting) return;
    ...
    {
        if (group()) group()->propagateRangeChange(target);
    }
}

void
IPNode::propagateImageStructureChange(PropagateTarget target)
{
    if (m_deleting) return;
    ...
    if (target & GroupPropagateTarget)
    {
        if (group()) group()->propagateImageStructureChange(target);
    }
}

void
IPNode::propagateMediaChange(PropagateTarget target)
{
    if (m_deleting) return;
    ...
    if (target & GroupPropagateTarget)
    {
        if (group()) group()->propagateMediaChange();
    }
}

void
IPNode::propagateInputChangeInternal()
{
    if (m_deleting) return;

    if (m_outputs.empty())
    {
        if (group()) group()->propagateInputChangeInternal();
    }
    else
    {
        for (size_t i=0; i < m_outputs.size(); i++)
...
void
IPNode::propagateStateChangeInternal()
{
    if (m_deleting) return;
    ...
    if (m_outputs.empty())
    {
        if (group()) group()->propagateStateChangeInternal();
    }
    else
    {
        for (size_t i=0; i < m_outputs.size(); i++)
...
void
IPNode::propagateRangeChangeInternal(PropagateTarget target)
{
    if (m_deleting) return;
    ...
    if (m_outputs.empty())
    {
        ...
        {
            if (!group() && graph()) graph()->rangeChanged(this);
        }
    }
    else
    {
...
void
IPNode::propagateImageStructureChangeInternal(PropagateTarget target)
{
    if (m_deleting) return;
...
void
IPNode::propagateMediaChangeInternal(PropagateTarget target)
{
    if (m_deleting) return;
    ...
    if (m_outputs.empty())
    {
        ...
        if (target & GraphPropagateTarget)
        {
            if (!group() && graph()) graph()->mediaChanged(this);
        }
    }
    else
    {
...
void
IPNode::propagateAudioConfigToInputs(const AudioConfiguration& config)
{
...
void
IPNode::propagateGraphConfigToInputs(const GraphConfiguration& config)
{
...
bool
IPNode::testInputs(const IPNodes& nodes, ostringstream& msg) const
{
    bool ok = true;
...
bool
IPNode::testInputsInternal(const IPNode* root) const
{
    if (this == root) return false;

    for (size_t i = 0; i < m_inputs.size(); i++)
    {
        if (!m_inputs[i]->testInputsInternal(root)) return false;
    }
...
IPNode::compareToInputs(const IPNodes& nodes) const
{
    if (m_inputs.size() <= nodes.size())
    {
        ...
        if (!differ)
        {
            if (m_inputs.size() == nodes.size()) return IdenticalResult;
            else return AppendedResult;
        }
    ...
    if (m_inputs.size() == nodes.size())
    {
        ...
        {
            if (a[i] != b[i]) return DiffersResult;
        }
...
void
IPNode::inputReordering(const IPNodes& newOrder, IndexArray& indices) const
{
...
void
IPNode::reorder(const IndexArray& indices)
{
...
void
IPNode::inputPartialReordering(const IPNodes& newOrder,
                               ...
                               IndexArray& reorderIndices) const
{
    ...
    for (size_t i = 0; i < s; i++)
    {
        ...
        if (!found) removeIndices.push_back(i);
    }
}

void
IPNode::findInputs(const IndexArray& indices, IPNodes& nodes) const
{
    nodes.clear();
...
void
IPNode::mapInputToEvalFrames(size_t inputIndex,
...
void
IPNode::MetaEvalPath::enter(const Context& c, IPNode* n)
{
    info.push_back(MetaEvalInfo(c.frame, n));
    if (n == leaf) found = true;
}

void
IPNode::MetaEvalPath::leave(const Context& c, IPNode* n)
{
    if (!found) info.pop_back();
}

bool
IPNode::MetaEvalPath::traverseChild(const Context& c, size_t index, IPNode* parent, IPNode* child)
{
    return child->isMetaSearchable() && !found;
}

void
IPNode::MetaEvalClosestByTypeName::enter(const Context& c, IPNode* node)
{
...
bool
IPNode::MetaEvalClosestByTypeName::traverseChild(const Context& c, size_t index, IPNode* parent, IPNode* child)
{
    return child->isMetaSearchable() && (info.empty() || info.back().node != parent);
}

void
IPNode::MetaEvalFirstClosestByTypeName::enter(const Context& c, IPNode* node)
{
    if (node->protocol() == typeName)
    {
        info.push_back(MetaEvalInfo(c.frame, node));
...
bool
IPNode::MetaEvalFirstClosestByTypeName::traverseChild(const Context& c, size_t index, IPNode* parent, IPNode* child)
{
    return child->isMetaSearchable() && info.empty();
}

bool
IPNode::PropertyAsFramesVisitor::traverseChild(size_t index, IPNode* parent, IPNode* child)
{
    //
...
void
IPNode::PropertyAsFramesVisitor::enter(IPNode* n)
{
...
void
IPNode::PropertyAsFramesVisitor::leave(IPNode* n)
{
...
bool
IPNode::ClosestByTypeNameVisitor::traverseChild(size_t index, IPNode* parent, IPNode* child)
{
    return child->isMetaSearchable() && currentDepth <= maxDepth;
}

void
IPNode::ClosestByTypeNameVisitor::enter(IPNode* n)
{
    if (n->protocol() == name)
    {
        nodes.insert(n);
...
void
IPNode::ClosestByTypeNameVisitor::leave(IPNode* n)
{
    if (n->protocol() == name)
    {
        currentDepth--;
...
void
IPNode::collectInputs(IPNodes& nodes)
{
    for (size_t i = 0; i < m_inputs.size(); i++)
    {
...
IPNode::inputs4() const
{
    ...
    else return inputs()[3];
}

void
IPNode::setInputs1(IPNode* node0)
{
    IPNodes inputs(1);
    inputs[0] = node0;
    setInputs(inputs);
...
void
IPNode::setInputs2(IPNode* node0, IPNode* node1)
{
    IPNodes inputs(2);
    inputs[0] = node0;
    inputs[1] = node1;
...
void
IPNode::setInputs3(IPNode* node0, IPNode* node1, IPNode* node2)
{
    IPNodes inputs(2);
    inputs[0] = node0;
    inputs[1] = node1;
...
void
IPNode::setInputs4(IPNode* node0, IPNode* node1, IPNode* node2, IPNode* node3)
{
    IPNodes inputs(2);
    inputs[0] = node0;
    inputs[1] = node1;
...
IPImage::ResourceUsage
IPNode::accumulate(const IPImageVector& images)
{
...
IPImage::ResourceUsage
IPNode::filterAccumulate(const IPImageVector& images)
{
    IPImage::ResourceUsage usage;
...
struct CompareCount
{
    CompareCount(FieldFunc F) : field(F) {}
    FieldFunc field;
...
bool
assignByResourceCount(const IPNode* node,
                      FieldFunc field,
                      ...
                      size_t maxValue)
{
    ...
    if (ncurrent <= maxValue) return false;
    ...
    for (size_t i = 0;
         i < sortedImages.size() && ncurrent > maxValue;
         i++)
    {
...
// if there is any IPImage with paint commands that doesn't have an intermediate
// IPImage has an ancestor.
// this is because if there is no intermediate, everyone will render to opengl
// default framebuffer. This makes it not possible for paint to use the content of the
// existing render as a texture (as an erase stroke might).
// an alternative to this is to copy the default framebuffer content to a framebuffer object
// in renderPaint, but that would be more expensive.
bool
needToInsertIntermediateRecursive(const IPImage* root)
{
    if (!root->commands.empty()) return true;

    if (root->destination != IPImage::IntermediateBuffer
        && root->children)
    {
        bool needToInsert = needToInsertIntermediateRecursive(root->children);
        if (needToInsert) return true;
    }

    if (root->next) return needToInsertIntermediateRecursive(root->next);

    return false;
}

IPImage*
IPNode::insertIntermediateRendersForPaint(IPImage* root, const Context& context) const
{
    if (root->destination == IPImage::IntermediateBuffer)
    {
        return root;
    ...
    if (root->children) needToInsert = needToInsertIntermediateRecursive(root->children);
    if (!needToInsert) return root;

    IPImage* nroot = new IPImage(this,
...
void
IPNode::balanceResourceUsage(UsageFunction accumFunc,
                             IPImageVector& images,
                             IPImageSet& modified,
                             size_t maxBuffers,
                             size_t maxCoords,
...
bool
IPNode::willConvertToIntermediate(IPImage* img)
{
    return (img->renderType == IPImage::BlendRenderType &&
        !img->shaderExpr &&
        !img->mergeExpr && img->children &&
        !img->noIntermediate);
}

bool
IPNode::convertBlendRenderTypeToIntermediate(IPImage* img)
{
...
void
IPNode::assembleMergeExpressions(IPImage* root,
                                 ...
                                 Shader::ExpressionVector& inExpressions)
{
    for (size_t i = 0; i < inImages.size(); i++)
    {
        ...
        {
            IPImage* newImage = new IPImage( this, IPImage::BlendRenderType,
                                             inImages[i]->width,
                                             inImages[i]->height,
                                             1.0,
                                             IPImage::IntermediateBuffer,
        ...
        else if (image->mergeExpr)
        {
            ...
            {
                inExpressions.push_back(replaceSourceWithExpression(image->shaderExpr,
                                                                    image->mergeExpr));
                image->mergeExpr   = 0;
                image->shaderExpr  = 0;
...
void
IPNode::filterLimits(const IPImageVector& inImages, IPImageSet& modifiedImages)
{
    //
...
void
IPNode::collectMemberNodes(IPNodeSet& nodeSet, size_t depth)
{
    if (depth != 0) nodeSet.insert(this);
}
...
//
std::string
IPNode::getUnqualifiedName (const std::string &name)
{
...
std::ostream& operator << (std::ostream& o, const IPNode::ImageComponent& c)
{
    ...
    {
        default:
        case IPNode::NoComponent:      o << "None:"; break;
        case IPNode::ViewComponent:    o << "View:"; break;
        case IPNode::LayerComponent:   o << "Layer:"; break;
        case IPNode::ChannelComponent: o << "Channel:"; break;
    }
    for (size_t i = 0; i < c.name.size(); ++i)
    {
        o << c.name[i];
        if (i < c.name.size()-1) o << ",";
    }
...
IPNode::isolate()
{
    ...
    if (!component("__graph"))
    {
        ...
        for (size_t i = 0; i < m_inputs.size(); i++) (*sp)[i] = m_inputs[i]->name();
...
bool
IPNode::isIsolated() const
{
    return graph() && graph()->m_isolatedNodes.count(name()) > 0;
}

void
IPNode::setGraph(IPGraph* graph)
{
    m_graph = graph;
}
...
IPNode::undoDeref()
{
    ...
    m_undoRefCount--;
    if (m_undoRefCount==0) delete this;
}

int
IPNode::mapToInputIndex(IPNode * srcNode, int srcOutIndex) const
{
    auto inputNdx = 0;

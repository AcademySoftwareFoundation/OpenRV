...
Application::Application() : TwkApp::Application()
{
    //
...
void
Application::loadOptionNodeDefinitions()
{
    string nodePath = optionValue<string>("nodePath", "");
    if (nodePath != "") m_nodeManager->loadDefinitionsAlongPathVar(nodePath);

    string nodeOverrideFile = optionValue<string>("nodeDefinitionOverride", "");
    if (nodeOverrideFile != "") m_nodeManager->loadDefinitions(nodeOverrideFile);
}

void
Application::createNewSessionFromFiles(const StringVector& files)
{
...
void
Application::startPlay(Session* s)
{
    for (int i=0; i < documents().size(); i++)
    {
        Session* session = static_cast<Session*>(documents()[i]);
        if (session->isUpdating() && session != s) return;
    }
...
Application::stopAll()
{
    ...
    for (int i=0; i < s; i++)
    {
        ...
        if (session->isUpdating()) session->stopCompletely();
    }
}
...
Application::resumeAll()
{
    ...
    {
        Session* session = static_cast<Session*>(documents()[i]);
        if (m_wasPlaying[i]) session->play();
    }
}

void
Application::stopPlay(Session* s)
{
    for (int i=0; i < documents().size(); i++)
    {
        Session* session = static_cast<Session*>(documents()[i]);
...
bool
Application::requestMemory(size_t bytes)
{
    lock();
    bool ok = false;
...
void
Application::freeMemory(size_t bytes)
{
    lock();
    m_usedMemory -= bytes;
    unlock();
...
static void
checkExists(const string& filename)
{
...
Application::FileKind
Application::fileKind(const std::string& filename)
{
...
string
Application::userGenericEventOnAll(const string& eventName,
...
Session*
Application::session(const string& name) const
{
    for (int i=0; i < documents().size(); i++)
    {
        Session* session = static_cast<Session*>(documents()[i]);
...
//
static int
myFind (string lookingIn, string lookingFor, Application::HostOS rvHostOS, bool wantLowerCase = false)
{
    // this lowercaseing used to be done with defines for windows only, 
    // but since there is no platform specific API calls, use our new enum instead.
    // allows for testing RV_OS pathswapping on all platforms from one platform
...
// added a wantLowerCase arg to enable case insenstive matching of windows based paths
// default false assignments allows previous API to be unchanged, especially for write cases.
string 
Application::mapToFrom (const string &s, PropEnvMap &em, bool wantLowerCase = false, bool wantPathSanitize = false)
{
    string newS = s; 
    string path;
    int maxSize = 0;
    ...
    for (PropEnvMap::const_iterator i = em.begin(); i != em.end(); ++i)
    {
        ...
        if (i->second.find("RV_PATHSWAP_") == string::npos) path_sanitation(k);
...
string
Application::mapFromVar (const string &in)
{
...
string
Application::mapToVar (const string &in)
{
    return mapToFrom (in, m_toVarMap);
}

void 
Application::setOptionValueFromEnvironment(const string& key, const std::string& envVar)
{
    if (const char* c = getenv(envVar.c_str())) setOptionValue<string>(key, c);
}

...
bool 
CombineIPInstanceNode::testInputs(const IPNodes& inputs, std::ostringstream& msg) const
{
    if (inputs.size() != 1)
    {
        string un = uiName();
...
void
CombineIPInstanceNode::findProps()
{
...
void 
CombineIPInstanceNode::propertyChanged(const Property* p)
{
    if (!isDeleting())
    {
...
void
CombineIPInstanceNode::computeStructure()
{
...
void 
CombineIPInstanceNode::propertyDeleted(const std::string&)
{
    findProps();
}

void 
CombineIPInstanceNode::newPropertyCreated(const Property*)
{
    findProps();
}

void 
CombineIPInstanceNode::inputImageStructureChanged(int inputIndex, PropagateTarget target)
{
    lock();
    m_structureInfoDirty = true;
    unlock();
...
{
    if (m_structureInfoDirty) lazyUpdateRanges();
    return m_structureInfo;
}

void
CombineIPInstanceNode::lazyUpdateRanges() const
{
    lock();
...
void
CombineIPInstanceNode::contextAtInput(Context& context, size_t i)
{
    if (IntProperty* p = m_frameProps[i]) context.frame = p->front();
    if (IntProperty* p = m_offsetProps[i]) context.frame += p->front();
...
IPImage*
CombineIPInstanceNode::evaluate(const Context& context)
{
    const Shader::Function* F = definition()->function();
        ...
        try
        {
            ...
            if (!current)
            {
                TWK_THROW_STREAM(EvaluationFailedExc, 
                                 "CombineIPInstanceNode evaluation failed on node " << nodes[0]->name());
            }
...
IPImageID*
CombineIPInstanceNode::evaluateIdentifier(const Context& context)
{
    const Shader::Function* F = definition()->function();
...
void 
CombineIPInstanceNode::metaEvaluate(const Context& context, MetaEvalVisitor& visitor)
{
    const Shader::Function* F = definition()->function();
...
void
CombineIPInstanceNode::readCompleted(const string& t, unsigned int v)
{
    lock();
    m_structureInfoDirty = true;
    unlock();

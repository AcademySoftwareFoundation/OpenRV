...
HistogramIPNode::evaluate(const Context& context)
{
    ...
    if (m_active && !m_active->front()) return image;
    ...
    if (scale > 1)
    {
        ...
        IPImage* smallImage = new IPImage(this,
    ...
    {
        // this is the case where the image has a fb, and cannot be converted by convertBlend func
        IPImage* insertImage = new IPImage(this,
                                          IPImage::BlendRenderType,
                                          newImage->width,
                                          newImage->height,
                                          1.0,
    ...
    IPImage* histo = new IPImage(this,
                                 IPImage::BlendRenderType,
                                 256,
                                 1,
                                 1.0,
    ...
    // this image will have a shaderExpr that uses the histogram result to render a histogram
    size_t width = 256;
    size_t height = m_height? m_height->front() : 100;
  
    IPImage* result = new IPImage(this,

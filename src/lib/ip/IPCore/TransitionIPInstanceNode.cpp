...
TransitionIPInstanceNode::TransitionIPInstanceNode(const std::string& name,
...
void
TransitionIPInstanceNode::setInputs(const IPNodes& nodes)
{
...
void 
TransitionIPInstanceNode::propertyChanged(const Property* p)
{
    if (!isDeleting())
    {
...
void
TransitionIPInstanceNode::lazyUpdateRanges() const
{
    lock();
...
void 
TransitionIPInstanceNode::inputRangeChanged(int inputIndex, PropagateTarget target)
{
    lock();
    m_rangeInfoDirty = true;
    unlock();
...
void 
TransitionIPInstanceNode::inputImageStructureChanged(int inputIndex, PropagateTarget target)
{
    lock();
    m_structureInfoDirty = true;
    unlock();
...
void
TransitionIPInstanceNode::computeRanges()
{
...
int
TransitionIPInstanceNode::inputFrame(size_t index, int frame, bool unconstrained)
{
    ...
    if (index == 0)
    {
        if (frame - 1 >= len) return fe;
        if (frame <= 1) return fs;
        return fs + frame - 1;
    }
    else
    {
...
IPImage*
TransitionIPInstanceNode::evaluate(const Context& context)
{
    lazyUpdateRanges();
    ...
    try
    {
        for (size_t i = 0; i < 2; i++)
        {
            ...
            if (ninputs == 1) return current;

            if (!current)
            {
                TWK_THROW_STREAM(EvaluationFailedExc, "TransitionIPInstanceNode evaluation failed on node "
                                 << nodes[i]->name());
            }
    ...
    balanceResourceUsage(F->isFilter() ? IPNode::filterAccumulate : IPNode::accumulate,
                         images,
                         modifiedImages,
                         8, 8, 81);
        ...
        for (size_t i = 0; i < ninputs; i++)
        {
            ...
            if (!imgid) 
            {
                ...
                TWK_THROW_STREAM(EvaluationFailedExc, 
                                 "NULL imgid in TransitionIPInstanceNode " << name()
...
void
TransitionIPInstanceNode::testEvaluate(const Context& context, TestEvaluationResult& result)
{
    lazyUpdateRanges();
    IPNodes  ins     = inputs();
...
void
TransitionIPInstanceNode::metaEvaluate(const Context& context, MetaEvalVisitor& visitor) 
{
    lazyUpdateRanges();
...
TransitionIPInstanceNode::imageRangeInfo() const
{
    lazyUpdateRanges();
    return m_info;
}
...
TransitionIPInstanceNode::audioFillBuffer(const AudioContext& context)
{
    ...
    AudioBuffer tempbuffer(context.buffer.size(),
    ...
    {
        //
        //  For foffset and transition sample bounds, we must take account of
        //  the fact that the second input is shifted in time (unless
        //  startFrame is one).
        //

        const long foffset = inputFrame(i, m_info.start, true) - m_rangeInfos[i].start -
                ((i == 1) ? m_startFrame->front() - m_info.start : 0);

        size_t transitionStartSample = timeToSamples(((i == 1) ? 0 : (m_startFrame->front() - 1)) / fps, rate);
        size_t transitionEndSample   = timeToSamples(((i == 1) ? m_duration->front() : m_startFrame->front() - 1 + m_duration->front() + 1) / fps, rate);
        ...
        if (!(gss >= gend || gse < gstart))
        {
            ...
            //
            //  Silence samples in this buffer that come from audio outside the
            //  input's frame range.
            //
...
void
TransitionIPInstanceNode::mapInputToEvalFramesInternal(size_t inputIndex, 
                                                       const FrameVector& inframes,
                                                       FrameVector& outframes) const
{
...
void
TransitionIPInstanceNode::propagateFlushToInputs(const FlushContext& context)
{
    Context c = context;
...
void
TransitionIPInstanceNode::readCompleted(const string& t, unsigned int v)
{
    lock();
    m_rangeInfoDirty = true;
    m_structureInfoDirty = true;

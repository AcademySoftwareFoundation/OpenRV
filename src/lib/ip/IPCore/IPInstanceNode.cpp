...
IPInstanceNode::init()
{
    ...
    if (const Component* c = def->component("parameters"))
    {
        ...
        //
        //  Add the new component to this Node (PropertyContainer) _before_ we
        //  add the parameters (properties) to the component.  Otherwise the
        //  properties do not "inherit" the node as their contiainer, so they
        //  do not have one at all, and things go wrong (in particular no
        //  message is sent by the graph when they are changed).
        //
...
IPInstanceNode::~IPInstanceNode()
{
    pthread_mutex_destroy(&m_lock);
}

bool 
IPInstanceNode::isActive() const
{
    return m_activeProperty->size() == 1 && m_activeProperty->front() == 1;
}

bool 
IPInstanceNode::testInputs(const IPNodes& inputs, std::ostringstream& msg) const
{
...
Shader::BoundSymbol* 
IPInstanceNode::boundSymbolFromSymbol(IPImage* image,
                                      const Shader::Symbol* s,
                                      const Context& context) const
{
    ...
    if ((s->name() == "_offset" || s->name() == "offset") && 
        s->type() == Shader::Symbol::Vec2fType &&
        s->isSpecial())
    {
        //
          ...
          case Shader::Symbol::Sampler2DRectType:
              {
                  ...
                  if (const ImageProperty* ip = dynamic_cast<const ImageProperty*>(prop))
                  {
                      if (zs != 0 && ws == 0)
                      {
                          ...
                          {
                            case Property::FloatLayout: dtype = FrameBuffer::FLOAT; break;
                            case Property::IntLayout:   dtype = FrameBuffer::UINT; break;
                            case Property::ShortLayout: dtype = FrameBuffer::USHORT; break;
                            case Property::HalfLayout:  dtype = FrameBuffer::HALF; break;
                            case Property::DoubleLayout: dtype = FrameBuffer::DOUBLE; break;
                            default:
                            case Property::ByteLayout:  dtype = FrameBuffer::UCHAR; break;
                          }

                          FrameBuffer* fb = new FrameBuffer(FrameBuffer::PixelCoordinates,
                                                            ys, zs, 0, xs,
                                                            dtype,
                                                            const_cast<unsigned char*>(ip->data<unsigned char>()),
                                                            NULL, FrameBuffer::BOTTOMLEFT,
                                                            false);
...
Shader::Expression*
IPInstanceNode::bind(IPImage* image,
                     ...
                     const Context& context) const
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        {
            // do nothing because we will send values of these parameters from the renderer at runtime
        }
    }
...
void 
IPInstanceNode::addFillerInputs(IPImageVector& images)
{
    const Shader::Function* F = definition()->function();

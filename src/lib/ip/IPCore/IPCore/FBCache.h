...
class FBCache : public TwkFB::Cache
{
  public:
    ...
    enum FreeMode
    {
        ...
        ActiveFreeMode,         /// allows freeing in in/out range behind display frame
        GreedyFreeMode          /// allows freeing in in/out range in FRONT of display frame
    };
    ...
    struct CacheStats
    {
        ...
        float            lookAheadSeconds;  /// as returned by computeLookAheadSecondsStat()
        FrameRangeVector cachedRanges;      /// as returned by computeCachedRangesStat()

        CacheStats() : capacity(0), used(0), lookAheadSeconds(0.0) {}
    };
    ...
    //  other FB references than those in <img> call trimFBsOfFrame() to ensure
    //  that.
    //
...
protected:
    struct CacheFrame
    {
        ...
        CacheFrame() : frame(0), utility(0.0), inc(0) {}
        CacheFrame(int f, float u) : frame(f), utility(u), inc(0) {}
    };
    ...
    // utility() returns a weighted value of the relative importance of a frame
    // with respect to the cache. The weighted value is adapted based on
    // whether the weighted value needs to be computed for caching a frame or
    // freeing a frame from the cache.
    enum UtilityMode 	
    {	
        FOR_CACHING,	
        FOR_FREEING	
    ...
    // When active tail caching is enabled, findBestCacheTarget() will consider 
    // frames both in the head and tail of the display frame.
    // When active tail caching is disabled, findBestCacheTarget() will only 

...
class Brush
{
  public:
    ...
    Brush(const std::string& name, FrameBuffer* fb, bool radial)
        : m_fb(fb), m_name(name), m_radial(radial), m_id(-1) {}
...
class Font
{
  public:
    ...
    std::pair<float,float> computeText(const std::string&, std::string&,
                                       float space, std::string origin) const;
...
{
  public:
    explicit CommandContext(TwkMath::Mat44f pid,
                           TwkMath::Mat44f m,
                           const GLFBO* i,
                           const GLFBO* t,
                           const GLFBO* c,
...
class PolyLine : public Command
{
  public:
    ...
    PolyLine(const Vec2* v=0, size_t n=0, float w=0, Color c=Color(0.0), bool o=false)
        : Command(), 
          npoints(n),
          width(w),
          smoothingWidth(1.0),
    ...
    ~PolyLine() { if (ownPoints) { delete []points; points = NULL; } }
    ...
    // the following 4 are computed / setup right after the geometry is computed
    // they only need to be recomputed if the geometry has changed
...
class Rectangle : public Command
{
  public:
    Rectangle(float h=0, float w=0, Color c = Color(0,0,0,1.0))
        : height(h), width(w), color(c), pos(0,0) {}

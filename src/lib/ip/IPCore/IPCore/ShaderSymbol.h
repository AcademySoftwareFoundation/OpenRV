...
//  Shader::Expression types together form a metalanguage which is used to
//  assemble the final Shader::Program. The metalanguage is primarily being
//  used to stich together known functions in the source languages.
//
...
//  as inlining).
//
//  Finally, the metalanguage is converted into the target language (GLSL,
//  OpenCL) and that is passed to the actual device.
//
...
//  function paramter. The GL type, array size (or 0 if its not an array)
//  and the name are stored with the Symbol.
//
...
class Symbol
{
  public:
    ...
    enum Qualifier
    {
        ...
        //  here. *Parameter* types are all "varying". Or in terms of GLSL >
        //  1.2 these are "out"
        //
    ...
    enum Type
    {
        ...
        //  add the assembly mechanism for the program and the underlying
        //  code that passes the value to the program.
        //
        //  built-in types
        //  --------------
        //
        ...
        VoidType, // degenerate
        FloatType,  Vec2fType, Vec3fType, Vec4fType,
        IntType,    Vec2iType, Vec3iType, Vec4iType,
        BoolType,   Vec2bType, Vec3bType, Vec4bType,
        Matrix4fType, Matrix3fType, Matrix2fType, 
        FloatArrayType, IntArrayType, BoolArrayType,
        Sampler1DType, Sampler2DType,
        Sampler2DRectType, Sampler3DType,
        Coord2DRectType, InputImageType, 
        OutputImageType,
    };
    ...
    bool                isSampler() const { return m_type == Sampler1DType || m_type == Sampler2DType || m_type == Sampler3DType || m_type == Sampler2DRectType; }
...
class BoundSymbol
{
  public:
    ...
    explicit BoundSymbol(const Symbol* p) : m_symbol(p), m_isExpression(false) {}
    virtual ~BoundSymbol() {}
...
class ProxyBoundSymbol : public BoundSymbol
{
  public:
    ...
    explicit ProxyBoundSymbol(const Symbol* p) : BoundSymbol(p) {}
...
struct ImageOrFB
{
    ImageOrFB() : fb(0), image(0), plane(0) {}
    ImageOrFB(const TwkFB::FrameBuffer* f, size_t p=0) : fb(f), image(0), plane(p) {}
    explicit ImageOrFB(const IPImage* i) : fb(0), image(i), plane(0) {}
    explicit ImageOrFB(const IPImage* img, const TwkFB::FrameBuffer* f, size_t p=0) : image(img), fb(f), plane(p) {}
...
{
    ImageCoord() : image(0) {}
    explicit ImageCoord(const IPImage* i) : image(i) {}
    const IPImage* image;
};

struct ImageCoordName
{
    explicit ImageCoordName() : name("UNINITIALIZED") {}
    explicit ImageCoordName(const std::string& s) : name(s) {}
    std::string name;
};

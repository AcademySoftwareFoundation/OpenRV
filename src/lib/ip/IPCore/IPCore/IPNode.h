...
#define IPGRAPH_CONSTRUCTOR_DECLARATION(NAME) \
    static IPNode* NAME(const std::string& name, const NodeDefinition*, IPGraph*, GroupIPNode*)
...
class IPNode : public TwkContainer::PropertyContainer
{
  public:
    ...
    //
    //  For some applications (EG session writing), we want to maintain a sorted
    //  set.  Note that the use of the name() method here _requires_ that the
    //  node not be deleted before it is removed from the set.  Also the name
    //  of a node should not change while it is in the set.
    //
    struct NodeComp
    {
    ...
    enum ThreadType
    {
        ...
        DisplayThread               = DisplayNoEvalThread |
                                      DisplayEvalThread |
                                      DisplayPassThroughThread |
                                      DisplayMaybeEvalThread |
    ...
    enum PropagateTarget : int
    {
        ...
        LegacyPropagateTarget   = GroupPropagateTarget | GraphPropagateTarget | OutputPropagateTarget
                                | MemberPropagateTarget | GroupAndGraphInOutputPropagateTarget,

        // new fast event propagation behaviour
        // we avoid to propagate in graph and group after an output is crossed
        FastPropagateTarget     = GroupPropagateTarget | GraphPropagateTarget | OutputPropagateTarget
    };

    struct ImageComponent
    {
        ImageComponent(ComponentType t = NoComponent) : type(t) {}
        ImageComponent(ComponentType t,
                       const std::string& a)
            : type(t), name(1) { name[0] = a; }
        ImageComponent(ComponentType t,
                       const std::string& a,
                       const std::string& b)
            : type(t), name(2) { name[0] = a; name[1] = b; }
        ImageComponent(ComponentType t,
                       const std::string& a,
                       const std::string& b,
                       const std::string& c)
            : type(t), name(3) { name[0] = a; name[1] = b; name[2] = c; }

        bool isValid() const { return (type == ViewComponent && name.size() == 1) ||
    ...
    {
        //
        //  All these vars refer to bits of state set by the evaluate() caller,
        //  so that stereo evaluators can do the right thing.  Typical case is
        //  that vars are set by DisplayStereoIPNode, so that
        //  SourceStereoIPNode can respond appropriately.
        //
        ...
        bool  flip;        //  Right-eye flip will happen somewhere closer to the root
        bool  flop;        //  Right-eye flop will happen somewhere closer to the root
        ...
        Vec2  rtranslate;  //  Additional right-eye translation **CURRENTLY UNUSED**
    };
    ...
    struct Context
    {
        ...
        mutable StereoContext stereoContext; /// set at display level to inform source-level stereo eval.
    ...
    struct ProtocolP
    {
        ProtocolP(const char* p) : protocol(p) {}
    ...
    struct FlushFBs
    {
        FlushFBs(const FlushContext& c_) : c(c_) {}
    ...
    {
        TestEvaluationResult() : poorRandomAccessPerformance(false) {}
    ...
    void deleteInputs(); // NOTE: actually deletes them not just setting to nothing
    ...
    struct ImageRangeInfo
    {
        ImageRangeInfo(int s=0, int e=0, int i=0, float f=0, int ci=0, int co=0, int b=false)
            : start(s), end(e), inc(i), fps(f), cutIn(ci), cutOut(co), isUndiscovered(b) {}
    ...
    struct MediaInfo
    {
        MediaInfo(const std::string& n="",
                  const TwkMovie::MovieInfo* i=0,
                  IPNode* nd=NULL) : name(n), info(i), node(nd) {}

        MediaInfo(const std::string& n,
                  const TwkMovie::MovieInfo& i,
                  IPNode* nd) : name(n), info(&i), node(nd) {}
    ...
    void findInEvaluationPath(int frame,
                              Predicate,
                              IPNodes&,
    ...
    struct MetaEvalInfo
    {
        MetaEvalInfo(int f, IPNode* n)
            : sourceFrame(f), node(n) {}
        MetaEvalInfo()
            : sourceFrame(0), node(0) {}

        bool operator<(const MetaEvalInfo& i) const
        {
            return (node < i.node || (node == i.node && sourceFrame < i.sourceFrame));
        }
    ...
    {
      public:
        MetaEvalInfoCollector(MetaEvalInfoVector& i) : info(i) {}
    ...
    {
      public:
        MetaEvalInfoCollectorByType(MetaEvalInfoVector& i) : info(i) {}
        MetaEvalInfoVector& info;

        virtual void enter(const Context& c, IPNode* n)
        {
            if (dynamic_cast<T*>(n)) info.push_back(MetaEvalInfo(c.frame, n));
        }
    };
    ...
    class ClosestByTypeNameVisitor : public NodeVisitor
    {
      public:
        ClosestByTypeNameVisitor(const std::string& typeName, size_t depth=1)
            : name(typeName), currentDepth(0), maxDepth(depth) {}
  ...
  protected:
    ...
    static bool convertBlendRenderTypeToIntermediate(const IPImageVector& inImages,
                                                     IPImageSet& modifiedImages);

    static std::string getUnqualifiedName (const std::string &name);


    void assembleMergeExpressions(IPImage* root,
...
template <typename Predicate>
void IPNode::findInEvaluationPath(int frame,
                                  Predicate P,
                                  IPNodes& nodes,
                                  bool ascendingOrder)
{
    ...
    {
        if (P(infos[i].node)) nodes.push_back(infos[i].node);
    }
}
...
template <class T>
IPNode* newIPNode(const std::string& name, const NodeDefinition* def, IPGraph* graph, GroupIPNode* group)
{

...
Expression* newColorCurveonY(Expression* FA,
...
Expression* newColorCineonLogToLinear (Expression*,
                                      double refBlack,
                                      double refWhite,
                                      double softClip);
Expression* newColorLinearToCineonLog (Expression*,
                                      double refBlack,
                                      double refWhite);
Expression* newColorLogCLinear(Expression*, 
                               float blackSignal,
                               ...
                               float linearOffset,
                               float cutoff);
Expression* newColorLinearLogC(Expression*, 
                               float blackSignal,
                               float encodingOffset,
                               float encodingGain,
                               float graySignal,
...
Expression* newColor3DLUT(Expression*,
                          const TwkFB::FrameBuffer*,
                          const TwkMath::Vec3f& outScale,
                          const TwkMath::Vec3f& outOffset);
...
Expression* newColorChannelLUT(Expression*,
                               const TwkFB::FrameBuffer*,
                               const TwkMath::Vec3f& outScale,
                               const TwkMath::Vec3f& outOffset);
Expression* newColorLuminanceLUT(Expression*,
                                 const TwkFB::FrameBuffer*);
Expression* newColorCDL(Expression*, 
                        const TwkMath::Vec3f& scale,
                        const TwkMath::Vec3f& offset,
                        const TwkMath::Vec3f& power,
                        float saturation,
                        bool noClamp);

Expression* newColorCDLForACES(Expression*, 
                               const TwkMath::Vec3f& scale,
                               const TwkMath::Vec3f& offset,
                               const TwkMath::Vec3f& power,
                               float saturation,
...
Expression* newColorClamp(Expression*,
                          float minValue = 0.0f,
                          float maxValue = 1.0f);

Expression* newConstantBG(const IPImage*,
                          Expression*,
                          const TwkMath::Vec3f&);
...
Expression* newDither(const IPImage*, 
                      Expression*,
                      size_t displayBits,
                      size_t seed);

Expression* newStencilBox(const IPImage*, 
                          Expression*);

Expression* newStencilBoxNegAlpha(const IPImage*, 
                                  Expression*);

Expression* newStereoChecker(const IPImage*, 
                             const float parityXOffset,
                             const float parityYOffset,
                             const std::vector<Expression*>&);

Expression* newStereoScanline(const IPImage*, 
                              const float parityYOffset,
                              const std::vector<Expression*>&);
...
Expression* newInlineBoxResize(bool adaptive,
                               const IPImage*, 
                               Expression*,
                               const TwkMath::Vec2f& scale);

Expression* newDownSample(const IPImage* image,
                          Expression*,
                          const int scale);
...
Expression* newUpSampleVertical(const IPImage* image,
                                Expression*,
                                bool);

Expression* newSimpleBoxFilter(const IPImage*,
                               Expression*,
                               const float size);
...
Expression* newLensWarp(const IPImage*,
                        Expression*,
                        float k1,
                        float k2,
                        float k3,
...
Expression* newLensWarp3DE4AnamorphicDegree6(
            const IPImage* image,
            Expression* FA,
            const TwkMath::Vec2f& c02,
            const TwkMath::Vec2f& c22,
            const TwkMath::Vec2f& c04,
            const TwkMath::Vec2f& c24,
            const TwkMath::Vec2f& c44,
            const TwkMath::Vec2f& c06,
            const TwkMath::Vec2f& c26,
            const TwkMath::Vec2f& c46,
            const TwkMath::Vec2f& c66,
            const TwkMath::Vec2f& center,
            const TwkMath::Vec2f& f,
            const TwkMath::Vec2f& cropRatio);
...
Expression* newICCLinearToSRGB(Expression*, 
...
IPImage* applyGaussianFilter(const IPNode* node, IPImage* image, size_t radius); // derive default sigma
...
IPImage* applyFastGaussianFilter(const IPNode* node,
                                 IPImage* image,
                                 size_t radius);
IPImage* applyFastGaussianFilter(const IPNode* node,
                                 IPImage* image,

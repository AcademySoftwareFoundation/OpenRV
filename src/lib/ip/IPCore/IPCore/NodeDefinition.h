...
/// NodeDefinition holds the state for a node loaded from filesystem or network
...
///     string node.evaluationType      determines IPInstanceNode sub-classes
///     int    node.userVisible         1 if user can make one of these from UI
///     int    function.fetches         number of textel fetches made by shader
///
...
class NodeDefinition : public TwkContainer::PropertyContainer
{
  public:
    ...
    NodeDefinition(const std::string& typeName, 
                   unsigned int version,
                   bool isGroup,
                   const std::string& defaultName,
                   NodeConstructor,
                   const std::string& summary,
                   const std::string& htmlDocs,
                   const ByteVector iconRGBA,
    ...
    template <typename T>
    T* newNodeOfType(const std::string& name, IPGraph* graph, GroupIPNode* group) const
    {

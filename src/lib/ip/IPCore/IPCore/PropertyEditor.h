...
//  For example:
//
//  PropertyEditor<FloatProperty> editor(graph(), "#RVSoundTrack.audio.volume");
//      -or-
...
PropertyEditor<T>::PropertyEditor(IPNode* node, const std::string& propName)
{
    ...
    if (prop) m_props.push_back(prop);
    m_graph = node->graph();
}

template <class T>
PropertyEditor<T>::PropertyEditor(T* prop, IPNode* node)
{
...
template <class T>
PropertyEditor<T>::PropertyEditor(IPGraph& graph, int frame, const std::string& name)
{
    graph.findPropertyOfType<T>(frame, m_props, name);
    if (m_props.empty()) TWK_THROW_STREAM(BadPropertyExc, "Unknown property " << name);
    m_graph = &graph;
}

template <class T>
PropertyEditor<T>::PropertyEditor(IPGraph& graph, const UntypedPropertyVector& propVector)
{
    ...
    for (size_t i = 0; i < s; i++) 
    {
        ...
        if (!p) TWK_THROW_STREAM(BadPropertyExc, "Bad property type");
        m_props[i] = p;
    }

    if (m_props.empty()) TWK_THROW_STREAM(BadPropertyExc, "No properties");
    m_graph = &graph;
}
...
template <class T>
typename PropertyEditor<T>::ValueType
PropertyEditor<T>::value()
{
    for (size_t i = 0; i < m_props.size(); i++)
    {
        if (m_props[i]->size() == 0) m_props[i]->resize(1);
    }
...
template <class T>
typename PropertyEditor<T>::ContainerType&
PropertyEditor<T>::container()
{
...
PropertyEditor<T>::setValue(const typename PropertyEditor<T>::ValueType& v)
{
    ...
    for (size_t i = 0; i < m_props.size(); i++)
    {
        ...
        if (m_props[i]->size() == 1 && m_props[i]->front() == v) continue;

        if (node) node->propertyWillChange(m_props[i]);

        m_props[i]->resize(1);
        m_props[i]->front() = v;

        if (node) node->propertyChanged(m_props[i]);
    }
}

template <class T>
void
PropertyEditor<T>::setValue(const ContainerVector& container)
{
    IPGraph::GraphEdit edit(*m_graph, m_props);
...
template <class T>
void
PropertyEditor<T>::setValue(const typename PropertyEditor<T>::ContainerType& v)
{
    IPGraph::GraphEdit edit(*m_graph, m_props);

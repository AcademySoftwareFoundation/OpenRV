...
//  implementations of the function (e.g. GLSL, OpenCL, CUDA, native) but
//  they are all expected to take some variant of the same set of
...
//  evaluation.
//
//  The constructor expects an approximation of the number of "fetches" (as
//  in texture fetches) that the function is expected to produce. If this
//  varies wildly supply a max expected number. For example, a source RGBA
//  shader which looks up a single RGBA color value from a texture does 1
//  fetch. A 3x3 blur filter does 9 fetches. A function which converts a
//  color from RGB to HSV does 0 fetches.
//
...
class Function
{
  public:
    ...
    {
        ResourceUsage() : fetches(0), buffers(0), coords(0) {}

        void accumulate(const ResourceUsage& u)
            { fetches += u.fetches; buffers += u.buffers; coords += u.coords; }

        void filterAccumulate(const ResourceUsage& u)
            { fetches *= u.fetches; buffers += u.buffers; coords += u.coords; }

        void clear()
            { fetches = 0; buffers = 0; coords = 0; }

        void set(size_t f, size_t b, size_t c) { fetches=f; buffers=b; coords=c; }
    ...
    struct ImageParameterInfo
    {
        ImageParameterInfo(const std::string& s, bool a, bool b, bool samp, bool lookup)
            : name(s), usesST(a), usesSize(b), usesSampling(samp), usesLookup(lookup) {}
    ...
    {                                   //             -------------
        UndecidedType       = 0,        //                            (inferred to be one of the below)
        Main                = 1 << 0,   //               () -> void
  ...
  private:
    //
    //  Function objects should not be directly deleted because pointers to
    //  them are used in sorting shader cache map (see ShaderProgram).
    //
    ~Function();

...
class ImageFBO
{
  public:
    ...
    // this is the type of FBO that belongs to device, are marked as such, and
    // cannot be grabbed by client rendering code.
    ImageFBO(size_t w,
             size_t h,
             GLenum iformat,
             bool belongToDevice=true) : available(0),
                                         target(0),
                                         type(0),
                                         format(iformat),
                                         expectedUseCount(0),
...
{
    // the manager also keeps track of how much memory has been allocated in FBO form
  public:
    typedef TwkGLF::GLFBO                       GLFBO;
    ...
    ImageFBOManager(): m_totalSizeInBytes(0) {}
    ...
    ImageFBO* newImageFBO(const IPImage*,
                          size_t fullSerialNum,
                          const std::string& identifier);
    
    ImageFBO* newImageFBO(const GLFBO*,
                          size_t fullSerialNum,
                          const std::string& identifier);
  
    ImageFBO* newImageFBO(size_t w,
                          size_t h,
                          GLenum target,

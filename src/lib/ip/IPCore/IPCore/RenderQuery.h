...
class RenderQuery
{
  public:
    ...
    // Constructor
    RenderQuery(const ImageRenderer* renderer): m_renderer(renderer) {}
    ...
    void imageCornersByHash(const IPImage::HashValue,
                            std::vector<Vec3f>&,
    ...
    void imageTransforms(const std::string& name,
                         Matrix& modelMatrix,
                         Matrix& projectionMatrix,
                         Matrix& textureMatrix,
                         Matrix& imageOrientation,
                         Matrix& imagePlacement
                         ) const;

    // Find frame-ratio by name. Name must match the exact source name.

    void imageFrameRatio(const std::string& name,
    ...
    void imageCorners(const std::string& source,
                      std::vector<Vec3f>& points,
                      bool stencil=true) const;
    
    void imageCorners(int index,
                      std::vector<Vec3f>& points,
                      bool stencil=true) const;

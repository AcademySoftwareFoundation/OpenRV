...
public:
    ...
    struct RendererParameters
    {
        ...
        Layout               layout;
        double               rate;
        double               latency;   // This represents the average device latency/av sync lag
    ...
    struct Module
    {
        Module(const std::string& n,
            ...
            : name(n), so(s), func(F), internal(isinternal) {}
    ...
    template<typename T> 
    static T toUnsignedType (float a)
    { 
        if (a > 1.0f) a = 1.0f;
        else if (a < -1.0f)  a = -1.0f;
    ...
    struct toUInt32_SMPTE299M_24_transformer
    {
        ...
        { 
            UInt32 b = bits;
            if (count % 192 == 0) b |= (1 << 3) | (1 << 30);
            count++;
            return toUInt32_SMPTE299M_24(a, b); 
        }
    };

    static void transformFloat32ToInt24 (TwkAudio::AudioBuffer::BufferPointer inData,
                                         char *outData,
                                         size_t dataCount,

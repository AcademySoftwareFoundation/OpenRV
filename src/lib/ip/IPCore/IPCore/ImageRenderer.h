...
//  ImageFBO manager keeps track of how much memory is currently in use for FBOs
//  GL State keeps track of how much memory is currently in use for textures
//  In the future GL State could track vbo memory too.
// 
 
//
//  Threaded Upload
//  Two modes in threaded upload: (1) prefetch fetches current frame
//  render waits until upload is done to proceed. this mode isn't that useful
//  (2) prefetch fetches what is predicted as the next frame (which could be wrong at times)
//  upload and render goes simultaneously. render thread will wait for upload thread
//  to return until it starts the next frame. Note return doesn't mean textures are uploaded
//  the texture fences are what actually tell us whether upload is fully finished
//

//
//  Description of what renderer does, for each frame
//  ImageRenderer::render() is the main entry point
//  i.    renderer does some book keeping (increase serial number, clean up old unused textures, etc.)
//  ii.   prepare all the textures that need to be uploaded (could be for current or next frame)
//  iii.  upload textures (could be for this or next frame, could be serial or threaded)
//  iv.   mark any ImageFBO we can reuse (hold on to them, make sure the renderer won't try to use them)
//  v.    start fresh
//        -- if necessary, wait for upload to finish before draw
//        -- assemble ImagePassState for each IPImage: match IPImage to all the textures that have been uploaded
//        -- draw whatever needs to be drawn (in some cases, nothing needs to be drawn)d
//        end
//  vi.   cleanup
//
...
public:
    ...
    struct TextureDescription
    {
        ...
        TextureDescription() : uploaded(false),
                               age(-1) {}
        ...
        int                 uncropHeight;
        int                 uncropX;      // position of the data window (can be negative) for uncropped
        int                 uncropY;
    ...
    typedef std::map<const std::string, TextureDescription*,
                     std::less<const std::string>, TextureDescriptionAlloc> FBToTextureMap;
    ...
    {
        LogicalImage()
        : width(0), height(0), depth(0), uncropWidth(0), uncropHeight(0),
        uncropX(0), uncropY(0), pixelAspect(1.0), initPixelAspect(1.0),
        isvirtual(true), isresident(false),
        node(0), Yw(0.0), stencilMin(0.0), stencilMax(0.0),
        render(false), touched(false), outputST(false), outputSize(false),
        coordinateSet(GLuint(-1)), imageNum(0), isPrimary(true) {}
    ...
    //
    // we do not cache these. they are assembled per render by tracing expressions
    // prefetch do not need these structs
    //
    ...
    typedef std::map<HashValue, ImagePassState*,
                     std::less<HashValue>, ImagePassStateAlloc> ImagePassStateMap;
    ...
    {
        RenderedImage(const std::string& name = ""): source(name),
                                                     node(0),
                                                     device(0),
                                                     serialNum(0) {}
    ...
    {
        GLTextureObject(GLuint id): textureID(id) {}
        GLTextureObject(): textureID(0) {}
        GLuint textureID;
    };
    ...
    struct Device
    {
        Device(const VideoDevice* d = 0,
               const GLVideoDevice* g = 0,
               const GLBindableVideoDevice* bd = 0,
               size_t ringBufferSize = 1,
               size_t nviews = 1);
    ...
    class AuxRender
    {
      public:
        AuxRender() {}
        virtual ~AuxRender() {}
        virtual void render(VideoDevice::DisplayMode,
                            bool leftEye,
                            bool rightEye,
                            bool forController,
    ...
    class AuxAudio
    {
      public:
        AuxAudio() {}
        virtual ~AuxAudio() {}
        virtual void* audioForFrame(int frame,
                                    size_t seqindex,
                                    size_t& n) const = 0;
    ...
    struct InternalRenderContext
    {
        ...
        bool                    mergeRender; // use the IPImage::mergeExpr instead of IPImage::shaderExpr
    ...
    {
        InternalGLMatricesContext(): targetFBOWidth(0),
                                     targetFBOHeight(0) {}
    ...
    bool hasMultipleOutputs() const     { return m_controlDevice.device != m_outputDevice.device
    ...
    void render(int frame,
                IPImage* root, 
                const AuxRender* aux=0,
                const AuxAudio* auxAudio=0,
                IPImage* uploadRoot = 0);
  ...
  private:
    ...
    void executeCLKernel(const CLContext& context,
    ...
    void renderPaint(const IPImage*,
                     const GLFBO*);
    ...
    void renderOutputs(int frame,
                       const IPImage*,
                       const AuxRender*,
                       const AuxAudio*);

    void renderMain(const GLFBO* Ftarget,
                    const GLFBO* Ltarget, 
                    const GLFBO* Rtarget,
                    const AuxRender* auxRenderer,
                    int frame, 
    ...
    void assignTextureUnits(const IPImage*,
                            bool);
    bool assignTextureUnits2(const IPImage*,
                             LogicalImage*,
                             size_t&,
                             size_t);
    ...
    void assembleAuxLogicalImage(const FrameBuffer*,
                                 LogicalImage*);
    
    void assemblePrimaryLogicalImage(const IPImage*,
                                     LogicalImage*);
    ...
    void uploadImage(LogicalImage*,
                     const IPImage*);
    
    void assignImage(LogicalImage*,
                     const IPImage*,
                     const GLFBO*,
                     bool,
                     bool,
    ...
    void assignAuxImage(LogicalImage* i,
                        const FrameBuffer* fb);

    void assignAuxImages(const IPImage*);

    void computeImageGeometry(const IPImage* img,
                              ImagePlane& plane) const;

    void computePlaneGeometry(const IPImage* img,
                              const FrameBuffer* fb,
                              ImagePlane& plane) const;
    ...
    void initializePlane(const IPImage* img,
                         const GLFBO*,
                         ImagePlane&) const;
    
    void initializeFBPlane(const IPImage* img,
                           const FrameBuffer*,
                           ImagePlane&,
                           float,
                           bool,
    ...
    void uploadPlane(const FrameBuffer*,
                     TextureDescription*,
                     GLuint filter);
    ...
    //
    // if any imageFBO matches any IPImage of the current render, make it as unavailable
    // this way we can prevent others from grabbing this imageFBO for this frame
    //
  ...
  private:
    ...
    FBToTextureMap            m_texturesToUpload; // render and upload thread will never read/write this simultaneously

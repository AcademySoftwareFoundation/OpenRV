...
IPImage*
ColorIPInstanceNode::evaluate(const Context& context)
{
    ...
    if (!current)
    {
        TWK_THROW_STREAM(EvaluationFailedExc, 
                         "ColorIPInstanceNode evaluation failed on node " << nodes[0]->name());
    }

    if (IntProperty* active =
        property<IntProperty>("color", "active"))
    {
        if (active->front() == 0) return current;
    }

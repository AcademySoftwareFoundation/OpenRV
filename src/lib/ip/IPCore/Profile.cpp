...
//
//  The 'tag' is an arbitary string that can be used for categorizing profiles
//  in the GUI and eliminanting name conflicts.  For example, all
//  DisplayPipelineGroup profiles use the tag 'display'.  It's up to the writer
//  of the profile file to put the tag in the filename.
//

static vector<path>
collectProfilesFromPath()
{
...
string
profileMatchingNameInPath(const string& name, const string& tag, IPGraph* graph)
{
    vector<path> paths = collectProfilesFromPath();

    path n = name;
    if (n.is_absolute() && exists(n)) return name;
...
void
profilesInPath(ProfileVector& profiles, const string& tag, IPGraph* graph)
{
    profiles.clear();
    vector<path> paths = collectProfilesFromPath();
    for (int i = 0; i < paths.size(); i++)
...
void
profileNodeInputs(const GTOReader& reader,
...
void
applyProfileGraph(const GTOReader& reader, 
                  IPNode* node,
                  PropertyContainer* profileNode,
                  const ConnectionContainer& connections)
{
    //
    //  Apply the state in profileNode to node (actual node in graph). If
    //  node is a group node recursively apply on all
    //  members. Recursively apply on inputs.
    //

    if (profileNode->protocol() != node->protocol()) return;
        ...
        if (GroupIPNode* group = dynamic_cast<GroupIPNode*>(node))
        {
            if (PropertyContainer* rootContainer = reader.findByName(rootName))
            {
                const ConnectionContainer& cons = 
                    profileNode->propertyContainer<StringPairProperty>("evaluation.connections");
        
                //
                //  Only recursively apply if there's actually a subgraph
                //  associated with the profileNode. 
                //
    ...
    if (profileInputs.size() == node->inputs().size())
    {
        ...
        for (size_t i = 0; i < profileInputs.size(); i++)
        {
            if (i < nodeInputs.size())
            {
                ...
                if (input) applyProfileGraph(reader, nodeInputs[i], input, connections);
            }
        }
    }
}
...
Profile::apply(IPNode* node)
{
    ...
    //
    //  Forbid, for now, applying a profile to a different type of group node
    //  from the type of the node from which the profile was saved.
    //

    if (m_root->protocol() != node->protocol())
    {
        TWK_THROW_EXC_STREAM("Profile: cannot apply " << m_root->protocol() << 
                             " profile to " << node->protocol() << " node.");
    }
...
{
    return m_header ? m_header->propertyValue<StringProperty>("root.comment", "") : "NOT LOADED";
}

string
Profile::structureDescription() const
{
    if (!m_header) return "NOT LOADED";
    ...
    for (size_t i = 0; i< containers.size(); i++)
    {
        ...
        if (IPNode* genericNode = m_graph->newNode(protocol, "_x"))
        {
            ...
            for (PropertyContainer::NamedPropertyMap::const_iterator q = pmap.begin();
                 q != pmap.end();
                 ++q)
            {

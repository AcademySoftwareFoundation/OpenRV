...
void
StackIPInstanceNode::updateOutOfRangePolicy()
{
    string policy = m_outOfRangePolicyProp->front();

    if      (policy == "blank") m_outOfRangePolicy = NoImageOutOfRange;
    else if (policy == "black") m_outOfRangePolicy = BlackOutOfRange;
    else if (policy == "hold") m_outOfRangePolicy = HoldOutOfRange;
    else m_outOfRangePolicy = NoImageOutOfRange;
}

void
StackIPInstanceNode::updateActiveAudioInput()
{
    m_activeAudioInputIndex = -2;
...
void
StackIPInstanceNode::setInputs(const IPNodes& nodes)
{
...
void 
StackIPInstanceNode::propertyChanged(const Property* p)
{
    if (!isDeleting())
    {
        if (p == m_useCutInfo || 
            p == m_alignStartFrames || 
            p == m_outOfRangePolicyProp || 
            p == m_outputFPS)
        {
            lock();
        ...
        if (p == m_activeAudioInput) updateActiveAudioInput();
    }
...
void
StackIPInstanceNode::lazyUpdateRanges() const
{
    lock();
...
void 
StackIPInstanceNode::inputRangeChanged(int inputIndex, PropagateTarget target)
{
    lock();
    m_rangeInfoDirty = true;
    unlock();
...
void 
StackIPInstanceNode::inputImageStructureChanged(int inputIndex, PropagateTarget target)
{
    lock();
    m_structureInfoDirty = true;
    unlock();
...
void
StackIPInstanceNode::computeRanges()
{
...
int
StackIPInstanceNode::inputFrame(size_t index, int frame, bool unconstrained)
{
...
IPImage*
StackIPInstanceNode::evaluate(const Context& context)
{
    ...
    try
    {
        ...
        for (unsigned int i = 0; i < ninputs; i++)
        {
            ...
            const bool outOfRange = useCutInfo 
                ? (inFrame < info.cutIn || inFrame > info.cutOut)
            ...
            if (!current)
            {
                //continue;
                IPNode* node = nodes[i];
                TWK_THROW_STREAM(EvaluationFailedExc, "MergeIPInstanceNode evaluation failed on node "
                                 << node->name());
            }
...
IPImageID*
StackIPInstanceNode::evaluateIdentifier(const Context& context)
{
    lazyUpdateRanges();
    IPNodes nodes   = inputs();
        ...
        for (size_t i = 0; i < ninputs; i++)
        {
            ...
            if (!imgid) 
            {
                ...
                TWK_THROW_STREAM(EvaluationFailedExc, 
                                 "NULL imgid in StackIPInstanceNode " << name()
...
void
StackIPInstanceNode::testEvaluate(const Context& context, TestEvaluationResult& result)
{
    lazyUpdateRanges();
    IPNodes  ins     = inputs();
    int      ninputs = inputs().size();
...
void
StackIPInstanceNode::metaEvaluate(const Context& context, MetaEvalVisitor& visitor) 
{
    lazyUpdateRanges();
...
StackIPInstanceNode::imageRangeInfo() const
{
    lazyUpdateRanges();
...
size_t
StackIPInstanceNode::audioFillBuffer(const AudioContext& context)
{
    lazyUpdateRanges();
...
void
StackIPInstanceNode::mapInputToEvalFrames(size_t inputIndex, 
...
void
StackIPInstanceNode::mapInputToEvalFramesInternal(size_t inputIndex, 
                                                  const FrameVector& inframes,
                                                  FrameVector& outframes) const
{
...
void
StackIPInstanceNode::readCompleted(const string& t, unsigned int v)
{
    updateActiveAudioInput();
    updateOutOfRangePolicy();

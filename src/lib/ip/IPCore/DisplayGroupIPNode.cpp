...
{
    if (graph()) graph()->removeDisplayGroup(this);
}


void 
DisplayGroupIPNode::setOutputVideoDevice(const VideoDevice* d)
{
    m_outputDevice = d;
}

void 
DisplayGroupIPNode::setPhysicalVideoDevice(const VideoDevice* d)
{
    m_physicalDevice = d;
...
bool 
DisplayGroupIPNode::dualOutputMode() const
{
    if (m_stereoNode)
    {
        string stype = m_stereoNode->stereoType();

        return  stype == "hardware" || 
            stype == "dual" || 
...
void 
DisplayGroupIPNode::initNewContext(const Context& context, Context& newContext) const
{
...
IPImage* 
DisplayGroupIPNode::evaluate(const Context& context)
{
    IPImage* root = 0;
    ...
    if (newContext.stereo)
    {
        ...
        try
        {
            ...
            if (!yuvConvert) left = insertIntermediateRendersForPaint(left, newContext);
            left->useBackground = true;
            
            newContext2.eye = 1;
            right = m_root->evaluate(newContext2);
            if (!yuvConvert) right = insertIntermediateRendersForPaint(right, newContext);
            right->useBackground = true;
            
            if (flip) left->transformMatrix = M * left->transformMatrix;
            if (flip) right->transformMatrix = M * right->transformMatrix;
        }
        catch (...)
        {
        ...
        IPImage* leftEimage = new IPImage(this,
                                          imageDevice(),
                                          IPImage::ExternalRenderType,
                                          IPImage::CurrentFrameBuffer,
                                          IPImage::Rect2DSampler,
                                          false);

        IPImage* rightEimage = new IPImage(this,
                                           imageDevice(),
                                           IPImage::ExternalRenderType,
                                           IPImage::CurrentFrameBuffer,
                                           IPImage::Rect2DSampler,
        ...
        IPImage* leftTarget  = new IPImage(this,
                                           imageDevice(),
                                           IPImage::BlendRenderType,
                                           IPImage::LeftBuffer);

        IPImage* rightTarget = new IPImage(this,
                                           imageDevice(),
                                           IPImage::BlendRenderType,
                                           IPImage::RightBuffer);
        ...
        if (yuvConvert)
        {
            IPImage* leftRgbImage = new IPImage(this,
                                                imageDevice(),
                                                IPImage::BlendRenderType,
            ...
            IPImage* rightRgbImage = new IPImage(this,
                                                 imageDevice(),
                                                 IPImage::BlendRenderType,
                                                 IPImage::IntermediateBuffer,
                                                 IPImage::Rect2DSampler,
    ...
    else
    {
        ...
        try
        {
            ...
            if (!yuvConvert) image = insertIntermediateRendersForPaint(image, newContext);

            if (flip) image->transformMatrix = M * image->transformMatrix;
            image->useBackground = true;
            
            IPImage* eimage = new IPImage(this,
                                          imageDevice(),
                                          IPImage::ExternalRenderType,
                                          IPImage::CurrentFrameBuffer,
                                          IPImage::Rect2DSampler,
            ...
            if (yuvConvert)
            {
                IPImage* rgbImage = new IPImage(this,
                                                imageDevice(),
                                                IPImage::BlendRenderType,
                                                IPImage::IntermediateBuffer,
                                                IPImage::Rect2DSampler,
...
IPImageID* 
DisplayGroupIPNode::evaluateIdentifier(const Context& context)
{
    IPImageID* root = 0;
    Context    newContext = context;
    initNewContext(context, newContext);
...
void
DisplayGroupIPNode::metaEvaluate(const Context& context, MetaEvalVisitor& visitor)
{
    if (!m_root) return;
...
void
DisplayGroupIPNode::mediaInfo(const Context& context, MediaInfoVector& infos) const
{
    Context newContext = context;
    initNewContext(context, newContext);
...
void
DisplayGroupIPNode::setInputs(const IPNodes& newInputs)
{
    //
...
DisplayGroupIPNode::imageDevice() const
{ 
    return m_outputDevice ? m_outputDevice : m_physicalDevice; 
}

void
DisplayGroupIPNode::incrementRenderHashCount()
{

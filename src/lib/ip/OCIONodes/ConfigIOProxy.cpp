...
namespace IPCore
{
    ...
    std::vector<uint8_t> ConfigIOProxy::getLutData(const char* filepath) const
    {
        ...
        {
            std::ifstream fstream(filepath, std::ios_base::in | std::ios_base::binary | std::ios::ate);
            if (fstream.fail())
            {
                std::ostringstream os;
    ...
    std::string ConfigIOProxy::getFastLutFileHash(const char* filename) const { return std::string(filename); }

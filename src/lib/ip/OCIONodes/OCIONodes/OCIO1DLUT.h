...
namespace IPCore
{
    ...
    {
    public:
        OCIO1DLUT(OCIO::GpuShaderDescRcPtr& shaderDesc, unsigned int idx, const std::string& shaderCacheID);
        virtual ~OCIO1DLUT() {};

        // Returns the LUT sampler type: sampler3D, sampler2D, or sampler1D
        // Note: a 2D texture is needed to hold large LUTs.
        std::string samplerType() const override { return m_height == 1 ? "sampler1D" : "sampler2D"; }

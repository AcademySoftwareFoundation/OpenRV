...
WrapExistingNodeInfo::WrapExistingNodeInfo() : CommandInfo("wrapExistingNode") { }
...
void
WrapExistingNode::setArgs(IPNode* node)
{
    m_graph = node->graph();
    m_node  = node;
    m_node->undoRef();
...
void
WrapExistingNode::doit()
{
    // nothing
}

void
WrapExistingNode::undo()
{
    m_graph->isolateNode(m_node);
}

void
WrapExistingNode::redo()
{

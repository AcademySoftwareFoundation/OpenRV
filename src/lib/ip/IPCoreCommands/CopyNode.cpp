...
CopyNodeInfo::CopyNodeInfo() : CommandInfo("copyNode") { }
CopyNodeInfo::~CopyNodeInfo() { }
TwkApp::Command* CopyNodeInfo::newCommand() const { return new CopyNode(this); }

//----------------------------------------------------------------------

CopyNode::CopyNode(const CopyNodeInfo* info) : NewNode(info)
{
}
...
void
CopyNode::setArgs(IPGraph* graph, IPNode* node, const string& name)
{
    m_copyNode = node;
...
CopyNode::doit()
{
    ...
    if (firstTime) 
    {
        if (IPNode* n = node<IPNode>()) n->copyNode(m_copyNode);
    }
}

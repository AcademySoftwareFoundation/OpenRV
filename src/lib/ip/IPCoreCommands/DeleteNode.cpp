...
DeleteNodeInfo::DeleteNodeInfo() : CommandInfo("deleteNode") { }
...
void
DeleteNode::setArgs(IPGraph* graph, const string& name)
{
    m_graph = graph;
    m_name  = name;
    ...
    {
        TWK_THROW_EXC_STREAM("DeleteNode: cannot delete " << m_name 
                             << " because its a group memeber");
    }

    if (m_node) m_node->undoRef();
}

void
DeleteNode::doit()
{
    if (!m_node) setArgs(m_graph, m_name);

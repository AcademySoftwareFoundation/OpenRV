...
template <typename T>
class NewProperty : public TwkApp::Command
{
  public:
    NewProperty(const TwkApp::CommandInfo* info) : TwkApp::Command(info), m_prop(0) {}
    virtual ~NewProperty() { if (m_prop) m_prop->unref(); }
...
template <typename T>
void
NewProperty<T>::setArgs(IPGraph* graph,
...
template <typename T>
class NewPropertyInfo : public TwkApp::CommandInfo
{
  public:
    NewPropertyInfo(const std::string& name): CommandInfo(name) { }

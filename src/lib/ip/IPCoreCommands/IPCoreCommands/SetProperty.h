...
template <class T>
class SetProperty : public TwkApp::Command
{
  public:
    SetProperty(const TwkApp::CommandInfo* i) : TwkApp::Command(i) {}
    virtual ~SetProperty() {}
...
template <typename T>
void 
SetProperty<T>::setArgs(IPGraph* graph,
                        int frame,
                        const std::string& propPath,
                        const ValueType& value)
{
...
template <typename T>
class SetPropertyInfo : public TwkApp::CommandInfo
{
  public:
    SetPropertyInfo(const std::string& name): CommandInfo(name) { }

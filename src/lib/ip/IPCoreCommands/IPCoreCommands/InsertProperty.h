...
template <class T>
class InsertProperty : public TwkApp::Command
{
  public:
    InsertProperty(const TwkApp::CommandInfo* i) : TwkApp::Command(i) {}
...
template <typename T>
void 
InsertProperty<T>::setArgs(IPGraph* graph, 
                           const std::string& nodeName,
                           const std::string& propPath,
                           Container& value,
                           size_t index)
...
{
  public:
    InsertPropertyInfo(const std::string& name): CommandInfo(name) { }

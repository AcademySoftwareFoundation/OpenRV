...
class SetSessionState : public TwkApp::Command
{
  public:
    SetSessionState(const TwkApp::CommandInfo* info)
        : TwkApp::Command(info), m_session(0), m_value(ValueType(0)) {}
...
class SetSessionStateInfo : public TwkApp::CommandInfo
{
  public:
    ...
    SetSessionStateInfo(const std::string& name,
                        SetFunction setF,
                        GetFunction getF,
...
template <typename ValueType>
ValueType
SetSessionState<ValueType>::get() const
{
...
template <typename ValueType>
void 
SetSessionState<ValueType>::set(ValueType v) const
{
    const SetSessionStateInfo<ValueType>* i = static_cast<const SetSessionStateInfo<ValueType>*>(info());
...
template <typename ValueType>
TwkApp::Command*
SetSessionStateInfo<ValueType>::newCommand() const
{
...
class SetSessionObjectState : public TwkApp::Command
{
  public:
    SetSessionObjectState(const TwkApp::CommandInfo* info)
        : TwkApp::Command(info), m_session(0) {}
...
class SetSessionObjectStateInfo : public TwkApp::CommandInfo
{
  public:
    ...
    SetSessionObjectStateInfo(const std::string& name,
                              SetFunction setF,
                              GetFunction getF,
                              TwkApp::CommandInfo::UndoType type)
        : CommandInfo(name, type),
...
template <typename ValueType>
const ValueType&
SetSessionObjectState<ValueType>::get() const
{
    const SetSessionObjectStateInfo<ValueType>* i =
        static_cast<const SetSessionObjectStateInfo<ValueType>*>(info());
    return (m_session->*(i->m_getFunction))();
}

template <typename ValueType>
void 
SetSessionObjectState<ValueType>::set(const ValueType& v) const
{
    const SetSessionObjectStateInfo<ValueType>* i =
        static_cast<const SetSessionObjectStateInfo<ValueType>*>(info());
    (m_session->*(i->m_setFunction))(v);

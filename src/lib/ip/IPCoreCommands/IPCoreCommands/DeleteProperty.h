...
template <typename T>
class DeleteProperty : public TwkApp::Command
{
  public:
    DeleteProperty(const TwkApp::CommandInfo* i) : TwkApp::Command(i), m_prop(0) {}
    virtual ~DeleteProperty() { if (m_prop) m_prop->unref(); }
...
template <typename T>
void
DeleteProperty<T>::setArgs(IPGraph* graph,
...
{
  public:
    DeletePropertyInfo(const std::string& name): CommandInfo(name) { }

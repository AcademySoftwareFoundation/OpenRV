...
{
    if (inV == outV) return Vec3f(0, 0, 0);
...
IPImage*
ColorTemperatureIPNode::evaluate(const Context& context)
{
    ...
    if (active)
    {
        ...
        {
            float blend = 0.5;
            if (inTemperature == outTemperature) return img;
            Vec3f c = temperatureToRGB(outTemperature);
            img->shaderExpr = Shader::newColorBlendWithConstant(img->shaderExpr,
                                                                Vec4f(c.x, c.y, c.z, blend));
        }
        else if (method == DANIELLE)
        {
            ...
            img->shaderExpr = Shader::newColorTemperatureOffset(img->shaderExpr,
                                                                Vec4f(c.x, c.y, c.z, blend));
        }
        else
        {
            ...
            if (inIlluminant == outIlluminant) return img;
            ...
            img->shaderExpr = Shader::newColorMatrix(img->shaderExpr,
                                                     conversionMatrix);
        }
    }

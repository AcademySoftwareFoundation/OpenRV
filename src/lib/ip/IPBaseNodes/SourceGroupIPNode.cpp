...
void
SourceGroupIPNode::setUINameFromMedia(int index)
{
    if (!m_sourceNode) return;
...
void
SourceGroupIPNode::init(const std::string& name,
                        const NodeDefinition* def,
                        SourceIPNode* sourceNode,
                        IPGraph* graph,
                        GroupIPNode* group)
{
    HOP_PROF_DYN_NAME(
        std::string(std::string("SourceGroupIPNode::init : ") + name).c_str());
    ...
    CropIPNode*          crop       = 0;
    if (!sourceNode) sourceNode     = newMemberNodeOfType<SourceIPNode>(sourceType, sourceType == "FileSource" ? "file" : "image");

    if (formatType != "") format = newMemberNode(formatType, "format");
    if (cropType != "") crop = newMemberNodeOfType<CropIPNode>(cropType, "crop");
    if (rotateType != "") rotate = newMemberNodeOfType<RotateCanvasIPNode>(rotateType, "rotate");
    ...
    if (sourceNode) cacheLUT->setInputs1(sourceNode);
    ...
    else overlay->setInputs1(tolinGroup);
...
void
SourceGroupIPNode::setInputs(const IPNodes& newInputs)
{
    //
    ...
    if (newInputs.size())
    {
        cout << "WARNING: SourceGroupIPNode: setInputs called with other than 0 nodes"
             << endl;
    }
...
void
SourceGroupIPNode::readCompleted(const std::string& type, unsigned int version)
{
}

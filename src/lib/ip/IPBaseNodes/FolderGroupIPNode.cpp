...
IPNode*
FolderGroupIPNode::newSubGraphForInput(size_t index, const IPNodes& newInputs)
{
...
void
FolderGroupIPNode::setInputs(const IPNodes& newInputs)
{
    if (isDeleting())
    {
...
void
FolderGroupIPNode::updateViewType()
{
    ...
    for (size_t i = 0; i < m_internalNodes.size(); i++)
    {
        ...
        {
            if (!entry.node) entry.node = newMemberNode(entry.type, entry.value);
            if (rootNode()) entry.node->setInputs(rootNode()->inputs());
            setRoot(entry.node);
        }
    }
}
...
void
FolderGroupIPNode::propertyChanged(const Property* p)
{
    if (!isDeleting())
    {
#if 0

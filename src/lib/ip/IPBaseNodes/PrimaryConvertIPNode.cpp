    ...
    : IPNode(name, def, graph, group)
{
    ...
    m_inChromaticitiesRed = declareProperty<Vec2fProperty>("inChromaticities.red",
                                                           Vec2f(cDefault.red.x, cDefault.red.y));
    m_inChromaticitiesGreen = declareProperty<Vec2fProperty>("inChromaticities.green",
                                                             Vec2f(cDefault.green.x, cDefault.green.y));
    m_inChromaticitiesBlue = declareProperty<Vec2fProperty>("inChromaticities.blue",
                                                            Vec2f(cDefault.blue.x, cDefault.blue.y));
    m_inChromaticitiesWhite = declareProperty<Vec2fProperty>("inChromaticities.white",
                                                             Vec2f(cDefault.white.x, cDefault.white.y));

    // Default out chromaticities
    m_outChromaticitiesRed = declareProperty<Vec2fProperty>("outChromaticities.red",
                                                            Vec2f(cDefault.red.x, cDefault.red.y));
    m_outChromaticitiesGreen = declareProperty<Vec2fProperty>("outChromaticities.green",
                                                              Vec2f(cDefault.green.x, cDefault.green.y));
    m_outChromaticitiesBlue = declareProperty<Vec2fProperty>("outChromaticities.blue",
                                                             Vec2f(cDefault.blue.x, cDefault.blue.y));
    m_outChromaticitiesWhite = declareProperty<Vec2fProperty>("outChromaticities.white",
                                                              Vec2f(cDefault.white.x, cDefault.white.y));

    // Default in and out illuminant white points.
    m_inIlluminantWhite = declareProperty<Vec2fProperty>("illuminantAdaptation.inIlluminantWhite",
                                                 Vec2f(cDefault.white.x, cDefault.white.y));
    m_outIlluminantWhite = declareProperty<Vec2fProperty>("illuminantAdaptation.outIlluminantWhite",
                                                 Vec2f(cDefault.white.x, cDefault.white.y));
...
PrimaryConvertIPNode::evaluate(const Context& context)
{
    ...
    //
    const bool  doConvert = ( (inChromaR != outChromaR) ||
                              (inChromaG != outChromaG) ||
                              (inChromaB != outChromaB) ||
                              (inChromaW != outChromaW) ||
                               adapt);
    ...
    if (doConvert)
    {
        ...
        image->shaderExpr = Shader::newColorMatrix(image->shaderExpr,
                                                   conversionMatrix);

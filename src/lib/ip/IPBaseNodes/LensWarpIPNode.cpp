...
float 
LensWarpIPNode::pixelAspect() const
{
    if (active())
    {
...
bool 
LensWarpIPNode::active() const
{
    return (m_activeProperty->front()?true:false);
}

IPImage*
LensWarpIPNode::evaluate(const Context& context)
{
    ...
    if (image->isNoImage()|| !image->shaderExpr) return image;
    if (!active())
    {
    ...
    if (pa != 0.0f)
    {
        ...
        // Consider changing the pixelAspect to 1.0 for the following images:
        ...
        {
            image->width = image->fb ? image->fb->uncropWidth() * pa
                                    : image->width/image->pixelAspect * pa;
        }
        if (pa < 1.0f || (pa == 1.0f && image->pixelAspect < 1.0))
        {
            image->height = image->fb ? image->fb->uncropHeight() / pa
    ...
    if (model == "brown")
    {
        ...
        image->shaderExpr = Shader::newLensWarp(image, 
                                                image->shaderExpr,
                                                k1, k2, k3, d, p1, p2, 
                                                center + offset,
                                                Vec2f(fx, fy),
    ...
    else if (model == "tde4_ldp_anamorphic_deg_6") // 3DE4 Anamorphic Degree 6
    {
        ...
        image->shaderExpr = Shader::newLensWarp3DE4AnamorphicDegree6(image, 
                                                image->shaderExpr,
                                                Vec2f(cx02, cy02),
                                                Vec2f(cx22, cy22),
                                                Vec2f(cx04, cy04),
...
LensWarpIPNode::imageStructureInfo(const Context& context) const
{
    ...
    if (pa != 0.0f)
    {
        if (pa > 1.0f) i.width = i.width/i.pixelAspect*pa;
        if (pa < 1.0f) i.height = i.height*i.pixelAspect/pa;
        if (pa == 1.0f){
            if(i.pixelAspect > 1.0f) i.width = i.width/i.pixelAspect;
            if(i.pixelAspect < 1.0f) i.height = i.height*i.pixelAspect;
        }
    }
    return i;
}
...
void 
LensWarpIPNode::readCompleted(const std::string& typeName, unsigned int version)
{
...
void 
LensWarpIPNode::propertyChanged(const Property* prop)
{

...
void
HistogramIPNode::computeHistorgram(IPImage* image)
{
    if (FrameBuffer* fb = image->fb)
    {
        ...
        MinMaxPair rmm = computeChannelHistogram(image->fb,
                                                 histogram,
                                                 size->front(),
                                                 false);
        ...
        for (int i = histogram.size() - 1; i >= 0; i--)
        {
            ...
            if (ch.channelName == "R" ||
                ...
                ch.channelName == "B")
            {
                if (ch.min > numeric_limits<float>::min()) min = std::min(min, ch.min);
                if (ch.max < numeric_limits<float>::max()) max = std::max(max, ch.max);
                hasRGB = true;
            }
            else
            {
                if (ch.min > numeric_limits<float>::min()) minAlt = std::min(minAlt, ch.min);
                if (ch.max < numeric_limits<float>::max()) maxAlt = std::max(maxAlt, ch.max);
            }
        }

        if (hasRGB)
        {
            if (std::fabs(min) < numeric_limits<float>::epsilon()) min = 0;
            if (std::fabs(max) < numeric_limits<float>::epsilon()) max = 0;

            image->fb->attribute<Vec2f>("ColorBounds") = Vec2f(min, max);
        }
        else
        {
            if (std::fabs(minAlt) < numeric_limits<float>::epsilon()) minAlt = 0;
            if (std::fabs(maxAlt) < numeric_limits<float>::epsilon()) maxAlt = 0;
...
IPImage*
HistogramIPNode::evaluate(const Context& context)
{
...
IPImageID*
HistogramIPNode::evaluateIdentifier(const Context& context)
{
...
void 
HistogramIPNode::propertyChanged(const Property* p) 
{
    //

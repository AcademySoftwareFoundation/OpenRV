...
namespace IPCore
{
    ...
    AudioAddIPNode::AudioAddIPNode(const std::string& name, const NodeDefinition* def, IPGraph* graph, GroupIPNode* group)
    ...
    size_t AudioAddIPNode::audioFillBuffer(const AudioContext& context)
    {
        ...
        if (nodes.size() > 1)
        {
            ...
            {
                AudioBuffer audioBuffer(context.buffer.size(), context.buffer.channels(), context.buffer.rate(),

...
IPImage*
AudioAddIPNode::evaluate(const Context& context)
{
    //
...
IPImageID*
AudioAddIPNode::evaluateIdentifier(const Context& context)
{
    const IPNodes& nodes = inputs();
...
size_t
AudioAddIPNode::audioFillBuffer(const AudioContext& context)
{
    const IPNodes& nodes = inputs();
    if (nodes.empty()) return 0;
    ...
    if (nodes.size() > 1)
    {
        for (size_t i = 1; i < nodes.size(); i++)
        {
            ...
            const Time contextStart = samplesToTime(context.buffer.startSample(), context.buffer.rate());
            const Time offset       = propertyValue<FloatProperty>(m_audioOffset, 0.0f);

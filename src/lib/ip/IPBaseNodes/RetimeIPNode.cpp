      ...
      m_grainEnvelope(0.006)
{
    ...
    //  * Each index in the array designates the output frame (startFrame + index).
...
bool 
RetimeIPNode::testInputs(const IPNodes& inputs, std::ostringstream& msg) const
{
    if (inputs.size() > 1)
    {
        string un = uiName();
...
void
RetimeIPNode::inputChanged(int index)
{
    //
...
void
RetimeIPNode::inputRangeChanged(int index, PropagateTarget target )
{
    {
...
void
RetimeIPNode::setInputs(const IPNodes& nodes)
{
    if (nodes.size() && nodes != inputs())
    {
        ...
        //
        //  Only reset the fps if we've never set it, or we're replacing existing inputs
        //
...
void
RetimeIPNode::updateExplicitData() const
{
    //
    //  Lock other threads out while we check validity of and possibly update
    //  the data.
    //
    ScopedLock lock(m_nodeMutex);
    ...
    if (!m_explicitDataValid)
    {
        ...
        if (!explicitPropertiesOK()) return;
        ...
        for (int i = 0; i < m_explicitInputFrames->size(); ++i)
        {
            ...
            if (f < minInputFrame) minInputFrame = f;
            if (f > maxInputFrame) maxInputFrame = f;
        }
        ...
        for (int i = 0; i < m_explicitInputFrames->size(); ++i)
        {
            ...
            if (m_explicitInToOut[inIndex] > outF) m_explicitInToOut[inIndex] = outF; 
        }
        ...
        {
            if (m_explicitInToOut[i] != numeric_limits<int>::max()) outFrame = m_explicitInToOut[i];
            else m_explicitInToOut[i] = outFrame;
        }
        ...
        for (int i = 0; i < m_explicitInToOut.size(); ++i)
        {
            cerr << "    " << (minInputFrame+i) << " -> " << m_explicitInToOut[i] << endl;
        }
        */
...
void
RetimeIPNode::updateWarpData() const
{
    //
    //  Lock other threads out while we check validity of and possubply update
    //  the warp data.
    //
    ScopedLock lock(m_nodeMutex);
    ...
    if (!m_warpDataValid)
    {
        ...
        //
        //  The TimeSteps are used to move forward in time, using the condition
        //  that neither the input nor output frame can changed in an amount of
        //  time that is _less_ than both TimeSteps.
        //
        ...
        //  since the corresponding sequence last had a frame added.  When the
        //  Gap > TimeStep - eps, we add a frame.
        //
        float inputGap  = 0.0;
        float outputGap = 0.0;
        ...
        cerr << "start " << m_inputInfo.start << " end " << m_inputInfo.end
                << " in fps " << m_inputInfo.fps << " target fps " << m_fps->front() << endl;
        */
        ...
        while (true)
        {
            ...
            if (warpKeyIndex >= 0)
            {
                ...
                {
                    if (warpKeyIndex != nextKeyIndex) warpKeyIndex = nextKeyIndex - 1;
                    //  cerr << "warpKeyIndex " << warpKeyIndex << " nextKeyIndex " << nextKeyIndex << endl;
                ...
                if (m_warpStyle->front() == 0) 
                {
                    ...
                    //
                    if      (inputFrame < aFrame) scale = aScale;
                    else if (inputFrame >= bFrame) scale = bScale;
                    else
                    {
                ...
                else
                {
                    ...
                    //
                    if      (inputFrame <  bFrame) scale = aScale;
                    else if (inputFrame >= bFrame) scale = bScale;
                }
                //  cerr << "scale " << scale << " aFrame " << aFrame << " aScale " << aScale << " bFrame " << bFrame << " bScale " << bScale << endl;
            }
            ...
            if (inputTimeStep < outputTimeStep) step = inputTimeStep;
            ...
            //
            //  Stop mapping process if we're about to run off the end of the
            //  input sequence.
            //
            if (needInputFrame && (inputFrame + 1) > (m_inputInfo.end - m_inputInfo.start + EPSILON)) break;

            //
            //  Add new frames as appropriate, and reset gap.  the "targetFrame
            //  for each mapping is 1 + the "current" frame in the other mapping if
            //  we are adding a frame to that mapping too, otherwise we "hold"
            //  the last frame we mapped to.
            //
            ...
            /*
            cerr << "step " << step << " (in fps " << 1.0/inputTimeStep << ", out fps " << 1.0/outputTimeStep << ")" << endl;

            cerr << "    input:  f " << inputFrame  << " gap " << oldInputGap  << " -> " << inputGap;
            if (needInputFrame)  cerr << ", frame "  << m_warpedInToOut.size()-1 << " -> " << m_warpedInToOut.back();
            cerr << endl;
            cerr << "    output: f " << outputFrame << " gap " << oldOutputGap << " -> " << outputGap;
            if (needOutputFrame) cerr << ", frame " << m_warpedOutToIn.size()-1 << " -> " << m_warpedOutToIn.back();
            cerr << endl;
            */
        }
...
int 
RetimeIPNode::retimedFrame(int frame) const
{
    if (m_explicitActive->front())
    {
        if (!explicitPropertiesOK()) return 1;

        int index = frame - m_explicitFirstOutputFrame->front();

        if (index <  0)                             index = 0;
        if (index >= m_explicitInputFrames->size()) index = m_explicitInputFrames->size()-1;
    ...
    if (m_warpActive->front()) 
    {
        ...
        if (frame < 1)                      frame = 1;
        if (frame > m_warpedOutToIn.size()) frame = m_warpedOutToIn.size(); 
...
int 
RetimeIPNode::invRetimedFrame0(int frame) const
{
    float scale = m_vscale->front();
    float offset = m_voffset->front();
    float fps = m_fps->front();
...
int
RetimeIPNode::invRetimedFrame(int frame) const
{
    if (m_explicitActive->front())
    {
        updateExplicitData();

        if (m_explicitInToOut.empty()) return m_explicitFirstOutputFrame->front();

        int index = frame - m_explicitFirstInputFrame;

        if (index <  0)                        index = 0;
        if (index >= m_explicitInToOut.size()) index = m_explicitInToOut.size()-1;
    ...
    if (m_warpActive->front()) 
    {
        ...
        if (frame < m_inputInfo.start) frame = m_inputInfo.start;
        if (frame > m_inputInfo.end)   frame = m_inputInfo.end; 
    ...
    if (frame != fi)
    {
        ...
        for (int g = f0; g <= f1; g++)
        {
            if (retimedFrame(g) == frame) return g;
        }
    }
...
void
RetimeIPNode::mapInputToEvalFrames(size_t index, 
...
IPImage*
RetimeIPNode::evaluate(const Context& context)
{
    IPImage* img = 0;
    ...
    //  example.
    //
    if (img && group() && dynamic_cast<const RetimeGroupIPNode*>(group())) img->node = this;
...
void 
RetimeIPNode::metaEvaluate(const Context& context, MetaEvalVisitor& visitor)
{
    Context c = context;
...
void 
RetimeIPNode::testEvaluate(const Context& context, TestEvaluationResult& result)
{
...
void 
RetimeIPNode::flushAllCaches(const FlushContext& context)
{
...
void 
RetimeIPNode::propagateFlushToInputs(const FlushContext& context)
{
    FlushContext newContext = context;
    newContext.frame = retimedFrame(context.frame);
...
void 
RetimeIPNode::propagateFlushToOutputs(const FlushContext& context)
{
    FlushContext newContext = context;
...
IPImageID*
RetimeIPNode::evaluateIdentifier(const Context& context)
{
    Context newContext = context;
    newContext.frame = retimedFrame(context.frame);
...
RetimeIPNode::imageRangeInfo() const
{
    ...
    if (m_explicitActive->front())
    {
        ...
        //  cerr << "rinfo s " << info.start << " i " << info.cutIn << " o " << info.cutOut << " e " << info.end << endl;
    ...
    if (info.cutIn  == info.start) iin  = istart;
    if (info.cutOut == info.end)   iout = iend;
    ...
    */
    if (fabs(m_vscale->front()) > 1.0 || info.fps != m_fps->front())
    {
        ...
        {
            if (m_warpActive->front() && iend+inc > m_warpedOutToIn.size()) break;
            iend += inc;
        }
        while (retimedFrame(iout+inc) == info.cutOut)
        {
            if (m_warpActive->front() && iout+inc > m_warpedOutToIn.size()) break;
            iout += inc;
        }
    }
...
void
RetimeIPNode::propertyChanged(const Property* p)
{
    propagateRangeChange();
...
size_t 
RetimeIPNode::audioFillBuffer(const AudioContext& context)
{
    ...
    for (size_t i = sync0; i <= syncN; i++)
    {
        ...
        if (IPNode::audioFillBuffer(rcontext) == 0) break;
...
bool 
RetimeIPNode::explicitPropertiesOK() const
{

...
string
LayoutGroupIPNode::retimeType()
{
    return definition()->stringValue("defaults.retimeType", "Retime");
}

string
LayoutGroupIPNode::transformType()
{
    return definition()->stringValue("defaults.transformType", "Transform2D");
}

string
LayoutGroupIPNode::stackType()
{
    return definition()->stringValue("defaults.stackType", "Stack");
}

string
LayoutGroupIPNode::paintType()
{
...
IPNode*
LayoutGroupIPNode::newSubGraphForInput(size_t index, const IPNodes& newInputs)
{
    layoutIfRequested();
...
IPNode*
LayoutGroupIPNode::modifySubGraphForInput(size_t index,
...
class LayoutItem
{
  public:
    ...
    void set(float x, float y, float s, float r = 0.0f)
        { xtrans = x; ytrans = y; scale = s; rotation = r; }
...
void
geometryAtNode(int frame, IPNode* node, float& width, float& height)
{
...
void
convertToLayoutItems(int frame,
...
void
setTransform(IPNode* node, float x, float y, float s, float r)
{
...
void
setTransforms(const IPNode::MetaEvalInfoVector& infos, const LayoutItems& items)
{
    for (size_t i = 0; i < infos.size(); i++)
    {
...
void
identityAllItems(LayoutItemPointers& items)
{
...
void
scaleAllItems(LayoutItemPointers& items, float s)
{
    for (size_t i = 0; i < items.size(); i++)
    {
        LayoutItem* item = items[i];
...
float
maxItemWidth(const LayoutItemPointers& items)
{
    float maxW = 0;
    for (size_t i = 0; i < items.size(); i++)
    {
...
void
layoutItemsInGrid(LayoutItemPointers& items, float aspect, int cols, int rows)
{
    if (items.size() == 0) return;

    //
    // Make sure the number of rows and columns is positive and no larger than
    // the number of items.
    //

    if (cols > items.size()) cols = items.size();
    if (rows > items.size()) rows = items.size();
    if (cols < 0) cols = 0;
    if (rows < 0) rows = 0;
    ...
    //  If neither the columns nor the rows are defined then we are going to
    //  automatically figure out the grid layout. The number of columns and rows
    //  we choose is the number that provides the largest scale. In other words
    //  we try all possible col x row layouts and choose the one where the
    //  items are as large as possible. The automatic layout will be a column
    //  fixed layout.
    //
    // case #2; cols is zero but rows is defined
    //  If we have a fixed set of rows then we calculate the number of columns
    //  required to display all of the items.
    //
    // case #3; cols is defined (rows is ignored/calculated)
    //  If we have a fixed set of columns then we calculate the number of rows
    //  required to display all of the items.
    //
    ...
    else
    {
        ...
        else
        {
            ...
            for (int colsC = 1; colsC < items.size(); colsC++)
            {
                ...
                float scaleC = min(
                    aspect / (float(colsC) * maxW), 1.0f / float(rowsC));
                if (scaleC > bestScale)
                {
                    bestScale = scaleC;
                    cols = colsC;
    ...
    //
    // The scale is the minimum of fitting the grid by width or by height.
    //

    float scale = min(aspect / (float(cols) * maxW), 1.0f / float(rows));

    //
    // If the layout is fixed by row count then we need to make sure to fill
    // rows before columns. If the layout has a fixed number of columns then
    // we need to do the opposite and make sure we hit each column before we
    // worry about the rows.
    //
...
float
rowWidthOfItems(const LayoutItemPointers& items)
{
    float xtotal = 0;
...
float
layoutRowOfItems(LayoutItemPointers& items, float y, float s, float off)
{
    float xaccum = 0;
...
bool
isAbleToLayoutRows(LayoutItemPointers& items,
                   const float aspect,
                   const size_t rowNo,
                   size_t& actualNo)
{
...
void
layoutItemsPacked(LayoutItemPointers& items, float aspect)
{
    //
    // this function figures out how to layout items in a way that satifies the following conditions:
    // i. the layout will maintain the original order of the items
    // ii. there is no empty space (gap) between consecutive items in any row,
    //     and there is no empty space between consecutive rows.
    // iii. all the items have to fit in the 'target' rendering size
    //
    // given the above constraints, this algorithm tries to find a 'optimal' layout
    // that will minimize empty space and balance out empty space among rows
    //
    ...
    // n is the minimum number of rows we will need. but n might not work out
    //
    ...
    //
    // rearrange to balance out empty space. try to evenly distribute the empty space
    // only allowed to move the last to next row
    // don't need to think about moving first to last row, because if it was possible,
    ...
    while(madechanges)
    {
        ...
        {
            // consider moving the last element if it helps balance out empty space
            if (row != actualRows - 1)
            {
                ...
                if (w + rowaspects[row+1] <= rowa)
                {
                    ...
                    const float es2 = max(rowa - rowaspects[row+1] - w,
    ...
    if (actualRows != rows)
    {
        ...
        for (size_t row = 0; row < actualRows; row++)
        {
            if (rowaspects[row] > lra) lra = rowaspects[row];
        }
        if (lra < rowa)
        {
            factor = rowa / lra;
    ...
    for (size_t row = 0; row < actualRows; row++)
    {
        ...
        layoutRowOfItems(tempRow,
                         0.5 * sy * actualRows - 0.5 * sy - row * sy,
                         sy,
...
void
layoutItemsPacked2(LayoutItemPointers& items, float aspect, int fixedRows = 0)
{
...
LayoutGroupIPNode::layout()
{
    ...
    if      (m == "packed")  layoutItemsPacked(itemPointers, aspect);
    else if (m == "packed2") layoutItemsPacked2(itemPointers, aspect);
    else if (m == "row")     layoutItemsInGrid(itemPointers, aspect, 0, 1);
    else if (m == "column")  layoutItemsInGrid(itemPointers, aspect, 1, 0);
    else if (m == "grid")    layoutItemsInGrid(itemPointers, aspect, cols, rows);
    else
    {
        setTrans = false;
        doSpacing = false;
    ...
    if (doSpacing)
    {
        ...
        if (spacing < 1.0 && spacing > 0.0) scaleAllItems(itemPointers, spacing);
    }

    if (setTrans) setTransforms(infos, items);
}

void
LayoutGroupIPNode::propertyChanged(const Property* p)
{
    if (!isDeleting())
    {
        ...
        if (p == m_retimeToOutput || p == m_stackNode->outputFPSProperty())
        {
            ...
            if (p == m_stackNode->outputFPSProperty()) return;
        }
    }
...
void
LayoutGroupIPNode::inputImageStructureChanged(int index, PropagateTarget target)
{
...
void
LayoutGroupIPNode::setInputs(const IPNodes& newInputs)
{
    HOP_PROF_FUNC();
    if (isDeleting())
    {
...
void
LayoutGroupIPNode::rebuild(int inputIndex)
{
    if (!isDeleting())
    {
...
void
LayoutGroupIPNode::readCompleted(const string& t, unsigned int v)
{
    requestLayout();
...
void
LayoutGroupIPNode::inputMediaChanged(
    IPNode * srcNode,
    int srcOutIndex,

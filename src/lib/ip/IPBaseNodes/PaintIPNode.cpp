...
PaintIPNode::PaintIPNode(const std::string& name,
...
void
PaintIPNode::compilePenComponent(Component* c)
{
...
void
PaintIPNode::compileTextComponent(Component* c)
{
...
void
PaintIPNode::compileFrame(Component* comp)
{
...
void
PaintIPNode::propertyChanged(const Property* p)
{
    if (const Component* c = componentOf(p))
    {
        if (c->name().size() > 4)
        {
            ...
            if (s == "pen:") compilePenComponent((Component*)c);
        }
        ...
        if (c->name().size() > 6)
        {
            ...
            if (s == "frame:") compileFrame((Component*)c);
        }
    }
...
static void 
fixV1Coordinates(Vec2fProperty* p, const IPNode::ImageStructureInfo& info)
{
    const float aspect = float(info.width) / float(info.height) / 2.0;
    const Mat44f& O = info.orientation;
    Mat33f M( O(0,0), O(0,1), O(0,2),
              O(1,0), O(1,1), O(1,2),
              O(2,0), O(2,1), O(2,2) );
...
void 
PaintIPNode::readCompleted(const string& typeName, unsigned int version)
{
    ...
    if (version == 1)
    {
        ...
        ImageStructureInfo info = 
            imageStructureInfo(graph()->contextForFrame(graph()->cache().inFrame()));
    ...
    for (size_t i = 0; i < comps.size(); i++)
    {
        ...
        {
            string s = c->name().substr(0, 4);
            if (s == "pen:") compilePenComponent(c);
        }

        if (c->name().size() > 5)
        {
            string s = c->name().substr(0, 5);
            if (s == "text:") compileTextComponent(c);
        }

        if (c->name().size() > 6)
        {
            string s = c->name().substr(0, 6);
            if (s == "frame:") compileFrame(c);
        }
    }

...
string
SequenceGroupIPNode::sequenceType()
{
    return definition()->stringValue("defaults.sequenceType", "Sequence");
}

string
SequenceGroupIPNode::retimeType()
{
    return definition()->stringValue("defaults.retimeType", "Retime");
}

string
SequenceGroupIPNode::paintType()
{
    return definition()->stringValue("defaults.paintType", "Paint");
}

string
SequenceGroupIPNode::audioAddType()
{
    return definition()->stringValue("defaults.audioAddType", "AudioAdd");
}

string
SequenceGroupIPNode::audioSourceType()
{
    return definition()->stringValue("defaults.audioSourceType", "FileSource");
}

IPNode*
SequenceGroupIPNode::newSubGraphForInput(size_t index, const IPNodes& newInputs)
{
...
IPNode*
SequenceGroupIPNode::modifySubGraphForInput(size_t index,
...
void
SequenceGroupIPNode::setInputs(const IPNodes& newInputs)
{
    HOP_PROF_FUNC();
...
void
SequenceGroupIPNode::rebuild(int inputIndex)
{
    if (!isDeleting())
    {
...
void
SequenceGroupIPNode::configureSoundTrack()
{
...
void
SequenceGroupIPNode::propertyChanged(const Property* p)
{
    if (!isDeleting())
    {
        if (p == m_retimeToOutput || p == m_sequenceNode->outputFPSProperty())
        {
            ...
            if (p == m_sequenceNode->outputFPSProperty()) return;
        }
...
SequenceGroupIPNode::EDLInfo
SequenceGroupIPNode::edlInfo() const
{
...
void SequenceGroupIPNode::inputMediaChanged(
    IPNode * srcNode,
    int srcOutIndex,

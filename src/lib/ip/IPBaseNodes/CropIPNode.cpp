...
CropIPNode::CropIPNode(const std::string& name,
...
IPImage*
CropIPNode::evaluate(const Context& context)
{
    ...
    if (image->isNoImage() || propertyValue<IntProperty>(m_active, 1) == 0) return image;
    ...
    CropSize cropSize = CropSize(image->width - left - right,
    ...
    if ( ((cropSize.width < 0 ||
           cropSize.height < 0 ||
           (cropSize.width == image->width && cropSize.height == image->height))) &&
         !manip)
    {
        return image;
    }
    ...
    IPImage* image2 = new IPImage(this,
...
void
CropIPNode::propertyChanged(const Property* p)
{

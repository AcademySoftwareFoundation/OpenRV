                                             ...
                                             GroupIPNode* group): IPNode(name, def, graph, group)
{
    ...
    m_colorspace    = declareProperty<StringProperty>("node.colorspace", 
...
void
ColorCDLIPNode::copyNode(const IPNode* node)
{
    IPNode::copy(node);
...
void
ColorCDLIPNode::propertyChanged(const Property* property)
{
    IPNode::propertyChanged(property);
    if (property == m_file) updateProperties();
}

void
ColorCDLIPNode::readCompleted(const std::string& type, unsigned int version)
{
...
void
ColorCDLIPNode::updateProperties()
{
    LockGuard lock(m_updateMutex);

    if (m_file->size() <= 0 || m_file->front() == "") return;
...
IPImage*
ColorCDLIPNode::evaluate(const Context& context)
{
    ...
    if ( offset != Vec3f(0.0f) ||
         slope != Vec3f(1.0f) ||
         power != Vec3f(1.0f) ||
         saturation != 1.0f )
    {
        bool noClamp = false;
        if (IntProperty* ip = m_noclamp) noClamp = ip->front();

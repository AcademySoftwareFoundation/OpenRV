...
ColorIPNode::ColorIPNode(const std::string& name,
...
void
ColorIPNode::setExposure(float e)
{
...
void
ColorIPNode::readCompleted(const std::string& type, unsigned int version)
{
    IPNode::readCompleted(type, version);

    if (const IntProperty* ip = m_lumlutActive)
    {
        if (!ip->empty() && ip->front()) generateLUT();
    }
}

void
ColorIPNode::propertyChanged(const Property* property)
{
    IPNode::propertyChanged(property);

    if (property == m_lumlutLUT || property == m_lumlutSize) generateLUT();
}

void
ColorIPNode::generateLUT()
{
    delete m_lumLUTfb;
    m_lumLUTfb = 0;
...
IPImage*
ColorIPNode::evaluate(const Context& context)
{
...
void
ColorIPNode::evaluateOne(IPImage* img, const Context& context)
{
    Vec2f cred;
    Vec2f cgreen;
    Vec2f cblue;
    ...
    bool useCDL = CDLactive && (
        CDL_offset != Vec3f(0.0f) ||
    ...
    {
        bool noClamp = false;
        if (IntProperty* ip = m_CDLnoclamp) noClamp = ip->front();
        ...
        {
            img->shaderExpr = Shader::newColorCDLForACES(img->shaderExpr,
                                                        CDL_slope,
                                                        CDL_offset,
                                                        CDL_power,
                                                        CDL_saturation,

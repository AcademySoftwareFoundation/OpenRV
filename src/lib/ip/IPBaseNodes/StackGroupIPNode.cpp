...
IPNode*
StackGroupIPNode::modifySubGraphForInput(size_t index, const IPNodes& newInputs, IPNode* subgraph)
{
    bool retimeInputs = m_retimeToOutput->front() ? true : false;
...
void
StackGroupIPNode::rebuild(int inputIndex)
{
    if (!isDeleting())
    {
...
void
StackGroupIPNode::setInputs(const IPNodes& newInputs)
{
    HOP_PROF_FUNC();
    if (isDeleting())
    {
...
void
StackGroupIPNode::propertyChanged(const Property* p)
{
    if (!isDeleting())
    {
...
void
StackGroupIPNode::inputMediaChanged(
    IPNode * srcNode,
    int srcOutIndex,

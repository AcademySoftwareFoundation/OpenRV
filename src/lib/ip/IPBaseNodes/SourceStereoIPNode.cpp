...
void
applyStereoContext(bool isLeft, IPNode::StereoContext& sc, IPImage* img)
{
    if (!sc.active) return;
...
void
SourceStereoIPNode::contextFromStereoContext(Context& c)
{
    //
    //  If swapping, we change nothing but the eye.  IE offsets, etc all stay
    //  the same, we just ask for different images in the eyes.
    //

    if (c.stereoContext.active && c.stereoContext.swap)
    {
        if      (c.eye == 0) c.eye = 1;
        else if (c.eye == 1) c.eye = 0;
    }
}

IPImage*
SourceStereoIPNode::evaluate(const Context& context)
{

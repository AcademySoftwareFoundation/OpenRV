...
Mat44f 
orientationMatrix(FrameBuffer::Orientation o)
{
    Mat44f T;
...
FrameBuffer*
allocateResizedFBs(FrameBuffer* in, float sx, float sy, FrameBuffer::DataType outType)
{
    ...
    {
        FrameBuffer* fb = new FrameBuffer(ow, oh, in->numChannels(), 
                                          outType, 0, &in->channelNames(),
                                          in->orientation());
        
        if (in->nextPlane()) fb->appendPlane(allocateResizedFBs(in->nextPlane(), sx, sy, outType));
        return fb;
    }
    else
    {
...
FrameBuffer* 
resizeFB(FrameBuffer* inFB, float scale)
{
...
void
FileSourceIPNode::addMedia(const SharedMediaPointer& sharedMedia, const SharedMediaPointer & proxySharedMedia)
{
    HOP_PROF_FUNC();
    ...
    if (m_workItemID)
    {
        ...
        addDispatchJob( Application::instance()->dispatchToMainThread(
                                    [this, sharedMedia, proxySharedMedia, workItemID]
                                    (Application::DispatchID dispatchID)
        {
            { 
            ...
            // The order of propagation is important. The AudioStatus event must be send the first one.
            // After we must send MediaChange event to notify a FPS change before send ImageStructureChange and rangeChange.
            // This new fps is used by layout trigges by the RangeChange event
...
void 
FileSourceIPNode::testEvaluate(const Context& context, TestEvaluationResult& result)
{
    bool slow = false;
...
FileSourceIPNode::getMediaFromContext(ImageComponent& selection, const Context& context)
{
...
FileSourceIPNode::MediaPointer
FileSourceIPNode::mediaForComponent(ImageComponent& c, const Context& context) 
{
    ...
    if (context.stereo && context.eye < m_mediaVector.size())
    {
        ...
        if (media->hasVideo())
        {
            switch (c.type)
            {
              default:
                  break;
              case ChannelComponent: if (!media->matchesChannel(c)) break;
              case LayerComponent:   if (!media->matchesLayer(c)) break;
              case ViewComponent:    if (!media->matchesView(c)) break;
                  return media;
            }
        }
    }
    ...
    for (size_t i = 0; i < m_mediaVector.size(); i++)
    {
        ...
        if (media->hasVideo())
        {
            ...
            if (c.type == LayerComponent)
            {
                if (c.name[0].empty())
                {
                    // This handles the case where the view has been omitted in the session mgr
                    // and therefore is "". This happens when there is just one view for all parts/layers.
                    // So we need to determine what the defaultView is
                    // to use as the view string when attempting to match the media.
                    // Note that if we are this point in the code, all prior media match
                    // attempts have failed.
                    // 
                    if (Movie* mov = movieForThread(media.get(), context))
                    {
                        ImageComponent newSelection = 
                            ImageComponent(LayerComponent, mov->info().defaultView, c.name[1]);
            ...
            if (c.type == ChannelComponent)
            {
                ...
                {
                    ImageComponent newSelection = 
                        ImageComponent(ChannelComponent, mov->info().defaultView, c.name[1], c.name[2]);
...
FileSourceIPNode::MediaPointer
FileSourceIPNode::defaultMedia(int eye) 
{
    const QReadLocker readLock(&m_mediaMutex);
...
void
FileSourceIPNode::setupRequest(const Movie* mov,
                               ...
                               Movie::ReadRequest& request) 
{
    ...
    {
        if (s > 0 && t >= ViewComponent) request.views.push_back(selection.name[0]);
        if (s > 1 && t >= LayerComponent) request.layers.push_back(selection.name[1]);
        if (s > 2 && t == ChannelComponent) request.channels.push_back(selection.name[2]);
    }
...
debuggingDelay()
{
    ...
    if (sleepMS == -1)
    {
        if (getenv("TWK_SOURCE_SLEEP_MS")) sleepMS = atoi(getenv("TWK_SOURCE_SLEEP_MS"));
        else                               sleepMS = 0;
    }
    if (sleepMS > 0)
    {
...
debuggingLoadDelay()
{
    ...
    if (sleepMS == -1)
    {
        if (getenv("RV_SOURCE_LOAD_DELAY_MS")) sleepMS = atoi(getenv("RV_SOURCE_LOAD_DELAY_MS"));
        else                                     sleepMS = 0;
    }
    if (sleepMS > 0)
    {
...
void FileSourceIPNode::configureAlphaAttrs( FrameBuffer* fb, IPImage* img )
{
    ...
    if (!hasAlphaAttr && 
        (fb->numChannels() == 2 || fb->numChannels() == 4))
    {
...
IPImage*
FileSourceIPNode::evaluate(const Context& context)
{
    ...
    MediaPointer   media;
    {
        ...
        if (m_mediaVector.size() == 0) return IPImage::newNoImage(this, "No Media");
    ...
    if (failed || empty)
    {
        ...
        catch (...)
        {
            ...
            catch (...)
            {
                ...
                cerr << "ERROR: FileSource: identifiersAtFrame error: unable to get any identifiers from movie." << endl;
            }
        }
        if (ids.size() == 0)
        {
            if (media->hasVideo())
            {
                //  This should never happen, but if it does still make sure that fbs array is not empty.
                cerr << "ERROR: FileSource eval: unable to get any identifiers from movie." << endl;
            }
        ...
        {
            FrameBuffer* fb = IPImage::newNoImageFrameBufferWithAttrs(this,
                                                                      mov->info().width, 
                                                                      mov->info().height,
                                                                      "No Source Image");
            fbs.push_back(fb);
            ...
            for ( int x = 0 ; x < tilingInfo.numTiles ; x++ )
            {
                ...
                while ( aTilePlaneFB && aFullPlaneFB )
                {
                    ...
                    aTilePlaneFB->restructure(
                        ...
                        aFullPlaneFB->scanlinePixelPadding(),
                        NULL,
...
IPImageID*
FileSourceIPNode::evaluateIdentifier(const Context& context)
{
    const bool     profile   = (context.thread & DisplayThread) && graph()->needsProfilingSamples();
    ImageComponent selection;
    MediaPointer   media;
    ...
    //
    //  Not an error for an audio-only source to have no IDs.  XXX should come back to this.
    //
    if (! media->hasVideo()) m.push_back("no-source-image");

    if (m.empty()) TWK_THROW_EXC_STREAM("FileSource: movie identifiers eval failed.");
...
FileSourceIPNode::imageRangeInfo() const
{
    const QReadLocker readLock(&m_mediaMutex);
...
FileSourceIPNode::imageStructureInfo(const Context& context) const
{
    ...
    for (size_t i = 0; i < m_mediaVector.size(); i++)
    {
        ...
        bool   audio = mov->hasAudio() && (!mov->hasVideo() || !m_noMovieAudio->front());
...
void
FileSourceIPNode::audioConfigure(const AudioConfiguration& config)
{
    ...
    for (size_t i = 0; i < m_mediaVector.size(); i++)
    {
        ...
        if (!mov->hasAudio() || (mov->hasVideo() && m_noMovieAudio->front())) continue;
        ...
        if (!mov->canConvertAudioRate())
        {
            ...
            size_t overflow = (timeToSamples(m_grainDuration + m_grainEnvelope * 2.0,
                                             m_adevRate) * grainSlop) / m_adevSamples;
            ...
            //
            // To prevent the resampler from returning zero samples at any particular
            // process pass, the blocksize has to be increased at large down sample rates.
            // Otherwise the blocksize is too small for the resampler to complete its process at
            // a single pass and may return zero samples processed to give it time to build
            // up more samples internally.
            // 
...
size_t
FileSourceIPNode::audioFillBuffer(const AudioContext& context)
{
    ...
    //
    //  So in some cases, this default m_adevRate/m_adevLayout might not equal the context
    //  buffer rate/layout. As such we call audioConfigure explicitly to ensure the
    //  audio config rate and layout matches the context buffer rate and layout always...
    //  (which is the audio config of the audio renderer in use)
    //
...
FileSourceIPNode::audioFillBufferInternal(const AudioContext& context)
{
    ...
    for (vector<size_t>::iterator am = audioMedia.begin(); am != audioMedia.end(); am++)
    {
        ...
        AudioBuffer mediaBuffer(context.buffer.size(),
...
TwkAudio::Time
FileSourceIPNode::offsetStartTime(const AudioContext context)
{
...
void
FileSourceIPNode::flushAllCaches(const FlushContext& context)
{
    const QReadLocker readLock(&m_mediaMutex);
    for (size_t i = 0; i < m_mediaVector.size(); i++)
    {
...
void
FileSourceIPNode::invalidateFileSystemInfo()
{
    const QReadLocker readLock(&m_mediaMutex);
    for (size_t i = 0; i < m_mediaVector.size(); i++)
    {
        const SharedMoviePointerVector& movies = m_mediaVector[i]->shared->movies;
        for (size_t q=0; q < movies.size(); q++) movies[q]->invalidateFileSystemInfo();
    }
}

void
FileSourceIPNode::prepareForWrite()
{
    Vec2i size(0,0);
    {
...
void 
FileSourceIPNode::writeCompleted()
{
    //
...
void 
FileSourceIPNode::readCompleted(const std::string& typeName, unsigned int version)
{
    {
        HOP_PROF("FileSourceIPNode::readCompleted - SourceIPNode::readCompleted");
...
std::string
FileSourceIPNode::filename() const
{
...
FileSourceIPNode::Movie* 
FileSourceIPNode::movieByMediaIndex(size_t index)
{
    const QReadLocker readLock(&m_mediaMutex);
...
FileSourceIPNode::fps() const
{
    ...
    for (size_t i = 0; i < m_mediaVector.size(); i++)
    {
        ...
        if (mov->info().fps > 0.5)
        {
            if (mov->hasVideo()) nfps = mov->info().fps;
            else                 afps = mov->info().fps;
        }
    }
...
bool
FileSourceIPNode::isBackwards() const
{
    //
...
void 
FileSourceIPNode::newPropertyCreated(const Property* p)
{
...
void 
FileSourceIPNode::propertyDeleted(const std::string& propName)
{
...
void 
FileSourceIPNode::updateHasAudioStatus()
{
    const QWriteLocker writeLock(&m_mediaMutex);
...
void 
FileSourceIPNode::propertyChanged(const Property* p)
{
    //
...
void 
FileSourceIPNode::propertyWillChange(const Property* p)
{
    IPNode::propertyWillChange(p);
}

size_t
FileSourceIPNode::numMedia() const 
{
    const QReadLocker readLock(&m_mediaMutex);
...
size_t
FileSourceIPNode::mediaIndex(const std::string& name) const
{
    const QReadLocker readLock(&m_mediaMutex);
    for (size_t i = 0; i < m_mediaVector.size(); i++)
    {
...
const string&
FileSourceIPNode::mediaName(size_t index) const
{
...
void 
FileSourceIPNode::setReadAllChannels(bool v)
{
...
void
FileSourceIPNode::reloadMediaFromFiles()
{
    HOP_PROF_FUNC();
    ...
    if (hasProgressiveSourceLoading())
    {
        if (!media.empty())
        {
            ...
            m_workItemID = graph()->addWorkItem([this, sharedMedias, media](){
                                ...
                                const SharedMediaPointer & proxySharedMedia)
{
    ...
    else if (TwkMediaLibrary::isLibraryMediaURL(file))
    {
        ...
        TwkMediaLibrary::NodeVector nodes = TwkMediaLibrary::libraryNodesAssociatedWithURL(file);
        for (int n = 0; n < nodes.size(); n++)
        {
            const TwkMediaLibrary::Node* node = nodes[n];
            if (const TwkMediaLibrary::MediaAPI* api = TwkMediaLibrary::api_cast<TwkMediaLibrary::MediaAPI>(node))
            {
...
bool
FileSourceIPNode::findCachedMediaMetaData(const string& filename, PropertyContainer* pc)
{
    //
...
FileSourceIPNode::Movie* 
FileSourceIPNode::openProxyMovie(const string& errorString, double minBeforeTime, const string filename, double defaultFPS)
{
    ...
    if (!errorString.empty())
    {
        proxy << ",errorString=" << TwkUtil::id64Encode(errorString)
              << ",minBeforeTime=" << minBeforeTime; // wait 2 seconds before to display this message
    }  
    proxy << ".movieproc";
...
void 
FileSourceIPNode::addDispatchJob(IPGraph::WorkItemID id)
{
...
void 
FileSourceIPNode::removeDispatchJob(IPGraph::WorkItemID id)
{
    LockGuard lock(m_dispatchIDListMutex);
...
void 
FileSourceIPNode::cancelAllDispatchJobs()
{
    LockGuard lock(m_dispatchIDListMutex);
...
void 
FileSourceIPNode::undispatchAllDispatchJobs()
{
    LockGuard lock(m_dispatchIDListMutex);

...
{
    m_sigma  = declareProperty<FloatProperty>("node.sigma", 1.0 / 33.0); // roughly r^2 / 3
...
IPImage*
FilterGaussianIPNode::evaluate(const Context& context)
{
    ...
    if (radius <= 10) gaussHorizontal = Shader::applyGaussianFilter(this, image, (size_t)radius, sigma);
    else gaussHorizontal = Shader::applyFastGaussianFilter(this, image, (size_t)radius);

...
{
    PropertyInfo* einfo = new PropertyInfo(PropertyInfo::Persistent |
                                           PropertyInfo::ExcludeFromProfile |
                                           PropertyInfo::RequiresGraphEdit);

    PropertyInfo* minfo = new PropertyInfo(PropertyInfo::Persistent |
                                           PropertyInfo::ExcludeFromProfile);
...
void
ImageSourceIPNode::set(const string& mediaName, const MovieInfo& mediaInfo)
{
    ...
    {
        std::copy(m_mediaInfo.layers.begin(),
                  m_mediaInfo.layers.end(),
                  m_layers->StringProperty::valueContainer().begin());
    }

    if (m_mediaInfo.views.size() > 0)
    {
        std::copy(m_mediaInfo.views.begin(),
                  m_mediaInfo.views.end(),
...
void
ImageSourceIPNode::readCompleted(const std::string& typeName, unsigned int version)
{
...
void
ImageSourceIPNode::writeCompleted()
{
    updateInfoFromProps();
    updateViewLayerInfo();
...
void
ImageSourceIPNode::updateInfoFromProps()
{
    ...
    if (b == 8 && !f) m_mediaInfo.dataType = FrameBuffer::UCHAR;
    else if (b == 16 && !f) m_mediaInfo.dataType = FrameBuffer::USHORT;
    else if (b == 32 && f) m_mediaInfo.dataType = FrameBuffer::FLOAT;
    else if (b == 16 && f) m_mediaInfo.dataType = FrameBuffer::HALF;
    else m_mediaInfo.dataType = FrameBuffer::UCHAR;
...
void
ImageSourceIPNode::updateViewLayerInfo()
{
    //
...
void
ImageSourceIPNode::propertyChanged(const Property* p)
{
    m_propMap[p]++;
...
ImageSourceIPNode::Property*
ImageSourceIPNode::findCreatePixels(int frame, 
                                    const string& view,
                                    const string& layer)
{
    ...
    if (Property* p = find("image", name)) return p;
    return makePixels("image", name);
}

IPNode::ImageRangeInfo 
ImageSourceIPNode::imageRangeInfo() const
{
    int in  = m_cutIn->front();
    int out = m_cutOut->front();
    return ImageRangeInfo(m_start->front(), 
                          m_end->front(), 
                          m_inc->front(),
                          m_fps->front(),
...
void
ImageSourceIPNode::pixelPropNamesViewLayer(const StringVector& suffixes1,
                                           ...
                                           bool existingOnly)
{
    ...
    if (frame < fs) frame = fs;
    if (frame > fe) frame = fe;
    ...
    //
    //  It's likely that layer names will contain ".", but this is illegal char
    //  for property names, so swap in "*".
    //
    ...
    for (size_t i = 0; i < suffixes1.size(); i++)
    {
        ...
        for (size_t q = 0; q < cleanSuffixes2.size(); q++)
        {
            if (!existingOnly)
            {
                ostringstream str;
                str << frame 
                    << separator1 << suffixes1[i]
                    << separator2 << cleanSuffixes2[q];
...
void
ImageSourceIPNode::pixelPropNames(const StringVector& views, 
                                  ...
                                  bool existingOnly)
{
    ...
    {
        pixelPropNamesViewLayer(views, layers,
                                ":", 
                                ";", 
                                frame, propNames, existingOnly);
    }
...
IPImage* 
ImageSourceIPNode::evaluate(const Context& context)
{
    IPImage* head = 0;
    LayerTree propTree;
    StringVector cviews;
    ...
    if (context.stereo)
    {
        ...
        if (newSelection.isValid()) selection = newSelection;
    }
    ...
    switch (selection.type)
    {
      ...
      case LayerComponent: 
      {
          ...
          {
              if (selection.name[0] != "") cviews.push_back(selection.name[0]); 
              clayers.push_back(selection.name[1]);
          }
          else clayers.push_back(selection.name[0]);
          break;
      }
      case NoComponent:
    ...
    if (!propTree.empty())
    {
        ...
        { 
            ...
            for (int v = 0; v < m_eyeViews->size(); v++)
            {
                ...
                if (name.size() - index == viewName.size()) { eye = v + 1; break; }
            }
        }
        ...
        for (size_t i = 0; i < propNames.size(); i++)
        {
            ...
            if (Property* pixels = find("image", name))
            {
                ...
                {
                  case Property::FloatLayout: d = FrameBuffer::FLOAT; break;
                  case Property::ShortLayout: d = FrameBuffer::USHORT; break;
                  case Property::HalfLayout:  d = FrameBuffer::HALF; break;
                  case Property::ByteLayout:  d = FrameBuffer::UCHAR; break;
                  default:
                      break;
                }
                ...
                for (size_t c = 0; c < numChannels; c++) chans[c] = m_channels->front().at(c);

                FrameBuffer* fb = new FrameBuffer(m_width->front(), 
                                                  m_height->front(), 
                                                  numChannels,
                                                  d, 
                                                  (unsigned char*)pixels->rawData(),
                                                  &chans);
                ...
                fb->idstream() << IPNode::name() 
                               << "/" << m_mediaName->front() 
                               << "/" << pixels->name();

                //
                //  The ID at this point (without the unique integer from
                //  propMap) is a handy substring we can use to flush any FBs
                //  previously generated by this Source.  We should flush them
                //  because we know they are out of date (we just generated a
                //  replacement FB, and otherwise they may not get deleted
                //  unless the user does something to regenerate the frame
                //  cache.
                //
...
IPImageID*
ImageSourceIPNode::evaluateIdentifier(const Context& context)
{
    LayerTree propTree;
    Component* c = component("image");
    if (!c) return 0;
    ...
    switch (selection.type)
    {
      ...
      case LayerComponent: 
      {
          if (selection.name.size() > 1)
          {
              if (selection.name[0] != "") views.push_back(selection.name[0]); 
              layers.push_back(selection.name[1]);
          }
          else layers.push_back(selection.name[0]);
          break;
      }
      case NoComponent:
      case ChannelComponent: 
...
size_t 
ImageSourceIPNode::numMedia() const 
{ 
    return 1;
}

size_t
ImageSourceIPNode::mediaIndex(const std::string& name) const
{
...
const string&
ImageSourceIPNode::mediaName(size_t index) const
{
    return m_mediaName->front();
}

void
ImageSourceIPNode::insertPixels(const string& view,
                                const string& layer,
                                int frame,
                                ...
                                size_t size)
{
    ...
    if (layer.find('/') != string::npos ||
        ...
        layer.find(';') != string::npos)
    {
        cerr << "ERROR: Characters /, @, #, *, :, ; are illegal in layer names" << endl; 
...
string 
ImageSourceIPNode::defaultView() const
{
    if (m_defaultView) return m_defaultView->front();
    else return "";
}

string 
ImageSourceIPNode::defaultLayer() const
{
    if (m_defaultLayer) return m_defaultLayer->front();
    else return "";
}

void 
ImageSourceIPNode::flushAllCaches(const FlushContext& context)
{
    //
    ...
    {
        cerr << "ERROR: while evaluateIdentifier in ImageSourceIPNode::flushAllCaches" 
             << endl
             << "CAUGHT: " << exc.what() 
             << endl;
    }

...
static void
applyLUT(const float* inp, float* outp, int channels, int nelements, void* inlut)
{
    ...
    if (lut->hasAttribute("inMatrix"))
    {
        ...
        if (M != IPImage::Matrix()) linearColorTransform(inp, outp, channels, nelements, &M);
    }
    ...
    if (lut->depth() > 1) lut2d = lut->nextPlane();
    else lut2d = lut;
    ...
    if (lut2d)
    {
        ...
        if (lut2d->hasAttribute("scale"))
        {
            ...
            IPImage::Matrix M(scale, 0, 0, offset,
                              0, scale, 0, offset,
                              0, 0, scale, offset,
                              0, 0, 0, 1);
...
struct ApplyLUTs
{
    ApplyLUTs (FrameBuffer* preLUT, FrameBuffer* LUT) : _preLUT(preLUT), _LUT(LUT) {}
    FrameBuffer* _preLUT;
    FrameBuffer* _LUT;
    ...
    { 
        if (FrameBuffer* fb = i->fb)
        {
            if (fb->isPlanar() || fb->isYRYBY() || fb->isYUV() || 
                fb->dataType() > TwkFB::FrameBuffer::PACKED_X2_B10_G10_R10)
            {
            ...
            fb->idstream() << ":CL";
            if (_preLUT) fb->idstream() << "+" << _preLUT->identifier();
            if (_LUT) fb->idstream() << "+" << _LUT->identifier();
...
struct AddLUTIDs
{
    AddLUTIDs (FrameBuffer* preLUT, FrameBuffer* LUT) : _preLUT(preLUT), _LUT(LUT) {}
    FrameBuffer* _preLUT;
    FrameBuffer* _LUT;
    ...
    { 
        ...
        str << i->id;
        str << ":CL";
        if (_preLUT) str << "+" << _preLUT->identifier();
        if (_LUT)    str << "+" << _LUT->identifier();
        i->id = str.str();
    }
};

IPImage*
CacheLUTIPNode::evaluate(const Context& context)
{
...
IPImageID*
CacheLUTIPNode::evaluateIdentifier(const Context& context)
{
...
void 
CacheLUTIPNode::propertyChanged(const Property* p) 
{
    //

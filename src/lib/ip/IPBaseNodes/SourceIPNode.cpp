...
IPImage* 
SourceIPNode::evaluate(const Context& context)
{
...
void
SourceIPNode::readCompleted(const std::string& typeName, unsigned int version)
{
    m_updateAction = "session";
}

void
SourceIPNode::addUserAttributes(FrameBuffer* fb)
{
...
IPNode::ImageComponent 
SourceIPNode::stereoComponent(const ImageComponent& component, size_t eye) const
{
...
string 
SourceIPNode::eyeView(size_t eye) const
{
    if (m_eyeViews->size() >= eye) return (*m_eyeViews)[eye];
    else return "";
}

void 
SourceIPNode::mediaInfo(const Context& context, MediaInfoVector& infos) const
{
...
const string&
SourceIPNode::mediaRepName() const
{
    return m_mediaRepName->front();
}

bool
SourceIPNode::isMediaActive() const
{
    return m_mediaActive->front()!=0;
}

void
SourceIPNode::setMediaActive(bool state)
{
...
void
SourceIPNode::updateStereoViews(const vector<string>& views, StringProperty* viewsP) const
{
    //
    //  If we have left and right views, use them, if we have 2 or more views
    //  assume first is left, last is right.  If we have just one incoming
    //  view, assume that is left (right may be coming from another layer in
    //  the source).
    //  
    //  This happens when you have EG left and right views in separate exr
    //  files each with multiView attr, but neither with "view" channels.  See
    //  separateMultiView*.exr in tweaklib.
    //

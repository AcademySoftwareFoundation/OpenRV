...
OverlayIPNode::OverlayIPNode(const string& name,
...
void
OverlayIPNode::compileRectangleComponent(Component* c)
{
...
void
OverlayIPNode::compileTextComponent(Component* c)
{
    ...
    if (frameCount < 1) lt.active = false; 
...
Paint::Text&
OverlayIPNode::LocalText::commandForFrame(int frame)
{
                              ...
                              const string& brush)
{
    ...
    memcpy((void*)outline.points, (void*)points, npoints * sizeof(Vec2f));
...
void assembleFrameCommands(
    OverlayIPNode::LocalWindow::FrameCommands& fc,
    int index,
    float ia,
    const Vec2f& ps,
    const Vec4f& oc,
    const Vec4f& wc,
    const FloatProperty* ULxP,
    const FloatProperty* ULyP,
    const FloatProperty* URxP,
    const FloatProperty* URyP,
    const FloatProperty* LLxP,
    const FloatProperty* LLyP,
    const FloatProperty* LRxP,
    const FloatProperty* LRyP,
    bool  antialias,
    const string& brush)
{
    ...
    Vec2f UL = Vec2f(index < ULxP->size() ? (*ULxP)[index] : ULxP->front(),
                     index < ULyP->size() ? (*ULyP)[index] : ULyP->front());

    Vec2f UR = Vec2f(index < URxP->size() ? (*URxP)[index] : URxP->front(),
                     index < URyP->size() ? (*URyP)[index] : URyP->front());

    Vec2f LL = Vec2f(index < LLxP->size() ? (*LLxP)[index] : LLxP->front(),
                     index < LLyP->size() ? (*LLyP)[index] : LLyP->front());

    Vec2f LR = Vec2f(index < LRxP->size() ? (*LRxP)[index] : LRxP->front(),
    ...
    if (ia == 0.0)
    //
    //  Use pixel scale to compute image aspect and normalize coordinates.
    //
    {
        ia = ps.x/ps.y;
    ...
    else
    {
        ...
        initializeOutline(fc.outline[0],
                          top,
                          2,
                          oc,
                          outlineWidth,
        ...
        initializeOutline(fc.outline[1],
                          left,
                          2,
                          oc,
                          outlineWidth,
        ...
        initializeOutline(fc.outline[2],
                          right,
                          2,
                          oc,
                          outlineWidth,
        ...
        initializeOutline(fc.outline[3],
                          bottom,
                          2,
                          oc,
                          outlineWidth,
        ...
        initializeOutline(fc.quadsOutline[0],
                          top,
                          2,
                          wc,
                          outlineWidth,
        ...
        initializeOutline(fc.quadsOutline[1],
                          left,
                          2,
                          wc,
                          outlineWidth,
        ...
        initializeOutline(fc.quadsOutline[2],
                          right,
                          2,
                          wc,
                          outlineWidth,
        ...
        initializeOutline(fc.quadsOutline[3],
                          bottom,
                          2,
                          wc,
                          outlineWidth,
...
void
OverlayIPNode::compileWindowComponent(Component* c)
{
    //  XXX locking ?
    ...
    //
    //  Make sure we have what we need, and if we return early all active flags
    //  are false:
    //

    w.windowActive = w.outlineActive = false;

    if (!windowULxP || !windowULyP ||
        !windowURxP || !windowURyP ||
        !windowLLxP || !windowLLyP ||
        !windowLRxP || !windowLRyP)
    {
        return;
    }

    int minSize = min(windowULxP->size(), min(windowULyP->size(),
                  min(windowURxP->size(), min(windowURyP->size(),
                  min(windowLLxP->size(), min(windowLLyP->size(),
                  min(windowLRxP->size(),     windowLRyP->size())))))));

    int maxSize = max(windowULxP->size(), max(windowULyP->size(),
                  max(windowURxP->size(), max(windowURyP->size(),
                  max(windowLLxP->size(), max(windowLLyP->size(),
                  max(windowLRxP->size(),     windowLRyP->size())))))));
  
    if (minSize < 1) return;

    if (maxSize > 1 && (!firstFrameP || firstFrameP->size() < 1)) return;
    ...
    if (ia == 0.0 && ps == Vec2f(0.0,0.0)) return;
    ...
    if (!w.windowActive && !w.outlineActive) return;

    //
    //  We have a set of paint commands per-frame, unless none of the vertices
    //  are animated, in which case we use one set of commands for all frames.
    //
    ...
    for (int i = 0; i < w.frameCommands.size(); ++i)
    {
        ...
        assembleFrameCommands(fc, i, ia, ps, oc, wc,
                              windowULxP, windowULyP,
                              windowURxP, windowURyP,
                              windowLLxP, windowLLyP,
                              windowLRxP, windowLRyP,
                              doAntialias,
...
void
OverlayIPNode::propertyChanged(const Property* p)
{
    if (const Component* c = componentOf(p))
    {
        if (c->name().size() > 5)
        {
            ...
            if (s == "rect:") compileRectangleComponent((Component*)c);
            if (s == "text:") compileTextComponent((Component*)c);
...
void 
OverlayIPNode::readCompleted(const string& typeName, unsigned int version)
{
...
IPImage*
OverlayIPNode::evaluate(const Context& context)
{
    IPImage* head = 0;
    ...
    if (mattesOnTop == -1)
    {
        if (getenv("TWK_MATTES_ON_TOP")) mattesOnTop = 1;
        else                             mattesOnTop = 0;
    }
    if (!mattesOnTop) addMattePaintCommands(head);
                        ...
                        if (w.windowActive)
                        {
                            ...
                            if (fc.antialiased)
                            {
                                ...
                                for (int i = 0; i < fc.quadsOutline.size(); ++i)
                                {
                                    ...
                                    if (fc.quadsOutline[i].width != oldWidth) fc.quadsOutline[i].built = false;
                        ...
                        if (w.outlineActive)
                        {
                            if (fc.antialiased)
                            {
                                ...
                                for (int i = 0; i < fc.outline.size(); ++i)
                                {
                                    ...
                                    if (fc.outline[i].width != oldWidth) fc.outline[i].built = false;
    ...
    if (!head->commands.empty()) head->commands.push_back(&m_executeAll);
        
    return head;
}

bool
OverlayIPNode::addMattePaintCommands(IPImage* head)
{
    ...
    if (showMatte && showMatte->size()) drawMatte = showMatte->front();
    if (drawMatte)
    {
        //
        ...
        // centerPoint: The center of the matte as well as the visbile section
        ...
        if (fp && fp->size()) matteOpacity = fp->front();
        if (matteOpacity <= 0) matteOpacity = 0.66;
        ...
        if (fp && fp->size()) matteRatio = fp->front();
        if (matteRatio <= 0) matteRatio = 1.33;
        ...
        if (fp && fp->size()) heightVisible = fp->front();
        ...
        if (vp && vp->size()) centerPoint = vp->front();
        ...
        //
        //  If we don't need to recalculate geometry, then add the cached rects
        //
        ...
        // ------------------------[0.5]------------------------
        // |                                                   |\
        // |                 topMatte (m_top)                  | } topMatteHeight
        // |(leftSideOfImage,topMatteVPosition)                |/
        // *---------------------------------------------------|
        // |           |                          |            |\
        // |           |                          |            | \
        // | leftMatte |      Visible Image       | rightMatte |  \
        // |  (m_left) |                          | (m_right)  |   } heightVisible
        // |           |                          |            |  /
        // |           |                          |            | /
        // |(leftSideOfImage,sideMattesVPosition) |(rightMatteHPosition,sideMattesVPosition)
        // *--------------------------------------*------------|
        // |                                                   |\
        // |              bottomMatte (m_bottom)               | } bottomMatteHeight
        // |(leftSideOfImage,-0.5)                             |/
        // *----------------------[-0.5]------------------------

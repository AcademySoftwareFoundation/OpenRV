...
namespace IPCore
{
    ...
    FileOutputGroupIPNode::FileOutputGroupIPNode(const std::string& name, const NodeDefinition* def, IPGraph* graph, GroupIPNode* group)
        : OutputGroupIPNode(name, def, graph, group)
    {
        m_filename = declareProperty<StringProperty>("output.filename", "out.mov");
        ...
        m_retimeNode->setProperty<FloatProperty>("output.fps", propertyValue(m_fps, 24.0f));
        setRoot(m_retimeNode);
    }

    FileOutputGroupIPNode::~FileOutputGroupIPNode() {}

    IPImage* FileOutputGroupIPNode::evaluate(const Context& context) { return OutputGroupIPNode::evaluate(context); }

    void FileOutputGroupIPNode::propertyChanged(const Property* p)
    {
        if (p == m_fps)
        {
            m_retimeNode->setProperty(m_retimeNode->outputFPSProperty(), propertyValue(m_fps, 24.0f));
        }

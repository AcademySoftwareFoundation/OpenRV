...
size_t 
hashBlock(const void* data, size_t size)
{
...
void
parseParametricCurve(char* rawTagData, size_t rawDataSize, ParaTag& para)
{
    ...
    for (size_t i = 0; i < ncoeffs; i++)
    {
        ...
        if (i == 0) para.gamma = frac;
        else para.coefficients.push_back(frac);
    }
}

IPNode::Vec3
parseXYZValue(char* rawTagData, size_t rawDataSize)
{
#ifdef TWK_LITTLE_ENDIAN
    swapWords(rawTagData, 2);
...
void 
logErrorHandlerFunction(cmsContext ContextID,
...
cmsHPROFILE
createLinearRec709ICCProfile()
{
    //
    //  Make a 1.0 gamma curve - basically we want an identity profile with
    //  Rec709 primaries
    //
...
IPNode::Vec3
XYZ_to_xyY(const IPNode::Vec3& v)
{
    cmsCIEXYZ a;
    a.X = v.x;
    a.Y = v.y;
...
cmsHPROFILE 
createSRGBProfileInProfileSpace(ICCIPNode::ProfileState& p)
{
    cmsCIEXYZ        white;
    cmsCIEXYZ        red;
    cmsCIEXYZ        green;
    ...
    white.X = p.white.x;    white.Y = p.white.y;    white.Z = p.white.z;
    red.X   = p.red.x;      red.Y   = p.red.y;        red.Z = p.red.z;
    green.X = p.green.x;    green.Y = p.green.y;    green.Z = p.green.z;
    blue.X  = p.blue.x;     blue.Y  = p.blue.y;      blue.Z = p.blue.z;
...
CSProfile
createCSProfileFromLCMSProfile(cmsHPROFILE p)
{
...
void
clearProfileState(ICCIPNode::ProfileState& state)
{
    if (state.profile) cmsCloseProfile(state.profile);
    if (state.data && state.ownsData) FileStream::deleteDataPointer(state.data);
...
void
parseTags(ICCIPNode::ProfileState& state)
{
    //
...
void
createProfileStateFromMemory(ICCIPNode::ProfileState& state, 
                             void *data,
                             size_t size)
{
...
void
createProfileStateFromFile(ICCIPNode::ProfileState& state, const string& filename)
{
    //
...
void
createLinearProfileState(ICCIPNode::ProfileState& state)
{
    clearProfileState(state);
...
void
createSRGBProfileState(ICCIPNode::ProfileState& state)
{
    clearProfileState(state);
...
void
createSRGBProfileStateInOtherProfileSpace(ICCIPNode::ProfileState& state,
                                          ICCIPNode::ProfileState& other)
{
    clearProfileState(state);
...
void
copyProfileToDataProperty(ICCIPNode::ProfileState& state,
...
ICCIPNode::ICCIPNode(const string& name,
      ...
      m_mode(TransformMode)
{
    ...
    if (type == "ICCDisplayTransform") m_mode = DisplayMode;
    else if (type == "ICCLinearizeTransform") m_mode = LinearizeMode;
    else if (type == "ICCTransform") m_mode = TransformMode;
    else abort();
...
void 
ICCIPNode::setInProfile(const std::string& s)
{
    if (m_inProfile)
    {
...
void 
ICCIPNode::setOutProfile(const std::string& s)
{
    if (m_outProfile)
    {
...
void 
ICCIPNode::setInProfileData(const void* data, size_t s)
{
    if (m_inProfileData)
    {
...
void 
ICCIPNode::setOutProfileData(const void* data, size_t s)
{
    if (m_outProfileData)
    {
...
string 
ICCIPNode::inProfile() const
{
    return propertyValue(m_inProfile, "");
}

string 
ICCIPNode::outProfile() const
{
...
void
ICCIPNode::updateProfileMetaData(const ProfileState& state, StringProperty* desc, FloatProperty* ver)
{
    char temp[256];
...
ICCIPNode::updateState()
{
    ...
    if (m_inProfile)
    {
        ...
        if (!m_outProfile) createSRGBProfileState(m_outState);
    }
    ...
    if (m_outProfile)
    {
        ...
        if (!m_inProfile) createSRGBProfileState(m_inState);
    }
    ...
    {
        m_transform = cmsCreateTransform(m_inState.profile,
                                         TYPE_RGB_FLT,
                                         m_outState.profile,
                                         TYPE_RGB_FLT,
                                         INTENT_PERCEPTUAL,
    ...
    if (m_cstransform) deleteColorSyncTransform(m_cstransform);
    ...
    str << "ICC:fb:lut:" 
        << m_method << ":" 
        << (m_method == LUT3DMethod ? samples3D : samples2D)
        << ":" << hex << m_inState.dataHash << "|" << inProfile()
    ...
    if (m_cstransform)
    {
        convert(m_cstransform, 
                m_method == LUT3DMethod ? s * s : s, 
                m_method == LUT3DMethod ? s : 1,
                &buffer.front(),
                m_fb->pixels<float>());
    }
#else
...
void
ICCIPNode::applyTransform(IPImage* root, bool sRGBIn, bool sRGBOut) const
{
    if (m_transform) 
    {
        ...
        if (!root->shaderExpr) root->shaderExpr = Shader::newSourceRGBA(root);
        //root->shaderExpr = Shader::newColorMatrix(root->shaderExpr, m_conversionMatrix);
        if (sRGBIn) root->shaderExpr = Shader::newColorLinearToSRGB(root->shaderExpr);
        ...
        if (m_method == LUT3DMethod)
        {
            ...
            {
                Vec3 grid = Vec3(1.0 / fb->width(),
                                 1.0 / fb->height(),
                                 1.0 / fb->depth());
                
                Vec3 scale = Vec3(Vec3(1.0, 1.0, 1.0) - grid);
 
                root->shaderExpr = Shader::newColor3DLUTGLSampling(root->shaderExpr,
                                                                   fb,
                                                                   scale,
                                                                   grid / 2.0f,
                                                                   outScale,
            ...
            {
                root->shaderExpr = Shader::newColor3DLUT(root->shaderExpr,
                                                         fb,
                                                         outScale, 
                                                         outOffset);
            }
        ...
        {
            root->shaderExpr = Shader::newColorChannelLUT(root->shaderExpr,
                                                          fb,
                                                          outScale,
                                                          outOffset);
        }

        if (sRGBOut) root->shaderExpr = Shader::newColorSRGBToLinear(root->shaderExpr);
    }
...
IPImage* 
ICCIPNode::evaluate(const Context& context)
{
    switch (m_mode)
    {
      case DisplayMode: return evaluateDisplay(context);
      case LinearizeMode: return evaluateLinearize(context);
      case TransformMode: return evaluateTransform(context);
      default:
...
IPImage* 
ICCIPNode::evaluateLinearize(const Context& context)
{
    if (IPImage* root = IPNode::evaluate(context))
    {
        if (!root->isBlank() && 
            !root->isNoImage() &&
            propertyValue(m_active, 1) != 0 && 
            m_transform) 
        {
...
IPImage* 
ICCIPNode::evaluateTransform(const Context& context)
{
    if (IPImage* root = IPNode::evaluate(context))
    {
        if (root->isBlank() || root->isNoImage()) return root;
        if (propertyValue(m_active, 1) != 0 && m_transform) applyTransform(root, false, false);
        return root;
    }
    else
    {
...
IPImage* 
ICCIPNode::evaluateDisplay(const Context& context)
{
    ...
    if (root->isBlank() || root->isNoImage()) return root;
    if (propertyValue(m_active, 1) == 0 || !m_transform) return root;
    ...
    if (m_transform) applyTransform(nroot, true, false);
    return nroot;
}

void 
ICCIPNode::propertyChanged(const Property* p)
{
    if (p == m_inProfile || p == m_outProfile || p == m_inProfileData || p == m_outProfileDesc)
    {
        if (!m_updating) updateState();
    }
}

void 
ICCIPNode::readCompleted(const std::string&, unsigned int)
{
    updateState();
}

string 
ICCIPNode::inProfileDescription() const
{
    return propertyValue(m_inProfileDesc, "");
}

string 
ICCIPNode::outProfileDescription() const
{
    return propertyValue(m_outProfileDesc, "");
}

void 
ICCIPNode::copyNode(const IPNode* node)
{
    IPNode::copyNode(node);
    if (node->definition() == definition()) updateState();
}

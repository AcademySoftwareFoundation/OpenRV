...
class RvSettings
{
  ...
  private:
    ...
    // The QSettings class does not reset its error status: once an error status
    // has been flagged, it is never cleared.
    // For reference : https://bugreports.qt.io/browse/QTBUG-23857
    // With the aim of preventing the occurence of numerous duplicated error
    // messages, this class will only report the first time an error is detected.
...
class PackageManager
{
  public:
    ...
    struct ModeEntry
    {
        ModeEntry() : loaded(false), active(false) {}
        QString     name;
        QString     package;
        QString     menu;
        QString     shortcut;

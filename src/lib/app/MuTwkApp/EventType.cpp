...
EventType::EventInstance::EventInstance(const Class *c) : ClassInstance(c),
      event(0),
      document(0)
{
}
...
EventType::EventType(Context* c, Class *super) : Class(c, "Event", super)
{
}
...
Object*
EventType::newObject() const
{
    return new EventInstance(this);
}

void
EventType::deleteObject(Object *obj) const
{
    delete static_cast<EventType::EventInstance*>(obj);
}

void 
EventType::outputValue(ostream &o, const Value &value, bool full) const
{
...
void 
EventType::outputValueRecursive(ostream &o,
...
EventType::load()
{
    ...
    MuLangContext* context = 
        (MuLangContext*)globalModule()->context();
    Context* c = context;
    ...
    globalScope()->addSymbols(
		   ...
		   new Function(c, "print", EventType::print, None,
				Return, "void", 
				Args, tn, End),
    ...
    addSymbols( new Function(c, "pointer", EventType::pointer, None,
                             Return, "vector float[2]",
                             Args, tn, End),

                new Function(c, "relativePointer", 
                             EventType::relativePointer, None,
                             Return, "vector float[2]",
                             Args, tn, End),

                new Function(c, "reference", EventType::reference, None,
                             Return, "vector float[2]",
                             Args, tn, End),

                new Function(c, "domain", EventType::domain, None,
                             Return, "vector float[2]",
                             Args, tn, End),

                new Function(c, "subDomain", EventType::subDomain, None,
                             Return, "vector float[2]",
                             Args, tn, End),

                new Function(c, "domainVerticalFlip", EventType::domainVerticalFlip, None,
                             Return, "bool",
                             Args, tn, End),

                new Function(c, "buttons", EventType::buttons, None,
                             Return, "int",
                             Args, tn, End),
                
                new Function(c, "modifiers", EventType::modifiers, None,
                             Return, "int",
                             Args, tn, End),

                new Function(c, "key", EventType::key, None,
                             Return, "int",
                             Args, tn, End),

                new Function(c, "name", EventType::name, None,
                             Return, "string",
                             Args, tn, End),

                new Function(c, "contents", EventType::contents, None,
                             Return, "string",
                             Args, tn, End),
                ...
                new Function(c, "sender", EventType::sender, None,
                             Return, "string",
                             Args, tn, End),

                new Function(c, "contentType", EventType::contentType, None,
                             Return, "int",
                             Args, tn, End),
                ...
                new Function(c, "timeStamp", EventType::timeStamp, None,
                             Return, "float",
                             Args, tn, End),

                new Function(c, "reject", EventType::reject, None,
                             Return, "void",
                             Args, tn, End),
                ...
                new Function(c, "pressure", EventType::pressure,
                             None,
                             Return, "float",
                             Args, tn, End),
                ...
                new Function(c, "rotation", EventType::rotation,
                             None,
                             Return, "float",
                             Args, tn, End),

                new Function(c, "xTilt", EventType::xTilt,
                             None,
                             Return, "int",
                             Args, tn, End),

                new Function(c, "yTilt", EventType::yTilt,
                             None,
                             Return, "int",
                             Args, tn, End),

                new Function(c, "activationTime", EventType::activationTime,
                             None,
                             Return, "float",
                             Args, tn, End),
...
NODE_IMPLEMENTATION(EventType::contentType, int)
{
        ...
        dynamic_cast<const DragDropEvent*>(i->event))
    {
        ...
        {
          case DragDropEvent::File: rval = 1; break;
          case DragDropEvent::URL: rval = 2; break;
          case DragDropEvent::Text: rval = 3; break;
...
NODE_IMPLEMENTATION(EventType::dataContents, Pointer)
{
    ...
    if (const RawDataEvent* e = dynamic_cast<const RawDataEvent*>(inst->event))
    {
        ...
        array->resize(sz);
        for (int i = 0; i < sz; ++i) array->element<Mu::byte>(i) = e->rawData()[i];
    }
    else
    {
        throwBadArgumentException(NODE_THIS, NODE_THREAD, "bad event type");
...
NODE_IMPLEMENTATION(EventType::key, int)
{
    ...
    if (const KeyEvent* ke = 
        dynamic_cast<const KeyEvent*>(e->event))
    {
        k = ke->key();
    }
    else
...
NODE_IMPLEMENTATION(EventType::pointer, Vector2f)
{
    ...
    if (const TabletEvent* te =
        dynamic_cast<const TabletEvent*>(e->event))
    {
        v[0] = te->gx();
        v[1] = te->gy();
    }

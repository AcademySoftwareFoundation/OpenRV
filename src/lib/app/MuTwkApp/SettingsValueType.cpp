...
SettingsValueType::SettingsValueType(Context* c) : VariantType(c, "SettingsValue") {}
SettingsValueType::~SettingsValueType() {}

void
SettingsValueType::load()
{
...
SettingsValueType::valueType(VariantInstance* vobj) const
{
    ...
    if (tt == _stringType) return StringType;
    else if (tt == _intType) return IntType;
    else if (tt == _boolType) return BoolType;
    else if (tt == _floatType) return FloatType;
    else if (tt == _stringArrayType) return StringArrayType;
    else if (tt == _floatArrayType) return FloatArrayType;
    else if (tt == _intArrayType) return IntArrayType;
    return NoType;
}

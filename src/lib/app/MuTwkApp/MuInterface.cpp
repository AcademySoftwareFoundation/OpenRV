...
CallEnv::CallEnv(Document* d) : Mu::CallEnvironment(), _doc(d)
{
}
...
const Value
CallEnv::call(const Function* F, Function::ArgumentVector& args) const
{
    if (_doc)
    {
        Document* current = Document::activeDocument();
        if (current != _doc) _doc->makeActive();
        const Value v = muProcess()->call(muAppThread(), F, args);
        if (current != _doc) current->makeActive();
        return v;
    }
    else
    {
...
const Value
CallEnv::callMethodByName(const char* Fname, Function::ArgumentVector& args) const
{
    if (_doc)
    {
        Document* current = Document::activeDocument();
...
{
    debugging = true;
    if (g_context) g_context->debugging(true);
}

void 
setCompileOnDemand(bool b)
{
       ...
       bool showType)
{
    ...
    if (*line)
    {
        try
        {
            Mu::TypedValue value = context->evalText(line, 
                                                     contextName,
                                                     process,
                                                     modules);
...
std::string
muEvalStringExpr(MuLangContext* context,
                 Process* process,
                 const Context::ModuleList& modules,
                 const char* line, 
                 const char* contextName)
...
cli()
{
    ...
    while (1)
    {
        ...
        try
        {
            Mu::TypedValue value = g_context->evalText(command.c_str(), "input",
                                                       g_process,
                                                       g_modules);
...
MuLangContext*
newMuContext(const char* batchFile, GCFilterFunc gc_filter, Context::ModuleList& modules)
{
#ifdef PLATFORM_DARWIN
    GarbageCollector::init();
    if (gc_filter != 0) GC_register_has_static_roots_callback(gc_filter);
#endif

    if (!g_machine) g_machine = new GenericMachine();
    if (!g_language) g_language = new MuLangLanguage;

    MuLangContext* context = new MuLangContext( batchFile ? "batch" : "cli",
                                                batchFile ? batchFile : "input" );
...
void
initMu(const char* batchFile, GCFilterFunc gc_filter)
{
    if (!g_context)
    {
...
void
initWithString(MuLangContext* context,
               Process* process,
               const Context::ModuleList& modules,
               const char* p)
{
...
void
initWithFile(MuLangContext* context,
             Process* process,
             const Context::ModuleList& modules,
             const char* file)
{
...
Menu*
createTwkAppMenu(const std::string& name, DynamicArray* array)
{
    ...
    for (int q=0; q < size; q++)
    {
        if (ClassInstance* i = array->element<ClassInstance*>(q))
        {
            ...
            menu->addItem(new Menu::Item(s->label->c_str(),
                                         action,
                                         s->key ? s->key->c_str() : "",
                                         sfunc,
                                         subMenu));

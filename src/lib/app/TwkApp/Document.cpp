...
void
Document::setFileName(const std::string& newName)
{
    m_fileName = newName;
...
std::string
Document::filePath() const
{
    return m_fileName;
}

void
Document::read(const string&, const ReadRequest&)
{
    // nothing
}

void
Document::write(const string&, const WriteRequest&)
{
    // nothing
}
...
void
Document::activateMode(const std::string& name)
{
    ...
    {
        cerr << "WARNING: tried to activate non-existant mode " 
             << name 
             << endl;
    }
}
...
void 
Document::deactivateMode(const std::string& name)
{
    if (Mode* m = findModeByName(name))
    {
        deactivateMode(m);
...
bool
Document::isModeActive(const std::string& name)
{
    if (Mode* m = findModeByName(name))
    {
...
void
Document::activateMode(Mode* mode)
{
    if (MajorMode *majorMode = dynamic_cast<MajorMode*>(mode))
    {
        if (m_majorMode == majorMode) return;
    ...
    else
    {
        ...
        MinorModes::iterator i = find(m_minorModes.begin(),
                                      m_minorModes.end(), minorMode);
    ...
    if (mode->menu()) invalidateMenu();
}

void
Document::deactivateMode(Mode* mode)
{
    if (MajorMode *majorMode = dynamic_cast<MajorMode*>(mode))
    {
        return;
...
bool
Document::isModeActive(Mode* m)
{
    if (m == m_majorMode) return true;
...
Mode *
Document::findModeByName(const std::string& name) const
{
    Modes::const_iterator i = m_modes.find(name);
    if (i == m_modes.end()) return 0;
    return (*i).second;
}

void
Document::addMode(Mode *mode)
{
...
void
Document::setSelection(const SelectionState &state)
{
...
void
Document::invalidateEventTables()
{
    m_eventTableDirty = true;
}
...
Document::buildEventTables()
{
    ...
    for (EventTableNameStack::const_iterator ni = m_eventTableNameStack.begin();
         ni != m_eventTableNameStack.end();
         ++ni)
    {
...
void
Document::setFocusTable(const EventTable* t)
{
    m_focusTable = t;
}

Document::ActionTablePair 
Document::queryEvent(const Event& event, const Action* after)
{
    ...
    for (int i=m_eventTableStack.size() - 1; 
         t == 0 && i >= 0; 
         i--)
    {
...
void
Document::executeAction(const Event& event)
{
    ...
    for (;;)
    {
        ...
        if (const Action* a = atp.first)
        {
            ...
            bool outputEvent = (showEvents && event.name() != "render" && 
            ...
            if (outputEvent)
            {
                ...
                XXX For some reason the docString() below is sometimes so
                    corrupted that RV will crash.  Haven't been able to figure out
                    why.
                ...
                */
                ...
                cerr << "Action complete on Event: '" << event.name() << "' " << 
                        ((event.handled) ? "handled" : "processed") << " by table ";
                if (atp.second->mode()) cerr << atp.second->mode()->name() << ":";
                cerr << atp.second->name() << " (" << 1000.0*(debugTimer->elapsed() - startTime) << "ms)" << endl;
            }

            if (event.handled) return;
            else 
            if (atp.second == m_focusTable) 
            //
...
void
Document::pushTable(const string& name)
{
    EventTableNameStack::iterator i = find(m_eventTableNameStack.begin(),
                                           m_eventTableNameStack.end(),
                                           name);
...
void
Document::popTable(const string& name)
{
    setFocusTable(0);
...
void
Document::debugEvents()
{
    showEvents = true;
}

const Document::EventTableStack& 
Document::eventTableStack() const
{
    if (m_eventTableDirty) const_cast<Document*>(this)->buildEventTables();
    return m_eventTableStack;
}

EventNode::Result
Document::receiveEvent(const Event& event)
{
    ...
    {
	cerr << event;
	if      (const RenderEvent*        e = dynamic_cast<const RenderEvent*>        (&event)) cerr << " '" << e->stringContent() << "'";
	else if (const DragDropEvent*      e = dynamic_cast<const DragDropEvent*>      (&event)) cerr << " '" << e->stringContent() << "'";
	else if (const GenericStringEvent* e = dynamic_cast<const GenericStringEvent*> (&event)) cerr << " '" << e->stringContent() << "'";
	cerr << endl;
    }
    m_event = &event;
    if (m_eventTableDirty) buildEventTables();
    executeAction(event);
    m_event = lastEvent;
    m_eventDocument = d;

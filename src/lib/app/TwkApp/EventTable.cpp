...
EventTable::EventTable(const string& name) : m_name(name), m_mode(0)
{
    m_bbox.makeInfinite();
}

EventTable::EventTable(const char* name) : m_name(name), m_mode(0)
{
    m_bbox.makeInfinite();
}
...
void
EventTable::bind(const std::string& event, Action* action)
{
    unbind(event);
    m_map[event] = action;
}
...
void
EventTable::bindRegex(const std::string& eventRegex, Action* action)
{
...
void
EventTable::unbind(const std::string& event)
{
...
void
EventTable::unbindRegex(const std::string& eventRegex)
{
...
const Action*
EventTable::query(const string& event) const
{

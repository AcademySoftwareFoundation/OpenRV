...
{
public:
    Selection(const SelectionType* type) : m_type(type) {}
    virtual ~Selection();
...
public:
    ...
    TypedSelection(const SelectionType *type) : Selection(type) {}
    ~TypedSelection();
...
TypedSelection<Container>::~TypedSelection()
{
    // nothing
}
...
template <class Container>
Selection*
TypedSelection<Container>::copy() const
{

...
#define OUTPUT_PLUGIN_VERSION 1.0
    ...
    {
    public:
        OutputPlugin(const std::string& file, void* handle, output_module_create_t* create_fct, output_module_destroy_t* destroy_fct)
            : m_file(file)
            , m_handle(handle)
        ...
        static std::shared_ptr<TwkApp::OutputPlugin> loadPlugin(const std::string& pathToPlugin);

...
class Document : public EventNode,
...
public:
    ...
    struct FileIORequest
    {
        ...
        //  const string foo = request.optionValue<string>("foo", "defaultValue");
        //  const int bar    = request.optionValue<int>("bar", 0);
        //  const float baz  = request.optionValue<float>("baz", 0.0f);
        //

        template <typename T>
        T optionValue(const std::string& name, const T& defaultValue = T()) const
        {
            OptionMap::const_iterator i = options.find(name);
            if (i != options.end()) return boost::any_cast<T>(i->second);
            return defaultValue;
        }
    ...
    struct MinorModeComp
    {
        bool operator() (MinorMode* a, MinorMode* b) const
        {
            return a->order() != b->order() ? 
                a->order() < b->order() : a->sortKey() != b->sortKey() ? 
                    a->sortKey() < b->sortKey() : 
                        a < b;
        }
    };

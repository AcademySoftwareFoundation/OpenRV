...
class RenderEvent : public Event
{
  public:
    //RenderEvent() : Event() {}
    RenderEvent(const std::string& name,
...
class ActivityChangeEvent : public Event
{
  public:
    ...
    ActivityChangeEvent(const std::string& name,
                        const EventNode* sender,
                        void* data=0)
        : Event(name, sender, data) {}
};
...
class ModifierEvent : public Event
{
  public:
    ...
    ModifierEvent(const std::string& name, 
                  const EventNode* sender,
                  unsigned int modifiers,
                  void* data=0) 
        : Event(name, sender, data), m_modifiers(modifiers) {}
...
class KeyEvent : public ModifierEvent
{
  public:
    KeyEvent(const std::string& name, const EventNode *sender,
        ...
        : ModifierEvent(name, sender, modifiers, data), m_key(key) {}
...
class PointerEvent : public ModifierEvent
{
  public:
    ...
    PointerEvent(const std::string& name, 
                 const EventNode* sender,
                 unsigned int modifiers,
        ...
        : ModifierEvent(name, sender, modifiers, data), 
          m_x(x), m_y(y), m_w(w), m_h(h), m_px(ox), m_py(oy),
...
class PointerButtonPressEvent : public PointerEvent
{
  public:
    PointerButtonPressEvent(const std::string& name, 
                            ...
                            void* data=0,
                            float aTime=0.0)
        : PointerEvent(name, sender, modifiers, 
                       x, y, w, h, ox, oy, 
                       buttonStates, data), m_activationTime(aTime) {}
...
class DragDropEvent : public PointerEvent
{
  public:
    ...
    enum Type        { Enter, Leave, Move, Release };
    enum ContentType { File, URL, Text };

    //DragDropEvent() : PointerEvent() {}

    DragDropEvent(const std::string& name, 
                  const EventNode* sender,
                  Type type,
                  ContentType ctype,
...
class GenericStringEvent : public Event
{
  public:
    ...
    GenericStringEvent(const std::string& name, 
                       const EventNode* sender,
                       const std::string& content,
                       void* data=0)
        : Event(name, sender, data),
...
class PixelBlockTransferEvent : public Event
{
  public:
    ...
    PixelBlockTransferEvent(const std::string& name, 
                            ...
                            void* data)
        : Event(name, sender, data),
          m_media(media), m_layer(layer), m_view(view),
...
class RawDataEvent : public Event
{
  public:
    ...
    RawDataEvent(const std::string& name, 
                 const EventNode* sender,
...
class TabletEvent : public PointerEvent
{
  public:
    ...
    TabletEvent(const std::string& name, 
                const EventNode *sender,
                unsigned int modifiers,
                int x, int y,
                int w, int h,
        ...
        : PointerEvent(name, sender, modifiers, 
                       x, y, w, h, ox, oy, 
                       buttonStates, data),
          _kind(kind), _device(device), 
          _xFloat(gx), _yFloat(gy),
          _pressure(pres), _tangentialPressure(tpres),
          _rotation(rot), _xTilt(xtilt), _yTilt(ytilt),
          _z(z) {}

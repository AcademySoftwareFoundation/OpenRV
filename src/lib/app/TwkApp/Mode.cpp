...
Mode::Mode(const char *name, Document *doc) 
        : m_name(name), m_document(doc), m_defaultMenu(0) {}
...
void
Mode::addEventTable(EventTable* table)
{
...
void 
Mode::removeEventTable(const std::string& name)
{
...
EventTable*
Mode::findTableByName(const std::string& name)
{
    EventTables::iterator i = m_eventTables.find(name);
...
MajorMode::MajorMode(const char *name, Document *doc) : Mode(name,doc) {}
MinorMode::MinorMode(const char *name, Document *doc) : Mode(name,doc), m_sortkey(name), m_order(0) {}
MajorMode::~MajorMode() {}
MinorMode::~MinorMode() {}

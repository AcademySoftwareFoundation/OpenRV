...
CommandHistory::~CommandHistory()
{
    ...
    if (m_compound) delete m_compound;
    clearHistory();
}

void
CommandHistory::beginCompoundCommand(const std::string& name)
{
    assert(m_historyState == EditingState);
    if (m_historyState != EditingState) return;
...
void
CommandHistory::endCompoundCommand()
{
    assert(m_historyState == EditingState);
    if (m_historyState != EditingState) return;
...
void
CommandHistory::clearUndoHistory()
{
    assert(m_historyState == EditingState);
...
void
CommandHistory::clearRedoHistory()
{
    assert(m_historyState == EditingState);
...
void
removeUniqueMarkers(MarkerCommand* m, CommandStack& commandStack)
{
    if (!m->isUnique()) return;
...
bool 
hasMarkerInCommandStack(const string& name, const CommandStack& commandStack)
{
    for (int i = 0; i < commandStack.size(); i++)
    {
        if (commandStack[i]->isMarker())
...
string
CommandHistory::topOfStackDescription(const CommandStack& s)
{
    if (!s.empty())
    {
...
bool 
CommandHistory::hasMarkerCommand(const string& name) const
{
...
bool 
CommandHistory::isMarkerCommandRecent(const string& name) const
{
...
void
CommandHistory::doCommand(Command* c)
{
    ScopedLock lock(m_commandQueueMutex);
    if (!m_commandQueue.empty()) flushCommandQueueInternal();
    doCommandInternal(c);
}

void
CommandHistory::doCommandInternal(Command* c)
{
    assert(m_historyState == EditingState);
...
void
CommandHistory::undoCommandInternal()
{
    if (!m_undoStack.empty())
    {
        ...
        if (!m_undoStack.empty() && c->isMarker()) undoCommandInternal();
    }
...
void
CommandHistory::redoCommandInternal()
{
    if (!m_redoStack.empty())
    {
        ...
        c->redo();
        m_undoStack.push_front(c);
        if (!m_redoStack.empty() && m_redoStack.front()->isMarker()) redoCommandInternal();
    }
...
void
CommandHistory::undoAllCommands()
{
    while (!m_undoStack.empty()) undoCommand();
}

void
CommandHistory::redoAllCommands()
{
    while (!m_redoStack.empty()) redoCommand();
}

void
CommandHistory::queueCommand(Command* c)
{
    ScopedLock lock(m_commandQueueMutex);
...
void
CommandHistory::flushCommandQueue()
{
    ScopedLock lock(m_commandQueueMutex);
...
void
CommandHistory::flushCommandQueueInternal()
{
...
CompoundCommandInfo::CompoundCommandInfo() : CommandInfo("compound")
{
}
...
Command*
CompoundCommandInfo::newCommand() const
{
...
MarkerCommandInfo::MarkerCommandInfo() : CommandInfo("marker", Marker) { }
...
void
HistoryCommand::setArgs(CommandHistory* h, CommandType t, const string& name)
{
    m_history     = h;
    m_commandType = t;
    m_name        = name;
...
HistoryCommandInfo::HistoryCommandInfo() : CommandInfo("history", EditOnly) { }

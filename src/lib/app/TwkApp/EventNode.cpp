...
EventNode::EventNode(const char *name) : m_name(name)
{
...
EventNode::~EventNode()
{
    while (m_listeners.size()) breakConnection(m_listeners.front());
    while (m_senders.size()) breakConnection(m_senders.front());
}

void
EventNode::listenTo(EventNode *node)
{
...
void
EventNode::breakConnection(EventNode *node)
{
...
EventNode::Result 
EventNode::receiveEvent(const Event&) 
{
    //
...
EventNode::Result
EventNode::sendEvent(const Event &event)
{
...
EventNode::Result
EventNode::propagateEvent(const Event &event)
{
    //
    //  If the receiver accepts the event, then stop
    //
    // std::cerr << "propagateEvent '" << event.name() << "' from '" << m_name << "'" << std::endl;

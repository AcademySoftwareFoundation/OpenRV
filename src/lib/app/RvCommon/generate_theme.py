...
def get_platform_template() -> str:
    ...
    # Check if we're being run from UTV root directory
    ...
    if os.path.basename(script_dir) == "RvCommon" and current_dir != script_dir:
        # Check if current directory looks like UTV root (has src/lib/app/RvCommon)
...
def adjust_output_path(output_file: str) -> str:
    """Adjust output path to RvCommon directory if running from UTV root"""
    # Check if we're being run from UTV root directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    current_dir = os.getcwd()
...
def adjust_config_path(config_file: Optional[str]) -> Optional[str]:
    """Adjust config file path to RvCommon directory if running from UTV root"""
    if not config_file:
        return config_file
...
def generate_theme(
    ...
    # Adjust output path if running from UTV root
    output_file = adjust_output_path(output_file)

    # Adjust config path if running from UTV root
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate an UTV theme from template (auto-detects platform)",

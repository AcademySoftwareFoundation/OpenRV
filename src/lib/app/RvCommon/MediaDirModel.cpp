...
struct MediaDirItem
{
    MediaDirItem(const QFileInfo& i, 
                 unsigned int detail,
                 unsigned int hidden,
...
void
fillData (QList<QVariant>& data, QFileInfo* fi)
{
    if (fi)
    {
...
MediaDirItem::MediaDirItem(const QFileInfo& i,
                           unsigned int d,
                           unsigned int h,
                           unsigned int s,
                           const FileTypeTraits* traits,
                           size_t fileTraitsIndex,
                           MediaDirItem* p)
    : info(i),
      parent(p),
      populated(false),
      ...
      hidden(h)
{
    if (parent)
    {
        ...
        switch (detail)
        {
          ...
          case 1:
              {
                  if (info.exists()) fillData(data, &info);
                  else 
                  {
              ...
              if (!info.isDir())
              {
                  if (MovieReader* reader = TwkMovie::GenericIO::movieReader(sfile, false))
                  {
                      try
                      {
                          ...
                          {
                            case TwkFB::FrameBuffer::UCHAR: t = "8i"; break;
                            case TwkFB::FrameBuffer::USHORT: t = "16i"; break;
                            case TwkFB::FrameBuffer::HALF: t = "16f"; break;
                            case TwkFB::FrameBuffer::FLOAT: t = "32f"; break;
                            case TwkFB::FrameBuffer::UINT: t = "32i"; break;
                            case TwkFB::FrameBuffer::DOUBLE: t = "64i"; break;
                          ...
                          if (nframes > 0) data << minfo.fps << nframes;
                      }
    ...
    else
    {
        switch (detail)
        {
          ...
          case 1:
              ...
              break;
          case 2:
              data << "Name" << "Width" << "Height" << "Type" << "Ch" << "FPS" << "#Frms";
              break;
        }
    }
}
...
MediaDirItem::populate()
{
    ...
    if (info.isDir())
    {
        ...
        {
            if (QFile::exists("/Volumes")) infos.push_back(QFileInfo("/Volumes"));
            if (QFile::exists("/Network")) infos.push_back(QFileInfo("/Network"));
        }
#endif
	...
	//
        ...
        for (int i=0; i < seqs.size(); i++)
        {
            ...
            if (seq[seq.size()-1] == '/') seq.erase(seq.size()-1, 1);

            // 
            // Qt 4.8 introduced a bug in using QDir::absoluteFilePath(file).
            // This is a work around and likely a speed up generating the path
            // directly.
            // 
            ...
            if (sinfo.isDir() || m_fileTraits->isKnown(m_fileTraitsIndex, sinfo.absoluteFilePath()))
            {
                ...
                children.push_back(new MediaDirItem(sinfo, detail, hidden, sortFlags, 
                                                    m_fileTraits, m_fileTraitsIndex, 
                                                    this));
            }
            else 
            {
...
bool
MediaDirItem::hasChildren()
{
    if (populated) return children.size() != 0;
...
MediaDirModel::MediaDirModel(const QDir& dir,
...
void
MediaDirModel::setFileTraitsIndex(size_t i)
{
    m_fileTraitsIndex = i;
    reload();
...
void 
MediaDirModel::setFileTraits(FileTypeTraits* t)
{
    delete m_fileTraits;
    m_fileTraits = t;
    reload();
...
MediaDirModel::reload()
{
    ...
    //cout << "reload = " << c << endl;
    m_root = new MediaDirItem(QFileInfo(dir), m_detail, m_hidden, m_sortFlags, 
...
void
MediaDirModel::setDirectory(const QDir& dir, Details d)
{
    ...
    m_root = new MediaDirItem(QFileInfo(rootStr),
                              (unsigned int)d, 
                              m_hidden,
                              m_sortFlags, 
                              m_fileTraits, m_fileTraitsIndex,
...
int 
MediaDirModel::columnCount(const QModelIndex& parent) const
{
    return m_root ? m_root->data.size() : 0;
}

QVariant 
MediaDirModel::data(const QModelIndex& index, int role) const
{
    if (!m_root || !index.isValid()) return QVariant();

    MediaDirItem* item = !index.isValid() 
        ? m_root 
        : static_cast<MediaDirItem*>(index.internalPointer());

    if (role == Qt::DisplayRole)
    {
        if (item->data.size() > index.column()) return item->data[index.column()];
    ...
    {
        // if (!item->info.isDir() &&
        //     !m_fileTraits->isKnown(m_fileTraitsIndex, item->info.absoluteFilePath()))
        // {
...
QModelIndex 
MediaDirModel::index(int row,
                     int column,
                     const QModelIndex& i) const
{
    if (!m_root || !hasIndex(row, column, i)) return QModelIndex();
...
QModelIndex 
MediaDirModel::parent(const QModelIndex & index) const
{
    if (m_root && index.isValid()) 
    {
        MediaDirItem* item = static_cast<MediaDirItem*>(index.internalPointer());
        if (item->parent == m_root || item->parent == 0) return QModelIndex();
...
int 
MediaDirModel::rowCount(const QModelIndex & index) const
{
    if (!m_root || index.column() > 0) return 0;
...
QVariant 
MediaDirModel::headerData(int section,
...
bool
MediaDirModel::hasChildren(const QModelIndex& index) const
{
    if (!m_root || index.column() > 0) return 0;
...
QString
MediaDirModel::absoluteFilePath(const QModelIndex& index) const
{
    if (!m_root || index.column() > 0) return 0;
...
Qt::ItemFlags 
MediaDirModel::flags(const QModelIndex &index) const
{
...
QModelIndex
MediaDirModel::indexOfPath(const QFileInfo& info) 
{
    ...
    //
    {
        ...
        for (int i=0; i < parts.size(); i++)
        {
            ...
            if (!matchItem)
            {
                ...
                if (item->info.isDir())
                {
                    ...
                    if (idir.exists(p1)) 
                    {
                        ...
                        item->children.push_back(new MediaDirItem(sinfo, 
                                                                item->detail,
                                                                item->hidden,
                                                                item->sortFlags, 
                                                                m_fileTraits, m_fileTraitsIndex,
            ...
            if (!matchItem)
            {
                ...
                cerr << "ERROR:     dir = " << dir.absolutePath().toUtf8().data() << endl;
                cerr << "ERROR:     info = " << info.absoluteFilePath().toUtf8().data() << endl;
                cerr << "ERROR:     parts.size() = " << parts.size() << endl;
                cerr << "ERROR:     i = " << i << endl;
                */
        ...
        if (!matchItem)
        {
            ...
            if (m_root->info.isDir())
            {
                ...
                if (idir.exists(baseName)) 
                {
                    ...
                    m_root->children.push_back(new MediaDirItem(sinfo, 
                                                                m_root->detail,
                                                                m_root->hidden,
                                                                m_root->sortFlags, 
                                                                m_fileTraits, m_fileTraitsIndex,
...
QMap<int, QVariant> 
MediaDirModel::itemData(const QModelIndex &index) const
{
...
void 
MediaDirModel::setSortFlags(QDir::SortFlags f)
{
    if (m_sortFlags != f)
    {
        m_sortFlags = f;
...
void 
MediaDirModel::setShowHiddenFiles(bool b)
{
    if (m_hidden != b)
    {
        m_hidden = b;
        if (m_root) setDirectory(m_root->info.absolutePath(), Details(m_root->detail));
    }
}

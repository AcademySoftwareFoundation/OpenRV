...
FileTypeTraits::fileAttributes(const QString& file) const
{
    ...
    list << "Created" << info.created().toString(Qt::DefaultLocaleShortDate);
...
QIcon
FileTypeTraits::fileIcon(const QString& file,
        QFileIconProvider::IconType knownType) const
{
    if (m_iconMode == NoIcons) return QIcon();
        ...
        QFileIconProvider::IconType knownType) const
{
    ...
    if (QFileIconProvider::Trashcan != knownType) return provider.icon(knownType);
...
bool
FileTypeTraits::hasImage(const QString&) const
{
    return false;
}

QImage 
FileTypeTraits::fileImage(const QString&) const
{
    return QImage();
}

void
FileTypeTraits::setIconMode(IconMode mode)
{
    m_iconMode = mode;
}

FileTypeTraits::IconMode
FileTypeTraits::iconMode() const
{
    return m_iconMode;
}

...
QMap<QString,QIcon> RvFileDialog::m_iconMap;
...
RvFileDialog::RvFileDialog(QWidget* parent,
                           FileTypeTraits* traits,
                           Role role,
      ...
      m_watcher(0)
{
    ...
    if (!traits) traits = new FileTypeTraits();
...
//#endif
    ...
    if (smode != -1) m_ui.sortCombo->setCurrentIndex(smode);
}

RvFileDialog::~RvFileDialog()
{
    delete m_fileTraits;
}

void
RvFileDialog::setFileTypeTraits(FileTypeTraits* traits)
{
...
void
RvFileDialog::setFileTypeIndex(size_t index)
{
    m_ui.fileTypeCombo->setCurrentIndex(index);
}

void 
RvFileDialog::setFileMode(FileMode m) 
{
    DB ("setFileMode " << m);
    m_fileMode = m; 
...
void 
RvFileDialog::setFilter(QDir::Filters filters) 
{ 
    m_filters = filters; 
}
...
void 
RvFileDialog::watchedDirChanged(const QString& dir)
{
    if (!m_reloadTimer->isActive()) m_reloadTimer->start(1000);
}

void
RvFileDialog::setDirectory(const QString& inpath, bool force)
{
    ...
    DB ("setDirectory " << 
        path.toUtf8().data() << " " << m_ui.currentPath->text().toUtf8().data());
    ...
    */
    ...
    else
    {
        ...
        if (m_visited.size() > 20) m_visited.pop_back();
    }
...
void
RvFileDialog::setCurrentFile(const QString& infileRef)
{
    ...
    if (info.isDir()) setDirectory(info.absoluteFilePath());
    else setDirectory(info.absolutePath());
    ...
    //
    //  Ensure that this item is selected and current and visible, if it's in
    //  the list.
    //
    if (!mdm || !view)
    {
    ...
    else
    {
        ...
        if (i.isValid() && view->currentIndex() != i) 
        {
            ...
            //  view->selectionModel()->select (i, QItemSelectionModel::ClearAndSelect);
        ...
        else 
        {
            if (QTreeView *tv = dynamic_cast <QTreeView *> (view)) 
            {
                ...
                //  view->selectionModel()->select (QModelIndex(), QItemSelectionModel::Clear);
            }
        }
        */
    }
      ...
      case ManyExistingFiles:
          {
              ...
              {
                  if ( TwkUtil::pathIsURL( files[i].toStdString() ) ) continue;
                  ...
                  if (!info.exists()) 
                  {
                      ...
                      if (!testinfo.exists()) ok = false;
                  }

                  if (info.isDir()) hasdir = true;
              }
              ...
              if (ok)
              {
                  if (m_fileMode == ManyExistingFiles)
                  {
                      if (!hasdir) QDialog::accept();
                      else if (1 == files.size()) setDirectory (files[0]);
                  }
                  else
                  {
                      QDialog::accept();
            ...
            else
            {
                ...
                if (!info.exists() || 
                    ...
                    ( (m_fileMode == OneFileName && !info.isDir()) ||
                        (m_fileMode == OneDirectoryName && info.isDir()) )))
                {
                    QDialog::accept();
                }
            }
...
void
RvFileDialog::reloadTimeout()
{
    if (!m_building) reload(true);
}

void
RvFileDialog::reload(bool)
{
    if (m_viewMode == ColumnView) m_columnModel->reload();
    QString current = m_ui.currentPath->text();
    setDirectory(current);
}

void
RvFileDialog::autoRefreshChanged(int state)
{
    ...
    if (m_allowAutoRefresh) reload(true);
}

void
RvFileDialog::columnSelectionChanged(const QItemSelection& selected, 
                                     const QItemSelection& deselected)
{
    DB ("columnSelectionChanged() m_building " << m_building << 
            " sel "   << selected.indexes().size() <<  
            " desel " << deselected.indexes().size());
...
void
RvFileDialog::treeSelectionChanged(const QItemSelection& selected, 
                                   const QItemSelection& deselected)
{
    DB ("treeSelectionChanged() m_building " << m_building << 
            " sel "   << selected.indexes().size() <<  
            " desel " << deselected.indexes().size());
    ...
    for (int i = 0; i < selection.size(); i++)
    {
        ...
        if (index.isValid() && index.column() == 0) ++reallyHowManySelected;
    }
    ...
    if (reallyHowManySelected == 1)
    {
        ...
        if (m_viewMode == DetailedFileView) model = m_detailFileModel;
...
void
RvFileDialog::pathComboChanged(int index)
{
...
void 
RvFileDialog::done(int r)
{
    {
        RV_QSETTINGS;
        settings.beginGroup(m_settingsGroup);
        if (m_visited.empty()) settings.remove("visited");
        else settings.setValue("visited", m_visited);
        ...
        int iconMode = 0;
        if (m_cpGeneric->isChecked()) iconMode = 1;
        else if (m_cpNone->isChecked()) iconMode = 2;
...
RvFileDialog::sidePanelTimeout()
{
   //
    ...
    for (int i=0; i < tbd.size(); i++) delete tbd[i];
...
RvFileDialog::saveSidePanel()
{
    ...
    if (paths.empty()) settings.remove("sidePanelPaths");
    else settings.setValue("sidePanelPaths", paths);
    settings.endGroup();
}

void 
RvFileDialog::sidePanelInserted(const QModelIndex& parent, int first, int last)
{
    //
...
void 
RvFileDialog::sidePanelPopup(const QPoint& pos)
{
    m_sidePanelPopup->popup(m_ui.sidePanelList->mapToGlobal(pos));
}

void
RvFileDialog::sidePanelClick(QListWidgetItem* item)
{
...
void
RvFileDialog::sidePanelDoubleClick(QListWidgetItem* item)
{
    sidePanelGo(true);
...
void
RvFileDialog::sidePanelAccept(bool)
{
    m_sidePanelAccept = true;
    accept();
}
...
void 
RvFileDialog::sidePanelRemove(bool)
{
...
void
RvFileDialog::hiddenViewChanged(bool b)
{
...
void 
RvFileDialog::iconViewChanged(QAction* action)
{
...
bool
RvFileDialog::event(QEvent* event)
{
...
bool 
RvFileDialog::eventFilter(QObject *object, QEvent *event)
{
...
QStringList
RvFileDialog::selectedFiles() const
{
    MediaDirModel* model = 0;
    QAbstractItemView* view = 0;
    ...
    DB ("RvFileDialog::selectedFiles() selection size " << selection.size() << 
            " really " << reallyHowManySelected);
...
QListWidgetItem* 
RvFileDialog::newItemForFile(const QFileInfo& info)
{
    DB ("newItemForFile()");
...
QIcon
RvFileDialog::iconForFile(const QFileInfo& ininfo)
{
...
void 
RvFileDialog::resizeColumns(const QModelIndex&)
{
    for (int i=0; i < 8; i++) m_detailTree->resizeColumnToContents(i);
}

void 
RvFileDialog::doubleClickAccept(const QModelIndex& index)
{
    if (!index.isValid()) return;

    if (const MediaDirModel* model = dynamic_cast<const MediaDirModel*>(index.model()))
    {
        if (model->flags(index) == Qt::NoItemFlags) return;
...
void
RvFileDialog::columnUpdatePreview(const QModelIndex& index)
{
    ...
    if (m_fileTraits->hasImage(file))
    {
        ...
        m_columnPreview->document()->addResource(QTextDocument::ImageResource,
                                                 QUrl("icon://file.raw"),
                                                 image);
    }
    else
...
void
RvFileDialog::lockViewMode(bool lock)
{
    if (lock) m_ui.viewCombo->hide();
    else      m_ui.viewCombo->show();
}

void
RvFileDialog::fileTypeChanged(int index)
{

      ...
      m_doc(doc)
{
    ...
    else
    {
        ...
        webChannelJs.append("\nnew QWebChannel(window.qt.webChannelTransport, function(channel) {window.rvsession = channel.objects.rvsession;});");
        ...
        if (m_eventNames[i].indexIn(name) != -1)
        {
                ...
                dynamic_cast<const PointerEvent*>(&event))
            {
                    ...
                    dynamic_cast<const DragDropEvent*>(pe))
                {
                    ...
                    switch (dd->type())
                    {
                      case DragDropEvent::Enter:    t = "enter"; break;
                      case DragDropEvent::Leave:    t = "leave"; break;
                      case DragDropEvent::Move:     t = "move"; break;
                      case DragDropEvent::Release:  t = "release"; break;
                    }

                    switch (dd->contentType())
                    {
                      case DragDropEvent::File: ct = "file"; break;
                      case DragDropEvent::URL:  ct = "url"; break;
                      case DragDropEvent::Text: ct = "text"; break;
                    }

                    emit eventDragDrop(pe->name().c_str(), 
                                       pe->x(), pe->y(), pe->w(), pe->h(),
                                       pe->startX(), pe->startY(),
                                       (int)pe->buttonStates(),
                                       t, ct, dd->stringContent().c_str());
                }
                else
                {
                    emit eventPointer(pe->name().c_str(), 
                                      pe->x(), pe->y(), pe->w(), pe->h(),
                                      pe->startX(), pe->startY(),
                                      pe->buttonStates(),
                                      atime);

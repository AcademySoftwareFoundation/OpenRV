...
namespace {
void addEventExts(const char* event, 
...
MediaFileTypes::MediaFileTypes(bool readable,
                               bool writeable,
         ...
         ++i)
    {
        ...
        for (int q=0; q < exts.size(); q++)
        {
            ...
            if ((readable && !(info.capabilities & MovieIO::MovieRead)) ||
                (writeable && !(info.capabilities & MovieIO::MovieWrite)))
            {
                continue;
            }
...
size_t 
MediaFileTypes::defaultIndex() const
{
    for (size_t i = 0; i < m_exts.size(); i++)
    {
        if (m_exts[i] == m_defaultExt) return i;
    }
...
FileTypeTraits*
MediaFileTypes::copyTraits() const
{
    return new MediaFileTypes(this);
}

QStringList 
MediaFileTypes::typeDescriptions()
{
    return m_formats;
}

QString 
MediaFileTypes::extension(int index)
{
    return m_exts[index];
}

QString 
MediaFileTypes::description(int index)
{
    return m_formats[index];
}

bool 
MediaFileTypes::isKnown(size_t index, const QString& file) const
{
    if (index >= m_exts.size()) return true;
    if (m_exts[index] == "*") return true;

    QString ext = file.split('.').last();
    if (m_exts[index] == "&")
    {
        return m_extSet.contains(ext) || 
               m_extSet.contains(ext.toLower()) || 
               m_extSet.contains("*");
    }
    else
    {
...
QStringList
MediaFileTypes::fileAttributes(const QString& file) const
{
    ...
    try
    {
        if (MovieReader* reader = TwkMovie::GenericIO::movieReader(filename, false))
        {
            ...
            if (i.video)
            {
                ...
                switch (i.dataType)
                {
                  case FrameBuffer::UCHAR: dtype = "8 bit"; break;
                  ...
                  case FrameBuffer::PACKED_R10_G10_B10_X2: dtype = "10 bit packed"; break;
                  case FrameBuffer::PACKED_X2_B10_G10_R10: dtype = "10 bit packed"; break;
                  case FrameBuffer::PACKED_Y8_Cb8_Y8_Cr8: dtype = "YCbCr packed"; break;
                  case FrameBuffer::PACKED_Cb8_Y8_Cr8_Y8: dtype = "YCbCr packed"; break;
                  default: dtype = "-";
                }

                list << "Geometry" << QString("%1 x %2, %3 ch %4")
                    .arg(i.width)
                    .arg(i.height)
                    .arg(i.numChannels)
                    .arg(dtype);
...
QIcon
MediaFileTypes::fileIcon(const QString& file) const
{
    QFileInfo info(file);
...
QIcon
MediaFileTypes::fileInfoIcon(const QFileInfo& info) const
{
...
bool
MediaFileTypes::hasImage(const QString&) const
{
    return false;
}

QImage 
MediaFileTypes::fileImage(const QString&) const
{
    return QImage();
}

bool 
MediaFileTypes::sameAs(const FileTypeTraits* traits) const
{   
    const MediaFileTypes* t = dynamic_cast<const MediaFileTypes*>(traits);
    return ( (t != nullptr) &&
             (iconMode()    == t->iconMode()  ) &&
             (m_readable    == t->m_readable  ) &&
             (m_writeable   == t->m_writeable ) &&
             (m_extSet      == t->m_extSet    ) &&
             (m_exts        == t->m_exts      ) &&
             (m_formats     == t->m_formats   ) &&

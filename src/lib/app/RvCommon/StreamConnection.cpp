...
Connection*
StreamConnection::connectionFactory(QObject* obj, bool pingpong)
{
...
void
StreamConnection::storeStream(const string &baseDir)
{
...
static int
validateStreamConnection (QDataStream* stream, const string& fileName, bool verbose)
{
    int magic;
    *stream  >> magic;
    if (verbose) cerr << "stream magic: " << magic << endl;
    ...
    int version;
    *stream  >> version;
    if (verbose) cerr << "stream version: " << version << endl;
...
void
StreamConnection::dumpStream(const string &fileName)
{
...
void
StreamConnection::handleNextEvent()
{
    int        currentType;
    QString    currentMessage;
    QString    currentInterp;
    ...
    *m_streamDataStream >> currentType;
    ...
    if (m_streamDataStream->atEnd()) return;
...
bool
StreamConnection::spoofStream(const string &fileName, bool verbose)
{
    m_verbose = verbose;
    ...
    if (m_verbose) cerr << "stream file: " << fileName << endl;
...
void
StreamConnection::startSpoofing(float timeScale)
{
...
bool 
StreamConnection::sendMessage(const QString &message)
{
...
bool 
StreamConnection::sendData(const QString& interp, const QByteArray& data)
{
    if (m_streamState == StoringState)
    {
...
bool 
StreamConnection::isLocal()
{
    if (m_streamState == SpoofingState) return true;
    else                                return TwkQtChat::Connection::isLocal();
}

const QString& 
StreamConnection::remoteName() const
{
    if (m_streamState == SpoofingState) return m_name; 
    else                                return TwkQtChat::Connection::remoteName();
}

const QString& 
StreamConnection::remoteApp() const
{
    if (m_streamState == SpoofingState) return m_app; 
    else                                return TwkQtChat::Connection::remoteApp();
}

const QString& 
StreamConnection::remoteContactName() const
{
    if (m_streamState == SpoofingState) return m_contactName;
    else                                return TwkQtChat::Connection::remoteContactName();
}

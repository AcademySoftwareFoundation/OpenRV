...
class FileTypeTraits
{
  public:
    ...
    QIcon fileIcon(const QString &, 
            QFileIconProvider::IconType knownType = QFileIconProvider::Trashcan) const;
    virtual QIcon fileInfoIcon(
            const QFileInfo &,
            QFileIconProvider::IconType knownType = QFileIconProvider::Trashcan) const;
    ...
    /// Note: This is exclusively used for optimization purposes and is therefore optional.
    /// It is perfectly valid to return false all the time.
    /// In which case the new traits will be considered different from the current ones.
    /// 

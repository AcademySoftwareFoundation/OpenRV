...
PermDelegate::PermDelegate(QObject *parent) : QItemDelegate(parent)
{
}
...
void
PermDelegate::setEditorData(QWidget *editor,
                            const QModelIndex &index) const
{
    ...
    if (i->text() == "Ask") box->setCurrentIndex(0);
    else if (i->text() == "Allow") box->setCurrentIndex(1);
    else if (i->text() == "Reject") box->setCurrentIndex(2);
}

void
PermDelegate::setModelData(QWidget *editor,

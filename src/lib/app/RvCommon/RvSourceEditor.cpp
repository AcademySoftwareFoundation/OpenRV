...
EventNode::Result 
RvSourceEditor::receiveEvent(const Event& event)
{
...
void
RvSourceEditor::updateSourceMargin(bool force)
{
...
void 
RvSourceEditor::sourceChanged()
{
    updateSourceMargin(false);
}

void
RvSourceEditor::initFromNode(IPCore::DynamicIPNode* node)
{
    if (m_updateLock) 
    {
        if (m_currentPage && m_stateMap.count(m_currentPage) > 0)
        {
            if (m_stateMap[m_currentPage].node == node) return;
        }
    }
...
void 
RvSourceEditor::editNode(const string& nodeName)
{
...
void
RvSourceEditor::typeChanged(int index)
{
}

void
RvSourceEditor::compile()
{
    if (m_node)
    {
        m_updateLock = true;
        ...
        for (size_t i = 0; i < n; i++)
        {
            if (m_typeComboBox->itemData(i).toString() == etype.value().c_str() &&
                m_typeComboBox->currentIndex() != i)
            {
                if (message.size() && message[message.size()-1] != '\n') message += "\n";
                message += "INFO: Compile succeeded but evaluation type changed to ";
                message += "\"";
                message += etype.value();
                message += "\"";
                m_typeComboBox->setCurrentIndex(i);
        ...
        if (message == "")
        {
            m_ui.outputTextEdit->append(QString("<i><font color=grey>Compilation Succeeded</font></i> - %1").arg(dtString));
        }
        else
        {
        ...
        if (message != "")
        {
            ...
            message = regex_replace(message, warningRE, "<font color=orange><b>WARNING:</b></font> <b>line \\2: </b>");
            ...
            message = regex_replace(message, warningRE2, "<font color=orange><b>WARNING:</b></font> <b>line \\1: </b>");
...
void 
RvSourceEditor::importFile()
{
}

void 
RvSourceEditor::publish()
{ 
...
/*
    QString fileName = QFileDialog::getSaveFileName(this, 
                                                    "Publish Node Definition",
                                                    ".",
                                                    "Node Definition File (*.gto)");
*/
...
void
RvSourceEditor::publishAccepted()
{
...
void 
RvSourceEditor::revert()
{
}

void 
RvSourceEditor::deselectAll()
{
...
void 
RvSourceEditor::tabChanged(int index)
{
    if (m_changeLock) return;

    if (QWidget* page = m_ui.tabWidget->widget(index))
    {
        if (m_stateMap.count(page) > 0) initFromNode(m_stateMap[page].node);
    }
}

void 
RvSourceEditor::tabClosed(int index)
{
    if (QWidget* page = m_ui.tabWidget->widget(index))
    {

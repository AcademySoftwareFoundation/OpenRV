...
int
depthFromPixelEncoding(CFStringRef pixEnc)
{
    int depth = 0;
    
    if (CFStringCompare(pixEnc, CFSTR(IO32BitDirectPixels), kCFCompareCaseInsensitive) == kCFCompareEqualTo) depth = 8;
    else if(CFStringCompare(pixEnc, CFSTR(IO16BitDirectPixels), kCFCompareCaseInsensitive) == kCFCompareEqualTo) depth = 5;
    else if(CFStringCompare(pixEnc, CFSTR(IO8BitIndexedPixels), kCFCompareCaseInsensitive) == kCFCompareEqualTo) depth = 2;
    else if(CFStringCompare(pixEnc, CFSTR(kIO30BitDirectPixels), kCFCompareCaseInsensitive) == kCFCompareEqualTo) depth = 10;
...
double
modeHz(CGDisplayModeRef mode, CFMutableArrayRef ioModesArray, int ioModeCount, double defaultHz)
{
    //
    ...
    for (CFIndex i = 0; i < ioModeCount; ++i)
    {
        if (CFMutableDictionaryRef dict =
            (CFMutableDictionaryRef) CFArrayGetValueAtIndex(ioModesArray, i))
        {
            ...
            if (n == modeID)
            {
                if (const CFDataRef data =
                    (CFDataRef) CFDictionaryGetValue(dict, CFSTR(kIOFBModeDMKey)))
                {
                    IODisplayModeInformation* ioModeInfo = 
                        (IODisplayModeInformation *) CFDataGetBytePtr(data);

                    if (ioModeInfo && 
                        ioModeInfo->nominalWidth == width && 
...
CGDisplayModeRef
displayModeRefFromIOMode(int32_t ioMode, CGDirectDisplayID cgScreen)
{
    io_service_t           ioFB           = CGDisplayIOServicePort(cgScreen);
    CFMutableDictionaryRef ioFBConfigDict = (CFMutableDictionaryRef) IORegistryEntryCreateCFProperty(ioFB, CFSTR(kIOFBConfigKey), kCFAllocatorDefault, kNilOptions);
...
void
CGDesktopVideoDevice::createFormats()
{
    ...
    CFMutableDictionaryRef ioFBConfigDict = (CFMutableDictionaryRef) IORegistryEntryCreateCFProperty(ioFB, CFSTR(kIOFBConfigKey), kCFAllocatorDefault, kNilOptions);
    CFMutableArrayRef      ioModesArray   = (ioFBConfigDict) ? (CFMutableArrayRef) CFDictionaryGetValue(ioFBConfigDict, CFSTR(kIOFBModesKey)) : 0;
    ...
    for (CFIndex i = 0, n = CFArrayGetCount(array); i < n; i++)
    {
        ...
        if ((flags & kDisplayModeSafetyFlags) && depth >= 8)
        {
            ...
            str.precision(4);
            if (hz != 0.0) str << " " << hz << "Hz ";
            if (flags & kDisplayModeStretchedFlag) str << " (stretched)";
            if (depth > 8) str << " " << depth << " bits/channel";
            ...
            m_videoFormats.push_back(DesktopVideoFormat(width, height, 1.0, 1.0, hz,
                                                        str.str(), (void*)(size_t)ioID));
        }
    }
    ...
    for (size_t i = 0; i < m_videoFormats.size(); i++)
    {
        ...
        if (mid == currentModeID) m_videoFormatIndex = i;
        m_ioModeMap[mid] = i;
    }
    ...
    if (m_videoFormatIndex == -1)
    {
        ...
        if (IPCore::debugPlayback) cout << "INFO: found new video modes" << endl;
        ...
        m_videoFormats.push_back(DesktopVideoFormat(w, h, 1.0, 1.0, cvRefHz, str.str(),
...
CGDesktopVideoDevice::~CGDesktopVideoDevice()
{
    close();
}

void 
CGDesktopVideoDevice::makeCurrent() const 
{ 
    m_viewDevice->makeCurrent();
}

bool
CGDesktopVideoDevice::isSyncing() const
{
...
void
CGDesktopVideoDevice::syncBuffers() const
{
...
void 
CGDesktopVideoDevice::open(const StringVector& args)
{
    ...
    attrs.push_back(kCGLPFAColorSize); attrs.push_back(3 * 8);
    attrs.push_back(kCGLPFAStencilSize); attrs.push_back(8);
    attrs.push_back(kCGLPFADisplayMask); attrs.push_back(CGDisplayIDToOpenGLDisplayMask(m_cgScreen));
    ...
    if (CGLError err = 
        CGLChoosePixelFormat((CGLPixelFormatAttribute*)&attrs.front(), &m_pfo, &m_npfo))
    {
...
bool
CGDesktopVideoDevice::isOpen() const
{
    return m_context != 0;
}

void 
CGDesktopVideoDevice::close()
{
    //  unbind() should be called by the renderer::setOutputVideoDevice, but just in case, call here too.
    //  
    unbind();

    ScopedLock lock(m_mutex);

    if (m_savedMode) CGDisplaySetDisplayMode(m_cgScreen, m_savedMode, NULL);
    if (m_context) CGLReleaseContext(m_context);
    if (m_cgScreen) CGDisplayRelease(m_cgScreen);
    if (m_savedMode) CFRelease(m_savedMode);
    m_savedMode = 0;
    delete m_viewDevice;
    m_viewDevice = 0;
    m_context = 0;
...
CGDesktopVideoDevice::resolution() const
{
    return format();
}

size_t 
CGDesktopVideoDevice::width() const
{
    return resolution().width;
}

size_t 
CGDesktopVideoDevice::height() const
{
    return resolution().height;
}

float 
CGDesktopVideoDevice::pixelScale() const
{
    return resolution().pixelScale;
}

VideoDevice::VideoFormat
CGDesktopVideoDevice::format() const
{
...
VideoDevice::Offset 
CGDesktopVideoDevice::offset() const
{
    return Offset(0,0);
}

VideoDevice::Timing 
CGDesktopVideoDevice::timing() const
{
    return format();
}

TwkApp::VideoDevice::ColorProfile
CGDesktopVideoDevice::colorProfile() const
{
    //

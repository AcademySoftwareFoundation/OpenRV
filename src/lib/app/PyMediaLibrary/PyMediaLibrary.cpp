...
namespace TwkMediaLibrary
{
  PyMediaLibrary::PyMediaLibrary( const std::string& appName )
      : Library( "py-media-library", appName ), m_root( new PyRootNode( this ) )
  {
  }

  PyMediaLibrary::~PyMediaLibrary()
  {
    delete m_root;
  }

  const Node* PyMediaLibrary::rootNode() const
  {
    return m_root;
  }

  size_t PyMediaLibrary::numNodeTypeNames() const
  {
    return 2;
  }

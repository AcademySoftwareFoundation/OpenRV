...
namespace TwkMediaLibrary
{
  ...
  PyMediaNode::~PyMediaNode()
  {
    Py_XDECREF( m_plugin );
  }

  const Node* PyMediaNode::node() const
  {
    return this;
  }

  URL PyMediaNode::mediaURL() const
  {
    return m_url;
  }
  ...
  const Node* PyMediaNode::baseMediaNode() const
  {
    return this;
  }
  ...
  HTTPCookieVector PyMediaNode::httpCookies() const
  {
    ...
    PyObject* ret =
        PyObject_CallMethod( m_plugin, "get_http_cookies", "s", m_url.c_str() );
    if( ret == nullptr )
    {
      PyErr_Print();
    }
    ...
    if( PyIter_Check( ret ) )
    {
      ...
      while( ( cookieDict = PyIter_Next( ret ) ) )
      {
        ...
        {
          // PyDict_GetItemString returns a borrowed reference, no need
          ...
          // PyDict_GetItemString returns a borrowed reference, no need to
          // decref
  ...
  HTTPHeaderVector PyMediaNode::httpHeaders() const
  {
    ...
    PyObject* ret =
        PyObject_CallMethod( m_plugin, "get_http_headers", "s", m_url.c_str() );
    if( ret == nullptr )
    {
      PyErr_Print();
    }
  ...
  URL PyMediaNode::httpRedirection() const
  {
    ...
    PyObject* ret = PyObject_CallMethod( m_plugin, "get_http_redirection", "s",
                                         m_url.c_str() );

...
namespace TwkMediaLibrary
{
    ...
    PropertyKeyVector Node::keys() const
    {
        //
    ...
    PropertyKeyVector Node::metaKeys(const PropertyKey& key) const
    {
        //
        //  Use the API to provide a default implementation for this
        //
    ...
    bool Node::hasMeta(const PropertyKey& key, const PropertyKey& meta) const
    {
        size_t s = numMetaKeys(key);
        for (size_t i = 0; i < s; i++)
            if (metaKey(key, i) == meta)
                return true;
        return false;
    }

    string Node::propertyValueAsString(const PropertyKey& key) const
    {
        ostringstream str;
        ...
        case UnknownType:
            ...
            break;
        case IntType:
            str << any_cast<int>(value);
            break;
        case BoolType:
            str << any_cast<bool>(value);
            break;
        case FloatType:
    ...
    const Node* Node::findChildByName(const string& name) const
    {
    ...
    ostream& Node::output(ostream& out) const
    {
    ...
    URL libraryURLtoMediaURL(const URL& inURL)
    {
    ...
    bool isLibraryURL(const URL& inURL) { return libraryOfURL(inURL) != NULL; }

    bool isNodeURL(const URL& inURL)
    {
    ...
    bool isPropertyURL(const URL& inURL)
    {
        if (Library* l = libraryOfURL(inURL))
        {
            QUrl url(inURL.c_str());
    ...
    const Node* Library::nodeOfMedia(const URL& url) const { return NULL; }

    bool Library::isLibraryMediaURL(const URL& inURL) const
    {
        //
        //  This is intended to be a fast test. The default case does a full
        //  look up.
        //
    ...
    void Library::cancelTask(const Task* item) const
    {
    ...
    void Library::cancelTasks(const TaskVector& tasks) const
    {
        if (!tasks.empty())
        {
            //
    ...
    void Library::threadMain()
    {
        ...
        while (!m_taskStop)
        {
            while (!m_taskStop && m_taskQueue.empty())
    ...
    const Node* nodeOfURL(const URL& inURL)
    {
        if (Library* l = libraryOfURL(inURL))
        {
    ...
    NodeVector libraryNodesAssociatedWithURL(const URL& inURL)
    {
        NodeVector nodes;
    ...
    URLVector libraryURLsAssociatedWithURL(const URL& inURL)
    {
        URLVector urls;
             ...
             i != globalLibraryMap.end(); ++i)
        {
            ...
            for (size_t q = 0; q < n.size(); q++)
                urls.push_back(n[q]->persistentURL());
        }

        return urls;
    }

    URLVector libraryURLsOfMedia(const URL& mediaURL)
    {
        URLVector urls;

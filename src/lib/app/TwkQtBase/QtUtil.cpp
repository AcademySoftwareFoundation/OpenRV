...
{
    return (index == 0) ? 
            "StringType::String* url = NODE_ARG_OBJECT" :
            "const StringType* stype = static_cast<const StringType*>";
}

};  //  anon namespace

string
encode(string pass, int key)
{
    ...
    for (int i = 0; i < passArray.size(); ++i) passArray[i] = passArray[i] ^ textArray[i % textArray.size()];

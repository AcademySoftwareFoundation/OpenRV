...
FileSpaceLinearizeIPNode::FileSpaceLinearizeIPNode(const std::string& name,
    ...
    : LUTIPNode(name, def, graph, group)
{
    ...
    // XXX we may have to always write these to file, since if they have been
    // set to "Linear" for some reason (IE we have a linear DPX file or
    // somethign), then when we load that session file, source_setup will think
    // they need to be configured (since they were not set in file).
...
IPImage*
FileSpaceLinearizeIPNode::evaluate(const Context& context)
{
    ...
    {
        //
        //  If input to this node is a blend, prepare head for shaderExpr mods, 
        //  etc.
        //
...
void
FileSpaceLinearizeIPNode::evaluateOne(IPImage* img, const Context& context)
{
    Vec2f cred;
    Vec2f cgreen;
    Vec2f cblue;
    ...
    //
    //  The intent of this CDL operation is for it to happen in "File Space",
    ...
    if (CDLactive && (CDL_offset != Vec3f(0.0f) ||
                      CDL_slope != Vec3f(1.0f) ||
                      CDL_power != Vec3f(1.0f) ||
                      CDL_saturation != 1.0f))
    {
        const bool noClamp = (propertyValue<IntProperty>(m_CDLnoclamp, 0) != 0);

        img->shaderExpr = Shader::newColorCDL(img->shaderExpr,
                                              CDL_slope,
                                              CDL_offset,
                                              CDL_power,
                                              CDL_saturation,
    ...
    if ( img->fb)
    {
        if (const TwkFB::FloatAttribute* lsa =
                ...
                img->fb->findAttribute(TwkFB::ColorSpace::LinearScale())))
        {
            Mat44f S;
            Vec3f scl(lsa->value());
            S.makeScale(scl);
    ...
    //  not "artistic" and you would expect them to operate on the RGB values
    //  regardless of the alpha.  I think.
    //
    ...
    if (willPremult)
    {
        ...
        if (unpremult)
        {
            // We have done an earlier unpremult shader op because there was a
            // color operation to perform.

...
outResFromParams(float w, float h, FormatIPNode::Params& s)
{
    float aspect;
    float fx = w;
    float fy = h;
...
void 
FormatIPNode::setCrop(int x0, int y0, int x1, int y1)
{
    if (m_xmin)
    {
...
void 
FormatIPNode::setUncrop(int w, int h, int x, int y)
{
...
void
FormatIPNode::setFitResolution(int w, int h)
{
...
void
FormatIPNode::setResizeResolution(int w, int h)
{
...
void
FormatIPNode::outputDisconnect(IPNode* node)
{
...
static 
FrameBuffer*
resizeImage(FrameBuffer* in, float sx, float sy, FrameBuffer::DataType outType)
{
    //
    //  sx here is really representing outWidth/inWidth (both integers), but
    //  due to imprecise representation of that rational number as a float, we
    //  _must_ round here to make sure we recover the correct outWidth and not
    //  outWidth-1.
    //
    ...
    if (ow != in->width() || oh != in->height() || outType != in->dataType())
    {
        ...
        */
        ...
        FrameBuffer* fb = new FrameBuffer(ow, oh, in->numChannels(), 
                                          ...
                                          in->orientation());
        if (fb->isRootPlane()) fb->idstream() << in->identifier();
        if (in->nextPlane()) fb->appendPlane(resizeImage(in->nextPlane(), sx, sy, outType));
        return fb;
    }
    else
    {
...
FormatIPNode::params() const
{
    ...
    if (const StringProperty* sp = m_resampleMethod)
    {
        if (sp->size())
        {
            if (sp->front() == "linear") s.method = TwkFBAux::LinearInterpolation;
        }
    }
    ...
    if (const IntProperty* bdepth = m_maxBitDepth)
    {
        ...
        if (s.maxBitDepth >= 8 ) s.bestIntegral = FrameBuffer::UCHAR;
        if (s.maxBitDepth >= 16) s.bestIntegral = FrameBuffer::USHORT;
        if (s.maxBitDepth >= 32) s.bestIntegral = FrameBuffer::UINT;
    }
    ...
    if (const IntProperty* allowFloat = m_allowFloatingPoint)
    {
        ...
        if (s.floatOk)
        {
            if (s.maxBitDepth >= 16) s.bestFloat = FrameBuffer::HALF;
            if (s.maxBitDepth >= 32) s.bestFloat = FrameBuffer::FLOAT;
        }
        else
        {
    ...
    if (m_leftCrop) s.leftCrop = m_leftCrop->front();
    if (m_rightCrop) s.rightCrop = m_rightCrop->front();
    if (m_topCrop) s.topCrop = m_topCrop->front();
    if (m_bottomCrop) s.bottomCrop = m_bottomCrop->front();
...
IPImage*
FormatIPNode::evaluate(const Context& context)
{
            ...
            s.xresize == 0 && s.yresize == 0)
        {
            if (s.crop)
            {
                ...
                try
                {
                    ...
                    // Adjust the img header's width and height accordingly since 
                    // we've just replaced its underlying FrameBuffer
                    img->width = cfb->width();
                    img->height = cfb->height();

                    //
                    //  Crop may have changed in's dataType.  If we had
                    //  previously matched inType to outType, make sure we do
                    //  so again.
                    //
                    if (sameType) outType = in->dataType();
                }
                catch (...)
                {
...
//                     cout << "s.uncrop = " << s.uncropWidth << ", " << s.uncropHeight
//                          << ", scale = " << scale << endl;
//                     cout << "fit = " << s.xfit << ", " << s.yfit << endl;
            }
...
IPImageID*
FormatIPNode::evaluateIdentifier(const Context& context)
{
    ...
    if (p.scale != 1.0f ||
        p.bestFloat != FrameBuffer::FLOAT ||
        p.bestIntegral != FrameBuffer::USHORT ||
        p.xfit ||
        p.yfit ||
        p.xresize ||
        p.yresize ||
...
void 
FormatIPNode::propertyChanged(const Property* p) 
{
    //
    //  All properties of this node affect caching of all frames of the corresponding
    //  source.  so if the props change, flush all frames from this source.
    //

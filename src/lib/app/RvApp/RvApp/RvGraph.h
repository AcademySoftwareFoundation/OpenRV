...
class RvGraph : public IPCore::IPGraph
{
  public:
    ...
    virtual IPCore::DisplayGroupIPNode* newDisplayGroup(const std::string& nodeName,
                                                        const TwkApp::VideoDevice* d = 0);
    virtual IPCore::OutputGroupIPNode* newOutputGroup(const std::string& nodeName,
                                                      const TwkApp::VideoDevice* d = 0);
    
    const Sources& imageSources() const { return m_imageSources; }
    IPCore::SourceIPNode* addSource(const std::string& nodeType, 
    ...
    // Scope of the fast add source mechanism which postpones the default views' 
    // inputs connection until all the new sources have been added
    void addSourceBegin() 
    { 
    ...
    bool isFastAddSourceEnabled() const { return m_fastAddSourceEnabled>0; }
    ...
    {
    public:
      explicit FastAddSourceGuard( RvGraph& rvgraph ) : m_rvgraph( rvgraph )
      {
        m_rvgraph.addSourceBegin();
      }
      ~FastAddSourceGuard()
      {
        m_rvgraph.addSourceEnd();
      }
    private:  
      RvGraph& m_rvgraph; 
    };    

...
static void
swapFloatData( float *data, int num )
{
...
bool
Reader::setInputFile( const char *fileName )
{
...
bool
Reader::setInputStream( std::istream &in, const char *name )
{
    m_inName = name;
    m_in = &in;
    ...
    m_in->read( (char*) &m_header.magicNumber,
                sizeof(m_header.magicNumber) );
    m_in->read( (char*) &m_header.imageWidth,
               sizeof(m_header.imageWidth) );
    m_in->read( (char*) &m_header.imageHeight,
               sizeof(m_header.imageHeight) );
...
void
Reader::readDepths( float *data, int num )
{
    size_t size = sizeof(float) * num;
    m_in->read( (char*) data, size );
    if ( m_swapped )
...
void
Reader::readStrip( float *data, int stripHeight )
{

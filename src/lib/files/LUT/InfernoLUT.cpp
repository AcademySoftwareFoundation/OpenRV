...
void
readInfernoLUT(const string& filename, 
               const string& type,
               LUTData& lut)
{
    ...
    //
    // get first un-commented line, which contains size and bit depth information
    //
    ...
    while ( nextline(file, buf, 1023) )
    {
        ...
        int len = buffer.size(); // that's right, the number of words is the dimensions
    ...
    //
    //  Allow for over-range data by assuming that if maxval is "closer" to the
    //  2N boundary beneath it, the LUT writer intended to target the smaller
    //  bit-depth, with some over-range data.
    //
    ...
    if (m != p)
    {
        ...
        if (fabs(m - lower) < fabs (m - p)) p = lower;
    }
    m = p;

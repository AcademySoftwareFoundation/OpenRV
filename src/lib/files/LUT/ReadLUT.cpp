...
bool 
isLUTFile(const std::string& filename)
{
    ...
    static const char* exts[] = {
        "rvchlut", "rv3dlut", "lt", "3dl", "cube", 
        "shakelt", "mga", "vf", "3dl", "csp", "a3d",
        "txt", 
        0
    };
...
static int 
comp(const LUTData::MappingPair& a, const LUTData::MappingPair& b)
{
    return a.first < b.first;
}

void
readFile(const string& filename, LUTData& data)
{
    const string ext = extension(filename);
...
static void 
ResampleLUT(LUTData& lut, int xsize, int ysize, int zsize)
{
    vector<int> sizes(3);
...
void
resamplePowerOfTwo(LUTData& lut)
{
    if (lut.dimensions.size() > 1)
    {
        ...
        if ( !Log2strict(lut.dimensions[0]) || 
             !Log2strict(lut.dimensions[1]) || 
             !Log2strict(lut.dimensions[2]) )
        {
            //
            // Set new_sizes to a power of 2.
            // There's a bug in our interpolation code where rounding up is
            // creating quantization, which is why we're rounding down instead.
            // (our Log2 function rounds up)
            //
...
static bool 
mappingPair2Comp (const LUTData::MappingPair& a,
...
void 
compilePreLUT(LUTData& inlut, size_t nsamples)
{
    ...
    if (s0 && s1 && s2)
    {
        ...
        for (size_t q = 0; q < 3; q++)
        {
            ...
            {
                if (plut[i].first > max) max = plut[i].first;
                if (plut[i].first < min) min = plut[i].first;
            }
        ...
        for (size_t i = 0; i < nsamples; i++)
        {
            ...
            for (size_t q = 0; q < 3; q++)
            {
                ...
                LUTData::ChannelMap::const_iterator p = 
                    lower_bound(plut.begin(),
                                plut.end(),
                                LUTData::MappingPair(t, 0),
                                mappingPair2Comp);
        ...
        Mat44f M (scales[0], 0, 0, offsets[0],
...
simplifyPreLUT(LUTData& lut)
{
    ...
    for (size_t i=0; i < 3; i++)
    {
        if (lut.prelut[i].size() >= 2)
        {
            ...
            {
                Vec3f v(lut.prelut[i][q].first,
                        lut.prelut[i][q].second,
                        0);
...
float 
normalize(LUTData& lut, bool powerOfTwo)
{
    ...
    if (powerOfTwo) 
    {
        ...
        while (p < m) p = pow(double(2), int(log2(m)+1));
        m = p;
    }

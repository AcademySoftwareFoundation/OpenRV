...
StdioBuf::StdioBuf(FILE* f) : std::streambuf(), m_file(f) {}

int 
StdioBuf::overflow(int c) 
{
   return c != EOF ? fputc(c, m_file) : EOF;
}
...
std::streampos 
StdioBuf::seekpos(std::streampos sp, std::ios_base::openmode which)
{
    return seekoff(sp, std::ios_base::beg, which);
}

std::streampos 
StdioBuf::seekoff(std::streamoff off, 

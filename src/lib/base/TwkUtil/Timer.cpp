...
int
Timer::gettimeofdayWrapper (struct timeval *tp) const
{
...
Timer::gettimeofdayWrapper (struct timeval *tp) const
{
    ...
    {
	if (getenv("RV_DEBUG_TIMERS")) debugTimers = true;
	firstTimerConstructed = false;
	if (debugTimers) fprintf(stderr, "WIN PERFORMANCE TIMER FREQUENCY %I64u\n", tickFrequency);
    }
    ...
    */
    ...
    if (m_winUsec + newUsec > 1000000i64) ++m_winSec;
    ...
    //  fprintf (stderr, "sec %u usec %u ticks %I64u leftover %I64u base %I64u\n", tp->tv_sec, tp->tv_usec, ticks, ticks % tickFrequency, m_winBaseTicks);
    //  fflush(stderr);
...
int 
Timer::gettimeofdayWrapper (struct timeval *tv) const
{
    union {
...
double Timer::stop()
{
    ...
    m_usecElapsed = (size_t(m_tp1.tv_sec) * size_t(1000000) + size_t(m_tp1.tv_usec) 
...
double Timer::elapsed() const
{
    ...
    size_t usecElapsed = (size_t(tp.tv_sec)    * size_t(1000000) + size_t(tp.tv_usec)
                        - size_t(m_tp0.tv_sec) * size_t(1000000) - size_t(m_tp0.tv_usec));

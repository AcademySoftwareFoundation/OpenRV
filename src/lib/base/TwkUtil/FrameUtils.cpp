...
#define DB(x)           if (DB_GENERAL & DB_LEVEL) cerr << "FrameUtils: " << x << endl
#define DBL(level, x)   if (level & DB_LEVEL)      cerr << "FrameUtils: " << x << endl
#else
...
static const string timeRange = "((([^0-9]-)?([0-9]+)?-?-?[0-9]+(x-?[0-9]+)?)(,(-?([0-9]+)?-?-?[0-9]+(x-?[0-9]+)?))*)?"
                                "([@#]+|%[-+ 0-9]*[di])";
...
SequencePredicateExtensionSet& 
predicateFileExtensions()
{
  ...
  while (!retStr.empty()) {
    ...
    if (m) {
        if (m.subLen(0) > maxLen) {
            ...
            // Do not consider the dash character '-' as a minus sign if it is
            // preceded by a place holder.
            // Example:
            //   Considering the file bar.10-001.foo
            //   The resulting pattern must be bar.DIGIT-DIGIT.foo and not bar.DIGITDIGIT.foo
...
//
SequenceNameList 
sequencesInDirectory( const string &path, 
                      SequencePredicate P,
...
static string
escapeString( string prose, string escapeString )
{
...
static string
escapeRegexConflicts( string prose )
{
...
SequenceNameList 
sequencesInFileList( const FileNameList& infiles, 
                     SequencePredicate P,
                     bool includeNonMatching,
                     bool frameRanges,
                     int minSequenceSize )
{
    if (defaultMinSequenceSize == -2)
    {
        ...
        else defaultMinSequenceSize = 5;
    }
    if (minSequenceSize == -1) minSequenceSize = defaultMinSequenceSize;
    ...
    // Make a copy of the file list so this function will be non-destructive.
    // Using a list rather than a vector will make item removal faster
	...
	//
	//  only patternize last element of path (don't look for frame numbers in directory names)
	//
	...
	DB ("    pattern '" << pattern << "' file '" << escapedFile << "'");
        ...
        if ( nsubs == 0 )
        {
            ...
            if ( includeNonMatching ) frameSequences.push_back( f );
            allfiles.pop_front();
            continue;
        }
	    ...
	    bool bestPatternHasDot = false;
            ...
            for ( int i = m.subCount()-2; i >= 0; i-- )
            {
                ...
                for ( list<int>::iterator iter = matchFrames.begin();
                        iter != matchFrames.end();
                        ++iter )
                {
                    if ( *iter == (lastFrame + 1) ) contiguous++;
                    else contiguous = 0;
                    
                    if ( contiguous > longestRun ) longestRun = contiguous;
                    lastFrame = *iter;
                }
		DB ("        subpattern '" << subpattern << "' hasDot " << hasDot << ", matches " << matchCount << " files");
		//
		//  Prefer pattern with the most contiguous frames or preceded by dot.
		//
            ...
            if ( bestPatternCount < minSequenceSize )
            {
                    ...
                    /* ++iter */ )
                {
                    ...
                    {
                        if ( includeNonMatching ) frameSequences.push_back( s );
                        allfiles.erase( iter++ );
                    }
                    else
                    {
            ...
            // Get a list of matching files and frame numbers, and determine the padding
            RegEx bestPatternRe( "^" + bestPattern + "$" );
            FrameList frames;
            frames.clear();
            int pad = 1000;
	    ...
	    //
	    //  Giant strings of range,range,range, etc are not useful, and on windows
	    //  they crash the qt file browser code because the columns need to be so wide,
	    //  and then reconsitiuting the frame list with pcre/RegEx causes a crash there,
	    //  so fall back to a simple range here.
	    //
	    if (timeStr.size() > 40) 
	    {
		...
		{
		    if (frames[i] < smallest) smallest = frames[i];
		    if (frames[i] > largest)  largest  = frames[i];
	        }
		...
		timeStr = rangeStr.str();
            ...
            {
                for ( int i = 0; i < pad; ++i ) timeStr += "@";
            }

            string frameSeq = bestPattern.replace( bestPattern.find( DIGIT_REGEX ), 
                                                        strlen( DIGIT_REGEX ), 
                                                        timeStr );
    ...
    if (DB_DOIT) for (int i = 0; i < frameSequences.size(); ++i) DB ("    frameseq #" << i << ": " << frameSequences[i]);
    return frameSequences;
}

bool
isStereoSequence(SequenceName seqName)
{
...
//  that an ExistingFile may or may not exist ;-) The "exists" data member of
//  ExistingFile will be true or false as appropriate.
//
...
ExistingFileList 
existingFilesInSequence(const SequenceName &inSeqName, bool justOne )
{
    ...
    if (splitSequenceName(seqName, timeStr, pattern))
    {
	    ...
	    if (seqDir == "." && seqName[0] != '.') seqDir = "";
	    else                                    seqDir = seqDir + "/";

	    std::set<string> dirFilesSet;
	    for (int i = 0; i < dirFiles.size(); ++i) dirFilesSet.insert(seqDir + dirFiles[i]);
            ...
            for ( unsigned int i = 0; i < frms.size(); ++i )
            {
                ...
                ef.name = filename;
		//
		//  fileExists calls stat, which is very slow on windows.  Since we know all these files
		//  are in the same dir, use the dir table entries to see if they exist.
		//
	    ...
	    //
	    //  Use NoEsc versions of regex first, since at this point there is
	    //  definitely no directory, and the frame number may be at the
	    ...
	    if (    (m = Match(unPaddedFrameSymRENoEsc, seq)) ||
	    ...
	    if (    (m = Match(pctSyntaxRENoEsc, seq)) ||
	    ...
	    DB ("    after matching seq '" << seq << "' fmt '" << fmt << "' frameMatcher '" << frameMatcher << "'");
            ...
            if (doReplace)
            //
            //  We are replacing a format string, and there are some matching files in the directory, 
            //  so find the min and max matching frames, assemble a timesStr, add to fmt and recurse.
            //  That way we we get a complete map of the frame range, not just the files that happen to
            //  currently exist.
            //
            {
...
bool 
splitSequenceName(const SequenceName& sequenceName,
                  string& timeStr,
...
SequencePattern 
sequencePattern(const ExistingFileList &efl, 
                bool onlyExisting,
                int minSequenceSize)
{
    FileNameList files;
    ...
    {
        if ( onlyExisting && ! efl[i].exists ) continue;
        files.push_back( efl[i].name );
    }

    SequenceNameList patterns = sequencesInFileList(files,
                                                    NULL,
                                                    true, 
                                                    true,
                                                    minSequenceSize);
...
//
string 
replaceFrameSymbols(const string& cs, int frame, int fps)
{
    Match m;
    string s = cs;
    ...
    if (!m) m = Match( unPaddedFrameSymRE, s );
    ...
    while ( m )
    {
        int f = frame;
        int pad = m.subLen(1);
        if ( m.hasSub(2) ) f += m.subInt(2);
        if ( m.hasSub(4) ) f *= m.subInt(4);
        if ( m.hasSub(5) ) f %= m.subInt(5);
        if ( m.hasSub(6) ) f += m.subInt(6);
        char fmtstr[16];
    ...
    if (!m) m = Match( paddedFrameSymRE, s );
    ...
    if (!m) m = Match( unPaddedTickSymRE, s );
    ...
    while ( s.find( "\\@" ) != s.npos ) s = s.replace( s.find( "\\@" ), 2, "@" );
    while ( s.find( "\\#" ) != s.npos ) s = s.replace( s.find( "\\#" ), 2, "#" );
    while ( s.find( "\\`" ) != s.npos ) s = s.replace( s.find( "\\`" ), 2, "`" );
    while ( s.find( "\\%" ) != s.npos ) s = s.replace( s.find( "\\%" ), 2, "%" );
...
bool
isFrameRange(const string& s)
{
    return Match(onlyTimeRE, s);
}


FrameList 
frameRange(const string& s)
{
    DB ("frameRange s " << s);
    FrameList frames;
...
int 
guessIncrement( FrameList frames, int startPos, int lookahead )
{
    // mR - consider when frames.size() <= 1
    // mR - consider when startPos >= frames.size()

    if ( frames.size() <= 1 || (startPos+1) >= frames.size() ) return 1;
...
string 
firstFileInPattern( const SequenceName& seq )
{
...
string
integrateFrameRange(const string& inpattern, const string& inrange)
{
    ...
    {
        cout << "WARNING: ignoring sequence \""
             << inpattern << " " << inrange << "\" : not convertable"
             << endl;
...
void 
convertNukeToShakeForm(const SequenceNameList& inlist, SequenceNameList& outlist)
{
...
PatternFramePair
sequenceOfFile(const std::string& infilename, 
               SequencePredicate P,
               bool frameRanges)
{
    string filename = pathConform(infilename);

    if (!fileExists(filename.c_str())) return PatternFramePair("",0);
    ...
    for (int i=0; i < seqs.size(); i++)
    {
        ...
        if (seq == file) return PatternFramePair(filename,0);
        string t, p;

        if (!splitSequenceName(seq, t, p)) continue;
        ...
        for (int q=0; q < p.size() && q < file.size(); q++)
        {
            if (p[p.size()-q-1] != file[file.size()-q-1]) break;
            end++;
        }

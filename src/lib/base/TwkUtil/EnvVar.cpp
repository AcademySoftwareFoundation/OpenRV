...
std::vector<int> EnvVarRegistryImp::search(
    const char* searchChr,
    int searchType,
    bool exact
)
{
    ...
    if ( !exact ) {
        ...
        std::transform( searchStr.begin(), searchStr.end(),
                        uppedSearchStr.begin(), EnvVarUtils::myToUpperFunctor );
    }
    ...
    for ( unsigned int i=0; i < _vars.size(); i++ ) {
        ...
        if ( !exact ) {
            ...
            std::transform( fieldValue.begin(), fieldValue.end(),
                fieldValue.begin(), EnvVarUtils::myToUpperFunctor );
        }
        if (
            // non exact search, search for substrings, case insensitive
            ( !exact && ( fieldValue.find( uppedSearchStr )
                != std::string::npos ) )
            ||
...
//------------------------------------------------------------------------------
//
void EnvVarRegistryImp::getFieldValueAsStr ( 
   char *val,
   int searchType,
   int envVarNum 
) const
...
{
   for ( Container::const_iterator it = _vars.begin();
         it != _vars.end();
         ++it ) {

...
Timecode 
timecodeFromFrameNumber(int frame, TCTime fps, TCType type)
{
    Timecode tc;
    ...
    if (ffps == fps || type == TCResidual) tcResidual(frame, fps, tc);
    else if (type == TCDropFrame) tcDropFrame(frame, fps, tc);
    else tcMonotonic(frame, fps, tc);
...
{
    //
    //  floor(log10(f)) + 1, but just in case somebody uses int64 here in
    //  the future this will continue to work.
    //
...
std::string
timecodeToString(const Timecode& tc,
                 int maxFrame,
                 char tsep,
                 char fsep,
                 bool elideHours)
{
    ...
    if (tc.rawFrame < 0) out << "-";

    int h = maxFrame
        ? std::floor(TCTime(maxFrame) / tc.rawFPS / TCTime(60.0) / TCTime(60.0))
        : tc.hours;

    if (!elideHours && h != 0) out << setw(std::max((unsigned int)2, digits(h))) << tc.hours << tsep;

    out << setw(2) << tc.minutes << tsep
        << setw(2) << tc.seconds << fsep
...
TCSeparatedString
timecodeToSeparatedString(const Timecode& tc,
                          int maxFrame,
                          bool elideHours)
{
    TCSeparatedString s;

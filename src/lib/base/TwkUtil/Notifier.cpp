...
{
    //  Messages are never destroyed.  We require this since multiple threads
    //  may access them through the m_messages list.
...
void 
Notifier::lockMessages()
{
    int err = pthread_mutex_lock(&m_messageMutex);

    #if DEBUG_MESSAGE_LOCKING
        if (err != 0) cerr << "ERROR: Notifier::lockMessages() failed, err: " << strerror(err) << endl;
    #endif
}

void 
Notifier::unlockMessages()
{
    int err = pthread_mutex_unlock(&m_messageMutex);

    #if DEBUG_MESSAGE_LOCKING
        if (err != 0) cerr << "ERROR: Notifier::unlockMessages() failed, err: " << strerror(err) << endl;
    #endif
}
...
void 
Notifier::destructReceiveInternal(Notifier *N)
{
    //
...
void 
Notifier::destructReceive(Notifier* N)
{
}
...
Notifier::Message*
Notifier::findMessage(MessageId id)
{
    lockMessages();
...
Notifier::Message*
Notifier::findMessage(const std::string &text)
{
    lockMessages();
...
std::string
Notifier::stringForMessageId(MessageId id)
{
...
Notifier::MessageId 
Notifier::registerMessage(const std::string &text)
{
...
void 
Notifier::debugMessage(MessageId id, bool state)
{
...
Notifier::Node* 
Notifier::findNode(Message *message) const
{
...
Notifier::Node* 
Notifier::findNode(Notifier *N) const
{
...
bool 
Notifier::addNotification(Notifier *N, MessageId id)
{
    if (Message *message = findMessage(id))
    {
...
bool 
Notifier::addNotification(Notifier *N, Message *message, bool createIfNone)
{
...
void 
Notifier::removeNotification(Notifier *N, MessageId id)
{
    if (Message *message = findMessage(id))
    {
...
void 
Notifier::removeNotification(Notifier *N)
{
    Node *node;
...
bool 
Notifier::receive(Notifier*, Notifier*, MessageId, MessageData *) 
{ 
    return true; 
}

void 
Notifier::receiveInternal(MessageId, MessageData *) 
{
}

void 
Notifier::sendWithData(MessageId id, MessageData *data)
{
    //
    ...
    {
	std::cerr << "Notifier \"" << name() 
		  << "\" can't send message id "
		  << id << " because it is invalid" << std::endl << std::flush;
    }
}
...
void 
Notifier::send( Notifier *originator,
		Notifier::Message *message,
		MessageData *data )
{
    if (Node *node = findNode(message))
        ...
        if (!node->m_lock)
        {
            ...
            for (int i=0, s=node->m_recipients.size(); i < s; i++)
            {
                ...
                if (receiver != originator)
                {
                    ...
                    if ( receiver->receive(originator, this, 
                                           message->m_id, data) )
                    {
...
bool 
Notifier::debugReceive(Notifier *,Notifier *N, MessageId id, MessageData *data)
{
    if (Message *message = findMessage(id))
    {
	std::cerr << N->name() << " sent \""
		  << message->m_text << "\" message to " 

...
void
Interrupt::addProgressFunc(ProgressFunc pf)
{
    m_progressFuncs.push_back(pf);
}

void
Interrupt::removeProgressFunc(ProgressFunc pf)
{
    stl_ext::remove(m_progressFuncs, pf);
}

void
Interrupt::addMessageFunc(MessageFuncs& funcs, MessageFunc f)
{
    funcs.push_back(f);
}

void
Interrupt::removeMessageFunc(MessageFuncs& funcs, MessageFunc f)
{
    stl_ext::remove(funcs, f);
}

void
Interrupt::callMessageFuncs(const MessageFuncs& funcs,
...
void 
Interrupt::beginComputation(const std::string& message)
{
#if defined _MSC_VER
    if (m_checkThread) if (pthread_self().p != m_mainThread.p) return;
#else
    if (m_checkThread) if (pthread_self() != m_mainThread) return;
#endif
...
void
Interrupt::continueComputation(float percent)
{
...
void 
Interrupt::endComputation(const std::string& message)
{
    /* AJG - this might not work that well */
#if defined _MSC_VER
    if (m_checkThread) if (pthread_self().p != m_mainThread.p) return;
    ...
    c.percentDone   = 1.0;
    c.timer.stop();
    if (c.timer.elapsed() > 0.5) callProgressFuncs();
    m_computations.pop_back();
}

void
Interrupt::error(const std::string& message)
{
...
void 
Interrupt::warning(const std::string& message)
{
...
void 
Interrupt::info(const std::string& message)
{

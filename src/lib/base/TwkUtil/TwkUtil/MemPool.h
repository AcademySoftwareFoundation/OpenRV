...
//  they are freed they are added to the free list (and not really freed).  If
//  there is not enough space in the free list (list > PoolSize) the oldest
//  (least-recently freed) blocks in the list are freed and removed from the
//  list until there is room for the incoming block.
//
//  When a new large block is requested, the free list is checked first and if
//  an appropriately-sized (relative size difference is < AllocSlop) block is
//  found, that block is returned.  If not we do a "real" alloc.
//  
//  We assume that the target pool size and minimum element size is such that
//  the pool will have a small number of elements (< 100).
//
...
class TWKUTIL_EXPORT MemPool
{
  public:
    ...
    //  Find and reuse a free block, or alloc a block (if we can't reuse one,
    //  or if the block is small).
    //
    static void* alloc(size_t size);

    //  Add a block to the FreeList (or "really free" if the block is small).
    //

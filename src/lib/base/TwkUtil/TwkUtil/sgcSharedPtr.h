...
class SharedPtr
{
  ...
  {
    if( _p && _p->decref() ) delete _p;  // NOLINT
    _p = 0;
  }  // NOLINT
 ...
 public:
  ...
  //!
  SharedPtr( const SharedPtr& r ) : _p( r._p )
  {
    _acquire();
  }
  ...
  //!
  SharedPtr( SharedPtr&& r ) noexcept : _p( r._p )
  {
    _acquire();
    r._release();
  }
  ...
  //! \return Pointer to the underlying instance.
  //!
  const RefCounted& operator*() const
  {
    return *get();
  }
  ...
  int use_count() const
  {
    return _p ? _p->refcount() : 0;
  }

  //! \brief Basic comparison operators.
  //!
  bool operator==( const RefCounted* p ) const
  {
    return get() == p;
  }
  ...
  {
    return static_cast<RefCounted*>( _p );
  }
...
class ConstSharedPtr
{
 ...
 public:
  ...
  //!
  ConstSharedPtr( const RefCounted* p = 0 ) : _p( p )
  {
    _acquire();
  }
  ...
  //!
  ConstSharedPtr( const ConstSharedPtr& r ) : _p( r._p )
  {
    _acquire();
  }
  ...
  //! \param r const shared pointer to copy
  //!
  ConstSharedPtr( ConstSharedPtr&& r ) noexcept : _p( r._p )
  {
    _acquire();
    r._release();
  }
  ...
  //!
  ConstSharedPtr( const SharedPtr<RefCounted>& r ) : _p( r._p )
  {
    _acquire();
  }
  ...
  //!
  bool operator==( const ConstSharedPtr& r ) const
  {
    return get() == r.get();
  }
  ...
  //! \see SharedPtr::get
  //!
  const RefCounted* get() const
  {
    return static_cast<const RefCounted*>( _p );
  }

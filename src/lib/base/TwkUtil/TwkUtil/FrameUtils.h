...
//  Files will not be counted as part of a sequence if the size of the sequence
//  they could make is fewer than minSequenceSize.
//
TWKUTIL_EXPORT SequenceNameList sequencesInFileList( const FileNameList& infiles, 
...
//
TWKUTIL_EXPORT SequenceNameList sequencesInDirectory( const std::string& path, 
                                       SequencePredicate P=NULL,
                                       bool includeNonMatching=true, 
                                       bool frameRanges=true,
                                       bool showDirs=false );
...
//
//  Returns a SequencePattern (i.e. foo.1-10#.tif) from an existing file list.
//  If onlyExisting is true, only files which exist on disk will be represented
...
//
// Returns an ordered list of ExistingFiles for each frame in a SequenceName.
// If the SequenceName contains a time range, there will be an entry
// for each frame, which may or may not exist on disk.  If there is a
...
//
//  Returns true if s looks like a frame range (e.g. "1-10" or "-1-10" or "1-10x2")
//

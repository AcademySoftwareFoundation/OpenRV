...
public:
    ...
    //    physFree: total amount of free physical memory. This value includes
    //              chached and inactive memory amounts since they can be
    //              reclaimed.
    //    physInactive: total amount inactive memory. Inactive memory is memory
    //                  that has not been recently used and can be reclaimed for
    //                  other purposes just like free memory. It is kept around
    //                  as an optimization in case it is required again.
    //    physCached: total amount of cached memory. Cached memory is memory that
    //                contains information cached by the OS and can be reclaimed
    //                for other purposes just like free memory. The biggest client
    //                of cached memory is the file cache.
    ...
    static bool getSystemMemoryInfo(
        size_t* physTotal,
        size_t* physFree,
        size_t* physUsed,
        size_t* physInactive,

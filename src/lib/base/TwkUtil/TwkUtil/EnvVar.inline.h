...
inline void initialize(const EnvVar<bool>& var)
{
   if ( !var._base._ready )
   {
      // Initialization not done yet, so cast-away constness to initialize now.
...
//------------------------------------------------------------------------------
//
inline const char* BaseEnvVar::getName() const
{
   return _name;
}
...
//------------------------------------------------------------------------------
//
inline void BaseRegEnvVar::dynSetValue( bool /*newValue*/ )
{
   assert(0);
}

inline void BaseRegEnvVar::dynSetValue( int /*newValue*/ )
{
   assert(0);
}

inline void BaseRegEnvVar::dynSetValue( float /*newValue*/ )
{
   assert(0);
}
...
//
template <class T>
const char* RegEnvVar<T>::getName() const
{
   return _var.getName();
}
...
//
template <class T>
BaseEnvVar::EnvVarType RegEnvVar<T>::getType() const
{
   return _var.getType();
}
...
//
template <class T>
bool RegEnvVar<T>::isReadOnly() const
{
...
//
template <class T>
void RegEnvVar<T>::dynSetValue( T newValue )
{
...
//
template <class T>
std::ostream& RegEnvVar<T>::dump( std::ostream& os ) const
{
   using namespace std;
...
//
template <class T> 
const char* EnvVar<T>::getName() const
{
...
//
template <class T> 
void EnvVar<T>::invokeChangeValueCallback()
{
...
//
template <class T> 
BaseEnvVar::EnvVarType EnvVar<T>::getType() const
{
...
//
template <class T>
T EnvVar<T>::getDefaultValue() const
{
   if ( !_base._ready )  // Static init sanity check (see comment in .h file).
      EnvVarUtils::initialize( *this );
...
//
template <class T>
void EnvVar<T>::setValue(T newValue)
{
   if ( !_base._ready )  // Static init sanity check (see comment in .h file).
      EnvVarUtils::initialize( *this );
   ...
   std::vector<int> found = 
      EnvVarRegistry::getInstance().search( _base._name , BaseEnvVar::EV_NAME, 1 );
...
//
template <class T>
void EnvVar<T>::setValueOnly(T newValue)
{
   if ( !_base._ready )  // Static init sanity check (see comment in .h file).
      EnvVarUtils::initialize( *this );
...
//
template <class T>
void EnvVar<T>::setDefaultValue(T newValue)
{
   if ( !_base._ready )  // Static init sanity check (see comment in .h file).
      EnvVarUtils::initialize( *this );
...
//
template <class T>
void EnvVar<T>::setReadOnlyStatus(bool state)
{
   _isReadOnly = state;
}
...
//
template <class T>
bool EnvVar<T>::isReadOnly() const
{
   return _isReadOnly;
}
...
//
template <class T> 
std::ostream& EnvVar<T>::dump( std::ostream& os ) const
{
   using namespace std;

   os <<
      "name : "   << _base.getName() << endl <<
      "value : "  << _value << endl;

...
//        job data depending on the operation being peformed by the dispatcher
//
namespace JobOps
{
  using Id = int;
...
class TWKUTIL_EXPORT JobDispatcher
{
  ...
  struct JobBase : public RefCountedMT
  {
    ...
    explicit JobBase( JobOps::Id dependency ) : m_dependency( dependency ) {}
  ...
  struct Job : public JobBase
  {
    ...
    // @brief trigger running state on start and finish state on completion
    //
 ...
 public:
  /// @brief constructor
  ///
  /// @param maxJobs The maximum number of concurrent jobs. Used to construct
  ///                the thread pool
  ...
  /// @brief blocking wait until all jobs are completed and stop all threads.
  ///
  ...
  ///                the thread pool
  ///
  /// @remark This will complete all running threads before the resize happens
  ///
  ...
  /// @return id of job
  ///
  /// @remark the job is queued for execution until there is a free thread in
  ///         the pool
  ///
  /// @remark when the job is run JobOps::update will be called with type RUN

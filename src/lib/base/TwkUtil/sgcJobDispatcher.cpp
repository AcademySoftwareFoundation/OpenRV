    ...
    for( size_t i = 0; i < m_threadPoolSize; ++i )
    {
      m_threadPool.emplace_back( [this, i]() -> void {
        ...
        while( 1 )
        {
          ...
          {
            // Wait for a signal (new job added or dependency complete) or
            // shutdown
            //
            MutexGuard lock{&m_mutex};
            m_signal.wait( lock.native(), [this]() -> bool {
              return m_numPendingJobs || m_shutdown;
            } );

            // If shutdown occurred, finish executing all remaining jobs and
            // then exit
            //
            if( m_shutdown && m_waiting.empty() ) return;

            // Find the next queued job that doesn't have a queued or running
            // dependency.  If one is not found, go back to waiting
            //
            bool foundJob = false;
            for( auto nextJob = m_waiting.begin(), end = m_waiting.end();
                 nextJob != end; ++nextJob )
            {
              foundJob =
                  findById( m_waiting, ( *nextJob )->m_dependency ) == end &&
                  findById( m_running, ( *nextJob )->m_dependency ) ==
                      m_running.end();
              if( foundJob )
              {
                job = *nextJob;
          ...
          // Signal any waiting threads waiting for a dependency to complete
          //
          {
            MutexGuard lock{&m_mutex};
            if( auto runningJob = findById( m_running, job->m_id );
...
//-----------------------------------------------------------------------------
//
void JobDispatcher::removeJob( JobOps::Id id )
{
  return m_imp->removeJob( id );
}

//-----------------------------------------------------------------------------
//
void JobDispatcher::wait() const
{
  m_imp->wait();
}
...
//-----------------------------------------------------------------------------
//
void JobDispatcher::resize( unsigned maxJobs )
{
  m_imp->resize( maxJobs );
}
...
//-----------------------------------------------------------------------------
//
void JobDispatcher::waitJob( JobOps::Id id )
{
  m_imp->waitJob( id );
}
...
//-----------------------------------------------------------------------------
//
void JobDispatcher::prioritizeJob( JobOps::Id id )
{
  m_imp->prioritizeJob( id );
}
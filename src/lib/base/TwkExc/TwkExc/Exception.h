...
template <typename T>
inline std::ostream&
operator << (Exception& e, const T& v)
{
    e.stream() << v;
    return e.stream();
}
...
EXC_TYPE::EXC_TYPE() throw() : BASE_TYPE()  {}                          \
EXC_TYPE::EXC_TYPE(const char *str) throw() : BASE_TYPE(str)   {}       \
EXC_TYPE::EXC_TYPE(const std::string& str) throw() : BASE_TYPE(str)   {} \
EXC_TYPE::EXC_TYPE(const std::ostringstream &str) throw() : BASE_TYPE(str)   {} \
...
public:                                                 \
    ...
    EXC_TYPE( const char *) : BASE_TYPE( TAG ) {}              \
    EXC_TYPE( const std::string&  ) : BASE_TYPE( TAG ) {}       \
    EXC_TYPE( const std::ostringstream & ) : BASE_TYPE( TAG ) {} \
};

...
fixed_block_allocator::fixed_block_allocator(size_t value_size)
    : _blocks(), _free(0), _value_size(value_size)
{
...
fixed_block_allocator::pointer 
fixed_block_allocator::allocate()
{
...
void
fixed_block_allocator::deallocate(fixed_block_allocator::pointer p)
{
    if (p)
    {
        pthread_mutex_lock(&_lock);
...
size_t
fixed_block_allocator::capacity()
{
    size_t bytes = 0;
...
bool
fixed_block_allocator::is_allocated(fixed_block_allocator::const_pointer ptr) const
{
...
fixed_block_allocator::is_live(fixed_block_allocator::const_pointer ptr)
{
    if ( is_allocated(ptr) )
    {
	for (dead_object_pointer p = _free; 
	     p; 
...
void
fixed_block_allocator::mark_free(bool b)
{
    pthread_mutex_lock(&_lock);
...
void
fixed_block_allocator::iterator::init()
{
...
void
fixed_block_allocator::iterator::next()
{
    if (!_p) return;

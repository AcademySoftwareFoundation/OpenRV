...
block_alloc_arena::~block_alloc_arena()
{
    ...
    for (auto p : _small_blocks) delete p;
    for (auto& p : _large_blocks) delete p.second;
}
...
block_alloc_arena::allocate(size_t n)
{
    ...
    else
    {
        ...
        {
            while ( _small_blocks.size() <= n ) _small_blocks.push_back(0);
    ...
    else
    {
        if (p)
        {
            ...
            if (n >= _large_block_size)
            {
                n = adjust_size(n);
                pthread_mutex_unlock(&_lock);
                if (allocator* a = _large_blocks[n]) a->deallocate(p);
            }
            else
            {
                n = adjust_size(n);
                pthread_mutex_unlock(&_lock);
                if ( _small_blocks.size() > n && _small_blocks[n] ) _small_blocks[n]->deallocate(p);
            }
        }
    }
}
...
block_alloc_arena::is_allocated(const void *p, size_t n) const
{
	...
	? _small_blocks[n]->is_allocated(p) : false;
}

bool
block_alloc_arena::is_allocated(const void *p) const
{
    for (auto b : _small_blocks) if (b && b->is_allocated(p)) return true;
    for (auto& bp : _large_blocks) if (bp.second->is_allocated(p)) return true;
    return false;
}
...
{
    n = adjust_size(n);
    if (_small_blocks.size() > n && _small_blocks[n]) return _small_blocks[n]->is_live(p);
    auto i = _large_blocks.find(n);
    if (i != _large_blocks.end()) return i->second->is_live(p);
    return false;
}
...
{
    for (auto b : _small_blocks) if (b && b->is_live(p)) return true;
    for (auto& bp : _large_blocks) if (bp.second->is_live(p)) return true;
    return false;
}
...
{
    pthread_mutex_lock(&_lock);
    for (auto block : _small_blocks) if (block) block->mark_free(b);
    pthread_mutex_unlock(&_lock);
}
...
block_alloc_arena::iterator::next()
{
	...
	if (_index >= _arena->_small_blocks.size()) { _index--; return; }
	_i = _arena->_small_blocks[_index]->begin();
    }
}
...
{
    size_t bytes = 0;
    for (auto block : _small_blocks) if (block) bytes += block->capacity();
    return bytes;
}

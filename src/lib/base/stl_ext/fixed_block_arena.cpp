...
fixed_block_arena::pointer 
fixed_block_arena::allocate()
{
    try
    {
...
void
fixed_block_arena::deallocate(fixed_block_arena::pointer p)
{
    if (p)
    {
...
fixed_block_arena::is_allocated(fixed_block_arena::const_pointer p) const
{
    for (int i=_allocators.size()-1; i >= 0; i--)
    {
        if (_allocators[i]->is_allocated(p)) return true;
    }
...
bool
fixed_block_arena::is_live(fixed_block_arena::const_pointer p)
{
    for (int i=_allocators.size()-1; i >= 0; i--)
    {
        if (_allocators[i]->is_live(p)) return true;
    }
...
void
fixed_block_arena::mark_free(bool b)
{
    for (int i=_allocators.size()-1; i >= 0; i--)
    {
...
void
fixed_block_arena::iterator::init()
{
    _it = _arena->_allocators.front()->begin();
}

void
fixed_block_arena::iterator::next()
{

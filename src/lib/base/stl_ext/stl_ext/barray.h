...
template <class T, unsigned int Bpower=8, class Alloc=std::allocator<T*> >
class barray
{
  public:
    ...
    {
      public:
	iterator(this_type &ba, size_t i=0) : _ba(&ba), _index(i) {}
    ...
    {
      public:
	const_iterator(const this_type &ba, size_t i) : _ba(&ba), _index(i) {}
	const T&		operator* () const { return (*_ba)[_index]; }
	void			operator++ () { _index++; }
	void			operator++ (int) { _index++; }
	void			operator-- () { _index--; }
    ...
    barray() : _blocks(), _size(0) {}
    barray(size_t s) : _blocks(), _size(0) { resize(s); }
    ~barray();

    size_t		    size() const { return _size; }
    size_t		    capacity() const 
                            { return _blocks.size() * (2 << Bpower); }
    void                    reserve(size_t n) 
                            { size_t s = size(); resize(n); resize(s); }
...
void barray<T,Bpower,Alloc>::resize(size_t nsize, T c)
{
    ...
    for (size_t i=_size; i < nsize; i++) (*this)[i] = c;
    _size = nsize;
}
...
{
    for (size_t i=it._index,s=size()=1; i<s; i++) (*this)[i] = (*this)[i+1];
    resize(size()-1);
}
...
void barray<T,Bpower,Alloc>::erase(iterator first, iterator last)
{
    ...
    for (size_t i=i0,s=size()=1; i<s; i++) (*this)[i] = (*this)[i+1];
}
...
bool barray<T,Bpower,Alloc>::is_allocated(const T *ptr)
{
	...
	if (_blocks[i] <= ptr && ptr < _blocks[i]+(1<<Bpower)) return true;
    return false;
}

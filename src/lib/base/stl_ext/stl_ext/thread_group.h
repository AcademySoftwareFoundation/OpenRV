...
class STL_EXT_EXPORT thread_group
{
  public:
    ...
    typedef int  (*thread_create)(pthread_t*, 
    ...
    //
    //  Create the group with an initial number of threads or call add_thread
    //  later.
    //
    ...
    thread_group(int num_initial_threads=0,
                 int stack_size_mult=1,
                 thread_api* = 0,
		 const func_vector* = 0,
		 const data_vector* = 0);
    ~thread_group();
    ...
    void		add_thread(int num_threads=1, 
				   size_t stack_mult=1, 
				   const func_vector* = 0, 
				   const data_vector* = 0);
  ...
  private:
    ...
    {
        //thread_package () {}
        //thread_package (thread_group* g, thread_function f, void* d) : group(g), func(f), data(d) {}

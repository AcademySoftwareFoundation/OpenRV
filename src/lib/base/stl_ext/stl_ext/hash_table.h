...
template<class T,class Traits>
inline
hash_table<T,Traits>::iterator::iterator(hash_table<T,Traits> *table)
    : _table(table), _index(0), _item(0) 
{
    if (_table && _table->_tableSize) this->operator++();
}
...
hash_table<T,Traits>::iterator::iterator(hash_table<T,Traits> &table)
    : _table(&table), _index(0), _item(0) 
{
    if (_table->_tableSize) this->operator++();
}

template<class T,class Traits>
void
hash_table<T,Traits>::iterator::operator++()
{
    if (_item) 
	if (!(_item = _item->next())) _index++;

    if (!_item && _table)
    {
	for (;_index < _table->_tableSize && !_table->_table[_index]; 
	     _index++);
	if (_index < _table->_tableSize) _item = _table->_table[_index];
    }
}
...
template<class T,class Traits>
hash_table<T,Traits>::item::item(const T &thing) : _data(thing), _next(0) {}

template<class T,class Traits>
hash_table<T,Traits>::hash_table() : _table(0), _tableSize(0), _size(0) 
{ 
    resize(); 
}
...
{
    if (const item *item = find(thing)) return item;
    if (_size++ >= _tableSize) resize();
...
hash_table<T,Traits>::find(const T& thing) const
{
	...
	if (Traits::equals(item->data(),thing)) return item;
    return 0;
}

template<class T,class Traits>
const typename hash_table<T,Traits>::item* 
hash_table<T,Traits>::first_item_of(typename hash_table<T,Traits>::hash_value h) const
{
...
hash_table<T,Traits>::resize()
{
    ...
    for (int i=0; i<_tableSize; i++) _table[i]=0;

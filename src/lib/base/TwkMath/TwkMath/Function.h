...
template <typename T>
inline T wrap( const T &a, const T &by )
{
    T rem = a % by;
...
template <typename T>
inline T betterMod( const T &a, const T &by )
{
    return wrap( a, by );
}
...
template <typename T>
inline T gradSmoothstep(const T &t)
{
    if (t <= T(0) || t >= T(1))
    {
        return T(0);
...
template <typename T>
inline bool zeroTol( const T &v, const T &tol )
{
...
template <typename T>
inline T areaOfTriangle( const Vec3<T> &va,
                         const Vec3<T> &vb,
                         const Vec3<T> &vc )
{
    Vec3<T> v0 = vb - va;
...
template <typename T>
inline T taperedClamp(T val, T lo, T hi)
{
    if (val < T(0)) val = lo;
...
template <typename T>
Vec2<T> intersectionOfLines(const Vec2<T>& a,
                            const Vec2<T>& b,
                            const Vec2<T>& c,
                            const Vec2<T>& d)
{
    ...
    return Vec2<T>( (d0 * mx1 - mx0 * d1) / d2,
                    (d0 * my1 - my0 * d1) / d2 );
}

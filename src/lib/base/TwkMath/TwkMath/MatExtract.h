...
//******************************************************************************
template <typename T>
inline Mat44<T> xRotationMatrix( const T &xRot )
{
    T cosr = Math<T>::cos( xRot );
    T sinr = Math<T>::sin( xRot );
    
    return Mat44<T>( T(1), T(0), T(0), T(0),
                     T(0), cosr, -sinr, T(0),
                     T(0), sinr, cosr, T(0),
                     T(0), T(0), T(0), T(1) );
}
...
void extractEulerXYZ( const Mat44<T> &mat, Vec3<T> &rot )
{
    ...
    T cy = Math<T>::sqrt( N[0][0]*N[0][0] +
                          N[1][0]*N[1][0] );
    
    rot.y = Math<T>::atan2( -N[2][0], cy );
    rot.z = Math<T>::atan2( -N[0][1],
                            N[1][1] );
}

//******************************************************************************
template <typename T> 
bool checkForZeroScaleInCol( const T& scl, 
                             const Vec3<T> &col,
                             bool throwOnError )
{
    for ( int i = 0; i < 3; ++i )
               ...
               Math<T>::abs( col[i] ) >= Math<T>::max() * scl ) )
	{
	    if ( throwOnError )
            {
                TWK_EXC_THROW_WHAT( ZeroScaleExc,
...
//******************************************************************************
template <typename T>
bool extractAndRemoveScalingAndShear( Mat44<T> &mat, 
                                      Vec3<T> &scl,
                                      Vec3<T> &shr,
                                      bool throwOnError )
{
    ...
    // Normalize 3rd col and correct the XZ and YZ shear factors for Z scaling.
    col[2] /= scl.z;
    shr[1] /= scl.z;
    shr[2] /= scl.z;
...
//******************************************************************************
template <typename T>
bool extractSHRT( const Mat44<T> &mat,
                  Vec3<T> &Scale,
                  Vec3<T> &sHear,
                  Vec3<T> &Rotation,
                  Vec3<T> &Translation,
...
{
    Mat44<T> rot = mat;
    if ( !extractAndRemoveScalingAndShear( rot,
                                           Scale,
                                           sHear,
                                           THROW_ON_ERROR ) )
    {
...
Mat44<T> alignZAxisWithTargetDir( Vec3<T> targetDir, Vec3<T> upDir )
{
    ...
    return Mat44<T>( col[0][0], col[1][0], col[2][0], T(0),
                     col[0][1], col[1][1], col[2][1], T(0),
                     col[0][2], col[1][2], col[2][2], T(0),
                     T(0), T(0), T(0), T(1) );
}

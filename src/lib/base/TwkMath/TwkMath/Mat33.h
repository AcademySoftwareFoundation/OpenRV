...
public:
    ...
    Mat33() :
      m00( T1 ), m01( T0 ), m02( T0 ),
      m10( T0 ), m11( T1 ), m12( T0 ),
      m20( T0 ), m21( T0 ), m22( T1 ) {}
    ...
    explicit Mat33( const Vec3<T>& diag ) :
      m00( diag.x ), m01( T0 ), m02( T0 ),
      m10( T0 ), m11( diag.y ), m12( T0 ),
      m20( T0 ), m21( T0 ), m22( diag.z ) {}

    // Direct constructor. Initializes elements directly
    Mat33( const T &v00, const T &v01, const T &v02,
    ...
    template <typename T2>
    Mat33<T> &operator=( const Mat33<T2> &copy )
    {
        m00 = copy.m00; m01 = copy.m01; m02 = copy.m02;
        m10 = copy.m10; m11 = copy.m11; m12 = copy.m12;
        m20 = copy.m20; m21 = copy.m21; m22 = copy.m22;
    ...
    void set( const T &v00, const T &v01, const T &v02,
              const T &v10, const T &v11, const T &v12,
              const T &v20, const T &v21, const T &v22 )
    {
        m00 = v00; m01 = v01; m02 = v02;
    ...
    {
        m00 = T1; m01 = T0; m02 = t.x;
        m10 = T0; m11 = T1; m12 = t.y;
        m20 = T0; m21 = T0; m22 = T1;
    }

    // Modify translations of matrix
    void setTranslation( const Vec2<T> &t ) { m02 = t.x; m12 = t.y; }
    void addTranslation( const Vec2<T> &t ) { m02 += t.x; m12 += t.y; }

    // Make a scale matrix
    void makeScale( const Vec2<T> &t )
    {
        m00 = t.x; m01 = T0; m02 = T0;
        m10 = T0; m11 = t.y; m12 = T0;
        m20 = T0; m21 = T0; m22 = T1;
    }
...
//******************************************************************************
template <typename T>
inline Mat33<T> Mat33<T>::transposed() const
{
    return Mat33<T>( m00, m10, m20,
                     m01, m11, m21,
                     m02, m12, m22 );
}
...
{
    return determinant3x3( m00, m01, m02,
                           m10, m11, m12,
                           m20, m21, m22 );
}
...
//******************************************************************************
template <typename T>
inline Mat33<T> Mat33<T>::inverted() const
{
    Mat33<T> ret( *this );
    ret.invert();
    return ret;
...
//******************************************************************************
template <typename T>
inline void Mat33<T>::makeRotation( const T &ang )
{
    const T cosa = Math<T>::cos( ang );
    const T sina = Math<T>::sin( ang );

    m00 = cosa; m01 = -sina; m02 = T0;
    m10 = sina; m11 = cosa;  m12 = T0;
    m20 = T0;   m21 = T0;    m22 = T1;
}

//******************************************************************************
template <typename T>
Vec2<T> Mat33<T>::transform( const Vec2<T> &v ) const
{
    if ( isAffine() )
    {
...
//******************************************************************************
template <typename T>
inline Mat33<T> &Mat33<T>::operator*=( const T &v )
{
    m00 *= v;
    m01 *= v;
    m02 *= v;
...
//******************************************************************************
template <typename T>
Mat33<T> &Mat33<T>::operator*=( const Mat33<T> &v )
{
...
//******************************************************************************
template <typename T>
inline typename Mat33<T>::size_type 
rowDimension( const Mat33<T> & )
{
    return 3;
}
...
//******************************************************************************
template <typename T>
inline void makeIdentity( Mat33<T> &m )
{
    m.makeIdentity();
}
...
//******************************************************************************
template <typename T>
inline void transpose( Mat33<T> &m )
{
    m.transpose();
}
...
//******************************************************************************
template <typename T>
inline Mat33<T> transposed( const Mat33<T> &m )
{
    return m.transposed();
}
...
//******************************************************************************
template <typename T>
inline T determinant( const Mat33<T> &m )
{
...
//******************************************************************************
template <typename T>
inline Mat33<T> inverted( const Mat33<T> &m )
{
    return m.inverted();
}
...
//******************************************************************************
template <typename T>
inline bool isAffine( const Mat33<T> &m )
{
    return m.isAffine();
}
...
//******************************************************************************
template <typename T>
inline Mat33<T> translationMatrix( const Vec2<T> &t )
{
    Mat33<T> ret( ( void * )1 );
...
//******************************************************************************
template <typename T>
inline void makeScale( Mat33<T> &m, const Vec2<T> &s )
{
    m.makeScale( s );
}
...
//******************************************************************************
template <typename T>
inline Mat33<T> scaleMatrix( const Vec2<T> &s )
{
    Mat33<T> ret( ( void * )1 );
    ret.makeScale( s );
    return ret;
...
template <typename T>
inline Mat33<T> operator+( const Mat33<T> &a,
                           const Mat33<T> &b )
{
    return Mat33<T>( a.m00 + b.m00,
                     a.m01 + b.m01,
                     a.m02 + b.m02,
...
inline Mat33<T> operator-( const Mat33<T> &a )
{
    return Mat33<T>( -a.m00, -a.m01, -a.m02,
                     -a.m10, -a.m11, -a.m12,
                     -a.m20, -a.m21, -a.m22 );
}
...
//******************************************************************************
template <typename T>
inline Mat33<T> operator*( const T &a,
                           const Mat33<T> &b )
{
    return Mat33<T>( a * b.m00, a * b.m01, a * b.m02,
...
//******************************************************************************
template <typename T>
inline Vec3<T> operator*( const Mat33<T> &m,
                          const Vec3<T> &v )
{
                          ...
                          const Mat33<T> &m )
{
    ...
    v3 = v3 * m;
    return Vec2<T>( v3.x / v3.z,
                    v3.y / v3.z );
}

//******************************************************************************
template <typename T>
inline Vec2<T> operator*( const Mat33<T> &m,
                          const Vec2<T> &v )
{
    Vec3<T> v3( v.x, v.y, T1 );
    v3 = m * v3;
...
//******************************************************************************
template <typename T>
inline bool operator==( const Mat33<T> &a,
                        const Mat33<T> &b )
{
    return ( ( a.m00 == b.m00 ) &&
             ( a.m01 == b.m01 ) &&
...
{
    if (Math<T>::abs(M.m00) > tolerance ||
        Math<T>::abs(M.m01) > tolerance ||
...
template <typename T>
Mat33<T> orthonormalize(const Mat33<T>& M)
{
    //

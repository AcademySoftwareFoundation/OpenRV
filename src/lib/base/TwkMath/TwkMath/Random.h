...
template <typename T>
class QuickRandGenerator : public QuickRand
{
public:
    QuickRandGenerator() : QuickRand() {}
    QuickRandGenerator(unsigned long seed) : QuickRand(seed) {}
    T operator()() { return T(nextFloat()); }
};

template <> 
inline int 
QuickRandGenerator<int>::operator()() 
{ 
    return nextLong(); 
}
...
template <> 
inline unsigned int 
QuickRandGenerator<unsigned int>::operator()() 
{ 
    return nextULong(); 
}
...
template <>
inline char
QuickRandGenerator<char>::operator()() 
{
...
template <>
inline short
QuickRandGenerator<short>::operator()() 
{ 
...
QuickRandGenerator<unsigned short>::operator()() 
{ 
...
{
public:
    GaussRand(RANDGEN &rng): m_randGen(&rng), m_savedGauss(0), m_saved(false) {}
    GaussRand(RANDGEN *rng): m_randGen(rng),  m_savedGauss(0), m_saved(false) {}
...
{
public:
    GaussRandGenerator(RANDGEN& r) : GaussRand<RANDGEN>(&r) {}
    GaussRandGenerator(RANDGEN* r) : GaussRand<RANDGEN>(r) {}

...
public:
    ...
    { x = a[0]; y = a[1]; z = a[2]; w = a[2]; m_counter = 0; }
...
//******************************************************************************
template <typename T>
void Quaternion<T>::value( Mat44<T> &m ) const
{
    const T norm =  ( x * x )
        + ( y * y ) 
        + ( z * z ) 
    ...
    m( 3, 0 ) = m( 3, 1 ) = m( 3, 2 ) = 
        m( 0, 3 ) = m( 1, 3 ) = m( 2, 3 ) = T0;
    m( 3, 3 ) = T1;
}

//******************************************************************************
template <typename T>
void Quaternion<T>::setValue( const Mat44<T> &m )
{
...
//******************************************************************************
template <typename T>
void Quaternion<T>::setValue( const Vec3<T> &axis, 
                              const T &theta )
{
...
{
    const T len = Math<T>::sqrt( ( w * w ) + ( x * x ) + 
                                 ( y * y ) + ( z * z ) );
    if ( len != T0 )
    {
        x /= len;
...
//******************************************************************************
template <typename T>
inline void Quaternion<T>::counterNormalize()
{
...
Quaternion<T> &Quaternion<T>::operator*=( const Quaternion<T> &qr )
{
    ...
    w = ( ql.w * qr.w ) - ( ql.x * qr.x ) - 
        ( ql.y * qr.y ) - ( ql.z * qr.z );
    x = ( ql.w * qr.x ) + ( ql.x * qr.w ) + 
        ( ql.y * qr.z ) - ( ql.z * qr.y );
    y = ( ql.w * qr.y ) + ( ql.y * qr.w ) + 
        ( ql.z * qr.x ) - ( ql.x * qr.z );
    z = ( ql.w * qr.z ) + ( ql.z * qr.w ) + 
        ( ql.x * qr.y ) - ( ql.y * qr.x );
...
//******************************************************************************
template <typename T>
inline bool areEquivalent( const Quaternion<T> &q1,
                           const Quaternion<T> &q2,
                           const T &tolerance )
{
    const T t = Math<T>::sqr( q1.x - q2.x ) +
        Math<T>::sqr( q1.y - q2.y ) +
        Math<T>::sqr( q1.z - q2.z ) +
...
//******************************************************************************
template <typename T>
inline void invert( Quaternion<T> &q )
{
    q.invert();
}
...
//******************************************************************************
template <typename T>
inline Quaternion<T> inverted( const Quaternion<T> &q )
{
    return q.inverted();
}
...
//*****************************************************************************
template <typename T>
Quaternion<T> slerp( const Quaternion<T> &p, 
                     const Quaternion<T> &q, 
                     const T &alph )
{
    T cosOmega = ( p.x * q.x ) + ( p.y * q.y ) + 
        ( p.z * q.z ) + ( p.w * q.w );
    ...
    return Quaternion<T>( ( beta * p.x ) + ( alpha * q.x ),
                          ( beta * p.y ) + ( alpha * q.y ),
                          ( beta * p.z ) + ( alpha * q.z ),
                          ( beta * p.w ) + ( alpha * q.w ) );
}
...
//*****************************************************************************
template <typename T>
void Quaternion<T>::setValue( const Vec3<T> &fromLook, 
                              const Vec3<T> &fromUp,
                              const Vec3<T> &toLook, 
                              const Vec3<T> &toUp )
{
...
//******************************************************************************
template <typename T>
inline bool operator==( const Quaternion<T> &q1,
                        const Quaternion<T> &q2 )
{
    return ( ( q1.x == q2.x ) &&
             ( q1.y == q2.y ) &&

...
namespace TwkMath
{
    ...
    template <typename T> inline void Plane<T>::set(const typename Plane<T>::Point& p, const typename Plane<T>::Vec& v)
    {
        normal = v.normalized();
        distance = dot(p, normal);
    }

    template <typename T> inline Plane<T>::Plane(const typename Plane<T>::Point& p, const typename Plane<T>::Vec& v) { set(p, v); }

    template <typename T> void Plane<T>::set(typename Plane<T>::Point a, typename Plane<T>::Point b, typename Plane<T>::Point c)
    {
        //
    ...
    template <typename T>
    inline Plane<T>::Plane(const typename Plane<T>::Point& a, const typename Plane<T>::Point& b, const typename Plane<T>::Point& c)
    {
        set(a, b, c);
    }
    ...
    template <typename T> typename Plane<T>::Point Plane<T>::reflect(const typename Plane<T>::Point& p) const
    {
        return -2.0 * distanceTo(p) * normal + p;
    }

    template <typename T> inline typename Plane<T>::Point Plane<T>::nearestPointTo(const typename Plane<T>::Point& p) const
    {
    ...
    template <typename T> inline typename Plane<T>::Point Plane<T>::intersection(const typename Plane<T>::LineT& line) const
    {
        const T u = intersectionU(line);
        return line(u);
    }

    template <typename T> inline typename Plane<T>::LineT Plane<T>::intersection(const Plane<T>& plane) const
    {

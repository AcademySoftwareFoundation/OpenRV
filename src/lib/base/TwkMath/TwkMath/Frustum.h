...
public:
    ...
    Frustum(value_type left, value_type right,
	    value_type bottom,  value_type top,
	    value_type nearPlane, value_type farPlane,
	    bool ortho = false);

    Frustum(value_type fovy, value_type aspect,
	    value_type nearPlane, value_type farPlane);
    //
    //  Member access
    //
    ...
    void window(value_type left,   value_type right,
		value_type bottom, value_type top,
		value_type nearPlane,   value_type farPlane,
		bool ortho=false);
    ...
    Frustum<T> crop(value_type left,   value_type right,
                    value_type bottom, value_type top) const;
...
template <typename T>
inline
Frustum<T>::Frustum(T left, T right, T bottom, T top, T nearPlane, T farPlane, bool ortho)
{
...
template <typename T>
inline
Frustum<T>::Frustum(T fovy, T aspect, T nearPlane, T farPlane)
{
...
template <typename T>
inline void
Frustum<T>::window(T left, T right, T bottom, T top, T nearPlane, T farPlane, bool ortho)
{
    m_left   = left;
    m_right  = right;
    m_bottom = bottom;
...
template <typename T>
inline void
Frustum<T>::perspective(T fovy, T aspect, T nearPlane, T farPlane)
{
...
template <typename T>
inline void
Frustum<T>::RIBperspective(T fov, T aspect, T nearPlane, T farPlane)
{
...
template <typename T>
void
Frustum<T>::adjustAspectHorizontally(T newAspect)
{
    //T w = m_right - m_left;
...
template <typename T>
Vec2<T>
Frustum<T>::fromScreen(const Vec2<T> &v)
{
...
template <typename T>
Vec2<T>
Frustum<T>::toScreen(const Vec2<T> &v)
{
    ...
    return vec2((v.x - copX) / copX,
                (v.y - copY) / copY);
}

template <typename T>
Frustum<T>
Frustum<T>::crop(T left, T right, T bottom, T top) const
{
    T w = m_right - m_left;
    T h = m_top - m_bottom;
    left = w * left + m_left;
...
Frustum<T>::matrix() const
{
    ...
    if (m_ortho)
    {
        ...
        return matrix_type( A,  0,  0,  -rpl / rml,
                            0,  B,  0,  -tpb / tmb,
                            0,  0,  C,  -fpn / fmn,
                            0,  0,  0,  T(1) );
    }
    else
    {
        ...
        return matrix_type( n2 / rml, 0,        rpl / rml,  0,
                            0,        n2 / tmb, tpb / tmb,  0,
                            0,        0,        -fpn / fmn, (-n2 * m_far) / fmn,
                            0,        0,        T(-1),      0);
    }
}

...
class Line
{
  public:
    ...
    Line() : direction(0), origin(0)   {}
    Line(const Line<T>& l) : direction(l.direction), origin(l.origin) {}
...
template <class T>
inline 
Line<T>::Line(const typename Line<T>::Point &p1,
...
template <class T>
inline bool
Line<T>::isParallelTo(const Line<T>& line) const
{
...
template <class T>
inline T 
Line<T>::nearestPointTo(const typename Line<T>::Point &p) const
{
...
template <class T>
inline typename T::value_type
Line<T>::nearestUTo(const Line<T> &line) const
{
    //
...
template <class T>
inline T
Line<T>::nearestPointTo(const Line<T> &line) const
{

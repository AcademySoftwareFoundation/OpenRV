...
inline VEC quadMap( const VEC &p00,
                    const VEC &p10,
                    const VEC &p11,
                    const VEC &p01,
                    VEC2 &uv )
{
    return lerp( lerp( p00, p10, uv[0] ),
                 lerp( p01, p11, uv[0] ), uv[1] );
}
...
struct BoolAndVec2
{
    BoolAndVec2() : status( false ), uv( ((T)0) ) {}
    BoolAndVec2( bool tf ) : status( tf ), uv( ((T)0) ) {}
    BoolAndVec2( bool tf, const Vec2<T> &v2 ) : status( tf ), uv( v2 ) {}
...
//-*****************************************************************************
template <typename T>
class UvStQuadInverter
{
public:
    UvStQuadInverter( const Vec2<T> &p00,
                      const Vec2<T> &p10,
                      const Vec2<T> &p11,
                      const Vec2<T> &p01 );
...
//-*****************************************************************************
template <typename T>
BoolAndVec2<T> quadInverseMap( const Vec2<T> &p00,
                               const Vec2<T> &p10,
                               const Vec2<T> &p11,
                               const Vec2<T> &p01,
                               const Vec2<T> &p );
...
//-*****************************************************************************
template <typename T>
void quadInverseMap( const Vec2<T> &p00,
                     const Vec2<T> &p10,
                     const Vec2<T> &p11,
                     const Vec2<T> &p01,
...
//-*****************************************************************************
template <typename T>
BoolAndVec2<T> quadInverseMap( const Vec3<T> &p00,
                               const Vec3<T> &p10,
                               const Vec3<T> &p11,
                               const Vec3<T> &p01,
                               const Vec3<T> &p );
...
//-*****************************************************************************
template <typename T>
void quadInverseMap( const Vec3<T> &p00,
                     const Vec3<T> &p10,
                     const Vec3<T> &p11,
                     const Vec3<T> &p01,
...
//-*****************************************************************************
template <typename T>
Vec2<T> quadClosestInverseMap( const Vec2<T> &p00,
                               const Vec2<T> &p10,
                               const Vec2<T> &p11,
                               const Vec2<T> &p01,
                               const Vec2<T> &p );
...
//-*****************************************************************************
template <typename T>
void quadClosestInverseMap( const Vec2<T> &p00,
                            const Vec2<T> &p10,
                            const Vec2<T> &p11,
                            const Vec2<T> &p01,
                            const std::vector<Vec2<T> > &p,
...
//-*****************************************************************************
template <typename T>
Vec2<T> quadClosestInverseMap( const Vec3<T> &p00,
                               const Vec3<T> &p10,
                               const Vec3<T> &p11,
                               const Vec3<T> &p01,
                               const Vec3<T> &p );
...
//-*****************************************************************************
template <typename T>
void quadClosestInverseMap( const Vec3<T> &p00,
                            const Vec3<T> &p10,
                            const Vec3<T> &p11,
                            const Vec3<T> &p01,
                            const std::vector<Vec3<T> > &p,
                                       ...
                                       const Vec2<T> &p01 )
{
    ...
    // Compute the bounds
    m_bounds.makeEmpty();
    m_bounds.min.x = std::min( std::min( s0, s1 ),
                               std::min( s2, s3 ) );
    m_bounds.max.x = std::max( std::max( s0, s1 ),
                               std::max( s2, s3 ) );
    m_bounds.min.y = std::min( std::min( t0, t1 ),
                               std::min( t2, t3 ) );
    m_bounds.max.y = std::max( std::max( t0, t1 ),
                               std::max( t2, t3 ) );
...
template <typename T>
BoolAndVec2<T>
UvStQuadInverter<T>::inverseMap( const Vec2<T> &st ) const
{
...
template <typename T>
Vec2<T>
UvStQuadInverter<T>::closestInverseMap( const Vec2<T> &st ) const
{
    ...
    // Check bounds of u, v
    return Vec2<T>( clamp( u, T0, T1 ),
                    clamp( v, T0, T1 ) );
}

//-*****************************************************************************
template <typename T>
inline void UvStQuadInverter<T>::inverseMap(
    const std::vector<Vec2<T> > &sts,
    std::vector<BoolAndVec2<T> > &uvs ) const
{
...
//-*****************************************************************************
template <typename T>
inline void UvStQuadInverter<T>::closestInverseMap(
    const std::vector<Vec2<T> > &sts,
    std::vector<Vec2<T> > &uvs ) const
{
    for ( size_t i = 0, s = sts.size(); i < s; ++i )
...
//-*****************************************************************************
template <typename T>
inline BoolAndVec2<T> quadInverseMap( const Vec2<T> &p00,
                                      const Vec2<T> &p10,
                                      const Vec2<T> &p11,
                                      const Vec2<T> &p01,
                                      const Vec2<T> &p )
...
//-*****************************************************************************
template <typename T>
inline void quadInverseMap( const Vec2<T> &p00,
                            const Vec2<T> &p10,
                            const Vec2<T> &p11,
                            const Vec2<T> &p01,
                            const std::vector<Vec2<T> > &p,
                     ...
                     std::vector<BoolAndVec2<T> > &uvs )
{
    ...
    Mat44<T> worldToPlane( v0[0], v0[1], v0[2], -dot( v0, p00 ),
                           v1[0], v1[1], v1[2], -dot( v1, p00 ),
                           n[0], n[1], n[2], -dot( n, p00 ),
                           T0, T0, T0, T1 );
...
//-*****************************************************************************
template <typename T>
inline BoolAndVec2<T> quadInverseMap( const Vec3<T> &p00,
                                      const Vec3<T> &p10,
                                      const Vec3<T> &p11,
                                      const Vec3<T> &p01,
                                      const Vec3<T> &p )
...
//-*****************************************************************************
template <typename T>
inline Vec2<T> quadClosestInverseMap( const Vec2<T> &p00,
                                      const Vec2<T> &p10,
                                      const Vec2<T> &p11,
                                      const Vec2<T> &p01,
                                      const Vec2<T> &p )
...
//-*****************************************************************************
template <typename T>
inline void quadClosestInverseMap( const Vec2<T> &p00,
                                   const Vec2<T> &p10,
                                   const Vec2<T> &p11,
                                   const Vec2<T> &p01,
                                   const std::vector<Vec2<T> > &p,
...
//-*****************************************************************************
template <typename T>
inline Vec2<T> quadClosestInverseMap( const Vec3<T> &p00,
                                      const Vec3<T> &p10,
                                      const Vec3<T> &p11,
                                      const Vec3<T> &p01,
                                      const Vec3<T> &p )

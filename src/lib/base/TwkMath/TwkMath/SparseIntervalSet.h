...
template <class T>
bool
SparseIntervalSet<T>::includes(const T& value) const
{
    value_type iv(value, value);
    const_iterator i = lower_bound(m_container.begin(),
                                   m_container.end(),
                                   iv,
                                   Comp<T>());
...
template <class T>
void
SparseIntervalSet<T>::insert(const T& value)
{
    ...
    else
    {
        ...
        iterator i = lower_bound(m_container.begin(),
                                 m_container.end(),
                                 iv,
                                 Comp<T>());
        ...
        for (iterator i = m_container.begin();
             (i + 1) < m_container.end();
             )
        {
            iterator j = i + 1;

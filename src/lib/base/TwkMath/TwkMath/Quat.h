...
class Quat
{
  public:
    ...
    Quat() : s(T(0)), v(T(0)) {}
    Quat(Scalar sc) : s(sc), v(T(0)) {}
    Quat(Scalar sc, Vec ve) : s(sc), v(ve) {}
    Quat(Scalar sc, Scalar i, Scalar j, Scalar k) : s(sc), v(i,j,k) {}

    template <typename S> Quat(const Mat33<S> &);
    template <typename S> Quat(Quat<S> q) : s(q.s), v(q.v) {}
    template <typename S> explicit Quat(const Vec4<S> &a) : s(a.x), v(a.y, a.z, a.w) {}
...
template <typename S>
inline Quat<T>&
Quat<T>::operator+=(const Quat<S> &a)
{
    s += a.s;
    v += a.v;
    return *this;
...
inline Quat<T> 
operator+(const Quat<T> &a, const Quat<S> &b)
{
...
template <typename T, typename S>
inline Vec3<T>
operator* (const Quat<T> &q, const Vec3<S> v)
{
    //
...
template <typename T>
inline T
magnitudeSquared(const Quat<T> &a)
{
...
template <typename T>
inline void
Quat<T>::setAxisAngle(const typename Quat<T>::Vec &axis, T radians)
{
...
template <typename T>
void
Quat<T>::rotateVector(const Vec3<T>& from, const Vec3<T>& to)
{
...
template <typename T, typename S>
inline T
dot(const Quat<T>& a, const Quat<S>& b)
{
...
template <typename T, typename S>
Quat<T> 
slerp( const Quat<T> &q1, const Quat<T> &q2, const S &t )
{

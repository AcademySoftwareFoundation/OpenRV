...
template <typename T>
inline Box<Vec3<T> >
operator*( const Mat44<T> &M, const Box<Vec3<T> > &box )
{
...
bool boxRayIntersection( const TwkMath::Box<VEC> &box,
                         const VEC &origin,
                         const VEC &dir,
                         T &resultT,
                         VEC &result )
...
template <class VEC>
inline bool boxRayIntersect( const Box<VEC> &box,
                             const VEC &origin,
                             const VEC &dir )
{
    static VEC dummy;
...
inline bool boxSegmentIntersection( const Box<VEC> &box,
                                    const VEC &origin,
                                    const VEC &dir,
                                    const T &tMin,
                                    const T &tMax,
...
// It's from Andrew Woo's Graphics Gem.
template <class VEC, class T>
inline bool boxSegmentIntersect( const Box<VEC> &box,
                                 const VEC &origin,
                                 const VEC &dir,
                                 const T &tMin,
                                 const T &tMax )
{
    static VEC dummy;
    static T dummyT;
    return boxSegmentIntersection( box, origin, dir, tMin, tMax,
                                   dummyT, dummy );
}

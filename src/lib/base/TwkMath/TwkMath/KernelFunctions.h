...
inline Scalar W_spiky(Scalar r, Scalar h)
{
    const Scalar x     = r / h;
...
template <typename Scalar>
inline Scalar dW_spiky(Scalar r, Scalar h)
{
    const Scalar h4 = h * Scalar(4);
    const Scalar hcube = h4 * h4 * h4;
    const Scalar x = h - r;
...
template <typename Scalar>
inline Scalar W_beta(Scalar r, Scalar h)
{
...
template <typename Scalar>
inline Scalar dW_beta(Scalar r, Scalar h)
{
    if (h <= std::numeric_limits<Scalar>::epsilon()) return Scalar(0);
    const Scalar x  = r / h;
    const Scalar W0 = Scalar(4) * Math<Scalar>::pi() * h * h * h;

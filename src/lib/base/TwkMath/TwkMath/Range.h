...
public:
    ...
    Range() : min( Math<T>::max() ), max( Math<T>::min() ) {}

    // Single value constructor, which sets both min and max to the
    // same value.
    explicit Range( const T &val ) : min( val ), max( val ) {}
    ...
    Range( const Range<T2> &copy ) : min( copy.min ), max( copy.max ) {}

    // Same type copy constructor
    Range( const Range<T> &copy ) : min( copy.min ), max( copy.max ) {}
    ...
    template <typename T2>
    Range<T> &operator=( const Range<T2> &copy )
    {
        min = copy.min;
        max = copy.max;
        return *this;
    ...
    //**************************************************************************
    void set( const T &mn, const T &mx ){ min = mn; max = mx; }
    ...
    //**************************************************************************
    // Arithmetic operations between ranges and other ranges are not defined.
    // Though the actual +-* / are fairly consistent,
    // it is not possible to globally decide for comparison operators
    // whether to use interval arithmetic & inclusion functions or to
    // compare by element. Therefore, we reserve the "interval" class for
    // interval arithmetic and specifically forbid arithmetic operations
    // between ranges.
    // Arithmetic operations between ranges and scalars applies the operation
    // evenly across min & max, reordering afterwards if necessary.
...
//******************************************************************************
template <typename T>
inline Range<T> &Range<T>::operator=( const T &val )
{
    min = val;
    max = val;
    return *this;
...
//******************************************************************************
template <typename T>
inline bool Range<T>::isInfinite() const
{
...
//******************************************************************************
template <typename T>
inline void Range<T>::makeInfinite()
{
    min = Math<T>::min();
    max = Math<T>::max();
}
...
//*****************************************************************************
template <typename T>
void Range<T>::extendBy( const T &v )
{
    if ( isInfinite() )
    {
        return;
...
//*****************************************************************************
template <typename T>
void Range<T>::extendBy( const Range<T> &b )
{
...
//*****************************************************************************
template <typename T>
bool Range<T>::intersects( const T &v ) const
{
...
//*****************************************************************************
template <typename T>
bool Range<T>::intersects( const Range<T> &b ) const
{
...
//******************************************************************************
template <typename T>
inline Range<T> &Range<T>::operator+=( const T &rhs )
{
    if ( !isEmpty() && !isInfinite() )
    {
        min += rhs;
...
//******************************************************************************
template <typename T>
inline T center( const Range<T> &r )
{
    return r.center();
}
...
//******************************************************************************
template <typename T>
inline bool isEmpty( const Range<T> &r )
{
    return r.isEmpty();
}
...
//******************************************************************************
template <typename T>
inline void makeEmpty( Range<T> &r )
{
    r.makeEmpty();
}
...
//******************************************************************************
template <typename T>
inline bool isInfinite( const Range<T> &r )
{
    return r.isInfinite();
}
...
//******************************************************************************
template <typename T>
inline void makeInfinite( Range<T> &r )
{
    r.makeInfinite();
}
...
//******************************************************************************
template <typename T>
inline void extendBy( Range<T> &r, const T &v )
{
    r.extendBy( v );
}
...
//******************************************************************************
template <typename T>
inline void extendBy( Range<T> &r, const Range<T> &r2 )
{
    r.extendBy( r2 );
}
...
//******************************************************************************
template <typename T>
inline bool intersects( const Range<T> &r, const T &v )
{
...
inline Range<T> intersection( const Range<T> &a, const Range<T> &b )
{
    if ( a.intersects( b ) )
    {
        return Range<T>( STD_MAX( a.min, b.min ),
                         STD_MIN( a.max, b.max ) );
    }
    else
    {

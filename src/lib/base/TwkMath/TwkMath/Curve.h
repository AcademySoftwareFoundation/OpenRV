...
inline VAL hermite( const VAL &vn1,
                    const VAL &v0,
                    const VAL &v1,
                    const VAL &v2,
                    const INTERP &t )
...
template <class VAL, class INTERP>
inline VAL hermiteBegin( const VAL &v0,
                         const VAL &v1,
                         const VAL &v2,
                         const INTERP &t )
{
...
template <class VAL, class INTERP>
inline VAL hermiteEnd( const VAL &vn1,
                       const VAL &v0,
                       const VAL &v1,
                       const INTERP &t )
{
             ...
             const INTERP &t )
{
    ...
    {
        return hermiteBegin( vals[0],
                             vals[1],
                             vals[2], t );
    }
    else if ( t >= (( INTERP )( numVals - 2 )) )
    {
        return hermiteEnd( vals[numVals-3],
                           vals[numVals-2],
                           vals[numVals-1],
                           t - ( numVals - 2 ) );
    }
...
template <class T>
Vec2<T> bezierCurve2D (const Vec2<T>& p0,
                       const Vec2<T>& p1,
                       const Vec2<T>& p2,
                       const Vec2<T>& p3,
                       const T t)
...
template <class T>
Vec2<T> bezierSamplingTangent2D (const Vec2<T>& p0,
                                 const Vec2<T>& p1,
                                 const Vec2<T>& p2)
{

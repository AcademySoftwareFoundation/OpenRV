...
public:
    ...
    Box() : min( Math<value_type>::max() ), max( Math<value_type>::min() ) {}

    // Single value constructor, which sets all values of min and max to
    // the value
    explicit Box( const value_type &val ) : min( val ), max( val ) {}

    // Single vector constructor, which sets min & max to that vector.
    explicit Box( const VEC &vec ) : min( vec ), max( vec ) {}

    // Two vector constructor, which sets min & max directly
    Box( const VEC &mn, const VEC &mx ) : min( mn ), max( mx ) {}

    // Non-similar type copy constructor
    template <class VEC2>
    Box( const Box<VEC2> &copy ) : min( VEC2(copy.min) ), max( VEC2(copy.max) ){}

    // Same type copy constructor
    explicit Box( const Box<VEC> &copy ) : min( copy.min ), max( copy.max ) {}
    ...
    Box<VEC> &operator=( const Box<VEC2> &copy )
    {
        min = copy.min;
        max = copy.max;
        return *this;
    ...
    //**************************************************************************
    void set( const VEC &mn, const VEC &mx ){ min = mn; max = mx; }
...
//******************************************************************************
template <class VEC>
inline Box<VEC> &Box<VEC>::operator=( const typename Box<VEC>::value_type &val )
{
    min = val;
    max = val;
    return *this;
...
//******************************************************************************
template <class VEC>
inline Box<VEC> &Box<VEC>::operator=( const VEC &vec )
{
    min = vec;
    max = vec;
    return *this;
...
//******************************************************************************
template <class VEC>
inline bool Box<VEC>::isEmpty() const
{
...
//******************************************************************************
template <class VEC>
inline bool Box<VEC>::isInfinite() const
{
    for ( size_type i = 0; i < dimension(); ++i )
    {
...
//******************************************************************************
template <class VEC>
inline void Box<VEC>::makeInfinite()
{
...
//******************************************************************************
template <class VEC>
size_t Box<VEC>::majorAxis() const
{
    VEC d = max - min;
    size_t dim = 0;
...
//******************************************************************************
template <class VEC>
size_t Box<VEC>::minorAxis() const
{
    VEC d = max - min;
    size_t dim = 0;
    value_type v = Math<value_type>::abs(d[0]);
...
//******************************************************************************
template <class VEC>
inline VEC Box<VEC>::center() const
{
...
template <class VEC>
inline typename 
Box<VEC>::value_type Box<VEC>::center( typename Box<VEC>::size_type i ) const
{
    if ( ( min[i] > max[i] ) ||
...
//******************************************************************************
template <class VEC>
void Box<VEC>::extendBy( const VEC &v )
{
    if ( isInfinite() )
    {
        return;
...
//******************************************************************************
template <class VEC>
void Box<VEC>::extendBy( const Box<VEC> &b )
{
...
//******************************************************************************
template <class VEC>
bool Box<VEC>::intersects( const VEC &v ) const
{
    for ( size_type i = 0; i < dimension(); ++i )
    {
...
//******************************************************************************
template <class VEC>
bool Box<VEC>::intersects( const Box<VEC> &b ) const
{
...
//******************************************************************************
template <class VEC>
Box<VEC> &Box<VEC>::operator+=( const VEC &rhs )
{
    if ( !isEmpty() && !isInfinite() )
    {
        min += rhs;
...
//******************************************************************************
template <class VEC>
inline VEC size( const Box<VEC> &b )
{
    return b.size();
}
...
template <class VEC>
inline typename 
Box<VEC>::value_type size( const Box<VEC> &b, typename Box<VEC>::size_type i )
{
    return b.size( i );
}
...
//******************************************************************************
template <class VEC>
inline VEC center( const Box<VEC> &b )
{
    return b.center();
}
...
template <class VEC>
inline typename 
Box<VEC>::value_type center( const Box<VEC> &b, 
...
//******************************************************************************
template <class VEC>
inline bool isEmpty( const Box<VEC> &b )
{
    return b.isEmpty();
}
...
//******************************************************************************
template <class VEC>
inline void makeEmpty( Box<VEC> &b )
{
    b.makeEmpty();
}
...
//******************************************************************************
template <class VEC>
inline bool isInfinite( const Box<VEC> &b )
{
    return b.isInfinite();
}
...
//******************************************************************************
template <class VEC>
inline void makeInfinite( Box<VEC> &b )
{
    b.makeInfinite();
}
...
//******************************************************************************
template <class VEC>
inline void extendBy( Box<VEC> &b, const VEC &v )
{
    b.extendBy( v );
}
...
//******************************************************************************
template <class VEC>
inline void extendBy( Box<VEC> &b, const Box<VEC> &b2 )
{
    b.extendBy( b2 );
}
...
//******************************************************************************
template <class VEC>
inline bool intersects( const Box<VEC> &b, const VEC &v )
{
...
inline Box<VEC> intersection( const Box<VEC> &a, const Box<VEC> &b )
{
    if ( a.intersects( b ) )
    {
        VEC min;
        VEC max;
        for ( typename Box<VEC>::size_type i = 0;
	      i < Box<VEC>::dimension();
	      ++i )
        {
...
//******************************************************************************
template <class VEC>
Box<VEC> operator+( const typename Box<VEC>::value_type &a, const Box<VEC> &b )
{
...
//******************************************************************************
template <class VEC>
Box<VEC> operator+( const VEC &a, const Box<VEC> &b )
{
    if ( b.isEmpty() || b.isInfinite() )
    {
        return b;
...
//******************************************************************************
template <class VEC>
Box<VEC> operator+( const Box<VEC> &a, const VEC &b )
{
    if ( a.isEmpty() || a.isInfinite() )
    {
        return a;
...
//******************************************************************************
template <class VEC>
Box<VEC> operator-( const Box<VEC> &a, const typename Box<VEC>::value_type &b )
{
    if ( a.isEmpty() || a.isInfinite() )
    {
        return a;

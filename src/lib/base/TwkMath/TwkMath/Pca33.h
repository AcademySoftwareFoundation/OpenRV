...
// FORCE ORTHOGONAL
template <typename T>
void forceOrthogonal( Vec3<T> vecs[] )
{
    // Keep 0 as is.
    vecs[0].normalize();
...
template <typename T>
size_t realCubicRoots( T a0, T a1, T a2, T roots[] )
{
    T Q = ( (T3*a1) - (a2*a2) ) / ((T)9);
...
template <typename T>
Vec3<T> findUnitEigenVector( T A, T B, T C,
                             T D, T E, T F,
                             T Y )
{
...
// F = sum( ( zi - zMean )^2 )
template <typename T>
void eigenAnalysisOf3x3CovarianceMatrix( T A, T B, T C,
                                         T D, T E, T F,
                                         Vec3<T> vecs[],
                                         T vals[] )
{
...
void principalComponentAnalysis3( ITER begin,
                                  ITER end,
                                  VEC &mean,
                                  VEC components[],
                                  TA vals[],
                                  TB dummy )
{
    ...
    Vec3<TB> evecs[3];
    TB evals[3];
    eigenAnalysisOf3x3CovarianceMatrix( mxx, mxy, mxz, myy, myz, mzz,
                                        evecs, evals );

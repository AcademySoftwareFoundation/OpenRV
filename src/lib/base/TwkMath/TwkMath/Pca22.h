...
// FORCE ORTHOGONAL
template <typename T>
void forceOrthogonal( Vec2<T> vecs[] )
{
    // Keep 0 as is.
    vecs[0].normalize();
...
template <typename T>
size_t realQuadraticRoots( T a0, T a1, T roots[] )
{
    // ( -b +- sqrt( b^2 - 4ac ) ) / 2a
...
template <typename T>
Vec2<T> findUnitEigenVector2( T A, T B,
                              T C, T Y )
{
    // Check degenerate:
    if ( ARE_EQUAL( A, Y ) )
...
// C = sum( ( yi - ymean )^2 )
template <typename T>
void eigenAnalysisOf2x2CovarianceMatrix( T A, T B, T C,
                                         Vec2<T> vecs[],
                                         T vals[] )
{
...
void principalComponentAnalysis2( ITER begin,
                                  ITER end,
                                  VEC &mean,
                                  VEC components[],
                                  TA vals[],
                                  TB dummy )
{
    ...
    Vec2<TB> evecs[2];
    TB evals[2];
    eigenAnalysisOf2x2CovarianceMatrix( mxx, mxy, myy,
                                        evecs, evals );

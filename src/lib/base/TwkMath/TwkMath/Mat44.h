...
public:
    ...
    Mat44() :
      m00( T1 ), m01( T0 ), m02( T0 ), m03( T0 ),
      m10( T0 ), m11( T1 ), m12( T0 ), m13( T0 ),
      m20( T0 ), m21( T0 ), m22( T1 ), m23( T0 ),
      m30( T0 ), m31( T0 ), m32( T0 ), m33( T1 ) {}
    ...
    // Specialization of above function
    template <typename S>
    Mat44<T> &operator=( const Mat44<S> &copy )
    {
        m00 = copy.m00; m01 = copy.m01; m02 = copy.m02; m03 = copy.m03;
        m10 = copy.m10; m11 = copy.m11; m12 = copy.m12; m13 = copy.m13;
        m20 = copy.m20; m21 = copy.m21; m22 = copy.m22; m23 = copy.m23;
    ...
    bool isAffine() const
    { return ( ( m33 == T1 ) &&
               ( m32 == T0 ) && ( m31 == T0 ) && ( m20 == T0 ) ); }

    // Create a translation matrix.
    template <typename S>
    void makeTranslation( const Vec3<S> &t )
    {
        m00 = T1; m01 = T0; m02 = T0; m03 = t.x;
        m10 = T0; m11 = T1; m12 = T0; m13 = t.y;
        m20 = T0; m21 = T0; m22 = T1; m23 = t.z;
        m30 = T0; m31 = T0; m32 = T0; m33 = T1;
    }

    // Modify translations of matrix
    template <typename S>
    void setTranslation( const Vec3<S> &t )
    { m03 = t.x; m13 = t.y; m23 = t.z; }

    template <typename S>
    void addTranslation( const Vec3<S> &t )
    { m03 += t.x; m13 += t.y; m23 += t.z; }

    // Make a scale matrix
    template <typename S>
    void makeScale( const Vec3<S> &t )
    {
        m00 = t.x; m01 = T0; m02 = T0; m03 = T0;
        m10 = T0; m11 = t.y; m12 = T0; m13 = T0;
        m20 = T0; m21 = T0; m22 = t.z; m23 = T0;
        m30 = T0; m31 = T0; m32 = T0; m33 = T1;
    }

    // Create rotation matrix
    template <typename S>
    void makeRotation( const Vec3<S> &axis,
    ...
    template <typename S>
    Vec3<S> transformDir( const Vec3<S> &v ) const;
...
{
    return Mat44<T>( m00, m10, m20, m30,
                     m01, m11, m21, m31,
                     m02, m12, m22, m32,
                     m03, m13, m23, m33 );
}
...
template <typename T>
inline Mat44<T> Mat44<T>::inverted() const
{
    Mat44<T> ret( *this );
    ret.invert();
    return ret;
...
template <typename T>
inline typename Mat44<T>::size_type 
rowDimension( const Mat44<T> & )
{
    return 4;
}
...
template <typename T>
inline void makeIdentity( Mat44<T> &m )
{
    m.makeIdentity();
}
...
template <typename T>
inline void transpose( Mat44<T> &m )
{
    m.transpose();
}
...
template <typename T>
inline Mat44<T> transposed( const Mat44<T> &m )
{
    return m.transposed();
}
...
template <typename T>
inline T determinant( const Mat44<T> &m )
{
...
template <typename T>
inline Mat44<T> invert( const Mat44<T> &m )
{
    return m.inverted();
}
...
template <typename T>
inline bool isAffine( const Mat44<T> &m )
{
    return m.isAffine();
}
...
template <typename T, typename S>
inline void makeRotation( Mat44<T> &m,
                          const Vec3<S> &axis, const T &angleInRadians )
{
...
template <typename T>
inline Mat44<T> operator+( const Mat44<T> &a,
                           const Mat44<T> &b )
{
    return Mat44<T>( a.m00 + b.m00,
                     a.m01 + b.m01,
                     a.m02 + b.m02,
                     a.m03 + b.m03,
...
inline Mat44<T> operator-( const Mat44<T> &a )
{
    return Mat44<T>( -a.m00, -a.m01, -a.m02, -a.m03,
...
template <typename T, typename S>
inline Mat44<T> operator*( const S &a,
                           const Mat44<T> &b )
{
    return Mat44<T>( a * b.m00, a * b.m01, a * b.m02, a * b.m03,
...
template <typename T, typename S>
inline Vec4<S> operator*( const Vec4<S> &v,
                          const Mat44<T> &m )
{
    return Vec4<S>(
...
template <typename T, typename S>
inline Vec4<S> operator*( const Mat44<T> &m,
                          const Vec4<S> &v )
{
    return Vec4<S>(
...
// Either direction.
template <typename T, typename S>
inline Vec3<S> operator*( const Vec3<S> &v,
                          const Mat44<T> &m )
{
    Vec4<S> v4( v.x, v.y, v.z, T1 );
    v4 = v4 * m;
    return Vec3<S>( v4.x / v4.w,
                    v4.y / v4.w,
                    v4.z / v4.w );
}
...
template <typename T, typename S>
inline Vec3<S> operator*( const Mat44<T> &m,
                          const Vec3<S> &v )
{
    Vec4<S> v4( v.x, v.y, v.z, T1 );
    v4 = m * v4;
...
template <typename T>
inline bool operator==( const Mat44<T> &a,
                        const Mat44<T> &b )
{
    return ( ( a.m00 == b.m00 ) &&
             ( a.m01 == b.m01 ) &&

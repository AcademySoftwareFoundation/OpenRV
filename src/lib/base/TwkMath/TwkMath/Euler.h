...
public:
    ...
    static void		simpleXYZRotation (Vec &xyzRot,
					   const Vec &targetXyzRot);
    static void		nearestRotation (Vec &xyzRot,
					 const Vec &targetXyzRot,
					 Order order = XYZ);
...
template <typename T>
Euler<T>::Euler() : Vec(0,0,0), m_order(Euler::ZYX)
{
}

template <typename T>
Euler<T>::Euler(const Euler<T> &euler, Order order)
{
...
Euler<T>::Euler(const Vec3<T> &angles, Order order) 
    : Vec(angles), m_order(order)
{
}

template <typename T>
Euler<T>::Euler(const Mat44<T> &M, Order order)
{
    decompose(M, order);
}
...
template <typename T>
inline void
Euler<T>::indexOrder(int& i, int& j, int &k) const
{
    i = m_order >> 12;
...
Euler<T>::matrix44() const
{
    ...
    if (repeating())
    {
	return Mat4( cj,	sj * si,	sj * ci,	T(0),
		     sj * sh,	-cj * ss + cc,	-cj * cs - sc,	T(0),
		     -sj * ch,	cj * sc * cs,	cj * cc - ss,	T(0),
		     T(0),	T(0),		T(0),		T(1) );
    }
    else
    {
	return Mat4( cj * ch,	sj * sc - cs,	sj * cc + ss,	T(0),
		     cj * sh,	sj * ss + cc,	sj * cs - sc,	T(0),
		     -sj,	cj * si,	cj * ci,	T(0),
		     T(0),	T(0),		T(0),		T(1) );
    }
}
...
template <typename T>
void
Euler<T>::decompose(const Mat44<T> &M, Euler<T>::Order order)
{
    m_order = order;
    int i,j,k;
    indexOrder(i, j, k);
...
template <class T>
void
Euler<T>::simpleXYZRotation (Vec &xyzRot, const Vec &targetXyzRot)
{
...
template <class T>
void
Euler<T>::nearestRotation (Vec &xyzRot, const Vec &targetXyzRot,
			   Order order)
{
    int i,j,k;

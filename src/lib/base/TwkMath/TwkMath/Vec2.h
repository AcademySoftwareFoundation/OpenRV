...
public:
    ...
    // automatically.
    explicit Vec2( const T a[] ) : x( a[0] ), y( a[1] ) {}
    ...
    // Copy from a vec2 of any type. Must be in-class due to VisualC++ problems.
    template <class T2>
    Vec2<T> &operator=( const Vec2<T2> &copy )
    {
        x = copy.x;
        y = copy.y;
        return *this;
...
//******************************************************************************
template <typename T>
inline typename Vec2<T>::iterator Vec2<T>::begin()
{
...
//******************************************************************************
template <typename T>
inline T Vec2<T>::magnitudeSquared() const
{
    return (x*x) + (y*y);
}
...
//******************************************************************************
template <typename T>
inline size_t dimension( Vec2<T> )
{
    return 2;
}
...
//******************************************************************************
template <typename T>
inline size_t majorComponent( const Vec2<T> &v )
{
    return v.x > v.y ? 0 : 1;
}
...
//******************************************************************************
template <typename T>
inline size_t minorComponent( const Vec2<T> &v )
{
    return v.x < v.y ? 0 : 1;
}
...
//******************************************************************************
template <typename T>
inline T magnitudeSquared( const Vec2<T> &v )
{
...
template <typename T>
inline Vec2<T> crossUp(const Vec2<T> &a)
{
    return Vec2<T>(a.y, -a.x);
}

template <typename T>
inline Vec2<T> crossDown(const Vec2<T> &a)
{
    return Vec2<T>(-a.y, a.x);
}

//******************************************************************************
// Absolute value of the angle between two vectors. Never more than PI, never
// less than zero.
template <typename T>
inline T angleBetween( const Vec2<T> &a, const Vec2<T> &b )
{
    ...
    T d = dot ( an, bn );
    //
    //  Even after normalizing above, the dot() can produce a value > or < 1,
    //  which will give a NAN from acos(), so trim the dot to the accepted
    //  input range for acos.
    //
    if (d < T(-1)) d = T(-1);
    if (d > T( 1)) d = T( 1);
...
// between -PI to PI
template <typename T>
inline T turnAngle( const Vec2<T> &p0,
                    const Vec2<T> &p1,
                    const Vec2<T> &p2 )
{
    Vec2<T> a = p1 - p0;

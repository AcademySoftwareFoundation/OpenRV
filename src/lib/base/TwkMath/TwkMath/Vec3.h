...
public:
    ...
    template<typename S>
    Vec3<T> &operator=( const Vec3<S> &copy )
    {
        x = copy.x;
        y = copy.y;
        z = copy.z;
...
template <typename T>
inline typename Vec3<T>::iterator Vec3<T>::begin()
{
...
template <typename T>
inline T Vec3<T>::magnitudeSquared() const
{
    return (x*x) + (y*y) + (z*z);
}
...
template <typename T>
inline size_t dimension( Vec3<T> )
{
    return 3;
}
...
template<typename T>
inline size_t majorComponent( const Vec3<T> &q )
{
    Vec3<T> v = Vec3<T>(Math<T>::abs(q.x),
			Math<T>::abs(q.y),
			Math<T>::abs(q.z));
    if (v.x > v.y)
    {
...
template<typename T>
inline size_t minorComponent( const Vec3<T> &q )
{
    Vec3<T> v = Vec3<T>(Math<T>::abs(q.x),
			Math<T>::abs(q.y),
			Math<T>::abs(q.z));
...
template <typename T>
inline T magnitudeSquared( const Vec3<T> &v )
{

...
inline void sampleMean( const ARRAY2D &samples,
                        int numSamples,
                        int dimension,
                        ARRAY1D &mean,
                        T dummy )
...
void sampleCovarianceMatrix( const ARRAY2D_1 &samples,
                             int numSamples,
                             int dimension,
                             const ARRAY1D &mean,
                             ARRAY2D_2 &covarMat,
...
//******************************************************************************
template <class T>
inline T safeHypot( const T &a, const T &b )
{
    T absa = Math<T>::abs( a );
    T absb = Math<T>::abs( b );
    ...
    {
        return absb * 
            Math<T>::sqrt( (( T )1) + Math<T>::sqr( absa / absb ) );
    }
}
...
inline bool relativeUnderflowTest( const T &testVal, const T &anorm )
{
    ...
    const double added = Math<double>::abs( (double)tv ) + 
        (double)an;
...
// its singular value decomposition, A = U * W * (V^T). The matrix U replaces
...
//******************************************************************************
template <class ARRAY2D_1, class ARRAY2D_2, class ARRAY1D, class T>
void singularValueDecomposition( ARRAY2D_1 &A, 
                                 int M, 
                                 int N, 
                                 ARRAY1D &W, 
                                 ARRAY2D_2 &V,
    ...
    for( k = N - 1; k >= 0; k-- )
    {
        for( its = 0; its < MAX_SVD_ITERS; its++ )
        {
            ...
            for( q = k; q >= 0; q-- )
            {
                ...
                nm = q - 1;
                if( q == 0 ||
                    relativeUnderflowTest( rv1[q], anorm ) )
                {
                    splitFlag = false;
                    break;
                }
...
inline void jacobiRotate( T1 &a0,
                          T1 &a1,
                          const T2 &s,
                          const T2 &tau )
{
...
template <class ARRAY2D_1, class ARRAY2D_2, class ARRAY1D, class T>
void jacobiEigenSolve( ARRAY2D_1 &a, 
                       int N, 
                       ARRAY1D &d, 
                       ARRAY2D_2 &v, 
                       int &numRotations,
    ...
    {   
        ...
        for( ip = 0; ip < N - 1; ip++ ) 
        {
            for( iq = ip + 1; iq < N; iq++ ) 
            {
                ...
                if( numRotations > 3 && 
                    relativeUnderflowTest2( d[ip], ( T )g ) &&
...
//******************************************************************************
template <class ARRAY2D, class ARRAY1D, class T>
void eigenSort( ARRAY1D &eigenValues,
                ARRAY2D &eigenVectorsTransposed,
                int dimension,
                T dummy )
{
    ...
    for ( vec = 0; vec < dimension; vec++ )
    {
        ...
        {
            (*tempMatIter) = 
                eigenVectorsTransposed[row][pairs[vec].index];
        }
    }

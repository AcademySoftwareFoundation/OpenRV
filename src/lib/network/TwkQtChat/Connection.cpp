...
#define DBL(level, x)   if (level & DB_LEVEL) cerr << "Connection: " << x << endl
#else
#define DB(x)    
#define DBL(level, x)
#endif
...
Connection::Connection(QObject *parent, bool pingpong) : QTcpSocket(parent)
{
    ...
    if (first)
    {
        ...
        if (getenv("RV_CONNECTION_PONG_TIMEOUT"))
        {
            ...
            if (i <= 0) m_pingpong  = false;
            else        PongTimeout = 1000*i;
            print = true;
        }
        if (getenv("RV_CONNECTION_PING_INTERVAL"))
        {
            ...
            if (i <= 0) m_pingpong   = false;
            else        PingInterval = 1000*i;
            print = true;
        }
        if (getenv("RV_CONNECTION_TRANSFERTIMEOUT"))
        {
            int i = atoi(getenv("RV_CONNECTION_TRANSFERTIMEOUT"));
            if (i > 0) TransferTimeout = 1000*i;
            print = true;
        }

        if (print) cerr << "INFO: Connection timing: pingpong " << m_pingpong << " pongtimeout " << PongTimeout 
                << " pinginterval " << PingInterval << " transtimeout " << TransferTimeout << endl;
...
void 
Connection::connectToContact(const QString& name,
...
const QHostAddress& 
Connection::expectedAddress() const
{
    return m_expectedAddress;
}

const QString&
Connection::expectedContactName() const
{
    return m_expectedContactName;
}

const QString&
Connection::remoteName() const
{
    return m_remoteName;
}

const QString&
Connection::remoteApp() const
{
    return m_remoteApp;
}

const QString&
Connection::remoteContactName() const
{
    return m_remoteContactName;
}

void
Connection::setGreetingMessage(const QString &message, const QString &app)
{
...
bool
Connection::sendMessage(const QString &message)
{
...
bool
Connection::sendData(const QString& interp, const QByteArray& data)
{
    ...
    out += " ";
    out += data;
    DB ("send: '" << interp.toUtf8().data() << " " << data.size() << " " << "<data>'");
    return write(out) == out.size();
}

void
Connection::timerEvent(QTimerEvent *timerEvent)
{
...
Connection::processReadyRead()
{
        ...
        {
            if (isprint(m_buffer[i])) printable.append(m_buffer[i]);
        }
    ...
    if (m_state == ReadingGreeting) 
    {
        ...
        {
            cerr << "ERROR: connection aborted because greeting data size does not match" << endl;
            abort();
            return;
        }
        ...
        {
            m_remoteContactName = m_remoteName
                + "@" 
                + hostInfo.hostName();
        }
        else
        ...
        if (!m_isGreetingMessageSent) sendGreetingMessage();
...
int
Connection::dataLengthForCurrentDataType()
{
...
Connection::readProtocolHeader()
{
    ...
    if      (m_buffer == "PING ") m_currentDataType = Ping;
    else if (m_buffer == "PONG ") m_currentDataType = Pong;
    else if (m_buffer == "MESSAGE ") m_currentDataType = PlainText;
    else if (m_buffer == "GREETING ") m_currentDataType = Greeting;
    else if (m_buffer == "NEWGREETING ") m_currentDataType = NewGreeting;
    else if (m_buffer == "PINGPONGCONTROL ") m_currentDataType = PingPongControl;
    else 
    {

...
#define DBL(level, x)   if (level & DB_LEVEL) cerr << "Client: " << x << endl
#else
#define DB(x)
#define DBL(level, x)
#endif
...
{
    //
    //  Most clients (who only want to communicate with RV) have no need of a
    //  Server, since they will not be listening for connections.  They can use
    //  a port arg of 0.
    //
    if (port)
    {
        ...
        QObject::connect(m_server, SIGNAL(newConnection(Connection *)),
...
Client::~Client()
{
    ...
    for (int i=0; i < values.size(); i++) removeConnection(values[i]);
    delete m_server;
}

void
Client::broadcastMessage(const QString &message)
{
...
void
Client::sendMessage(const QString& who, const QString &message)
{
    if (message.isEmpty()) return;
    ...
    {
        if (c->remoteContactName() == who && c->isValid()) c->sendMessage(message);
    }
}

void
Client::sendData(const QString& who, const QString &interp, const QByteArray& data)
{
    if (interp.isEmpty() || data.isEmpty()) return;
...
void
Client::broadcastEvent(const QString& eventName, 
...
void
Client::sendEvent(const QString& who,
...
bool
Client::waitForMessage(const QString& who)
{
    QList<Connection *> connections = m_connectionMap.values();
...
bool
Client::waitForSend(const QString& who)
{
    QList<Connection *> connections = m_connectionMap.values();
...
QString
Client::identifierName() const
{
    QString name = QHostInfo::localHostName();
    if (name.endsWith(".local")) name.chop(6);
    return m_contactName + "@" + name;
}

bool
Client::hasConnection(const QHostAddress &senderIp, int senderPort) const
{
    if (senderPort == -1)
...
void
Client::connectTo(const QString& name, const QHostAddress& ip, int port)
{
    if (!hasConnection(ip, port)) 
    {
        Connection *c = 0;
        if (m_connectionFactory) c = (*m_connectionFactory)(this, true);
        else                     c = new Connection(this, true);
...
void
Client::connectTo(const QString& name, const QString& host, int port)
{
...
void
Client::newConnection(Connection *connection)
{
...
void 
Client::disconnectFrom(const QString& contact)
{
...
Connection*
Client::connectionByName(const QString& contact) const
{
    for (ConnectionMap::const_iterator i = m_connectionMap.constBegin();
         i != m_connectionMap.constEnd();
         ++i)
    {
        Connection* c = *i;
        if (c->remoteContactName() == contact) return c;
    }
...
void
Client::rejectConnection(Connection* c)
{
    c->abort();
...
Client::readyForUse()
{
    ...
    connect(connection, SIGNAL(newData(const QString&, const QString&,
                                       const QByteArray&)),
                               this, SIGNAL(newData(const QString&,
                                                    const QString&,
                                                    const QByteArray&)));

    m_connectionMap.insert(connection->peerAddress(), connection);
    QString contact = connection->remoteContactName();
    if (!m_pingpong) connection->pingPongControl(false);
    if (!contact.isEmpty()) emit newContact(contact);
}
...
void
Client::connectionError(QAbstractSocket::SocketError err)
{
    if (Connection *connection = qobject_cast<Connection *>(sender()))
    {
        ...
        //
        if (connection->incoming() && connection->state() == Connection::WaitingForGreeting) return;

        QString msg   = connection->errorString();
        QString cname = connection->remoteContactName();

        if (cname == "unknown") cname = connection->expectedContactName();

        emit contactError(cname,
                          //cname + "@" + connection->expectedAddress().toString(), 
                          connection->expectedAddress().toString(),
                          msg);
...
void
Client::removeConnection(Connection *connection)
{
...
void
Client::signOff (const QString& contact)
{

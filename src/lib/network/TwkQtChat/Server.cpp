    ...
    m_connectionFactory(fact)
{
    ...
    if (myPort) 
    {
        ...
        {
            cerr << "ERROR: RvNetwork could not listen to the network port " << port 
                 << " - " <<  errorString().toStdString() << endl;
        }
    }
    else
    {
...
Server::incomingConnection(qintptr socketDescriptor)
{
    ...
    if (m_connectionFactory) connection = (*m_connectionFactory)(this, true);
    else                     connection = new Connection(this, true);

...
{
    return buffer && 
           buffer[0] == CommandPrefix()[0] && 
           buffer[1] == CommandPrefix()[1];
}

StringPair 
parseMessage(const char* processName,
             ostream* log,
             const string& msg)
{
    ...
    if (log)
    {
        //(*log) << "[" << processName << "]:  got CMD = \"" << cmd << "\", ARG = \"" << arg << "\"" << endl;
    }
...
size_t
newMessage(const char* processName,
           ostream* log,
           const string& cmd,
           const string& arg,
           char* outBuffer)
{
    if (log)
    {
        //(*log) << "[" << processName << "]: send CMD = \"" << cmd << "\", ARG = \"" << arg << "\"" << endl;
    }
...
void
decodeStringVector(const string& buffer, TwkFB::FBInfo::StringVector& v)
{
    split(v, buffer, is_any_of(string(",")));
    if (v.size() == 1 && v[0] == "") v.clear();
}


string
encodeStringVector(const TwkFB::FBInfo::StringVector& v)
{
    ostringstream str;
...
string 
encodeMovieReadRequest(const Movie::ReadRequest& request)
{
    ostringstream str;
    ...
    str << request.frame 
        << "|" << request.stereo 
        << "|" << request.missing 
        << "|" << encodeStringVector(request.views)
...
void
decodeMovieReadRequest(const std::string& buffer, Movie::ReadRequest& request)
{
    vector<string> v;

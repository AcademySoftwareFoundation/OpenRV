...
void shutdownMovieSideCars(int sig)
{
    ...
    for (std::list<MovieSideCar*>::iterator it=movieSideCarsRunning.begin(); 
...
void
decodeChannelInfoVector(const string& buffer, TwkFB::FBInfo::ChannelInfoVector& v)
{
    vector<string> parts;
...
void
decodeLayerInfoVector(const string& buffer, TwkFB::FBInfo::LayerInfoVector& v)
{
    vector<string> parts;
    split(parts, buffer, is_any_of(string("`")));
...
void
decodeViewInfoVector(const string& buffer, TwkFB::FBInfo::ViewInfoVector& v)
{
    vector<string> parts;
    split(parts, buffer, is_any_of(string("|")));
...
void
decodeAudioChannelsVector(const string& buffer, TwkAudio::ChannelsVector& v)
{
    vector<string> parts;
    split(parts, buffer, is_any_of(string("|")));
...
Movie* 
MovieSideCar::clone() const
{
    if (!m_cloneable) return 0;
...
void 
MovieSideCar::sendCommand(const string& cmd, const string& args)
{
    ...
    for (bool valid = false; !valid; valid = Message::isCommand(m_buffer))
    {
        ...
        try
        {
            ...
            *m_buffer = 0;
            ...
            bool timedout = !m_responseQueue->timed_receive(m_buffer,
                                                            Message::SizeInBytes(),
                                                            returnSize,
                                                            priority,
                                                            timeout);
            if (returnSize == 0) m_buffer[0] = 0;
...
void
MovieSideCar::open(const string &filename, const MovieInfo &info,
    const Movie::ReadRequest& request)
{
    ...
    for (size_t tries=0; ; tries++)
    {
        try
        {
            m_commandQueue  = new Queue(interprocess::create_only,
                                        m_commandQueueName.c_str(),
                                        Message::MaxInQueue(),
                                        Message::SizeInBytes());
            
            m_responseQueue = new Queue(interprocess::create_only,
                                        m_responseQueueName.c_str(),
                                        Message::MaxInQueue(),
                                        Message::SizeInBytes());
    ...
    cmd << '"' << m_sidecarPath << '"' 
        << " --command " << m_commandQueueName
        << " --response " << m_responseQueueName
...

    {
        ...
        gtoreader.readImageInMemory(gfbs,
                                    m_imageRegion,
                                    m_imageRegionSize,
                                    m_filename,
                                    gfbrequest);
        ...
        for (size_t i = 0; i < gfbs.size(); i++) delete gfbs[i];
    }
...
bool 
MovieSideCar::canConvertAudioRate() const
{
    return m_canConvertAudio;
}

void 
MovieSideCar::imagesAtFrame(const ReadRequest& request, FrameBufferVector& fbs)
{
...
void
MovieSideCar::identifier(int frame, std::ostream& o)
{
...
void 
MovieSideCar::identifiersAtFrame(const ReadRequest& request, IdentifierVector& ids)
{
    ostringstream str;
...
size_t 
MovieSideCar::audioFillBuffer(const AudioReadRequest& request, AudioBuffer& buffer)
{
    ...
    buffer.ownData();
    buffer.reconfigure(num - 2 * request.margin, 
                       m_info.audioChannels,
                       m_canConvertAudio ? buffer.rate() : m_info.audioSampleRate,
                       request.startTime,
                       margin);
...
void 
MovieSideCar::audioConfigure(const AudioConfiguration& config)
...
#define PARSE_FIELD(INDEX,NAME) { istringstream istr(fields[INDEX]); istr >> FIELD(m_info,NAME); }
...
void
MovieSideCar::deserializeInfo(const string& info)
{
    vector<string> parts;
...
void
MovieSideCar::decodeMemoryInfo(const string& info)
{
    vector<string> parts;
    size_t imageOffet;
    size_t audioOffset;
    ...
    { istringstream istr(parts[0]); istr >> m_sharedObjectName; }
    { istringstream istr(parts[1]); istr >> imageOffet; }
    { istringstream istr(parts[2]); istr >> m_imageRegionSize; }
    { istringstream istr(parts[3]); istr >> audioOffset; }
    { istringstream istr(parts[4]); istr >> m_audioRegionSize; }

    m_sharedObject = new SharedMemory(interprocess::open_only, 
                                      m_sharedObjectName.c_str(),
                                      interprocess::read_only);

    m_sharedRegion = new MappedRegion(*m_sharedObject, 
                                      interprocess::read_only);

...
string 
MovieSideCarIO::about() const
{
    return "Tweak SideCar Bridge";
}

MovieReader*
MovieSideCarIO::movieReader() const
{
    return new MovieSideCar(m_sidecarPath, m_cloneable);
}

MovieWriter*
MovieSideCarIO::movieWriter() const
{
    return 0;
}

void
MovieSideCarIO::getMovieInfo(const std::string &filename, MovieInfo& info) const
{
    //
...
{
    unsigned int capabilities = MovieIO::MovieRead |
                                MovieIO::MovieReadAudio |
    ...
    addType("r3d", "RED Movie", capabilities, video, audio, eparams, dparams);
}

MovieR3DSideCarIO::~MovieR3DSideCarIO() {}

string 
MovieR3DSideCarIO::about() const
{
    ...
    : MovieSideCarIO(p, false)
{
    ...
    addType("mov", "Quicktime Movie", capabilities, video, audio, eparams, dparams);
    addType("qt", "Quicktime Movie", capabilities, video, audio, eparams, dparams);
...
string 
MovieQT7SideCarIO::about() const
{
    return "Apple Quicktime 7";
}
...
string 
MovieLQTSideCarIO::about() const
{
    return "libquicktime side car";
}

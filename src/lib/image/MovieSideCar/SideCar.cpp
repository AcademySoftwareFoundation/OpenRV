...
string
encodeChannelInfoVector(const FBInfo::ChannelInfoVector& v)
{
    //
...
string
encodeLayerInfoVector(const FBInfo::LayerInfoVector& v)
{
    //
...
string
encodeViewInfoVector(const FBInfo::ViewInfoVector& v)
{
    ostringstream str;
    ...
    for (size_t i = 0; i < v.size(); i++)
    {
        ...
        str << vi.name 
            << '|' << encodeLayerInfoVector(vi.layers)
...
string
encodeAudioChannelsVector(const TwkAudio::ChannelsVector& v)
{
    ostringstream str;
...
SideCar::SideCar(const string& cname,
                 const string& rname,
                 size_t pid,
                 MovieIO* io)
...
void 
SideCar::respond(const string& msg, const string& args)
{
    //size_t size = Message::newMessage("sidecar", &fout, msg, args, m_buffer);
...
SideCar::nextCommand()
{
    ...
    for (bool valid = false; !valid; valid = Message::isCommand(m_buffer))
    {
        ...
        *m_buffer = 0;
        ...
        bool timedout = !m_commandQueue.timed_receive(m_buffer,
                                                      Message::SizeInBytes(),
                                                      returnSize,
                                                      priority,
                                                      timeout);
...
bool
SideCar::openForRead(const string& filename)
{
    //
...
void
SideCar::createSharedData(const string& name, size_t size)
{
    //
    //  Create a managed shared memory object and map the entire thing
    //  

    m_sharedObject = new SharedMemory(interprocess::create_only,
                                      name.c_str(),
...
size_t
SideCar::serializeImages(const FrameBufferVector& fbs)
{
    //
...
//#define NOCOPY
    ...
    if (!p)
    {
        //cout << "ERROR: sidecar cannot allocated " << size << " bytes for image transfer" << endl;
    }
#ifndef NOCOPY
    else
    {
...
void
SideCar::decodeAudioConfigure(const string& arg)
{
    ...
    m_audioBuffer.reconfigure(bufferSize,
                              TwkAudio::layoutChannels(layout),
                              rate);
...
size_t
SideCar::decodeAudioFillBuffer(const string& arg)
{
    ...
    Movie::AudioReadRequest request(startTime,
                                    duration,
                                    margin);

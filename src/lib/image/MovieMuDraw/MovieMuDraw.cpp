...
MovieMuDraw::MovieMuDraw(Movie* movie, 
                         MuLangContext* context,
                         Process* process,
                         TwkGLF::GLVideoDevice* d)
  : Movie(),
    m_context(context),
    m_process(process),
    m_main(0),
...
Movie*
MovieMuDraw::clone() const
{
    return 0;
}

void
MovieMuDraw::setFunction(const string& moduleName, const StringVector& args)
{
    ...
    if (m_main = 
        module->findSymbolOfType<Mu::Function>(m_context->internName("main")))
    {
    ...
    if (m_init = 
        module->findSymbolOfType<Mu::Function>(m_context->internName("init")))
    {
...
void 
MovieMuDraw::imagesAtFrame(const ReadRequest& request, FrameBufferVector& fbs)
{
    ...
    for (size_t i = 0; i < fbs.size(); i++)
    {
        ...
        if (fb->attribute<string>("renderer") == "sw" && fbs.size() > 1)
        {
            ...
            glDrawPixels(fb->width(), fb->height(),
                         ctype, dtype,
                         fb->pixels<GLvoid>());
        }
...
void
MovieMuDraw::identifiersAtFrame(const ReadRequest& request, IdentifierVector& ids)
{
    m_movie->identifiersAtFrame(request, ids);
}

size_t
MovieMuDraw::audioFillBuffer(const AudioReadRequest& request, AudioBuffer& buffer)
{

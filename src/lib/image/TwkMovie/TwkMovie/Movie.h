...
{
    ChapterInfo() : startFrame(0), endFrame(0) {}
...
{
    MovieInfo() : FBInfo(), start(1), end(0), inc(0), fps(0.0), 
                  quality(1.0f), audio(false), audioSampleRate(0),
                  video(true), slowRandomAccess(false), privateData(0), hasSlate(false) {}

    MovieInfo(const MovieInfo& i) : TwkFB::FBInfo(i)
    {
        video              = i.video;
        start              = i.start;
        end                = i.end;
...
/// The Movie class is a base class for a Reader or Filter for images and audio.
...
public:
    ...
    {
        WriteRequest() : TwkFB::FrameBufferIO::WriteRequest(),
                         audioQuality(1.0),
    ...
    /// AudioReadRequest is the parameter used to get audio data from a movie
    ...
    virtual void imagesAtFrame(const ReadRequest&, 
    ...
    virtual void identifiersAtFrame(const ReadRequest&,
                                    IdentifierVector&);
    ...
    //
    //  Invalidate any cached info about directory contents, modification times, file
    //  existance, etc held by this Movie.
    //

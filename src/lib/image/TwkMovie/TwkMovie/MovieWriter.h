...
public:
    ...
    ///
    /// Open the writer. The MovieInfo can indicate desired dataType, number of
    /// channels, resolution, etc. The returned MovieInfo indicates to the
    /// caller what the writer wants.
    ///
    /// This function will be called once before the write() function is called.
    /// If open() is called, the writer can expect that it will be provided with
    /// framebuffers in the requested format (when it calls imagesAtFrame().
    ///

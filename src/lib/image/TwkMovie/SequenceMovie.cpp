...
SequenceMovie::SequenceMovie(int fs) : m_fs(fs)
{
}
...
void
SequenceMovie::addMovie(Movie* m, int fs, int fe)
{
    InputMovie im;
    im.movie = m;
    im.frames = fe - fs + 1;
...
void
SequenceMovie::audioConfigure(const AudioConfiguration& conf)
{
...
size_t
SequenceMovie::audioFillBuffer(const AudioReadRequest& request,
...
void
SequenceMovie::imagesAtFrame(const ReadRequest& request, FrameBufferVector& fbs)
{
...
void
SequenceMovie::identifiersAtFrame(const ReadRequest& request, 

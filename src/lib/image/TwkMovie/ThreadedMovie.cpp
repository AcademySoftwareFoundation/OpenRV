...
ThreadedMovie::ThreadedMovie(const Movies& movies,
                             const Frames& frames,
                             size_t stackMultiplier,
                             ThreadAPI* api,
                             InitializeFunc F)
    : m_movies(movies),
...
ThreadedMovie::~ThreadedMovie()
{
    ...
    {
        if (i > 0 && m_movies[i] == m_movies[i-1]) continue;
        delete m_movies[i];
    }
    ...
    for (FBMap::iterator i = m_map.begin();
         ...
         ++i)
    {
        for (size_t q = 0; q < i->second.size(); q++) delete i->second[q];
    }
...
ThreadedMovie::lock()
{
    m_threadGroup.lock(m_mapLock);
}

void 
ThreadedMovie::unlock()
{
    m_threadGroup.unlock(m_mapLock);
}

void
ThreadedMovie::threadMain()
{
    ...
    do
    {
        ...
        if (current - requested < threads * 2 && current < m_frames.size())
        {
            ...
            if (!exists)
            {
                ...
                try
                {
                    //cout << "thread " << td->id << " @ frame " << td->request.frame << endl;
                    td->movie->imagesAtFrame(td->request, fbs);
                }
                catch (std::exception& exc)
                {
                    cerr << "WARNING: an exception was raised evaluting frame " << frame 
                         << ":" << endl;
                    cerr << exc.what() << endl;
                    unlock();
                    break;
            ...
            {
                //cout << "thread " << td->id << " @ frame " << td->request.frame 
                     //<< " already in cache"
                     //<< endl;
            }
        }
        ...
        {
            unlock();
            //cout << "thread " << td->id << " finished, current = " << current
                 //<< ", requested = " << requested
                 //<< endl;
            break;
        }
...
void
ThreadedMovie::imagesAtFrame(const ReadRequest& request, FrameBufferVector& fbs)
{
    const size_t threads = m_threadGroup.num_threads();
...
void 
ThreadedMovie::identifiersAtFrame(const ReadRequest& request, IdentifierVector& ids)
{
    m_threadData.front().movie->identifiersAtFrame(request, ids);
}

size_t 
ThreadedMovie::audioFillBuffer(const AudioReadRequest& request, AudioBuffer& buffer)
{
    return m_threadData.front().movie->audioFillBuffer(request, buffer);
}

void 
ThreadedMovie::audioConfigure(unsigned int channels,
                              TwkAudio::Time rate,
                              size_t bufferSize)
{
}

...
void
dumpAudio(AudioBuffer* buffer, string filename, bool isBackwards, ofstream** dumpfile)
{
    if (isBackwards) buffer->reverse();

    if (!*dumpfile) *dumpfile = new ofstream(filename.c_str(), ios::binary | ios::out);
...
void
ResamplingMovie::setDebug(bool b)
{
    ResamplingMovie::debug = b;
}

void
ResamplingMovie::setDumpAudio(bool b)
{
    ResamplingMovie::dump = b;
}

TwkAudio::SampleTime
ResamplingMovie::offsetStart(double rate)
{
    double sourceRate = audioMovie()->info().audioSampleRate;
    bool doOffset = ( !audioMovie()->canConvertAudioRate() &&
                      sourceRate < rate &&
...
void
ResamplingMovie::shiftBuffer(size_t samples, unsigned int channels)
{
    if (samples != m_accumSamples && samples != 0)
    {
        memmove(m_accumBuffer,
                m_accumBuffer + samples * channels,
                (m_accumSamples - samples) * channels * sizeof(float));
    }
}

void
ResamplingMovie::reset(int channels, float factor, size_t samples, int blocksize)
{
    ...
    channels = (!audioMovie()->canConvertAudioChannels()) ?
        audioMovie()->info().audioChannels.size() : channels;
    ...
    {
        if (m_resampler) delete m_resampler;
        if (m_accumBuffer) delete m_accumBuffer;
        m_accumSamples = 0;
        m_accumStart = 0;
        m_readPosition = 0;
        m_accumAllocated = 0;
...
size_t
ResamplingMovie::audioFillBuffer(AudioBuffer& buffer)
{
    Movie* mov = audioMovie();

    const bool needsChannelMix      = !(mov->canConvertAudioChannels() ||
        buffer.channels() == mov->info().audioChannels);
    const ChannelsVector audioChannels = (needsChannelMix) ?
        mov->info().audioChannels : buffer.channels();
    ...
    //
    //  If this media cannot resample itself and has a different source audio
    //  sampling rate from the device, then we need to retime the audio sample
    //  target and prepare the resampler.
    //

    Time length = needsResample ?
        retimeAudioSampleTarget(buffer, startTime, numChannels) :
        samplesToTime(numSamples, rate);

    //
    //  Don't bother asking for samples if we are out of the valid range
    //
    //  NOTE: The orginal buffer start sample is used and not the offset start
    //  sample as calculated for resampling. This is because the min and max
    //  available samples are calculated without taking resampling skew into
    //  consideration.
    //
    ...
    {
        if (ResamplingMovie::debug) cerr << "ERROR: read " << offsetStart(rate) << " is out of range" << endl;
        return 0;
    }

    //
    //  Fill in the audio read request and call the movie with a temporary audio
    //  buffer.
    //
    ...
    if (ResamplingMovie::dump) dumpAudio(&buffer, "audio-source-dump", m_isBackwards, &sourceAudioDump);

    if (nread == 0)
    {
        // This might happen on loopback when the startSample goes to a very large number.
        return 0;
    }
    ...
    while (fp < lim)
    {
        if (*fp !=  *fp) { ++nanCount;    *fp =   0.0; }
        if (*fp >  1.0f) { ++outOfBounds; *fp =  1.0f; }
        if (*fp < -1.0f) { ++outOfBounds; *fp = -1.0f; }
        ++fp;
    }
    ...
    {
        cerr << "ERROR: " << nanCount
             << " NANs in audio stream and " << outOfBounds
             << " floats out of bounds." << endl;
    }
    ...
    if (buffer.size() != expectedSamples) buffer.resize(expectedSamples);
    if (nread < expectedSamples) 
    {
        memset(buffer.pointer() + nread * numChannels,
               0, 
    ...
    {
        AUDIOBUFFER_CHECK(buffer, "FileSourceIPNode::audioFillFromMedia() from media (before resample)")
    ...
    if (ResamplingMovie::dump) dumpAudio(&buffer, "audio-source-resampled-dump", m_isBackwards, &resampledAudioDump);

    return nread;
}

TwkAudio::Time
ResamplingMovie::retimeAudioSampleTarget(AudioBuffer buffer,
    const Time startTime, const int numChannels)
{
    ...
    //
    //  NOTE: we're assuming that we'll never be asked for a region larger than
    //  the device samples size multiplied by a magic number at least 2.
    //
    //  Find out if the requested time window starts anywhere inside our buffer.
    //
    ...
    //
    //  If there are samples in our read buffer already, and if readStart falls
    //  within our (potential) read buffer, we'll continue reading where we left
    //  off.
    //
        ...
        tdiff < samplesToTime(m_accumAllocated, rate))
    {
        ...
        //  sample, we need to shift the buffer back so that readStart is the
        //  first sample. Once this is accomplished we can fill the remaining
        //  buffer.
        //
        ...
        {
            //
            //  The shift is beyond the data we have so far, if it's within the
            //  readable region. Just nuke the existing samples and continue to
            //  read as much as possible. Otherwise, we can't continue and need
    ...
    else
    {
        ...
        // 1) For forward play, when the startSample is less than the minAvailableSample.
        // 2) For backwards play, when the startSample is greater than the maxAvailableSample.
        //
        // When backwards the "first" sample is startSample + numSamples.
        //
        // In other words the resampler gets reset at first/last sample transitions and
        // loop backs. This prevents discontinuitites at these points to make their way into
        // the resampler's internal buffer (which is about 4k).
        //
        // NOTE: Just like when deciding whether or not to request audio from the underlying
        // plugin we use the non-resampled original buffer start sample when comparing to min
        // and max available samples as they are reported indifferent to resampling skew.
        //
...
SampleTime
ResamplingMovie::minAvailableSample(double audioSampleRate)
{
...
SampleTime
ResamplingMovie::maxAvailableSample(double audioSampleRate)
{
...
void
ResamplingMovie::resampleAudio(AudioBuffer& buffer, const size_t nread,
    const size_t numSamples, const Time startTime, const ChannelsVector audioChannels,
    const double rate)
{
    ...
    if (m_isBackwards) ping->reverse();
    nresampled = m_resampler->process(ping->pointerIncludingMargin(),
                                    ping->sizeIncludingMargin(),
                                    pong->pointerIncludingMargin(),
    ...
    //
    // NB: the duration we add on is really the duration actually obtained from the
    // plugin... i.e. its possible that length != samplesToTime(nread, srate). 
    //
    ...
    //
    // We accumulate the readPositionOffset; otherwise we get a slip from accumulated differences
    // in readPosition values from the amount of time read by the plugin (samples in source rate)
    // versus this amount of time after conversion... which is the amount of time stored in 
    // accumBuffer (computed in device rate as device rate samples after resampling)
    // NB: This slip only happens for the case srate < rate because for this
    //     case length is computed in terms of rate i.e. since its higher sample rate.
    //     In the case srate > rate, length is calculated in terms of srate and so
    ...
    if (m_readContinuation)
    {
        ...
        if (rsamps)
        {
            memcpy(pong->pointer() + numChannels * asamps,
                   ping->pointer(),
        ...
        if (m_accumSamples + resampledSamplesToAccumulate > m_accumAllocated)
        {
            ...
            // But if we did this will prevent an out of bounds memory write.
            //
    ...
    {
        //
        //  Outside the buffer window. In this case copy the used ones into a
        //  new properly sized buffer and then update the accum buffer with the
        //  unused samples.
        //
        ...
        if (ping->size())
        {
            size_t resampledSamplesToAccumulate = (ping->size() >= numSamples) ?
                ping->size() - numSamples : ping->size();

...
void
LeaderFooterMovie::audioConfigure(const AudioConfiguration& conf)
{
    if (m_leader) m_leader->audioConfigure(conf);
    m_movie->audioConfigure(conf);
    if (m_footer) m_footer->audioConfigure(conf);
}

size_t
LeaderFooterMovie::audioFillBuffer(const AudioReadRequest& request,
                                   AudioBuffer& inbuffer)
{
    ...
    int leaderFrames = (!m_leader) ? 0 :
        m_leader->info().end - m_leader->info().start + 1;
    ...
    if (osamps != nsamps)
    {
        memmove((void*)(inbuffer.pointer() + (osamps - nsamps) * nc),
                (void*)inbuffer.pointer(),
                nsamps * sizeof(float) * nc);

        memset(inbuffer.pointer(),
               0,
...
static void 
copyExtraData(TwkMovie::Movie::ReadRequest& dst, 
...
void
LeaderFooterMovie::imagesAtFrame(const ReadRequest& request, FrameBufferVector& fbs)
{
    int frame  = request.frame;
    int leaderFrames = 0;
    if (m_leader) leaderFrames = m_leader->info().end - m_leader->info().start + 1;
    ...
    else if (frame < m_fs)
    {
        if (m_leader)
        {
            ReadRequest r(frame - (m_fs - leaderFrames) + m_leader->info().start, 
                          request.stereo);
...
void
LeaderFooterMovie::identifiersAtFrame(const ReadRequest& request, 

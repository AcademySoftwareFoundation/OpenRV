...
static void
planeNames(int nc, FrameBuffer::StringVector& planes)
{
    const char* ch = 0;

    if (nc > 31) TWK_THROW_STREAM(IOException, "Too many channels in SGI file");
...
IOsgi::IOsgi() : FrameBufferIO("IOsgi", "m99")
{
...
void
IOsgi::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
    ...
    if (header.magic == Magic || header.magic == Cigam)
    {
        ...
        fbi.dataType    = header.bytesPerChannel == 1 
                          ? FrameBuffer::UCHAR : FrameBuffer::USHORT;
    }
    else
    {
...
void
IOsgi::readVerbatim(ifstream& infile,
                    const Header& header,
                    FrameBuffer& fb,
                    bool swap) const
{
...
static void
expandBytes(unsigned char* optr, unsigned char* iptr, size_t expected)
{
...
static void
expandShorts(unsigned short* optr, 
             unsigned short* iptr, 
             size_t expected)
{
               ...
               bool swap) const
{
    ...
    for (FrameBuffer* p = fb.firstPlane(); 
         p; 
...
void
IOsgi::readImage(FrameBuffer& fb,
                 ...
                 const ReadRequest& request) const
{
    ...
    if (header.magic == Magic || header.magic == Cigam)
    {
        ...
        if (rle) readRLE(infile, header, fb, swap);
        else readVerbatim(infile, header, fb, swap);
...
void
IOsgi::writeImage(const FrameBuffer& img,
                  const std::string& filename,

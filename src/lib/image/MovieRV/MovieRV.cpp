...
void
MovieRV::initThreading()
{
    pthread_mutex_init(&m_lock, 0);
}

void
MovieRV::destroyThreading()
{
              ...
              size_t audioPacketSize)
{
    ...
    //m_device->makeCurrent(&fbtemp);     //make windows happy
...
MovieRV::imagesAtFrame(const ReadRequest& request, FrameBufferVector& fbs)
{
    ...
    for (size_t i = 0; i < fbs.size(); i++)
    {
        ...
        fb->restructure(m_info.width, m_info.height, 1,
                        4, TwkFB::FrameBuffer::FLOAT,
                        0, 0,
                        TwkFB::FrameBuffer::NATURAL, 
                        true);
        ...
        //for (const IPImage* i = m_session->displayImage(); i; i = i->next)
        //{

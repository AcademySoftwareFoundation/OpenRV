...
struct MaxMatrix43
{
    // In MAX, matrices have 4 rows of 3 columns. Usually, 3D matrices are 4x4
    // instead of 4x3. It's also the case for MAX, but as the last column is
    // always [0 0 0 1] for such matrices, it's implicitly managed by the class
    // and functions dealing with it.
    //
    ...
    TwkMath::Mat44f asMat44() const 
                        { return TwkMath::Mat44f( m[0][0], m[0][1], m[0][2], 0.0f,

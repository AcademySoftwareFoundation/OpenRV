...
IOrla::IOrla() : FrameBufferIO("IOrla", "m99"), m_error(false)
{
    //
...
void
IOrla::throwError(const std::string& filename, FILE* file=0) const
{
    if (m_error)
    {
        m_error = false;
        if (file) fclose(file);
        TWK_THROW_STREAM(TwkFB::IOException, "RLA: failed to open file \""
                         << filename);
    }
}
...
void
IOrla::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
    RLAHeader header;
    FILE* infile = NULL;
    ...
    {
        TWK_THROW_STREAM(TwkFB::IOException, "RLA: failed to read header of \""
                         << filename);
    }
    ...
    if ( header.chan_bits <= 8 ) fbi.dataType = FrameBuffer::UCHAR;
    else if ( header.chan_bits <= 16 ) fbi.dataType = FrameBuffer::USHORT;
    else if ( header.chan_bits == 32 ) fbi.dataType = FrameBuffer::FLOAT;
    else TWK_THROW_STREAM(IOException, "RLA: unsupported bit depth: " << filename)
...
void
IOrla::readImage( FrameBuffer& fb,
                  ...
                  const ReadRequest& request) const
{
    ...
    //
    // Open the file and read the standard RLA header, byte-swapping if necessary
    //
    if ( !(infile = TwkUtil::fopen(filename.c_str(), "rb")) )
    {
        TWK_THROW_STREAM(IOException, "RLA: cannot open " << filename);
    ...
    // TODO: Handle the case where different channels have different bit depths!

//     if ( header.num_matte > 0 && header.matte_bits != header.chan_bits )
//     {
//         TWK_THROW_STREAM(TwkFB::IOException, "RLA: All channels must have the same bit depth "
//                          << filename);
//     }
...
//     }
    ...
    //
    // Allocate and read the scanline offset table, byte-swapping if necessary
    //
    std::vector<int> offsets( dataHeight );
    if ( fread( &offsets.front(), sizeof( int ), dataHeight, infile ) != dataHeight )
    {
        TWK_THROW_STREAM(TwkFB::IOException, "RLA: failed to read offset table of "
                         << filename);
    }
    ...
    if ( maxExtras & MAX_GB_Z ) allChannelNames.push_back( "Z" );
    if ( maxExtras & MAX_GB_MTL_ID ) allChannelNames.push_back( "MTL_ID" );
    if ( maxExtras & MAX_GB_NODE_ID ) allChannelNames.push_back( "NODE_ID" );
    if ( maxExtras & MAX_GB_UV ) allChannelNames.push_back( "UV" );
    if ( maxExtras & MAX_GB_NORMAL ) allChannelNames.push_back( "NORMAL" );
    if ( maxExtras & MAX_GB_REALPIX ) allChannelNames.push_back( "REALPIX" );
    if ( maxExtras & MAX_GB_COVERAGE ) allChannelNames.push_back( "COVERAGE" );
    if ( maxExtras & MAX_GB_BG ) allChannelNames.push_back( "BG" );
    if ( maxExtras & MAX_GB_NODE_RENDER_ID ) allChannelNames.push_back( "NODE_RENDER_ID" );
    if ( maxExtras & MAX_GB_COLOR ) allChannelNames.push_back( "COLOR" );
    if ( maxExtras & MAX_GB_TRANSP ) allChannelNames.push_back( "TRANSP" );
    if ( maxExtras & MAX_GB_VELOC ) allChannelNames.push_back( "VELOC" );
    if ( maxExtras & MAX_GB_WEIGHT ) allChannelNames.push_back( "WEIGHT" );
    if ( maxExtras & MAX_GB_MASK ) allChannelNames.push_back( "MASK" );
    ...
    if ( header.chan_bits <= 8 )     dataType = FrameBuffer::UCHAR;
    else if ( header.chan_bits <= 16 ) dataType = FrameBuffer::USHORT;
    else if ( header.chan_bits == 32 ) dataType = FrameBuffer::FLOAT;
    else TWK_THROW_STREAM(IOException, "RLA: unsupported bit depth: " << filename)
    ...
    fb.restructure(width, height, depth, numChannels, dataType, 
                   NULL, &channelNames, FrameBuffer::NATURAL,
                   true);
    ...
    if ( maxExtras & MAX_RENDER_INFO )
    {
        ...
        snprintf( renderTimeStr, 32, "%d, %d - %d, %d",
                  maxRenderInfo.region.left, 
                  maxRenderInfo.region.top, 
                  maxRenderInfo.region.right, 
                  maxRenderInfo.region.bottom );
        fb.newAttribute( "RLA/3DS/region", std::string( renderTimeStr ) );
        
    }
    if ( maxExtras & MAX_NODE_TABLE ) fb.newAttribute<string>( "RLA/3DS Max Node name table", "Yes" );
    if ( maxExtras & MAX_LAYER_DATA ) fb.newAttribute<string>( "RLA/3DS Max Layer Data", "Yes" );
    if ( maxExtras & MAX_UNPREMULT ) fb.newAttribute<string>( "RLA/Non-premultiplied alpha", "Yes" );
    ...
    snprintf( revisionStr, 32, "%s (0x%hX)", 
              header.revision == RLA_MAGIC ? "Wavefront" : "3DS Max", header.revision );
        ...
        || header.window.top != header.active_window.top )
    {
        ...
        fb.newAttribute("DataWindowOrigin", TwkMath::Vec2i( header.window.left, 
                                                            header.window.bottom ) );
        fb.newAttribute("DisplayWindowSize", TwkMath::Vec2i( width, height ) );
        fb.newAttribute("DisplayWindowOrigin", TwkMath::Vec2i( header.active_window.left, 
    ...
    for( int y = 0; y < dataHeight; ++y )
    {
        ...
        for( int c = 0; c < numChannels; ++c )
        {
            ...
            else if (header.chan_bits <= 16)
            {
                ...
                // of the machine:
#if defined(TWK_LITTLE_ENDIAN) || defined(__LITTLE_ENDIAN__)
                unsigned char *linepos = rlaDecodeScanline( &lineBuffer.front(), 
                                                            scanline+offset+1, 
                                                            width, width, stride );
                rlaDecodeScanline( linepos, scanline+offset, 
                                   width, width, stride );
#else
                unsigned char *linepos = rlaDecodeScanline( &lineBuffer.front(), 
                                                            scanline+offset, 
                                                            width, width, stride );
                rlaDecodeScanline( linepos, scanline+offset+1, 
                                   width, width, stride );
#endif
            }

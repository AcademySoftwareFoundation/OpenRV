...
void
IOmray::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
    ifstream file(UNICODE_C_STR(filename.c_str()));

    if (!file) TWK_THROW_STREAM(Exception, "No file");
...
void
IOmray::readImage(FrameBuffer& fb,

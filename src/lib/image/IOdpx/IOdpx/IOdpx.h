...
public:
    ...
    {
        U32   magic_num;        // C: magic number 0x53445058 (SDPX) or 0x58504453 (XPDS) 
        U32   offset;           // C: offset to image data in bytes 
        ASCII vers[8];          // C: which header format version is being used (v1.0)
    ...
    struct DPXFilmHeader
    {
        ...
        void swap() 
        { 
            TwkUtil::swapWords(&frame_position, 5); 
        }
    };
    ...
    virtual void readImages(FrameBufferVector&,

...
{
    //
    //  Total insanity.  An unsubstantiated rumor states that dpx header time codes
    //  are actually base 10.  IE each byte of the timecode (frames,
    //  seconds, minutes, hours) is represented as a 2 hexadecimal
...
static int U32TCToFrames(U32 u, float fps)
{
    ...
    for (size_t i = 0; i < 4; i++)
    {
        ...
        {
          case 0: f += v; break;
          case 1: f += v * fps + .49999; break;
          case 2: f += 60 * v * fps + .49999; break;
          case 3: f += 60 * 60 * v * fps + .49999; break;
        }
    }
...
static inline bool hasData(IOdpx::R32 p)
{
    return  p == p &&                    // spec requires undefined R32 = 0xffffffff (NaN)
            p < DPX_NO_DATA_R32_CUTOFF;  // shake puts high values in R32 fields
}

static inline bool hasData(IOdpx::U32 p)
{
    return p != DPX_NO_DATA_U32;
}

static inline bool hasData(IOdpx::U8 p)
{
    return p != DPX_NO_DATA_U8;
}
...
static const char* 
nameForValue(const NameValue* ptr, unsigned int value)
{
...
#define GET(T,p) *(T*)p; p+= sizeof(T)
#define GETSTRING(o,p,s) memcpy(o, p, sizeof(char)*s); p+= sizeof(char)
...
IOdpx::IOdpx(StorageFormat format,
             bool useChromaticies,
             IOType type,
             size_t chunksize,
             int maxAsync) 
...
IOdpx::IOdpx(const std::string& format,
             bool useChromaticies,
             IOType type,
             size_t chunksize,
             int maxAsync)
    : StreamingFrameBufferIO("IOdpx", "m5", type, chunksize, maxAsync),
      m_useChromaticities(useChromaticies)
{
    if (format == "RGB8") m_format = RGB8;
    else if (format == "RGBA8") m_format = RGBA8;
    else if (format == "RGB16") m_format = RGB16;
    else if (format == "RGBA16") m_format = RGBA16;
    else if (format == "RGB8_PLANAR") m_format = RGB8_PLANAR;
    else if (format == "RGB16_PLANAR") m_format = RGB16_PLANAR;
    else if (format == "RGB10") m_format = RGB10_A2;
    else if (format == "RGB10_A2") m_format = RGB10_A2;
    else if (format == "A2_BGR10") m_format = A2_BGR10;
    else m_format = RGB8;
    init();
}
...
IOdpx::init()
{
    ...
    eparams.push_back(StringPair("transfer", "Transfer function (LOG, DENSITY, REC709, USER, VIDEO, SMPTE274M, REC601-625, REC601-525, NTSC, PAL, or number)"));
    eparams.push_back(StringPair("colorimetric", "Colorimetric specification (REC709, USER, VIDEO, SMPTE274M, REC601-625, REC601-525, NTSC, PAL, or number)"));
    ...
    eparams.push_back(StringPair("orientation", "Pixel Origin string or int (TOP_LEFT, TOP_RIGHT, BOTTOM_LEFT, BOTTOM_RIGHT, ROTATED_TOP_LEFT, ROTATED_TOP_RIGHT, ROTATED_BOTTOM_LEFT, ROTATED_BOTTOM_RIGHT)"));
...
bool 
IOdpx::getBoolAttribute(const std::string& name) const
{
    if (name == "useChromaticies") return m_useChromaticities;
    return StreamingFrameBufferIO::getBoolAttribute(name);
}

void 
IOdpx::setBoolAttribute(const std::string& name, bool value)
{
    if (name == "useChromaticies") m_useChromaticities = value;
    StreamingFrameBufferIO::setBoolAttribute(name, value);
}

std::string 
IOdpx::getStringAttribute(const std::string& name) const
{
    if (name == "format")
    {
        switch (m_format)
...
void 
IOdpx::setStringAttribute(const std::string& name, const std::string& value)
{
    if (name == "format")
    {
        if (value == "RGB8") m_format = RGB8;
        else if (value == "RGB16") m_format = RGB16;
        else if (value == "RGBA8") m_format = RGBA8;
        else if (value == "RGBA16") m_format = RGBA16;
        else if (value == "RGB10_A2") m_format = RGB10_A2;
        else if (value == "A2_BGR10") m_format = A2_BGR10;
        else if (value == "RGB8_PLANAR") m_format = RGB8_PLANAR;
        else if (value == "RGB16_PLANAR") m_format = RGB16_PLANAR;
...
static
void setAttr(FrameBuffer& fb, const char* name, char* value, size_t size)
{
    if (hasData(value)) 
    {
        ...
        {
            if ((int) value[i] < 0 || (int) value[i] >= 127) buffer[i] = '?';
...
void
IOdpx::readAttrs(FrameBuffer& fb, DPXHeader& header, int e) const
{
    ...
    {
        fb.setPixelAspectRatio( float(double(header.orientation.pixel_aspect_H) /
    ...
    if (transfer == DPX_PRINTING_DENSITY ||
    ...
    else if (transfer == DPX_SMPTE274M ||
             transfer == DPX_ITU_R709)
    {
        fb.setTransferFunction(ColorSpace::Rec709());
    }
    else if (transfer == DPX_ITU_R601_625L ||
    ...
    if (hasData(header.tv.integration_times)) fb.newAttribute("DPX-TV/IntegrationTimes", (header.tv.integration_times));
    if (hasData(header.tv.white_level)) fb.newAttribute("DPX-TV/WhiteLevel", (header.tv.white_level));
    if (hasData(header.tv.break_point)) fb.newAttribute("DPX-TV/BreakPoint", (header.tv.break_point));
    if (hasData(header.tv.black_gain)) fb.newAttribute("DPX-TV/BlackGain", (header.tv.black_gain));
    if (hasData(header.tv.black_level)) fb.newAttribute("DPX-TV/BlackLevel", (header.tv.black_level));
    if (hasData(header.tv.gamma)) fb.newAttribute("DPX-TV/Gamma", (header.tv.gamma));
    if (hasData(header.tv.time_offset)) fb.newAttribute("DPX-TV/TimeOffset", (header.tv.time_offset));
    if (hasData(header.tv.frame_rate)) fb.newAttribute("DPX-TV/FrameRate", (header.tv.frame_rate));
    if (hasData(header.tv.ver_sample_rate)) fb.newAttribute("DPX-TV/VeritcalSampleRate", (header.tv.ver_sample_rate));
    if (hasData(header.tv.hor_sample_rate)) fb.newAttribute("DPX-TV/HorizontalSampleRate", (header.tv.hor_sample_rate));

    if (header.tv.video_signal != 255) fb.newAttribute("DPX-TV/VideoSignal", int(header.tv.video_signal));
    if (header.tv.field_num != 255) fb.newAttribute("DPX-TV/FieldNumber", int(header.tv.field_num));
    ...
    if (hasData (header.film.shutter_angle)) fb.newAttribute("DPX-MP/ShutterAngle", header.film.shutter_angle);
    if (hasData (header.film.frame_rate)) fb.newAttribute("DPX-MP/FrameRate", header.film.frame_rate);
    if (hasData (header.film.held_count)) fb.newAttribute("DPX-MP/HeldCount", header.film.held_count);
    ...
    if (hasData(header.film.prefix))
    {
        sprintf(temp, "%c%c%c%c%c%c", 
                header.film.prefix[0],
                header.film.prefix[1],
                header.film.prefix[2],
                header.film.prefix[3],
                header.film.prefix[4],
    ...
    for (int i=0; i < header.image.element_number; i++)
    {
        ...
        fb.newAttribute(temp, string(nameForValue(packing,
                                                  header.image.image_element[i].packing)));
        ...
        fb.newAttribute(temp, string(nameForValue(colorspace,
                                                  header.image.image_element[i].transfer)));
        sprintf(temp, "DPX-%d/Colorimetric", i);
        fb.newAttribute(temp, string(nameForValue(colorspace,
                                                  header.image.image_element[i].colorimetric)));
        ...
        fb.newAttribute(temp, string(nameForValue(pixelFormat,
    ...
    fb.newAttribute("DPX/Orientation", string(nameForValue(orientation,
    ...
    {
      case DPX_PIXEL_UNKNOWN: numChannels = 0; break;
      case DPX_PIXEL_R: numChannels = 1; break;
      case DPX_PIXEL_G: numChannels = 1; break;
      case DPX_PIXEL_B: numChannels = 1; break;
      case DPX_PIXEL_A: numChannels = 1; break;
      case DPX_PIXEL_Y: numChannels = 1; break;
      case DPX_PIXEL_CbCr: numChannels = 2; break;
      case DPX_PIXEL_Z: numChannels = 1; break;
      case DPX_PIXEL_COMPOSITE_VIDEO: numChannels = 3; break;
      case DPX_PIXEL_RGB: numChannels = 3; break;
      case DPX_PIXEL_RGBA: numChannels = 4; break;
      case DPX_PIXEL_ABGR: numChannels = 4; break;
      case DPX_PIXEL_CbYCrY_422: numChannels = 3; break;
      case DPX_PIXEL_CbYACrYA_4224: numChannels = 3; break;
      case DPX_PIXEL_CbYCr_444: numChannels = 3; break;
      case DPX_PIXEL_CbYCrA_4444: numChannels = 3; break;
      case DPX_PIXEL_USER_2_COMPONENT: numChannels = 2; break;
      case DPX_PIXEL_USER_3_COMPONENT: numChannels = 3; break;
      case DPX_PIXEL_USER_4_COMPONENT: numChannels = 4; break;
      case DPX_PIXEL_USER_5_COMPONENT: numChannels = 5; break;
      case DPX_PIXEL_USER_6_COMPONENT: numChannels = 6; break;
      case DPX_PIXEL_USER_7_COMPONENT: numChannels = 7; break;
      case DPX_PIXEL_USER_8_COMPONENT: numChannels = 8; break;
    }
...
void
IOdpx::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
    ...
    //  want for the header), but some filesystems don't support it so fall back
    ...
    if (header.file.magic_num != 0x53445058)
    {
        if (header.file.magic_num == 0x802A5FD7)
        {
            ...
            IOcin cin(IOcin::StorageFormat(m_format),
                      m_useChromaticities,
                      IOcin::IOType(m_iotype),
    ...
    if (((inputBits == 10 || inputBits == 12) && packing0 == IOdpx::DPX_PAD_MSB_WORD) ||
          inputBits == 32 || 
          inputBits == 64 ||
          !threeOrFourChannel)
    {
    ...
    switch (header.image.orientation)
    {
      ...
      case DPX_TOP_RIGHT: orient = FrameBuffer::TOPRIGHT; break;
      case DPX_ROTATED_TOP_LEFT:
      case DPX_TOP_LEFT: orient = FrameBuffer::TOPLEFT; break;
      case DPX_ROTATED_BOTTOM_LEFT:
      case DPX_BOTTOM_LEFT: orient = FrameBuffer::BOTTOMLEFT; break;
      case DPX_ROTATED_BOTTOM_RIGHT:
      case DPX_BOTTOM_RIGHT: orient = FrameBuffer::BOTTOMRIGHT; break;
    }
    ...
    //
    //  Above switch handles 10- or 12-bit files, but if file is 8 or 16 bit we
    //  always read to that format.
    //
    if (inputBits == 8)  fbi.dataType = FrameBuffer::UCHAR;
    if (inputBits == 16) fbi.dataType = FrameBuffer::USHORT;
...
void
IOdpx::readImages(FrameBufferVector& fbs,
                 ...
                 const ReadRequest& request) const
{
    ...
    {
        if (request.views.empty() && i > 0) break;
	...
	const bool threeOrFourChannel = packedYUV || fmt == IOdpx::DPX_PIXEL_RGBA || fmt == IOdpx::DPX_PIXEL_RGB;
        ...
        if (inputBits == 10 &&
            packing == IOdpx::DPX_PAD_NONE &&  // DPX_PAD_NONE == 0
            maxData == (w * h * sizeof(U32)))
        {
            ...
            cout << "WARNING: DPX: image claims no padding but data size indicates otherwise, "
                 << "reading as if LSB padded"
                 << endl;
        ...
        if (((inputBits == 10 || inputBits == 12) && packing == IOdpx::DPX_PAD_MSB_WORD) ||
              inputBits == 32 || 
              inputBits == 64 ||
	      !threeOrFourChannel)
        {
        ...
        try
        {
            if (inputBits == 10)
            {
                ...
                else
                {
                    switch (m_format)
                    {
                      ...
                      case RGB8: 
                          {
                              if (alpha) Read10Bit::readRGBA8(filename, data, fb, w, h, maxData, true, swap); 
                              else Read10Bit::readRGB8(filename, data, fb, w, h, maxData, swap); 
                              readit = true;
                              break;
                          }
                      case RGBA8: 
            ...
            else if (inputBits == 12)
            {
                ...
                else
                {
                    switch (m_format)
                    {
                      ...
                      case RGB8: 
                      case RGBA8: 
                      case RGB8_PLANAR: Read12Bit::readRGB8_PLANAR(filename, data, fb, w, h, maxData, swap); break;
                      case RGB10_A2: 
                      case A2_BGR10: 
                      case RGB16: 
                      case RGBA16: 
                      case RGB16_PLANAR: Read12Bit::readRGB16_PLANAR(filename, data, fb, w, h, maxData, swap); break;
                    }
            ...
            else if (inputBits == 16)
            {
                if (alpha)
                {
                    ...
                    readit = true;
                    if (partial) fb.attribute<float>("PartialImage") = 1.0;
                }
                else
                {
                    ...
                    {
                        didUseRaw = true;
                        Read16Bit::readRGB16(filename, data, fb, w, h, maxData, swap, 
                                             didUseRaw, (unsigned char*)fstream.data()); 
                    }
                    else
                    {
        ...
        switch (header.image.orientation)
        {
          ...
          case DPX_ROTATED_TOP_LEFT:
              ...
              break;    // this is the default for DPX, the reading code all does this itself
        ...
        if (!readit &&
            fb.hasAttribute("ChannelsInFile") && 
            4 == fb.attribute<int>("ChannelsInFile"))
        {
            cerr << "WARNING: This type of RGBA DPX file is not currently supported." << endl;
        }
    }
}
...
void
IOdpx::writeImage(const FrameBuffer& img, 
                  const std::string& filename,
                  const WriteRequest& request) const
{
    ...
    if (!outfile) TWK_THROW_STREAM(IOException, "DPX: cannot open " << filename);
    ...
    if (img.hasAttribute(ColorSpace::TransferFunction()))
    {
        ...
        if (t == ColorSpace::Rec709()) transfer = DPX_ITU_R709;
        else if (t == ColorSpace::CineonLog()) transfer = DPX_PRINTING_DENSITY;
        else if (t == ColorSpace::Linear()) transfer = DPX_LINEAR;
        else transfer = DPX_USER_DEFINED_COLOR;
    }
    ...
    for (size_t i = 0; i < request.parameters.size(); i++)
    {
        ...
        if (name == "output/start_frame") fs = atoi(value.c_str());
        else if (name == "output/end_frame") fe = atoi(value.c_str());
        else if (name == "output/frame") fc = atoi(value.c_str());
        else if (name == "output/frame_position") fpos = atoi(value.c_str());
        else if (name == "output/sequence_len") seqlen = atoi(value.c_str());
        ...
        {
            if (value == ColorSpace::Rec709()) transfer = DPX_ITU_R709;
            else if (value == ColorSpace::sRGB()) transfer = DPX_USER_DEFINED_COLOR;
            else if (value == ColorSpace::CineonLog()) transfer = DPX_PRINTING_DENSITY;
            else if (value == ColorSpace::Linear()) transfer = DPX_LINEAR;
        }
    ...
    for (size_t i = 0; i < request.parameters.size(); i++)
    {
        ...
        {
            if (value == "LOG") transfer = DPX_LOGARITHMIC;
            else if (value == "DENSITY")  transfer = DPX_PRINTING_DENSITY;
            else if (value == "LINEAR")  transfer = DPX_LINEAR;
            else if (value == "REC709")  transfer = DPX_ITU_R709;
            else if (value == "USER")  transfer = DPX_USER_DEFINED_COLOR;
            else if (value == "VIDEO")  transfer = DPX_UNSPECIFIED_VIDEO;
            else if (value == "SMPTE274M")  transfer = DPX_SMPTE274M;
            else if (value == "REC601-625")  transfer = DPX_ITU_R601_625L;
            else if (value == "REC601-525")  transfer = DPX_ITU_R601_525L;
            else if (value == "NTSC")  transfer = DPX_NTSC;
            else if (value == "PAL")  transfer = DPX_PAL;
        ...
        {
            if (value == "REC709")  colorimetric = DPX_ITU_R709;
            else if (value == "USER")  colorimetric = DPX_USER_DEFINED_COLOR;
            else if (value == "VIDEO")  colorimetric = DPX_UNSPECIFIED_VIDEO;
            else if (value == "SMPTE274M")  colorimetric = DPX_SMPTE274M;
            else if (value == "REC601-625")  colorimetric = DPX_ITU_R601_625L;
            else if (value == "REC601-525")  colorimetric = DPX_ITU_R601_525L;
            else if (value == "NTCS")  colorimetric = DPX_NTSC;
            else if (value == "PAL")  colorimetric = DPX_PAL;
            else if (value.size() >= 1)
            {
                if (value[0] >= '0' && value[0] <= '9')
                {
        ...
        {
            if (value == "TOP_LEFT")  orient = DPX_TOP_LEFT;
            else if (value == "TOP_RIGHT")  orient = DPX_TOP_RIGHT;
            else if (value == "BOTTOM_LEFT")  orient = DPX_BOTTOM_LEFT;
            else if (value == "BOTTOM_RIGHT")  orient = DPX_BOTTOM_RIGHT;
            else if (value == "ROTATED_TOP_LEFT")  orient = DPX_ROTATED_TOP_LEFT;
            else if (value == "ROTATED_TOP_RIGHT")  orient = DPX_ROTATED_TOP_RIGHT;
            else if (value == "ROTATED_BOTTOM_LEFT")  orient = DPX_ROTATED_BOTTOM_LEFT;
            else if (value == "ROTATED_BOTTOM_RIGHT")  orient = DPX_ROTATED_BOTTOM_RIGHT;
            else if (value.size() >= 1)
            {
                if (value[0] >= '0' && value[0] <= '9')
                {
        ...
        else if (name == "creator") creator = value;
        else if (name == "copyright" || name == "output/copyright") copyright = value;
        else if (name == "project") project = value;
        else if (name == "reserved") reserved = value;
        else if (name == "create_time") createtime = value;
        else if (name == "film/mfg_id") film_mfg_id = value;
        else if (name == "film/type") film_type = value;
        else if (name == "film/offset") film_offset = value;
        else if (name == "film/prefix") film_prefix = value;
        else if (name == "film/count") film_count = value;
        else if (name == "film/format") film_format = value;
        ...
        else if (name == "film/fps") film_fps = atof(value.c_str());
        else if (name == "film/shutter_angle") film_shutter_angle = atof(value.c_str());
        else if (name == "film/frame_id") film_frame_id = value;
        else if (name == "film/slate_info") film_slate_info = value;
        ...
        else if (name == "tv/interlace") tv_interlace = atoi(value.c_str());
        else if (name == "tv/field_num") tv_field_num = atoi(value.c_str());
        else if (name == "tv/video_signal") tv_video_signal = atoi(value.c_str());
        else if (name == "tv/horizontal_sample_rate") tv_hor_sample_rate = atof(value.c_str());
        else if (name == "tv/vertical_sample_rate") tv_ver_sample_rate = atof(value.c_str());
        else if (name == "tv/frame_rate") tv_frame_rate = atof(value.c_str());
        else if (name == "tv/time_offset") tv_time_offset = atof(value.c_str());
        else if (name == "output/gamma" || name == "tv/gamma") tv_gamma = atof(value.c_str());
        else if (name == "tv/black_level") tv_black_level = atof(value.c_str());
        else if (name == "tv/black_gain") tv_black_gain = atof(value.c_str());
        else if (name == "tv/break_point") tv_break_point = atof(value.c_str());
        else if (name == "tv/white_level") tv_white_level = atof(value.c_str());
        else if (name == "tv/integration_times") tv_integration_times = atof(value.c_str());
        else if (name == "source/x_offset") o_x_offset = atoi(value.c_str());
        else if (name == "source/y_offset") o_y_offset = atoi(value.c_str());
        else if (name == "source/x_original_size") o_x_orig_size = atoi(value.c_str());
        else if (name == "source/y_original_size") o_y_orig_size = atoi(value.c_str());
        else if (name == "source/x_center") o_x_center = atof(value.c_str());
        else if (name == "source/y_center") o_y_center = atof(value.c_str());
        else if (name == "source/file_name") o_file_name = value;
        else if (name == "source/creation_time") o_creation_time = value;
        else if (name == "source/input_dev") o_input_dev = value;
        else if (name == "source/input_serial") o_input_serial = value;
        else if (name == "source/border_XL") o_border_XL = atoi(value.c_str());
        else if (name == "source/border_XR") o_border_XR = atoi(value.c_str());
        else if (name == "source/border_YT") o_border_YT = atoi(value.c_str());
        else if (name == "source/border_YB") o_border_YB = atoi(value.c_str());
        else if (name == "output/pa/numerator" || name == "source/pixel_aspect_H") o_pixel_aspect_H = atoi(value.c_str());
        else if (name == "output/pa/denominator" || name == "source/pixel_aspect_V") o_pixel_aspect_V = atoi(value.c_str());
    }

...
static Img4f *readRLE( FILE *f, RgbeIff::SGI_HDR &header, bool swapped )
{
    ...
    for( int z = 0; z < header.zsize; ++z )
    {
        for( int y = 0; y < header.ysize; ++y )
        {
            ...
            {
                char msg[256];
                snprintf( msg, 256, "Error seeking to image data at "
                          "offset %d, channel %d, line %d", seekTo, z, y );
            ...
            {
                char msg[256];
                snprintf( msg, 256, "Error reading image data at "
                          "channel %d, line %d", z, y );
                TWK_EXC_THROW_WHAT( Exception, msg );
            }
            ...
            {
                char msg[256];
                snprintf( msg, 256, "Bad RLE data in image at "
                          "channel %d, line %d", z, y );
                TWK_EXC_THROW_WHAT( Exception, msg );
            }

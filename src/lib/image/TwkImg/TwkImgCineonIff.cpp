...
// Generic Image Data
//
// The generic image data header is just that - it contains all the general
// information about the image (how big the headers are, what's in the image,
// how big the image is, etc.). The information contained is generally common
// to all image manipulation/graphics programs.
struct FileInformation
{
    ...
    ASCII create_date[12];  // file creation date "yyyy:mm:dd:hh:mm:ss:LTZ"
    ASCII create_time[12];  // file creation date "yyyy:mm:dd:hh:mm:ss:LTZ"
...
void FileInformation::read( std::istream &stream )
{
    ...
    {
        std::cerr << "Bad magic number. was supposed to be: " << 0x802A5FD7
                 << std::endl << "Instead got: " << magic_num
                 << std::endl;
...
{
    stream << "\tDesignator 1: "
           << ( ( designator1 == 0 ) ? "Universal Metric" : "Vendor Specific" )
           << std::endl
           << "\tDesignator 2: ";

    switch( designator2 )
    {
    case 0: stream << "B&W" << std::endl; break;
    case 1: stream << "red (printing density)" << std::endl; break;
    case 2: stream << "green (printing density)" << std::endl; break;
    case 3: stream << "blue (printing density)" << std::endl; break;
    case 4: stream << "red (CCIR XA/11)" << std::endl; break;
    case 5: stream << "green (CCIR XA/11)" << std::endl; break;
    case 6: stream << "blue (CCIR XA/11)" << std::endl; break;
    default: stream << "Reserved" << std::endl; break;
    };
...
void ImageInformation::writeAscii( std::ostream &stream ) const
{
    ...
    switch( orientation )
    {
    case 0: stream << "left to right, top to bottom" << std::endl; break;
    case 1: stream << "left to right, bottom to top" << std::endl; break;
    case 2: stream << "right to left, top to bottom" << std::endl; break;
    case 3: stream << "right to left, bottom to top" << std::endl; break;
    case 4: stream << "top to bottom, left to right" << std::endl; break;
    case 5: stream << "top to bottom, right to left" << std::endl; break;
    case 6: stream << "bottom to top, left to right" << std::endl; break;
    case 7: stream << "bottom to top, right to left" << std::endl; break;
    }
...
void DataFormatInformation::writeAscii( std::ostream &stream ) const
{
    stream << "------- Data Format Header Information ("
           << sizeof( DataFormatInformation ) << " bytes) -------" << std::endl;

    stream << "Data Interleave: ";
    switch( interleave )
    {
    case 0: stream << "pixel" << std::endl; break;
    case 1: stream << "line" << std::endl; break;
    case 2: stream << "channel" << std::endl; break;
    case 3: stream << "user-defined" << std::endl; break;
    }

    stream << "Packing: ";
    switch( packing )
    {
    case 0: stream << "Use all bits" << std::endl; break;
    case 1: stream << "8-bit boundaries, left justified" << std::endl; break;
    case 2: stream << "8-bit boundaries, right justified" << std::endl; break;
    case 3: stream << "16-bit boundaries, left justified" << std::endl; break;
    case 4: stream << "16-bit boundaries, right justified" << std::endl; break;
    case 5: stream << "32-bit boundaries, left justified" << std::endl; break;
    case 6: stream << "32-bit boundaries, right justified" << std::endl; break;
    default: stream << "Unknown packing" << std::endl; break;
    }

    stream << "Packing Tightness: "
           << ( ( tightness == 0 ) ? "At most one pixel per cell" :
                "As many fields as possible per cell" ) << std::endl;

    stream << "Data Sign: "
           << ( ( data_sign == 0 ) ? "unsigned" : "signed" ) << std::endl;

    stream << "Image Sense: "
           << ( ( image_sense == 0 ) ? "positive" : "negative" ) << std::endl;
...
void printingDensityToRelativeExposure( const PIXEL &pd,
                                        Col3f &re,
                                        int codeShiftR,
                                        int codeShiftG,
                                        int codeShiftB )
...
//*****************************************************************************
void relativeExposureToPrintingDensity( const Col3f &re,
                                        PIXEL &pd,
                                        int codeShiftR,
                                        int codeShiftG,
                                        int codeShiftB )
{
    ...
    if ( tmp.x < minFloatVal ){ tmp.x = minFloatVal; }
    if ( tmp.y < minFloatVal ){ tmp.y = minFloatVal; }
    if ( tmp.z < minFloatVal ){ tmp.z = minFloatVal; }
...
//*****************************************************************************
Img4f *CineonIff::read( std::istream &cinStream,
                        int codeShiftR,
                        int codeShiftG,
                        int codeShiftB,
                        bool linearize )
{
    ...
    for ( int y = height - 1; y >= 0; --y )
    {
        ...
        for ( int x = 0; x < width; ++x, ++dstPixel )
        {
            ...
            if ( linearize )
            {
                printingDensityToRelativeExposure( srcPixel,
                                                   (*(( Col3f * )dstPixel)),
                                                   codeShiftR,
                                                   codeShiftG,
                                                   codeShiftB );
...
//*****************************************************************************
Img4f *CineonIff::read( const char *imgFileName,
                        ...
                        bool linearize )
{
    ...
    Img4f *ret = CineonIff::read( imgStream,
                                  codeShiftR, codeShiftG, codeShiftB,
                                  linearize );
                       ...
                       bool deLinearize )
{
    ...
    for ( int y = height - 1; y >= 0; --y )
    {
        ...
        for ( int x = 0; x < width; ++x, ++srcPixel )
        {
            if ( deLinearize )
            {
                relativeExposureToPrintingDensity( (*(( Col3f * )srcPixel)),
                                                   ( PIXEL & )dstPixel,
                                                   codeShiftR,
                                                   codeShiftG,
                                                   codeShiftB );
            ...
            {
                const float tr =
                    std::clamp( ( srcPixel->x * 1023.0f ) + codeShiftR,
                                    0.0f, 1023.0f );
                const float tg =
                    std::clamp( ( srcPixel->y * 1023.0f ) + codeShiftG,
                                    0.0f, 1023.0f );
                const float tb =
                    std::clamp( ( srcPixel->z * 1023.0f ) + codeShiftB,
                                    0.0f, 1023.0f );
                       ...
                       bool deLinearize )
{
    ...
    CineonIff::write( img, imgStream, imgFileName,
                      codeShiftR, codeShiftG, codeShiftB,
                      deLinearize );

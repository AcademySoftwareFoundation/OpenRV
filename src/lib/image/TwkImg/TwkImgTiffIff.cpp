        ...
        case 32:
            for( int y = 0; y < height; ++y )
            {
                ...
                if( hasAlpha )
                {
                    ...
                    (*dstPixel).x = (unsigned char)
                        ( TwkMath::clamp( (*srcPixel).x * 255.0f, 0.0f, 255.0f ) );
                    (*dstPixel).y = (unsigned char)
                        ( TwkMath::clamp( (*srcPixel).y * 255.0f, 0.0f, 255.0f ) );
                    (*dstPixel).z = (unsigned char)
                        ( TwkMath::clamp( (*srcPixel).z * 255.0f, 0.0f, 255.0f ) );
                    (*dstPixel).w = (unsigned char)
                        ( TwkMath::clamp( (*srcPixel).w * 255.0f, 0.0f, 255.0f ) );
                }
                else
                {
                    Col3f *srcPixel = (Col3f *)buf;
    ...
    switch( depth )
    {
        ...
        case 16:
            for( int y = 0; y < img->height(); ++y )
            {
                ...
                for( int x = 0; x < img->width(); ++x, dstPixel++, ++srcPixel )
                {
                    (*dstPixel).x = (unsigned short)
                        ( TwkMath::clamp( (*srcPixel).x * 65535.0f, 0.0f, 65535.0f ) );
                    (*dstPixel).y = (unsigned short)
                        ( TwkMath::clamp( (*srcPixel).y * 65535.0f, 0.0f, 65535.0f ) );
                    (*dstPixel).z = (unsigned short)
                        ( TwkMath::clamp( (*srcPixel).z * 65535.0f, 0.0f, 65535.0f ) );
                    (*dstPixel).w = (unsigned short)
                        ( TwkMath::clamp( (*srcPixel).w * 65535.0f, 0.0f, 65535.0f ) );
                }
                if( TIFFWriteScanline( tif, buf, y, 0 ) == -1 )
                {
                    IffExc exc( "Error while writing TIFF scanline" );
...
static bool TiffIff_write_uc( const Img4uc *img, const char *fileName,
                              const Mat44f *Mcamera,
                              const Mat44f *Mscreen )
{
    TIFF* tif = TIFFOpen( fileName, "w" );
    if( tif == NULL )
    {

...
Img4f *ExrIff::read( const char *imgFileName )
{
    ...
    file.setFrameBuffer( &exrBuffer[0][0] - dw.min.x - dw.min.y * width, 
                         1, width );
                     ...
                     TwkMath::Mat44f &Mscreen )
{
    ...
    const Imf::M44fAttribute *w2cAttr =
        file.header().
        findTypedAttribute<Imf::M44fAttribute>( "worldToCamera" );
    if ( w2cAttr != NULL )
    {
    ...
    const Imf::M44fAttribute *w2sAttr =
        file.header().
        findTypedAttribute<Imf::M44fAttribute>( "worldToScreen" );
    if ( w2sAttr != NULL )
    {
        // Note that we're NOT reading it TRANSPOSED!!!
        for( int i = 0; i < 4; ++i )

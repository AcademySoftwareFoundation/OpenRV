...
public:
    ...
    MipMap( const TwkImg::Image<COLOR> &img,
            int maxLods = MAX_NUM_LODS,
...
protected:
    ...
    void specialCaseSquare( Image<COLOR> &srcImg,
                            Image<COLOR> &dstImg );
    void specialCaseRectangle( Image<COLOR> &srcImg,
                               Image<COLOR> &dstImg );
...
//*****************************************************************************
template <class COLOR>
static void gammaImg( Image<COLOR> &img, float g )
{
...
//******************************************************************************
template <class COLOR>
MipMap<COLOR>::MipMap( const Image<COLOR> &img, int maxLods,
                       float workGamma )
{
          ...
          m_numLods++ )
    {
        ...
        COLOR *dstRowLast = 
            dstRow + ( dstStride * ( dstImg->height() - 1 ) );
              ...
              src0 += 2, src1 += 2, src2 += 2, src3 += 2, dst++ )
        {
            // src0 is invalid.
            (*dst) = 
                ( src3[0] + src3[3] ) * corner4x3 +
                ( src1[0] + src1[3] +
                  src2[0] + src2[3] + src3[1] + src3[2] ) * edge4x3 +
                  ...
                  dstRow += dstStride ) // Increment
        {
            ...
            // Index 0 is invalid.
            (*dst ) =
                ( src0[3] + src3[3] ) * corner4x3 +
                ( src0[1] + src0[2] + src1[3] +
                  src2[3] + src3[1] + src3[2] ) * edge4x3 +
                ( src1[1] + src1[2] + src2[1] + src2[2] ) * center4x3;
                  ...
                  src0 += 2, src1 += 2, src2 += 2, src3 += 2, dst++ )
            {
                ...
                (*dst) = 
                    ...
                    ( src0[1] + src0[2] + src1[0] + src1[3] +
                      src2[0] + src2[3] + src3[1] + src3[2] ) * edge4x4 +
            ...
            if ( wOdd )
            {
                ...
                (*dst) = 
                    ( src0[0] + src3[0] ) * corner4x2 +
                    ( src0[1] + src1[0] +
                      src2[0] + src3[1] ) * edge4x2 +
            ...
            else
            {
                ...
                (*dst) = 
                    ...
                    ( src0[1] + src0[2] + src1[0] +
                      src2[0] + src3[1] + src3[2] ) * edge4x3 +
                    ( src1[1] + src1[2] + src2[1] + src2[2] ) * center4x3;
            }
        }
...
//******************************************************************************
template <class COLOR>
inline int MipMap<COLOR>::numLods() const
{
    return m_numLods;
}
...
//******************************************************************************
template <class COLOR>
inline Image<COLOR> *MipMap<COLOR>::lod( int d )
{
...
//******************************************************************************
template <class COLOR>
void MipMap<COLOR>::specialCaseSquare( Image<COLOR> &src,
                                       Image<COLOR> &dst )
{
    ...
    if ( srcWidth == 3 )
    {
        if ( srcHeight == 3 )
        {
            dst.pixel(0,0) =
                ( src.pixel(0,0) + src.pixel(1,0) +
                  src.pixel(0,1) + src.pixel(1,1) ) * center3x3 +

                ( src.pixel(0,2) + src.pixel(1,2) +
                  src.pixel(2,0) + src.pixel(2,1) ) * edge3x3 +
        ...
        else
        {
            ...
            dst.pixel(1,0) =
                ( src.pixel(2,0) ) * center2x1 +
                ( src.pixel(1,0) ) * edge2x1;
        }
    }
    else
    {
        ...
        {
            dst.pixel(0,0) =
                0.25f * ( src.pixel(0,0) + src.pixel(1,0) +
        ...
        else
        {
            ...
            dst.pixel(0,0) =
                0.5f * ( src.pixel(0,0) + src.pixel(1,0) );
        }
    }
                                          ...
                                          Image<COLOR> &dstImg )
{
    ...
    if ( srcWidth == 3 )
    {
              ...
              dst += dstStride, tmp += tmpStride2 )
        {
            ...
            dst[0] = ( tmp[2] + tmp[4] ) * center4x1 +
                ( tmp[0] + tmp[6] ) * edge4x1;
            dst[1] = ( tmp[3] + tmp[5] ) * center4x1 +
                ( tmp[1] + tmp[7] ) * edge4x1;
        }
        ...
        else
        {
            ...
            dst[0] = ( tmp[2] + tmp[4] ) * center3x1 +
                ( tmp[0] ) * edge3x1;
            dst[1] = ( tmp[3] + tmp[5] ) * center3x1 +
                ( tmp[1] ) * edge3x1;
        }

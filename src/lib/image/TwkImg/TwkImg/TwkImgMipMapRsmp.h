...
public:
    ...
    float sample( const MipMap<COLOR> &mip,
    ...
    float sampleAbs( const MipMap<COLOR> &mip,
                     const TwkMath::Vec2f &loc,
                     float minSampRad,
                     float maxSampRad,
                     float lodBias,
                              ...
                              float rad )
{
    ...
    TwkMath::Box2f bBox( TwkMath::Vec2f( ( float )( bnds.min[0] ), 
                                         ( float )( bnds.min[1] ) ) - 0.5f,
                         TwkMath::Vec2f( ( float )( bnds.max[0] ), 
                                         ( float )( bnds.max[1] ) ) + 0.5f );
...
//******************************************************************************
template <class COLOR, class MIN_RSMP, class MAX_RSMP>
inline MmLinRsmp<COLOR, MIN_RSMP, MAX_RSMP>::MmLinRsmp( 
    MIN_RSMP &minRsmp, MAX_RSMP &maxRsmp )
  : m_minRsmp( minRsmp ),
    m_maxRsmp( maxRsmp )
{
    // Nothing
...
float MmLinRsmp<COLOR, MIN_RSMP, MAX_RSMP>::sample( 
    const MipMap<COLOR> &mip, const TwkMath::Vec2f &loc,
    float minSampRad, float maxSampRad, float lodBias, COLOR &samp ) const
{
    ...
    {
        const float locCov = 
            m_maxRsmp.sample( *( mip.lod( 0 ) ), loc, samp );
        if ( locCov != 0.0f )
        {
            samp *= cov / locCov;
            return cov;
    ...
    if ( lod <= 0.0f || ( mip.numLods() < 2 ) )
    {
        ...
        const float lod0cov = m_maxRsmp.sample( *( mip.lod( 0 ) ),
                                                loc, samp );
        if ( lod0cov > 0.0f )
        {
            samp /= lod0cov;

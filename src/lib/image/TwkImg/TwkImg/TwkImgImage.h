...
public:
    ...
    Image( const vec_loc_type &orig,
           const vec_size_type &sze );
    ...
    // The tile, in relative coordinates, in the SRC image that will be copied
    // from is ( srcOffset, srcOffset + dstTile.size() - 1 ).
    void copyRect( const Image<COLOR> &src, 
                   const vec_size_type &srcOffset,
                   const bounds_type &dstTile,
                   bool black = true );
    ...
    // is represented by dstTile.
    // The tile, in absolute coordinates, in the SRC image that will be copied
    // from is ( srcOffset, srcOffset + dstTile.size() - 1 ).
    void copyRectAbs( const Image<COLOR> &src,
                      const vec_loc_type &srcOffset,
                      const bounds_type &dstTile,
                      bool black = true );
...
//******************************************************************************
template <typename C>
void Image<C>::initPixels(C* pixels)
{
    if ( m_size[0] <= 0 ||
         m_size[1] <= 0 )
    {
...
Image<C>::Image( const typename Image<C>::bounds_type &bnds )
  : m_size( bnds.size() ),
    m_bounds( Image<C>::vec_loc_type( 0, 0 ), 
              Image<C>::vec_loc_type( bnds.size( 0 ) - 1,
                                      bnds.size( 1 ) - 1 ) ),
    m_boundsAbs( bnds ),
    m_pixels( NULL ),
    m_mypixels(true)
...
template <typename C>
Image<C> &Image<C>::operator=( const Image<C> &copy )
{
    if ( (&copy) != this )
    {
        delete[] m_pixels;
...
//******************************************************************************
template <typename C>
inline const C &Image<C>::operator()( const typename Image<C>::vec_size_type &loc ) const
{
...
//******************************************************************************
template <typename C>
inline const C *Image<C>::operator[]( const typename Image<C>::size_type y ) const
{
...
//******************************************************************************
template <typename C>
inline const C &Image<C>::pixel( const typename Image<C>::vec_size_type &loc ) const
{
    assert( loc[0] >= 0 && loc[0] < m_size[0] &&
            loc[1] >= 0 && loc[1] < m_size[1] );
...
//******************************************************************************
template <typename C>
inline const C &Image<C>::pixelAbs( const typename Image<C>::vec_loc_type &loc ) const
{
...
//******************************************************************************
template <typename C>
inline const C *Image<C>::pixels() const
{
    return m_pixels;
}
...
//******************************************************************************
template <typename C>
inline void Image<C>::clearRectAbs( const typename Image<C>::bounds_type &rect )
{
...
//******************************************************************************
template <typename C>
inline void Image<C>::fill( const C &fillCol )
{
...
//******************************************************************************
template <typename C>
void Image<C>::fillRect( const typename Image<C>::bounds_type &rect, const C &fillCol )
{
    // Fix rect.
    bounds_type irect = TwkMath::intersection( m_bounds, rect );
    if ( irect.isEmpty() )
...
//******************************************************************************
template <typename C>
inline void Image<C>::fillRectAbs( const typename Image<C>::bounds_type &rect, 
                                   const C &fillCol )
{
...
//******************************************************************************
template <typename C>
void Image<C>::copyRect( const Image<C> &src,
                         const typename Image<C>::vec_size_type &srcOffset,
                         const typename Image<C>::bounds_type &uncheckedDstTile,
                         bool black )
{
    ...
    for ( ; dstRow < blackTStart; dstRow += stride() )
    {
        ...
        // do dstTile
        memcpy( ( void * )dstCol, ( const void * )srcRow, 
                dstTileSize * sizeof( C ) );
        dstCol += dstTileSize;
        srcRow += src.stride();
...
//******************************************************************************
template <typename C>
inline void Image<C>::copyRectAbs( const Image<C> &src,
                                   const typename Image<C>::vec_loc_type &srcOffset,
                                   const typename Image<C>::bounds_type &uncheckedDstTile,
                                   bool black )
{
                ...
                PIXELFUNC &pfunc )
{
    ...
    C *blackTStart = &( dst.pixel( irect.min.x, dstTile.max.y ) ) + 
        dst.stride();
...
//******************************************************************************
template <typename C, class PIXELFUNC>
inline void applyRectAbs( Image<C> &dst,
                          const Image<C> &src,
                          const typename Image<C>::vec_loc_type &srcOffset,
                          const typename Image<C>::bounds_type &uncheckedDstTile,
                          PIXELFUNC &pfunc )

...
//******************************************************************************
SgiIffBase::Header::Header()
{
    memset( ( void * )this, 0, sizeof( Header ) );
}

//******************************************************************************
SgiIffBase::Header::Header( int W,
                            int H,
                            int NC )
{
    assert( W > 0 );
...
//******************************************************************************
void SgiIffBase::readPixelsRLE( Img4uc &img,
                                SgiIffBase::Header &header,
                                std::istream &stream )
{
...
//******************************************************************************
void SgiIffBase::readPixels( Img4uc &img,
                             SgiIffBase::Header &header,
                             std::istream &stream )
{
    // Allocate a scanline
...
//******************************************************************************
void SgiIffBase::writePixels( const Img4uc &img,
                              SgiIffBase::Header &header,
                              std::ostream &stream )
{
    // Allocate a scanline
...
//******************************************************************************
void SgiIff4uc::write( const Img4uc *img,
                       std::ostream &imgOutputStream )
{
    // Verify
    assert( img != NULL );
...
//******************************************************************************
void SgiIff4f::write( const Img4f *img,
                      std::ostream &imgOutputStream )
{
    // Verify
    assert( img != NULL );
    ...
    for ( ; dstPixel < dstEnd; ++srcPixel, ++dstPixel )
    {
        (*dstPixel).x = ( unsigned char )
            (std::clamp((*srcPixel).x, 0.0f, 1.0f)*255.0f);
        (*dstPixel).y = ( unsigned char )
            (std::clamp((*srcPixel).y, 0.0f, 1.0f)*255.0f);
        (*dstPixel).z = ( unsigned char )
            (std::clamp((*srcPixel).z, 0.0f, 1.0f)*255.0f);
        (*dstPixel).w = ( unsigned char )
            (std::clamp((*srcPixel).w, 0.0f, 1.0f)*255.0f);
    }

...
#define DB(x)           if (DB_GENERAL & DB_LEVEL) cerr << dec << "MFBWriter: " << x << endl
#define DBL(level, x)   if (level & DB_LEVEL)      cerr << dec << "MFBWriter: " << x << endl
#else
#define DB(x)    
#define DBL(level, x)
#endif
...
{
  public:
    enum State { Ready, InProgress, Finished };
...
class WriteTaskManager
{
  ...
  private:
    ...
    // Note: Lock the managerLock before reading/writing currentlyAddingATask
...
{ 
    ...
    for (int i = 0; i < size; ++i) threadData.push_back (ThreadData (this, i));
...
void
WriteTaskManager::lock()
{
    threadGroup.lock (managerLock);
}

void
WriteTaskManager::unlock()
{
    threadGroup.unlock (managerLock);
}

int
WriteTaskManager::nextReadyTaskIndex()
{
...
void
WriteTaskManager::finishTask (int index)
{
    DB ("finishTask " << index);
...
const WriteTask &
WriteTaskManager::taskByIndex (int index)
{
    return tasks[index];
}

void
WriteTaskManager::threadMain (int threadNumber)
{
    ...
    {
        // If there is no more tasks to process but that a new task is currently
        // being added, then we do not want to exit the thread function otherwise
        // this could potentially lead to a thread synchronization issue.
        ...
        try { TwkFB::GenericIO::writeImages (t.fbs, t.filename, t.request); }
        ...
        for (int i=0; i < t.fbs.size(); i++) delete t.fbs[i];
...
void
WriteTaskManager::addTask (WriteTask &t)
{
    ...
    while (! addedTask)
    {
        ...
        if (! addedTask)
        {
            DB ("addTask waiting");
            //
            //  Wait for worker thread to signal that there's room for more tasks
            //
            ...
            if (!stl_ext::thread_group::wait_cond_time(waitCond, managerLock, waitCondTimeOutInSecs * 1e6))
            {
                ...
                // Note that this is to compensate for an inherent race condition 
                // that can occur with this thread_group implementation where the 
                // WriteTaskManager::threadMain() might have exited because there 
                // was no more tasks to process but before the thread_group 
                // registers it as being finished (thread_group::_num_finished) a
                // new task is added by WriteTaskManager::addTask() and the 
                // dispatchThreads() ending up as a noop because it is seen as
                // not finished yet.
                dispatchThreads();
            }
        }
...
void
WriteTaskManager::dispatchThreads()
{
...
bool
MovieFBWriter::write(Movie* inMovie,
                     ...
                     WriteRequest& writeRequest)
{
    ...
    if (writeRequest.timeRangeOverride)
    {
        ...
        copy(writeRequest.frames.begin(),
             writeRequest.frames.end(),
             frames.begin());
    }
    else
    {
    ...
    const bool hasPatterns = splitSequenceName(imagePattern, 
                                               timeStr,
                                               sequencePattern);
    if (hasPatterns)
    {
        ...
        for (unsigned int i=0; i < frames.size(); i++)
        {
            ...
            if (writeRequest.views.size()) request.views = writeRequest.views;
            ...
            if (verbose)
            {
                cout << "INFO: writing frame " << f 
                     << " (" << int(float(i) / float(frames.size()-1) * 10000.0) / float(100.0) << "% done)"
                     << endl;
            }
    ...
    {
        //
        //  Its just one output frame so the input range is kind of pointless
        //
        ...
        if (verbose) cout << "INFO: writing " << imagePattern << endl;

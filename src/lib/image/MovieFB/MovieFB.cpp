...
#define DB(x)           if (DB_GENERAL & DB_LEVEL) cerr << "MovieFB: " << x << endl
#define DBL(level, x)   if (level & DB_LEVEL)      cerr << "MovieFB: " << x << endl
#else
...
MovieFB::clone() const
{
    ...
    if (m_filename != "") m->open(m_filename, m_info);
    return m;
}

bool
MovieFB::getImageInfo(const std::string& filename)
{
    ...
    bool success = false;
    if (const FrameBufferIO* imgio = TwkFB::GenericIO::findByExtension(extension(m_imagePattern),
                                                      FrameBufferIO::ImageRead))
    {
        try
        {
...
void
MovieFB::open(const string &filename, const MovieInfo &info,
    const Movie::ReadRequest& request)
{
    ...
    // 
    // Attempt to find a file with correct Image info.  Given that the first frame
    // can be a slate, we attempt to extract the movie info from the following frames
    // 1. the 2nd frame
    ...
    for (int t = 0; t < 4 && !infoSuccess; ++t)
    {
            ...
            existingFiles[testFrame].exists) 
        {
            ...
            if (infoSuccess) successFrame = testFrame;
        }
    }
    ...
    // Assume the first frame is a slate if it is different than the rest of the 
    // sequence. Note that we need to make sure that we are not misinterpreting
    // a sequence of images having different bounding boxes as a slate so this
    // is why we are NOT using the == operator here.
    if (infoSuccess)
    {
        const bool sameInfo = 
                ...
                ( m_imgInfo.viewInfos.empty()
                  ||
                  (m_imgInfo.viewInfos.front().name == m_info.viewInfos.front().name) );
        if(!sameInfo)
        {
    ...
    if (m_info.proxy.hasAttribute("DPX-MP/FrameRate"))
    {
        ...
        if (m_info.fps) m_info.proxy.newAttribute<float>("FPS", m_info.fps);
    }
...
bool
MovieFB::fileAndIdAtFrame(int& frame,
                          string& filename,
                          ostream& idstream,
                          bool nearby)
{
    ...
    if (frame < m_frames.front()) frame = m_frames.front();
    if (frame > m_frames.back())  frame = m_frames.back();
    ...
    if (f == m_frameMap.end())
    {
        if (nearby)
        {
            ...
            FrameMap::const_iterator n = 
                lower_bound(m_frameMap.begin(),
                            m_frameMap.end(),
                            FrameMap::value_type(frame, FrameFile("")),
                            cffp);
...
void
MovieFB::imagesAtFrame(const ReadRequest& mrequest, FrameBufferVector& fbs)
{
    updateFrameInfo();
...
void
MovieFB::identifiersAtFrame(const ReadRequest& request, IdentifierVector& ids)
{
    //
    ...
    //  tries to read the (channel * layer * view) images (including the default view
    //  and layer cases) in the sense that detectable illegal combinations
    ...
    {
        if (!request.missing) TWK_THROW_EXC_STREAM("id failed");
    }

    string baseid = idstr.str() + "/";

    for (unsigned int i = 0; 
         (i == 0 && request.views.empty()) || i < request.views.size(); 
          i++)
    {
        ostringstream id;

        const string& viewName = (m_info.hasSlate && request.frame == m_info.start) ?
...
void
MovieFB::updateFrameInfo()
{
    if (m_frameInfoValid) return;
    ...
    //
    // If this is one on disk frame simply snag it and guess the frame number
    //
    {
        TwkUtil::FrameList frames = frameRange(timeStr);
        if (frames.size() > 0) m_frames.push_back(frames[0]);
        else                   m_frames.push_back(1);
    ...
    //
    //  If we just have one frame/file and it really exists, just add it with
    //  the frame number we got from frameRange() above.  Otherwise use the
    //  frame numbers reported by existingFilesInSequence().
    //
    ...
    for (FrameMap::iterator i = m_frameMap.begin(); i != m_frameMap.end(); ++i)
    {
        ...
        if (oldI != oldFrameMap.end() && oldI->second.fileName == i->second.fileName)
        {
            ...
            //  If the file has been written since we last read it, inc generation.
            //  Note that we don't even check if we've never read this file.
...
MovieFBIO::MovieFBIO() : MovieIO("Frame Sequence", "u")
{
         ...
         ++i)
    {
        ...
        for (unsigned int q=0; q < infos.size(); q++)
        {
            ...
            if (info.capabilities & FrameBufferIO::ImageRead) capabilities |= MovieIO::MovieRead; 
            if (info.capabilities & FrameBufferIO::ImageWrite) capabilities |= MovieIO::MovieWrite;
            if (info.capabilities & FrameBufferIO::BruteForceIO) capabilities |= MovieIO::MovieBruteForceIO; 
            ...
            addType(info.extension, 
                    info.description,
                    capabilities,
                    info.compressionSchemes,
                    audioCodecs,
                    dparams,
                    eparams);
        }
...
std::string 
MovieFBIO::about() const
{
    return "Tweak Image I/O Movie Sequence";
}

MovieReader* 
MovieFBIO::movieReader() const
{
    ++m_readerCount;
    return new MovieFB();
}
...
MovieWriter* 
MovieFBIO::movieWriter() const
{
    return new MovieFBWriter();
}

void
MovieFBIO::getMovieInfo(const std::string& filename, MovieInfo& minfo) const
{

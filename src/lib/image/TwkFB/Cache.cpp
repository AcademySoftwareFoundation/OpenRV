...
#define DB(x)           if (DB_GENERAL & (DB_LEVEL)) cerr << dec << "TwkFB::Cache: " << x << endl
#define DBL(level, x)   if ((level) & (DB_LEVEL))      cerr << dec << "TwkFB::Cache: " << x << endl
#else
#define DB(x)    
#define DBL(level, x)
#define DB_LEVEL 0x0
...
void
TrashCan::remove (FrameBuffer* fb)
{
...
bool
TrashCan::contains (FrameBuffer* fb)
{
    TrashMap::iterator tmi = trashMap.find(fb);
...
Cache::Cache() 
    : m_full(false),
      m_maxBytes(1024 * 1024 * 250),    // default is 250Mb
      m_currentBytes(0),
      m_retrieveTime(0)
{
...
void
Cache::setMemoryUsage(size_t m)
{
    //  cerr << "Cache::setMemoryUsage before m " << m << " " << m_maxBytes << endl;
    //
    //  With 32-bit vm, we can't reliably use more than 2.5GB in the
    //  cache, so trim it here.
    //
    if (4 == sizeof(void*)) m = min(m,size_t(2560)*size_t(1024*1024));
    m_maxBytes = m;
...
bool
Cache::flush(const IDString& idstring)
{
...
bool
Cache::isCached(const IDString& idstring) const
{
...
FrameBuffer*
Cache::checkOut(const IDString& idstring) 
{
    FrameBuffer* fb = 0;
    ...
    {
        if (Cache::debug()) cout << "CACHE: missed " << fb << " : " << idstring << endl;
    }
...
Cache::checkOut(FrameBuffer* fb)
{
    ...
    if (!fb->inCache())
    {
        TWK_THROW_STREAM(CacheMismatchException,
                         "fb " << fb << " : " 
                         << fb->identifier() << " is not in cache " << this
                         << ", lock = " << fb->m_cacheLock
                         << ", refs = " << fb->m_cacheRef);
    }

    if (Cache::debug()) cout << "CACHE: checking out " << fb << " : " << fb->identifier() << endl;
...
Cache::checkIn(FrameBuffer* fb)
{
    ...
    if (Cache::debug()) cout << "CACHE checking in " << fb << " : " << fb->identifier() << endl;
    unlock(fb);
    dereferenceFB(fb);
}
...
int
Cache::trashCount() const
{
    return m_trashCan->size();
}

bool
Cache::freeTrash (size_t bytes)
{
    //DBL (DB_TCFREE, "freeTrash() bytes " << bytes << " trashCount " << trashCount());
...
Cache::freeAllTrash ()
{
    return freeTrash (m_currentBytes);
}

bool
Cache::free(size_t bytes)
{
    //  XXX
    //
    ...
    */
    ...
    for (int i=0; i < numfbs; i++, it++) fbs[i] = it->second;

    size_t targetBytes = m_maxBytes - bytes;

    for (int i=0; 
         i < fbs.size() && (m_currentBytes > targetBytes);
         i++)
    {
        partial_sort(fbs.begin() + i, 
                     fbs.end(), 
                     fbs.begin() + (i + 1), 
        ...
        {
            if (Cache::debug()) cout << "CACHE: freeing " << fb->allocSize() 
                              << " of needed " << bytes
                              << ", fb = " << fb->identifier()
                              << endl;
    ...
    if (m_currentBytes > m_maxBytes) return false;
    return m_maxBytes - m_currentBytes >= bytes;
}


bool
Cache::add(FrameBuffer* fb, bool force)
{
    DB ("add() fb " << fb << " force " << force);
    bool ok = true;
    if (fb->inCache()) return ok;

    size_t bytes = fb->totalImageSize();

    //  cerr << "Cache::add " << bytes << " to " << m_currentBytes << " of " << m_maxBytes << " full " << m_full << endl;

    if ((bytes + m_currentBytes) > m_maxBytes && !free(bytes))
    {
        m_full = (m_currentBytes >= m_maxBytes);
        if (!force) return false;
        if (Cache::debug()) cout << "INFO: forcing " << fb->identifier() << endl;
        ok = false;
    }
    ...
    if (i != m_map.end())
    {
        ...
        {
            if (Cache::debug()) cout << "WARNING: CACHE: removing cache locked fb" << endl;
        }
        
        if (Cache::debug()) cout << "INFO: CACHE: removing old " << fb->identifier() << endl;
        deleteFB( i->second );
    }

    m_map[fb->identifier()] = fb;
    if (Cache::debug()) cout << "CACHE: added " << fb->identifier() << endl;
...
FrameBuffer*
Cache::recycledFB(size_t allocSize[4])
{
    return new FrameBuffer();
}

void 
Cache::referenceFB(FrameBuffer* fb)
{ 
    fb->m_cacheRef++; 
    DBL (DB_REF, "referenceFB: " << fb << " " << fb->identifier() << ", ref count now " << fb->m_cacheRef);
    if (fb->m_cacheRef > 1) m_trashCan->remove(fb);
...
void 
Cache::dereferenceFB(FrameBuffer* fb)
{ 
    if (fb->m_cacheRef > 0) fb->m_cacheRef--; 
    ...
    //
    if (fb->m_cacheRef <= 0) flush(fb->identifier());
    if (fb->m_cacheRef == 1) m_trashCan->add(fb);
}

bool
Cache::trashContains(FrameBuffer* fb)
{
    return m_trashCan->contains(fb);
}

// This function handles the presence of "proxy buffers" (FrameBuffer) in the
// cache. Those buffers have a data pointer that refers to another buffer's data 
// (eg.: tiles). When the cache wants to delete a FrameBuffer, instead of simply
// calling 'delete' on it, it will use this function. The function will then
// detect if the buffer is a proxy buffer or a master buffer (ie.: a buffer 
// referred-to by proxy buffers).
//
...
//
// If the buffer is a proxy buffer, it will unregister itself from the list of
// proxy buffers kept by its master buffer.
//
// The function returns the number of deleted bytes, including proxy buffers,
// so the cache can properly keep trace of the allocated memory.
...
size_t Cache::deleteFB(FrameBuffer* fb )
{
    ...
    if ( fb )
    {
        ...
        if (const TwkFB::TypedFBVectorAttribute<FrameBuffer*>* proxyBuffersAtt =
                dynamic_cast<const TwkFB::TypedFBVectorAttribute<FrameBuffer*>*>(
                    fb->findAttribute("ProxyBuffers")))
        {
                 ...
                 proxyBufferIndex++ )
            {
                ...
                if ( proxyBuffer )
                {
                    ...
                    // If the proxy buffer is not in the cache map, it has been
                    // deleted prior the master buffer.
                    if (i != m_map.end())
                    {
                        m_map.erase(i);

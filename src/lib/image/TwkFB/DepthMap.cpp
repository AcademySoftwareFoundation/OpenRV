...
static void
parse(Mat44f& M, const string& value)
{
    stringstream str;
...
float 
DepthMap::depthAtNDC(float ndcx, float ndcy) const
{
...
Vec2f
DepthMap::gradAtNDC(float ndcx, float ndcy) const
{
    const float x = ndcx * float(width()-1);
    const float y = ndcy * float(height()-1);
...
Vec2f
DepthMap::gradAtPixel(size_t x, size_t y) const
{
    size_t x0 = x > 0 ? x-1 : x;
...
float 
DepthMap::depthAtWorldPoint(Point p) const
{
...
float 
DepthMap::depthOfWorldPoint(Point p) const
{
    return (m_worldToCamera * p).z;
}

float 
DepthMap::depthAtPixel(size_t x, size_t y) const
{
    float p[4];
    if (x >= m_fb->width()) x = m_fb->width() - 1;
    if (y >= m_fb->height()) y = m_fb->height() - 1;
...
float 
DepthMap::relativeDepthAtWorldPoint(Point p) const
{

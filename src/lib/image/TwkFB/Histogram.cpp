...
template <typename T>
void
floatScanlineHistogram(const T* scanline, 
                       size_t len, 
                       size_t nchannels,
                       ...
                       float maxRange)
{
    ...
    for (FBHistorgram::iterator i = hbegin;
         i != hend;
         ++i, index++)
    {
...
template <typename T>
void
integralScanlineHistogram(const T* scanline, 
                          size_t len, 
                          size_t nchannels,
                          FBHistorgram::iterator hbegin,
                          FBHistorgram::iterator hend)
{
         ...
         ++i, index++)
    {
        ...
        for (const T* p = scanline + index; p < e; p += nchannels)
        {
            ...
            if (h.max < fv) h.max = fv;
            if (h.min > fv) h.min = fv;
        }
    }
}
...
template <typename T>
MinMaxPair
histogram(const FrameBuffer* fb, 
          FBHistorgram::iterator& hbegin,
          FBHistorgram::iterator& hend,
          float minRange = 0.0,
          float maxRange = 1.0)
{
    for (size_t i = 0; i < fb->height(); i++)
    {
        ...
        {
            integralScanlineHistogram(scanline, 
                                      fb->width(),
                                      fb->numChannels(),
                                      hbegin, hend);
        }
        ...
        {
            floatScanlineHistogram(scanline, 
                                   fb->width(),
                                   fb->numChannels(),
                                   hbegin, hend,
                                   minRange, maxRange);
...
MinMaxPair
computeChannelHistogram(const FrameBuffer* fb,
                        FBHistorgram& h,
                        size_t bins,
                        bool fullRangeOverOne)
{
    if (fb->isPlanar())
    {

...
{
  public:
    FBAttribute(const std::string& name ) : m_name(name) {}
    virtual ~FBAttribute() {}
...
{
  public:
    DataContainerAttribute(std::string name,
                           const void* memory,
                           size_t size)
        : FBAttribute(name),
          m_container(size)
...
template<typename T>
class TypedFBAttribute : public FBAttribute 
{
  public:
...
TypedFBAttribute<T>::TypedFBAttribute(const std::string& name, T value)
    : FBAttribute(name), m_value(value) {}
...
template<typename T> 
TypedFBAttribute<T>::~TypedFBAttribute() {}

template<typename T> 
FBAttribute* TypedFBAttribute<T>::copy() const
{
    return new TypedFBAttribute<T>(name(), m_value);
}

template<typename T> 
FBAttribute* TypedFBAttribute<T>::copyWithPrefix(const std::string& prefix) const
{
...
template<typename T>
std::string
TypedFBAttribute<T>::valueAsString() const
{
...
template<typename T>
class TypedFBVectorAttribute : public FBAttribute 
{
public:
    typedef std::vector<T> TVector;

    TypedFBVectorAttribute(const std::string& name,
                           const TVector& value) 
        : FBAttribute(name),
          m_value(value) {}
...
template<typename T> 
TypedFBVectorAttribute<T>::~TypedFBVectorAttribute() {}

template<typename T> 
FBAttribute* 
TypedFBVectorAttribute<T>::copy() const
{
...
template<typename T>
std::string
TypedFBVectorAttribute<T>::valueAsString() const
{
    std::ostringstream str;

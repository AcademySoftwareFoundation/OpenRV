...
//
//  The Primaries attribute is not used directly, it indicates the name of
//  the color space primaries. E.g, Rec709. Images with non-Rec709
//  primaries should include the actual values in
//  Red/Green/Blue/WhitePrimaries attributes. The name becomes useful in
//  conjunction with the actual values when there are multiple values for
//  the primaries. By providing the name, users can see the intended space
//  -- reasonable color people (!?) may differ in their interpretation of
//  the exact primary values (for historic or histrionic reasons).
//  
//  The TransferFunction indicates which non-linear (or not) function
//  should be used to convert to linear. In the case of data thats already
//  linear (e.g., exr) the value should be ColorSpace::Linear or
//  ColorSpace::Identity. If TransferFunction is not defined its assumed to
//  be ColorSpace::Identity.
//
//  Conversion is the name (like Rec709) of the conversion matrix that goes
//  from YCbCr->RGB. This is similar to the Primaries attr in that it is
//  not directly used. ConversionMatrix holds the actual matrix. The value
//  Rec601Full is used exclusively as a conversion.
//
//  Note that these are tags on the FB there is no transform applied to the
//  pixels when a tag is set on the FB.
//
...
class TWKFB_EXPORT FrameBuffer 
{
  public:
    ...
    //  
    //  Note: Using that constructor, the FrameBuffer instance will manage the data buffer.
    //

    FrameBuffer( CoordinateTypes coordinateType,
    ...
    void restructure(int width                          = 640,
                     int height                         = 480,
                     int depth                          = 0,
                     int numChannels                    = 3, 
    ...
    void restructurePlanar(int width, 
                           int height,
    ...
    FrameBuffer& operator= (const FrameBuffer& fb) { copyFrom(&fb); return *this; }
    ...
    bool        needsUncrop() const { return m_uncrop && (m_uncropWidth != m_width || m_uncropHeight != m_height ||
    ...
    void setPixelAspectRatio(float pa) { m_pixelAspect = pa; attribute<float>("PixelAspectRatio") = pa; }
    ...
    //
    // Optionally, you can request an individual channel from a pixel, like so:
    //
    //     img->pixel<float>( x, y, 1 )
    //
    // Note that because the pixel function returns a reference, it doesn't 
    // like to be templated to a pointer--this is bad: img->pixel<char *>(x,y)
    // Use this instead:  &img->pixel<char>(x,y)
    //
    ...
    template<typename T> 
    void removeChannelByType( int channel );
    ...
    //  The insertion operation checks if the new attribute is equivalent to an
    //  attribute already in the container, and if so, removes the old one before
    //  adding the new one.
    //
    ...
    template<typename T> 
    TypedFBAttribute<T>* newAttribute(const std::string& name,
                                      T value );
    ...
    void setPrimaries(float xWhite, float yWhite,
    ...
    void setMatrixAttribute(std::string name,
...
template<typename T>
inline T &FrameBuffer::pixel( int x, int y, int c )
{
...
template<typename T>
inline T *FrameBuffer::scanline( int y )
{
...
template<typename T>
inline const T *FrameBuffer::scanline( int y ) const
{
    assert( y >= 0 && y < m_slopHeight );
...
template<typename T> 
inline const T *FrameBuffer::pixels() const
{
...
template<typename T> 
inline const T *FrameBuffer::end() const
{
    size_t off = m_scanlinePaddedSize * m_height;
...
template<typename T>
void FrameBuffer::removeChannelByType(int channel )
{
...
template<typename T> 
const T& 
FrameBuffer::attribute(const std::string& name) const
{
...
template<typename T> 
T& FrameBuffer::attribute(const std::string& name) 
{
...
template<typename T> 
std::vector<T>& 
FrameBuffer::vectorAttribute(const std::string& name) 
{
    if (FBAttribute* a = findAttribute(name))
    {
...
template<typename T>
TypedFBAttribute<T>*
FrameBuffer::newAttribute(const std::string& name, T value)
{
    if (FBAttribute* old = findAttribute(name)) deleteAttribute(old);
...
FrameBuffer::newAttribute(const std::string& name, const std::vector<T>& value)
{
    if (FBAttribute* old = findAttribute(name)) deleteAttribute(old);

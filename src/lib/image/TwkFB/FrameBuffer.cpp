...
#define SWAPSHORT(x) ((uint16_t)((((uint16_t)(x) & 0xff00) >> 8) |\
                      (((uint16_t)(x) & 0x00ff) << 8)))
...
FrameBuffer::FrameBuffer(const FrameBuffer& fb) : m_staticRef(0)
{
    //cout << "FB COPY CONSTRUCT: " << this << endl;
    copyFrom(&fb);
}

FrameBuffer::FrameBuffer( CoordinateTypes coordinateType,
      ...
      m_staticRef(0)
{
    ...
    restructure( width, height, depth,
                 channelNames->size(), 
                 dataType, data, 
                 channelNames,
                 BOTTOMLEFT, 
                 true,  /* Manage the data buffer */
                 0,     /* No extra lines */
                 0 );   /* No extra pixels */
}
      ...
      m_staticRef(0)
{
    ...
    restructure(width, height, 1, numChannels, 
                dataType, data, channelNames,
                orient, (data ? false : true),
                extraScanlines,
                extraScanlinePixels);
}
      ...
      m_staticRef(0)
{
    ...
    restructure(width, height, depth, numChannels, 
                dataType, data, channelNames,
                orient, deleteOnDestruction,
                extraScanlines,
                extraScanlinePixels);
...
FrameBuffer::HashStream&
FrameBuffer::idstream()
{
    assert(isRootPlane());
#ifdef NDEBUG
    if (!isRootPlane())
...
std::string
FrameBuffer::identifier() const
{
    if (isRootPlane()) return m_idstream.str();
...
void
FrameBuffer::setIdentifier(const std::string& s)
{
    assert(isRootPlane());
#ifdef NDEBUG
    if (!isRootPlane())
...
#endif
    m_idstream.str(s);
    if (!s.empty()) m_idstream.seekp(s.size());
}

void
FrameBuffer::restructure(int width, int height, int depth,
                         int numChannels, 
                         DataType dataType, unsigned char *data,
                         const StringVector *channelNames,
                         Orientation orient,
                         ...
                         bool clearAttributes)
{
    ...
    if (depth == 0) depth = 1;
    else if (depth>1) m_coordinateType = NormalizedCoordinates;
    ...
    if (clearAttributes) m_attributes.clear();
...
void
FrameBuffer::restructurePlanar(int w, 
                               int h,
                               ...
                               size_t nchannels)
{
    ...
    for (int q=1; q < nplanes; q++)
    {
        ...
        {
            fb = new FrameBuffer(PixelCoordinates,
                                 w/xSamplings[q], 
                                 h/ySamplings[q], 
                                 0, nchannels, 
                                 type, 0, 0, 
...
FrameBuffer::~FrameBuffer()
{
    ...
    m_previousPlane = 0;
    m_firstPlane = 0;
    if (m_nextPlane) delete m_nextPlane;
...
size_t
FrameBuffer::totalImageSize() const
{
    size_t total = 0;
...
void 
FrameBuffer::setChannelName( int channel, const std::string& name )
{
...
void 
FrameBuffer::recalcStrides()
{
    switch (m_dataType)
    {
#ifdef HAVE_HALF                
      case HALF:      m_bytesPerChannel = sizeof( half ); break;
#endif
      case UCHAR:     m_bytesPerChannel = sizeof( unsigned char ); break;
      //case USHORT_BIG:
      case USHORT:    m_bytesPerChannel = sizeof( unsigned short ); break;
      //case UINT_BIG:
      case UINT:      m_bytesPerChannel = sizeof( unsigned int ); break;
      //case FLOAT_BIG:
      case FLOAT:     m_bytesPerChannel = sizeof( float ); break;
      //case DOUBLE_BIG:
      case DOUBLE:    m_bytesPerChannel = sizeof( double ); break;
      ...
      case PACKED_Y8_Cb8_Y8_Cr8:
          ...
          break;
      default:        TWK_THROW_EXC_STREAM("Unknown data type " << m_dataType);
    }
...
void 
FrameBuffer::convertTo( DataType newDataType )
{
    abort();
      ...
      case UCHAR:
          {
              ...
              switch( m_dataType )
              {
                ...
                case HALF:
                    for( int i = 0; i < npc; ++i )
                    {
                        newData[i] = (unsigned char)clamp( pixels<half>()[i] * 255.0f,
                                                           0.0f, 255.0f );
                    }
                    break;
#endif                
                case FLOAT:
                    for( int i = 0; i < npc; ++i )
                    {
                        newData[i] = (unsigned char)clamp( pixels<float>()[i] * 255.0f,
                                                           0.0f, 255.0f );
                    }
                    break;
                case DOUBLE:
                    for( int i = 0; i < npc; ++i )
                    {
                        newData[i] = (unsigned char)clamp( pixels<double>()[i] * 255.0,
                                                           0.0, 255.0 );
                    }
                    break;
...
void 
FrameBuffer::insertChannel(const std::string& name, int channel )
{
    switch( m_dataType )
    {
      case UCHAR: insertChannelByType<unsigned char>( name, channel ); break;
      case USHORT: insertChannelByType<unsigned short>( name, channel ); break;
      case UINT: insertChannelByType<unsigned int>( name, channel ); break;
#ifdef HAVE_HALF
      case HALF: insertChannelByType<half>( name, channel ); break;
#endif
      case FLOAT: insertChannelByType<float>( name, channel ); break;
      case DOUBLE: insertChannelByType<double>( name, channel ); break;
      default: break;
    }
}
...
void 
FrameBuffer::removeChannel( int channel )
{
    switch( m_dataType )
    {
      case UCHAR: removeChannelByType<unsigned char>( channel ); break;
      //case USHORT_BIG:
      case USHORT: removeChannelByType<unsigned short>( channel ); break;
      //case UINT_BIG:
      case UINT: removeChannelByType<unsigned int>( channel ); break;
#ifdef HAVE_HALF
      //case HALF_BIG:
      case HALF: removeChannelByType<half>( channel ); break;
#endif
      //case FLOAT_BIG:
      case FLOAT: removeChannelByType<float>( channel ); break;
      //case DOUBLE_BIG:
      case DOUBLE: removeChannelByType<double>( channel ); break;
      default: break;
    }
}
...
void 
FrameBuffer::removeChannel(const string& name)
{
...
bool 
FrameBuffer::hasChannel(const std::string& name ) const
{
...
bool 
FrameBuffer::hasAttribute(const std::string& name) const
{
...
void 
FrameBuffer::setPixel3f( float r, float g, float b, int x, int y )
{
...
void 
FrameBuffer::setPixel4f( float r, float g, float b, float a, int x, int y )
{
    if (x >= m_width || y >= m_height || m_width == 0 || m_height == 0)
    {
        throw NullImageException();
    ...
    switch( m_dataType )
    {
      case FLOAT:
          pixel<float>( x, y, ir ) = r;
          if (m_numChannels > 1) pixel<float>( x, y, ig ) = g;
          if (m_numChannels > 2) pixel<float>( x, y, ib ) = b;
          if (m_numChannels > 3) pixel<float>( x, y, ia ) = a;
          break;
      case UCHAR:
          pixel<unsigned char>( x, y, ir ) = (unsigned char)clamp( r * 255.0f, 0.0f, 255.0f );;
          if (m_numChannels > 1) pixel<unsigned char>( x, y, ig ) = (unsigned char)clamp( g * 255.0f, 0.0f, 255.0f );;
          if (m_numChannels > 2) pixel<unsigned char>( x, y, ib ) = (unsigned char)clamp( b * 255.0f, 0.0f, 255.0f );;
          if (m_numChannels > 3) pixel<unsigned char>( x, y, ia ) = (unsigned char)clamp( a * 255.0f, 0.0f, 255.0f );;
          break;
      case USHORT:
          pixel<unsigned short>( x, y, ir ) = (unsigned short)clamp( r * 65535.0f, 0.0f, 65535.0f);
          if (m_numChannels > 1) pixel<unsigned short>( x, y, ig ) = (unsigned short)clamp( g * 65535.0f, 0.0f, 65535.0f);
          if (m_numChannels > 2) pixel<unsigned short>( x, y, ib ) = (unsigned short)clamp( b * 65535.0f, 0.0f, 65535.0f);
          if (m_numChannels > 3) pixel<unsigned short>( x, y, ia ) = (unsigned short)clamp( a * 65535.0f, 0.0f, 65535.0f);
          break;
      case UINT:
          pixel<unsigned int>( x, y, ir ) = (unsigned int) r;
          if (m_numChannels > 1) pixel<unsigned int>( x, y, ig ) = (unsigned int) g;
          if (m_numChannels > 2) pixel<unsigned int>( x, y, ib ) = (unsigned int) b;
          if (m_numChannels > 3) pixel<unsigned int>( x, y, ia ) = (unsigned int) a;
          break;
#ifdef HAVE_HALF                
      case HALF:
          pixel<half>( x, y, ir ) = (float)r;
          if (m_numChannels > 1) pixel<half>( x, y, ig ) = (float)g;
          if (m_numChannels > 2) pixel<half>( x, y, ib ) = (float)b;
          if (m_numChannels > 3) pixel<half>( x, y, ia ) = (float)a;
          break;
#endif
      case DOUBLE:
          pixel<double>( x, y, ir ) = (double)r;
          if (m_numChannels > 1) pixel<double>( x, y, ig ) = (double)g;
          if (m_numChannels > 2) pixel<double>( x, y, ib ) = (double)b;
          if (m_numChannels > 3) pixel<double>( x, y, ia ) = (double)a;
          break;
...
void 
FrameBuffer::getPixel4f( int x, int y, float *p ) const
{
    float &r = p[0];
    float &g = p[1];
    float &b = p[2];
    ...
    switch( m_dataType )
    {
      case FLOAT:
          ...
          if( m_numChannels > 1 ) g = pixel<float>( x, y, ig );
          if( m_numChannels > 2 ) b = pixel<float>( x, y, ib );
          if( m_numChannels > 3 ) a = pixel<float>( x, y, ia );
          break;
      case UCHAR:
          r = pixel<unsigned char>( x, y, ir ) / 255.0f;
          if( m_numChannels > 1 ) g = pixel<unsigned char>( x, y, ig ) / 255.0f;
          if( m_numChannels > 2 ) b = pixel<unsigned char>( x, y, ib ) / 255.0f;
          if( m_numChannels > 3 ) a = pixel<unsigned char>( x, y, ia ) / 255.0f;
          break;
      case USHORT:
          r = pixel<unsigned short>( x, y, ir ) / 65535.0f;
          if( m_numChannels > 1 ) g = pixel<unsigned short>( x, y, ig ) / 65535.0f;
          if( m_numChannels > 2 ) b = pixel<unsigned short>( x, y, ib ) / 65535.0f;
          if( m_numChannels > 3 ) a = pixel<unsigned short>( x, y, ia ) / 65535.0f;
          break;
      case UINT:
          r = pixel<unsigned int>( x, y, ir );
          if( m_numChannels > 1 ) g = pixel<unsigned int>( x, y, ig );
          if( m_numChannels > 2 ) b = pixel<unsigned int>( x, y, ib );
          if( m_numChannels > 3 ) a = pixel<unsigned int>( x, y, ia );
          break;
#ifdef HAVE_HALF                
      case HALF:
          r = pixel<half>( x, y, ir );
          if( m_numChannels > 1 ) g = pixel<half>( x, y, ig );
          if( m_numChannels > 2 ) b = pixel<half>( x, y, ib );
          if( m_numChannels > 3 ) a = pixel<half>( x, y, ia );
          break;
#endif
      case DOUBLE:
          r = pixel<double>( x, y, ir );
          if( m_numChannels > 1 ) g = pixel<double>( x, y, ig );
          if( m_numChannels > 2 ) b = pixel<double>( x, y, ib );
          if( m_numChannels > 3 ) a = pixel<double>( x, y, ia );
          break;
...
void 
FrameBuffer::getPixelBilinear4f( float x, float y, float *p ) const
{
    float &r = p[0];
    float &g = p[1];
    float &b = p[2];
    ...
    if (x > m_width-1  && x < m_width ) x = m_width -1;
    if (y > m_height-1 && y < m_height) y = m_height-1;
      ...
      case PACKED_R10_G10_B10_X2:
          {
              ...
              r00 = p00.red; r01 = p01.red; r10 = p10.red; r11 = p11.red;
              g00 = p00.green; g01 = p01.green; g10 = p10.green; g11 = p11.green;
              b00 = p00.blue; b01 = p01.blue; b10 = p10.blue; b11 = p11.blue;
...
void 
FrameBuffer::getPixelBilinearRGB4f( float x, float y, float *p ) const
{
...
FrameBuffer::copyPlane() const
{
    ...
    FrameBuffer* fb = new FrameBuffer(m_coordinateType,
...
FrameBuffer::referenceCopy() const
{
    ...
    copyAttributesTo(fb);
    if (isRootPlane()) fb->setIdentifier(identifier());
    fb->setUncrop(m_uncropWidth, m_uncropHeight, m_uncropX, m_uncropY);
    fb->setUncropActive(m_uncrop);
...
void
FrameBuffer::copyFrom(const FrameBuffer* fb)
{
    if (fb == this) return;
...
void
FrameBuffer::referenceCopyFrom(const FrameBuffer* fb)
{
    m_coordinateType = fb->coordinateType();

    restructure(fb->width(), fb->height(), fb->depth(),
                fb->numChannels(),
                fb->dataType(), const_cast<FrameBuffer*>(fb)->pixels<unsigned char>(), 
                &fb->channelNames(), 
                fb->orientation(),
                false,
                fb->extraScanlines(),
                fb->scanlinePixelPadding());
    
    fb->copyAttributesTo(this);
    setPixelAspectRatio(fb->pixelAspectRatio());
    if (isRootPlane()) setIdentifier(fb->identifier());
...
void
FrameBuffer::outputAttrs(ostream& o) const
{
...
void
FrameBuffer::outputInfo(ostream& o) const
{
    const char* type = 0;
    const char* otype = 0;
    ...
    switch (dataType())
    {
      case BIT: type = "1 bit"; break;
      case UCHAR: type = "8 bit int"; break;
      //case USHORT_BIG:
      case USHORT: type = "16 bit int"; break;
      //case UINT_BIG:
      case UINT: type = "32 bit int"; break;
      //case HALF_BIG:
      case HALF: type = "16 bit float"; break;
      //case FLOAT_BIG:
      case FLOAT: type = "32 bit float"; break;
      //case DOUBLE_BIG:
      case DOUBLE: type = "64 bit float"; break;
      case PACKED_Cb8_Y8_Cr8_Y8: type = "32 bit packed VYUY"; break;
      case PACKED_Y8_Cb8_Y8_Cr8: type = "32 bit packed YUYV"; break;
      case PACKED_R10_G10_B10_X2: type = "32 bit packed 10 bit RGB"; break;
      case PACKED_X2_B10_G10_R10: type = "32 bit packed 10 bit BGR"; break;
      default:
          type = "bad bit depth"; break;
    }

    switch (orientation())
    {
      case NATURAL: otype = "bottom left"; break;
      case TOPLEFT: otype = "top left"; break;
      case TOPRIGHT: otype = "top right"; break;
      case BOTTOMRIGHT: otype = "bottom right"; break;
      default: break;
    }
...
void 
FrameBuffer::appendAttributesAndPrefixTo(FrameBuffer* fb, const std::string& prefix) const
{
    if (fb == this) return;
...
void
FrameBuffer::copyAttributesTo(FrameBuffer* fb) const
{
    if (fb == this) return;
...
const FBAttribute*
FrameBuffer::findAttribute(const std::string& name) const
{
    for (int i = 0; i < m_attributes.size(); i++)
    {
        if (m_attributes[i]->name() == name)
...
FBAttribute*
FrameBuffer::findAttribute(const std::string& name)
{
    for (int i = 0; i < m_attributes.size(); i++)
    {
        if (m_attributes[i]->name() == name)
...
void
FrameBuffer::deleteAttribute(const FBAttribute* a)
{
    AttributeVector::iterator i = find(m_attributes.begin(),
                                       m_attributes.end(),
                                       a);
...
void 
FrameBuffer::setAdoptedNeutral(float x, float y)
{
    attribute<Vec2f>(ColorSpace::AdoptedNeutral())  = Vec2f(x, y);
}

void 
FrameBuffer::setPrimaries(float xWhite, float yWhite,
...
void 
FrameBuffer::setPrimaries(const Chromaticities& c)
{
...
FrameBuffer::Chromaticities 
FrameBuffer::chromaticities() const
{
    Chromaticities chr = Chromaticities::Rec709();

    if (hasAttribute(ColorSpace::BluePrimary()))  chr.blue  = attribute<Vec2f>(ColorSpace::BluePrimary());
    if (hasAttribute(ColorSpace::GreenPrimary())) chr.green = attribute<Vec2f>(ColorSpace::GreenPrimary());
    if (hasAttribute(ColorSpace::RedPrimary()))   chr.red   = attribute<Vec2f>(ColorSpace::RedPrimary());
    if (hasAttribute(ColorSpace::WhitePrimary())) chr.white = attribute<Vec2f>(ColorSpace::WhitePrimary());
...
void 
FrameBuffer::setMatrixAttribute(std::string name, 
                                float r0c0, float r0c1, float r0c2, float r0c3,
                                float r1c0, float r1c1, float r1c2, float r1c3,
                                float r2c0, float r2c1, float r2c2, float r2c3,
                                float r3c0, float r3c1, float r3c2, float r3c3)
{
    attribute<Mat44f>(name) = Mat44f(r0c0, r0c1, r0c2, r0c3,
                                     r1c0, r1c1, r1c2, r1c3,
                                     r2c0, r2c1, r2c2, r2c3,
                                     r3c0, r3c1, r3c2, r3c3);
}
...
void 
FrameBuffer::setRGBToXYZMatrix(float r0c0, float r0c1, float r0c2, float r0c3,
                               float r1c0, float r1c1, float r1c2, float r1c3,
                               float r2c0, float r2c1, float r2c2, float r2c3,
                               float r3c0, float r3c1, float r3c2, float r3c3)
{
    setMatrixAttribute(ColorSpace::RGBtoXYZMatrix(),
                       r0c0, r0c1, r0c2, r0c3,
                       r1c0, r1c1, r1c2, r1c3,
                       r2c0, r2c1, r2c2, r2c3,
                       r3c0, r3c1, r3c2, r3c3);
...
const string& 
FrameBuffer::primaryColorSpace() const
{
...
const string& 
FrameBuffer::transferFunction() const
{
...
void
FrameBuffer::setICCprofile(const void *prof, size_t len)
{
...
FrameBuffer::ownData()
{
    ...
    // If this FrameBuffer is a proxy buffer, ie.: if its data is a reference
    // to another FrameBuffer's data, it can't be made owner of the data.
    if ( findAttribute("ProxyBufferOwnerPtr") )
    {
        return;
    }

    // If this FrameBuffer has already been made owner of its data, do nothing.
    if (m_deleteDataOnDestruction) return;
    unsigned char* data = (unsigned char*)allocateLargeBlock(m_allocSize);

    //cout << "FB: allocated (2) " << (void*)data << endl;
    if (!data) TWK_THROW_STREAM (IOException, "Out of memory");
    memcpy(data, m_data, m_allocSize);
    m_data = data;
    m_deleteDataOnDestruction = true;

    if (nextPlane()) nextPlane()->ownData();
}

void
FrameBuffer::relinquishDataAndReset()
{
    assert(m_data != (unsigned char*)0xdeadc0de);
...
void
FrameBuffer::appendPlane(FrameBuffer* fb)
{
    if (!fb) return;
    ...
    FrameBuffer* f = this;
    while (f->m_nextPlane) f = f->m_nextPlane;
    f->m_nextPlane = fb;
...
void
FrameBuffer::removePlane(FrameBuffer* fb)
{
    ...
    while (f->m_nextPlane != fb) f = f->m_nextPlane;
...
size_t
FrameBuffer::numPlanes() const
{
    if (!firstPlane()) return 1;
    size_t count = 0;
    for (const FrameBuffer* f = firstPlane(); f; f = f->nextPlane()) count++;
    return count;
}

void
FrameBuffer::deleteAllPlanes()
{
    if (isPlanar())
    {
        assert(isRootPlane());
...
void
FrameBuffer::shallowCopy(FrameBuffer* fb)
{
    if (fb == this) return;

    restructure(fb->width(), fb->height(), fb->depth(),
                fb->numChannels(), fb->dataType(),
                fb->pixels<unsigned char>(), &fb->channelNames(),
                fb->orientation(), false);
...
bool
FrameBuffer::isYRYBYPlanar() const
{
    if (numChannels() == 1)
    {
        if (channelName(0) == "Y")
...
bool
FrameBuffer::isYUVBiPlanar() const
{
    if (hasChannel("Y"))
    {
...
bool
FrameBuffer::isYA2C2Planar() const
{
    if (numChannels() == 2)
    {
...
void
FrameBuffer::setUncropActive(bool b)
{
    m_uncrop = b;
    if (nextPlane()) nextPlane()->setUncropActive(b);
}

void
FrameBuffer::setUncrop(int w, int h, int x, int y)
{
    m_uncropWidth  = w;
    m_uncropHeight = h;
    m_uncropX      = x;
...
void
FrameBuffer::setUncrop(const FrameBuffer* fb)
{
    if (fb == this) return;
    setUncrop(fb->uncropWidth(), fb->uncropHeight(), fb->uncropX(), fb->uncropY());
    setUncropActive(fb->uncrop());
...
float
FrameBuffer::uncroppedAspect() const
{
    return float(m_uncropWidth) / float(m_uncropHeight);
}

void* 
FrameBuffer::allocateLargeBlock(size_t s)
{
    return TwkUtil::MemPool::alloc(s);
}

void 
FrameBuffer::deallocateLargeBlock(void* p)
{
    return TwkUtil::MemPool::dealloc(p);
}

size_t 
FrameBuffer::sizeOfDataType(DataType t)
{
    switch (t)
    {
      default:
      case UCHAR: return sizeof(unsigned char);
      case HALF: return sizeof(half);
      //case USHORT_BIG:

...
bool
IOtarga::sanityCheck(const TGAFileHeader& header) const
{
    U8 type = header.imageType;
    const ColorMapSpec& cs = header.colorMapSpec;

    if ((type != NoImageDataType &&
         type != RawColorMapType &&
         type != RawTrueColorType &&
         type != RawBlackAndWhiteType &&
         type != RLEColorMapType &&
         type != RLETrueColorType &&
...
const char*
IOtarga::typeToString(ImageType t) const
{
    switch (t)
    {
      case NoImageDataType: return "No Image Data";
      case RawColorMapType: return "Uncompressed Color-map";
      case RawTrueColorType: return "Uncompressed True-color";
      case RawBlackAndWhiteType: return "Uncompressed Black-and-White";
      case RLEColorMapType: return "RLE Color-map";
      case RLETrueColorType: return "RLE True-color";
      case RLEBlackAndWhiteType: return "RLE Black-and-White";
    }
...
void
IOtarga::readAttributes(FrameBuffer& fb, const TGAFileHeader& header) const
{
...
void
IOtarga::readExtensionArea(FrameBuffer& fb, const unsigned char* indata) const
{
    const char* data = (const char*)indata;
    U16 extSize = *(U16*)data; data += 2;
    if (extSize != 495) return;

    fb.attribute<string>("TARGA/AuthorName") = data; data += 41;
    fb.attribute<string>("TARGA/AuthorComments") = data; data += 324;

    U16 month = *(U16*)data; data += 2;
    U16 day = *(U16*)data; data += 2;
    U16 year = *(U16*)data; data += 2;
    U16 hour = *(U16*)data; data += 2;
    U16 minute = *(U16*)data; data += 2;
    U16 sec = *(U16*)data; data += 2;
    ...
    fb.attribute<string>("TARGA/JobID") = data; data += 41;

    hour = *(U16*)data; data += 2;
    minute = *(U16*)data; data += 2;
    sec = *(U16*)data; data += 2;
    ...
    fb.attribute<string>("TARGA/SoftwareID") = data; data += 41;

    U16 ver = *(U16*)data; data += 2;
    ASCII verletter = *data; data++;
    ...
    U16 gammaNum = *(U16*)data; data += 2;
    U16 gammaDen = *(U16*)data; data += 2;
    ...
    U32 ccOffset = *(U32*)data; data += 4;
    U32 psOffset = *(U32*)data; data += 4;
    U32 slOffset = *(U32*)data; data += 4;
    U8 attrType = *data; data++;

    {
        ...
        switch (attrType)
        {
          case 0: str << "No Alpha"; break;
          case 1: str << "Undefined Ignored"; break;
          case 2: str << "Undefinied Not Ignored"; break;
          case 3: str << "Unpremulted"; break;
          case 4: str << "Premulted"; break;
...
void
IOtarga::readHeader(const unsigned char* data, TGAFileHeader& header) const
{
    header.IDLength = *data; data++;
    header.colorMapType = *data; data++;
    header.imageType = *data; data++;
    header.colorMapSpec.startIndex = *(U16*)data; data += 2;
    header.colorMapSpec.length = *(U16*)data; data += 2;
    header.colorMapSpec.entrySize = *data; data++;
    memcpy(&header.imageSpec, data, sizeof(ImageSpec)); data += sizeof(ImageSpec);
}

void
IOtarga::readFooter(const unsigned char* data, TGAFileFooter& footer) const
{
    footer.extOffset = *(U32*)data; data += 4;
    footer.devOffset = *(U32*)data; data += 4;
    memcpy(footer.signature, data, 18);
}

void
IOtarga::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
...
void
IOtarga::readImage(FrameBuffer& fb,
    ...
    switch (header.imageSpec.pixelDepth)
    {
      ...
      case 16:
          ...
          else if (header.imageType == RLETrueColorType)
          {
              ...
              {
                  const size_t repCount  = *data; data++;
...
void
IOtarga::writeImage(const FrameBuffer& img,
                    const std::string& filename,
                    const WriteRequest& request) const
{
    ...
    {
      case FrameBuffer::BOTTOMRIGHT: imgd &= 1 << 4; break;
      case FrameBuffer::TOPRIGHT: imgd &= 3 << 4; break;
      case FrameBuffer::TOPLEFT: imgd &= 2 << 4; break;
      default:
          break;
    }
...
void
IOtarga::writeRLE(ofstream* outfile, unsigned char* pixel, int numChannels,
    int count) const
{
...
bool
IOtarga::pixelsMatch(const unsigned char* p1, const unsigned char* p2, bool alpha)
    const
{

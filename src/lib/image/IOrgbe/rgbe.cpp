...
int 
IOrgbe::rgbe_error(int rgbe_error_code, char *msg, bool silent) const
{
    if (!silent)
    {
...
void 
IOrgbe::rgbe2float(float *red, float *green, float *blue, unsigned char rgbe[4]) const
{
  float f;
...
int 
IOrgbe::RGBE_WriteHeader(FILE *fp, int width, int height, rgbe_header_info *info) const
{
...
int
IOrgbe::RGBE_ReadHeader(FILE* fp, int *width, int* height, rgbe_header_info* info, bool silent) const
{
    if (info) 
    {
        info->valid          = 0;
...
int 
IOrgbe::RGBE_ReadHeader_OLD(FILE *fp, int *width, int *height, rgbe_header_info *info, bool silent) const
{
  char buf[128];
  int found_format;
  float tempf;
...
int 
IOrgbe::RGBE_WritePixels(FILE *fp, float *data, int numpixels) const
{
...
int 
IOrgbe::RGBE_ReadPixels(FILE *fp, float *data, int numpixels) const
{
  unsigned char rgbe[4];
...
int 
IOrgbe::RGBE_WritePixels_RLE(FILE *fp, float *data, int scanline_width, int num_scanlines) const
{
  unsigned char rgbe[4];
...
int 
IOrgbe::RGBE_ReadPixels_RLE(FILE *fp, float *data, int scanline_width, int num_scanlines) const
{

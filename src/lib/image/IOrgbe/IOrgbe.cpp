...
IOrgbe::about() const
{ 
    return "RGBE (Radiance)"; 
}


void
IOrgbe::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
...
void
IOrgbe::readImage(FrameBuffer& fb,
                 ...
                 const ReadRequest& request) const
{
    ...
    if (RGBE_ReadHeader(infile, &w, &h, 0) == RGBE_RETURN_SUCCESS)
    {
            ...
            != RGBE_RETURN_SUCCESS)
        {
            fclose(infile);
            TWK_THROW_STREAM(IOException, 
                             "RGBE I/O: can't get pixel data for " << filename);
        }
...
void
IOrgbe::writeImage(const FrameBuffer& img, 
                  const std::string& filename,

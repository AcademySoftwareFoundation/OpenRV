...
IOgto::IOgto(IOType type,
             size_t chunkSize,
             int maxAsync) 
    : StreamingFrameBufferIO("IOgto",
                             "m1",
                             type,
                             chunkSize,
                             maxAsync)
...
void
IOgto::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
...
void
IOgto::readImages(FrameBufferVector& fbs,
...
void
IOgto::readImageStream(FrameBufferVector& fbs,
                       istream& instream,
                       const string& filename,
                       const ReadRequest& request) const
{
...
void
IOgto::readImageInMemory(FrameBufferVector& fbs,
                         void* data,
                         size_t size,
                         const string& filename,
                         const ReadRequest& request) const
...
void
IOgto::writeImages(const ConstFrameBufferVector& imgs,
...
void
IOgto::writeImageStream(const ConstFrameBufferVector& fbs,
                        ostream& outstream,
                        const WriteRequest& request) const
{
    ...
    for (size_t i = 0; i < fbs.size(); i++)
    {
        ...
        {
            ostringstream str;
            for (size_t q = 0; q < f->numChannels(); q++) str << f->channelName(q);
...
void
IOgto::declareOneImageStream(Gto::Writer& writer,
                             WriteState& state,
                             const WriteRequest& request) const
{
    //
    ...
    writer.property("size", Gto::Int, 2);    // only 2D images for the time being
    ...
    for (size_t i = 0; i < attrs.size(); i++)
    {
        ...
        if (atype != Gto::ErrorType)
        {
            ...
            if (p == string::npos) aname = a->name();
            else aname = a->name().substr(p + 3, string::npos);
    ...
    for (size_t i = 0; i < state.planeNames.size(); i++)
    {
        ...
        switch (plane->dataType())
        {
          ...
          case FrameBuffer::UCHAR:  ptype = Gto::Byte;  break;
          case FrameBuffer::USHORT: ptype = Gto::Short; break;
          case FrameBuffer::FLOAT:  ptype = Gto::Float; break;
          case FrameBuffer::HALF:   ptype = Gto::Half;  break;
          case FrameBuffer::DOUBLE: ptype = Gto::Double; break;
...
void
IOgto::writeOneImageStream(Gto::Writer& writer,
                           WriteState& state,
                           ostream& outstream,
                           const WriteRequest& request) const
{

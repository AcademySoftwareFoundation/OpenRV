...
//
//  int[1][]      size                          dimension and sizes of image window
//  int[1][]      dataWindowOrigin              data window 
//  int[1][]      dataWindowSize                data window
//  float[1][]    PixelAspectRatio              pixel aspect
//  string[1][]   planes                        plane names (e.g. plane0, plane1, ...)
//
//  int[1][]      image.plane0.size            must match dim of window
//  string[1]     image.plane0.encoding        "none" could be "packed10", etc
//  string[1]     image.plane0.codec           "raw", "jpeg", whatever we support
//  string[1][]   image.plane0.channels        channel names
...
//  byte[N][]     image.plane0.pixels          could also be float, half, short
//
//
...
class ImageReader : public Gto::Reader
{
  public:
    ...
    struct Plane
    {
        Plane() : fb(0), orientation(0), bitdepth(0), 
                  scanlinePadded(0), extraScanlines(0), 
                  dataType(0), pixelAspect(0.0f) {}
    ...
    ImageReader(FrameBufferVector& fbs,

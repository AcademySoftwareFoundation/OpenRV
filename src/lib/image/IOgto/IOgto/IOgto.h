...
class IOgto : public StreamingFrameBufferIO
{
  public:
    ...
    IOgto(IOType ioMethod = StandardIO,
    ...
    virtual void readImageStream(FrameBufferVector&,
                                 std::istream&,
                                 const std::string& filename,
                                 const ReadRequest&) const;

    virtual void readImageInMemory(FrameBufferVector&,
                                   void* data,
                                   size_t size,
                                   const std::string& filename,
                                   const ReadRequest&) const;
  ...
  private:
    void writeOneImageStream(Gto::Writer&,
                             WriteState&,
                             std::ostream&,
                             const WriteRequest&) const;

    void declareOneImageStream(Gto::Writer&,
                               WriteState&,
                               const WriteRequest&) const;
};

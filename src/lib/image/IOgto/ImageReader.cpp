...
ImageReader::ImageReader(FrameBufferVector& fbs,
                         istream& instream, 
...
ImageReader::copyAttrs()
{
    for (size_t i = 0; i < m_images.size(); i++)
    {
        ...
        for (size_t q = 0; q < image.attributes.size(); q++)
        {
            ...
            if (a->name() != "View") image.fb->addAttribute(a);
...
ImageReader::Request
ImageReader::object(const std::string &name,
                    ...
                    const ObjectInfo &info)
{
    ...
    if (protocol == "image")
    {
        ...
        while (m_fbs.size() < m_images.size()) m_fbs.push_back(new FrameBuffer());
...
ImageReader::Request
ImageReader::component(const std::string &n,
                       const std::string &i,
...
ImageReader::Request
ImageReader::property(const std::string& inname,
                     ...
                     const PropertyInfo &info)
{
    ...
    if (m_inAttributes)
    {
        ...
        if (attr) image.attributes.push_back(attr);
    }
...
bool
ImageReader::isSingleValueInt(const PropertyInfo& info)
{
    return info.size == 1 && info.dims.x == 1 && info.dims.y == 0 && info.type == Gto::Int;
}

bool
ImageReader::isSingleValueFloat(const PropertyInfo& info)
{
    return info.size == 1 && info.dims.x == 1 && info.dims.y == 0 && info.type == Gto::Float;
}

bool
ImageReader::isSingleValueString(const PropertyInfo& info)
{
    return info.size == 1 && info.dims.x == 1 && info.dims.y == 0 && info.type == Gto::String;
}

void*
ImageReader::prepIntVector(IntVector& v, const PropertyInfo& info)
{
...
void*
ImageReader::data(const PropertyInfo& info, size_t bytes)
{
    Image& image      = m_images[size_t(info.component->object->objectData)];
    string compInterp = stringFromId(info.component->interpretation);
    ...
    else if (m_inGeometry)
    {
        ...
        {
            if (m_propName == "size") return prepIntVector(image.imageSizes, info);
            else if (m_propName == "dataWindowOrigin") return prepIntVector(image.dataOrigin, info);
            else if (m_propName == "dataWindowSize") return prepIntVector(image.dataSize, info);
            return 0;
        }
    ...
    else if (m_inPlane)
    {
        ...
        if (m_propName == "pixels" && m_propInterp == "pixels")
        {
            ...
            fb->restructure(w, h, 0, 
                            plane.channels.size(),
                            (FrameBuffer::DataType)plane.dataType,
                            NULL,
                            &plane.channels,
                            (FrameBuffer::Orientation)plane.orientation,
                            true,
                            plane.extraScanlines,
        ...
        {
            if (m_propName == "orientation") return &plane.orientation;
            else if (m_propName == "bitdepth") return &plane.bitdepth;
            else if (m_propName == "scanlinePadding") return &plane.scanlinePadded;
            else if (m_propName == "extraScanlines") return &plane.extraScanlines;
            else if (m_propName == "dataType") return &plane.dataType;
        }
        else if (isSingleValueFloat(info))
        {
            if (m_propName == "pixelAspectRatio") return &plane.pixelAspect;
        }
...
void
ImageReader::dataRead(const PropertyInfo& info)
{
    Image& image      = m_images[size_t(info.component->object->objectData)];
    string compInterp = stringFromId(info.component->interpretation);
    ...
    else if (m_inGeometry)
    {
        if (info.type == Gto::Int)
        {
            ...
            if (m_propName == "dataWindowOrigin" &&
                info.type == Gto::Int &&
                info.size == 2 &&
                info.dims.x == 1 &&
                info.dims.y == 0)
            {
                fb->setUncrop(fb->uncropWidth(),
                              fb->uncropHeight(),
                              image.dataOrigin[0],
                              image.dataOrigin[1]);
            }
    ...
    else if (m_inPlane)
    {
        ...
        {
            if (m_propName == "encoding") plane.encoding = stringFromId(m_stringBuffer[0]);
            else if (m_propName == "codec") plane.codec = stringFromId(m_stringBuffer[0]);
            else if (m_propName == "role") plane.role = stringFromId(m_stringBuffer[0]);
        }
        else if (isSingleValueFloat(info))
        {
...
void
ImageReader::descriptionComplete()
{
}

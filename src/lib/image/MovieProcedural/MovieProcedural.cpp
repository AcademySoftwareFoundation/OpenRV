...
void
MovieProcedural::open(const string &filename, const MovieInfo &info,
    const Movie::ReadRequest& request)
{
    ...
    for (int i=1; i < tokens.size(); i++)
    {
        ...
        if      (name == "start")    m_info.start = atoi(val);
        else if (name == "end")      m_info.end = atoi(val);
        else if (name == "fps")      m_info.fps = atof(val);
        else if (name == "inc")      m_info.inc = atoi(val);
        else if (name == "red")      m_red = atof(val);
        else if (name == "gray")     m_red = atof(val), m_green = m_red, m_blue = m_red;
        else if (name == "grey")     m_red = atof(val), m_green = m_red, m_blue = m_red;
        else if (name == "green")    m_green = atof(val);
        else if (name == "blue")     m_blue = atof(val);
        else if (name == "alpha")    m_alpha = atof(val);
        else if (name == "width")    m_info.uncropWidth = m_info.width = atoi(val);
        else if (name == "height")   m_info.uncropHeight = m_info.height = atoi(val);
        else if (name == "hpan")     m_hpan = atoi(val);
        else if (name == "flash")    m_flash = true;
        else if (name == "interval") m_interval = atof(val);
        else if (name == "freq")     m_audioFreq = atof(val);
        else if (name == "amp")      m_audioAmp = atof(val);
        else if (name == "rate")     m_info.audioSampleRate = atof(val);
        else if (name == "audioChannels") numAudioChans = atoi(val);
        ...
        else if (regex_match(name, m, hdSpecRE))
        {
            ...
            ostringstream width; width << m_info.width;
            ostringstream height; height << m_info.height;
    ...
    {
        //
        //  Make flash white by default; This helps testing with sync meters.
        //
    ...
    if (m_flash) m_flashImg.restructure(m_info.width, m_info.height, 1, 4, dataType);
    ...
    if (m_info.audio)
    {
        if (numAudioChans < 1) numAudioChans = 1;
        if (numAudioChans > 8) numAudioChans = 8;
    ...
    if (m_flash) m_flashImg.attribute<float>("FPS") = m_info.fps;
    ...
    if (m_flash) m_flashImg.attribute<string>("Duration") = attr.str();

    m_img.copyAttributesTo(&m_info.proxy);
}

void
MovieProcedural::renderImage(const string& type, FrameBuffer& fb)
{
...
void
MovieProcedural::renderNoise(FrameBuffer& fb)
{
...
void
MovieProcedural::renderHRamp(FrameBuffer& fb,
...
void
MovieProcedural::renderSolid(FrameBuffer& fb, float r, float g, float b, float a)
{
    ...
    for (int y=1; y < fb.height(); y++)
    {
        memcpy(fb.scanline<unsigned char>(y), 
               fb.scanline<unsigned char>(0), 
               fb.scanlineSize());
    }
}

void
MovieProcedural::renderSMPTEBars(FrameBuffer& fb)
{
    static Col3f mainBars[] = 
    {
        Col3f(0.8), 
        Col3f(.8,.8,0), 
        Col3f(0,.8,.8), 
    ...
    static Col3f botbars[] =
    {
        Col3f(.031373, 0.243137, 0.349020),
        Col3f(1.0),
...
//
void
MovieProcedural::renderSRGBMacbethColorChart(FrameBuffer& fb)
{
...
//
void
MovieProcedural::renderACESMacbethColorChart(FrameBuffer& fb)
{
    // 24 color patches listed from the bottom to
    // top row of a Macbeth chart.
...
void
MovieProcedural::imagesAtFrame(const ReadRequest& request, FrameBufferVector& fbs)
{
    ...
    if (m_hpan)
    {
        ...
        for (size_t row = 0; row < height; row++)
        {
            ...
            if (negative)
            {
                if (offset0) memcpy(dst, src + offset1, offset0);
                if (offset1) memcpy(dst + offset0, src, offset1);
            }
            else
            {
                if (offset1) memcpy(dst, src + offset0, offset1);
                if (offset0) memcpy(dst + offset1, src, offset0); 
            }
        }
    }
    else
...
void
MovieProcedural::identifiersAtFrame(const ReadRequest& request,
...
void
MovieProcedural::identifier(int frame, ostream& o)
{
    if (m_imageType == "blank") frame = 0;
    o << m_filename << ":" << frame;
}

bool 
MovieProcedural::hasAudio() const
{
    return m_info.audio;
}

bool
MovieProcedural::canConvertAudioRate() const
{
    return m_info.audioSampleRate == 0;
}

size_t
MovieProcedural::audioFillBuffer(const AudioReadRequest& request, AudioBuffer& buffer)
{
    //
    ...
    if (sine || sync)
    {
        ...
        for (SampleTime i = 0; i < num; i++)
        {
            ...
            bool inSync = ((start-samplesToTime(i, rate)<1.0/rate)?false:(loc - floor(loc)) < (1.0 / double(adjustedFPS) * 0.9));
            ...
            {
                for (int ch = 0; ch < numChannels; ch++, p++) *p = 0.0f;
            }
        }
    }
...
MovieReader*
MovieProceduralIO::movieReader() const
{
    return new MovieProcedural();
}

MovieWriter*
MovieProceduralIO::movieWriter() const
{
    return 0;
}

void
MovieProceduralIO::getMovieInfo(const std::string &filename, MovieInfo& minfo) const
{

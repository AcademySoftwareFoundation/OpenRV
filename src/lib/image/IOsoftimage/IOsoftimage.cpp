...
IOsoftimage::IOsoftimage() : FrameBufferIO()
{
...
string
IOsoftimage::about() const
{ 
    return "Softimage .pic (Tweak)"; 
}

void
IOsoftimage::readAttrs(TwkFB::FrameBuffer& fb,
                       const Header& header, 
                       const ChannelPacketVector& packets) const
{
    ...
    {
      default:
      case NoField: fieldString = "No Field"; break;
      case OddField:  fieldString = "Odd"; break;
      case EvenField: fieldString = "Even"; break;
      case FullFrame: fieldString = "Full Frame"; break;
    }
    ...
    for (size_t i = 0; i < packets.size(); i++)
    {
        ...
        {
          case ColorChannels: str << "RGB:"; break;
          case AlphaChannel: str << "A:"; break;
        }
        ...
        {
          case Uncompressed:
              str << "Uncompressed"; break;
          case MixedRunLength:
              str << "RLE"; break;
          default:
              str << "Unknown(" << packet.type << ")"; break;
        }
...
void
IOsoftimage::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
    ifstream infile(UNICODE_C_STR(filename.c_str()), ios::binary | ios::in);

    if (!infile)
    {
        TWK_THROW_STREAM(IOException, "Unable to open softimage .pic file \"" 
    ...
    {
        if (header.magic == Cigam) header.swap();
        header.valid();
        ...
        if (allchannels & RedChannel) fbi.numChannels++;
        if (allchannels & GreenChannel) fbi.numChannels++;
        if (allchannels & BlueChannel) fbi.numChannels++;
        if (allchannels & AlphaChannel) fbi.numChannels++;
    ...
    {
        TWK_THROW_STREAM(IOException, "Bad magic number in softimage file \""
                         << filename << "\"");
    }
}

void
IOsoftimage::readPixels(ifstream& infile, 
                        const Header& header, 
                        FrameBuffer& fb, 
                        bool swap, 
                        const ChannelPacketVector& packets) const
{
    ...
    for (FrameBuffer* f = &fb; f; f = f->nextPlane()) fbs.push_back(f);
...
void
IOsoftimage::readImage(FrameBuffer& fb,
                       ...
                       const ReadRequest& request) const
{
    ...
    int nchannels = 0;
    if (allchannels & RedChannel) nchannels++;
    if (allchannels & GreenChannel) nchannels++;
    if (allchannels & BlueChannel) nchannels++;
    if (allchannels & AlphaChannel) nchannels++;
...
void
IOsoftimage::writeImage(const FrameBuffer& img,
                        const std::string& filename,

...
ColorManagementSystem::Transform::inputSpace() const { return RGB709; }

ColorManagementSystem::Transform::ColorSpace 
ColorManagementSystem::Transform::outputSpace() const { return RGB709; }
...
void
ColorManagementSystem::addTransform(Transform* t)
{
...
void
ColorManagementSystem::splitPath(vector<string>& tokens, const string& str)
{
    tokenize(tokens, str, ":");
}
...
void
ColorManagementSystem::addPlugin(ColorManagementSystem* cms)
{
...
void
ColorManagementSystem::loadPlugins(const string& envvar)
{
#ifndef _MSC_VER
    ...
    for (int i = 0; i < pluginFiles.size(); i++)
    {
        if (void *handle = dlopen(pluginFiles[i].c_str(), RTLD_NOW|RTLD_GLOBAL))
        {
            ...
            {
                cerr << "WARNING: exception thrown while creating CMS plugin "
                     << pluginFiles[i].c_str()
                     << ", ignoring"
                     << endl;
        ...
        {
            cerr << "ERROR: cannot open CMS plugin " 
                 << pluginFiles[i] 
                 << ": "
                 << dlerror() << endl;
        }

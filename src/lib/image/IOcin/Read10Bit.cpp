...
void
Read10Bit::planarConfig(FrameBuffer& fb, int w, int h, FrameBuffer::DataType type)
{
    //
...
void
Read10Bit::readRGBA8(const string& filename, 
                     ...
                     bool swap)
{
    fb.restructure(w, h, 0, 4, 
                   FrameBuffer::UCHAR, 
                   0, 0, FrameBuffer::TOPLEFT,
                   true);
    ...
    if (alpha)
    {
        ...
        for (int y = 0; y < h; y++)
        {
            if (maxBytes && ((char*)in - (char*)data) > maxBytes) break;
            ...
            for (U8* out = start, *end = start + ss; out < end; out++)
            {
                ...
                switch (inindex)
                {
                  case 0: *out = U8(p.red == 0x3ff   ? p.red_most   : ((p.red + 1) >> 2)); break;
                  case 1: *out = U8(p.green == 0x3ff ? p.green_most : ((p.green + 1) >> 2)); break;
                  case 2: *out = U8(p.blue == 0x3ff  ? p.blue_most  : ((p.blue + 1) >> 2)); 
                      break;
                }
    ...
    else
    {
        if (swap)
        {
            for (int y = 0; y < h; y++)
            {
                ...
                if (maxBytes && ((char*)lim - (char*)data) > maxBytes) break;
...
void
Read10Bit::readRGB8_PLANAR(const string& filename, 
                           const unsigned char* data,
                           FrameBuffer& fb,
                           int w, int h,
    ...
    if (swap)
    {
        for (int y = 0; y < h; y++)
        {
            ...
            for (U8* restrict end = r + w; r < end; r++, g++, b++, in++)
            {
                ...
                //const U8 rv = U8((float(p.red) / 1023.0f + slop) * 255.0f);
                //const U8 gv = U8((float(p.green) / 1023.0f + slop) * 255.0f);
                //const U8 bv = U8((float(p.blue) / 1023.0f + slop) * 255.0f);
...
void
Read10Bit::readRGB8(const string& filename, 
                    const unsigned char* data,
                    FrameBuffer& fb,
                    int w, int h,
                    size_t maxBytes, bool swap)
{
    fb.restructure(w, h, 0, 3, 
                   FrameBuffer::UCHAR, 
                   0, 0, FrameBuffer::TOPLEFT,
                   true);
...
void
Read10Bit::readRGB16(const string& filename, 
                     const unsigned char* data,
                     FrameBuffer& fb,
                     int w, int h,
                     size_t maxBytes, bool swap)
    ...
    for (int y = 0; y < h; y++)
    {
        ...
        if (swap)
        {
            for (U16* end = out + w * 3; out < end; in++)
            {
                ...
                //  shifting (16bit = 10bit << 6), which mapps the 10bit values
                //  onto the "lattice points" within the 16bit range separated
                //  by 2^6.  But these values are "really" supposed to
                //  represent the floating range 0.0-1.0, so only hitting these
                //  lattice points is actually wrong.  The mappping is correct
                //  for 0.0, but the error increases as we approach 1.0.  In
                //  particular:
                //
                //  "10bit 1.0" == 1111111111 is mapped to 1111111111000000 != 1111111111111111 = "16bit 1.0"
                //
                //  This would be fine if we were going to just shift back at
                //  some point, but these 16bit values really _will_ be
                //  converted to floating point during rendering, and rounding
                //  errors etc mean that even a 10bit "pass through" with 16bit
                //  textures may result in errors large enough that the output
                //  10bit value will differ from the input by 1 code value.
                //
                //  So we now convert from 10bit to 16bit by "passing through"
                //  a floating point value, to ensure that 0->0 and 1->1 (and
                //  presumably everything inbetween is optimally mapped.
                //
...
void
Read10Bit::readRGBA16(const string& filename, 
                      const unsigned char* data,
                      FrameBuffer& fb,
                      int w, int h,
                      size_t maxBytes, 
                      ...
                      bool swap) 
{
    ...
    if (alpha)
    {
        ...
        for (int y = 0; y < h; y++)
        {
            ...
            for (U16* out = start, *end = start + 4*ss; out < end; out++)
            {
                ...
                switch (inindex)
                {
                  case 0: *out = U16((double(65535l*p.red  )/1023.0l)); break;
                  case 1: *out = U16((double(65535l*p.green)/1023.0l)); break;
                  case 2: *out = U16((double(65535l*p.blue )/1023.0l)); break;
                }
	...
	for (int y = 0; y < h; y++)
	{
		...
		for (U16* end = out + w * 4; out < end; in++)
		{
		    ...
		    *out = gv; out++;
		    *out = bv; out++;
		    *out = numeric_limits<U16>::max(); out++;
		}
	    }
	    else
	    {
...
void
Read10Bit::readRGB16_PLANAR(const string& filename, 
                            const unsigned char* data,
                            FrameBuffer& fb,
                            int w, int h,
                            size_t maxBytes, bool swap)
...
void
Read10Bit::readRGB10_A2(const string& filename,
                        const unsigned char* data,
                        FrameBuffer& fb,
                        int w, int h,
                        size_t maxBytes, bool swap,
...
{
    fb.restructure(w, h, 0, 1, 
                   FrameBuffer::PACKED_R10_G10_B10_X2, 
                   useRaw ? (unsigned char*)data : 0, 0,
                   FrameBuffer::TOPLEFT,
                   true, 0, 0, useRaw ? deletePointer : 0);
    //
...
void
Read10Bit::readA2_BGR10(const string& filename,
                        const unsigned char* data,
                        FrameBuffer& fb,
                        int w, int h,
                        size_t maxBytes, bool swap)
...
void 
Read10Bit::readYCrYCb8_422_PLANAR(const std::string& filename,
                                  const unsigned char* data,
                                  ...
                                  bool swap)
{
    ...
    fb.restructurePlanar(w, h, 
                         xsamplings, ysamplings,
                         planeNames, 
                         FrameBuffer::UCHAR, 
                         FrameBuffer::TOPLEFT,
                         1);
    ...
    for (int y = 0; y < h; y++)
    {
        ...
        for (U8* end = start + ss; outY < end;)
        {
            ...
            switch (inindex)
            {
              case 0: value = U8(p.red >> 2); break;
              case 1: value = U8(p.green >> 2); break;
              case 2: value = U8(p.blue >> 2); break;
            }
            ...
            switch (outindex)
            {
              case 2: *outV = value; outV++; break;
              case 0: *outU = value; outU++; break;
              case 1:
              case 3: *outY = value; outY++; break;
            }

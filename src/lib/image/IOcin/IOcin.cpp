...
IOcin::IOcin(StorageFormat format,
...
IOcin::IOcin(const std::string& format,
             bool useChromaticies,
             IOType type,
             size_t iosize,
             int maxAsync)
...
bool 
IOcin::getBoolAttribute(const std::string& name) const
{
...
void 
IOcin::setBoolAttribute(const std::string& name, bool value)
{
...
std::string 
IOcin::getStringAttribute(const std::string& name) const
{
    if (name == "format")
    {
        switch (m_format)
...
void 
IOcin::setStringAttribute(const std::string& name, const std::string& value)
{
    if (name == "format")
    {
...
void
IOcin::readAttrs(FrameBuffer&            fb ,
                 ...
                 FilmInformation&        filmHeader) const
{
    ...
    if (red != vundef && green != vundef && blue != vundef && white != vundef)
    {
        ...
        {
            fb.setPrimaries(white.x, white.y,
                            red.x, red.y,
                            green.x, green.y,
                            blue.x, blue.y);
        }
    ...
    if (*infoHeader.label) fb.newAttribute("CIN/Label", string(infoHeader.label));

    const char* ostring = infoHeader.orientation < 8 
...
void
IOcin::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
    ...
    switch (infoHeader.orientation)
    {
      ...
      case CIN_TOP_RIGHT: orient = FrameBuffer::TOPRIGHT; break;
      case CIN_ROTATED_BOTTOM_RIGHT:
      case CIN_BOTTOM_RIGHT: orient = FrameBuffer::BOTTOMRIGHT; break;
      case CIN_ROTATED_TOP_LEFT:
      case CIN_TOP_LEFT: orient = FrameBuffer::TOPLEFT; break;
      case CIN_ROTATED_BOTTOM_LEFT:
      case CIN_BOTTOM_LEFT: orient = FrameBuffer::BOTTOMLEFT; break;
    }
    ...
    switch (m_format)
    {
      ...
      case RGB8:
      case RGB8_PLANAR: 
      case RGBA8: fbi.dataType = FrameBuffer::UCHAR;  break;
      case RGB16:
      case RGB16_PLANAR: 
      case RGBA16: fbi.dataType = FrameBuffer::USHORT;  break;
      case RGB10_A2: 
...
void
IOcin::readImage(FrameBuffer& fb,
...
void IOcin::readImage(FileStream& fmap,
                      FrameBuffer& fb,
                      const std::string& filename,
                      const ReadRequest& request) const
{
    ...
    {
      default:
      case RGB8: Read10Bit::readRGB8(filename, data, fb, w, h, maxData, swap); break;
      case RGB16: Read10Bit::readRGB16(filename, data, fb, w, h, maxData, swap); break;
      case RGBA8: Read10Bit::readRGBA8(filename, data, fb, w, h, maxData, false, swap); break;
      case RGBA16: Read10Bit::readRGBA16(filename, data, fb, w, h, maxData, false, swap); break;
      case RGB10_A2: Read10Bit::readRGB10_A2(filename, data, fb, w, h, maxData, swap); break;
      case A2_BGR10: Read10Bit::readA2_BGR10(filename, data, fb, w, h, maxData, swap); break;
      case RGB8_PLANAR: Read10Bit::readRGB8_PLANAR(filename, data, fb, w, h, maxData, swap); break;
      case RGB16_PLANAR: Read10Bit::readRGB16_PLANAR(filename, data, fb, w, h, maxData, swap); break;
    }
    ...
    switch (infoHeader.orientation)
    {
      case CIN_TOP_LEFT:
      case CIN_ROTATED_TOP_LEFT:
          break;    // this is the default for CIN, the reading code all does this itself
...
void
IOcin::writeImage(const FrameBuffer& img, 
                  const std::string& filename,
                  const WriteRequest& request) const
{
    ...
    if (request.preferCommonFormat)
    {
        ...
        if (needflop || needflip)
        {
            ...
            if (needflop) flop(outfb);
            if (needflip) flip(outfb);
        }
    }
    ...
    TwkImg::Img4f image(outfb->width(),
                        outfb->height(),
                        outfb->pixels<Col4f>());

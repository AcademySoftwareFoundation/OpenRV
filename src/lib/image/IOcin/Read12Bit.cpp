...
void
Read12Bit::planarConfig(FrameBuffer& fb, int w, int h, FrameBuffer::DataType type)
{
...
void
Read12Bit::readRGB8_PLANAR(const string& filename, 
...
void
Read12Bit::readRGB16_PLANAR(const string& filename, 
                            const unsigned char* data,
                            FrameBuffer& fb,
                            int w, int h,
                            size_t maxBytes, bool swap)
...
void
Read12Bit::readNoPaddingRGB16(const string& filename, 
                              const unsigned char* data,
                              FrameBuffer& fb,
                              int w, int h,
                              size_t maxBytes, bool swap)
{
    ...
    for (const unsigned char* p = (swap ? data2 : data), *e = p + maxBytes; p < e;)
    {
        ...
        if (valueIndex == 0) *(elements[elemIndex]) = 0;
        *(elements[elemIndex]) |= nibble << (valueIndex * 4 + 4 * (1 - nibbleIndex));
        ...
        if (nibbleIndex == 0) p++;
        if (valueIndex == 0) elements[elemIndex]++;

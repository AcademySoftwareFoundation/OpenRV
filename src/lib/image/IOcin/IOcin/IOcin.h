...
class IOcin : public StreamingFrameBufferIO 
{
  public:
    ...
    struct ImageInformation
    {
        ...
        void swap() 
        { 
            for (int i=0; i < 8; i++) image_element[i].swap();
    ...
    {
        U8    film_mfg_id; // Film manufacturer ID code (2 digits from film edge code)
    ...
    IOcin(StorageFormat format = RGB16,
          bool useChromaticies=false,
          IOType type = StandardIO,
          size_t iosize = 61440,
          int maxAsync = 16);

    IOcin(const std::string& format,
          bool useChromaticies=false,
          IOType type = StandardIO,
          size_t iosize = 61440,
          int maxAsync = 16);
    ...
    virtual void readImage(FrameBuffer&,
                           const std::string& filename,
                           const ReadRequest& request) const;

    virtual void writeImage(const FrameBuffer&, 
                            const std::string& filename,
    ...
    void readImage(TwkUtil::FileStream&,
                   FrameBuffer&,
                   const std::string& filename,
                   const ReadRequest& request) const;

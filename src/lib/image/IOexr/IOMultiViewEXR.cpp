...
void
IOexr::readMultiViewChannelList(const std::string& filename,
                                ...
                                IOexr::ReadWindow window)
{
    ...
    for (Imf::ChannelList::ConstIterator i = cl.begin();
         i != cl.end();
         ++i, totalChannels++)
    {
    ...
    if (!allChannels && layer == "" && view == "")
    {
        ...
        vector<string>::iterator i = remove_if(channelNames.begin(),
                                               channelNames.end(),
    ...
    if (useRGBAReader)
    {
        ...
        if (!stripAlpha) channelNames.push_back("A");
    }
    ...
    {
        if (redIndex(channelNames) != -1 &&
            greenIndex(channelNames) != -1 &&
            blueIndex(channelNames) != -1 &&
            !stripAlpha)
        {
            //
            ...
            if (alpha != "") channelNames.push_back(alpha);
        }
    }
    ...
    {
        numChannels = 4;
        if (stripAlpha && (channelNames[3] == "A" || channelNames[3] == "a")) numChannels = 3;
    }
    else
    {
    ...
    switch (exrPixelType)
    {
      case Imf::FLOAT: dataType = FrameBuffer::FLOAT; break;
      case Imf::HALF:  dataType = FrameBuffer::HALF; break;
      case Imf::UINT:  dataType = FrameBuffer::UINT; break;
      default:
    ...
    if (!useRGBAReader && (planarYRYBY || planar3))
    {
        ...
        Imf::ChannelList::Iterator ci3 = 
            findChannelWithBasename("A", cl);
                                   ...
                                   bool requestedViewIsDefaultView) const
{
    ...
    if (!requestedChannel.empty())
    {
        ...
        {
            requestedFullChannel =  requestedView + "."
                                    + requestedFullChannel;
        }

        if (!requestedLayer.empty())
        {
            requestedFullChannel =  requestedLayer + "."
                                    + requestedFullChannel;
        }
        ...
        //
        if (requestedChannel == "A" || requestedChannel == "a") stripAlpha = false;
    ...
    if (requestedLayer.empty())
    {
        readMultiViewChannelList(filename,
                        requestedLayer,
                        requestedView,
                        *fbs.back(), 
                        file,
    ...
    else
    {
        ...
        cl.channelsInLayer(requestedLayer,
                           cstart,
                           cend);

        Imf::ChannelList ncl;
        for (Imf::ChannelList::ConstIterator ci = cstart; 
             ci != cend; 
             ++ci)
        {
...
void
IOexr::getMultiViewImageInfo(const Imf::MultiPartInputFile& file,
                             const ViewNames &views,
                             FBInfo& fbi) const
{
    const int partNum = 0;
    ...
    if (!views.empty())
    {
        ...
        for (size_t i = 0; i < fbi.viewInfos.size(); i++)
        {
            ...
            if (!noview) 
            {
                ...
                if (vinfo->name == fbi.defaultView) defaultViewIndex = i;
            }
        ...
        //
        //  Then we added the noview channels to the defaultView, so get rid of
        //  the last view in our list, which was only to hold noview channels.
        //
        {
             ...
             ++i, count++)
        {
            ...
            setChannelInfo(baseChannelName(ci.name()),
                           ci.channel().type,
    ...
    set_difference(layers.begin(), layers.end(),
                   fbi.views.begin(), fbi.views.end(),
                   back_inserter(fbi.layers));
    ...
    switch (exrPixelType)
    {
      case Imf::FLOAT:  fbi.dataType = FrameBuffer::FLOAT; break;
      case Imf::HALF:   fbi.dataType = FrameBuffer::HALF; break;
      case Imf::UINT:   fbi.dataType = FrameBuffer::UINT; break;
      default:
          TWK_THROW_STREAM(Exception, "EXR: Unsupported data type: "
                           << exrPixelType);
    }
...
void
IOexr::writeImagesToMultiViewFile(const ConstFrameBufferVector& fbs, 
                                  ...
                                  const WriteRequest& request) const
{
    ...
    for (int i=0; i < fbs.size(); i++)
    {
          ...
          case FrameBuffer::FLOAT: 
              {
                  ...
                  if (fb != outfb && fb != fbs[i]) delete fb;
                  break;
              }
        ...
        {
            const FrameBuffer* fb = outfb;
            if (outfb == fbs[i]) outfb = outfb->copy();
    ...
    for (int i=0; i < outfbs.size(); i++)
    {
        ...
        string prefix("");
        if (fbs.size() == 2 && i == 1) prefix = "right.";

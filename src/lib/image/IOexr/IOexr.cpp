...
bool
IOexr::isAMultiPartSharedAttribute(const std::string &name)
{
...
bool
IOexr::isAces(const Imf::Chromaticities& c)
{
...
// Note: This method was taken from the internal (not exported) Imf::acesChromaticities()
const Imf::Chromaticities &
IOexr::acesChromaticities()
{
...
static bool
isXYZ(const Imf::Chromaticities& c)
{
    return c.red    == Imath::V2f(1,0) &&
           c.green  == Imath::V2f(0, 1) &&
           c.blue   == Imath::V2f(0, 0) &&
           c.white  == Imath::V2f(0.333333);
}

template <class T>
std::ostream &
output (std::ostream &s, const Imath::Matrix33<T> &m)
{
    s << m[0][0] <<
        " " << m[0][1] <<
        " " << m[0][2] << "\n" <<
...
template <class T>
std::ostream &
output (std::ostream &s, const Imath::Matrix44<T> &m)
{
    s << m[0][0] <<
        " " << m[0][1] <<
        " " << m[0][2] <<
...
int
IOexr::indexOfChannelName(const vector<string>& channelNames, 
                          const char* names[],
                          bool exact)
{
    for (size_t i = 0; i < channelNames.size(); i++)
    {
        ...
        if (!exact && index != string::npos) name = name.substr(index + 1, name.size() - index);
...
int
IOexr::indexOfChannelName(const vector<MultiPartChannel>& channelsMP, 
                          const char* names[],
                          bool exact)
{
...
int
IOexr::alphaIndex(const vector<string>& channelNames)
{
...
int
IOexr::alphaIndex(const vector<MultiPartChannel>& channelsMP)
{
    static const char* names[] = {"a", "A", "alpha", "Alpha", NULL};
...
int
IOexr::redIndex(const vector<string>& channelNames)
{
...
int
IOexr::redIndex(const vector<MultiPartChannel>& channelsMP)
{
    static const char* names[] = {"r", "R", "red", "Red", NULL};
    return indexOfChannelName(channelsMP, names);
}
...
int
IOexr::greenIndex(const vector<string>& channelNames)
{
...
int
IOexr::greenIndex(const vector<MultiPartChannel>& channelsMP)
{
    static const char* names[] = {"g", "G", "green", "Green", NULL};
    return indexOfChannelName(channelsMP, names);
}
...
int
IOexr::blueIndex(const vector<string>& channelNames)
{
...
int
IOexr::blueIndex(const vector<MultiPartChannel>& channelsMP)
{
    static const char* names[] = {"b", "B", "blue", "Blue", NULL};
    return indexOfChannelName(channelsMP, names);
}
...
bool
IOexr::channelIsRGB(const string &channelName)
{
    static const char* names[] = {"r", "R", "red", "Red",
                                  "g", "G", "green", "Green",
                                  "b", "B", "blue", "Blue", NULL};
    string name = channelName;
    string::size_type index = name.rfind(".");
    if (index != string::npos) name = name.substr(index + 1, name.size() - index);
...
string
IOexr::findAnAlpha(const Imf::MultiPartInputFile& file,
                   int partNum,
                   const string& layer,
                   const string& view)
{
    ...
    for (Imf::ChannelList::Iterator i = cl.begin();
         i != cl.end();
         ++i)
    {
        string t = i.name();
    ...
    if (viewA != "") return viewA;
    return defaultA;
}


string
IOexr::fullChannelName(const MultiPartChannel& mpChannel)
{
...
void
IOexr::findAnAlphaInView(const Imf::MultiPartInputFile& file,
                         const string& view,
                         ...
                         MultiPartChannel& alpha)
{
    ...
    for (int p=0; p < noOfParts; ++p)
    {
             ...
             (view == ""))
        {
            ...
            for (Imf::ChannelList::Iterator ci = cl.begin();
                 ci != cl.end();
                 ++ci)
            {
                string t = ci.name();
...
string
IOexr::baseChannelName(const string& name)
{
...
void
IOexr::channelSplit(const string& name, string& base, string& layerView)
{
    layerView.clear();
    base.clear();
...
void
IOexr::canonicalName(string& a)
{
    //
    //  (We need to call blue "B" for any value of BLUE -- the *third* color value).
    //  Because EXR has lexically ordered channels, you actually have to know
    //  which one is red, green, and blue. That's a big problem. I
    //  keep seeing prman output with r g b instead of R G B. 
    //  I have now seen one with "red" too.
    //
...
{
    //
    //  Check first for exact matches, otherwise we'll return "BLAH.R" for "R"
    //  if it happens to appear earlier in the channel list.
    //
...
IOexr::findChannelWithBasenameMP(const string& cname,
...
bool
IOexr::LayerOrNamedViewChannel (const string& c)
{
    return c.rfind(".") != string::npos;
}

bool
IOexr::LayerOrNamedViewChannelMP (const MultiPartChannel& mp)
{
    return LayerOrNamedViewChannel(mp.name);
}

int
IOexr::channelOrder (string &s)
{
    if (s == "R")  return 1;
    if (s == "G")  return 2;
    if (s == "B")  return 3;
...
bool 
IOexr::ChannelComp (const string& ia, const string& ib)
{
    //
    ...
    if (ap != bp)
    {
        // Because we ingest media from everyone, there are unfortunately cases where
        // Nuke appears to break the EXR specification and doesn't write a default
        // layer/view with multipart EXRs.  In these cases, rgba appears to be the most
        // fruitful default layer.  Prefer that layer over other layers.
    ...
    if (aDotCount < bDotCount) return true;
    if (aDotCount > bDotCount) return false;
...
bool 
IOexr::ChannelCompMP (const MultiPartChannel& ia, const MultiPartChannel& ib)
{
...
IOexr::IOexr(bool rgbaOnly,
             bool convertYRYBY,
             bool planar3channel,
             bool inheritChannels,
             bool noOneChannelPlanes,
             bool stripAlpha,
             bool readWindowIsDisplayWindow,
...
{
    unsigned int cap = ImageRead | ImageWrite | BruteForceIO
                    | PlanarRead | PlanarWrite
                    | Float16Capable | Float32Capable;
    ...
    codecs.push_back(StringPair("B44A", "lossy 4-by-4 pixel block compression, flat fields are comressed more"));
...
bool 
IOexr::getBoolAttribute(const std::string& name) const
{
    if (name == "convertYRYBY") return m_convertYRYBY;
    else if (name == "planar3channel") return m_planar3channel;
    else if (name == "rgbaOnly") return m_rgbaOnly;
    else if (name == "inheritChannels") return m_inheritChannels;
    else if (name == "noOneChannelPlanes") return m_noOneChannelPlanes;
    else if (name == "stripAlpha") return m_stripAlpha;
    else if (name == "readWindowIsDisplayWindow") return m_readWindowIsDisplayWindow;
    return StreamingFrameBufferIO::getBoolAttribute(name);
}

void 
IOexr::setBoolAttribute(const std::string& name, bool value)
{
    if (name == "convertYRYBY") m_convertYRYBY = value;
    else if (name == "planar3channel") m_planar3channel = value;
    else if (name == "rgbaOnly") m_rgbaOnly = value;
    else if (name == "inheritChannels") m_inheritChannels = value;
    else if (name == "noOneChannelPlanes") m_noOneChannelPlanes = value;
    else if (name == "stripAlpha") m_stripAlpha = value;
    else if (name == "readWindowIsDisplayWindow") m_readWindowIsDisplayWindow = value;
    else StreamingFrameBufferIO::setBoolAttribute(name, value);
}

int
IOexr::getIntAttribute(const std::string& name) const
{
    if (name == "readWindow") return int(m_readWindow);
    return StreamingFrameBufferIO::getIntAttribute(name);
}

void 
IOexr::setIntAttribute(const std::string& name, int value)
{
    if (name == "readWindow") m_readWindow = (ReadWindow)value;
    else StreamingFrameBufferIO::setBoolAttribute(name, value);
}
...
void
IOexr::setRGBAOnly(bool only)
{
    m_rgbaOnly = only;
}

void
IOexr::convertYRYBY(bool convert)
{
    m_convertYRYBY = convert;
}

void
IOexr::planar3Channel(bool convert)
{
    m_planar3channel = convert;
}

void
IOexr::noOneChannelPlanes(bool hack)
{
    m_noOneChannelPlanes = hack;
}

void
IOexr::stripAlpha(bool strip)
{
    m_stripAlpha = strip;
}

void
IOexr::inheritChannels(bool b)
{
    m_inheritChannels = b;
}

void 
IOexr::setReadWindow(ReadWindow window)
{
    m_readWindow = window;
}

void
IOexr::setWriteMethod(WriteMethod choice)
{
    m_writeMethod = choice;
}



void
IOexr::readAllAttributes(const Imf::MultiPartInputFile& file,
                         FrameBuffer& fb,
                         const set<int>& parts)
{
             ...
             ++i)
        {
                ...
                dynamic_cast<const Imf::StringAttribute*>(attr))
            {
                ...
                else
                {
                    fb.newAttribute(name, a->value());
                     ...
                     dynamic_cast<const Imf::StringVectorAttribute*>(attr))
            {
                fb.newAttribute(name, a->value());
            }
                     ...
                     dynamic_cast<const Imf::LineOrderAttribute*>(attr))
            {
                ...
                switch (a->value())
                {
                  ...
                  case Imf::DECREASING_Y: 
                      str << "DECREASING_Y"; 
                      break;
                  case Imf::RANDOM_Y:str << "RANDOM_Y"; 
                      break;
                  default:
                      str << "** bad value **"; break;
                }
                fb.newAttribute(name, str.str());
                     ...
                     dynamic_cast<const Imf::CompressionAttribute*>(attr))
            {
                ostringstream str;
                switch (a->value())
                {
                  case Imf::NO_COMPRESSION:     str << "NO_COMPRESSION"; break;
                  case Imf::RLE_COMPRESSION:    str << "RLE_COMPRESSION"; break;
                  case Imf::ZIPS_COMPRESSION:   str << "ZIPS_COMPRESSION"; break;
                  case Imf::ZIP_COMPRESSION:    str << "ZIP_COMPRESSION"; break;
                  case Imf::PIZ_COMPRESSION:    str << "PIZ_COMPRESSION"; break;
                  case Imf::PXR24_COMPRESSION:  str << "PXR24_COMPRESSION"; break;
                  case Imf::B44_COMPRESSION:    str << "B44_COMPRESSION"; break;
                  case Imf::B44A_COMPRESSION:   str << "B44A_COMPRESSION"; break;
                  case Imf::DWAA_COMPRESSION:   str << "DWAA_COMPRESSION"; break;
                  case Imf::DWAB_COMPRESSION:   str << "DWAB_COMPRESSION"; break;
                  default:
                      str << a->value(); break;
                }
                fb.newAttribute(name, str.str());
                     ...
                     dynamic_cast<const Imf::M33fAttribute*>(attr))
            {
                ...
                fb.newAttribute(name, str.str());
                     ...
                     dynamic_cast<const Imf::ChromaticitiesAttribute*>(attr))
            {
                Imf::Chromaticities c = a->value();

                fb.setPrimaries( c.white.x, c.white.y,
                                 c.red.x, c.red.y,
                                 c.green.x, c.green.y,
                                 c.blue.x, c.blue.y );
                ...
                else
                {
                    fb.setPrimaryColorSpace(ColorSpace::Generic());
                     ...
                     dynamic_cast<const Imf::TileDescriptionAttribute*>(attr))
            {
                ...
                switch (a->value().mode)
                {
                  case Imf::ONE_LEVEL: mode = "ONE_LEVEL"; break;
                  case Imf::MIPMAP_LEVELS: mode = "MIPMAP_LEVELS"; break;
                  case Imf::RIPMAP_LEVELS: mode = "RIPMAP_LEVELS"; break;
                  default: break;
                }
                ...
                {
                  case Imf::ROUND_DOWN: mode = "ROUND_DOWN"; break;
                  case Imf::ROUND_UP: mode = "ROUND_UP"; break;
                  default: break;
                }
...
size_t
IOexr::channelListIteratorDifference(Imf::ChannelList::ConstIterator a,
...
size_t
IOexr::channelListSize(const Imf::ChannelList& cl)
{
    return channelListIteratorDifference(cl.begin(), cl.end());
}

void
IOexr::setChannelInfo(const Imf::ChannelList::ConstIterator& i, FBInfo::ChannelInfo& info)
{
    info.name = i.name();

    switch (i.channel().type)
    {
      case Imf::FLOAT:  info.type = FrameBuffer::FLOAT; break;
      case Imf::HALF:   info.type = FrameBuffer::HALF; break;
      case Imf::UINT:   info.type = FrameBuffer::UINT; break;
      default: break;
    }
}

void
IOexr::setChannelInfo(const string &name, Imf::PixelType type, FBInfo::ChannelInfo& info)
{
    info.name = name;
...
void
IOexr::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
    //
...
void
IOexr::readImages(FrameBufferVector& fbs,
...
void
IOexr::readImagesFromFile(Imf::MultiPartInputFile& file,
                          ...
                          const ReadRequest& request) const
{
    ...
    if (file.parts() > 1)
    {
        ...
        if (request.views.empty() || request.views.front().empty())
        {
            ...
            if ( const Imf::StringAttribute* sAttr = 
                    header.findTypedAttribute<Imf::StringAttribute>("defaultView") )
            {
        ...
        // Implies read a MultiPart file
        readImagesFromMultiPartFile(file,
                                    fbs,
                                    filename,
                                    requestedView,
                                    requestedLayer,
    ...
    else
    {
        ...
        readImagesFromMultiViewFile(file,
                                    fbs,
                                    filename,
                                    requestedView,
                                    requestedLayer,
...
void
IOexr::writeImages(const ConstFrameBufferVector& fbs, 
                   const std::string& filename,
                   const WriteRequest& request) const
{
    ...
    {
        // Implies read a MultiPart file
        writeImagesToMultiPartFile(fbs,
                                   filename,
                                   request);
    }
    else
    {
        writeImagesToMultiViewFile(fbs,
                                   filename,
                                   request);
    }
}

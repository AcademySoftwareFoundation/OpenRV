...
bool 
FileStreamIStream::isMemoryMapped () const
{
    return true;
}

bool
FileStreamIStream::read (char c[/*n*/], int n)
{
    const char* p = ((char*)m_stream.data()) + m_index;
    const size_t size = m_stream.size();

    if (m_index + n >= size) n = size - m_index;

    if (n <= 0) TWK_THROW_STREAM(Exception, "Incomplete EXR file: " << size << " bytes");
...
char*
FileStreamIStream::readMemoryMapped (int n)
{
...
uint64_t
FileStreamIStream::tellg ()
{
    return uint64_t(m_index);
}

void 
FileStreamIStream::seekg (uint64_t pos)
{
    m_index = size_t(pos);
}

void 
FileStreamIStream::clear ()
{
    //m_index = 0;
}

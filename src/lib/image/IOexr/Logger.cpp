...
Logger::Logger()
{
    mLogLevel = LOG_DEBUG_VERBOSE;
}

Logger::Logger(Logger const&cpy)
{
    mLogLevel = cpy.mLogLevel;
}
Logger& Logger::operator=(Logger const &/*eq*/)
{
    return Instance();
}
...
void Logger::setLogLevel(int defaultLevel)
{
    mLogLevel = defaultLevel;
}

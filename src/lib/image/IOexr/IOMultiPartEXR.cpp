...
//
void
IOexr::getBiggerFrameBufferAndEXRPixelType (const Imf::Channel& channel, 
...
void
IOexr::readMultiPartChannelList(const std::string& filename,
                                const std::string& view,
                                FrameBuffer& fb, 
                                Imf::MultiPartInputFile& file,
                                vector<MultiPartChannel>& channelsRead,
                                bool convertYRYBY,
                                bool planar3channel,
                                bool allChannels,
                                ...
                                IOexr::ReadWindow window)
{
    ...
    switch (window)
    {
      case IOexr::DataWindow:
          ...
          if (readWindowIsDisplayWindow) dspWin = bufWin;
          break;
      default:
      ...
      case IOexr::DisplayWindow:
          ...
          if (unionWin != dspWin) outfb = new FrameBuffer();
          break;
    ...
    if (!allChannels)
    {
        ...
        // - If we are guessing the inheritance, we use the first r/g/b/a we find from any part
        ...
        //
        //  Sort the channels so that RGBA layer > RGBA channel are first in the list and
        ...
        if (r != -1 && g != -1 && b != -1){
            ...
            if(a != -1) channelsReorderedByPart.push_back(channelsReadPartZero[a]);
    ...
    {
        if (redIndex(channelsRead) != -1 &&
            greenIndex(channelsRead) != -1 &&
    ...
    if (!allChannels)
    {
        ...
        {
            // Alpha channel need to come from the same part unless we are guessing/inheriting
    ...
    if (planarYRYBY || planar3)
    {
        ...
        {
            getBiggerFrameBufferAndEXRPixelType(planeChannels[i]->channel.type,
                                                dataType,
                                                exrPixelType);
        }
...
#endif
        outfb->restructurePlanar(width, height, 
                                 xsamps, ysamps,
                                 planeNames,
                                 dataType, 
    ...
    else
    {
        ...
        {
            getBiggerFrameBufferAndEXRPixelType(channelsRead[c].channel.type,
                                                dataType,
                                                exrPixelType);
        }
    ...
    if (isMultiPart)
    {
        ...
        nameStr.clear(); nameStr.str("");
        numStr.clear(); numStr.str("");
    ...
    // Read ReadWindow Attribute
    {
        ...
        switch (window)
        {
          case IOexr::DataWindow:    rw = "Data"; break;
          case IOexr::DisplayWindow: rw = "Display"; break;
          case IOexr::DataInsideDisplayWindow: rw = "Data Cropped to Display"; break;
          default:
          case IOexr::UnionWindow:   rw = "Union"; break;
        }
...
#ifdef DEBUG_IOEXR
                LOG.log("XXX Reading part channel number: %d name: %s xs=%d ys=%d",
    ...
    if (isPartialImage)
    {
        cerr << "WARNING: EXR: incomplete image \"" 
             << filename << "\"" << endl;
...
//
bool
IOexr::doesRequestedLayerExists(bool viewHasChannelConflict,
                                ...
                                const string& channelNameInFile)
{
    ...
    if (viewHasChannelConflict)
    {
        ...
        // Lets check if part names contain no occurrences of .<view> in their names.
        // i.e. orig part name = <rv layer>
        if (layerNameInFile == requestedLayer) return true;

        if (!requestedView.empty())
        {
            // Lets check if view was stripped at the end of orig part name =  <rvlayer>.<view>
            string requestedLayerInFile = requestedLayer + "." + requestedView;
            if (layerNameInFile == requestedLayerInFile) return true;

            // Lets check if view was stripped at the beginning of orig part name =  <view>.<rvlayer>
            requestedLayerInFile = requestedView + "." + requestedLayer;
            if (layerNameInFile == requestedLayerInFile) return true;

            // Lets check if view was stripped at locations of "." of orig part name.
            size_t pos = 0; 
...
//
bool
IOexr::doesRequestedChannelExists(bool viewHasChannelConflict,
                                  const string& requestedView,
                                  const string& requestedLayer,
                                  ...
                                  const string& channelNameInFile)
{
    ...
    if (viewHasChannelConflict)
    {
        ...
        // Lets check if part names contain no occurrences of .<view> in their names.
        // i.e. orig part name = <rv layer>
        if (fullChannelNameInFile == requestedFullChannelNameInFile) return true;

         if (!requestedView.empty())
         {
            // Lets check if view was stripped at the end of orig part name =  <rvlayer>.<view>
            requestedFullChannelNameInFile = requestLayerDot
                                        + requestedView + "."
                                        + requestedChannel;
            if (fullChannelNameInFile == requestedFullChannelNameInFile) return true;

            // Lets check if view was stripped at the beginning of orig part name =  <view>.<rvlayer>
            requestedFullChannelNameInFile = requestedView + "."
                                        + requestLayerDot
                                        + requestedChannel;
            if (fullChannelNameInFile == requestedFullChannelNameInFile) return true;
    ...
    {
        if (channelNameInFile == requestedFullChannelNameInFile) return true;
    }
    return false;
}

void
IOexr::addToMultiPartChannelList (vector<MultiPartChannel>& rcl,
...
void
IOexr::readImagesFromMultiPartFile (Imf::MultiPartInputFile& file,
                                    ...
                                    const bool requestedAllChannels) const
{
    ...
    bool viewHasChannelConflict = false;
    {
        ...
        for (int p=0; p < numOfParts; ++p)
        {
            ...
            if (view != requestedView) continue;
        ...
        if (!requestedLayer.empty())
        {
            ...
            if (!requestedChannel.empty())
            {
                ...
                for (Imf::ChannelList::ConstIterator ci = cl.begin(); ci != cl.end(); ++ci)
                {
                    ...
                    if (doesRequestedChannelExists(viewHasChannelConflict,
                                                   requestedView,
                                                   requestedLayer,
                                                   requestedChannel,
                                                   partName,
                                                   ch))
                    {
                        addToMultiPartChannelList (requestedMPChannelList,
                                                   p,
                                                   partName,
                                                   ch,
                                                   ci.channel());
                        ...
                        //
                        if (ch == "A" || ch == "a") stripAlpha = false;

                        p = numOfParts; // exit the search as we hv already found the channel
                        break;
                    }
                }
            }
            else
            {
                ...
                {
                    string ch = ci.name();
                    if (doesRequestedLayerExists(viewHasChannelConflict,
                                                 requestedView,
                                                 requestedLayer,
                                                 partName,
                                                 ch))
    ...
    if (requestedMPChannelList.empty())
    {
        ...
        cerr << "ERROR: EXR: invalid " 
             ...
             << "request made on image \"" << filename << "\":"
             << endl;
        return;
    }
    ...
    readMultiPartChannelList(filename,
                            requestedView,
                            *fbs.back(), 
                            file,
                            requestedMPChannelList,
                            m_convertYRYBY,
                            m_planar3channel,
                            requestedAllChannels,
                            m_inheritChannels,
                            m_noOneChannelPlanes,
                            stripAlpha,
                            m_readWindowIsDisplayWindow,
                            m_readWindow);
...
bool
IOexr::stripViewFromName(string& name,
                         const string& view)
{
    bool didStrip = false;
...
string
IOexr::viewStrippedPartQualifiedName(bool doStrip,
...
{
    string result = 
        (file.header(partnum).hasName()?file.header(partnum).name() + "." + name:name);

    if (doStrip) stripViewFromName(result, view);

    return result;
}

void
IOexr::getMultiPartImageInfo(const Imf::MultiPartInputFile& file,
                             FBInfo& fbi) const
{
#ifdef DEBUG_IOEXR
    ...
    {
        const string& view = *v;
        if (!viewChannelConflictTable[view]) continue;
#ifdef DEBUG_IOEXR
        LOG.log("***View %s has conflicting channels", view.c_str());
#endif
        ...
        if (!view.empty())
        {
            for (int p=0; p < numOfParts; ++p)
            {
                ...
                if (partView != view) continue;

                // Determine if there is a part name conflict if we do view stripping
                // of part names.
        ...
        for (int p=0; p < numOfParts; ++p)
        {
            ...
            for (Imf::ChannelList::ConstIterator ci = cl.begin(); ci != cl.end(); ++ci)
            {
                string partNamedChannel = 
                    viewStrippedPartQualifiedName(doViewStrip, file, p, view, ci.name());
	...
	for ( set<string>::const_iterator v = views.begin(); v != views.end(); ++v, ++vindex)
    {
        ...
        const Imf::ChannelList& vcl = (viewChannelConflictTable[view]?
          ...
          it != exrPixelType.end(); ++it )
    {
        ...
        // We have the worst case buffer size so dont bother checking anymore.
...
void
IOexr::writeImagesToMultiPartFile(const ConstFrameBufferVector& fbs, 
                                  const std::string& filename,
                                  const WriteRequest& request) const
{
    ...
    for (size_t i = 0; i < request.parameters.size(); i++)
    {
        ...
        else if (name == "output/chromaticities")
        {
            ...
            while (parts.size() < 8) parts.push_back(parts.back());
        ...
        else if (name == "output/neutral")
        {
            vector<string> parts;
            stl_ext::tokenize(parts, value, ",");
            while (parts.size() < 2) parts.push_back(parts.back());
        ...
        if (m)
        {
            ...
            else if (t == "sv")
            {
                ...
                vector<string> parts;
                stl_ext::tokenize(parts, value, ",");
                for (size_t q = 0; q < parts.size(); q++) v.push_back(parts[q]);
            ...
            else if (t == "v2i")
            {
                ...
                vector<string> parts;
                stl_ext::tokenize(parts, value, ",");
                if (parts.size() == 1) parts.push_back(parts[0]);
            ...
            else if (t == "v3i")
            {
                ...
                stl_ext::tokenize(parts, value, ",");
                if (parts.size() == 1) parts.push_back(parts[0]);
                if (parts.size() == 2) parts.push_back(parts[1]);
                v[0] = atoi(parts[0].c_str());
                v[1] = atoi(parts[1].c_str());
            ...
            else if (t == "b2i")
            {
                ...
                vector<string> parts;
                stl_ext::tokenize(parts, value, ",");
                while (parts.size() < 4) parts.push_back(parts.back());
            ...
            else if (t == "m33f")
            {
                ...
                vector<string> parts;
                stl_ext::tokenize(parts, value, ",");
                while (parts.size() < 9) parts.push_back(parts.back());
            ...
            else if (t == "m44f")
            {
                ...
                vector<string> parts;
                stl_ext::tokenize(parts, value, ",");
                while (parts.size() < 16) parts.push_back(parts.back());
                mx[0][0] = atof(parts[0].c_str());
                mx[0][1] = atof(parts[1].c_str());
                mx[0][2] = atof(parts[2].c_str());
    ...
    for (int p=0; p < numOfParts; ++p)
    {
          ...
          case FrameBuffer::FLOAT: 
              {
                  ...
                  if (fb != outfb && fb != fbs[p]) delete fb;
                  break;
              }
        ...
        //
        //  Actually, we always have to tell EXR about our slices from top to bottom,
        //  so this only effects how the writing is done.  We could just as easily always
        //  do INCREASING_Y.
        //
        ...
        {
          case FrameBuffer::TOPLEFT: needsFlip = true; break;
          case FrameBuffer::TOPRIGHT: needsFlip = true; needsFlop = true; break;
          case FrameBuffer::BOTTOMRIGHT: needsFlop = true; break;
          default: break;
        }
        ...
        {
            const FrameBuffer* fb = outfb;
            if (outfb == fbs[p]) outfb = outfb->copy();
            if (needsFlip) flip(const_cast<FrameBuffer*>(outfb));
            if (needsFlop) flop(const_cast<FrameBuffer*>(outfb));
            if (fb != outfb && fb != fbs[p]) delete fb;
        }
    ...
    if      (request.compression == "ZIP") compression = Imf::ZIP_COMPRESSION;
    else if (request.compression == "ZIPS") compression = Imf::ZIPS_COMPRESSION;
    else if (request.compression == "PXR24") compression = Imf::PXR24_COMPRESSION;
    else if (request.compression == "RLE") compression = Imf::RLE_COMPRESSION;
    else if (request.compression == "B44") compression = Imf::B44_COMPRESSION;
    else if (request.compression == "B44A") compression = Imf::B44A_COMPRESSION;
    else if (request.compression == "DWAA") compression = Imf::DWAA_COMPRESSION;
    else if (request.compression == "DWAB") compression = Imf::DWAB_COMPRESSION;
    else if (request.compression == "NONE") compression = Imf::NO_COMPRESSION;
    else if (request.compression == "PIZ") compression = Imf::PIZ_COMPRESSION;
    ...
    if (acesFile)
    {
            ...
            compression != Imf::NO_COMPRESSION)
        {
            ...
            {
                cerr <<" WARNING:: IOexr: using B44A compression instead of "
                     << request.compression
                     << " for ACES output"
                     << endl;
                compression = Imf::B44A_COMPRESSION;
        ...
        for (size_t i = 0; i < userNames.size(); i++)
        {
            ...
            if (userNames[i] == "chromaticities" &&
                (a = dynamic_cast<Imf::ChromaticitiesAttribute*>(userAttrs[i])))
            {
                if (!isAces(a->value()))
                {
    ...
    for (int p=0; p < numOfParts; ++p)
    {
        ...
        for (size_t q = 0; q < attrs.size(); q++)
        {
            ...
            if (name.find("IOexr/") == 0 ||
                name.find("ColorSpace/") == 0 ||
                name == "")
            {
                continue;
            }
        ...
        else
        {
            for (int c = 0; c < outfb->numChannels(); ++c)
            {
                ...
                output_frameBuffers[p].insert(chname.c_str(),               // name
                   Imf::Slice(pixelTypes[p],                                // type
                              &outfb->pixel<char>(0, outfb->height()-1, c), // base
                              outfb->pixelSize(),                           // xStride
                              -outfb->scanlineSize() ) );                   // yStride
            }
        }
    ...
    {
        if (outfbs[i] != fbs[i]) delete outfbs[i];
    }

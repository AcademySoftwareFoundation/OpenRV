...
void
MovieStdinFB::open(const string &filename, const MovieInfo &info,
    const Movie::ReadRequest& request)
{
    ...
    {
        TWK_THROW_EXC_STREAM("StdFB: Unknown file type \"" 
                             << filename 
                             << "\"" << endl);
    }
...
void
MovieStdinFB::imagesAtFrame(const ReadRequest& request, FrameBufferVector& fbs)
{
    ...
    if (FrameBuffer* ifb = m_cache[frame - m_info.start])
    {
        if (ifb->width() != 0)
        {
            fb.restructure(ifb->width(), ifb->height(), ifb->depth(),
                           ...
                           ifb->pixels<unsigned char>(), &ifb->channelNames(),
                           ifb->orientation(), false);
...
void
MovieStdinFB::identifiersAtFrame(const ReadRequest& request, IdentifierVector& ids)
{
    int frame = request.frame;
    ids.resize(1);
    ostringstream str;
...
void
MovieStdinFB::identifier(int frame, std::ostream& o)
{
    o << m_filename << ":" << frame;
}
...
MovieStdinFBIO::MovieStdinFBIO() : MovieIO("MovieStdinFB", "z")
{
...
MovieReader*
MovieStdinFBIO::movieReader() const
{
    return new MovieStdinFB();
}

MovieWriter*
MovieStdinFBIO::movieWriter() const
{
    return 0;
}

void 
MovieStdinFBIO::getMovieInfo(const std::string& filename, MovieInfo&) const
{
    if (extension(filename) != "stdinfb")
    {

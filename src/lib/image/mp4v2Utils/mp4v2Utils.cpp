...
bool
readFile(string filename, void*& fileHandle)
{
...
bool
modifyFile(string filename, void*& fileHandle)
{
    // Check for MP4v2 Metadata
    MP4LogSetLevel(MP4_LOG_NONE);
...
bool
getFOURCC(void* fileHandle, int streamIndex, string& fourcc)
{
...
bool
assembleAtomName(void* fileHandle, int streamIndex, string atom, string& atomName)
{
    // Assemble the Atom name
    string fourcc;
    if(!getFOURCC(fileHandle, streamIndex, fourcc)) return false;
...
bool
getColrType(void* fileHandle, int streamIndex, string& colrType)
{
    string atomName;
    if(!assembleAtomName(fileHandle, streamIndex, "colr", atomName)) return false;

    // Skip tracks without COLR
    if (!MP4HaveAtom(fileHandle, atomName.c_str())) return false;
...
bool
getNCLCValues(void* fileHandle, int streamIndex, uint64_t& prim, uint64_t& xfer, uint64_t& mtrx)
{
    string atomName;
    if(!assembleAtomName(fileHandle, streamIndex, "colr", atomName)) return false;
...
bool
getPROFValues(void* fileHandle, int streamIndex, unsigned char*& profile, uint32_t& size)
{
    string atomName;
    if(!assembleAtomName(fileHandle, streamIndex, "colr", atomName)) return false;
...
bool
addColrAtom(void* fileHandle, int streamIndex, uint64_t prim, uint64_t xfer, uint64_t mtrx)
{
    // Assemble the Atom name
    MP4TrackId id = MP4FindTrackId(fileHandle, streamIndex);
...
bool
getAVdnRange(void* fileHandle, int streamIndex, uint64_t& range)
{
    // Assemble the Atom name
    string atomName;
    if(!assembleAtomName(fileHandle, streamIndex, "ACLR", atomName)) return false;
...
bool
getTmcdName(void* fileHandle, int streamIndex, string& reelName)
{
    string atomName;
    if(!assembleAtomName(fileHandle, streamIndex, "name", atomName)) return false;
...
bool
setTmcdName(void* fileHandle, int streamIndex, string reelName)
{
    string atomName;
    if(!assembleAtomName(fileHandle, streamIndex, "name", atomName)) return false;

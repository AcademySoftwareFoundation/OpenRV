...
create()
{
    ...
    if (const char* args = getenv("MOVIEMISTIKA_ARGS"))
    {
        try
        {
            ...
            desc.add_options()
                ("format", value<string>()->default_value(format), "");
    ...
    if (format == "RGB8") pixelFormat = TwkMovie::MovieMistika::RGB8;
    else if (format == "RGBA8") pixelFormat = TwkMovie::MovieMistika::RGBA8;
    else if (format == "RGB8_PLANAR") pixelFormat = TwkMovie::MovieMistika::RGB8_PLANAR;
    else if (format == "RGB10_A2") pixelFormat = TwkMovie::MovieMistika::RGB10_A2;
    else if (format == "RGB16") pixelFormat = TwkMovie::MovieMistika::RGB16;
    else if (format == "RGBA16") pixelFormat = TwkMovie::MovieMistika::RGBA16;
    else if (format == "RGB16_PLANAR") pixelFormat = TwkMovie::MovieMistika::RGB16_PLANAR;
    else pixelFormat = TwkMovie::MovieMistika::RGB8_PLANAR;
...
void 
destroy(TwkMovie::MovieMistikaIO* plug)
{
    delete plug;
}

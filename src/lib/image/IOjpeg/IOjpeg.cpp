...
IOjpeg::IOjpeg(StorageFormat format,
               IOType type,
               size_t chunkSize,
               int maxAsync) 
...
void
IOjpeg::throwError(const FileState& state) const
{
    if (m_error)
    {
        m_error = false;
        TWK_THROW_STREAM(TwkFB::IOException, 
                         "JPEG: failed to open jpeg file \"" << state.filename << "\"");
    }
}
...
void
IOjpeg::getImageInfo(const std::string& filename, FBInfo& fbi) const
{
    ...
    jpeg_save_markers(&cinfo, JPEG_COM, 0xFFFF);        // comment marker (0xffff == all)
...
void
IOjpeg::readImage(FrameBuffer& fb,
                  ...
                  const ReadRequest& request) const
{
    ...
    if (m_iotype == StandardIO)
    {
        if (!(state.file = TwkUtil::fopen(state.filename.c_str(), "rb")))
        {
            TWK_THROW_STREAM(IOException, 
                             "Cannot open " << state.filename << " for reading");
        }
    ...
    {
        state.stream = new FileStream(state.filename, 
    ...
    if (outformat == YUV) cinfo.raw_data_out = TRUE;
    ...
    {
      default:
      case RGB:  readImageRGB(fb, state, cinfo); break;
      case RGBA: readImageRGBA(fb, state, cinfo); break;
      case YUV:  readImageYUV(fb, state, cinfo); break;
    }
...
void
IOjpeg::planarConfig(FrameBuffer& fb, Decompressor& cinfo) const
{
    FrameBuffer* Y = &fb;
    FrameBuffer* U = 0;
    FrameBuffer* V = 0;
...
//
// Taken from http://docs.scribus.net/devel/scimgdataloader__jpeg_8cpp-source.html
//
...
void
IOjpeg::readAttributes(FrameBuffer& fb, struct jpeg_decompress_struct& cinfo) const
{
    //
    //  COM/EXIF/ICC data
    //
            ...
            if(exif)
            {
                ...
                for (int i=0; i < EXIF_IFD_COUNT; i++)
                {
                    ...
                    for (int j = 0; j < con->count; j++)
                    {
                        ...
                        if (e->tag == EXIF_TAG_COLOR_SPACE) // Color Space is a special case
                        {
                        ...
                        {
                            //
                            //  We'll just use libexif to make all properties into strings.
                            //  If you want a non-string, you'll have to make a special case.
                            //
    ...
    {
      case 1: den << " dpi"; break;
      case 2: den << " dots/cm"; break;
    }
...
void
IOjpeg::readImageRGB(FrameBuffer& fb, 
...
void
IOjpeg::readImageRGBA(FrameBuffer& fb, 
                      const FileState& state,
                      Decompressor& cinfo) const
{
    //
...
bool
IOjpeg::canReadAsYUV(const Decompressor& cinfo) const
{
    int w = cinfo.output_width;
...
void
IOjpeg::readImageYUV(FrameBuffer& fb,
                     const FileState& state,
                     Decompressor& cinfo) const
{
    //
    ...
    //if (yrowsize != Y->width() || urowsize != U->width() || vrowsize != V->width() ||
        //yrows != Y->height() || urows != U->height() || vrows != V->height())
    if (yrowsize != Y->width() || urowsize != U->width() || vrowsize != V->width())
    {
        //
        ...
        for (int i=0; i < yrows; i++) yscanlines[i] = &ybuffer[i * yrowsize];
        for (int i=0; i < urows; i++) uscanlines[i] = &ubuffer[i * urowsize];
        for (int i=0; i < vrows; i++) vscanlines[i] = &vbuffer[i * vrowsize];

        for (int yr=0, ur=0, vr=0; 
             yr < Y->height(); 
    ...
    else
    {
        ...
        for (int i=0; i < yrows; i++) yscanlines[i] = Y->scanline<JSAMPLE>(i);
        for (int i=0; i < urows; i++) uscanlines[i] = U->scanline<JSAMPLE>(i);
        for (int i=0; i < vrows; i++) vscanlines[i] = V->scanline<JSAMPLE>(i);
...
void
IOjpeg::writeImage(const FrameBuffer& img,
                   const std::string& filename,
                   const WriteRequest& request) const
{
    ...
    for (size_t i = 0; i < request.parameters.size(); i++)
    {
        ...
        else if (name == "output/pa/numerator") X_density = atoi(value.c_str());
        else if (name == "output/pa/denominator") Y_density = atoi(value.c_str());
        else if (name == "output/gamma") gamma = atof(value.c_str());
    }

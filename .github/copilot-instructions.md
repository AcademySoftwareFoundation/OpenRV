...
UTV is an open-source, hardware-accelerated image and video sequence viewer built for visual effects (VFX) and animation production pipelines. It is maintained under the Academy Software Foundation (ASWF). It runs on macOS, Windows, and RHEL-based Linux distributions. The build targets the VFX Reference Platform specification for dependency compatibility across studios.
